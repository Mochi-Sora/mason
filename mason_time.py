"""Timezone-aware clock for Mason.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``MASON_TIMEZONE`` env var, then ``timezone`` in ``~/.mason/config.yaml``, else server-local
...
# MASON_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("MASON_TIMEZONE", "").strip()
