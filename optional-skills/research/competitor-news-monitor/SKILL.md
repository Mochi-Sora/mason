...
version: 0.1.0
author: Ben Barclay (benbarclay), Mason Agent
license: MIT
...
Use `rss-feeds` (bundled) or `blogwatcher` (optional, stateful) for feeds, `reddit-reading` for community discussion, and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `~/.mason/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the competitor-news-monitor skill and run the tick for the watch contract at ~/.mason/competitor-watches/<watch-slug>.json.",
