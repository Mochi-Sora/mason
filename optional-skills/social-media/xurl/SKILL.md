...
version: 1.1.3
author: xdevplatform + openclaw + Mason Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Mason tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Mason Docker layout, `/opt/data` is `MASON_HOME`, but Mason tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Mason-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Mason tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Mason subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Mason adaptation: reformatted for Mason skill conventions; safety guardrails preserved verbatim.
