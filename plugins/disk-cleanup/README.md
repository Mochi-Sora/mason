...
Auto-tracks and cleans up ephemeral files created during Mason Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MASON_HOME` and `/tmp/mason-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MASON_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under MASON_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `MASON_HOME` or `/tmp/mason-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MASON_HOME/disk-cleanup/` is itself excluded
- `$MASON_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
