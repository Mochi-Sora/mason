...
after 14 days; empty dirs under MASON_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly MASON_HOME and /tmp/mason-*; never ~/.mason/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$MASON_HOME/disk-cleanup/<name>`` — deliberately outside ``$MASON_HOME/logs/``."""
    return get_mason_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under MASON_HOME or ``/tmp/mason-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_mason_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("mason-")
...
# Top-level MASON_HOME dirs whose empty subdirs are never swept (last row: user project trees).
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    "logs", "memories", "sessions", "cron", "cronjobs",
    "cache", "skills", "plugins", "disk-cleanup", "optional-skills",
    "mason-agent", "backups", "profiles", ".worktrees",
    "patches", "projects", "skins", "themes", "contributors"})

_EMPTY_DIR_SWEEP_PRUNE_DIRS = frozenset({
    ".git", "node_modules", "venv", ".venv", "site-packages", "__pycache__"})

# Top-level MASON_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
@functools.lru_cache(maxsize=1)  # built lazily so MASON_HOME resolves once
def _protected_cron_paths() -> frozenset:
    ...
    return frozenset(str(x) for parent in ("cron", "cronjobs") for base in (get_mason_home() / parent,)
                     for x in (base, base / "output", base / "jobs.json", base / ".tick.lock"))


# Paths under $MASON_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MASON_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_mason_home())
...
def _sweep_empty_dirs(mason_home: Path) -> int:
    """Remove empty dirs under MASON_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under MASON_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(mason_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under MASON_HOME (/tmp/mason-*) — name rules only
        rel = path.resolve().relative_to(get_mason_home())
