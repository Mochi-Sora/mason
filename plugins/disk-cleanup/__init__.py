"""disk-cleanup plugin — auto-cleanup of ephemeral Mason session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/mason-x/test_foo.py`
...
All operations are scoped to MASON_HOME and /tmp/mason-*.
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside MASON_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command("disk-cleanup", handler=_handle_slash,
                         description="Track and clean up ephemeral Mason session files.")
