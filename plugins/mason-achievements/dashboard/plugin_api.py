"""Mason Achievements dashboard plugin backend, mounted at /api/plugins/mason-achievements/.
...
ACHIEVEMENTS: List[Dict[str, Any]] = [
    ...
    _ach("let_him_cook", "Let Him Cook", "Let Mason run a serious autonomous tool chain in one session.", "Agent Autonomy", "flame", metric="max_tool_calls_in_session", tiers=[200, 500, 1200, 3000, 8000]),
    _ach("autonomous_avalanche", "Autonomous Avalanche", "Accumulate a lifetime avalanche of Mason tool calls across sessions.", "Agent Autonomy", "avalanche", metric="total_tool_calls", tiers=[1000, 3000, 8000, 20000, 50000]),
    _ach("toolchain_maxxer", "Toolchain Maxxer", "Use a wide spread of distinct Mason tools in one session.", "Agent Autonomy", "nodes", metric="max_distinct_tools_in_session", tiers=[18, 28, 45, 70, 100]),
    ...
    # Mason Native
    _ach("skillsmith", "Skillsmith", "Work with Mason skills enough to leave fingerprints.", "Mason Native", "hammer_scroll", metric="skill_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("skill_issue_skill_created", "Skill Issue? Skill Created.", "Create or patch durable procedures instead of repeating yourself.", "Mason Native", "anvil", metric="skill_manage_events", tiers=[25, 75, 200, 600, 1500]),
    _ach("memory_keeper", "Memory Keeper", "Persist durable knowledge with memory or Mnemosyne.", "Mason Native", "crystal", metric="memory_events", tiers=[100, 300, 1000, 3000, 8000]),
    _ach("memory_palace", "Memory Palace", "Build a serious durable-memory trail.", "Mason Native", "palace", metric="memory_write_events", tiers=[100, 300, 1000, 3000, 8000]),
    _ach("context_dragon", "Context Dragon", "Brush against compression, huge context, or token pressure repeatedly.", "Mason Native", "dragon", metric="context_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("gateway_dweller", "Gateway Dweller", "Live through gateway-connected Mason workflows.", "Mason Native", "antenna", metric="gateway_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("plugin_goblin", "Plugin Goblin", "Use or develop plugins enough that the dashboard notices.", "Mason Native", "puzzle", metric="plugin_events", tiers=[1000, 3000, 8000, 20000, 50000]),
    _ach("rollback_wizard", "Rollback Wizard", "Invoke rollback/checkpoint recovery magic.", "Mason Native", "rewind", metric="rollback_events", tiers=[500, 1500, 4000, 10000, 25000], secret=True),
    ...
    _ach("multi_model_mage", "Multi-Model Mage", "Use a real spread of distinct model names across Mason history.", "Model Lore", "prism", metric="distinct_model_count", tiers=[10, 20, 40, 80, 160]),
    ...
    _ach("provider_polyglot", "Provider Polyglot", "Use models from multiple providers across Mason history.", "Model Lore", "swap", metric="distinct_provider_count", tiers=[2, 3, 5, 8, 12]),
    ...
    _ach("open_weights_pilgrim", "Open Weights Pilgrim", "Actually chat with local/open-weight models through Mason session metadata.", "Model Lore", "terminal", metric="local_model_chat_sessions", tiers=[1, 3, 10, 30, 100]),

    # Workflow Intelligence
    _ach("toolset_cartographer", "Toolset Cartographer", "Navigate Mason toolsets deliberately instead of treating tools as a blur.", "Mason Native", "compass", metric="toolset_events", tiers=[20, 60, 200, 600, 1500]),
    _ach("config_surgeon", "Config Surgeon", "Operate on real config files, manifests, env files, and dashboard settings without flinching.", "Mason Native", "key", metric="config_events", tiers=[100, 300, 1000, 3000, 10000]),
    ...
    # Lifestyle
    _ach("marathon_operator", "Marathon Operator", "Accumulate a serious number of Mason sessions.", "Lifestyle", "marathon", metric="session_count", tiers=[75, 200, 500, 1500, 5000]),
    _ach("weekend_warrior", "Weekend Warrior", "Run Mason on weekends enough times to make it a lifestyle.", "Lifestyle", "calendar", metric="weekend_sessions", tiers=[25, 75, 200, 600, 1500]),
...
def _data_dir() -> Path:
    """Durable data root (``<mason home>/plugin-data/mason-achievements/``). State used to
    live in the install tree and died on ``mason plugins remove``/``update``; legacy files
    migrate on first read (see ``_data_file``)."""
    try:
        from plugins.plugin_storage import plugin_data_dir
        return plugin_data_dir("mason-achievements")
    except Exception:
        # Standalone dashboard import (no plugins package on sys.path): same layout, computed locally.
        root = get_mason_home() / "plugin-data" / "mason-achievements"
...
def _data_file(name: str) -> Path:
    path = _data_dir() / name
    if not path.exists():
        legacy = get_mason_home() / "plugins" / "mason-achievements" / name
        if legacy.exists():
            try:
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": hits(r"plugin|dashboard-plugins|__MASON_PLUGIN|manifest\.json"),
...
METRIC_LABELS = {
    ...
    "max_distinct_tools_in_session": "distinct Mason tools used in one session",
    ...
    "skill_events": "Mason skill mentions or tool use",
    ...
    "total_tool_calls": "lifetime Mason tool calls",
    ...
    "local_model_chat_sessions": "Mason sessions whose model metadata is local/open-weight",
...
def criteria_for(definition: Dict[str, Any]) -> str:
    if definition.get("secret") and definition.get("state") == "secret":
        return "Secret: exact requirement hidden until Mason sees the first matching signal. Keep using Mason across debugging, tools, memory, skills, plugins, and model workflows to reveal it."
    if "threshold_metric" in definition:
        tiers_list = sorted(definition.get("tiers", []), key=lambda t: t["threshold"])
        if not tiers_list:
            return "Requirement: use Mason in the matching workflow."
    ...
    return "Requirement: complete the matching Mason behavior."


def display_achievement(item: Dict[str, Any]) -> Dict[str, Any]:
    clean = dict(item)
    if clean.get("state") == "secret":
        return {**clean, "name": "???", "description": "Secret achievement: hidden until Mason detects the first relevant behavior in your session history.", "criteria": criteria_for(clean), "icon": "secret"}
...
def scan_sessions(limit: Optional[int] = None, progress_callback: Optional[Any] = None, progress_every: int = 250) -> Dict[str, Any]:
    """Scan Mason sessions and build per-session achievement stats.
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        if existing is not None and existing.is_alive():
            return
        thread = threading.Thread(target=_run_scan_and_update_cache, kwargs={"publish_partial_snapshots": True}, name="mason-achievements-scan", daemon=True)
