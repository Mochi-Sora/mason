...
_AUTH_REQUIRED_MSG = ("No xAI credentials found. Sign in via `mason auth add xai-oauth` "
...
def _xai_headers(api_key: str) -> Dict[str, str]:
    return {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json",
            "User-Agent": _xai_http("mason_xai_user_agent", "mason-agent/video_gen")}
...
def _media_ref_to_xai_url(value: str, *, kind: str, fallback_mime: str) -> str:
    ...
    through; a readable local file of the right MIME class is inlined as base64 (after Mason' read deny-list /
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Auth resolution lives in the shared ``xai_grok`` post_setup hook (mason_cli/tools_config.py): no API-key
...
async def _submit_xai_video_payload(api_key: str, base_url: str, endpoint: str, payload: Dict[str, Any], *, modality: str, operation: str,
    ...
    try:
        ...
        storage_options = build_xai_storage_options("video_gen", filename_prefix="mason-xai-video", extension="mp4")
