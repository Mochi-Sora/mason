    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(f"Nous Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}). This model may not yet be enabled "
                             ...
                             f"  • Pick a different model via `mason tools` → Video Generation.") from exc
        raise
...
_NO_BACKEND_MSG = ("No FAL backend available. Either set FAL_KEY (run `mason tools` → Video Generation → FAL to configure) "
                   "or sign in to Nous (`mason setup`) for managed gateway access.")
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `mason tools` → Video Generation.",
