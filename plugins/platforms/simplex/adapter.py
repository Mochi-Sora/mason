...
MASON_SIMPLEX_TEXT_BATCH_DELAY (quiet seconds, default 0.8, merging rapid-fire inbound text).
...
_CORR_PREFIX = "mason-"  # marks requests we sent so our own echoes can be ignored
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(os.getenv("MASON_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
async def _standalone_send(
    ...
    this process (``mason cron``). ``thread_id``/``force_document`` are signature parity only;
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``mason setup gateway`` → SimpleX; writes ``~/.mason/.env``."""
    print(
        "\nSimpleX Chat setup\n------------------\nRequirements:\n"
    ...
    try:
        ...
        print("mason_cli.config not available; set SIMPLEX_* vars manually in ~/.mason/.env")
        return
