...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        payload = {"msgtype": "markdown", "markdown": {"title": "Mason", "text": self._normalize_markdown(content[: self.MAX_MESSAGE_LENGTH])}}
        try:
    ...
    async def _create_and_stream_card(self, chat_id: str, message: Any, content: str, *, finalize: bool = True) -> Optional[SendResult]:
        ...
        try:
            ...
            if not token:
                return None
            out_track_id, models = f"mason_{uuid.uuid4().hex[:12]}", dingtalk_card_models
            is_group = str(getattr(message, "conversation_type", "1")) == "2"
...
def interactive_setup() -> None:
    ...
    from mason_cli.cli_output import prompt, prompt_yes_no, print_header, print_success, print_warning
    ...
    if prompt_choice("Choose setup method", choices, default=0) == 0:
        try:
            from mason_cli.dingtalk_auth import dingtalk_qr_auth
