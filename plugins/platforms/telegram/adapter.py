...
# the heartbeat loop to trigger its own recovery path. Refs: NousResearch/mason-agent#59614
...
# NousResearch/mason-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("MASON_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "MASON_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", 0.3, min_value=0.08, max_value=2.0)
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "MASON_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 1.0, min_value=self._text_batch_delay_seconds, max_value=4.0)
    ...
    def _dm_topic_fallback(metadata: Optional[Dict[str, Any]]) -> bool:
        """True for Mason private-chat topic lanes (``telegram_dm_topic_reply_fallback``)."""
    ...
    def _thread_kwargs_for_send(
        ...
        metadata; Mason private-chat topic lanes are marked ``telegram_dm_topic_reply_fallback``. Anchor-less synthetic sends
        prefer the Mason topic's ``message_thread_id`` (the native DM-topic id renders in a different chat lane).
        ...
        restart) prefer the Mason topic's ``message_thread_id`` so they stay in the active topic lane
        (#87051); ``direct_messages_topic_id`` is only used when no topic thread resolves, since the native
        DM-topic id does not match the Mason topic lane and can render the message in a different chat
        lane.
        """
        ...
        if fallback and reply_to_mode != "off":
            ...
            if reply_to_message_id is None:
                # Anchor-less synthetic send: prefer the Mason topic thread id (see docstring).
                # Anchor-less synthetic sends (loop wakeups, watch notifications, restart-resumed
                # follow-ups) must stay in the active topic lane: prefer the Mason topic thread id when it
    ...
    def _prune_stale_dm_topic_binding(self, chat_id: Any, thread_id: Any, *, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        Rows are namespaced by profile: the send's ``mason_profile`` wins over the adapter's stamp.
        ...
        binding, so the send's ``mason_profile`` metadata wins over the adapter's own profile stamp;
        ...
        if db is None or not hasattr(db, "delete_telegram_topic_binding"):
            return
        try:
            profile_name = (metadata or {}).get("mason_profile") or getattr(self, "_mason_profile_name", None) or "default"
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "or another process is using the same bot token. To recover: ensure no other Mason or OpenClaw instance is running "
            "with this token, then restart the gateway with 'mason gateway restart'."
    ...
    async def _register_command_menu(self) -> None:
        ...
        from mason_cli.commands_platforms import telegram_menu_commands, telegram_menu_max_commands
        if not self._bot:
            return
    ...
    async def _build_ptb_requests(self) -> tuple:
        ...
        request_kwargs = {
            "connection_pool_size": env_int("MASON_TELEGRAM_HTTP_POOL_SIZE", 512),
            "pool_timeout": env_float("MASON_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
            "connect_timeout": env_float("MASON_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
            "read_timeout": env_float("MASON_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
            "write_timeout": env_float("MASON_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
        ...
        disable_fallback = os.getenv("MASON_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"}
        fallback_ips = [] if disable_fallback else self._fallback_ips()
        if not fallback_ips and not disable_fallback:
            discovery_timeout = self._env_float_clamped("MASON_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT", 5.0, min_value=0.0)
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        _max_connect = 8
        _init_timeout = env_float("MASON_TELEGRAM_INIT_TIMEOUT", 30.0)  # per attempt
            ...
            try:
                if asyncio.get_running_loop().time() >= _total_deadline:
                    raise OSError(
                        ...
                        f"Check network connectivity to api.telegram.org or set MASON_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                        f"MASON_TELEGRAM_INIT_TIMEOUT to a lower value.")
                ...
                if _attempt >= _max_connect - 1:
                    raise OSError(
                        ...
                        f"or set MASON_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value.")
    ...
    async def _start_webhook_mode(self, webhook_url: str, *, is_reconnect: bool) -> None:
        ...
        if not webhook_secret:
            raise RuntimeError(
                ...
                "https://github.com/NousResearch/mason-agent/security/advisories/GHSA-3vpc-7q5r-276h.\n\n"
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        try:
            from mason_cli.models_catalog_static import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(self, query, data: str, chat_id: str) -> None:
        ...
        if data.startswith("mp:"):  # provider selected: show model buttons (page 0)
            ...
            group_id = data[4:]
            try:
                from mason_cli.models_catalog_static import PROVIDER_GROUPS
    ...
    # `gt:<verb>` -> (script in ~/.mason/scripts/gmail-triage/, extra-args, success-label, is_state). The callback
    ...
    async def _handle_gmail_triage_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        ...
        script_path = _Path.home() / ".mason" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # Server-side MessageEntity values are authoritative: raw substrings like "foo@mason_bot.example"
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # counted as incoming unread in the Mason inbox (#52363). Otherwise a BotFather rename leaves the
    ...
    def _reload_dm_topics_from_config(self) -> None:
        ...
        try:
            from mason_cli.config import load_config_readonly  # canonical loader: managed overlay + ${VAR}
...
# _PLATFORMS["telegram"] static dict in mason_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("MASON_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    from mason_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mason setup` to install Telegram support.", setup_fn=interactive_setup, apply_yaml_config_fn=_apply_yaml_config,
