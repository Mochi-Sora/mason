...
def _make_activity_event(*, hook_event_name: str, session_id: Any, status: str = "ok", tool_name: Any = None,
    ...
    event: Dict[str, Any] = {"schema": ACTIVITY_EVENT_SCHEMA, "eventId": f"mason-{uuid.uuid4()}",
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # No payload["schema"] gate: the bridge owns schema evolution; Mason only checks content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Mason turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """``mason gateway setup`` flow: persists ``RAFT_PROFILE`` to the Mason env file.
    CLI helpers are lazy-imported so the plugin stays importable in gateway runtime and tests."""
    from mason_cli.cli_output import print_header, print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    if existing_profile:
        ...
        if not prompt_yes_no("Reconfigure Raft?", False):
            ...
            return
    for line in ("Connect Mason to Raft as an external agent.", "Create the External Agent in Raft first, then run:",
