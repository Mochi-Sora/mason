...
legacy single-user mode). Files under ``${MASON_HOME}``: ``google_chat_user_tokens/
...
from mason_constants import display_mason_home, get_mason_home
...
# human-readable under ``ls ~/.mason/google_chat_user_tokens/``.
...
def _token_rel(email: Optional[str]) -> str:
    """MASON_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def _user_tokens_dir() -> Path:
    return get_mason_home() / "google_chat_user_tokens"


def _token_path(email: Optional[str] = None) -> Path:
    """Per-user token path for ``email``, or the legacy single-user path."""
    return get_mason_home() / _token_rel(email)


def _client_secret_path() -> Path:
    return get_mason_home() / "google_chat_user_client_secret.json"


def _pending_auth_path(email: Optional[str] = None) -> Path:
    if email:
        return get_mason_home() / "google_chat_user_oauth_pending" / f"{_sanitize_email(email)}.json"
    return get_mason_home() / "google_chat_user_oauth_pending.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `mason setup` to install Google Chat support."
        )
        return None
    try:
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `mason setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into MASON_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    print(f"Profile path: {display_mason_home()}/{_token_rel(email)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Mason (native attachment delivery)"
    )
