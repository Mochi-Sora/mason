...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from mason_constants import get_mason_home as _get_mason_home
            _mason_home = _get_mason_home()
        except (ModuleNotFoundError, ImportError):
            _mason_home = _Path.home() / ".mason"
        self._thread_count_store = _ThreadCountStore(_mason_home / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from mason_constants import get_mason_home as _get_mason_home
        return _get_mason_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a mason MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "mason_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Mason is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Mason is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. mason-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``mason setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.mason/.env``."""
    from mason_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.mason/.env")
    print_info("Restart the gateway: mason gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``mason cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mason setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Mason is thinking…' marker message "
