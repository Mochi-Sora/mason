...
from mason_constants import (find_node_executable, get_mason_dir, with_mason_node_path)
...
def _is_allowed_bridge_path(url: str) -> bool:
    """Absolute bridge path resolves (symlinks included) inside a Mason cache dir — a rogue bridge could hand back /etc/passwd."""
    try:
...
def check_whatsapp_requirements() -> bool:
    """Node.js (Mason-managed first, so a bad system Node on PATH can't break Windows) is available."""
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path = Path(extra.get("session_path", get_mason_dir("platforms/whatsapp/session", "whatsapp/session")))
    ...
    def _ensure_bridge_deps(self, bridge_dir: Path) -> bool:
        ...
        _dep_stamp = bridge_dir / "node_modules" / ".mason-pkg-hash"  # holds the package.json hash of the last install
        ...
        # Mason-managed portable Node's npm.cmd first (Windows), then PATH.
        ...
        try:  # Default 300s accommodates slow systems like an Unraid NAS.
            install_result = subprocess.run([_npm_bin, "install", "--silent"], cwd=str(bridge_dir), timeout=env_int("WHATSAPP_NPM_INSTALL_TIMEOUT", 300),
                                            env=with_mason_node_path(), **_RUN_TEXT)
        ...
        self._set_fatal_error("whatsapp_npm_install_failed", f"WhatsApp bridge npm install failed{detail}. Run `cd {bridge_dir} && {_npm_bin} install` "
                              "manually, then restart `mason gateway`.", retryable=False)
        return False
    ...
    def _bridge_env(self) -> dict:
        ...
        bridge_env = with_mason_node_path()
        ...
        for _key, _v in [("WHATSAPP_MODE", _wenv("WHATSAPP_MODE", "self-chat"))] + [(k, _wenv(k)) for k in _BRIDGE_PASSTHROUGH_ENV]:
            if _v:
                bridge_env[_key] = _v
        # Without these the bridge hardcodes ~/.mason/{image,audio,document}_cache (wrong under MASON_HOME/profiles/cache layout).
        img_dir, audio_dir, _video_dir, doc_dir = _cache_dirs()
        bridge_env.update(MASON_IMAGE_CACHE_DIR=str(img_dir), MASON_AUDIO_CACHE_DIR=str(audio_dir), MASON_DOCUMENT_CACHE_DIR=str(doc_dir))
        return bridge_env
    ...
    async def _wait_for_bridge(self) -> bool:
        ...
        if data.get("status") != "connected":
            ...
            if connected is None:
                ...
                print(f"[{self.name}]   If session expired, re-pair: mason whatsapp")
        return True
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js and re-run `mason gateway`."),
            ...
            (creds_path.exists, ("[%s] WhatsApp is enabled but not paired (no creds.json at %s). Pair from the dashboard or run "
                                 "`mason whatsapp`; remove WHATSAPP_ENABLED from your .env to disable.", self.name, creds_path),
             "whatsapp_not_paired", "WhatsApp enabled but not paired — pair from the dashboard or run `mason whatsapp`."),
        )
...
# ── Plugin glue: register(ctx) plus the hooks for gateway/run.py, gateway/config.py, mason_cli/gateway.py, send_message_tool.py.
...
def interactive_setup() -> None:
    ...
    from mason_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and (getattr(config, "extra", {}) or {}):
        return True
    # Via mason_cli.gateway.get_env_value (not os.getenv) so setup-status callers that patch it observe the same value.
