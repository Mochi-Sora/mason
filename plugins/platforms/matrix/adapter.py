...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Mason ``/command`` text."""
...
from mason_constants import get_mason_dir as _get_mason_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Mason
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        self._store_dir = _get_mason_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        return (self._store_dir or _get_mason_dir("platforms/matrix/store", "matrix/store")) / "crypto.db"
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("MASON_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("MASON_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Mason Agent",
    ...
    async def _connect_setup_e2ee(self, client: Any, api: Any, state_store: Any) -> bool:
        ...
        try:
            ...
            _acct_id = self._user_id or "mason"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Mason Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        txn_id = f"mason_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Mason delivers cron job results and notifications.",
...
def _is_connected(config) -> bool:
    """Connected = homeserver + token (or password). Reads via mason_cli.gateway.get_env_value so
