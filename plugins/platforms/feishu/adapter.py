...
# Mason monkey-patches ``websockets.connect`` on the shared module to inject ping settings.
...
# (``loop``), and Mason also monkey-patches ``websockets.connect`` on the shared ``websockets`` module to
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_mason_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(32, env_int("MASON_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE)),
            text_batch_delay_seconds=env_float("MASON_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS),
            text_batch_split_delay_seconds=env_float("MASON_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0),
            text_batch_max_messages=max(1, env_int("MASON_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES)),
            text_batch_max_chars=max(1, env_int("MASON_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS)),
            media_batch_delay_seconds=env_float("MASON_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS),
    ...
    def _get_sdk_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(max_workers=10, thread_name_prefix="mason-feishu-sdk")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Mason gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        """Approval-button card; ``mason_action`` in each button value lets the click callback
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return _card_button(label, btn_type, {"mason_action": action_name, "approval_id": approval_id})
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return _card_button(label, btn_type, {"mason_update_prompt_action": answer, "update_prompt_id": prompt_id})
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Mason does not act on."""
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        if isinstance(action_value, dict):
            if action_value.get("mason_action"):
                return self._handle_approval_card_action(event=event, action_value=action_value, loop=loop)
            if action_value.get("mason_update_prompt_action"):
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("mason_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        answer = str(action_value.get("mason_update_prompt_action", "") or "").strip().lower()
    ...
    async def _download_remote_document(self, file_url: str, *, default_ext: str, preferred_name: str) -> tuple[str, str]:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                file_url, headers={"User-Agent": "Mozilla/5.0 (compatible; MasonAgent/1.0)", "Accept": "*/*"},
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Mason webhook mode")
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    qr_url += ("&" if "?" in qr_url else "?") + "from=mason&tp=mason"
    return {
...
# static dict in mason_cli/gateway.py, and the _send_feishu dispatch in tools/send_message_tool.py).
# ──────────────────────────────────────────────────────────────────────────
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `mason setup` to install Feishu support."}
    try:
...
def interactive_setup() -> None:
    ...
    from mason_cli.setup import prompt_choice
    from mason_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success, print_warning
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mason setup` to install Feishu support.", setup_fn=interactive_setup,
