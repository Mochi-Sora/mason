...
(enabled/policy/allow_from) falling back independently. Config ~/.mason/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.mason/feishu_comment_pairing.json."""
...
# long after profile/MASON_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_mason_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_mason_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("mason_cli.env_loader", fromlist=["load_mason_dotenv"]).load_mason_dotenv()
    except Exception:
        pass
