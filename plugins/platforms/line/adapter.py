...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, MASON_HOME).
        ...
        try:
            ...
            mason_home = Path(get_mason_home()).resolve()
        except Exception:
            mason_home = Path.home().joinpath(".mason").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), mason_home)):
...
def is_connected(config) -> bool:
    """Surface in ``mason status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``mason status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``mason setup line`` (writes ``~/.mason/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from mason_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("mason_cli.config not available; set LINE_* vars manually in ~/.mason/.env")
        return
