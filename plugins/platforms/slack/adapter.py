...
# User-Agent prefix (``MasonAgent/<version>``) for platform-partner attribution of API calls.
try:
    from mason_cli import __version__ as _MASON_VERSION
except Exception:
    _MASON_VERSION = "unknown"
_MASON_SLACK_USER_AGENT_PREFIX = f"MasonAgent/{_MASON_VERSION}"
...
_MODEL_PICKER_PROVIDER_ACTION = "mason_model_provider"
_MODEL_PICKER_MODEL_ACTION = "mason_model_model"
_MODEL_PICKER_BACK_ACTION = "mason_model_back"
_MODEL_PICKER_CANCEL_ACTION = "mason_model_cancel"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    if not text.startswith("!"):
        return text
    try:
        from mason_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # URL); labels after dates (dates carry a label); bot mention after labels (``<@U…|mason>``).
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if team_key not in warned and "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "from `mason slack` produces a manifest with these already included.",
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        # Catch-all no-op ack for any other subscribed event type that Mason has no listener for (e.g.
        ...
        async def handle_unhandled_event(event, body, logger):
            logger.debug(
                ...
                "events not handled by Mason can be removed from the Slack app manifest via "
                "`mason slack manifest`)",
        ...
        # ALSO be declared in the app manifest (`mason slack manifest`): Socket Mode won't
        # deliver undeclared commands at all.
        from mason_cli.commands_platforms import slack_native_slashes
        _slash_names = [name for name, _d, _h in slack_native_slashes()]
        if _slash_names:
            _slash_pattern = re.compile(
                r"^/(?:" + "|".join(re.escape(n) for n in _slash_names) + r")$")
        else:  # pragma: no cover - registry always non-empty
            _slash_pattern = re.compile(r"^/mason$")

        @self._app.command(_slash_pattern)
        async def handle_mason_command(ack, command):
        ...
        self._app.action("mason_feedback")(self._handle_feedback_action)
        # Clarify buttons (tools/clarify_gateway.py); indexed action IDs because
        # Block Kit requires unique IDs within an actions block.
        self._app.action(re.compile(r"^mason_clarify_choice_\d+$"))(self._handle_clarify_action)
        self._app.action("mason_clarify_other")(self._handle_clarify_action)
    ...
    def _register_plugin_action_handlers(self) -> None:
        ...
        try:
            from mason_cli.plugins import get_plugin_manager
    ...
    def _new_web_client(token: str, proxy_url: Optional[str]) -> Any:
        client = AsyncWebClient(token=token, user_agent_prefix=_MASON_SLACK_USER_AGENT_PREFIX)
        _apply_slack_proxy(client, proxy_url)
        return client
    ...
    def _fatal_missing_env(self, env_name: str) -> None:
        ...
        logger.error(
            "[Slack] %s not set — this is a permanent config error; set %s via `mason "
            "gateway setup` or in the active profile's ~/.mason/.env file, then restart the "
            "gateway.", env_name, env_name)
        self._set_fatal_error(
            f"missing_{env_name.lower()}",
            f"{env_name} not configured. Use `mason gateway setup` "
            "or add it to your active profile's ~/.mason/.env file, then restart the gateway.",
            retryable=False)
    ...
    def _hint_allow_bots(self) -> None:
        ...
        if _allow_bots_cfg != "none":
            logger.info(
                ...
                "appropriate (run 'mason slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Mason handoff — *{(name or 'session').strip()[:80]}*"
    ...
    async def send_native_task_card_progress(
        self, chat_id: str, tasks: List[Dict[str, str]], *, title: str = "Mason is working",
    ...
    def _synthetic_reaction_event(
        ...
        from conflating it with the reacted-to message; ``_mason_force_process`` skips the mention
        requirement (user auth and allowed_channels still apply); ``_mason_reaction`` is
        ...
        if target_channel:
            ...
            synthetic["_mason_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_mason_no_thread_response"] = True
        return synthetic
    ...
    def _session_thread_ts(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
            return thread_ts
        if event.get("_mason_no_thread_response"):
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        force_process = bool(event.get("_mason_force_process"))
    ...
    async def send_exec_approval(
        ...
        def _build() -> Tuple[str, list]:
            ...
            actions = [
                self._button("Allow Once", "mason_approve_once", session_key, style="primary")]
            if not smart_denied and allow_session:
                actions.append(self._button("Allow Session", "mason_approve_session", session_key))
                if allow_permanent:
                    actions.append(
                        self._button("Always Allow", "mason_approve_always", session_key))
            actions.append(self._button("Deny", "mason_deny", session_key, style="danger"))
            blocks = [
                {
                    "type": "section",
        ...
        def _build() -> Tuple[str, list]:
            ...
            blocks = [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*{_title}*\n\n{body}"}},
                {
                    "type": "actions",
                    "elements": [
                        self._button("Approve Once", "mason_confirm_once", value, style="primary"),
                        self._button("Always Approve", "mason_confirm_always", value),
                        self._button("Cancel", "mason_confirm_cancel", value, style="danger")]}]
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from mason_cli.providers import get_label
    ...
    async def send_clarify(
        ...
        """Clarify prompt as Block Kit buttons: one ``mason_clarify_choice_<idx>`` per option
        (value ``clarify_id|idx``) plus "✏️ Other…" (``mason_clarify_other``), which flips the
        ...
        def _build() -> Tuple[str, list]:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append(
                    self._button(
                        label[:75], f"mason_clarify_choice_{idx}",
                        f"{clarify_id}|{idx}", emoji=True))
            elements.append(
                self._button("✏️ Other…", "mason_clarify_other", f"{clarify_id}|other", emoji=True)
            )
    ...
    _APPROVAL_CHOICES: ClassVar[Dict[str, str]] = {
        "mason_approve_once": "once", "mason_approve_session": "session",
        "mason_approve_always": "always", "mason_deny": "deny"}
    ...
    _CONFIRM_CHOICES: ClassVar[Dict[str, str]] = {
        "mason_confirm_once": "once", "mason_confirm_always": "always",
        "mason_confirm_cancel": "cancel"}
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "mason_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        ``/mason <subcommand> [args]``; other text after ``/mason`` is a regular message."""
        ...
        # events only: free-form "/mason <question>" replies must stay public.
    ...
    def _slash_command_text(command: dict) -> str:
        ...
        payload verbatim (internal/trailing spacing). ``/mason`` (or a missing ``command``) maps
...
# in ``mason_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in ``tools/send_message_tool.py``).
# ──────────────────────────────────────────────────────────────────────────
...
def _load_slack_bot_tokens(raw_token: str, *, quiet: bool) -> List[str]:
    ...
    try:
        ...
        tokens_file = get_mason_home() / "slack_tokens.json"
...
_SETUP_HOME_CHANNEL_HELP = (
    "📬 Home Channel: where Mason delivers cron job results,",
...
def _write_slack_manifest_and_instruct() -> None:
    """Write the manifest under MASON_HOME and print paste instructions; non-fatal."""
    from mason_cli.cli_output import print_info, print_success, print_warning
    try:
        from mason_cli.slack_cli import _build_full_manifest
        from mason_constants import get_mason_home
        manifest = _build_full_manifest(
            bot_name="Mason", bot_description="Your Mason agent on Slack")
        target = _Path(get_mason_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `mason slack manifest --write` anytime to refresh after "
            "Mason adds new commands.")
    except Exception as e:
...
def interactive_setup() -> None:
    ...
    from mason_cli.config import get_env_value, remove_env_value, save_env_value
    from mason_cli.cli_output import (
    ...
    if get_env_value("SLACK_BOT_TOKEN"):
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `mason update`)", True):
                _write_slack_manifest_and_instruct()
            return
    for line in _SETUP_STEPS:
        print_info(line)
    print()
    print_info("   Full guide: https://mason-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def register(ctx) -> None:
    """Plugin entry point — called by the Mason plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `mason setup` to install Slack support.",
