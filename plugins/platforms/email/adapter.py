"""Email platform adapter for the Mason gateway: users talk to Mason by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _mason_version = "0"
        imap.xatom("ID", f'("name" "mason-agent" "version" "{_mason_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `mason gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Mason Agent"), ("Date", formatdate(localtime=True))):
            msg[key] = value
