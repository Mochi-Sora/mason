...
_DEFAULT_DESCRIPTION = "Mason Agent — a general-purpose agent reachable over A2A."
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"mason-{socket.gethostname()}"
    except Exception:
        return "mason-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("MASON_PROFILE", "default") or "default"
...
def _profile_home(profile: str) -> Optional[str]:
    ...
    if profile and profile != "default":
        return os.path.expanduser(f"~/.mason/profiles/{profile}")
    with contextlib.suppress(Exception):
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "name": str(val.get("name") or f"Mason {slug}"),
                "description": str(val.get("description") or f"Mason profile '{profile or slug}' exposed over A2A."),
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``mason chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["mason", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
            env = {**os.environ, "MASON_A2A_PEER": peer}
