...
def build_agent_card(*, name: str, url: str, description: str, skills: Optional[list[dict]] = None,
    ...
    card: dict[str, Any] = {
        ...
        "version": "1.0.0",
        "provider": {"organization": os.getenv("A2A_PROVIDER_ORG", "Mason Agent"), "url": os.getenv("A2A_PROVIDER_URL", "") or url},
...
def skills_from_toolsets(toolsets: "list[str] | dict[str, list[str]] | None") -> list[dict]:
    ...
    skills = [{"id": f"toolset.{name}", "name": name, "description": f"Mason '{name}' capabilities",
...
def _conv_path(context_id: str) -> Path:
    ...
    return _mason_home() / "a2a_conversations" / f"{safe}.jsonl"
...
def list_conversations() -> list[str]:
    ...
    return sorted(p.stem for p in (_mason_home() / "a2a_conversations").glob("*.jsonl"))
