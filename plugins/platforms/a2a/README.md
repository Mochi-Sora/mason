# A2A — Agent-to-Agent protocol for Mason
...
peer (another Mason, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
mason gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Mason serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.mason/a2a_audit.jsonl`.
- Conversations persist to `~/.mason/a2a_conversations/` — they survive context
