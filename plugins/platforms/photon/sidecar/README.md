...
Small Node helper that bridges Mason Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Mason is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Mason-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Mason can receive inbound messages
...
The Mason plugin's `mason photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Mason is Python, so replies go through this sidecar
