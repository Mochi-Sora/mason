...
// `message.content.text` before Mason can see it. We rewrite the two inbound
...
const MARKER = "Mason patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Mason to patch; keep the legacy v8 path below for older pinned installs.
    if (
