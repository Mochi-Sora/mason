// Mason Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Mason): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Mason -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Mason-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `mason photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Mason can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "mason-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Mason'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Mason allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Mason, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-mason-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Mason supplied them so a known-good
