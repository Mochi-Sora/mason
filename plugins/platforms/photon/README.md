...
This plugin connects Mason Agent to iMessage (and other Spectrum
...
TypeScript-only, Mason runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Mason channel,
...
Run `mason photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.mason/.env` (the same place every other
...
Management metadata lives in `~/.mason/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `mason photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Mason
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Mason coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to Mason, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for Mason-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** Mason posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `mason send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<mason-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `mason photon status`, a DM and a group roundtrip,
