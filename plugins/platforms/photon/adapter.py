...
if TYPE_CHECKING:  # type checkers see httpx as always-imported; runtime keeps it optional
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Mason dep
...
# Out-of-process senders (cron, `mason send`) need the live sidecar's port + spawn-time
...
# suppress those so Mason sees the link once.
...
_DEFAULT_MENTION_PATTERNS = [r"(?<![\w@])@?mason\s+agent\b[,:\-]?", r"(?<![\w@])@?mason\b[,:\-]?"]
...
def _runtime_record_path() -> Path:
    from mason_constants import get_mason_home  # honors profile overrides
    return get_mason_home() / "runtime" / _RUNTIME_RECORD_NAME
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        logger.debug("photon: spectrum-ts not installed at %s%s — run: mason photon setup", _sidecar_dir(), hint)
        return False
    return True


def _sidecar_deps_stale() -> bool:
    """True when node_modules predates the lockfile (`mason update` rewrites it without
...
def _reinstall_sidecar_deps() -> None:
    ...
    if not npm:
        ...
        return
    from mason_cli._subprocess_compat import windows_hide_flags  # no console flash on Windows
...
def _attachment_body(space_id: str, safe_path: str, *, kind: str, name: Optional[str] = None,
    ...
    so optional keys are only sent when Mason supplied them."""
    body: Dict[str, Any] = {
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _sidecar_headers(self) -> Dict[str, str]:
        return {"X-Mason-Sidecar-Token": self._sidecar_token}
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not self._project_id or not self._project_secret:
            self._set_fatal_error(
                "MISSING_CREDENTIALS",
                "PHOTON_PROJECT_ID and PHOTON_PROJECT_SECRET are required. Run: mason photon setup",
                retryable=False)
            return False
    ...
    def _pid_is_sidecar(cls, pid: int) -> bool:
        """True if ``pid``'s command line is a Photon sidecar (any Mason checkout)."""
    ...
    async def _ensure_sidecar_deps(self) -> None:
        ...
        if not sidecar_deps_installed():
            # Hosted images have no CLI for `mason photon setup`: connect bootstraps deps itself.
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `mason photon setup`)",
                    code="SIDECAR_DEPS_MISSING", retryable=False)
        # `mason update` bumps the lockfile without reinstalling node_modules; the sidecar
    ...
    async def _start_sidecar(self) -> None:
        ...
        from mason_cli._subprocess_compat import windows_hide_flags  # hide child console on Windows
...
def _standalone_token_from_record(port: int) -> Tuple[Optional[str], int, str]:
    ...
    return None, port, (
        "Photon standalone send requires a running sidecar. Start the Mason gateway (which spawns "
        f"the sidecar and records its address under <mason-home>/runtime/{_RUNTIME_RECORD_NAME}), "
...
async def _standalone_send(
    ...
    headers = {"X-Mason-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Mason plugin loader at startup."""
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: mason photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."),
        setup_fn=_cli.gateway_setup,  # surfaces Photon in the unified `mason gateway setup` wizard
