"""``mason photon ...`` CLI subcommands (registered via ``ctx.register_cli_command()``):
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `mason photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None, help="Project name (default: 'Mason Agent')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    try:
        from mason_cli.config import write_platform_config_field
    ...
    print("\n✓ Photon setup complete.\n  Start the gateway:  mason gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_deps_installed() else '✗ run `mason photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`mason photon telemetry on|off`)")
    return 0


def _telemetry_enabled() -> bool:
    """PHOTON_TELEMETRY from env / ~/.mason/.env; truthy set mirrors the sidecar's."""
    try:
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `mason photon telemetry on` / `mason photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.mason/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  mason gateway restart")
    return 0
...
def gateway_setup() -> None:
    """Run Photon first-time setup from the unified `mason gateway setup` wizard (same flow
    as ``mason photon setup``; phone is prompted when stdin is a TTY)."""
