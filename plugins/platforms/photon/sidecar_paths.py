...
``$MASON_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `mason photon install-sidecar` on npm failure so check_requirements() can
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_mason_home() / "photon" / "sidecar"
    try:
