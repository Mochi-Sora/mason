...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.mason/.env``; management
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a mason dependency
...
# published CLI client until Mason gets its own client_id.
...
DEFAULT_PROJECT_NAME = "Mason Agent"
DEFAULT_POLL_INTERVAL = 5  # RFC 8628 polling defaults; Photon's `interval` / `expires_in` win
DEFAULT_POLL_TIMEOUT = 1800
E164_RE = re.compile(r"^\+[1-9]\d{6,14}$")


# -- auth.json helpers (shares the file with the rest of mason-agent) ------------

def _auth_json_path() -> Path:
    """``~/.mason/auth.json`` honouring the active Mason profile."""
    try:
        from mason_constants import get_mason_home
        return Path(get_mason_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.mason")) / "auth.json"
...
def _store_pool_record(key: str, record: Dict[str, Any]) -> None:
    ...
    from mason_cli.auth import _auth_store_lock
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.mason/.env`` (secret never bound to a printable local
    in a caller — CodeQL clean flow)."""
    try:
        from mason_cli.config import save_env_value
    except ImportError:
        logger.warning("photon: mason_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `mason photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `mason photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `mason photon setup`)")]
    emit("\n".join(rows))
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `mason photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `mason photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `mason photon setup`)"
