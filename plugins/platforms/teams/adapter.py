...
``<mason-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``mason cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Mason owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Mason"}))
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (compatible; MasonAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        mason_action = data.get("mason_action", "")
        session_key = data.get("session_key", "")
        if not mason_action or not session_key:
        ...
        choice = _APPROVAL_CHOICES.get(mason_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def send_exec_approval(
        ...
        def _action(title: str, mason_action: str, **kw) -> "ExecuteAction":
            return ExecuteAction(
                title=title, verb="mason_approve", data={**btn_data_base, "mason_action": mason_action}, **kw)
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Mason" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from mason_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
    ...
    print()
    print_success("Teams configuration saved to ~/.mason/.env")
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Mason venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
