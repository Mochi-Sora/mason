...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("reset", "Reset your Mason session", (), "/reset", "Session reset~"),
    ...
    ("status", "Show Mason session status", (), "/status", "Status sent~"),
    ("sethome", "Set this chat as the home channel", (), "/sethome", None),
    ("stop", "Stop the running Mason agent", (), "/stop", "Stop requested~"),
    ...
    ("reload-skills", "Re-scan ~/.mason/skills/ for new or removed skills", (), "/reload-skills", None),
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update Mason Agent to the latest version", (), "/update", "Update initiated~"),
    ("restart", "Gracefully restart the Mason gateway", (), "/restart", "Restart requested~"),
    ...
    ("thread", "Create a new thread and start a Mason session in it", (), None, None),
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://mason-agent.nousresearch.com/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning("Ignoring invalid MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r", raw)
    return 30.0
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("MASON_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("MASON_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="MASON_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="MASON_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_mason_home())
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("MASON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        from mason_constants import get_mason_home
        directory = get_mason_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Mason work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Mason/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:mason|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Mason manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "mason_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Mason pairing grant."""
    ...
    def _register_thread_slash(self, tree, name: str, description: str) -> None:
        ...
        @discord.app_commands.describe(
            name="Thread name", message="Optional first message to send to Mason in the thread",
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from mason_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from mason_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill", description="Run a Mason skill", callback=_skill_handler,
            )
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from mason_cli.commands_platforms import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Mason: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Real semantic naming is done after the first agent turn, when Mason has an LLM-generated session
        ...
        thread_name = content[:80] if content else "Mason"
        if len(content) > 80:
    ...
    def _stamp_auto_thread_name(thread: Any, thread_name: str) -> Any:
        ...
        try:
            setattr(thread, "_mason_auto_thread_initial_name", thread_name)
        except Exception:
            pass
        return thread
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Mason: **{thread_name}**"
                    )
    ...
    async def rename_thread(
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Mason semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Mason handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
    ...
    async def send_exec_approval(
        ...
        def _build(_channel):
            ...
            prompt_prefix = (
                ...
                "Do you want Mason to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        def _build(_channel):
            ...
            content = self._self_contained_prompt_content(
                "❓ **Mason needs your input**", str(question or "").strip(), tail=f"\n\n{hint}",
            )
    ...
    async def send_update_prompt(
        ...
        """Yes/No prompt for the gateway ``/update`` watcher when ``mason update --gateway`` needs input."""
        def _build(_channel):
    ...
    async def _handle_message(
                ...
                if thread:
                    ...
                    try:
                        ...
                        await message.channel.send(
                            "⚠️ Mason could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_mason_auto_thread_initial_name", None)
...
def _define_discord_view_classes() -> None:
    ...
    class _MasonView(discord.ui.View):
        """Shared plumbing for Mason component views: allowlist auth, single-use
    ...
    class ExecApprovalView(_MasonView):
    ...
    class SlashConfirmView(_MasonView):
    ...
    class UpdatePromptView(_MasonView):
        """Yes/No buttons for ``mason update`` prompts; the answer is written to
        ...
        async def _respond(self, interaction: discord.Interaction, answer: str, color: discord.Color, label: str):
            ...
            try:
                from mason_constants import get_mason_home
                response_path = get_mason_home() / ".update_response"
    ...
    class ModelPickerView(_MasonView):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                from mason_cli.model_selection_guards import combined_selection_warning
    ...
    class ChoicePickerView(_MasonView):
    ...
    class ClarifyChoiceView(_MasonView):
...
# process (e.g. standalone ``mason cron``); same forum/thread/multipart logic via Discord REST.
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "MASON_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "MASON_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
...
def _is_connected(config) -> bool:
    ...
    Looks up ``mason_cli.gateway.get_env_value`` at call time so tests can patch it (ambient env)."""
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `mason setup` to install Discord support.",
