...
class WeComAdapter(WeComStreamMixin, WeComMediaMixin, ChatSendQueueMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("MASON_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("MASON_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=mason&scode="
...
def qr_scan_for_bot_info(*, timeout_seconds: int = _QR_POLL_TIMEOUT) -> Optional[Dict[str, str]]:
    ...
    def _get_json(url: str, timeout: int) -> Dict[str, Any]:
        req = urllib.request.Request(url, headers={"User-Agent": "MasonAgent/1.0"})
    ...
    try:
        raw = _get_json(f"{_QR_GENERATE_URL}?source=mason", 15)
    except Exception as exc:
...
_ACCESS_CHOICES = (
    ...
    ("Use DM pairing (unknown users request access, you approve with 'mason pairing approve')", (("WECOM_DM_POLICY", "pairing"),),
     (("success", "DM pairing mode — users will receive a code to request access."), ("info", "Approve with: mason pairing approve <platform> <code>"))),
    ("Disable direct messages", (("WECOM_DM_POLICY", "disabled"),), (("warning", "Direct messages disabled."),)),
    ("Skip for now (bot will deny all users until configured)", (), (("info", "Skipped — configure later with 'mason gateway setup'"),)),
)
...
def register(ctx) -> None:
    common = dict(install_hint="Run `mason setup` to install WeCom support.", emoji="💼", allow_update_command=True)
