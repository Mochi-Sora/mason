"""Per-plugin persistent storage: ``<mason home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<mason home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `mason plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<mason home>/plugin-data/<name>/``; resolves ``get_mason_home()`` on
    ...
    root = get_mason_home() / "plugin-data" / _validate_name(name)
