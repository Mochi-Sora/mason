"""Thin Spotify Web API helper used by Mason native tools.
...
from mason_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def _friendly_spotify_error_message(*, status_code: int, detail: str, path: str, retry_after: Optional[str]) -> str:
    ...
    if status_code == 401:
        return "Spotify authentication failed or expired. Run `mason auth spotify` again."
    if status_code == 403:
        if is_playback_path:
        ...
        if "scope" in detail.lower() or "permission" in detail.lower():
            return "Spotify rejected the request because the current auth scope is insufficient. Re-run `mason auth spotify` to refresh permissions."
