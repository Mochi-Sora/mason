...
reflects package importability; the plugin registers either way so ``mason tools`` can offer to
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    if _test_hook:
        ...
        env["MASON_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(BaseWebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        tool-registration time and on every ``mason tools`` paint."""
        try:
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        hung native ``primp`` call cannot freeze the Mason process.
