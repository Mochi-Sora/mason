...
so rows match every other Mason web provider. Config: ``web.backend: "xai"``; optional ``web.xai``:
``model`` (default grok-build-0.1), ``allowed_domains`` / ``excluded_domains`` (max 5, mutually
exclusive), ``timeout`` (default 90s). Auth: Grok OAuth via ``mason auth``, else XAI_API_KEY.
"""
...
from tools.xai_http import has_xai_credentials, mason_xai_user_agent, resolve_xai_http_credentials
...
class XAIWebSearchProvider(BaseWebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        auth-store lock, since this runs on every ``mason tools`` repaint."""
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return _fail("No xAI credentials found. Run `mason auth` to sign in with xAI Grok OAuth, or set XAI_API_KEY.")
    ...
    def _post_responses(base_url: str, payload: Dict[str, Any], api_key: str, timeout: float, *, is_oauth_path: bool) -> tuple[Any, Optional[Dict[str, Any]]]:
        ...
        """
        import httpx
        headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "User-Agent": mason_xai_user_agent()}
