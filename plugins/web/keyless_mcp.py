...
def _fail_msg(vendor: str, kind: str, exc: Any, *, other_backends: bool = True) -> str:
    ...
    alt = " or another web backend via `mason tools`" if other_backends else ""
...
def _per_url(urls: List[str], fetch: Callable[[str], Dict[str, Any]], vendor: str, catch: Any = Exception, hint: bool = False) -> List[Dict[str, Any]]:
    """Per-URL extract loop: a ``catch`` failure becomes an error entry (``hint`` adds the ``mason tools`` hint)."""
...
def provider_tier(name: str) -> str:
    """``web.provider_tier.<name>`` (``mason tools`` Free/Paid rows): ``free``, ``paid``, or ``auto`` (anything else/unset)."""
    try:
...
def mcp_call(url: str, tool: str, arguments: Dict[str, Any], timeout: int = _TIMEOUT_SECONDS) -> str:
    ...
    headers = {"Content-Type": "application/json", "Accept": "application/json, text/event-stream", "User-Agent": "mason-agent"}
    try:
