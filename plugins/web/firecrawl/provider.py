...
def check_firecrawl_api_key() -> bool:
    """True when the route selected via ``mason tools`` (or, on a never-configured
...
def _get_firecrawl_client() -> Any:
    ...
    def _unconfigured_message() -> str:
        ...
        if _backend_helpers.managed_nous_tools_enabled():
            return message + " With your Nous subscription you can also use the Tool Gateway. run `mason tools` and select Nous Subscription as the web provider."
