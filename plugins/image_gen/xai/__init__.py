...
from tools.xai_http import (
    build_xai_storage_options, mason_xai_user_agent, maybe_mark_xai_storage_notice_seen,
...
def _fetch_live_models() -> Dict[str, Dict[str, Any]]:
    ...
    response = requests.get(
        ...
        headers={"Authorization": f"Bearer {api_key}", "User-Agent": mason_xai_user_agent()},
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_factory(provider_name, aspect_ratio)(
                "No xAI credentials found. Configure xAI OAuth in `mason model` or set XAI_API_KEY.",
                "missing_api_key")
        ...
        storage_options = build_xai_storage_options(
            "image_gen", filename_prefix="mason-xai-image", extension="png")
