...
models from :func:`mason_cli.models._fetch_deepinfra_models_by_tag`; no ids hardcoded).
...
class DeepInfraImageGenProvider(StaticImageGenProvider):
    ...
    def generate(self, prompt: str, aspect_ratio: str = DEFAULT_ASPECT_RATIO, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not api_key:
            return fail(
                "DEEPINFRA_API_KEY not set. Run `mason tools` → Image "
                "Generation → DeepInfra to configure, or `mason setup` "
                "to add the key.",
                "auth_required")
        ...
        from mason_cli.models import deepinfra_base_url
