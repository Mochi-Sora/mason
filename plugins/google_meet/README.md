...
Let the mason agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where mason runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `mason meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `mason meet` CLI |
...
| `cli.py` | `mason meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$MASON_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `mason meet node {run,list,approve,remove,status,ping}` |
...
mason meet install                                      # pip + Chromium
mason meet setup                                        # preflight
mason meet auth                                         # optional
mason meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
mason meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.mason/.env
mason meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
mason meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
mason meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.mason/.env
mason meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, mason will **not** switch your system audio input automatically — the
...
mason plugins enable google_meet
mason meet node run --display-name my-mac --host 0.0.0.0 --port 18789
