...
Standalone subprocess spawned by ``process_manager.py``; configured via ``MASON_MEET_*`` env,
status + transcript written under ``$MASON_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``MASON_MEET_URL=... MASON_MEET_OUT_DIR=/tmp/x MASON_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__masonMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__masonMeetInstalled) return;
  window.__masonMeetInstalled = true;
  window.__masonMeetQueue = [];
  ...
  window.__masonMeetDrain = () => {
    const out = window.__masonMeetQueue.slice();
    window.__masonMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__masonMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'mason_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in MASON_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``MASON_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("MASON_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("MASON_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("MASON_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("MASON_MEET_AUTH_STATE", "").strip(),
        guest_name=env("MASON_MEET_GUEST_NAME", "Mason Agent"),
        duration_s=_parse_duration(env("MASON_MEET_DURATION", "")),
        realtime=env("MASON_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # MASON_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("MASON_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("MASON_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("MASON_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("MASON_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("MASON_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__masonMeetDrain && window.__masonMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — MASON_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: MASON_MEET_OUT_DIR is required\n")
        return 2
