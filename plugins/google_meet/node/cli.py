"""`mason meet node ...` subcommand tree, wired under the ``mason meet`` parser."""
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    run.add_argument("--display-name", default="mason-meet-node")
...
def _cmd_run(args: argparse.Namespace, reg: NodeRegistry) -> int:
    ...
    print(f"[meet-node] display_name={server.display_name}\n"
          ...
          f"             mason meet node approve <name> ws://<host>:{args.port} {token}")
    try:
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``mason meet node ...``; returns a process exit code."""
