"""Remote node server — hosts the Meet bot on another machine (``mason meet node run``).

WebSocket endpoint accepting token-signed RPC requests dispatched to ``process_manager``.
Token: 32 hex chars minted on first boot, persisted at ``$MASON_HOME/workspace/meetings/
node_token.json`` so approved gateways survive restarts; the operator copies it to the gateway
via ``mason meet node approve <name> <url> <token>``. ``websockets`` is imported lazily.
"""
...
class NodeServer:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 18789, token_path: Optional[Path] = None,
                 display_name: str = "mason-meet-node") -> None:
        self.host = host
        self.port = port
        self.display_name = display_name
        self.token_path = Path(token_path) if token_path is not None else (
            Path(get_mason_home()) / "workspace" / "meetings" / "node_token.json")
