...
One active meeting at a time, recorded in ``$MASON_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_mason_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Mason Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "MASON_MEET_URL": url, "MASON_MEET_OUT_DIR": str(out),
           "MASON_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "MASON_MEET_HEADED"),
        (auth_state, "MASON_MEET_AUTH_STATE"),
        (duration, "MASON_MEET_DURATION"),
        (mode, "MASON_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "MASON_MEET_REALTIME_MODEL"),
        (realtime_voice, "MASON_MEET_REALTIME_VOICE"),
        (realtime_instructions, "MASON_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("MASON_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["MASON_MEET_REALTIME_KEY"] = realtime_api_key
