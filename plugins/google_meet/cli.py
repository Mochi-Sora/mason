"""CLI commands for the google_meet plugin (``mason meet <subcommand>``).
...
def _auth_state_path() -> Path:
    return Path(get_mason_home()) / "workspace" / "meetings" / "auth.json"


# ``mason meet <sub>`` in help order.
_SUBCOMMAND_HELP = (
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``mason meet`` argparse tree (called at plugin load time)."""
    ...
    p["join"].add_argument("--guest-name", default="Mason Agent")
...
_DISPATCH = {
    ...
    "stop": lambda a: _print_result(pm.stop(reason="mason meet stop")),
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: mason meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
...
def _cmd_setup() -> int:
    ...
    print("  google auth    : " + (f"ok ({auth_path})" if auth_path.is_file() else "not saved — run: mason meet auth"))
    print()
    all_ok = system_ok and pw_ok and chromium_ok
    print("ready. Join a meeting:  mason meet join https://meet.google.com/abc-defg-hij" if all_ok
    ...
    if not realtime:
        ...
        if system == "Linux":
            ...
            print("\n  NOTE: macOS does not auto-route audio. Open\n    System Settings → Sound → "
                  ...
                  "mason will not switch your default input for you.")
    print("\ndone. verify with: mason meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: mason meet join <url>")
    return 0
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="mason meet")
    register_cli(parser)
