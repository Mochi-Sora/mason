"""langfuse — Mason plugin tracing conversations, LLM calls and tool usage to Langfuse.

Activated via ``plugins.enabled``; hooks are inert without the ``langfuse`` SDK
and credentials. Env: MASON_LANGFUSE_PUBLIC_KEY / SECRET_KEY (required),
...
def _debug(message: str) -> None:
    if _env("MASON_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _capture_mode() -> str:
    ...
    value = _env("MASON_LANGFUSE_CAPTURE").lower()
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid MASON_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _build_client() -> Optional[Langfuse]:
    ...
    if Langfuse is None:
        logger.warning(
            ...
            "tracing is disabled. Run `mason tools` and configure Langfuse "
            "Observability to reinstall it."
        )
        return None

    public_key, secret_key = (_env(f"MASON_LANGFUSE_{n}") or _env(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    placeholder_issues = [issue for issue in (
        _validate_langfuse_key("MASON_LANGFUSE_PUBLIC_KEY", public_key),
        _validate_langfuse_key("MASON_LANGFUSE_SECRET_KEY", secret_key),
    ) if issue]
    if placeholder_issues:
        logger.warning(
            "Langfuse plugin: credentials look like placeholders, traces will "
            "NOT be emitted (%s). Set real Langfuse keys (pk-lf-... / sk-lf-...) "
            "or unset MASON_LANGFUSE_PUBLIC_KEY / MASON_LANGFUSE_SECRET_KEY to "
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _env(f"MASON_LANGFUSE_{name}") or _env(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _env("MASON_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
            kwargs["sample_rate"] = float(sample_rate)
        except ValueError:
            logger.warning("Invalid MASON_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("MASON_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    """Translate canonical Mason usage into Langfuse usage and cost maps."""
    ...
    # need an explicit total (Mason estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "mason", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Mason turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Mason turn",
                                      tags=["mason", "langfuse"]):
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Mason versions:
