...
when a client_id (``dashboard.oauth.client_id`` / ``MASON_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
def _settings() -> dict:
    ...
    client_id = resolve_env_or_cfg("MASON_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("MASON_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "MASON_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Mason Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"MASON_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
