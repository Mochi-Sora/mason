...
secret (``MASON_DASHBOARD_DRAIN_SECRET``, env-only — it is a credential); an inbound bearer
...
from mason_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("MASON_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "MASON_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
    ...
    if reason is not None:
        raise SkipRegistration(
            f"MASON_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    reason) when ``MASON_DASHBOARD_DRAIN_SECRET`` is unset or fails the entropy gate. On
    ...
    try:
        from mason_cli.dashboard_auth.token_auth import register_token_route
