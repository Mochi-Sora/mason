...
``MASON_DASHBOARD_BASIC_AUTH_*`` env vars (env wins when non-empty; see ``_settings``).
"""
...
from mason_cli.dashboard_auth import DashboardAuthProvider, InvalidCredentialsError, RefreshExpiredError, Session
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    raw = resolve_env_or_cfg("MASON_DASHBOARD_BASIC_AUTH_SECRET", cfg_section.get("secret"))
    if not raw:
        logger.info(
            ...
            "MASON_DASHBOARD_BASIC_AUTH_SECRET) for stable sessions.")
...
def _settings() -> dict:
    ...
    username = setting("MASON_DASHBOARD_BASIC_AUTH_USERNAME", "username")
    password_hash = setting("MASON_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", "password_hash")
    plaintext = setting("MASON_DASHBOARD_BASIC_AUTH_PASSWORD", "password")
    ttl_raw = setting("MASON_DASHBOARD_BASIC_AUTH_TTL_SECONDS", "session_ttl_seconds")
    if not username:
        raise SkipRegistration(
            "dashboard.basic_auth.username is not set (and MASON_DASHBOARD_BASIC_AUTH_USERNAME "
    ...
    plaintext_from_env = os.environ.get("MASON_DASHBOARD_BASIC_AUTH_PASSWORD", "").strip()
...
_PLUGIN_COMPAT_LAZY = {
    'LoginStart': ('mason_cli.dashboard_auth', 'LoginStart'),
}
