...
``dashboard.oauth.self_hosted.{issuer,client_id,scopes,client_secret}`` or ``MASON_DASHBOARD_OIDC_*``.
"""
...
from mason_cli.dashboard_auth import LoginStart, ProviderError, Session
...
def _settings() -> dict:
    ...
    issuer = setting("MASON_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("MASON_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            "Self-hosted OIDC dashboard auth is not configured. Set both an issuer and "
            "a client_id — either as env vars (MASON_DASHBOARD_OIDC_ISSUER + "
            "MASON_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("MASON_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.mason/.env. Empty ⇒ public client.
        "client_secret": setting("MASON_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('mason_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('mason_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('mason_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('mason_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
