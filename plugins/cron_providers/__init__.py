...
``$MASON_HOME/plugins/<name>/`` (bundled wins on collision). The built-in InProcessCronScheduler
...
_USER_NAMESPACE = "_mason_user_cron"
...
def _user_provider_dirs() -> List[Path]:
    """User-installed ``$MASON_HOME/plugins/<name>/`` dirs that look like cron providers."""
