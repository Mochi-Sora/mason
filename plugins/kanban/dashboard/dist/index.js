/**
 * Mason Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.mason/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__MASON_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__MASON_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "mason-kanban-dot-triage",
    todo: "mason-kanban-dot-todo",
    ready: "mason-kanban-dot-ready",
    running: "mason-kanban-dot-running",
    blocked: "mason-kanban-dot-blocked",
    review: "mason-kanban-dot-review",
    done: "mason-kanban-dot-done",
    archived: "mason-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-mason-task";
  ...
  const DOCS_URL = "https://mason-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://mason-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "mason.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="mason-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])mason-kanban-md-code\1/i.test(attrs)) {
      return ' class="mason-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "mason-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "mason-kanban-md",
  ...
  // standard `drop` event and our `mason-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("mason-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("mason-kanban-column--drop");
          if (target) target.classList.add("mason-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("mason-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("mason-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("mason-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__MASON_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "mason-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "mason-kanban-attention",
        "mason-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "mason-kanban-attention-bar" },
        h("span", { className: "mason-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "mason-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "mason-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "mason-kanban-attention-row",
                  "mason-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "mason-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "mason-kanban-attention-row-id" }, task.id),
                h("span", { className: "mason-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "mason-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "mason-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "mason-kanban-diag-action-btn",
      action.suggested ? "mason-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " mason-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".mason-kanban-drawer-comment-row input, .mason-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "mason-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("mason-kanban-diag", sevClass) },
      h("div", { className: "mason-kanban-diag-header" },
        h("span", { className: "mason-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "mason-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "mason-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "mason-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "mason-kanban-diag-data-row" },
                  h("span", { className: "mason-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "mason-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "mason-kanban-diag-data-row" },
                h("span", { className: "mason-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "mason-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "mason-kanban-diag-reassign-row" },
            h("span", { className: "mason-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "mason-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "mason-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "mason-kanban-diag-msg",
              msg.ok ? "mason-kanban-diag-msg--ok" : "mason-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "mason-kanban-section" },
      h("div", { className: "mason-kanban-section-head-row" },
        h("span", { className: "mason-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "mason-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "mason-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "mason-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "mason-kanban-docs-link",
      title: "Open Mason Kanban docs in a new tab",
      "aria-label": "Mason Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `mason kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "mason-kanban-msg-ok" : "mason-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "mason-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "mason-kanban-boardswitcher" },
      h("div", { className: "mason-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "mason-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "mason-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "mason-kanban-dialog-title" },
        ...
        h("div", { className: "mason-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Mason profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "mason-kanban-bulk" },
      h("span", { className: "mason-kanban-bulk-count" },
      ...
      h("div", { className: "mason-kanban-bulk-priority",
      ...
      h("div", { className: "mason-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Mason profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "mason-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("mason-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("mason-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "mason-kanban-trash",
        dragOver ? "mason-kanban-trash--drop" : "",
        props.draggingTaskId ? "mason-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "mason-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "mason-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".mason-kanban-card")) return true;
      if (target.closest && target.closest(".mason-kanban-column-add")) return true;
      if (target.closest && target.closest(".mason-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("mason-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("mason-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".mason-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "mason-kanban-columns",
        isScrollable ? "mason-kanban-columns--scrollable" : "",
        isPanning ? "mason-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("mason-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("mason-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "mason-kanban-column",
        dragOver ? "mason-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "mason-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "mason-kanban-col-check",
        ...
        h("span", { className: cn("mason-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "mason-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "mason-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "mason-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "mason-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "mason-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "mason-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "mason-kanban-lane" },
                  h("div", { className: "mason-kanban-lane-head" },
                    h("span", { className: "mason-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "mason-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "mason-kanban-card--stale-red";
    if (age >= tier.amber) return "mason-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".mason-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "mason-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "mason-kanban-card",
        props.selected ? "mason-kanban-card--selected" : "",
        props.failed ? "mason-kanban-card--failed" : "",
        props.draggingSource ? "mason-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "mason-kanban-card-content" },
          h("div", { className: "mason-kanban-card-row" },
            h("label", {
              className: "mason-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "mason-kanban-card-check",
            ...
            h("span", { className: "mason-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or mason kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "mason-kanban-warning-badge",
                    "mason-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "mason-kanban-needs-assignee",
          ...
          h("div", { className: "mason-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "mason-kanban-card-row mason-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "mason-kanban-assignee",
                            title: `Assigned to Mason profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "mason-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "mason-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "mason-kanban-ago",
  ...
  // centered modal reusing the mason-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "mason-kanban-dialog mason-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Mason profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Mason profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "mason-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "mason-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "mason-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "mason-kanban-drawer-close",
        ...
        data ? h("div", { className: "mason-kanban-drawer-comment-foot" },
          h("div", {
            className: "mason-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "mason-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "mason-kanban-section" },
      h("div", { className: "mason-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "mason-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "mason-kanban-drawer-body" },
      h("div", { className: "mason-kanban-drawer-title" },
        h("span", { className: cn("mason-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "mason-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "mason-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "mason-kanban-section" },
            h("div", { className: "mason-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "mason-kanban-section" },
            h("div", { className: "mason-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "mason-kanban-done-no-result mason-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "mason-kanban-section" },
            h("div", { className: "mason-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "mason-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "mason-kanban-section" },
        h("div", { className: "mason-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "mason-kanban-comment" },
            h("div", { className: "mason-kanban-comment-head" },
              h("span", { className: "mason-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "mason-kanban-diag-action-btn",
      ...
      h("div", { className: "mason-kanban-section" },
        h("div", { className: "mason-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "mason-kanban-comment" },
                h("div", { className: "mason-kanban-comment-head" },
                  h("span", { className: "mason-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "mason-kanban-comment-ago" },
      ...
      h("div", { className: "mason-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "mason-kanban-event",
              isDiag ? "mason-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "mason-kanban-event-header" },
                  h("span", { className: "mason-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "mason-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "mason-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "mason-kanban-event-header-plain" },
                  h("span", { className: "mason-kanban-event-kind" }, e.kind),
                  h("span", { className: "mason-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "mason-kanban-event-phantom-row" },
                  h("span", { className: "mason-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "mason-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "mason-kanban-event-payload" },
    ...
    return h("div", { className: "mason-kanban-section" },
      h("div", { className: "mason-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "mason-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `mason-kanban-run--${r.outcome || r.status || "ended"}`
          : "mason-kanban-run--active";
        return h("div", { key: r.id, className: cn("mason-kanban-run", outcomeClass) },
          h("div", { className: "mason-kanban-run-head" },
            h("span", { className: "mason-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "mason-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "mason-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "mason-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "mason-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "mason-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "mason-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "mason-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "mason-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "mason-kanban-pre mason-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "mason-kanban-meta-row" },
      h("span", { className: "mason-kanban-meta-label" }, props.label),
      h("span", { className: "mason-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "mason-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "mason-kanban-meta-row" },
        h("span", { className: "mason-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "mason-kanban-meta-value mason-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "mason-kanban-meta-row" },
        h("span", { className: "mason-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "mason-kanban-meta-value mason-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "mason-kanban-meta-row" },
        h("span", { className: "mason-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "mason-kanban-meta-value mason-kanban-editable",
    ...
    return h("div", { className: "mason-kanban-meta-row" },
      h("span", { className: "mason-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "mason-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "mason-kanban-section" },
      h("div", { className: "mason-kanban-section-head-row" },
        h("span", { className: "mason-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "mason-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "mason-kanban-section" },
      h("div", { className: "mason-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "mason-kanban-deps-row" },
        h("span", { className: "mason-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "mason-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "mason-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "mason-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "mason-kanban-dep-chip-x",
      ...
      h("div", { className: "mason-kanban-deps-row" },
        h("span", { className: "mason-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "mason-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "mason-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "mason-kanban-section" },
      ...
      h("div", { className: "mason-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "mason-kanban-home-sub mason-kanban-home-sub--on"
              : "mason-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__MASON_PLUGINS__ && typeof window.__MASON_PLUGINS__.register === "function") {
    window.__MASON_PLUGINS__.register("kanban", KanbanPage);
  }
})();
