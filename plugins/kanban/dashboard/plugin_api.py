...
Every handler is a thin wrapper around ``mason_cli.kanban_db`` (the same code paths the CLI
...
from mason_cli import kanban_db
...
from mason_cli import kanban_diagnostics as kd
from mason_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from mason_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    ``MASON_KANBAN_BOARD`` env var (concurrent requests for different boards would cross-write)."""
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``mason_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"mason_cli.{module}"), fn)(task_id, author=(author or None))
...
def get_board(
    ...
    (``MASON_KANBAN_BOARD`` env → on-disk ``current`` pointer → ``default``)."""
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different MASON_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from mason_cli.kanban import _check_dispatcher_presence
                from mason_constants import get_mason_home
                running, message = _check_dispatcher_presence(mason_home=get_mason_home())
...
class UpdateTaskBody(BaseModel):
    ...
    # Handoff fields forwarded to complete_task on -> 'done' (parity with ``mason kanban complete``).
...
def list_diagnostics(
    ...
    consumed by ``mason kanban diagnostics`` when the dashboard runs."""
    with _board_conn(board) as (board, conn):
...
def reclaim_task_endpoint(task_id: str, payload: ReclaimBody, board: Optional[str] = Query(None)):
    ...
    (``mason kanban reclaim <task_id> --reason ...``)."""
    with _board_conn(board) as (board, conn):
...
def specify_task_endpoint(task_id: str, payload: SpecifyBody, board: Optional[str] = Query(None)):
    """Flesh out a triage task via the auxiliary LLM (``mason kanban specify``). Non-OK is NOT
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    (``mason kanban reassign <task_id> <profile> [--reclaim]``)."""
    with _board_conn(board) as (board, conn):
...
def _active_profile_name() -> str:
    """Current Mason profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    (same substrate as the Models page) so it can't offer a pair Mason rejects. Skips pricing
    and custom-provider probes: a slow/offline local endpoint must not hang the drawer."""
    try:
        from mason_cli.inventory import build_models_payload, load_picker_context
...
def _resolve_project(ref: Optional[str]) -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    with _errors_to_500("projects unavailable"):
        from mason_cli import projects_db as pdb
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from mason_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            from mason_constants import get_mason_home  # type: ignore
            profile_dir = Path(get_mason_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    """``mason profile describe <name> --auto``: persist with ``description_auto: true``.
    Non-OK outcomes are NOT HTTP errors — the UI renders the reason inline."""
    with _errors_to_500("describer crashed"):
        from mason_cli import profile_describer
...
def decompose_task_endpoint(task_id: str, payload: DecomposeBody, board: Optional[str] = Query(None)):
    """Fan a triage task out into child tasks via the auxiliary LLM (``mason kanban decompose``).
