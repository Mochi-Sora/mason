...
Config chain: $MASON_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
"""
...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def save_config(self, values, mason_home):
        """Merge ``values`` into $MASON_HOME/honcho.json (Honcho SDK native format)."""
        from pathlib import Path
        from utils import atomic_json_write
        from plugins.memory.honcho.client import _read_config
        config_path = Path(mason_home) / "honcho.json"
        try:
    ...
    def _resolve_session_key(self, cfg, session_id: str, **kwargs) -> str:
        ...
        ) or session_id or "mason-default"
    ...
    def _run_session_init(self, label: str) -> bool:
        ...
        try:
            self._do_session_init(self._config, self._lazy_init_session_id or "mason-default", **dict(init_kwargs))
        except Exception as e:
            self._manager = None
            self._session_initialized = False
            detail: object = e
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        if cfg.session_strategy == "per-session":
            ...
            try:
                ...
                self._manager.migrate_memory_files(self._session_key, str(get_mason_home() / "memories"))
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return ("[Honcho memory status] Authentication with the Honcho memory backend has expired and automatic "
                ...
                "Tell the user (once) that Honcho memory is paused and that running 'mason honcho setup' "
