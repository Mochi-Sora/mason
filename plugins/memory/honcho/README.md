...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/mason>
...
```bash
mason memory setup honcho   # configure Honcho directly (works on a fresh install)
mason memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
mason config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.mason/.env
```

> `mason honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `mason memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `mason model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MASON_HOME/honcho.json` | Profile-local (isolated Mason instances) |
| 2 | `~/.mason/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Mason profile: `mason` (default) or `mason_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `mason honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Mason session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`mason-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`mason`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-mason-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening mason in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Mason session ID (timestamp + hex). Every `mason` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Mason profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`mason`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Mason profile: `mason` (default) or `mason_<profile>` (e.g. `mason -p coder` -> host key `mason_coder`). Older `mason.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MASON_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `mason-agent`) |
...
|---------|-------------|
| `mason memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `mason honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `mason memory setup`) |
| `mason honcho status` | Show resolved config for active profile |
| `mason honcho enable` / `disable` | Toggle Honcho for active profile |
| `mason honcho mode <mode>` | Change recall or observation mode |
| `mason honcho peer --user <name>` | Update user peer name |
| `mason honcho peer --ai <name>` | Update AI peer name |
| `mason honcho tokens --context <N>` | Set context token budget |
| `mason honcho tokens --dialectic <N>` | Set dialectic max chars |
| `mason honcho map <name>` | Map current directory to a session name |
| `mason honcho sync` | Create host blocks for all Mason profiles |
