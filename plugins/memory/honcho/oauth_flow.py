...
core (the code can arrive via the loopback listener here or a ``mason://``
...
# Loopback redirect registered for the Mason OAuth client. IP-literal so the browser can't resolve the
...
_DEFAULT_CLIENT_ID = "mason-agent"
...
_CALLBACK_HTML = _CALLBACK_PAGE.format(
    title="Honcho connected", body="Connected to Honcho. You can close this tab and return to Mason."
).encode()
...
def start_loopback_flow_background(
    *, config_path: Path | None = None, host: str | None = None, source: str = "mason-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    global _flow_thread
    # Resolve under the caller's profile scope NOW — a context-local MASON_HOME override can't reach the worker.
