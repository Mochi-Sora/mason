"""``mason honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from mason_cli.config import cfg_get

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'mason honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_mason_home() / "honcho.json"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "mason" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'mason honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'mason honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `mason update`; no output, no exceptions."""
...
def _resolve_effective_identity_mapping(cfg: dict, mason_host: dict) -> tuple[bool, dict, str, bool, bool]:
    ...
    pin_sources = (mason_host.get("pinUserPeer"), mason_host.get("pinPeerName"),
                   cfg.get("pinUserPeer"), cfg.get("pinPeerName"))
    pin = bool(next((v for v in pin_sources if v is not None), False))

    def _inherit(key):
        if key in mason_host:
            return mason_host.get(key), False
        val = cfg.get(key)
...
def _apply_runtime_prefix(mason_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        mason_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(mason_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(mason_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {mason_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {mason_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(mason_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(mason_host)
    mason_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        mason_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(mason_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, mason_host: dict, current_peer: str) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, mason_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Gateway identity mapping routes platform users to memory peers.",
             "Running the Mason gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
    ...
    peer_target = mason_host.get("peerName") or current_peer or "user"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(mason_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(mason_host)  # each shape starts from a clean slate
        mason_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(mason_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if mason_cli internals move."""
    try:
        from mason_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(mason_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    mason_host["apiKey"] = cred.access_token
    mason_host["oauth"] = cred.oauth_block()
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        mason_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, mason_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = mason_host.get("apiKey", "")
    ...
    if new_local_key:
        mason_host["apiKey"] = new_local_key
...
def _setup_device_login(mason_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(mason_host, cred)
        return True
    return False


def _setup_browser_login(mason_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="mason-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, mason_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(mason_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(mason_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(mason_host, write_path)
    ...
    print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
          "  Run 'mason honcho setup' again once you have a key.\n")
    return False
...
def _choice_step(mason_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        mason_host[key] = new
    elif fallback is not None:
        mason_host[key] = fallback


def _setup_tuning(cfg: dict, mason_host: dict) -> None:
    ...
    _choice_step(mason_host, "observationMode", _pref(mason_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(mason_host, cfg, "writeFrequency", "async")))
    mason_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(mason_host, cfg, "recallMode", "hybrid")
    _choice_step(mason_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    current_ctx_tokens = _pref(mason_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(mason_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        mason_host["dialecticCadence"] = 2
    elif val >= 1:
        mason_host["dialecticCadence"] = val
    ...
    _choice_step(mason_host, "dialecticReasoningLevel", _pref(mason_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(mason_host, "sessionStrategy", _pref(mason_host, cfg, "sessionStrategy", "per-session"),
...
def cmd_setup(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Mason persistent cross-session memory.\n  Config: {write_path}")
    ...
    mason_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(mason_host)
    ...
    if is_local:
        _setup_local_auth(cfg, mason_host)
    elif not _setup_cloud_auth(cfg, mason_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = mason_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(mason_host, cfg, "aiPeer", "mason")),
        ("workspace", "Workspace ID", _pref(mason_host, cfg, "workspace", "mason")),
    ):
        if new := _prompt(label, default=default):
            mason_host[key] = new

    _setup_identity_mapping(cfg, mason_host, current_peer)

    _setup_tuning(cfg, mason_host)
    mason_host["enabled"] = True
    mason_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        mason_config = load_config()
        mason_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(mason_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: mason config set memory.provider honcho")
  ...
  Other commands:
    mason honcho status     -- show full config
    mason honcho mode       -- change recall/observation mode
    mason honcho tokens     -- tune context and dialectic budgets
    mason honcho peer       -- update peer names
    mason honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Mason profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("mason.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: mason honcho setup\n")
        return
...
Honcho peers
{RULE}
  User peer:   {_pref(mason, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(mason, cfg, 'aiPeer') or _host_key()}
    Mason' identity in Honcho. Seed with 'mason honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(mason, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(mason, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: mason honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(mason, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(mason, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(mason, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Mason asks Honcho's AI peer a question
  ...
  Set with: mason honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'mason honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'mason honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    mason honcho identity --show        — show both peer representations
    mason honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Mason + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Mason\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Mason persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  mason honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'mason honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'mason honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  mason honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'mason'
  with Honcho configured and no prior session history.
  (Mason calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    mason honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'mason honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Mason, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'mason honcho setup' first, then seed manually:")
            print("\n".join(f"    mason honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  mason honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Mason:   per-session by default — each run gets its own session
              Map a custom name:  mason honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. mason honcho setup              — configure API key (required)\n"
              "  2. mason honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. mason honcho status             — verify Honcho connection
  2. mason                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. mason honcho identity --show    — verify AI peer representation
  4. mason honcho tokens             — tune context and dialectic budgets
  5. mason honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to mason memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Mason Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'mason memory setup'...\n")
        from mason_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``mason honcho`` argparse subcommand tree on the ``mason honcho`` parser."""
