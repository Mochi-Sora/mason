...
Config file resolution: $MASON_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Mason profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: MASON_HONCHO_HOST env, else the active profile. The config's
    ``defaultHost`` is honored only for the default profile so named profiles stay isolated."""
    explicit = os.environ.get("MASON_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $MASON_HOME/honcho.json -> default profile's honcho.json
    ...
    local_path = get_mason_home() / "honcho.json"
    if local_path.exists():
        return local_path
    default_path = _get_default_mason_home() / "honcho.json"
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Mason spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_mason_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "mason", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), mason_home=get_mason_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Mason profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, mason_home=get_mason_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_mason_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, "
                         "then run 'mason honcho setup' or set HONCHO_API_KEY. "
...
def _build_client(config: HonchoClientConfig) -> "Honcho":
    ...
    try:
        ...
        raise ImportError("honcho-ai is required for Honcho integration. Install it with: pip install honcho-ai  "
                          "(or run `mason honcho setup` to configure).")
