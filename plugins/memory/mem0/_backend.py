...
def _register_direct_openai_provider() -> None:
    ...
    if not isinstance(provider_map, dict) or not callable(register_provider):
        raise RuntimeError("mem0 LlmFactory does not support the provider registration required for the Mason OpenAI OSS backend")
    ...
    def __init__(self, oss_config: dict):
        ...
        if str(config["llm"].get("provider") or "").strip().lower() == "openai":
            ...
            try:
                ...
                raise RuntimeError("mem0 MemoryConfig does not expose a mutable llm.provider for the Mason OpenAI OSS backend") from exc
