...
```bash
mason memory setup    # select "mem0"
```

Or manually:
```bash
mason config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.mason/.env
```

## Config

Behavioral settings live in `$MASON_HOME/mem0.json` (set them via `mason memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.mason/.env`.
...
| `user_id` | `mason-user` | User identifier on Mem0 |
| `agent_id` | `mason` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   mason memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   mason memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.mason/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.mason/.env
   ```
   or in `$MASON_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Mason session and call `mem0_search` — it connects to your server.
...
```bash
mason memory setup mem0 --mode oss \
...
```bash
mason memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$MASON_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.mason/mem0_qdrant"}}
  }
}
```
...
```bash
mason memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
mason memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$MASON_HOME/mem0.json`).
