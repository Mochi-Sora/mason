...
```bash
mason memory setup    # select "byterover"
```

Or manually:
```bash
mason config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.mason/.env
```
...
Working directory: `$MASON_HOME/byterover/` (profile-scoped).
