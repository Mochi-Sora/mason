...
``$MASON_HOME/plugins/<name>/``, project ``./.mason/plugins/<name>/`` (opt-in via
MASON_ENABLE_PROJECT_PLUGINS), then ``mason_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "mason_agent.memory_providers"
...
_USER_NAMESPACE = "_mason_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.mason/plugins/`` or None. Gated on MASON_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from mason_cli.plugins import _env_enabled

        if not _env_enabled("MASON_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".mason" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``mason <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from mason_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from mason_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _mason_user_memory.<name>.cli, usually before the
