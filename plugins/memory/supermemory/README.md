...
- Hosted: API key from [app.supermemory.ai/integrations?connect=mason](http://app.supermemory.ai/integrations?connect=mason)
...
```bash
mason memory setup    # select "supermemory"
```

Or manually:

```bash
mason config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.mason/.env
```
...
Before running `mason memory setup`, add the local endpoint to
`$MASON_HOME/supermemory.json`:
...
Then run `mason memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$MASON_HOME/supermemory.json`
...
| `container_tag` | `mason` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `mason-{identity}` → `mason-coder`). |
...
`https://api.supermemory.ai`. Mason resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: mason`, and document writes stamp
`metadata.sm_source: mason`. This is a **functional routing key, not telemetry**:
it groups Mason-written memories into a dedicated "Mason" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Mason profile:

```json
{
  "container_tag": "mason-{identity}"
}
```

For a profile named `coder`, this resolves to `mason-coder`. The default profile resolves to `mason-default`. Without `{identity}`, all profiles share the same container.
