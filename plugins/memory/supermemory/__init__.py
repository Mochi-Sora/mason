...
_DEFAULT_CONTAINER_TAG = "mason"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=mason"
...
def _load_supermemory_config(mason_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $mason_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if mason_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(mason_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, mason_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(mason_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "mason"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Mason writes into the "Mason" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "mason", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, mason_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(mason_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._mason_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `mason memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, mason_home):
        ...
        _save_supermemory_config(sanitized, mason_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from mason_constants import get_mason_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_mason_home())))}

    def post_setup(self, mason_home: str, config: dict) -> None:
        from mason_cli.config import save_config
        from mason_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, mason_home=mason_home)
        ...
        status = _probe_supermemory_connection(api_key, mason_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from mason_constants import get_mason_home
        self._mason_home = kwargs.get("mason_home") or str(get_mason_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._mason_home)
