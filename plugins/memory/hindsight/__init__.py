...
Config: $MASON_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Mason Agent and the User"
...
def _context_thread(target, name: str) -> threading.Thread:
    ...
    closed without the profile's secret scope + MASON_HOME override. (The shared
...
def _load_config() -> dict:
    """$MASON_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_mason_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
        if path.exists():
    ...
    return {
        ...
        "banks": {"mason": {"bankId": os.environ.get("HINDSIGHT_BANK_ID", "mason"),
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Mason event time with an explicit UTC offset."""
    event_time = _mason_now()
    # mason_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "mason", "mid", ""
    ...
    def save_config(self, values, mason_home):
        """Merge *values* into $MASON_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(mason_home) / "hindsight" / "config.json"
    ...
    def post_setup(self, mason_home: str, config: dict) -> None:
        ...
        run_setup(self, mason_home, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "mason"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: mason-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Mason Agent and the User"},
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "mason"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "mason"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "mason", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "mason"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            msg = ("Hindsight local_embedded mode cannot run as root "
                   ...
                   "memory daemon. Run Mason as a non-root user, or switch "
                   "to cloud / local_external mode via 'mason memory setup'.")
            logger.warning(msg)
            # Also print: otherwise the user would only see Mason get sluggish.
            with contextlib.suppress(Exception):
                # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
                # the user would only see Mason get sluggish. (issue #13125)
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_mason_home() / "logs" / "hindsight-embed.log"
        ...
        try:
            ...
            profile = self._config.get("profile", "mason")
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # See NousResearch/mason-agent#5820.
