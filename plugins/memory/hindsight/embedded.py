...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    at import, so Mason degrades instead of retrying a broken backend).
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and any(m in text for m in ("hindsight'", 'hindsight"', "hindsight_embed")):
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'mason memory setup'. "
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    on the Mason .env during post_setup, where a Notepad BOM would stick to the first key."""
    if not path.exists():
        return {}
...
def _embedded_profile_env_path(config: dict[str, Any]) -> Path:
    profile = str(config.get("profile", "mason") or "mason")
