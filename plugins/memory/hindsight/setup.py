"""`mason memory setup` wizard for the Hindsight provider (``post_setup``)."""
...
def _select(title: str, items: list, values: list, current) -> str | None:
    ...
    from mason_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
...
def _prompt_embedded_llm(llm_provider: str, provider_config: dict, env_writes: dict, mason_env: Path) -> None:
    ...
    env_writes["HINDSIGHT_LLM_API_KEY"] = llm_key or _load_simple_env(mason_env).get("HINDSIGHT_LLM_API_KEY", "")


def run_setup(provider, mason_home: str, config: dict) -> None:
    ...
    if mode == "cloud":
        ...
        _prompt_embedded_llm(llm_provider, provider_config, env_writes, mason_env)

    provider_config.setdefault("bank_id", "mason")
    ...
    save_config(config)
    provider.save_config(provider_config, mason_home)
    if env_writes:
        _write_env(mason_env, env_writes)
    ...
    if _hs_templates.supported_for_mode(mode):
        from mason_cli.memory_setup import _CANCELLED, _curses_select
        ...
        _hs_templates.run_template_step(
            ...
            bank_id=provider_config.get("bank_id", "mason"),
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            materialized_config = json.loads(
                (Path(mason_home) / "hindsight" / "config.json").read_text(encoding="utf-8")
            )
        llm_api_key = (
            env_writes.get("HINDSIGHT_LLM_API_KEY", "")
            or _load_simple_env(mason_env).get("HINDSIGHT_LLM_API_KEY", "")
