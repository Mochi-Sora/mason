...
catalog, filter to the ``mason`` integration, apply a manifest via
...
def fetch_mason_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``mason`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "mason" in (e.get("integrations") or [])]
