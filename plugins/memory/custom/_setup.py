...
from mason_constants import get_mason_home  # noqa: F401 — patched by tests
...
_PGVECTOR_CONTAINER, _PGVECTOR_IMAGE, _PGVECTOR_PASSWORD = "mason-pgvector", "pgvector/pgvector:pg17", "mason"
...
def _prompt_api_key(label: str, env_var: str, mason_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    env_path = Path(mason_home) / ".env"
...
def _persist_provider_config(mason_home: str, config: dict, provider_config: dict, env_writes: dict[str, str], label: str, key_line: str, server: str | None = None) -> None:
    ...
    Mem0MemoryProvider().save_config(provider_config, mason_home)
    if env_writes:
        _write_env(Path(mason_home) / ".env", env_writes)
    if server:
...
def _setup_platform(mason_home: str, config: dict, flags: dict[str, str]) -> None:
    """Platform mode setup — prompts for API key (secret -> .env), user/agent ids and rerank (-> mem0.json)."""
    provider_config = _read_mem0_json(Path(mason_home) / "mem0.json")
    print("\n  Configuring mem0:\n")
    env_writes = _api_key_writes(flags, "Mem0 Platform API key", url="https://app.mem0.ai")
    for key, desc, default in (("user_id", "User identifier", "mason-user"), ("agent_id", "Agent identifier", "mason")):
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(f"\n  ⚠ MEM0_HOST is set in your environment ({os.environ['MEM0_HOST']}). It overrides platform mode — remove it from ~/.mason/.env (or unset it) or Mason will keep routing to the self-hosted server.")
    _persist_provider_config(mason_home, config, provider_config, env_writes, "mem0", "API keys saved to .env")
...
def _setup_selfhosted(mason_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt("User identifier", default=provider_config.get("user_id") or "mason-user")
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "mason")
    if flags.get("dry_run"):
    ...
    _persist_provider_config(mason_home, config, provider_config, env_writes, "mem0 (self-hosted)", "API key saved to .env", server=host)
...
def _finish_oss(mason_home: str, config: dict, oss_config: dict, env_writes: dict[str, str], user_id: str, agent_id: str, pgvector_config: dict | None = None) -> None:
    """Shared OSS tail: write secrets + mem0.json, install deps, activate, check, summarize."""
    if env_writes:
        _write_env(Path(mason_home) / ".env", env_writes)
    config_path = Path(mason_home) / "mem0.json"  # merge-write, plain text (platform path uses save_config's 0600 atomic write)
...
def _setup_oss(mason_home: str, config: dict, flags: dict[str, str]) -> None:
    """OSS mode — non-interactive when --mode was given, otherwise curses pickers."""
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(mason_home, config)
        return
    ...
    if flags.get("dry_run"):
        ...
        print("  [dry-run] No files written.\n")
        return
    _finish_oss(mason_home, config, oss_config, env_writes, flags.get("user_id") or os.getenv("USER", "mason-user"), "mason")
...
def _configure_model_provider(kind: str, registry: dict, mason_home: str, env_writes: dict[str, str], llm: tuple[str, dict] | None = None) -> tuple[str, dict, str, str | None]:
    ...
    if pdef["needs_key"]:
        if llm is None or pid != llm[0]:
            if key := _prompt_api_key(pdef["label"] if llm is None else f"{pdef['label']} embedder", pdef["env_var"], mason_home):
...
def _setup_oss_interactive(mason_home: str, config: dict) -> None:
    env_writes: dict[str, str] = {}
    llm_id, llm_def, llm_model, llm_url = _configure_model_provider("LLM", LLM_PROVIDERS, mason_home, env_writes)
    embedder_id, _, embedder_model, embedder_url = _configure_model_provider("Embedder", EMBEDDER_PROVIDERS, mason_home, env_writes, llm=(llm_id, llm_def))
    ...
    user_id = _input("User ID", os.getenv("USER", "mason-user"))
    agent_id = _input("Agent ID", "mason")
    flags = {
    ...
    _finish_oss(mason_home, config, oss_config, env_writes, user_id, agent_id, pgvector_config)
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        ...
        try:
            # Environment-aware install: sealed hosted venvs redirect to the durable data-volume target instead of /opt/mason.
...
def post_setup(mason_home: str, config: dict) -> None:
    """Entry point for `mason memory setup`: routes on --mode (platform / selfhosted / oss), else shows a picker.
    ...
    handler(mason_home, config, flags)
