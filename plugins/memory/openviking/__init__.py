...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-mason/{_MASON_VERSION}"
...
_SYNC_TRACE_ENV = "MASON_OPENVIKING_SYNC_TRACE"
...
_RETRY_LATER = (
    "OpenViking memory is temporarily unavailable; Mason will retry on a later access or when the config changes."
)
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Mason slash-skill scaffolding before sending content to OpenViking
...
def _normalize_openviking_url(url: str) -> str:
    ...
    try:
        ...
        raise _OpenVikingEndpointError("OpenViking endpoint safety validation failed; Mason refused the connection.") from exc
    if blocked:
...
def _load_mason_openviking_config() -> dict:
    try:
...
def _mason_home_path() -> Path:
    try:
        ...
        return Path(env_home).expanduser() if env_home else Path.home() / ".mason"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        return _LOCAL_SERVER_OCCUPIED, (
            f"Port {host}:{port} is occupied by {_describe_local_port_listener(host, port)}. Mason did not start "
    ...
    log_path = _mason_home_path() / _OPENVIKING_SERVER_LOG_RELATIVE_PATH
    try:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        # Strip PYTHONPATH: the Desktop backend puts the Mason venv on it, which
        # would shadow openviking-server's own site-packages (and on Windows lock
        # the Mason venv's .pyd files, breaking `mason update`).
        # Do not let the server child inherit this process's PYTHONPATH. If inherited, openviking-server
        # would import aiohttp and friends from the Mason venv instead of its own (its venv's site-packages
        # are shadowed because PYTHONPATH precedes them) — and on Windows the loaded DLLs then lock the
        # Mason venv, aborting `mason update` with access-denied on .pyd files. (#78153)
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._session_id, self._turn_count, self._mason_home = "", 0, ""
        ...
        # increment. See mason-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_mason_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], mason_home: str) -> None:
    ...
    def post_setup(self, mason_home: str, config: dict) -> None:
        """Interactive setup that can reuse OpenViking's shared CLI config (see ``_setup``)."""
        _setup.run_setup(mason_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_mason_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        self._mason_home = str(kwargs.get("mason_home") or "").strip() or str(_mason_home_path())
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        so OPENVIKING_* values added to ``~/.mason/.env`` after startup never reach the live client and
        tools keep running against stale auth until the user restarts mason (#21130).
        """
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Mason will retry on a later access (after cooldown) or when the config changes.",
                health_message,
            )
        else:
    ...
    def _extract_current_turn_messages(messages: Optional[List[Dict[str, Any]]], user_content: str, assistant_content: str) -> List[Dict[str, Any]]:
        """Slice the completed turn out of Mason' full canonical transcript: the last
    ...
    def _messages_to_openviking_batch(messages: List[Dict[str, Any]], *, assistant_peer_id: str = "") -> List[Dict[str, Any]]:
        """Convert Mason canonical messages into OpenViking batch payloads.
    ...
    def _state_path(self, kind: str, name: str) -> Optional[Path]:
        """Marker/lock file under MASON_HOME: ``pending`` -> pending_sessions/<sid>.json,
        ``lock`` -> runs/<run_id>.lock; an empty run id maps to the legacy recovery lock."""
        name = str(name or "").strip()
        if not self._mason_home or (not name and kind != "lock"):
            return None
        if kind == "pending":
            return Path(self._mason_home) / _PENDING_SESSIONS_RELATIVE_DIR / f"{quote(name, safe='')}.json"
        return Path(self._mason_home) / _RUN_LOCKS_RELATIVE_DIR / (f"{quote(name, safe='')}.lock" if name else _LEGACY_RECOVERY_LOCK_FILENAME)
    ...
    def _pending_sessions(self) -> List[tuple[str, str]]:
        ...
        directory = Path(self._mason_home) / _PENDING_SESSIONS_RELATIVE_DIR if self._mason_home else None
    ...
    def _tool_remember(self, args: dict) -> str:
        """Submit content through a dedicated session so it never touches the live Mason session."""
        ...
        session_id = f"mason-remember-{uuid.uuid4().hex[:12]}"
        ...
        def failure(message: str, *, stage: str, message_status: str) -> str:
            return tool_error(
                ...
                recovery_note=(
                    ...
                    "Mason. Otherwise, do not resubmit automatically; report the uncertain state to the user."
                ),
            )
        try:
