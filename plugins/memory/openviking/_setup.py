"""Interactive ``mason memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (Mason
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that Mason then
...
def _save_mason_only_config(*, config: dict, provider_config: dict, env_path: Path, values: dict) -> None:
    ov = _ov()
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Mason session to activate.\n")
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    save_choice = select("  Save OpenViking config",
                         [("Keep in Mason only", "write values only to Mason .env"),
    ...
    _save_mason_only_config(config=config, provider_config=provider_config, env_path=env_path, values=values)
    _print_openviking_ready("Connection saved to Mason .env.")
    return True


def run_setup(mason_home: str, config: dict) -> None:
    """Entry point for ``OpenVikingMemoryProvider.post_setup``."""
    from mason_cli.config import save_config
    from mason_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt
