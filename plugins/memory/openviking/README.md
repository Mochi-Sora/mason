...
- OpenViking server running and reachable from Mason

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Mason can identify older servers that expose the legacy status-only health
...
Then configure Mason:

```bash
mason memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Mason, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
mason config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.mason/.env`; for a named profile use
`~/.mason/profiles/<profile>/.env`.
...
OpenViking's server config is separate from Mason:
...
Mason-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Mason lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Mason sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Mason also sends `User-Agent: openviking-memory-mason/<version>` on
OpenViking requests. This standard harness identifier contains the Mason
...
does not ask for a peer ID. Without a configured peer, Mason sends neither
...
order remains environment, linked OpenViking config, then Mason YAML. To use
...
on the old implicit `mason` peer now use user memory for new writes. Without
...
Set `agent: mason` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `mason-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Mason
...
Use the same OpenViking profile and credentials as Mason for manual recovery.
...
Mason built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Mason action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Mason
...
file URIs, such as
`viking://user/default/peers/mason/memories/preferences/mem_abc123.md` (any
