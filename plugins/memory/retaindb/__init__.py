...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "mason-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "mason"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > mason-<profile> > "default" (API auto-creates "default").
        project = os.environ.get("RETAINDB_PROJECT") or cfg.get("project")
        if not project:
            profile_name = os.path.basename(str(kwargs.get("mason_home", "")))
            project = f"mason-{profile_name}" if profile_name not in {"", ".mason"} else "default"
        ...
        self._agent_id = kwargs.get("agent_id", "mason") or "mason"
