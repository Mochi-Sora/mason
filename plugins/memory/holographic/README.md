...
```bash
mason memory setup    # select "holographic"
```

Or manually:
```bash
mason config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.mason-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MASON_HOME/memory_store.db` | SQLite database path |
