...
An out-of-tree ACP provider (``~/.mason/plugins/model-providers/`` or a pip entry point)
...
copilot_acp = CopilotACPProfile(
    ...
    # mason_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("MASON_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="MASON_COPILOT_ACP_ARGS",
)
