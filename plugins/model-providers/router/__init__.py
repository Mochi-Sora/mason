...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``mason_cli/providers.py``
...
def _resolve_api_key() -> str:
    ...
    try:
        from mason_cli.config import get_env_value_prefer_dotenv as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_mason_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
router = RouterProfile(
    ...
    default_headers={"User-Agent": f"Mason-Agent/{_MASON_VERSION}"},
