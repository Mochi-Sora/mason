...
Plain mixin for ``mason_state.SessionDB`` (no ``__init__``/state of its own).
Must never import mason_state (cycle); shared constants live in mason_state_common.
"""
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._mason_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._mason_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
        ...
        if attempts >= _FTS_HOLDER_ESCALATE_ATTEMPTS and now - first_seen >= _FTS_HOLDER_ESCALATE_SECONDS:
            ...
            if foreign_holders:
                logger.error(
                    ...
                    "`mason sessions optimize-storage` with the gateway stopped. "
                    "`mason doctor` reports this degraded state.", attempts, foreign_holders,
                )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched mason_constants.get_mason_home.
            from mason_constants import get_mason_home as _home
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `mason sessions optimize-storage` performs it. The FTS
        ...
        # as before, and we only record a flag advertising that the optimization is available. `mason
    ...
    def _backfill_gateway_metadata_from_sessions_json(self, cursor: sqlite3.Cursor) -> None:
        ...
        sessions_file = get_mason_home() / "sessions" / "sessions.json"
