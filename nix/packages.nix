# nix/packages.nix — Mason Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./mason-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.masonTui;
        web = full.masonWeb;
        desktop = full.masonDesktop;

        update-npm-lockfile = full.masonNpmLib.updateNpmLockfile;
      };
    };
}
