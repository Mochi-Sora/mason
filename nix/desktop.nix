# nix/desktop.nix — Mason Desktop (Electron) app build + wrapper
#
# `masonAgent` is the fully-built `.#default` package — it ships the
# `mason` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `MASON_DESKTOP_MASON` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Mason CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # MASON_HOME and MASON_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "mason-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "mason-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = masonNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "mason-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe masonNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/mason-desktop $out/bin
    cp -r ${renderer}/* $out/share/mason-desktop/
    ...
    substituteInPlace $out/share/mason-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/mason-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # MASON_DESKTOP_MASON to the absolute path of the nix-built `mason`
    # binary so the desktop's resolver step 4 ("existing Mason CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/mason-desktop \
      --add-flags "$out/share/mason-desktop" \
      --set MASON_DESKTOP_MASON "${lib.getExe masonAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/mason.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../mason_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/mason-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/mason.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/mason.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Mason Agent";
    homepage = "https://github.com/NousResearch/mason-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "mason-desktop";
  };
}
