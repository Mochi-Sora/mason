...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      masonNpmLib = self'.packages.default.passthru.masonNpmLib;
      ...
      masonAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "mason" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../mason} $out/bin/mason
          '')
        ...
        shellHook = ''
          ${masonAgentDevShellHook}
          ${masonNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export MASON_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Mason Agent dev shell in $MASON_PYTHON_SRC_ROOT"
          echo "Ready. Run 'mason' or 'sandbox mason' to start."
        '';
      };
    };
}
