...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Mason derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            mason-agent = prev.mason-agent.overrideAttrs (_old: {
              MASON_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $MASON_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$MASON_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        mason-agent = prev.mason-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "mason-agent-env" {
    mason-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "mason-agent-editable-env" {
    mason-agent = dependency-groups;
  };
}
