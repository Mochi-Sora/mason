# nix/nixosModules.nix — the NixOS module for mason-agent
#
...
#
# Container mode: mason runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MASON_HOME/.env
# and read by mason at startup — no container recreation needed for env changes.
#
# Tool resolution: the mason wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.mason-agent = {
#     enable = true;
#     settings.model.default = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."mason/env".path ];
#   };
#
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.mason-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      mason-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      masonHome = "${cfg.stateDir}/.mason";
      ...
      # MASON_HOME via addToSystemPackages, so they can save settings through the
      ...
      # mason user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "mason-container-entrypoint" ''
        set -eu

        MASON_UID="''${MASON_UID:?MASON_UID must be set}"
        MASON_GID="''${MASON_GID:?MASON_GID must be set}"

        # ── Group: ensure a group with GID=$MASON_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$MASON_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="mason"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$MASON_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$MASON_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$MASON_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$MASON_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="mason"
          TARGET_HOME="/home/mason"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$MASON_UID" -g "$MASON_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$MASON_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$MASON_UID:$MASON_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure MASON_HOME is owned by the target user.
        ...
        if [ -n "''${MASON_HOME:-}" ] && [ -d "$MASON_HOME" ]; then
          find "$MASON_HOME" \! -user "$MASON_UID" -exec chown "$MASON_UID:$MASON_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/mason-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/mason-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/mason ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/mason
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$MASON_UID" --regid="$MASON_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $MASON_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "mason-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        mason_bin=${containerDataDir}/current-package/bin/mason
      '';

      # Default: /var/lib/mason/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the mason group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit masonHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.mason-agent =
        common.sharedOptions {
          defaultPackage = mason-agent;
          defaultPackageText = lib.literalExpression "mason-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              ...
              description = "State directory. Contains .mason/ subdir (MASON_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the mason CLI to environment.systemPackages and export
                MASON_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.mason symlink to the service
                  stateDir. These users are automatically added to the mason group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.mason-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the mason CLI to system PATH and export MASON_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.mason/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.MASON_HOME = masonHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.mason-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.mason-agent.workingDirectory;
                optionPath = "services.mason-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.mason-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the mason user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.mason-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed mason
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure mason is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${masonHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${masonHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."mason-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${masonHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${masonHome} ${cfg.workingDirectory}
                  ...
                  find ${masonHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${masonHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${masonHome}/$_subdir"
                    chmod 2770 "${masonHome}/$_subdir"
                    find "${masonHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg masonHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${masonHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${masonHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${masonHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.mason";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${masonHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "mason-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.mason -> stateDir/.mason for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.mason-agent = {
              description = "Mason Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $MASON_HOME/.env by the activation script. load_mason_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.mason-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.mason-agent = {
              description = "Mason Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  MASON_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  MASON_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env MASON_UID="$MASON_UID" \
                    --env MASON_GID="$MASON_GID" \
                    --env MASON_HOME=${containerDataDir}/.mason \
                    --env MASON_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/mason gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
