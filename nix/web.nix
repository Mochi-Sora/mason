# nix/web.nix — Mason Web Dashboard (Vite/React) frontend build
{ masonNpmLib, ... }:
masonNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @mason/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../mason_cli/web_dist for the
