# nix/homeManagerModules.nix — the Home Manager module for mason-agent
#
# This module is the user-level equivalent of nixosModules.default. Mason is
...
#
# `services.mason-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.mason-agent.enable and
#                                        home.sessionVariables
#   added     programs.mason-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.mason")  -> masonHome, set directly
#
# To use the module:
#   imports = [ mason-agent.homeManagerModules.default ];
#   programs.mason-agent = {
#     enable = true;          # the mason CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.mason-agent;
      ...
      # not reach it, and the application would open ~/.mason while the
      # services use masonHome. Thus the launcher carries the value itself.
      #
      # MASON_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        MASON_HOME = cfg.masonHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) MASON_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        MASON_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("MASON_DESKTOP_REMOTE_URL is set
      # but MASON_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          MASON_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export MASON_DESKTOP_REMOTE_TOKEN
        else
          echo "mason-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "mason-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.mason-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Mason needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.mason-agent` stays the authority for the state and the
      # configuration. This module reads masonHome and the backend address
      # from it, and never the reverse.
      options.programs.mason-agent = {
        enable = lib.mkEnableOption ''
          the Mason Agent command line application.

          This adds `mason` to home.packages, and exports MASON_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.mason-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.mason-agent.package";
          description = ''
            The mason-agent package to install.

            The default follows `services.mason-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Mason Desktop application (Electron).

            This adds `mason-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Mason
            runtime that `package` gives, and reads the MASON_HOME of
            `services.mason-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Mason runtime with
            the usual distribution. This module gives it the Nix package
            instead, with MASON_DESKTOP_MASON. It installs no second copy
            of Mason, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.masonDesktop;
            defaultText = lib.literalExpression "config.programs.mason-agent.package.masonDesktop";
            description = ''
              The mason-desktop package to use.

              The default follows `package`, and thus also
              `services.mason-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Mason runtime, and this module cannot make
      ...
      options.services.mason-agent =
        ...
        // {
          masonHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.mason";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.mason"'';
            description = ''
              The value of MASON_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.mason` to it.
              This module sets MASON_HOME directly. Thus an existing
              ~/.mason continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.mason-work";
          };

          # `installPackage` moved to `programs.mason-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `mason` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.mason-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.MASON_HOME = cfg.masonHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # MASON_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put MASON_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `mason` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.mason-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.masonAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) masonHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.mason-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.mason-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.mason-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "mason-agent";
              };
            })
            ...
            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.mason-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "mason-backend";
              };
            })
          ]
        ))
