...
#
# `services.mason-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  masonHome, programs.mason-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "mason-config-attrs";
    description = "Mason YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $MASON_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Mason Agent";
      ...
      package = mkOption {
        ...
        description = "The mason-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Mason configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `mason config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $MASON_HOME/.env. Mason reads that file at each start, with
          load_mason_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."mason/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Mason refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Mason reads SOUL.md and memories/ from MASON_HOME, so put those
          files in `masonHomeFiles`.
      ...
      masonHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into MASON_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Mason reads SOUL.md and the memory files from MASON_HOME and not
          from the working directory. Declare those files here, or Mason
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the mason plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Mason discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          mason_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as mason (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in mason-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `mason gateway`.";
      };
      ...
      # ── The backend: `mason serve` or `mason dashboard` ──────────────
      # `mason serve` and `mason dashboard` are the same entry point,
      # mason_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Mason Desktop
      ...
      # controls an external gateway, with `mason gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Mason
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            MASON_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Mason Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."mason/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `mason` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.mason-agent.installPackage was removed. Mason now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.mason-agent.enable = ${lib.boolToString (value != false)};  # the mason CLI, and MASON_HOME for your shells
        programs.mason-agent.desktop.enable = true;  # the desktop application

      `services.mason-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "mason-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "mason-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "mason-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "mason-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "mason-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # MASON_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Mason make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${masonHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Mason
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${masonHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${masonHome}/config.yaml
            ${run}chmod ${modes.config} ${masonHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "mason-managed" managedSystem} ${masonHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${masonHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${masonHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${masonHome}/auth.json"
        else
          ''
            if [ ! -e ${masonHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${masonHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree masonHome cfg.masonHomeFiles}
      ...
      ${run}find ${masonHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "mason-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${masonHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/mason"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps mason as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "mason-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "mason-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "mason-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          MASON_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export MASON_DASHBOARD_SESSION_TOKEN

          if [ -z "$MASON_DASHBOARD_SESSION_TOKEN" ]; then
            echo "mason-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "mason-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "mason-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "mason-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "mason-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "mason-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "mason-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Mason Agent web dashboard and desktop backend"
    else
      "Mason Agent backend for Mason Desktop";

  # The environment that each Mason process needs, from either module.
  #
  # managedSystem gives the value of MASON_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Mason an identity and a memory, use
          ${optionPath}.masonHomeFiles instead. Those files go to
          MASON_HOME. Mason reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of MASON_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
