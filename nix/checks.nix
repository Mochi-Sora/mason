...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      mason-agent = self'.packages.default;
      masonVenv = mason-agent.masonVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "mason-check";
                homeDirectory = "/home/mason-check";
                stateVersion = "24.11";
              };
            }
            { services.mason-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.mason-agent = programs;
              services.mason-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.mason-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.mason-agent);
      ...
      configKeys = pkgs.runCommand "mason-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${masonVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "mason-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "mason-build-package" { } ''
          echo "PASS: package built at ${mason-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "mason-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.MASON_TEST = "1";
                environmentFiles = [ "/run/secrets/mason-env" ];
                masonHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "mason" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "mason" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.masonAgentSetup.data;

            failures =
              lib.optional (names != [
                "mason-agent"
                "mason-backend"
              ]) "expected mason-agent + mason-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/mason gateway" (argvOf "mason-agent")
              ) "gateway process does not run `mason gateway`: ${argvOf "mason-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/mason serve" (argvOf "mason-backend")
              ) "backend process does not run `mason serve`: ${argvOf "mason-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "mason-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/mason-check/.mason" (envOf n)) names
              ) "gateway and backend must share one MASON_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.MASON_HOME or null != "/home/mason-check/.mason"
              ) "programs.mason-agent.enable must export MASON_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "mason-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/mason-check/.mason/SOUL.md" activation
              ) "masonHomeFiles must install into MASON_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads MASON_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "MASON_MANAGED=home-manager" (envOf n)) names
              ) "processes must report MASON_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "mason-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "mason-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/mason-check";
            ...
            cases = [
              ...
              {
                name = "masonHomeFiles needs no directory";
                ok = accepts { masonHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "mason-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one MASON_HOME ───────────────
        # `programs.enable` exports MASON_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.mason
        # while the services use the MASON_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/mason-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                masonHome = "/home/mason-check/.mason-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "mason-desktop") cfg.home.packages;
            ...
            # second Mason runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "mason-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.mason-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.mason-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one mason-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "MASON_HOME" != "/home/mason-check/.mason-work"
              ) "the launcher must carry MASON_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "MASON_HOME")})"
              ++ lib.optional (
                setValue "MASON_MANAGED" != "home-manager"
              ) "the launcher must report MASON_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "MASON_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "MASON_DESKTOP_MASON" != "${lib.head agentPackages}/bin/mason"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "MASON_DESKTOP_MASON")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "MASON_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "MASON_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "MASON_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "MASON_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "MASON_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "mason-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares MASON_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set MASON_HOME" wrapper
              ) "the launcher must carry MASON_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "MASON_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "MASON_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? mason-backend || cfg.launchd.agents ? mason-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "mason-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `mason`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.mason-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.mason-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "mason-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "mason-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "mason" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."mason-agent-setup".text;

            failures =
              lib.optional (names != [
                "mason-agent"
                "mason-backend"
              ]) "expected mason-agent + mason-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/mason gateway" (execOf "mason-agent")
              ) "gateway unit does not run `mason gateway`: ${execOf "mason-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/mason dashboard" (execOf "mason-backend")
              ) "backend unit does not run `mason dashboard`: ${execOf "mason-backend"}"
              ++ lib.optional (
                units.mason-agent.environment.MASON_HOME != units.mason-backend.environment.MASON_HOME
              ) "gateway and backend must share one MASON_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/mason/.mason/SOUL.md" activation
              ) "masonHomeFiles must install into MASON_HOME";
          ...
          in
          pkgs.runCommand "mason-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs mason, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.mason-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/mason serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec mason directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "mason-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "mason-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec mason, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/mason dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "mason-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $MASON_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                MASON_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "mason-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'MASON_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--mason-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "mason-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "mason-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${mason-agent}/bin/mason || (echo "FAIL: mason binary missing"; exit 1)
          test -x ${mason-agent}/bin/mason-agent || (echo "FAIL: mason-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${mason-agent}/bin/mason --version 2>&1 | grep -qi "mason" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "mason-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in mason mason-agent mason-acp; do
            test -x ${mason-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "mason-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking mason --help ==="
          ${mason-agent}/bin/mason --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${mason-agent}/bin/mason --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "mason-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${mason-agent}/share/mason-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${mason-agent}/share/mason-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MASON_BUNDLED_SKILLS" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MASON_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${mason-agent}/share/mason-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${mason-agent}/share/mason-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "MASON_OPTIONAL_SKILLS" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, MASON_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "mason-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${mason-agent}/share/mason-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${mason-agent}/share/mason-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MASON_BUNDLED_PLUGINS" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MASON_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "mason-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${mason-agent}/share/mason-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${mason-agent}/share/mason-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${mason-agent}/share/mason-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "MASON_BUNDLED_LOCALES" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: MASON_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + MASON_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && MASON_BUNDLED_LOCALES=${mason-agent}/share/mason-agent/locales \
            ${masonVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with MASON_BUNDLED_LOCALES set"; exit 1)
        ...
        # MASON_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "mason-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${mason-agent}/share/mason-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${mason-agent}/share/mason-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "MASON_OPTIONAL_MCPS" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: MASON_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${mason-agent}/bin/mason mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: mason mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "mason-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${mason-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${mason-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MASON_TUI_DIR" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MASON_TUI_DIR set in wrapper"
        ...
        # Verify MASON_NODE is set in wrapper and points to Node 26+
        # (Mason pins its toolchain to Node 26 everywhere)
        mason-node = pkgs.runCommand "mason-node-version" { } ''
          set -e
          echo "=== Checking MASON_NODE in wrapper ==="
          grep -q "MASON_NODE" ${mason-agent}/bin/mason || \
            (echo "FAIL: MASON_NODE not set in wrapper"; exit 1)
          echo "PASS: MASON_NODE present in wrapper"

          MASON_NODE=$(sed -n "s/^export MASON_NODE='\(.*\)'/\1/p" ${mason-agent}/bin/mason)
          test -x "$MASON_NODE" || (echo "FAIL: MASON_NODE=$MASON_NODE not executable"; exit 1)
          echo "PASS: MASON_NODE executable at $MASON_NODE"

          NODE_MAJOR=$("$MASON_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Mason requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All MASON_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MASON_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "mason-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MASON_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MASON_MANAGED guards ==="
          check_blocked "config set" ${mason-agent}/bin/mason config set model foo
          check_blocked "config edit" ${mason-agent}/bin/mason config edit
        ...
        extra-python-packages = let
          ...
          masonWithExtra = mason-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "mason-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${masonWithExtra}/bin/mason || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${masonWithExtra}/bin/mason || \
          ...
          if grep -q "PYTHONPATH" ${mason-agent}/bin/mason; then
        ...
        extra-dependency-groups = let
          masonWithGroups = mason-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "mason-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${masonWithGroups}"
          echo "venv: ${masonWithGroups.masonVenv}"
        ...
        messaging-variant = pkgs.runCommand "mason-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.masonVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "mason-config-roundtrip" {
          ...
          merge_and_load() {
            local mason_home="$1"
            export MASON_HOME="$mason_home"
            ${configMergeScript} ${nixSettings} "$mason_home/config.yaml"
            ${masonVenv}/bin/python3 -c '
import json, sys
