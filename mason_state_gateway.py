...
from mason_state_common import _RECOVERABLE_END_REASONS_SQL, _RESET_END_REASONS_SQL, _sql_session_last_active
...
class SessionGatewayMixin:
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        from mason_state import psutil
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from mason_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
        now = time.time()
        candidates = []
    ...
    def session_gateway_runtime(session_meta: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        from mason_state import _BARE_BILLING_PROVIDERS
