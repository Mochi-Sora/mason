"""Windows UTF-8 bootstrap for Mason entry points (no-op on POSIX).
...
point (``mason``, ``mason-agent``, ``mason-acp``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``mason_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``mason_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Mason modules.

    Mason ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Mason root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("MASON_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``MASON_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("MASON_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import mason_bootstrap`` first.
