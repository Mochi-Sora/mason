"""Shared constants and helpers for the SessionDB family of modules.  Lives outside mason_state so
...
# optimized via ``mason sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Mason-Session-Key / build_session_key)
...
# and must not serve reads until `mason sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `mason db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `mason_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import mason_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Mason
# processes routinely share one state.db (gateway service, the Desktop app's `mason serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `mason_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not mason_state) because the search/schema mixins
# cannot import mason_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Mason)"
    age = ""
