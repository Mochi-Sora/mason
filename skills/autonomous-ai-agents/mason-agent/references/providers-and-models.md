...
Set via `mason model` (picker) or `mason setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://mason-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `mason auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `mason auth add openai-codex` |
| qwen-oauth | OAuth | `mason auth add qwen-oauth` |
| minimax-oauth | OAuth | `mason auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`mason auth`).
Fallback chain when the primary fails: `mason fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`mason_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   mason config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
