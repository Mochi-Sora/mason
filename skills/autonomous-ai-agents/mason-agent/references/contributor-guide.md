...
For occasional contributors and PR authors. Full developer docs: https://mason-agent.nousresearch.com/docs/developer-guide/
...
├── cli.py                # Interactive CLI (MasonCLI)
├── mason_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── mason_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.mason/config.yaml` (settings), `~/.mason/.env` (API keys) — both under `$MASON_HOME` when it is set.
...
`_MASON_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_mason_home()` for paths,
never hardcode `~/.mason`. For custom/local-only tools, write a plugin in
`~/.mason/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `mason_cli/commands.py`
...
- Tests auto-redirect `MASON_HOME` to temp dirs — never touch real `~/.mason/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/mason_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_mason_home()` from `mason_constants` for all paths (profile-safe)
