# Themes / Skins — Author a Mason Color Theme

Author a Mason **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`mason_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.mason/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Mason itself.
...
- Write access to the Mason home dir — `~/.mason` by default, or `$MASON_HOME`
  / the active profile's dir. Skins live in `<mason-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `mason config set`).
...
3. `write_file` it to `<mason-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `mason config set display.skin default` (or they can `/skin default`).
...
```
mason skin set <key> <hex>      # e.g. mason skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `mason skin set background "#08201f"` changes
only the background; `mason skin use <name>` / `mason skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.mason`** when a profile is active — resolve the real home
  from `$MASON_HOME` first, falling back to `~/.mason`.
...
- **Never hand-edit `config.yaml` to activate.** Use `mason config set
...
- **You apply it, not the user.** `mason config set display.skin <name>` is
...
- `read_file` the written `<mason-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `mason config get display.skin` and confirm it reports `<name>`.
