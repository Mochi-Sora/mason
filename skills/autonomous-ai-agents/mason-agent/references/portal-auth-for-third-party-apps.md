...
## Layer 1 — Is this thing a Mason plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Mason process**. Its LLM calls go through Mason's already-configured provider. | Already uses Portal if user's Mason is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Mason. That LLM call is already authenticated
via Mason's provider config. The plugin's own server auth (e.g.
...
What Mason Agent has that *feels* like OAuth — `mason login --provider nous`
opening a browser, user signs in, token lands in `~/.mason/auth.json` — is a
**Mason-specific browser flow**. Under the hood it produces a credential
Mason uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Mason's existing Portal credential out of `~/.mason/auth.json`.
...
rides on the credential Mason already holds.

Where this could live in Mason:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Mason itself.
...
1. First decide: Mason plugin (runs inside Mason) or separate app? If plugin,
   it already uses Portal via Mason's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Mason agent itself") is fully supported. Mason treats agent
...
pitch in the Tool Gateway announcement is specifically about Mason Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Mason is configured to use Portal as its provider. It is
