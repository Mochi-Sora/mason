...
Mason runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`mason config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Mason's env scrubber dropping
...
Mason-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Mason tool and most Windows APIs. Prefer forward slashes in code
