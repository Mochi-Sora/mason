...
Edit with `mason config edit` or `mason config set section.key value`.
Full reference: https://mason-agent.nousresearch.com/docs/user-guide/configuration
...
`mason config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `mason tools` (interactive) or `mason tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MASON_CORE_TOOLS` is the default bundle most platforms inherit).
