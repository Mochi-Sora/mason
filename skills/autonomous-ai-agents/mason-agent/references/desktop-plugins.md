...
Write plugins for the Mason desktop app: statusbar items, layout panes,
...
system (`~/.mason/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MASON_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MASON_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Mason desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$MASON_HOME/desktop-plugins/` (usually
  `~/.mason/desktop-plugins/`).

## How to Run

1. Create `$MASON_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.mason/...` by default, or `~/.mason/profiles/<profile>/...` under a
...
The ONLY import surface is `@mason/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Mason
  ...
  (`mason://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.mason/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@mason/plugin-sdk`, `react`, and
