...
Author widget apps for the Mason TUI (`mason --tui`): glanceable ambient
...
- The TUI must be in use (`mason --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.mason/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.mason/logs/tui_gateway_crash.log` if a widget never appears.
