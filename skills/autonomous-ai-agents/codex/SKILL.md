...
metadata:
  mason:
    ...
    related_skills: [claude-code, mason-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Mason terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Mason itself, `model.provider: openai-codex` uses Mason-managed Codex
OAuth from `~/.mason/auth.json` after `mason auth add openai-codex`. For the
...
## Mason Gateway Caveat

When invoking the Codex CLI from a Mason gateway/service context (for example,
