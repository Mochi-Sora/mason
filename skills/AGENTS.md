...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `mason skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.mason.tags`, `metadata.mason.category`,
`metadata.mason.related_skills`, `metadata.mason.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.mason.*` by the loader.
...
2. **Prose references native Mason tools or the MCP servers the skill expects, in backticks**
...
4. **`author` credits the human first.** External contributor's real name + GitHub handle first,
   "Mason Agent" second. A commit authored as "Mason Agent" (they drafted with Mason) is replaced
...
in the `mason-agent-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.mason/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `mason_cli/curator.py` → `mason curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.mason/skills/.usage.json`
