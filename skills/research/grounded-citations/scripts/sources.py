...
Ledger path resolution (first wins):
  --ledger PATH
  $MASON_CITATION_LEDGER
  $MASON_HOME/cache/citations/ledger.json
"""
...
from _mason_home import get_mason_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MASON_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_mason_home() / "cache" / "citations" / "ledger.json"
