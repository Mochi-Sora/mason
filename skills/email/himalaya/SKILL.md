...
This skill is separate from the Mason Email gateway adapter. The gateway
adapter lets people email the agent and uses Mason' built-in IMAP/SMTP
...
To reply non-interactively from Mason, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Mason)** — pipe the message via stdin:
