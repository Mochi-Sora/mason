...
| Change | Upstream | Mason |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `mason` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Mason infra |
...
| File operations | Bash commands | Mason file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains Mason-specific adaptations.
