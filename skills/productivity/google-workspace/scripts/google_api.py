#!/usr/bin/env python3
"""Google Workspace API CLI for Mason Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Mason-facing JSON contract and falls back to the Python client
...
from _mason_home import get_mason_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MASON_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Mason Agent")
