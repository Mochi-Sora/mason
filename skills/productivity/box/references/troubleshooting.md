...
| remote OAuth browser ends on an unreachable localhost page | expected `--code` redirect or wrong topology | if Mason is remote, return the URL's `code` and `state` to the waiting CLI; if Mason and the browser are on the same host, stop and restart without `--code` |
...
If two Mason profiles or sessions appear to change each other's Box actor, remember that a private npm installation does not isolate Box CLI environments for the same OS user. List environments, verify the current actor, and ask before switching. On Linux, if the CLI reports plaintext credential fallback, warn about `~/.box` without reading or printing its credential files and recommend configuring Secret Service/libsecret or an isolated runtime user.
