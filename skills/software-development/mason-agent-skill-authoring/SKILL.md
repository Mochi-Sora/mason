---
name: mason-agent-skill-authoring
...
metadata:
  mason:
    tags: [skills, authoring, mason-agent, conventions, skill-md]
    related_skills: [requesting-code-review]
---

# Authoring Mason-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.mason/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the mason-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with mason-agent
...
- Don't use for: personal skills in `~/.mason/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `mason skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Mason Agent
license: MIT
...
- Credit the **human first**, then "Mason Agent" as secondary collaborator: `Ben Barclay (benbarclay), Mason Agent`.
- Never `author: Mason Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Mason Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.mason/skills/`.
...
|---|---|
| Mason tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.mason/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Mason Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.mason.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Mason tools
