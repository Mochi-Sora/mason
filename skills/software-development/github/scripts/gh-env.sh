#!/usr/bin/env bash
# GitHub environment detection helper for Mason Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _mason_env="${MASON_HOME:-$HOME/.mason}/.env"; [ -f "$_mason_env" ] && grep -q "^GITHUB_TOKEN=" "$_mason_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_mason_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
elif [ -f "$HOME/.git-credentials" ]; then
    GITHUB_TOKEN=$(uv run python3 "${MASON_HOME:-$HOME/.mason}/skills/github/github-auth/scripts/git-credential-token.py")
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
fi
