...
- Give it a name like "mason-agent"
- Select scopes:
...
- Give it a title like "mason-agent-<machine-name>"
...
uv run python "${MASON_HOME:-$HOME/.mason}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _mason_env="${MASON_HOME:-$HOME/.mason}/.env"; [ -f "$_mason_env" ] && grep -q "^GITHUB_TOKEN=" "$_mason_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_mason_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${MASON_HOME:-$HOME/.mason}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
