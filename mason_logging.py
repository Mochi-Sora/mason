"""Centralized logging setup for Mason Agent.
...
# in Mason (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from mason_constants import get_config_path, get_mason_home, mkdir_under_mason_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_mason_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.mason_home = str(get_mason_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.mason_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._mason_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``mason logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "mason_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("mason_cli", "cli"),
    "cron": ("cron",),
    "gui": ("mason_cli.web_server", "mason_cli.pty_bridge", "tui_gateway", "uvicorn"),
}


def setup_logging(
    *,
    mason_home: Optional[Path] = None,
    ...
    """Configure the Mason logging subsystem; returns the ``logs/`` directory.
    ...
    home = mason_home or get_mason_home()
    log_dir = mkdir_under_mason_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_mason_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._mason_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from mason_cli.config import is_managed
...
def _new_file_handler(
    ...
    mkdir_under_mason_home(path.parent)
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Mason home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._mason_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "mason_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._mason_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="mason-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_mason_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_mason_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # mason_cli.main's early parse (one config.yaml parse per process);
        # fall back to a direct parse for bare mason_logging consumers.
        try:
            from mason_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
