...
def _is_dispatcher_owned_worker() -> bool:
    """False when MASON_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # Carry profile + approval/sudo context so get_mason_home() resolves correctly.
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        bool(os.environ.get("MASON_KANBAN_TASK")), bool(skip_tool_search_assembly),
...
def _apply_toolset_selection(tools: set, names: List[str], quiet_mode: bool, *, disable: bool) -> None:
    ...
    for name in names:
        if validate_toolset(name):
            ...
            if disable and (name.startswith("mason-") or (get_toolset(name) or {}).get("posture")):
                ...
                if not quiet_mode and name.startswith("mason-") and name not in _WARNED_DISABLED_BUNDLES:
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if (os.environ.get("MASON_KANBAN_TASK") and not _is_delegated_child_context()
    ...
    # toolset is stripped even when a composite (mason-cli) re-enables it.
    # This ensures that even if a composite toolset (like mason-cli) is enabled, any tools belonging to a
...
def _emit_post_tool_call_hook(
    *, function_name: str, function_args: Dict[str, Any], result: Any,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from mason_cli.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if bridged is not None:
        ...
        # Mason working set: described tools join the session set (native next
        # turn); called tools reset the escape-hatch counter. Keyed by session.
        try:
            from tools import working_set as _ws
            _bsid = ids.session_id or ""
            _bargs = function_args if isinstance(function_args, dict) else {}
            if function_name == "tool_describe":
                _dnames = _bargs.get("names", [])
                if isinstance(_dnames, str):
                    _dnames = [_dnames]
                _dnames = [str(n) for n in (_dnames or []) if str(n).strip()]
                if _dnames:
                    _ws.note_described(_bsid, _dnames)
            elif function_name == "tool_call" and underlying is not None:
                _ws.note_called(_bsid, str(underlying[0]))
        except Exception:
            pass
