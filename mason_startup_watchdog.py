...
timeout. Config is env-only (``MASON_STARTUP_WATCHDOG=0``,
``MASON_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "MASON_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "MASON_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (mason_cli.main / mason_cli.gateway /
...
def _process_mason_home() -> Path:
    """MASON_HOME for diagnostic files — stdlib-only replica of the mason_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<MASON_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_mason_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``MASON_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``MASON_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
