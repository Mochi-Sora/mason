...
# Set by ``mason gateway run --external-supervisor``. Unlike systemd's INVOCATION_ID
# and launchd's XPC_SERVICE_NAME, this survives wrappers that replace the child
# environment (e.g. ``sudo env -i``).
EXTERNAL_GATEWAY_SUPERVISOR_ENV = "MASON_GATEWAY_EXTERNAL_SUPERVISOR"
...
def is_gateway_supervisor_process(environ: Mapping[str, str] | None = None) -> bool:
    ...
    return bool(env.get("INVOCATION_ID") or env.get("MASON_S6_SUPERVISED_CHILD") or (xpc_service and xpc_service != "0")
