...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any] = None,
        ...
        self.output_dir = get_mason_home() / "cron" / "output"
    ...
    def _save_full_output(self, content: str, job_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = get_mason_home() / "cron" / "output" / f"{job_id}_{timestamp}.txt"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(content, encoding="utf-8")
        return path

    def _filter_silence_narration_enabled(self) -> bool:
        """``MASON_FILTER_SILENCE_NARRATION`` env overrides the ``gateway.filter_silence_narration`` flag."""
        env = os.getenv("MASON_FILTER_SILENCE_NARRATION")
