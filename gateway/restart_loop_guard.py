...
Defenses 1-2 (``_MASON_GATEWAY`` guard on ``mason gateway stop|restart`` /
...
persisted to ``<MASON_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_mason_home() / "gateway" / "restart_loop.json"
