...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    from mason_cli.install_identity import _fsync_directory
    ...
    return hmac.new(material, b"mason-hosted-room-installation-grant-v1", hashlib.sha256).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local MASON_HOME override; the process environment
        # retains the installation root and is the authority here.
        root = os.environ.get("MASON_HOME") or get_mason_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(api_key.encode("utf-8"), b"mason-hosted-room-grant-v1", hashlib.sha256).digest()
...
def catalog_mapping(
    *, installation_id: str, protocol_versions: Iterable[int] = (PROTOCOL_VERSION,),
    ...
    persistent_process = bool(persistent_process and os.getenv("MASON_DESKTOP") != "1")
    profile = str(target_profile or "").strip() or (os.getenv("MASON_PROFILE") or "default").strip() or "default"
...
def _configured_room_link_url() -> str | None:
    ...
    if (override := os.getenv("MASON_ROOM_LINK_URL")) is not None:
        return override
