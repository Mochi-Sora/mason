...
class TurnRunner:
    ...
    def _progress_onboarding_hint(self, kwargs: dict) -> None:
        ...
        from gateway.run import _mason_home, _load_gateway_config
        ctx = self._ctx
        try:
            if (kwargs.get("duration") or 0) >= ctx._LONG_TOOL_THRESHOLD_S and ctx.progress_mode == "all":
                ...
                if gate_on and not is_seen(cfg, TOOL_PROGRESS_FLAG):
                    ...
                    mark_seen(_mason_home / "config.yaml", TOOL_PROGRESS_FLAG)
        except Exception as err:
    ...
    async def _task_card_publish(self, st) -> None:
        ...
        if not st.native_failed:
            result = await st.adapter.send_native_task_card_progress(
                chat_id=ctx.source.chat_id, tasks=st.visible_tasks(), title="Mason is working",
    ...
    def _skip_context_files(self, platform_key) -> bool:
        ...
        # ``mason gateway setup`` writes ``gateway.platforms`` as a LIST of enabled platform names,
    ...
    def run_sync(self):
        ...
        — never os.environ["MASON_SESSION_KEY"], which would misroute approvals across sessions.
        """
        ...
        # tool worker threads. We deliberately do NOT write os.environ["MASON_SESSION_KEY"] here:
        ...
        # *subprocess* exports MASON_SESSION_KEY (from its own --session-key argv, a separate process) — so
