...
boundary. POSIX: ``$MASON_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
file when the home path exceeds ``sun_path``); Windows: named pipe ``\\\\.\\pipe\\mason-gateway-<hash>``.
...
def windows_pipe_name(home: Path) -> str:
    """Per-MASON_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\mason-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    name = f"mason-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if env.get("INVOCATION_ID"):
        return "systemd"
    if sys.platform == "darwin" and (env.get("XPC_SERVICE_NAME", "").startswith("ai.mason")
                                     or env.get("LAUNCHD_SOCKET")):
        return "launchd"
    if env.get("MASON_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "mason_home")},
        "profile": _profile_label_for_home(record.get("mason_home") or ""),
