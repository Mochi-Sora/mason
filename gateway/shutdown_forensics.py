...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    with contextlib.suppress(Exception):  # noqa: BLE001 — never raise from a signal handler
        ...
        if mason_home_str:
            raw = _read_marker(Path(mason_home_str) / ".gateway-takeover.json")
            if raw is not None:
                ctx["takeover_marker"] = raw[:300]
                ctx["takeover_marker_for_self"] = (f'"target_pid": {pid}' in raw
                                                   or f"'target_pid': {pid}" in raw)
            raw = _read_marker(Path(mason_home_str) / ".gateway-planned-stop.json")
            if raw is not None:
...
def check_systemd_timing_alignment(
    ...
    (upgraded without re-running ``mason setup``) can have ``TimeoutStopSec`` below the stop
    ...
    # /proc/self/cgroup: "0::/user.slice/.../mason-gateway.service"
...
def _systemd_timeout_stop_us(unit_name: str) -> Optional[int]:
    """``TimeoutStopUSec`` of ``unit_name`` in microseconds; ``--user`` first (mason' usual)."""
    for flag in (["--user"], []):
        try:
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    mason_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
