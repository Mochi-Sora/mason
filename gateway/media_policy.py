...
``validate_media_delivery_path`` reads ``MASON_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``MASON_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``MASON_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``mason cron run``, ``mason send``) calls
...
_FLAG_ENVS = (("strict", "MASON_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "MASON_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "MASON_MEDIA_ALLOW_DIRS"
