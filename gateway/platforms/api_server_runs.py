...
_ROOM_RETENTION_REQUEST_KEY = (
    RequestKey("mason.room_run_retention_until", float) if RequestKey is not None
    else "mason.room_run_retention_until")
...
def _remember_room_retention(request: "web.Request", claims: dict[str, Any]) -> None:
    ...
    try:
        ...
        setattr(request, "_mason_room_run_retention_until", value)
...
def _room_retention_until(request: "web.Request") -> float:
    try:
        ...
        value = getattr(request, "_mason_room_run_retention_until", 0)
...
def _set_run_status(self, run_id: str, status: str, **fields: Any) -> Dict[str, Any]:
    ...
    current.update({"object": "mason.run", "run_id": run_id, "status": status, "updated_at": now})
...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    # Same precedence as /v1/responses: body session_id > response chain > X-Mason-Session-Key
...
def _run_agent_sync(self, run: _RunLaunch, agent, approval_notify, *, _api_server):
    ...
    with self._profile_scope(run.request_profile):
        try:
            ...
            # tools.async_delegation sees no MASON_SESSION_CHAT_ID and forces delegations sync.
...
async def _handle_run_approval(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    return web.json_response({
        "object": "mason.run.approval_response", "run_id": run_id, "choice": choice, **request_id_field,
        "resolved": resolved})
...
async def _handle_steer_run(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    return web.json_response({"object": "mason.run.steer", "run_id": run_id, "accepted": True})
