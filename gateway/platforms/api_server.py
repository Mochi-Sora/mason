...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from mason_cli.profiles import profile_matches_home
...
_STATIC_FEATURE_FLAGS = {
    ...
    "session_continuity_header": "X-Mason-Session-Id",
    "session_key_header": "X-Mason-Session-Key"}
...
_BROWSER_CONTROL_WS_PROTOCOL = "mason-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "mason-browser-control-ticket."
...
def _mason_version() -> str:
    """Canonical Mason version: ``mason_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from mason_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("MASON_MAX_TOKENS")
    if env_max_tokens:
...
def _request_agent_overrides(
    ...
    The virtual model (``mason-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Mason-native
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_mason_home() / "response_store.db")
        ...
        from mason_state_wal import apply_wal_with_fallback
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Mason session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through mason-agent's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Mason-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "mason-agent"
        (precedence owned by ``mason_cli.model_switch.resolve_effective_model``)."""
        from mason_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "mason-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from mason_cli.auth import has_usable_secret
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_mason_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from mason_cli.profiles import get_profile_dir
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Mason-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Mason-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
        or ``(None, error)``. Requires API-key auth so a client can't guess another scope."""
        raw = request.headers.get("X-Mason-Session-Key", "").strip()
        if not raw:
            return None, None
        if not self._api_key:
            logger.warning(
                "X-Mason-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping.")
            return None, _error_response(
                "X-Mason-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_mason_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from mason_constants import get_mason_home
            return self._open_and_cache_session_db(get_mason_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "mason-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "mason_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "mason_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `mason auth add` without `mason model`).
        if not model and runtime_kwargs.get("provider"):
            with suppress(Exception):
                from mason_cli.models import get_default_model_for_provider
    ...
    def _select_agent_runtime(
        ...
        # mason_cli.model_switch.resolve_effective_model.
        from mason_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "mason-agent", "version": _mason_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "mason-agent",
            "version": _mason_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — mason-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "mason", "permission": [],
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from mason_cli.inventory import build_model_options_payload, load_picker_context
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "mason.api_server.capabilities", "platform": "mason-agent",
            "model": self._model_name,
            "auth": {"type": "bearer", "required": bool(self._api_key)},
            "runtime": {
                "mode": "server_agent", "tool_execution": "server", "split_runtime": False,
                "description": (
                    "The API server creates a server-side Mason AIAgent; "
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<MASON_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Mason home.
            try:
                from mason_state import get_mason_home
                root = Path(get_mason_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Mason sessions."""
        db = await self._ensure_session_db_async()
        if db is None:
        ...
        # Exact-title lookup (`mason peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `mason peer dm`
            ...
            try:
                ...
                # which would fail `mason peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Mason session row. Existence check, insert and
        ...
        # would persist "mason-agent" and later send it to the provider literally.
        ...
        return web.json_response({"object": "mason.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "mason.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "mason.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "mason.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Mason-Session-Id`` (+ ``X-Mason-Session-Key`` when declared) response headers."""
        headers = {"X-Mason-Session-Id": session_id}
        if gateway_session_key:
            headers["X-Mason-Session-Key"] = gateway_session_key
        return headers
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the MASON_HOME
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"object": "mason.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "mason.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `mason cron run` /
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_mason_api_runtime", {}) or {})
