...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (platform == "slack" and isinstance(raw_message, dict)
            and raw_message.get("_mason_no_thread_response")):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``MASON_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "mason_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def _config_section(name: str) -> dict:
    ...
    try:
        from mason_cli.config import load_config_readonly as _load_config
...
from mason_constants import get_default_mason_root, get_mason_dir, get_mason_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.mason/.env manually.")
...
IMAGE_CACHE_DIR = get_mason_dir("cache/images", "image_cache")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_mason_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_mason_dir(new_subpath, old_name)
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; MasonAgent/1.0)", "Accept": accept}
...
AUDIO_CACHE_DIR = get_mason_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_mason_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_mason_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_mason_dir("cache/screenshots", "browser_screenshots")
...
_MASON_ROOT = get_default_mason_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "MASON_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "MASON_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "MASON_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "MASON_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_MASON_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the MASON_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active MASON_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (MASON_HOME symlinked).

    ``MASON_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when MASON_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
    try:
        profile_dirs = [p for p in (_MASON_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []
...
def _kanban_root() -> Path:
    ...
    return Path(os.environ.get("MASON_KANBAN_HOME", "").strip() or _MASON_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("MASON_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    return [*map(Path, _MEDIA_DELIVERY_DENIED_PREFIXES),
            ...
            *(r / rel for r in (_MASON_HOME, _MASON_ROOT) for rel in _ROOT_CREDENTIAL_PATHS),
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.mason/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Mason cache dirs (``/root/.mason/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.mason/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `mason serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.mason/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.mason"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``MASON_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a Mason cache, an operator root (``MASON_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, Mason-root secrets).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?mason\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+mason[.!?\s]*$", re.IGNORECASE))
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``MASON_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for Mason commands ("!" where the client eats a leading "/").
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("MASON_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower() or "interrupt")
        self._busy_text_debounce_seconds: float = _float_env("MASON_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35)
        self._busy_text_hard_cap_seconds: float = _float_env("MASON_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0)
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-MASON_HOME
        ...
        remedy = (f" Stop that gateway first (mason --profile {owner_profile} gateway stop)."
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from mason_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    def _get_human_delay() -> float:
        """Random human-like pacing delay (s) from MASON_HUMAN_DELAY_MODE: "off" (default) |
        "natural" 800-2500ms | "custom" via MASON_HUMAN_DELAY_MIN_MS /
        MASON_HUMAN_DELAY_MAX_MS."""
        mode = os.getenv("MASON_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        lo, hi = 800, 2500
        if mode != "natural":  # custom mode tolerates malformed env vars
            lo = _or_default(lambda: int(os.getenv("MASON_HUMAN_DELAY_MIN_MS", str(lo))), lo)
            hi = _or_default(lambda: int(os.getenv("MASON_HUMAN_DELAY_MAX_MS", str(hi))), hi)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: MASON_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def _wants_auto_tts(self, event: MessageEvent, session_key: str, interrupt_event: asyncio.Event,
        ...
        generation = getattr(interrupt_event, "_mason_run_generation", None)
        return bool(
    ...
    async def _fire_post_delivery_callback(self, session_key: str, interrupt_event: asyncio.Event) -> None:
        ...
        _post_cb = self.pop_post_delivery_callback(
            session_key, generation=getattr(interrupt_event, "_mason_run_generation", None))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            self._streaming_tts_completed_turns.discard(self._streaming_tts_turn_key(
                session_key, getattr(interrupt_event, "_mason_run_generation", None),
                event=event) or "")
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile MASON_HOME isolation."""
