...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _send_identify(self) -> None:
        ...
        payload = {"op": 2, "d": {
            ...
            "shard": [0, 1],
            "properties": {"$os": "macOS", "$browser": "mason-agent", "$device": "mason-agent"}}}
    ...
    async def _default_interaction_dispatch(self, event: InteractionEvent) -> None:
        ...
        ``~/.mason/.update_response``; anything else is ignored at DEBUG."""
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``.update_response``, polled by the detached ``mason update --gateway`` watcher."""
        try:
    ...
    async def send_update_prompt(
        ...
        are written to ``~/.mason/.update_response`` by the interaction callback."""
