...
def _get_mason_version() -> str:
    """Return the mason-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    """``QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Mason/<mason_version>)``."""
    v = sys.version_info
    return (f"QQBotAdapter/{QQBOT_VERSION} (Python/{v.major}.{v.minor}.{v.micro}; "
            f"{platform.system().lower()}; Mason/{_get_mason_version()})")
