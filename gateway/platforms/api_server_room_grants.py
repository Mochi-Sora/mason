...
def _room_grant_token(request: "web.Request") -> str:
    ...
    return token.strip() if separator and scheme.lower() == "masonroom" else ""
...
async def _handle_room_member_invitation(
    ...
    return web.json_response({
        "object": "mason.room_member.invitation", "grant": token, "target_profile": profile,
...
async def _handle_room_member_capabilities(
    ...
    return web.json_response({
        "object": "mason.room_member.capabilities", **{k: claims[k] for k in _ROOM_IDENTITY_FIELDS},
...
async def _handle_room_member_grant_refresh(
    ...
    try:
        ...
        return _room_grant_error_response(exc, _openai_error=_openai_error)
    return web.json_response({
        "object": "mason.room_member.grant", "grant": token, "expires_at": now + dispatch_ttl,
...
async def _handle_room_member_grant_revoke(
    ...
    return web.json_response({"object": "mason.room_member.grant.revocation", "revoked": True})
