...
Override via ``MASON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY`` / ``MASON_GATEWAY_HTTPX_MAX_KEEPALIVE``.
"""
...
def platform_httpx_limits() -> "httpx.Limits | None":
    ...
    return httpx.Limits(
        max_keepalive_connections=_positive_env(
            "MASON_GATEWAY_HTTPX_MAX_KEEPALIVE", _DEFAULT_MAX_KEEPALIVE, int),
        keepalive_expiry=_positive_env(
            "MASON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", _DEFAULT_KEEPALIVE_EXPIRY_S, float),
    )
