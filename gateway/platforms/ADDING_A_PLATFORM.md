...
There are two ways to add a platform to the Mason gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.mason/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Mason code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `mason gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `mason_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Mason core.
...
"mason-your-platform": {
    ...
    "tools": _MASON_CORE_TOOLS,
    "includes": []
},
```

And add it to the `mason-gateway` composite:

```python
"mason-gateway": {
    "includes": [..., "mason-your-platform"]
}
```
...
## 12. Status Display (`mason_cli/status.py`)
...
## 13. Gateway Setup Wizard (`mason_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ mason_cli/ toolsets.py \
  --include="*.py" -l | sort -u
