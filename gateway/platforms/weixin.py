...
def _account_dir(mason_home: str) -> Path:
    path = Path(mason_home) / "weixin" / "accounts"
...
def save_weixin_account(mason_home: str, *, account_id: str, token: str, base_url: str, user_id: str = "") -> None:
    path = _account_dir(mason_home) / f"{account_id}.json"
...
def load_weixin_account(mason_home: str, account_id: str) -> Optional[Dict[str, Any]]:
    return _read_json(_account_dir(mason_home) / f"{account_id}.json")


class ContextTokenStore:
    """Disk-backed ``context_token`` cache keyed by account + peer."""

    def __init__(self, mason_home: str):
        self._root = _account_dir(mason_home)
...
def _load_sync_buf(mason_home: str, account_id: str) -> str:
    data = _read_json(_account_dir(mason_home) / f"{account_id}.sync.json")
    return data.get("get_updates_buf", "") if isinstance(data, dict) else ""


def _save_sync_buf(mason_home: str, account_id: str, sync_buf: str) -> None:
    atomic_json_write(_account_dir(mason_home) / f"{account_id}.sync.json", {"get_updates_buf": sync_buf})
...
async def qr_login(mason_home: str, *, bot_type: str = "3", timeout_seconds: int = 480) -> Optional[Dict[str, str]]:
    if not AIOHTTP_AVAILABLE:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "wait":
                ...
                if not creds["account_id"] or not creds["token"]:
                    ...
                    return None
                save_weixin_account(mason_home, **creds)
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._mason_home = mason_home = str(get_mason_home())
        self._token_store = ContextTokenStore(mason_home)
        ...
        persisted = load_weixin_account(mason_home, self._account_id) if self._account_id and not self._token else None
        if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "ordinary-group events for these accounts, so group messages may never reach Mason regardless of "
                "this policy. If group delivery doesn't work, the limitation is on the iLink side, not in Mason.",
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._mason_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if response.get("get_updates_buf"):
                    ...
                    _save_sync_buf(self._mason_home, self._account_id, sync_buf)
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"mason-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(self, chat_id: str, path: str, caption: str, force_file_attachment: bool = False) -> str:
        ...
        if caption:
            await _send_message(
                ...
                context_token=context_token, client_id=f"mason-weixin-{uuid.uuid4().hex}")
        last_message_id = f"mason-weixin-{uuid.uuid4().hex}"
        await _send_items(
...
async def send_weixin_direct(
    *, extra: Dict[str, Any], token: Optional[str], chat_id: str, message: str, media_files: Optional[List[Tuple[str, bool]]] = None,
    ...
    token_store = ContextTokenStore(str(get_mason_home()))
