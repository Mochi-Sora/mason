...
def _mason_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-Mason-Session-Key scopes long-term memory per channel; independent of
        # X-Mason-Session-Id (the key persists across transcripts, the id rotates on /new).
        gateway_session_key, key_err = self._parse_session_key_header(request)
        if key_err is not None:
            return key_err
        # X-Mason-Session-Id continues an existing session (history from state.db, not the body);
        # requires a configured API key or any client could read history by guessing ids.
        provided_session_id = request.headers.get("X-Mason-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Mason-Session-Id rejected: "
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``mason.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        response_headers = {"X-Mason-Session-Id": result.get("session_id", session_id)}
        if gateway_session_key:
            response_headers["X-Mason-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["mason"] = {
                "completed": completed, "partial": is_partial, "failed": is_failed}
            response_headers["X-Mason-Completed"] = "false"
            response_headers["X-Mason-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Mason extras.
        response_data = {
        ...
        if is_partial or is_failed or not completed:
            response_data["mason"] = _mason_extras(
                completed, is_partial, is_failed, err_msg, finish_reason)
            response_headers["X-Mason-Completed"] = "false"
            response_headers["X-Mason-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Mason-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _prepare_sse_response(
        ...
        if session_id:
            sse_headers["X-Mason-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Mason-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    await response.write(_sse_frame(delta[1], event="mason.tool.progress"))
                else:
            ...
            if finish_reason != "stop":
                ...
                finish_chunk["mason"] = _mason_extras(
                    completed, is_partial, is_failed, err_msg, finish_reason)
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Session precedence: previous_response_id chain > declared X-Mason-Session-Key > fresh
        ...
        response_headers = {"X-Mason-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Mason-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
