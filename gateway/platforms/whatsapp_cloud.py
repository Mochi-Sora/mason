...
# Under the mason dir so it survives restarts/reloads — same as the Baileys bridge.
_INBOUND_MEDIA_CACHE = Path(get_mason_dir("platforms/whatsapp_cloud/media", "whatsapp_cloud/media"))
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        for ok, code, message in (
            (check_whatsapp_cloud_requirements(), "whatsapp_cloud_deps_missing",
             "aiohttp and httpx are required for whatsapp_cloud — reinstall mason-agent."),
    ...
    async def send_voice(self, chat_id: str, audio_path: str, caption: Optional[str] = None, reply_to: Optional[str] = None, **kwargs) -> SendResult:
        ...
        local MP3 (Mason TTS output) is converted via ffmpeg first; other audio is sent as-is."""
        mime_type: Optional[str] = None
