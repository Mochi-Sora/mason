...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.mason/pairing/
"""
...
# path would freeze the MASON_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``mason pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever MASON_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# mason_constants.set_mason_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``mason pairing`` CLI) re-imports this module later with the final environment already
...
# (NousResearch/mason-agent#93449). ``_default_pairing_dir()`` below resolves this fresh on every call in
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_mason_dir("platforms/pairing", "pairing")
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from mason_cli.config import save_env_value, remove_env_value
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `mason` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> mason pairing approve ...` as root, "
            "re-run with `docker exec -u mason <container> ...` and "
            "chown the existing file to the mason user, or restart the "
...
class PairingStore:
    ...
    profile's MASON_HOME exactly as ``mason -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_mason_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
