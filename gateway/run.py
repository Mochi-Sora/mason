...
# mason_bootstrap must be the very first import (UTF-8 stdio on Windows; no-op on POSIX).
try:
    import mason_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # a partial ``mason update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
from mason_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while Mason still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "mason", "off"}:
        mode = "native"
    if mode != "mason":
    ...
    # copy_context carries profile secret scope / MASON_HOME override (executors don't propagate ContextVars).
...
def _ensure_windows_gateway_venv_imports() -> None:
    """Make detached Windows gateway runs see the Mason venv packages.
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from mason_cli.commands_platforms import _sanitize_telegram_name
...
def _coerce_gateway_timestamp(value: Any) -> Optional[float]:
    ...
    if isinstance(value, (int, float)):
        # Some platform events use milliseconds; Mason state rows use seconds.
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("MASON_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("MASON_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from mason_time import get_timezone as _get_msg_tz
...
def _restart_notification_pending() -> bool:
    ...
    return (_mason_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _mason_home / ".restart_pending.json"
...
os.environ["_MASON_GATEWAY"] = "1"

_ensure_ssl_certs()

sys.path.insert(0, str(Path(__file__).parent.parent))

from mason_constants import get_mason_home, get_mason_home_override
_mason_home = get_mason_home()

# Load ~/.mason/.env first: user-managed env files must override stale shell exports on restart.
from mason_cli.env_loader import load_mason_dotenv
_env_path = _mason_home / '.env'
load_mason_dotenv(mason_home=_mason_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env per turn for rotated keys while config.yaml stays authoritative for budgets (else a
    stale MASON_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_mason_dotenv(
            mason_home=_mason_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_mason_home)
...
def _current_max_iterations() -> int:
    ...
    from mason_cli.config import resolve_turn_limit as _resolve_turn_limit
    return _resolve_turn_limit(os.getenv("MASON_MAX_ITERATIONS"))
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from mason_constants import set_mason_home_override, reset_mason_home_override
    # Caller already hydrated external sources off-loop (#99519).
    from agent.secret_scope import build_profile_secret_scope
    from mason_cli.env_loader import hydrate_profile_secret_sources

    home_token = set_mason_home_override(str(profile_home))
    try:
        hydrate_profile_secret_sources(Path(profile_home))
        return build_profile_secret_scope(Path(profile_home))
    finally:
        reset_mason_home_override(home_token)
...
def _profile_runtime_scope(
    ...
    ``set_mason_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_mason_home()``'s config, so an unscoped call only connects the launch
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.mason/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from mason_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["MASON_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "MASON_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "MASON_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "MASON_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "MASON_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "MASON_SESSION_STALL_TIMEOUT",
    # Internal bridge only — config.yaml (agent.reconnect_attention_after) is the documented setting.
    "reconnect_attention_after": "MASON_RECONNECT_ATTENTION_AFTER_SECONDS",
    "restart_drain_timeout": "MASON_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "MASON_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "MASON_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "MASON_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "MASON_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "MASON_CJK_FTS", "search_slow_ms": "MASON_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "MASON_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "MASON_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "MASON_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["MASON_MAX_ITERATIONS"] = str(raw)
    elif "MASON_MAX_ITERATIONS" in os.environ:
        del os.environ["MASON_MAX_ITERATIONS"]
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. MASON_MAX_ITERATIONS=60 written by an old `mason setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "MASON_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["MASON_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["MASON_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["MASON_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`mason cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["MASON_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
def _load_bridge_config(config_path: Path) -> dict:
    ...
    from mason_cli.config import _expand_env_vars, read_user_config_raw
...
_config_path = _mason_home / 'config.yaml'
_cfg: dict = {}
if _config_path.exists():
    try:
        ...
        print(
            ...
            "your current config.yaml. Run `mason doctor` to investigate.",
            file=sys.stderr)

# IPv4 preference must apply before any HTTP clients are created.
try:
    from mason_constants import apply_ipv4_preference
...
try:
    from mason_cli.config import print_config_warnings
    print_config_warnings()
except Exception as _bootstrap_exc:
    print(f"  Warning: config validation failed: {_bootstrap_exc}", file=sys.stderr)

try:
    from mason_cli.config import warn_deprecated_cwd_env_vars
    warn_deprecated_cwd_env_vars()
except Exception as _bootstrap_exc:
    print(f"  Warning: deprecation check failed: {_bootstrap_exc}", file=sys.stderr)

os.environ["MASON_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# MASON_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from mason_cli.runtime_provider import (
        resolve_runtime_provider, format_runtime_provider_error, _get_model_config)
    from mason_cli.auth import AuthError, is_rate_limited_auth_error
    ...
    max_tokens = None
    _env_mt = os.environ.get("MASON_MAX_TOKENS")
    if _env_mt:
...
def _resolve_gateway_model_context(model: Optional[str] = None) -> _GatewayModelContext:
    ...
    def _read_config() -> None:
        ...
        try:
            from mason_cli.config import get_compatible_custom_providers
    ...
    def _pin_still_applies() -> bool:
        ...
        from mason_cli.route_identity import should_clear_context_pin
...
def _resolve_runtime_agent_kwargs_for_provider(provider: str) -> dict:
    ...
    from mason_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from mason_cli.config import _deep_merge
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `mason skills config`")

        # Check optional skills (shipped with repo but not installed)
        from mason_constants import get_optional_skills_dir
        ...
        if optional_dir.exists():
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                return (
                    ...
                    f"Install it with: `mason skills install {install_path}`")
    except Exception:
        pass
    return None
...
def _gateway_config_home() -> Path:
    """Return the Mason home that gateway config reads should use."""
    override = get_mason_home_override()
    return Path(override) if override else _mason_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    Defaults to the active gateway home (``_mason_home`` monkeypatches apply); multiplexers pass a path.
    """
    ...
    try:
        from mason_cli.config import get_config_path, read_raw_config
    ...
    try:
        ...
        from mason_cli.config import _normalize_root_model_keys
...
def _checkpoint_agent_kwargs(config: dict | None) -> dict:
    ...
    if isinstance(cp_cfg, bool):
        ...
        cp_cfg = {}
    from mason_cli.config import DEFAULT_CONFIG
...
def _load_gateway_runtime_config() -> dict:
    ...
    if not isinstance(cfg, dict) or not cfg:
        return {}
    from mason_cli.config import _expand_env_vars
...
def _resolve_mason_bin() -> Optional[list[str]]:
    """Mason update command argv: ``mason`` on PATH, else ``python -m mason_cli.main``, else None."""
    import shutil
    mason_bin = shutil.which("mason")
    if mason_bin:
        return [mason_bin]
    try:
        import importlib.util
        if importlib.util.find_spec("mason_cli") is not None:
            return [sys.executable, "-m", "mason_cli.main"]
    except Exception:
        pass
    return None
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("MASON_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall mason-agent"),
...
class GatewayRunner(
    ...
    def _init_startup_checks(self) -> None:
        ...
        try:
            from mason_cli.config import load_config as _load_full_config
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS MASON_HOME silently loses /resume etc.
        ...
        try:
            ...
            if _ckpt_cfg.get("auto_prune", False):
                ...
                # unmounted share); orphan cleanup is only via explicit `mason checkpoints prune`.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local MASON_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local MASON_HOME override
        ...
        #88235 broadcast.
        """
        from mason_state import AsyncSessionDB, _default_db_path
        from mason_state_registry import acquire
        from gateway.session_db_recovery import RecoverableHandleCache
        ...
        def _open():
            ...
            if borrowed is not None:
                ...
                wrapper.__dict__["_mason_borrowed_handle"] = True
                return wrapper
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        """
        def _close(db) -> None:
            if getattr(db, "__dict__", {}).get("_mason_borrowed_handle"):
                return
            ...
            # instead (#90837).
            from mason_state_registry import release_or_close
            try:
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.mason/cache/documents:/output') is configured. "
            "This is fine if the model already emits host-visible paths, but MEDIA file delivery can fail "
            "for container-local paths like '/workspace/...' or '/output/...'.")

    _VOICE_MODE_PATH = _mason_home / "gateway_voice_mode.json"
    ...
    def _thread_metadata_for_source(
        ...
        if profile and metadata is not None:
            ...
            metadata["mason_profile"] = profile
        return metadata
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="mason-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's MASON_HOME serves this source: ``source.profile``, then
        ``_profile_name_for_source`` (sources bypassing ``build_source``), then the active profile."""
        from gateway.profile_routing import ProfileRouteRejected
        from mason_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            logger.warning(
                ...
                "falling back to global MASON_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``mason gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``mason gateway stop`` signals the gateway. The
...
def _housekeeping_paste_sweep() -> None:
    from mason_cli.debug import _sweep_expired_pastes
...
def _housekeeping_auto_archive() -> None:
    ...
    from mason_cli.config import load_config as _load_full_config
    from mason_state_registry import acquire, release_or_close
    _sess_cfg = (_load_full_config().get("sessions") or {})
    if _sess_cfg.get("auto_archive", False):
        _adb = acquire()
        try:
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from mason_state_registry import live_shared_session_dbs
...
def _housekeeping_memory_trim() -> None:
    ...
    from mason_cli.mem_trim import trim_memory
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_mason_home()):
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no MASON_HOME), it is only a
    ...
    # (no service supervisor — e.g. `mason gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_mason_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_mason_home)
        our_home = _get_process_mason_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("mason_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates mason_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_mason_home(recorded_home, our_home):
            return refuse("pid record belongs to a different MASON_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / MASON_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "MASON_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """MASON_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "MASON_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--mason-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this MASON_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        mason_home = str(get_mason_home())
        logger.error(
            "Another gateway instance is already running (PID %d, MASON_HOME=%s). "
            "Use 'mason gateway restart' to replace it, or 'mason gateway stop' first.",
            existing_pid, mason_home)
        print(
            f"\n❌ Gateway already running (PID {existing_pid}).\n"
            f"   Use 'mason gateway restart' to replace it,\n"
            f"   or 'mason gateway stop' to kill it first.\n"
            f"   Or use 'mason gateway run --replace' to auto-replace.\n")
        return False
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_mason_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (MASON_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_mason_home())
        return False
    ...
    with suppress(Exception):
        (get_mason_home() / "gateway.pid").unlink(missing_ok=True)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from mason_logging import setup_logging, _safe_stderr
    setup_logging(mason_home=_mason_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from mason_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from mason_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(mason_home=_mason_home, config=_best_effort(_raw_cfg))
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _mason_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its MASON_HOME, so from here on "does a socket answer?" is
        ...
        def _pause_for_update_handler() -> dict:
            try:
                from mason_cli.gateway import _get_restart_drain_timeout
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`mason gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from mason_cli.nous_auth_keepalive import stop_nous_auth_keepalive
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to MASON_HOME; distinct-home multi-profile setups coexist.
    ...
    # Windows has no add_signal_handler, so `mason gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `mason gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `mason
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``MASON_IGNORE_USER_CONFIG=1``) as ``mason_cli/main.py``."""
    from mason_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in mason_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``mason-agent`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from mason_startup_watchdog import arm_startup_watchdog
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Mason Gateway - Multi-platform messaging")
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from mason_logging import drain_log_queue
