...
become a bypass of the host denylist. Strict mode (``MASON_MEDIA_DELIVERY_STRICT``) keeps its
...
# Credential dirs under the sandbox home plus the Mason stores, which live at ``~/.mason`` there.
_DENIED_HOME_RELATIVE = tuple(PurePosixPath(s) for s in _MEDIA_DELIVERY_DENIED_HOME_SUBPATHS) + tuple(
    PurePosixPath(".mason", *PurePosixPath(rel.replace(os.sep, "/")).parts) for rel in _ROOT_CREDENTIAL_PATHS)
...
def _active_remote_env():
    ...
    if backend not in _REMOTE_TERMINAL_BACKENDS and not _plugin_backend_is_remote(backend):
        return None
    return get_active_env(get_session_env("MASON_SESSION_ID") or get_session_env("MASON_SESSION_KEY") or "default")
