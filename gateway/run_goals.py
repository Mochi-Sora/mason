...
class GatewayGoalsMixin:
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        top-level ``goals`` block is only reachable via mason_cli.config.load_config()."""
        try:
            goals_cfg = (
    ...
    async def _warm_goals_session_db(self, label: str) -> None:
        ...
        try:
            from mason_cli.goals import _get_session_db as _warm_goals_db
    ...
    async def _defer_goal_status_notice_after_delivery(self, source: Any, message: str) -> None:
        ...
        if session_key and hasattr(adapter, "register_post_delivery_callback"):
            try:
                ...
                generation = getattr(active, "_mason_run_generation", None) if active is not None else None
    ...
    async def _loop_wakeup_fire_one(self, sid: str, state: Any, now: float, warned_no_route: set) -> None:
        ...
        from mason_cli.loops import LoopManager, goal_blocks_loop_tick
        ...
        # fired. The context-preserving executor keeps the profile MASON_HOME override under multiplex.
    ...
    async def _loop_wakeup_watcher(self, interval: float = 15.0) -> None:
        ...
        while self._running:
            try:
                from mason_cli.loops import list_active_loops
