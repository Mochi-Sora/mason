...
class GatewayAdapterLifecycleMixin:
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        override = self._env_timeout_override("MASON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT")
    ...
    def _platform_connect_timeout_secs(self, platform=None, *, initial: bool = False) -> float:
        ...
        override = self._env_timeout_override("MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT")
    ...
    def _spawn_supervised(
        ...
        task._mason_supervised_watcher = True  # type: ignore[attr-defined]
    ...
    async def _handoff_watcher(self, interval: float = 2.0, drain_timeout: float = 30.0) -> None:
        ...
        async def _tick(profile_name: Optional[str] = None) -> None:
            ...
            for row in pending:
                ...
                # ``set_mason_home_override`` and ``set_secret_scope`` are ContextVar-based — ensure_future
    ...
    def _flag_reconnect_needs_attention(self, platform, info: dict, now: float) -> None:
        ...
        logger.warning(
            ...
            "credentials, missing intents, broken sidecar). Check `mason status` / `/platform list`.",
    ...
    async def _start_secondary_profile_adapters(self) -> int:
        ...
        Each profile connects under its own MASON_HOME + secret scope; credential/listener collisions
    ...
    def _record_served_profiles(self, active: str, profile_homes) -> None:
        ...
        than "has a connected adapter") for `mason status`; seed per-profile PairingStores."""
    ...
    def _configure_profile_adapter(
        ...
        # See #76423.
        adapter._mason_profile_name = profile_name
    ...
    def _make_default_profile_message_handler(self):
        ...
        from gateway.run import _async_profile_runtime_scope, get_mason_home
        default_home = Path(get_mason_home())
    ...
    def _make_default_profile_platform_event_handler(self):
        ...
        from gateway.run import _profile_runtime_scope, get_mason_home
        default_home = Path(get_mason_home())
    ...
    def _adapter_credential_fingerprint(adapter: Any) -> Optional[str]:
        ...
        for obj, attr in candidates:
            ...
            if isinstance(val, str) and val.strip():
                import hashlib
                return hashlib.sha256(("mason-mux:" + val.strip()).encode("utf-8")).hexdigest()[:16]
        return None
    ...
    def _make_adapter_auth_check(
        ...
        """
        from gateway.run import get_mason_home
        transport_home = Path(get_mason_home()) if self._multiplex_on() and profile_name is None else None
