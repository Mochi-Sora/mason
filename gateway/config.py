...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from mason_cli.profiles import normalize_profile_name, validate_profile_name
...
# of truth for gateway/run.py and mason_cli/web_server.py validation).
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_mason_home() / "sessions")
    ...
    # route); MASON_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_mason_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.mason/config.yaml > legacy gateway.json > defaults."""
    from gateway import config_loader

    _home = get_mason_home()
