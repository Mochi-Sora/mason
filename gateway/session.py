...
def build_session_context_prompt(context: SessionContext, *, redact_pii: bool = False) -> str:
    ...
    lines.append(f"- `\"local\"` → Save to local files only ({display_mason_home()}/cron/output/)")
    for platform, home in context.home_channels.items():
...
def build_channel_continuity_note(entry: "SessionEntry", source: SessionSource) -> Optional[str]:
    ...
    return (
        f"[System note: This {where} had an earlier Mason session (session_id: {prev}) that was "
...
class SessionStore(
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig, has_active_processes_fn=None):
        ...
        # redirected ``get_mason_home()`` for the turn (its docstring lists "sessions" among what it
        ...
        self._profile_home_cache: Dict[str, Optional[Path]] = {}  # profile -> MASON_HOME (hits)
        ...
        try:
            ...
            self._routing_home: Optional[Path] = Path(get_mason_home())
        except Exception:
