...
All hooks write atomic JSON payloads under ``<mason_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active MASON_HOME."""
    from mason_constants import get_mason_home
    flush_dir = get_mason_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<mason_home>/pending_messages/``), so a runtime cap rotation no longer silently
