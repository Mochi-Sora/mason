...
(or removes) ``{MASON_HOME}/.drain_request.json`` and a gateway watcher reacts;
an ACTIVE marker means ``gateway_state -> "draining"``.  Two lenient staleness
signals (either suffices): epoch mismatch (MASON_HOME is a durable volume on
Mason Cloud, so a marker survives the restart a drain-gated action ends in and
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting MASON_HOME."""
    return Path(home if home is not None else get_mason_home()) / _DRAIN_REQUEST_FILENAME
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    survived a container/VM restart (MASON_HOME is a durable Fly volume on Mason Cloud) and the lifecycle
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    machine restart on the durable MASON_HOME volume, or an expired same-epoch orphan (#85433), is ignored
