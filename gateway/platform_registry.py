...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``mason setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``mason gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MASON_HOME and overlay the
        ...
        # `mason` invocation, so the import happens only when a lookup asks for it.
