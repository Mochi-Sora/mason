...
class GatewayInboundMixin:
    ...
    async def _hm_offer_pairing_code(self, source: SessionSource) -> None:
        ...
        if code:
            ...
            reply = (
                ...
                f"`mason {profile_arg}pairing approve "
    ...
    async def _hm_admit_event(
        ...
        # spawning context, which may carry ANOTHER message's MASON_SESSION_* ContextVars; until
        ...
        if (
            ...
            and not is_internal
            and not getattr(event, "_mason_startup_restore_replay", False)
        ):
    ...
    def _hm_estop_turn_allowed(self, event: "MessageEvent", source: SessionSource) -> bool:
        ...
        with suppress(Exception):
            ...
            if _estop_cmd:
                from mason_cli.commands import resolve_command as _resolve_estop_cmd
    ...
    def _hm_estop_gate(
        ...
        """Global emergency-stop (`mason pause`) notice when this turn must be blocked, else None.
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        response_path = _mason_home / ".update_response"
        try:
            tmp = response_path.with_suffix(".tmp")
            tmp.write_text(response_text, encoding="utf-8")
            tmp.replace(response_path)
            (_mason_home / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    def _hm_update_prompt_reply(self, event: "MessageEvent", _quick_key: str) -> Optional[str]:
        ...
        if cmd in {"approve", "yes"}:
            ...
            if cmd:
                with suppress(Exception):
                    from mason_cli.commands import resolve_command as _resolve_update_cmd
    ...
    def _hm_evict_idle_stale_agent(self, _quick_key: str) -> None:
        ...
        _raw_stale_timeout = _float_env("MASON_AGENT_TIMEOUT", 1800)
    ...
    async def _hm_busy_slash_or_photo(
        ...
        from mason_cli.commands import resolve_command as _resolve_cmd_inner
    ...
    def _hm_busy_telegram_grace_queue(
        ...
        _grace = float(os.getenv("MASON_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0"))
    ...
    async def _hm_resolve_command(
        ...
        from mason_cli.commands import is_gateway_known_command, resolve_command as _resolve_cmd
    ...
    async def _hm_cmd_egress(self, event, source, _quick_key):
        from mason_cli.proxy_cli import format_status_text
    ...
    async def _hm_dispatch_quick_and_plugin_commands(
        ...
        if command:
            try:
                from mason_cli.plugins import get_plugin_command_handler
    ...
    def _hm_unknown_slash_reply(command: str, source: SessionSource) -> Optional[str]:
        ...
        from mason_cli.commands import GATEWAY_KNOWN_COMMANDS
            ...
            if _plat and (_skill_name or extra_keys):
                ...
                if _disabled_extra:
                    return (
                        ...
                        f"Enable them with: `mason skills config`"
                    )
    ...
    async def _dispatch_plugin_message_injection(
        ...
        await adapter.handle_message(MessageEvent(
            ...
            metadata={
                "mason_plugin_id": plugin_id, "mason_plugin_injection": True,
