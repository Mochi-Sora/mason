...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``mason update --gateway`` and its watcher exchange under MASON_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_mason_home / ".update_pending.json",
            claimed=_mason_home / ".update_pending.claimed.json", output=_mason_home / ".update_output.txt",
            exit_code=_mason_home / ".update_exit_code",
            prompt=_mason_home / ".update_prompt.json", response=_mason_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``mason update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Mason update finished." if exit_code == 0
                        else "❌ Mason update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Mason update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Mason update finished." if exit_code == 0 else "❌ Mason update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Mason update finished successfully." if exit_code == 0 else
                        "❌ Mason update failed. Check the gateway logs or run `mason update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _mason_home, _non_conversational_metadata
        notify_path = _mason_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Mason is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if not error:
            return
        from mason_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / MASON_HOME do not live under ~/.mason).
            db_path = _default_db_path()
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                "1. Run `mason doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   mason sessions recover --source {db_path} "
                "--inspect-only\n"
                "   (if it reports recoverable) mason sessions recover "
                ...
                "3. Restore from a backup in ~/.mason/backups/\n"
                "Run `mason doctor` for sanitized diagnostics."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `mason doctor` for diagnostics."
            )
    ...
    async def _inject_watch_notification(self, synth_text: str, evt: dict) -> Optional[bool]:
        ...
        if not source:
            # API-server sessions bind the RAW X-Mason-Session-Id key, not a structured ``agent:...`` key.
