...
``<MASON_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _process_mason_home() -> Path:
    """MASON_HOME for process-level identity files (ignore profile overrides)."""
    val = os.environ.get("MASON_HOME", "").strip()
    return Path(val) if val else get_mason_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_mason_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<MASON_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
