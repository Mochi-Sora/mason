...
# exit (bounded), then spawns ``mason gateway restart``.
...
class GatewayShutdownMixin:
    ...
    def _scale_to_zero_has_live_background_work(self) -> bool:
        ...
        PERMANENT supervised watchers (_mason_supervised_watcher, incl. the scale-to-zero watcher
        itself) are excluded, else this would be True forever and the gateway could never go dormant.
        """
        if any(
            not t.done() and not getattr(t, "_mason_supervised_watcher", False)
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "resume %s` to retry, or `mason gateway restart` to restart the gateway.",
    ...
    async def _finalize_session_off_loop(self, *, session_id: Any, platform: str, reason: str, **extra: Any) -> None:
        """Run mason_cli.lifecycle.finalize_session off-loop, bounded; on timeout the worker is left alone."""
    ...
    def _stuck_loop_counts_path(self) -> Path:
        ...
        return _mason_home / self._STUCK_LOOP_FILE
    ...
    def _restart_watcher_env() -> dict:
        """Watcher env minus ``_MASON_GATEWAY`` (else the CLI's self-restart guard refuses; gateway stays down)."""
        from tools.environments.local import build_subprocess_env
        watcher_env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
        watcher_env.pop("_MASON_GATEWAY", None)
        return watcher_env

    @staticmethod
    def _spawn_windows_restart_watcher(mason_cmd: list, current_pid: int, restart_after_s: float) -> None:
        ...
        # it owns one hidden console, inherited by the `mason gateway restart` child, so nothing flashes.
        # See #54220, #56747.
        ...
        watcher_argv = [
            ...
            str(current_pid), str(restart_after_s), *mason_cmd, "gateway", "restart",
        ]
    ...
    async def _launch_detached_restart_command(self) -> None:
        from gateway.run import _resolve_mason_bin
        import shutil
        import subprocess
        mason_cmd = _resolve_mason_bin()
        if not mason_cmd:
            logger.error("Could not locate mason binary for detached /restart")
            return
        ...
        if sys.platform == "win32":
            GatewayShutdownMixin._spawn_windows_restart_watcher(mason_cmd, current_pid, restart_after_s)
            return
        cmd = " ".join(shlex.quote(part) for part in mason_cmd)
        shell_cmd = (
    ...
    def _wedged_agent_count(self) -> int:
        ...
        timeout = _float_env("MASON_AGENT_TIMEOUT", 1800)
        if timeout <= 0:
            return 0
    ...
    def _start_systemd_watchdog(self) -> bool:
        ...
        watchdog.ready("Mason Gateway running")
        return True
    ...
    def _stop_quiesce_and_close_session_dbs(self, timeout: float, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        def _close_shared() -> None:
            ...
            # (#90837).
            from mason_state_registry import close_all
            closed = close_all()
            if closed:
    ...
    def _stop_persist_exit_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        from gateway.run import _mason_home, _planned_restart_notification_path, _shutdown_gateway_health_export
        from utils import atomic_json_write
        ...
        if not ctx.timed_out:
            with suppress(Exception):
                (_mason_home / ".clean_shutdown").touch()
        else:
            logger.info(
