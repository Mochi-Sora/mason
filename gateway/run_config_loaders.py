...
from mason_cli.config import cfg_get, resolve_ephemeral_system_prompt_from_config
...
class GatewayConfigLoadersMixin:
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        MASON_PREFILL_MESSAGES_FILE env wins, then top-level prefill_messages_file in config.yaml,
        then legacy agent.prefill_messages_file. Relative paths resolve from ~/.mason/.
        """
        from gateway.run import _mason_home, _load_gateway_runtime_config
        file_path = os.getenv("MASON_PREFILL_MESSAGES_FILE", "")
        if not file_path:
    ...
    def _load_ephemeral_system_prompt() -> str:
        """MASON_EPHEMERAL_SYSTEM_PROMPT env first, then ``display.personality`` / ``agent.system_prompt``."""
        from gateway.run import _load_gateway_runtime_config
        prompt = os.getenv("MASON_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
    ...
    def _resolve_model_for_channel(
        ...
        Precedence lives in :func:`mason_cli.model_switch.resolve_effective_model` (shared with the
    ...
    def _load_reasoning_config(model: str = "") -> dict | None:
        """Reasoning effort from config.yaml via :func:`mason_constants.resolve_reasoning_config`.
    ...
    def _load_busy_input_mode(cls) -> str:
        ...
        mode = cls._env_or_cfg_str("MASON_GATEWAY_BUSY_INPUT_MODE", "display", "busy_input_mode").lower()
    ...
    def _load_busy_text_mode(cls) -> str:
        ...
        legacy = cls._env_or_cfg_str("MASON_GATEWAY_BUSY_TEXT_MODE", "display", "busy_text_mode").lower()
    ...
    def _load_restart_drain_timeout(cls) -> float:
        ...
        raw = cls._env_or_cfg_str("MASON_RESTART_DRAIN_TIMEOUT", "agent", "restart_drain_timeout")
    ...
    def _load_restart_after_turn_timeout(cls) -> float:
        ...
        return cls._load_env_or_agent_cfg_timeout(
            "MASON_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout",
    ...
    def _load_cron_drain_timeout(cls) -> float:
        ...
        """
        return cls._load_env_or_agent_cfg_timeout(
            "MASON_CRON_DRAIN_TIMEOUT", "cron_drain_timeout",
    ...
    def _load_background_notifications_mode() -> str:
        ...
        from gateway.run import _load_gateway_runtime_config
        mode = os.getenv("MASON_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
    ...
    def _refresh_fallback_model(self) -> list | None:
        ...
        ``self._fallback_model`` at process start, so a chain configured (or changed) after ``mason
        ...
        try:
            ...
            cfg_path = _mason_home / "config.yaml"
