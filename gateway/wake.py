...
matches the raw ``X-Mason-Session-Id`` real turns use (invisible parallel session), so we self-POST
...
async def deliver_wake(adapter: Any, *, text: str, session_id: str = "", source: Any = None) -> None:
    ...
    (``X-Mason-Session-Id`` / state.db key) — required for non-push adapters. ``source`` is the
...
async def _self_post_chat_completion(adapter: Any, *, text: str, session_id: str) -> None:
    ...
    own bind host/port/key. Session continuation via ``X-Mason-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError("wake self-post requires API_SERVER_KEY: session continuation via "
                           "X-Mason-Session-Id is rejected (403) on an unauthenticated API "
                           "server, so the wake cannot reach the target session")
    if ":" in host and not host.startswith("["):
        host = f"[{host}]"  # bare IPv6 literal
    url = f"http://{host}:{port}/v1/chat/completions"
    headers = {"Authorization": f"Bearer {api_key}", "X-Mason-Session-Id": session_id}
    payload = {"model": str(getattr(adapter, "_model_name", "") or "mason-agent"),
