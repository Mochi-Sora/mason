...
class GatewayGoalCommandsMixin:
    ...
    def _goal_gate(self, mgr, gate_arg: str, event: MessageEvent) -> str:
        ...
        if gate_lower.startswith("add "):
            ...
            # choose that string is authenticated RCE under the Mason process account — and with
    ...
    async def _handle_heartbeat_command(self, event: MessageEvent) -> str:
        ...
        from mason_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        args = (event.get_command_args() or "").strip()
        lower = args.lower()
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `mason cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: MessageEvent) -> str:
        ...
        try:
            from mason_cli.loops import LoopManager, dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
