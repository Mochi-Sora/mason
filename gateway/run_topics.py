...
def _collapse_title(title: str) -> str:
    return re.sub(r"\s+", " ", str(title or "")).strip() or "Mason Chat"
...
class GatewayTopicThreadsMixin:
    ...
    def _telegram_topic_root_lobby_message(self) -> str:
        return (
            ...
            "To start a new Mason chat, open the All Messages topic at the top "
            "of this bot interface and send any message there. Telegram will "
            "create a new topic for that message; each topic works as an "
            "independent Mason session."
        )

    def _telegram_topic_root_new_message(self) -> str:
        return (
            "To start a new parallel Mason chat, open the All Messages topic "
            "at the top of this bot interface and send any message there. "
            "Telegram will create a new topic for it.\n\n"
            "Each topic is an independent Mason session. Use /new inside an "
            "existing topic only if you want to replace that topic's current session."
        )

    def _telegram_topic_new_header(self, source: SessionSource) -> Optional[str]:
        return (
            "Started a new Mason session in this topic.\n\n"
    ...
    def _telegram_topic_help_text(self) -> str:
        return (
            ...
            "\n"
            "How it works:\n"
            "1. Run /topic once in this DM — Mason checks BotFather Threads\n"
            "   Settings are enabled and flips on multi-session mode.\n"
            "2. Tap All Messages at the top of the bot and send any message.\n"
            "   Telegram creates a new topic for that message; each topic is\n"
            "   an independent Mason session (fresh history, fresh context).\n"
    ...
    def _record_telegram_topic_binding(self, source: SessionSource, session_entry) -> None:
        """Persist the Telegram topic -> Mason session binding for topic lanes (off-loop)."""
    ...
    def _sync_telegram_topic_binding(self, source: SessionSource, session_entry, *, reason: str) -> None:
        ...
        fresh process resumes the right Mason session. See #20470, #29712, #33414.
        """
    ...
    async def _ensure_telegram_system_topic(self, source: SessionSource) -> None:
        ...
        try:
            send_result = await adapter.send(
                source.chat_id, "System topic for Mason commands and status.", metadata={"thread_id": str(thread_id)},
            )
    ...
    def _is_discord_auto_thread_lane(self, source: SessionSource) -> bool:
        """Return True only for Discord threads Mason just auto-created."""
        return (
    ...
    async def _await_relay_auto_thread_info(self, source: SessionSource) -> Optional[Tuple[str, str]]:
        ...
        timeout = _float_env("MASON_AGENT_TIMEOUT", 1800) or 1800
        with suppress(Exception):
    ...
    async def _rename_telegram_topic_for_session_title(self, source: SessionSource, session_id: str, title: str) -> None:
        """Best-effort rename of a Telegram DM topic when Mason auto-titles a session."""
    ...
    async def _disable_telegram_topic_mode_for_chat(self, source: SessionSource) -> str:
        ...
        return (
            ...
            "normal Mason chat again. Run /topic to re-enable later."
        )
    ...
    async def _telegram_topic_root_status_message(self, source: SessionSource) -> str:
        lines = [
            ...
            "",
            "To create a new Mason chat, open All Messages at the top of this "
    ...
    async def _restore_telegram_topic_session(self, event: MessageEvent, raw_session_id: str) -> str:
        """Restore an existing Telegram-owned Mason session into this topic."""
        source = event.source
        db = self._session_db
