...
_SESSIONS_JSON_README = (
    ...
    "table in ~/.mason/state.db). Maps messaging session keys (agent:main:<platform>:...) to "
    "active session IDs. This is NOT the session list. ALL sessions (CLI, TUI, and gateway) live "
    "in ~/.mason/state.db and are shown by `mason sessions list` and `/sessions`. Disable this "
...
class SessionPersistenceMixin:
    ...
    def _open_session_db_for_active_scope(self, db_path: Optional[Path] = None):
        ...
        ``_default_db_path()`` follows the context-local MASON_HOME (resolved per call so
    ...
    def _profile_home_for_key(self, session_key: Optional[str]) -> Optional[Path]:
        """MASON_HOME of the profile owning *session_key*, or None (no named owner or
        unresolvable)."""
        ...
        try:
            from mason_cli.profiles import get_profile_dir, profile_exists
    ...
    def _db_for_key(self, session_key: Optional[str]):
        ...
        is encoded in the key). ``_db`` follows the ambient MASON_HOME that only the inbound message
    ...
    def close_all_db_handles(self) -> None:
        ...
        def _close(db) -> None:
            from mason_state_registry import release_or_close  # shared instances no-op on close()
            try:
                release_or_close(db)
            except Exception as exc:
