...
``gateway.<key>`` form (what ``mason config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``mason config set
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    # mason_cli.config.load_config, so a managed session_reset / quick_commands / stt would otherwise be ignored).
