...
``gateway/assets/status_phrases.yaml``; users add profile-relative catalogs under ``MASON_HOME``
via ``status_phrases.yaml`` / ``status_phrases/*.yaml`` or ``display.status_phrases: {path:
<MASON_HOME-relative>, mode: append|replace}``. Absolute paths and ``..`` escapes are ignored on
...
# Mason UI surfaces, not app/vendor buckets.  Long-running-only: regular tool/thinking/interim
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None,
    ...
    _merge_phrase_paths(catalog, list(_CONVENTIONAL_RELATIVE_PATHS), base_dir=mason_home)
    ...
    for section in sections:
        for key in _CONFIG_KEYS:
            _merge_phrase_config(catalog, section.get(key), base_dir=mason_home)
    return catalog


def classify_status_context(kind: str, *, tool_name: str | None = None, preview: str | None = None,
                            args: Any = None) -> str:
    """Classify an internal gateway event into a Mason UI-surface bucket."""
