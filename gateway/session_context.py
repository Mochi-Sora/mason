"""Session-scoped context variables for the Mason gateway.

Replaces the old ``os.environ``-based ``MASON_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "MASON_SESSION_PLATFORM", "MASON_SESSION_SOURCE", "MASON_SESSION_CHAT_ID",
    "MASON_SESSION_CHAT_TYPE", "MASON_SESSION_CHAT_NAME", "MASON_SESSION_THREAD_ID",
    "MASON_SESSION_USER_ID", "MASON_SESSION_USER_ID_ALT", "MASON_SESSION_USER_NAME",
    "MASON_SESSION_SCOPE_ID", "MASON_SESSION_KEY", "MASON_SESSION_ID",
    "MASON_UI_SESSION_ID", "MASON_SESSION_MESSAGE_ID", "MASON_SESSION_PROFILE",
    "MASON_BROWSER_CONTROL_PRINCIPAL", "MASON_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "MASON_CRON_SESSION",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("MASON_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("MASON_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("MASON_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("MASON_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``MASON_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``MASON_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds MASON_SESSION_PLATFORM, CLI/TUI/
# desktop bind MASON_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``MASON_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("MASON_PLATFORM") or get_session_env("MASON_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("MASON_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See NousResearch/mason-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``MASON_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
