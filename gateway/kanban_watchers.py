...
class GatewayKanbanWatchersMixin:
    ...
    def _kanban_dispatcher_boot(self) -> Optional[tuple]:
        ...
        try:
            ...
            return None
        env_override = os.environ.get("MASON_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via MASON_KANBAN_DISPATCH_IN_GATEWAY env")
            return None
        try:
            cfg = _load_config()
        except Exception as exc:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        loop exits and an external `mason kanban daemon` is expected. Each
        ...
        while self._running:
            ...
            try:
                # Emergency stop (`mason pause`): no auto-decompose or
                ...
                if bad_ticks >= _HEALTH_WINDOW and now - last_warn_at >= 300:
                    logger.warning(
                        ...
                        "`mason kanban list --status ready`.",
                        bad_ticks,
                    )
                    last_warn_at = now
