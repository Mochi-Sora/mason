...
``<MASON_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_mason_home() -> Path:
    """MASON_HOME for process-level identity files (ignore task overrides)."""
...
def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_mason_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<MASON_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `mason doctor`.",
            verdict,
        )
