"""Relay/connector support package for the Mason gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from mason_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Mason:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``mason gateway enroll``.
    ...
    if not token_url:
        from mason_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    ``~/.mason/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'mason'}"
