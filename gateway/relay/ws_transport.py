...
def _normalize_slack_parent_command(text: str, message_type: MessageType) -> tuple[str, MessageType]:
    """Mirror native Slack ``/mason`` routing for authenticated relay text."""
    parent_parts = text.strip().split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/mason":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # Multiplex mode: the connector stamps the target Mason profile; None on
    ...
    if platform_enum == Platform.SLACK:
        ...
        # gateway command rather than the legacy `mason` parent name.
