...
from mason_cli.config import atomic_config_write, clear_model_endpoint_credentials
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: mason gateway restart"
        ),
    )
...
async def _persist_model_switch_to_config(result, config_path) -> None:
    ...
    """
    from mason_cli.config import read_user_config_raw, save_config
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from mason_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _record_model_switch(
        ...
        from mason_cli.model_switch import format_model_for_display
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from mason_cli.model_switch import format_model_for_display, resolve_display_context_length_async
    ...
    async def _send_model_picker(self, event: MessageEvent, source, adapter, session_key: str, listing_kwargs: dict, on_model_selected) -> bool:
        ...
        from mason_cli.model_switch_providers import list_picker_providers
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (mason_state.py:1994-2009)
            ...
            source=source,
            config_path=(profile_home or _mason_home) / "config.yaml",
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (mason_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
