...
Built on gateway startup, refreshed every 5 min, saved to ~/.mason/channel_directory.json.
...
def _directory_path() -> Path:
    return DIRECTORY_PATH or get_mason_home() / "channel_directory.json"


def _aliases_path() -> Path:
    return CHANNEL_ALIASES_PATH or get_mason_home() / "channel_aliases.json"
...
def _build_from_sessions_json(platform_name: str) -> List[Dict[str, str]]:
    ...
    sessions_path = get_mason_home() / "sessions" / "sessions.json"
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``platforms`` overrides the on-disk directory (``mason send --list`` merges in
