"""Gateway runtime status helpers: PID/lock/marker files under ``{MASON_HOME}`` (one set per
...
from mason_constants import _get_platform_default_mason_home, get_mason_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_mason_home() / "gateway-starts.log"
...
def _get_process_mason_home() -> Path:
    """Launch-home MASON_HOME for identity files (PID, lock, status, markers):
    ``get_mason_home()`` honors the per-session ``_MASON_HOME_OVERRIDE`` and would misroute
    them."""
    val = os.environ.get("MASON_HOME", "").strip()
    return Path(val) if val else _get_platform_default_mason_home()


def _canonical_mason_home(path: Path | str) -> Path:
    """Stable absolute MASON_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_mason_home(left: Path | str, right: Path | str) -> bool:
    """Compare MASON_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_mason_home(left)))
    return left_c == os.path.normcase(str(_canonical_mason_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT MASON_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``mason_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("mason_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_mason_home()
        return not _same_mason_home(recorded_home, base)
    except Exception:
        return True


# Mirrors mason_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (mason_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_mason_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import mason_constants
    default_homes = (mason_constants.get_default_mason_root, _get_platform_default_mason_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_mason_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_mason_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``MASON_GATEWAY_LOCK_DIR`` overrides."""
    override = os.getenv("MASON_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "mason" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Mason gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    Mason entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("mason-gateway", "mason-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "mason_cli.main" not in joined and "mason_cli/main.py" not in joined and not any(
        b in ("mason", "mason.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `mason gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    Mason-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the Mason gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``mason_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``MASON_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return any(needle in command_lc for needle in (
            f"--profile {profile_lc}", f"-p {profile_lc}", f"mason_home={home_lc}"
        ))
    # Default profile: accept unless argv names another profile or a conflicting explicit
    # MASON_HOME= (its absence is not disqualifying -- MASON_HOME usually arrives via the env).
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    return not ("mason_home=" in command_lc and f"mason_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "mason_home": str(_canonical_mason_home(_get_process_mason_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``mason_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `mason update` (and the dashboard) prove whether a running gateway actually
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``mason_home`` matches the current process (legacy records: True);
    another MASON_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("mason_home")
    return not record_home or _same_mason_home(record_home, _get_process_mason_home())
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's mason_home must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to mason_home.
    profile = _profile_label_for_home(_get_process_mason_home())
    if profile:
...
# token. ``mason.service`` + ``mason- gateway.service``). See #5646.
...
def _get_takeover_marker_path(mason_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``mason_home`` is given only for a verified cross-home handoff."""
    home = _canonical_mason_home(mason_home or _get_process_mason_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_mason_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``mason gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-MASON_HOME --replace while ignoring a marker accidentally written into another
    # profile's directory. Legacy markers have no target field: keep the same-replacer-home rule.
    # See #29092.
    our_home = _get_process_mason_home()
    target_home = record.get("target_mason_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_mason_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_mason_home")
        if replacer_home is not None and not _same_mason_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_mason_home(target_home or _get_process_mason_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_mason_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_mason_home": str(_canonical_mason_home(_get_process_mason_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("mason_home")
    if (
        ...
        return None
    target_home = _canonical_mason_home(raw_home)
    ...
    pid_record_home = pid_record.get("mason_home")
    if (
        ...
        or not _same_mason_home(pid_record_home, target_home)
    ):
        return None
