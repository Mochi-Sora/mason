...
from mason_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while Mason is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while Mason is busy."),
}
...
def _spawn_detached_update(mason_cmd, output_path, exit_code_path) -> None:
    """Spawn ``mason update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\mason.exe — that shim holds its own file open, and the
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "mason_cli.main", "update", "--gateway"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **windows_detach_popen_kwargs())
        return
    mason_cmd_str = " ".join(shlex.quote(part) for part in mason_cmd)
    update_cmd = (
        f"PYTHONUNBUFFERED=1 {mason_cmd_str} update --gateway"
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach Mason commands (adapter ``typed_command_prefix``,
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `mason gateway restart` to reset."
        if not queued:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _mason_home
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _mason_home / name, build(), indent=None)
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Mason version."""
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `mason skills diff <name>`, which diffs a bundled skill against its stock version).
        if sub == "diff" and len(out) > 3000:
            pending_id = args[1] if len(args) > 1 else "<id>"
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.mason/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Mason is working."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_mason_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return ("No skill bundles installed.\nCreate one on the host with:\n"
                    "  `mason bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``mason debug share`` from the CLI does full uploads."""
        from mason_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_mason_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``mason update`` detached (``setsid``) so it survives the gateway
        restart it may trigger; marker files let this or the next gateway process notify the user."""
        import json
        from gateway.run import _mason_home, _resolve_mason_bin
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update Mason')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        mason_cmd = _resolve_mason_bin()
        if not mason_cmd:
            return t("gateway.update.mason_cmd_not_found")
        pending_path = _mason_home / ".update_pending.json"
        output_path = _mason_home / ".update_output.txt"
        exit_code_path = _mason_home / ".update_exit_code"
        pending = {
        ...
        try:
            _spawn_detached_update(mason_cmd, output_path, exit_code_path)
        except Exception as e:
