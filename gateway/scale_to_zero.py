...
SCALE_TO_ZERO_ENV = "MASON_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(mason_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under MASON_HOME."""
    if mason_home is None:
        from mason_constants import get_mason_home
        mason_home = get_mason_home()
    return Path(mason_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
