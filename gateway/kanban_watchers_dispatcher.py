...
def _kbc():
    from mason_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from mason_cli import kanban_db_dispatch
...
class _KanbanDispatcher:
        ...
        try:
            ...
            if self.is_corrupt_board_db_error(exc):
                ...
                logger.error(
                    ...
                    "then run `mason kanban init` if you need a fresh board.",
                    slug, fingerprint[0],
                )
                return None
    ...
    def auto_decompose_tick(self, auto_decompose_per_tick: int) -> int:
        ...
        """
        try:
            from mason_cli import kanban_decompose as _decomp
        ...
        for slug in self._board_slugs():
            ...
            prev_env = os.environ.get("MASON_KANBAN_BOARD")
            try:
                os.environ["MASON_KANBAN_BOARD"] = slug
                try:
                ...
                if prev_env is None:
                    ...
                    os.environ["MASON_KANBAN_BOARD"] = prev_env
        return successes
