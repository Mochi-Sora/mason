...
def active_session_limit_message(
    ...
    return (
        f"Mason is at the active session limit ({active_count}/{max_sessions})."
...
def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_mason_home())
...
class ActiveSessionLease:
    ...
    enabled: bool = True
    released: bool = False
    # Pinned at acquisition: a lease taken under the root MASON_HOME must release
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "Mason could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile MASON_HOME override.
    # See #85431.
