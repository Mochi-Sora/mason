...
def format_nous_portal_entitlement_message(
    ...
    if account_info is None:
        return (
            f"Mason could not verify your Nous Portal entitlement, so {capability} is unavailable. "
            f"Run `mason model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Mason cannot verify your Nous Portal "
                f"paid access for {capability}. Log in with `mason model` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `mason model`. "
            f"Billing and credits are managed at {billing_url}."
        )
    if account_info.paid_service_access is None:
        detail = f"Mason could not verify your Nous Portal paid access, so {capability} is unavailable."
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `mason model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Mason could not find a Nous Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `mason model` to authenticate again; if the problem persists, contact Nous support."
        )
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `mason model` to refresh Mason."
        return message
    return (
...
def _no_paid_access_message(account_info: NousPortalAccountInfo, capability: str, billing_url: str) -> str:
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f"member spend cap at {billing_url}, then run `mason model` to refresh."
        )
    if active and paid:
        return (
...
def nous_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from mason_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NousPortalAccountInfo:
    ...
    try:
        from mason_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    try:
        from mason_cli.auth import _decode_jwt_claims
    except Exception:
        return None
