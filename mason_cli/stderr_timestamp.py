...
def _is_mason_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Mason ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Mason gateway shapes get ``--external-
...
def _prepare_child_command(command: Sequence[str], environ: Mapping[str, str] | None = None) -> list[str]:
    ...
    plists put ``--external-supervisor`` on the inner ``gateway run`` so ``mason update`` can see
    the flag on the live process argv.
    """
    argv = [str(part) for part in command]
    env = os.environ if environ is None else environ
    xpc_service = str(env.get("XPC_SERVICE_NAME", "")).strip()
    if EXTERNAL_SUPERVISOR_FLAG not in argv and xpc_service and xpc_service != "0" and _is_mason_gateway_run_argv(argv):
