...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``mason_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``mason_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from mason_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (mason_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``mason acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (mason_constants is stdlib-only)."""
    # mason_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Mason root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("mason", "mason-acp")
...
def _default_mason_root() -> Path | None:
    """The DEFAULT Mason root (not ``get_mason_home()``, which under ``mason -p <name>`` is
    ...
    try:
        return Path(get_default_mason_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``mason`` launchers when they vanish.

    On Windows, ``mason`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Mason root's ``bin``
    (``%LOCALAPPDATA%\\mason\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_mason_home()``
    would point inside ``profiles\\<name>`` under ``mason -p``, so the anchor here is
    :func:`mason_constants.get_default_mason_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_mason_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every mason_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored mason launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``MASON_HOME\\bin`` launcher layout (``mason update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ mason launchers now resolve from {home_bin} "
...
def _quarantine_running_mason_exe(
    ...
    """Rename live mason*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``mason acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_mason_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `mason` gone from PATH. Restore only renames back when the
