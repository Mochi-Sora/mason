...
# to the checkout (NOT in MASON_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from mason_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    Update-time clears can't close the stale-bytecode class: ``mason update`` runs
    ...
    pull inside ``mason update``, a manual ``git pull``, a ZIP update, a file-sync restore) while
    ...
    """
    from mason_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``mason_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "mason_cli" / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Content hash, NOT mtime: ``git checkout`` / ``mason update`` rewrite source
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $MASON_HOME."""
    from mason_constants import get_mason_home
    return get_mason_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``mason`` launcher present but ``mason_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the mason
...
def _run_npm_install_deterministic(
    ...
    run_env["PATH"] = with_mason_node_path(run_env)["PATH"]
...
def _web_npm_install_context(web_dir: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    closure as ``mason update``'s ``_update_node_dependencies()`` (ui-tui + web +
    ...
    # When running from the workspace root, this must name the SAME closure as `mason update`'s
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (mason web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    soft warning (used by ``mason web``). Returns True when the build succeeded
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_mason_node_path())
