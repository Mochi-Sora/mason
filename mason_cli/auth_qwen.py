...
def _read_qwen_cli_tokens() -> Dict[str, Any]:
    from mason_cli.auth import _qwen_cli_auth_path
...
def _save_qwen_cli_tokens(tokens: Dict[str, Any]) -> Path:
    from mason_cli.auth import _qwen_cli_auth_path, _write_private_file_atomic
    auth_path = _qwen_cli_auth_path()
...
def _mark_qwen_oauth_active(creds: Dict[str, Any]) -> None:
    ...
    from mason_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store, _save_provider_state
...
def resolve_qwen_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from mason_cli.auth import _qwen_cli_auth_path, _refresh_qwen_cli_tokens
    ...
    return {
        "provider": "qwen-oauth",
        "base_url": os.getenv("MASON_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    from mason_cli.auth import _qwen_cli_auth_path, resolve_qwen_runtime_credentials
    auth_path = _qwen_cli_auth_path()
    try:
        # Refresh-validate: otherwise stale CLI tokens read as "logged in" and break `mason model`.
