...
from mason_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from mason_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from mason_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.mason/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_mason_home() / "logs"
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from mason_cli.web_server import PROJECT_ROOT
...
def _spawn_mason_action(
    subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]] = None
) -> subprocess.Popen:
    """Spawn ``mason <subcommand>`` detached (via ``mason_cli.main``) and record the handle."""
    from mason_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "mason_cli.main", *subcommand]
    # The dashboard runs inside the gateway process, so os.environ carries _MASON_GATEWAY=1;
    ...
    action_env = {**os.environ, "MASON_NONINTERACTIVE": "1"}
    action_env.pop("_MASON_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("MASON_ACTION_ID")
    if action_id:
...
def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    from mason_cli.web_server_profiles import _profile_cli_args
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from mason_cli.web_server import _spawn_gateway_restart
    try:
