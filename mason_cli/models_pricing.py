...
DeepInfra pricing adapters. Split out of ``mason_cli.models``; helpers still defined there are
looked up on ``mason_cli.models`` at call time so ``patch("mason_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from mason_cli.models_reasoning_caps import _seed_reasoning_caps
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the mason names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    headers = {"Accept": "application/json", "User-Agent": _MASON_USER_AGENT}
    if api_key:
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    try:
        from mason_cli.nous_account import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from mason_cli.models import _pricing_profile_key
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from mason_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from mason_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _MASON_USER_AGENT}
