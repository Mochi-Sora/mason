"""Profile distributions — shareable, packaged Mason profiles via git.
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            mason_requires=_str(data, "mason_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("mason_requires", self.mason_requires),
...
def check_mason_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Mason {op}{target}, but you have {current_version}.")


def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    """Generate a ``.env.template`` body from env_requires."""
    lines = [
        "# Environment variables required by this Mason distribution.",
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Mason profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from mason_cli.profiles import _canon_valid, get_profile_dir
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Mason distribution."
        )
    check_mason_requires(manifest.mason_requires, mason_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.mason).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from mason_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from mason_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="mason_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `mason profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from mason_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`mason profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="mason_dist_update_") as tmp:
