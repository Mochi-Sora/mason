...
_TASK_NAME_DEFAULT = "Mason_Gateway"
_TASK_DESCRIPTION = "Mason Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("MASON_GATEWAY_DETACHED", "1"), ("MASON_SUPERVISED_CHILD", "1"))
...
def _mason_home() -> Path:
    from mason_cli.config import get_mason_home

    return Path(get_mason_home())


def _preserve_mason_home_path(path: str | Path) -> str:
    r"""Render Mason-owned paths under the configured MASON_HOME spelling.

    ``%LOCALAPPDATA%\mason`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under MASON_HOME.
    """
    candidate = Path(path)
    try:
        home = _mason_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Mason profile."""
    from mason_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "mason_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"MASON_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "MASON_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "MASON_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from mason_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<MASON_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _mason_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at MASON_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m mason_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "mason_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, mason_home, profile_arg) for generated launchers."""
    from mason_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    mason_home = str(_mason_home())
    return (
        _preserve_mason_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        mason_home,
        _profile_arg(mason_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_mason_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_mason_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, mason_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "MASON_HOME={mason_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_mason_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, mason_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('MASON_HOME')}) = {q(mason_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_mason_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from mason_cli.gateway import PROJECT_ROOT

    python_path, working_dir, mason_home, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"MASON_HOME": mason_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_mason_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_mason_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (MASON_HOME,
    ...
    try:
        mason_home = str(_mason_home().resolve())
    except Exception:
        mason_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "MASON_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if mason_home:
        env_overlay["MASON_HOME"] = mason_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("MASON_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("MASON_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from mason_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"mason {profile_arg} gateway start" if profile_arg else "mason gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Mason gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: mason gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: mason gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: mason gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path() -> Path:
    return _mason_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_mason_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
...
def check_start_attestation(current_pids: list[int] | None = None) -> str | None:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `mason gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_mason_home()}\\logs\\gateway.log\n    type {_mason_home()}\\logs\\gateway-stdio.log")
        _print_task_run_hint("  Recovery: schtasks /Run /TN {}   (starts the gateway outside any Job Object)")


def _print_next_steps() -> None:
    print("\nNext steps:\n  mason gateway status                      # Check status")
    print(f"  type {_mason_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Mason gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: mason gateway status")
                    return
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: mason gateway install")
            return
        install(force=False)
        if not is_task_registered() and not is_startup_entry_installed():
            print("⚠ Gateway install did not complete in this process.")
            print("  If a UAC prompt opened, approve it, then run: mason gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `mason gateway status`."
        )
