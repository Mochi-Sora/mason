...
def _root() -> Path:
    ...
    # MASON_HOME) cannot move the reader away from the helper's token.
    return get_default_mason_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, mason_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(mason_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is the mason exe or (normally) the base Python, whose path varies by venv/uv
    ...
    launches_module = (
        argv[1:3] == ["-m", "mason_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "mason_cli.main" in argv[2]))
...
def terminate_owned(pid: int, creation_time_ns: int, mason_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, mason_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    process that both owns the port and is the one we lock. mason_cli's parent is prepended
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the mason_cli package")
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["masonPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Mason path must be absolute")
    mason_path = os.path.abspath(configured_path)
    token_path = str(_token_path(ownership_id, spawn_nonce))
    profile = str(payload.get("profile") or "")
    if len(profile) > 256 or any(ch in profile for ch in "\x00\r\n"):
        raise ValueError("invalid profile")
    venv_dir = os.path.dirname(mason_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Mason Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('mason_cli.main',run_name='__main__',alter_sys=True)")
...
def inspect_mason(mason_path: str) -> dict[str, Any]:
    path = os.path.abspath(mason_path)
    if not os.path.isabs(mason_path) or not os.path.isfile(path):
        raise ValueError("Mason path is not an executable file")
...
def _probe(*_: str) -> dict[str, Any]:
    import platform
    return {"os": "Windows", "arch": platform.machine(), "masonHome": str(get_default_mason_root()), "python": sys.executable}
