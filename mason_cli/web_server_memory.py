...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (mason_home / f"{name}.json", mason_home / name / "config.json"):
    ...
    # Holographic stores under plugins.mason-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("mason-memory-store")
