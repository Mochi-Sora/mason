"""API connectivity probes for ``mason doctor`` (split out of ``doctor.py``).
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from mason_cli.providers import normalize_provider as _normalize_provider
...
_OPENROUTER_STATUS = {
    ...
    402: ("(out of credits — payment required)",
          ...
          "Fix: run 'mason config set model.provider <provider>' "
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _MASON_USER_AGENT}
