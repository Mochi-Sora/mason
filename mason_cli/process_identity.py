...
Three layers make every long-lived Mason process positively identifiable, so reapers (``mason
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``MASON_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from mason_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``MASON_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``MASON_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_mason_root()) / LEDGER_FILENAME
    except Exception:
        from mason_cli.config import get_mason_home

        return Path(get_mason_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `mason serve --host X --port N --profile P` while bounding
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's MASON_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("MASON_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("MASON_PARENT_START_MARKER", "")
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import Mason code). Records the child's
