...
_MANAGED_FILES_ROOT_ENV = "MASON_DASHBOARD_FILES_ROOT"
...
def _default_mason_root_is_opt_data() -> bool:
    raw = os.environ.get("MASON_HOME", "").strip()
    if not raw:
        return False
    try:
        from mason_constants import get_default_mason_root

        root = get_default_mason_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``mason update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the mason-webui image), where
    ...
    if _default_mason_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Mason
    # root actually IS /opt/data or MASON_DASHBOARD_FILES_ROOT is set.
    if _default_mason_root_is_opt_data():
