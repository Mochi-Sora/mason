...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: Mason-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_mason_oauth_credentials, _get_mason_oauth_file
        mason_creds = read_mason_oauth_credentials()
    except Exception:
        mason_creds = None
    if mason_creds and mason_creds.get("accessToken"):
        return _token_status("mason_pkce", f"Mason PKCE ({_get_mason_oauth_file()})", mason_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Mason can't read, so the unverified
    state reads "managed by the Copilot CLI" — never signed out."""
    try:
        from mason_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``MASON_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
        from mason_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "nous", "name": "Nous Portal", "flow": "device_code", "cli_command": "mason auth add nous",
     "docs_url": "https://portal.nousresearch.com", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "mason auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "mason auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "mason auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     "cli_command": "mason auth add xai-oauth",
     "docs_url": "https://mason-agent.nousresearch.com/docs/guides/xai-grok-oauth", "status_fn": None},
    ...
    # Login works via the terminal (`mason auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "mason auth add anthropic",
...
def _nous_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    from mason_cli.web_server_profiles import _profile_scope
    from mason_cli.auth import _poll_for_token, persist_nous_credentials, refresh_nous_oauth_from_state
    import httpx
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    via ``_minimax_save_auth_state`` so the system ends up as after ``mason auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `mason auth add xai-oauth`: first credential may become active; never overwrite.
