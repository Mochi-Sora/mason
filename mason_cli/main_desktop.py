"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (mason gui/desktop).
...
from mason_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $MASON_HOME."""
    from mason_constants import get_mason_home
    return get_mason_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Mason.app/Contents/MacOS/Mason → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Mason.app/Contents/MacOS/Mason"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Mason.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("mason", "Mason")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Mason.exe to the launcher. Prefer
        ...
        # wrong-architecture Mason.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Mason.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``mason update`` used to report
...
# (Desktop → mason-setup --update → `mason update` → `mason desktop --build-only` → relaunch) rebuilds
# Mason.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Mason.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Mason.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 mason-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Mason.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Mason.exe from backup.")
        print("    Your existing version was kept and still works. Run `mason desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `mason desktop --force-build` to rebuild, or re-run the Mason")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from mason_constants import find_node_executable, with_mason_node_path
    ...
    dl_env = with_mason_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Mason is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Mason.app/Contents/MacOS/Mason  ->  app bundle = .../Mason.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="mason-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:masonlocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Mason Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.mason"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Mason Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Mason Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Mason Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``mason uninstall --gui`` removes it.
    """
    from mason_cli.main import PROJECT_ROOT
    try:
        from mason_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from mason_cli.main import PROJECT_ROOT
    from mason_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_mason_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Mason.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Mason.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Mason.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Mason.exe, close any")
            print("  running Mason desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> mason desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from mason_constants import with_mason_node_path
    # with_mason_node_path() copies os.environ when called with no arg.
    env = with_mason_node_path()
    for attr, key in (
        ("fake_boot", "MASON_DESKTOP_BOOT_FAKE"), ("ignore_existing", "MASON_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "mason_root", None):
        env["MASON_DESKTOP_MASON_ROOT"] = str(Path(args.mason_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["MASON_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "MASON_DESKTOP_DISABLE_GPU" not in os.environ:
        env["MASON_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "MASON_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["MASON_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from mason_cli.main import PROJECT_ROOT
    from mason_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Mason Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  mason gui")
            sys.exit(1)
    ...
    if source_mode:
        print("→ Launching Mason Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged Mason Desktop: {' '.join(launch_command)}")
