...
Split out of ``mason_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from mason_cli.kanban_db import Task
...
# ``MASON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Mason profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``mason kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``mason_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from mason_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Mason profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Mason ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``MASON_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``mason -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``mason kanban tail`` shows why the task looks stuck.
...
def _module_mason_argv() -> list[str]:
    """Interpreter-bound Mason CLI invocation (``mason_cli.main`` is the
    console-script target — there is no top-level ``mason`` package)."""
    return [sys.executable, "-m", "mason_cli.main"]


def _absolute_mason_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Mason shim."""
...
def _mason_path_argv(path: str) -> list[str]:
    """argv for a resolved Mason executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_mason_argv()
    return [_absolute_mason_path(path)]


def _resolve_mason_argv() -> list[str]:
    """Resolve the ``mason`` invocation as argv for ``Popen``: ``$MASON_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("mason")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    mason_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_mason_bin``; local because
    ``mason_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("MASON_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _mason_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _mason_path_argv(resolved_env_bin)
        return _module_mason_argv()

    mason_bin = _safe_which_no_cwd("mason") if _kb._IS_WINDOWS else shutil.which("mason")
    if mason_bin:
        return _mason_path_argv(mason_bin)
    return _module_mason_argv()
...
def _resolve_worker_cli_toolsets(mason_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``MASON_KANBAN_TASK`` is set.
    """
    if not mason_home:
        return None
    try:
        ...
        token = set_mason_home_override(mason_home)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for MASON_HOME=%r (%s)",
            mason_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, mason_home: Optional[str]) -> list[str]:
    """Build the ``mason -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_mason_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped MASON_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(mason_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `mason kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``mason -p <profile> chat -q ...`` subprocess.
    ...
    ``MASON_KANBAN_DB`` / ``MASON_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from mason_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject MASON_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_mason_home() falls back to the DEFAULT
    # profile root because `mason -p` applies its override before
    # mason_constants is imported.
    try:
        env["MASON_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # MASON_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["MASON_TENANT"] = task.tenant
    env["MASON_KANBAN_TASK"] = task.id
    env["MASON_KANBAN_WORKSPACE"] = workspace
    ...
    env["MASON_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["MASON_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["MASON_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["MASON_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["MASON_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["MASON_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `mason -p` rewrites MASON_HOME (symlink / Docker layouts).
    env["MASON_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["MASON_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["MASON_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["MASON_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads MASON_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["MASON_PROFILE"] = profile_arg
    # `--cli` is the highest-precedence TUI override; dropping MASON_TUI covers
    # older mason builds on PATH that predate the flag's precedence.
    env.pop("MASON_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("MASON_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`mason` executable not found on PATH. "
            "Install Mason Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``mason kanban dispatch`` — the standalone
...
from mason_cli import kanban_db as _kb  # noqa: E402
from mason_cli import kanban_db_connect as _kbc  # noqa: E402
from mason_cli import kanban_db_workspace as _kbw  # noqa: E402
