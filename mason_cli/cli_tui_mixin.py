...
interactive CLI. Mixin bound onto ``MasonCLI`` via the MRO; cli.py-internal symbols are imported
...
from mason_cli.commands_completion import SlashCommandAutoSuggest, SlashCommandCompleter
...
class CLITuiMixin:
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from mason_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from mason_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import MasonCLI, _panel_box_width, _wrap_panel_text
        ...
        scroll_offset, visible = MasonCLI._compute_model_picker_viewport(
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        mason_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_handle_ctrl_z(self, event):
        ...
        agent_name = get_active_skin().get_branding("agent_name", "Mason Agent")
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _mason_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_mason_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
        ...
        try:
            ...
            if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                ...
                mark_seen(_mason_home / "config.yaml", BUSY_INPUT_FLAG)
    ...
    def _tui_enter_overlay(self, event) -> bool:
        ...
        if self._model_picker_state:
            try:
                ...
                from mason_cli.model_switch import resolve_persist_behavior
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.mason/pastes and return the placeholder for the buffer."""
        from cli import _mason_home, datetime, logger
        self._tui_paste_counter += 1
        paste_dir = _mason_home / "pastes"
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from mason_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_init_run_state(self):
        ...
        from mason_cli.config import get_config_path as _get_config_path
    ...
    def _tui_set_base_style(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#888888 italic',
            'status-bar': 'bg:#0b0f1a #94a3b8',
            'status-bar-strong': 'bg:#0b0f1a #22d3ee bold',
            'status-bar-dim': 'bg:#0b0f1a #334155',
            'status-bar-good': 'bg:#0b0f1a #34d399 bold',
            'status-bar-warn': 'bg:#0b0f1a #22d3ee bold',
            'status-bar-bad': 'bg:#0b0f1a #f87171 bold',
            'status-bar-critical': 'bg:#0b0f1a #ef4444 bold',
            'status-bar-yolo': 'bg:#0b0f1a #FF4444 bold',
            'status-bar-session-title': 'bg:#22d3ee #0b0f1a bold',
            'input-rule': '#6366f1',
            'image-badge': '#87CEEB bold',
            'completion-menu': 'bg:#0b0f1a #e0f2fe',
            'completion-menu.completion': 'bg:#0b0f1a #e0f2fe',
            'completion-menu.completion.current': 'bg:#1e293b #22d3ee',
            'completion-menu.meta.completion': 'bg:#0b0f1a #888888',
            'completion-menu.meta.completion.current': 'bg:#1e293b #38bdf8',
            'clarify-border': '#6366f1',
            'clarify-title': '#22d3ee bold',
            'clarify-question': '#e0f2fe bold',
            'clarify-choice': '#AAAAAA',
            'clarify-selected': '#22d3ee bold',
            'clarify-active-other': '#22d3ee italic',
            'clarify-answer': '#98FB98',
            'clarify-countdown': '#6366f1',
            'sudo-prompt': '#ef4444 bold',
            'sudo-border': '#6366f1',
            'sudo-title': '#ef4444 bold',
            'sudo-text': '#e0f2fe',
            'approval-border': '#6366f1',
            'approval-title': '#f87171 bold',
            'approval-desc': '#e0f2fe bold',
            'approval-cmd': '#AAAAAA italic',
            'approval-choice': '#AAAAAA',
            'approval-selected': '#22d3ee bold',
            'voice-prompt': '#87CEEB',
            'voice-recording': '#FF4444 bold',
            'voice-processing': '#FFA500 italic',
            'voice-status': 'bg:#0b0f1a #87CEEB',
            'voice-status-recording': 'bg:#0b0f1a #FF4444 bold'}
