"""``mason logs`` — view and filter Mason log files.

``mason logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``mason logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_mason_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Mason runs — try 'mason chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_mason_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_mason_home()}/logs/")
        return

    print(f"Log files in {display_mason_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'mason chat' to generate logs)")
