...
from mason_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from mason_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.mason. ``custom`` (MASON_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, MASON_HOME)."""
    from mason_cli import profiles as profiles_mod
...
def _cron_store_scope(home: Path):
    """Point MASON_HOME and the cron.jobs store at one profile's home for the block.
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_mason_root()
        _log.info(
    ...
    try:
        ...
        # expansion) — never a raw yaml.safe_load (tests/mason_cli/test_config_read_guard.py).
        from mason_constants import reset_mason_home_override, set_mason_home_override
        token = set_mason_home_override(str(listener_home))
        try:
