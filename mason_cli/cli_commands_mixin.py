"""Slash-command handlers for the interactive CLI (``MasonCLI`` inherits ``CLICommandsMixin``).
...
from mason_constants import display_mason_home, is_termux as _is_termux_environment
from agent.turn_context import extract_api_content_sidecar
from mason_cli.browser_connect import (
...
class _TTYBuf(StringIO):
    """StringIO that claims to be a TTY so ``mason_cli.colors.color()`` still emits ANSI escapes."""
...
# /busy mode -> what Enter does while Mason is working (status line / post-set explanation).
_BUSY_MODE_SHORT = {
    "queue": "queues for next turn", "steer": "steers into current run (after next tool call)",
    "interrupt": "redirects current run immediately"}
_BUSY_MODE_LONG = {
    "queue": "Enter will queue follow-up input while Mason is busy.",
    "steer": "Enter will steer your message into the current run (after the next tool call).",
    "interrupt": "Enter will redirect the current run while Mason is busy; /stop still cancels it.",
}
...
# /bg AIAgent provider-routing kwargs -> MasonCLI attribute carrying the value.
_BG_PROVIDER_KWARGS = {
...
def _db_unavailable_line() -> str:
    from mason_state import format_session_db_unavailable
...
def _browser_connect(cli, cdp_url: str) -> None:
    ...
    if hasattr(cli, '_pending_input'):
        cli._pending_input.put(
            ...
            "just because CDP is connected. This is typically a Mason-managed isolated debug "
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        mgr = self._checkpoint_manager((
            "  Checkpoints are not enabled.", "  Enable with: mason --checkpoints",
    ...
    def _handle_diff_command(self, command: str):
        ...
        cwd; ``session`` is everything Mason changed since the checkpoint baseline."""
        stat_only = False
        mode = "working"
        paths: list[str] = []
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            return print("  No changes — Mason hasn't edited any files here yet.")
        if stat:
    ...
    def _snapshot_list(self, parts) -> None:
        from mason_cli.backup import list_quick_snapshots
        snaps = list_quick_snapshots()
        if not snaps:
            return _pr("  No state snapshots yet.", "  Create one: /snapshot create [label]")
        print(f"  State snapshots ({display_mason_home()}/state-snapshots/):\n")
    ...
    def _snapshot_restore(self, parts) -> None:
        from mason_cli.backup import list_quick_snapshots, restore_quick_snapshot
        if len(parts) < 3:
    ...
    def _snapshot_prune(self, parts) -> None:
        from mason_cli.backup import prune_quick_snapshots
        keep = 20
        if len(parts) > 2:
            try:
                keep = int(parts[2])
    ...
    def _handle_export_command(self, command: str):
        ...
        from mason_cli.profiles import export_profile, get_active_profile_name, get_profile_export_path
        ...
        try:
            ...
            _pr(f"  ✓ Exported '{name}' to {result}",
                "  Share it: the other user runs /import or `mason profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        new profile."""
        from mason_cli.profiles import check_alias_collision, create_wrapper_script, import_profile
        ...
        print(f"  Use it: mason -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `mason journey`). Read-only views render
        ...
        from mason_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from mason_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from mason_cli.clipboard import is_remote_shell_session, write_clipboard_text
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            tip = f'Tip: type your next message, or run mason chat -q --image {example} "What do you see?"'
            _cp(_dim_line(tip))
    ...
    def _run_tools_config(self, **ns) -> None:
        ...
        from mason_cli.tools_config import tools_disable_enable_command
    ...
    def _handle_profile_command(self):
        ...
        from mason_cli.slash_exec import CommandContext, execute_command
    ...
    def _handoff_wait(self, platform_name: str, session_title: str) -> bool:
        ...
        try:  # pending timed out: CAS-clear so the user can retry
            ...
            pass
        return self._handoff_keep(
            "  Timed out waiting for the gateway. Is `mason gateway` running?",
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            return _cp("  Tip:   Use /history or `mason sessions list` to find sessions.")
        ...
        # /sessions <id>, which delegates here) lands in the same directory as a startup `mason
    ...
    def _resolve_resume_target(self, target: str):
        ...
        if target.isdigit():
            ...
            from mason_cli.main import _resolve_session_by_name_or_id
            target_id = _resolve_session_by_name_or_id(target) or target
        session_meta = self._session_db.get_session(target_id)
        if not session_meta:
            return _cp(f"  Session not found: {target}",
                       "  Use /sessions or `mason sessions list` to see available sessions.")
        try:
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=new_session_id, source=os.environ.get("MASON_SESSION_SOURCE", "cli"),
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        ``new`` moves this session into the tree (as ``mason -w``: kept on exit only with
    ...
    def _worktree_prune(self, repo_root: str, rest: str) -> None:
        import cli as _cli
        from mason_cli import worktree_gc
        rest = rest.lower()
    ...
    def _worktree_new(self, repo_root: str, rest: str) -> None:
        ...
        # Retarget the session's terminal/file tools at the new tree (as `mason -w` does).
        try:
            os.chdir(wt_info["path"])
        except OSError as e:
            print(f"  ⚠ Created worktree but could not enter it: {e}")
        os.environ["TERMINAL_CWD"] = wt_info["path"]
        # Same keep-if-unpushed cleanup as `mason -w`. Only one tree is "active" per process;
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        persistence goes through mason_cli.personality, the single owner of personality state."""
        from mason_cli.personality import (
        ...
        if not name:
            ...
            try:
                from mason_cli.config import cfg_get, read_raw_config
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from mason_cli.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        try:
            from mason_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from mason_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        """Handle /curator — delegates to mason_cli.curator so the CLI and the `mason curator`
        subcommand share the same handler set."""
        tokens = shlex.split(cmd)[1:] if cmd else []
        try:
            from mason_cli.curator import cli_main
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        from mason_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to mason_cli.skills_hub, after intercepting the
        ...
        if args and args[0].lower() in review_words:
            from mason_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                return print(out)
        from mason_cli.skills_hub import handle_skills_slash
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        inspects the workspace read-only and saves the plan under ``.mason/plans/``."""
    ...
    def _handle_init_command(self, cmd: str):
        ...
        from mason_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        """In-session ``/bundles`` — show installed skill bundles (``mason bundles list`` rendered
        ...
        if not bundles:
            return _cp("  No skill bundles installed.",
                       _dim_line('Create one with: mason bundles create <name> --skill <s1> --skill <s2>'),
        ...
        _cp("\n" + _dim_line("Invoke a bundle with /<slug>. Manage with `mason bundles`."))
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        Session-scoped and in-process — use `mason cron` for durable schedules."""
        from mason_cli.heartbeat import format_interval
        arg = _command_arg(cmd)
        lower = arg.lower()
    ...
    def _heartbeat_set(self, mgr, arg: str) -> None:
        ...
        from mason_cli.heartbeat import parse_interval, format_interval
        ...
        if interval < 0:
            from mason_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cp(f"  ♥ Heartbeat set (every {format_interval(state.interval_seconds)}): {state.prompt}",
            _dim_line("Fires as a normal turn whenever the session is idle and the interval has "
                      ...
                      "Mason process runs — use `mason cron` for durable schedules."))
    ...
    def _goal_set(self, mgr, arg: str) -> None:
        ...
        from mason_cli.goals import parse_contract
        ...
        _cp(_dim_line(f"After each turn, a judge model checks if the goal is done{against}. "
                      "Mason keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from mason_cli.goals import draft_contract
        mgr = self._session_manager(self._get_goal_manager, "Goals")
        if mgr is None:
            return
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        if mgr is None:
            return
        from mason_cli.loops import dispatch_loop_command
        result = dispatch_loop_command(mgr, arg)
        for line in (result.get("output") or "").splitlines():
            _cp(f"  {line}")
        if result.get("created"):
            with suppress(Exception):
                from mason_cli.loops import goal_blocks_loop_tick
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from mason_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if not new_skin:  # show current skin and list available
            ...
            return _pr("\n  Usage: /skin <name>",
                       f"  Custom skins: drop a YAML file in {display_mason_home()}/skins/\n")
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="mason_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from mason_cli.colors import Colors as _Colors
        from mason_cli.focus_view import (
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        from mason_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from mason_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from mason_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from mason_cli.approval_mode import run_approval_mode_command
        parts = (cmd_original or "").strip().split(None, 1)
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy [status|queue|steer|interrupt] — what Enter does while Mason is working."""
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        from mason_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        anthropic = _probe("mason_cli.models", "_is_anthropic_fast_model", None, model)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — exit the session and relaunch as ``mason update``. Returns True when
        ...
        from mason_cli.config import is_managed, format_managed_message
        if is_managed():
            print(f"  ✗ {format_managed_message('update Mason Agent')}")
            return False
        # prompt_toolkit-native modal: renders above the composer, no raw input() races.
        choices = [("once", "Update Now", "exit the current session and update Mason Agent"),
                   ("cancel", "Cancel", "keep the current session")]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Mason Agent",
            detail="This will exit the current session and run `mason update`.", choices=choices)
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Mason' hotword listener. The toggle IS the
