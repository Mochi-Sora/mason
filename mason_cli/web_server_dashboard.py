...
from mason_cli.config import cfg_get, get_process_mason_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``mason_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from mason_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Mason Teal until
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="mason-theme-bootstrap">'
                ":root{"
...
_HEADLESS_MSG = (
    "Headless backend (mason serve): web UI disabled — use "
    "`mason dashboard` for the browser UI."
)
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /mason``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__MASON_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from mason_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from mason_cli.web_deps import _server

    # `mason serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("MASON_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__MASON_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `mason update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__MASON_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__MASON_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `mason
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__MASON_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__MASON_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__MASON_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__MASON_BASE_PATH__="{prefix}";'
            f"window.__MASON_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"</script>"
        )
        if prefix:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Mason Teal",         "description": "Classic dark teal — the canonical Mason look"},
    {"name": "default-large", "label": "Mason Teal (Large)", "description": "Mason Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.mason/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_mason_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``MASON_HOME``.
    """
    themes_dir = get_process_mason_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``MASON_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the mason root — so the default
    ...
    # context-local MASON_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``MASON_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the mason root (``~/.mason/plugins``). Scan the default root as well (``get_default_mason_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``MASON_HOME`` unchanged when it
    # *is* the root), mirroring how ``mason_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any) authoritative over same-named root
    # plugins.
    user_plugin_roots = [get_process_mason_home() / "plugins"]
    root_plugins = get_default_mason_root() / "plugins"
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``mason_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("MASON_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".mason" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``mason_cli.plugins``."""
    plugins = []
    seen_names: set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from mason_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from mason_cli.web_server import _get_dashboard_plugins
    from mason_cli.config import get_mason_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from mason_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_mason_home() / "plugins").resolve()
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.mason/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.mason/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from mason_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"mason_dashboard_plugin_{plugin['name']}"
