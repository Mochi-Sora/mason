"""Image-authored deployment provenance for immutable Mason runtimes.

The image bakes ``/etc/mason/image-provenance.json`` outside ``$MASON_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/mason/image-provenance.json")
