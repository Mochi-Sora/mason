...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from mason_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`mason_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `mason model` to reactivate. "
                "Showing the saved model only."
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Mason device flow or Claude Code
    ...
    try:
        ...
        readers = (read_mason_oauth_credentials, read_claude_code_credentials)
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Mason overlay declares it keyless."""
    try:
        from mason_cli.providers import MASON_OVERLAYS
        overlay = MASON_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `mason model` to configure ({auth_type})")
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from mason_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from mason_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from mason_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (mason_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="mason-picker-pricing-prewarm", daemon=True)
