...
Imports of mason_cli.auth / config / models stay lazy (tests patch them at call time).
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from mason_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    from mason_cli.config import save_env_value
    from mason_cli.models import _PROVIDER_MODELS
    mantle_base_url = f"https://bedrock-mantle.{region}.api.aws/v1"

    # Check env var and credential pool (keys added via `mason auth`)
