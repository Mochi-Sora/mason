...
def resolve_pack_plugins(pack: PluginPack) -> List[ResolvedPackPlugin]:
    ...
    for entry in pack.plugins:
        if entry.install_identifier is not None:
            ...
            continue
        try:
            from mason_cli.plugin_index import load_index, resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print("[dim]Install them manually, e.g. `mason skills install <id>`.[/dim]")
    console.print(
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-mason-pack") -> tuple[str, List[str]]:
    ...
    import yaml
    from mason_cli.plugins_cmd import _get_enabled_set, _plugins_dir, _read_install_metadata
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `mason plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def _load_and_review(console, source: str):
    ...
    from mason_cli.plugins_cmd import _fail
    try:
...
def cmd_pack_show(source: str) -> None:
    """``mason plugins pack show <path-or-url>`` — dry-run review."""
    from mason_cli.plugins_cmd import _console
    console = _console()
    ...
    console.print("\n[dim]Dry run only. Install with `mason plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``mason plugins pack install <path-or-url>``: mandatory review screen -> one pack-level
    consent -> pinned fan-out installs -> per-plugin capability consent. Exit 1 if any failed."""
    from mason_cli.plugins_cmd import _ask_yes, _console, _fail, _is_tty
    console = _console()
    pack, resolved = _load_and_review(console, source)
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-mason-pack") -> None:
    """``mason plugins pack export [--enabled-only]`` — pack YAML on stdout."""
...
def pack_command(args) -> None:
    """Dispatch ``mason plugins pack <action>``."""
    handler = _PACK_ACTIONS.get(getattr(args, "pack_action", None))
    if handler is None:
        from mason_cli.plugins_cmd import _console, _fail
        _fail(_console(), "[red]Error:[/red] Usage: mason plugins pack {install|export|show}")
    handler(args)
...
_PACK_ACTIONS = {
    ...
    "export": lambda args: cmd_pack_export(
        ...
        name=getattr(args, "name", None) or "my-mason-pack"),
