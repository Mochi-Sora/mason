"""Unified tool configuration for Mason Agent."""
...
from mason_cli.cli_output import print_info as _print_info
from mason_cli.colors import Colors, color
from mason_cli.config import cfg_get, load_config, save_config, get_env_value
from mason_cli.nous_subscription import (
    NousSubscriptionFeatures, apply_nous_managed_defaults, get_nous_subscription_features)
from mason_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from mason_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``mason_cli.tools_config.X`` callers and test patch targets resolving.
from mason_cli.tools_config_cua import (  # noqa: F401
...
from mason_cli.tools_config_post_setup import (  # noqa: F401
...
from mason_cli.tools_config_providers import (  # noqa: F401
...
from mason_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _MASON_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `mason tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from mason_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from mason_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``mason tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``mason-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"mason-{platform}"
...
_LANGFUSE_PUBLIC = ("MASON_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("MASON_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Mason routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Mason, text-only (no screenshots)",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; MASON_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("MASON_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
#: "declined": saving ``mason tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[mason-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (mason-acp, mason-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``mason-cli`` would re-enable disabled toolsets). A mixed list (``[mason-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``mason-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[mason-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[mason-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `mason config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``mason tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("mason-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``mason`` for ``mason-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `mason tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (mason-cli, ...) resolve to ALL tools and would silently override the user's
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from mason_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``mason tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `mason tools` / `mason setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    if getattr(args, "summary", False):
        ...
        return
    print(color("⚕ Mason Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: README.md in your Mason checkout", Colors.DIM))
    print()
    if first_install:
    ...
    print(color(f"  Tool configuration saved to {display_mason_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'mason' or gateway restart.", Colors.DIM))
    print()
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('mason_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('mason_cli.nous_account', 'format_nous_portal_entitlement_message'),
