...
Split out of ``mason_cli/auth.py``; origin helpers are imported lazily per function so
``mason_cli.auth.<helper>`` patches still intercept and no cycle forms.
"""
...
from mason_cli.auth_codex import _load_auth_store_maybe_locked, _refresh_payload_access_token
...
_RELOGIN = "Re-authenticate with `mason model`."
...
def _xai_oauth_state_from_store(auth_store: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    from mason_cli.auth import _load_provider_state
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    from mason_cli.auth import _load_global_auth_store
    ...
    if not state:
        raise _xai_err(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `mason model`.",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    """
    from mason_cli.auth import _global_auth_file_path, _persist_provider_state_to_store
    global_path = _global_auth_file_path()
    if global_path is None:  # classic mode (profile == root); the profile save already hit root
        return
    # Seat belt: under pytest never write the real ~/.mason/auth.json (mirrors the read-side guard
...
def _save_xai_oauth_tokens(
    ...
    """
    from mason_cli.auth import _auth_store_lock, _global_auth_file_path, _load_auth_store, _load_provider_state_with_source, _same_path, _save_auth_store, _store_provider_state, _utc_now_z, _write_through_xai_oauth_to_global_root
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    message = {
        ...
        "origin": (
            ...
            f"discovery; re-authenticate with `mason model` to re-fetch."
        ),
    }[problem]
...
def refresh_xai_oauth_pure(
    ...
    from mason_cli.auth import _nonempty_str, _utc_now_z, _xai_oauth_discovery
    del access_token
    ...
    if response.status_code != 200:
        ...
        # exists but the account isn't on the allowlist for API access). Re-running ``mason model`` won't
...
def _refresh_xai_oauth_tokens(
    ...
    from mason_cli.auth import _load_auth_store, _load_provider_state, refresh_xai_oauth_pure
    try:
...
def _quarantine_xai_oauth_tokens(exc: AuthError) -> None:
    ...
    """
    from mason_cli.auth import _last_auth_error_marker, _load_auth_store, _load_provider_state, _save_auth_store, _store_provider_state
    try:
        ...
        # ``mason auth add openai-codex`` created (which must not be overwritten — see #39236).
        state["tokens"] = tokens
...
def _xai_oauth_inference_base_url() -> str:
    return _xai_validate_inference_base_url(
        os.getenv("MASON_XAI_BASE_URL", "").strip().rstrip("/") or os.getenv("XAI_BASE_URL", "").strip().rstrip("/"),
...
def resolve_xai_oauth_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from mason_cli.auth import _auth_store_lock, _is_terminal_xai_oauth_refresh_error, _refresh_xai_oauth_tokens, _xai_oauth_discovery
    ...
    refresh_timeout_seconds = env_float("MASON_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
def _login_xai_oauth(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    from mason_cli.auth import _is_remote_session, _offer_existing_oauth_credentials, _print_login_success, _update_config_for_provider, _xai_oauth_device_code_login, resolve_xai_oauth_runtime_credentials, unsuppress_credential_source
    del pconfig
    ...
    print("(Mason creates its own local OAuth session)")
    print()
    ...
    # by ``mason auth remove xai-oauth``. Deliberately NOT inside _save_xai_oauth_tokens — the
...
def _xai_oauth_poll_device_token(
    ...
    from mason_cli.auth import _poll_device_token_generic
...
def _xai_oauth_device_code_login(*, timeout_seconds: float = 20.0, open_browser: bool = True) -> Dict[str, Any]:
    from mason_cli.auth import _can_open_graphical_browser, _is_remote_session, _print_device_code_instructions, _utc_now_z, _xai_oauth_discovery, _xai_oauth_poll_device_token
