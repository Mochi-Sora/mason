...
from mason_cli.local_runtime.context_policy import (
    FLOOR, RUNTIME_OVERHEAD_BYTES, TARGET_WINDOW, ub_logits_bytes)
from mason_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
from mason_cli.local_runtime.gguf import model_id_from_stem
...
_CATALOG_URL = ("https://raw.githubusercontent.com/NousResearch/mason-agent"
                "/main/mason_cli/local_runtime/catalog.json")
_SCHEMA_VERSION = 1
...
def _packaged_catalog() -> "tuple[CatalogEntry, ...]":
    ...
    raw = files("mason_cli.local_runtime").joinpath("catalog.json").read_text(encoding="utf-8")
...
def refresh_catalog(force: bool = False) -> bool:
    ...
    try:
        req = urllib.request.Request(_CATALOG_URL, headers={"User-Agent": "mason-local-runtime"})
