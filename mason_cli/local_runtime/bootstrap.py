...
from mason_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Mason process
...
def _detect_gpu_vendor() -> str | None:
    ...
    from mason_cli.local_runtime.hardware import _nvidia_smi_path
...
def models_dir() -> Path:
    ...
    return get_default_mason_root() / "models"
...
def _stop_state_server(state: dict) -> None:
    ...
    server."""
    from mason_cli.local_runtime.endpoint import _pid_alive
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from mason_cli.local_runtime.hardware import probe_budget
    from mason_cli.local_runtime.presets import generate_presets
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Mason process may already be supervising — reuse via state, but ONLY while its
