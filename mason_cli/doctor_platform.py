"""Host-platform checks for mason doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``mason_cli/doctor.py``, which re-exports every name so ``mason_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from mason_cli.config import is_nix_install_method, recommended_update_command_for_method
from mason_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from mason_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from mason_cli.doctor import PROJECT_ROOT
    from mason_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Mason containers", "apt": f"run `{cmd}`"}.get(method, "run `mason update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _mason_database_paths(mason_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Mason-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from mason_cli.backup import _QUICK_STATE_FILES
    entries = [(name, mason_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((mason_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(mason_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``mason_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from mason_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(mason_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from mason_cli.doctor import MASON_HOME
    from mason_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _mason_database_paths(mason_home if mason_home is not None else MASON_HOME)
    except Exception as exc:
        check_warn(f"Could not list Mason databases: {exc}")
        return
    exposed = []
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs mason_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from mason_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != mason_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'mason update', or set mason_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from mason_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-mason", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `mason profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `mason doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from mason_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `mason update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `mason desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.nousresearch.mason` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Mason once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Mason.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Mason.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Mason-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Mason.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from mason_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Mason "
                   ...
                   "    then enable your terminal (and Mason.app if you use Desktop), and restart them once. "
                   "With Mason' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from mason_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `mason doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from mason_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Mason already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "mason" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(mason not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    if link.is_symlink():
        ...
        if target == expected:
            return check_ok(f"{display}/mason → correct target")
        check_warn(f"{display}/mason points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/mason — run 'mason doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/mason exists (non-symlink)")
    else:
        check_fail(f"{display}/mason not found", "(mason command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/mason symlink — run 'mason doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/mason → {venv_bin}")
    f.fixed += 1
