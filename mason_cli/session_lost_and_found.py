...
# Mason session ids are timestamps (20260812_135332_ab12cd): the strongest sentinel for schema-less rows.
...
# 3.50.7 and 3.44.6 — the same version gate mason_state applies to the
...
# caught exactly this shell converting a live Mason state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Mason database.
#
# The predicate lives in mason_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from mason_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Mason database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="mason-recover-probe-")
