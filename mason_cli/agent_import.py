"""mason import-agent — import Claude Code / Codex CLI setups into Mason.
...
# Entry delimiter of the Mason memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `mason config edit` (or move it aside), then re-run the import."
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record("slash-commands", commands_dir, None, "skipped",
                        "Claude slash commands have no direct Mason equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Mason config")
                continue
            mason_srv, stripped = _translate_mcp_server(name, srv)
            self.stripped_secrets.extend(stripped)
            if not mason_srv:
                self.record(kind, name, None, "skipped", "Server has neither a command nor a url")
                continue
            existing[name] = mason_srv
            added += 1
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → MASON_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``mason import-agent`` (invoked from mason_cli.main)."""
    from mason_cli.config import get_config_path, load_config, save_config
    from mason_constants import get_mason_home
    from mason_cli.setup import (Colors, color, print_header, print_info, print_success,
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: mason import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: mason import-agent claude-code   or   mason import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Mason — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info(f"Specify a custom path: mason import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    print_info(f"Target:      {mason_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'mason setup' for credentials")
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), mason_home.resolve(),
    ...
    if not args.yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: mason import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'mason setup' "
               "to configure providers, or add them to ~/.mason/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from mason_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped := report.get("stripped_secrets"):
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'mason setup' or ~/.mason/.env.")
        print()
