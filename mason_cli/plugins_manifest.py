...
Split out of :mod:`mason_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``mason``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "mason", "depends",
}

# Highest manifest schema version this Mason understands.
...
def _plugins_debug() -> bool:
    from mason_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Mason "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `mason plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `mason plugins list`: ``disk-cleanup``
    ...
    # VALIDATED AND SURFACED ONLY — Mason never auto-installs these (isolation design for the install seam
