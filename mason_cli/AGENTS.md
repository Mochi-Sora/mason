# mason_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `MasonCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`mason_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`MasonCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.mason/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`mason_cli/curses_ui.py`; example
`mason_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`mason_cli/commands.py`)
...
## Config system (`mason_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`mason tools/setup`, most subcommands, `mason_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`mason_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.mason/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`mason update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `mason update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `mason-gateway*` unit /
  `ai.mason.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.mason/logs/update_receipts/`
...
`_apply_profile_override()` in `mason_cli/main.py` sets `MASON_HOME` before any module import, so
every `get_mason_home()` scopes to the active profile (rules in root). Profiles are independent
