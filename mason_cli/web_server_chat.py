...
from mason_cli.pty_session import PtySessionRegistry

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("mason_cli.web_server")


# /api/pty spawns ``mason --tui`` behind a pseudo-terminal and forwards bytes +
...
try:
    if sys.platform.startswith("win"):
        from mason_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    else:
        from mason_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from mason_cli.web_server import _is_accepted_host, app
...
_GATEWAY_WS_PROTOCOL = "mason-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "mason-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    """
    from mason_cli.web_server import _SESSION_TOKEN, app
    auth_required = bool(getattr(app.state, "auth_required", False))
    if auth_required:
        # Lazy import — keeps this function importable in test harnesses
        # that don't bring in the dashboard_auth layer.
        from mason_cli.dashboard_auth.audit import AuditEvent, audit_log
        from mason_cli.dashboard_auth.ws_tickets import (
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._mason_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    """Resolve the argv + cwd + env for the chat PTY (what ``mason --tui`` runs).

    Tests monkeypatch this with a tiny fake command.  Env contract: resume goes
    through ``MASON_TUI_RESUME`` (``ui-tui`` does not parse argv), resolved to
    the newest descendant; ``MASON_TUI_GATEWAY_URL`` attaches to this process's
    ...
    ``profile`` scopes the ENTIRE chat by pointing ``MASON_HOME`` at the profile
    dir, the same propagation ``mason -p <name>`` performs.
    """
    from mason_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from mason_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from mason_cli.main import PROJECT_ROOT
    from mason_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides MASON_HOME before config is bridged into the env.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["MASON_HOME"] = str(profile_dir)
    try:
    ...
    env.setdefault("MASON_TUI_DISABLE_MOUSE", "1")
    env.setdefault("MASON_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["MASON_TUI_RESUME"] = resume

    if sidecar_url:
        env["MASON_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # which inherits the profile MASON_HOME set above.
    if profile_dir is None and (gateway_ws_url := _build_gateway_ws_url()):
        env["MASON_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    """Host the in-container WS client should dial: ``MASON_DASHBOARD_WS_HOST``
    wins always; a wildcard bind becomes ``127.0.0.1``; others verbatim."""
    from mason_cli.web_server import app
    explicit = os.environ.get("MASON_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _server_internal_ws_url(path: str, **extra_qs) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from mason_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    from mason_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from mason_cli.web_server import _get_pty_active_session_files
    files = _get_pty_active_session_files(app)
    if files.get(channel) is None:
        fd, raw_path = tempfile.mkstemp(prefix="mason-pty-active-", suffix=".json")
        os.close(fd)
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix="mason-console")
