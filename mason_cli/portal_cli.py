"""``mason portal`` — the human-readable entry point for Nous Portal."""
...
DOCS_URL = "https://mason-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `mason model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `mason portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    Reuses the exact wiring behind ``mason setup --portal`` so the commands stay in lockstep.
    """
    from mason_cli.setup import _run_portal_one_shot
...
# Default (None/"") is the one-shot onboarding (alias for `mason auth add nous --type oauth` /
# `mason setup --portal`). `status` kept as a back-compat alias for `info`.
_SUBCOMMANDS = {
    None: _cmd_login,
    "": _cmd_login,
    "login": _cmd_login,
...
def portal_command(args) -> int:
    """Top-level dispatch for `mason portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    handler = _SUBCOMMANDS.get(sub)
    if handler is not None:
        return handler(args)
    print(f"Unknown portal subcommand: {sub}", file=sys.stderr)
    print("Run `mason portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `mason portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `mason portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `mason auth add "
            "nous --type oauth`, identical to `mason setup --portal`). "
            "Subcommands: login (default), info, open, tools."
        ),
    )
    portal_sub = portal_parser.add_subparsers(dest="portal_command")

    # `status` is a hidden (no help) back-compat alias; registration order = `mason portal -h` order.
