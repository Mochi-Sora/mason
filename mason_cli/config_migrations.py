...
in :func:`run_migrations`. Every write goes through ``mason_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_mason_home()}/config.yaml and run `mason setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``mason_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$MASON_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_mason_home() / "plugins"
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `mason plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `mason curator status`
    # had no stable logs dir. Only keys the user hasn't set are written.
    _c = _cfg()
    DEFAULT_CONFIG = _c.DEFAULT_CONFIG

    try:
        curator_dir = _c.get_mason_home() / "logs" / "curator"
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `mason config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from mason_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `mason tools` → "
