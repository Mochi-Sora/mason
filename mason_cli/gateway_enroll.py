"""``mason gateway enroll`` — enroll a self-hosted gateway with a relay connector.
    ...
    try:
        ...
        if exc.code == 401:
            message = (
                ...
                "token could not be verified — try `mason auth add nous` and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        _fail(
            "✗ `mason gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.mason/.env and never
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    try:
        ...
        default_root = Path(get_default_mason_root()).resolve()
