...
Split out of ``mason_cli.models``. Catalog fetchers defined in ``mason_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("mason_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``mason_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `mason moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from mason_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from mason_cli import models as _m
    from mason_cli import models_local as _ml
    from mason_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_ollama_native(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if models is None:
        return _soft_accept(
            ...
            "Mason will save the model name, but local Ollama model discovery could not verify it."
        )
...
def _validate_custom(req: _Request) -> dict[str, Any]:
    ...
    message = (
        ...
        f"Mason will still save `{req.requested}`, but the endpoint should expose `/models` for verification."
    )
    if anthropic_style:
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Mason picker convention, so an unknown `*-900k` can never be a real
...
def _validate_minimax(req: _Request) -> Optional[dict[str, Any]]:
    ...
    return match.verdict(req) or _soft_accept(
        ...
        f"{match.suggestion_text}"
        "\n  MiniMax does not expose a /models endpoint, so Mason cannot verify the model name."
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # docs-hosted manifest catches up; `mason chat` already accepts those at model-list build
