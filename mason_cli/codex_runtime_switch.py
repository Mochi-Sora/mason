...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "mason": "auto"}

_MASON_TOOLS_CALLBACK_NOTE = (
    "Mason tool callback registered: codex can now use "
    "web_search, web_extract, browser_*, vision_analyze, "
    "image_generate, skill_view, skills_list, text_to_speech, "
    "kanban_* (worker + orchestrator) via MCP.",
    "  (delegate_task, memory, session_search, todo run "
    "only on the default Mason runtime — they need the "
    "agent loop context.)")
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from mason_cli.codex_runtime_plugin_migration import migrate
        mig_report = migrate(config)
        # The mason-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != "mason-tools"]
        if user_servers:
        ...
        if "mason-tools" in mig_report.migrated:
            lines.extend(_MASON_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, mason-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Mason' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Mason.
        ...
        msg_lines.append(
            ...
            "Mason tools available via MCP callback).")
        msg_lines.append(
            "Effective on next session — current cached agent keeps "
            "the prior runtime to preserve prompt cache.")
    else:
        msg_lines.append("OpenAI/Codex turns will use the default Mason runtime.")
