...
CLI. Mixin on ``MasonCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLITerminalMixin:
    ...
    def _recover_after_resize(self, app, original_on_resize) -> None:
        ...
        handled by ``_mason_call_output_screen_diff`` (#83874). Suppression is cleared by
    ...
    def _try_attach_clipboard_image(self) -> bool:
        """Save a clipboard image to ~/.mason/images/ and attach it; True if attached."""
        from cli import datetime
        from mason_cli.clipboard import save_clipboard_image
        self._image_counter += 1
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        img_path = get_mason_home() / "images" / f"clip_{ts}_{self._image_counter}.png"
