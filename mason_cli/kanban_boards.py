"""``mason kanban boards …`` — board directories, the ``current`` pointer and ``board.json``.
...
from mason_cli.kanban_output import _err, _fmt_counts, _json_out


def _dispatch_boards(args: argparse.Namespace) -> int:
    """``mason kanban boards <action>`` — filesystem-only, so it works before ``kanban init``."""
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, boards):
        return 0
    if not boards:
        print("(no boards — create one with `mason kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `mason kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `mason kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print(f"Exported board {res['board']!r} → {res['archive']}\n"
          ...
          "Import it with `mason kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `mason kanban boards switch {res['board']}`.")
    return 0
