"""Safe Mason Console command engine."""
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["MasonConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'mason doctor'") and last.startswith("Run 'mason setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from mason_cli._subprocess_compat import split_command_line
    try:
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `mason ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from mason_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from mason_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        parser = _ArgumentParser(prog="mason", add_help=False)
        subparsers = parser.add_subparsers(dest="_console_command")
        module = importlib.import_module(self.module)
        entry = getattr(module, self.builder)
        main_handler = (
            (lambda: getattr(importlib.import_module("mason_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"mason_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"mason_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "mason_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "mason_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "mason_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "mason_cli.send_cmd", "register_send_subparser")


def _register_command_family(
    engine: "MasonConsoleEngine", root: str, surface: _CliSurface, paths: str) -> None:
    ...
    for child_path, mutating in _paths(paths):
        ...
        def handler(_engine: MasonConsoleEngine, args: list[str], fixed=child_path) -> str:
            return _dispatch(surface, root, fixed, args, namespace_update)

        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `mason {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `mason {usage}`?")
...
_BLOCKED_PAIRS = {
    ("config", "edit"): "`config edit` opens an editor and is not available in Mason Console.",
    ("mcp", "serve"): "`mcp serve` starts a server and is not available in Mason Console.",
    ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Mason Console.",
    ("skills", "config"): "`skills config` is interactive and is not available in Mason Console.",
    ("skills", "publish"): "`skills publish` is not available in Mason Console.",
    ("portal", "login"): "`portal login` is interactive and is not available in Mason Console.",
    ("portal", "open"): "`portal open` opens a browser and is not available in Mason Console.",
    ("kanban", "tail"): "`kanban tail` streams output and is not available in Mason Console.",
    ("kanban", "watch"): "`kanban watch` streams output and is not available in Mason Console.",
    ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Mason Console.",
    ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Mason Console.",
    ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Mason Console.",
    ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Mason Console.",
    ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Mason Console.",
    ("kanban", "gc"): "`kanban gc` is not available in Mason Console.",
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Mason Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Mason Console.",
}


class MasonConsoleEngine:
    """Curated line-command executor for Mason Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "mason":
                tokens = tokens[1:]
            if not tokens:
                return ConsoleResult("ok", output=self.help_text())
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Mason Console does not run shell syntax. Use one supported "
                    "Mason command at a time.")
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Mason Console", "", "Supported commands:"]
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Mason Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Mason Console."
        if first in _BLOCKED_TOP:
            return f"`mason {first}` is not available in Mason Console."
...
def _captured(fn):
    ...
    @functools.wraps(fn)
    def wrapper(engine: MasonConsoleEngine, args: list[str]) -> str:
...
def _simple_command(usage: str, module: str, name: str, make_args=lambda: (), **kwargs):
    ...
    def handler(_engine: MasonConsoleEngine, args: list[str]) -> str:
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Mason Console.")
...
_version = _simple_command(
    "version", "mason_cli._startup_fast", "print_fast_version_info", check_updates=True)


def _status(_engine: MasonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "status")
    from mason_cli.status import show_status
...
_doctor = _simple_command(
    "doctor", "mason_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "mason_cli.config", "show_config")
_cron_status = _simple_command("cron status", "mason_cli.cron", "cron_status")


def _logs(_engine: MasonConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Mason Console.")
    ns = _parse(
    ...
    from mason_cli.logs import list_logs, tail_log
    if ns.log_name == "list":
...
def _sessions_list(_engine: MasonConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: MasonConsoleEngine, args: list[str]) -> str:
...
def _config_path(_engine: MasonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from mason_cli.config import get_config_path
    return str(get_config_path())


def _config_set(_engine: MasonConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    from mason_cli.config import set_config_value
    return _capture_output(lambda: set_config_value(args[0], " ".join(args[1:])))


@_captured
def _config_migrate(_engine: MasonConsoleEngine, args: list[str]) -> None:
    _expect_no_args(args, "config migrate")
    from mason_cli.config import migrate_config
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from mason_state import SessionExportTooLargeError, resolved_max_export_messages
...
@_captured
def _sessions_export(_engine: MasonConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_rename(_engine: MasonConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_optimize(_engine: MasonConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_repair(_engine: MasonConsoleEngine, args: list[str]) -> None:
    ns = _parse(
        "sessions repair", args, (("--check-only",), dict(action="store_true")),
        (("--no-backup",), dict(action="store_true")))
    from mason_state import DEFAULT_DB_PATH
    from mason_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: MasonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch(_CLI_FAMILIES["profile"][0], "profile", (), ())


def _cron_list(_engine: MasonConsoleEngine, args: list[str]) -> str:
    ns = _parse("cron list", args, (("--all",), dict(action="store_true")))
    from mason_cli.cron import cron_list
...
def _cron_pause(_engine: MasonConsoleEngine, args: list[str]) -> str:
    from cron.jobs import pause_job
    return _cron_job_action(
        args, "cron pause <job>", "Paused",
        lambda ref: pause_job(ref, reason="paused from mason console"))


def _cron_resume(_engine: MasonConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _mason_now, rearm_oneshot, resume_job
    try:
        job = (
            rearm_oneshot(ns.job, _mason_now().isoformat() if ns.run_now else ns.at)
...
def _cron_run(_engine: MasonConsoleEngine, args: list[str]) -> str:
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Mason component status.", _status, ""),
    (("version",), "version", "Show Mason version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Mason logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Mason configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    """Run the local ``mason console`` REPL."""
    stdin, stdout, stderr = stdin or sys.stdin, stdout or sys.stdout, stderr or sys.stderr
    if interactive is None:
        interactive = bool(getattr(stdin, "isatty", lambda: False)())
    engine = MasonConsoleEngine()

    def say(text: str, **kw) -> None:
        if interactive:
            print(text, file=stdout, **kw)

    say("Mason Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("mason> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
