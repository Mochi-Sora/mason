"""Git plumbing for ``mason update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = "git remote add upstream https://github.com/NousResearch/mason-agent.git"
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from mason_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/mason-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/mason-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && mason update\n"
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/mason-agent.git",
    "git@github.com:NousResearch/mason-agent.git",
    "https://github.com/NousResearch/mason-agent",
    "git@github.com:NousResearch/mason-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/mason-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_mason_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from mason_constants import get_mason_home
        (get_mason_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from mason_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Mason repository.\n"
        "  This means you may miss updates from NousResearch/mason-agent.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/NousResearch/mason-agent.git")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from mason_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped MASON_HOME), then profile home as a fallback for custom layouts.

    The Mason-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped MASON_HOME (``<root>/profiles/<name>``), so a profile-scoped ``mason update`` must look
    there (monerostar review, #87876).
    """
    from mason_cli.update_cmd import get_default_mason_root, get_mason_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_mason_root(), Path(get_mason_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from mason_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``mason update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``mason update``, which ships with the checkout
