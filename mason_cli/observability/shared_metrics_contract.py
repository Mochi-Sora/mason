"""Bounded product contract for the first Mason shared-metrics slice."""
...
SCHEMA_KEY = "mason.metrics.schema_version"
SCHEMA_VERSION = "mason.metrics.event.v2"
MODEL_CALL_SCOPE = "mason.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "mason.task_run"
TOOL_CALL_SCOPE = "mason.tool_call"
CLIENT_ACTIVE_MARK = "mason.client.active"
TOOL_APPROVAL_MARK = "mason.tool_approval"
SKILL_LIFECYCLE_MARK = "mason.skill.lifecycle"
SKILL_LOAD_MARK = "mason.skill.load"
SUBSCRIBER_NAME = "mason.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "mason.client.active"
LEGACY_MODEL_CALL_METRIC = "mason.model_call.count"
MODEL_ROUTE_METRIC = "mason.model_route.count"
TASK_STARTED_METRIC = "mason.task_run.started"
TASK_FINISHED_METRIC = "mason.task_run.finished"
TOOL_CALL_METRIC = "mason.tool_call.count"
TOOL_APPROVAL_METRIC = "mason.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "mason.skill.lifecycle.count"
SKILL_LOAD_METRIC = "mason.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "mason_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Mason installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    mason_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(mason_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "mason_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("mason_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Mason logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "mason.call_role"
    )
    call_role = (metadata or {}).get("mason.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from mason_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Mason terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "mason-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Mason registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Mason tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Mason."""
