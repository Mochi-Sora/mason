"""Direct NeMo Relay integration for Mason shared client metrics."""
...
def _retry_ordinal(event: dict[str, Any]) -> int:
    """Mason's provider-local retry ordinal; 0 when absent or malformed."""
...
class _Runtime:
    """Own shared-metrics state layered on the Mason core Relay host."""

    def __init__(self, host: relay_runtime.RelayRuntime | None = None) -> None:
        resolved_host = host or relay_runtime.get_runtime()
        if resolved_host is None:
            raise RuntimeError("Mason core Relay runtime is unavailable")
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Mason task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # attempt. Provider fallback resets Mason's provider-local retry
            ...
            if task is not None:
                ...
                if _retry_ordinal(event) > 0:
                    # A real Mason retry can advance api_request_id while carrying the
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            self._flush_and_export("Mason shared-metrics task flush failed")
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(
                "Mason shared-metrics session %s closed with errors: subscriber flush failed: %s",
                session.session_id,
                exc,
            )
        else:
    ...
    def shutdown(self) -> None:
        ...
        if not self._registered:
            return
        self._flush_and_export("Mason shared-metrics shutdown flush failed")
        self._deregister()
        self._release()
    ...
    def _finish_tool_call(
        ...
        self._guarded(
            "Mason shared-metrics tool call close failed",
    ...
    def _finish_model_call(self, session: _MetricsSession, model_call_key: tuple[str, str]) -> None:
        ...
        if model_call is None:
            return
        self._guarded(
            "Mason shared-metrics model call close failed",
    ...
    def _finish_task(self, session: _MetricsSession, task_id: str, event: dict[str, Any]) -> bool:
        ...
        try:
            self._guarded(
                "Mason shared-metrics task close failed",
        ...
        with self._send_lock:
            ...
            if self._send_thread is None or not self._send_thread.is_alive():
                self._send_thread = threading.Thread(
                    ...
                    name="mason-shared-metrics-send", daemon=True,
                )
                self._send_thread.start()

    def _run_send_pass(self, endpoint: str) -> None:
        from mason_cli.observability.shared_metrics_sender import SharedMetricsSender
    ...
    def _safe(cls, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        return cls._guarded("Mason shared metrics operation failed", callback, *args, **kwargs)


def _raw_config() -> dict[str, Any]:
    """Read-only config snapshot (lazy import: tests patch ``mason_cli.config``).
    ...
    """
    from mason_cli.config import read_raw_config_readonly
...
def _resolved_send_config():
    ...
    from mason_cli.observability.shared_metrics_send_config import resolve_send_config
...
def enabled() -> bool:
    """Return the shared-metrics policy for the active Mason profile."""
    profile_key = relay_runtime.current_profile_key()
    try:
        config: Any = _raw_config()
    except Exception:
        logger.debug("Unable to read Mason shared-metrics policy", exc_info=True)
        config = None
...
def _reconcile_send_consent_once() -> None:
    ...
    AND consent is off: nothing to protect, and creating ``~/.mason/telemetry`` for every
    ...
    try:
        ...
        default_path = get_mason_home() / "telemetry" / "shared_metrics" / "metrics.sqlite3"
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Mason lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning("Mason shared metrics hook failed: %s", hook_name, exc_info=True)


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Mason's runtime registry."""
...
def start_task_run(
    *, session_id: str, task_id: str, platform: str, parent_session_id: str = ""
    ...
    """Start task metrics at the outer Mason execution boundary."""
    _run_task_hook(
    ...
    *, retry_failed: bool = False, host: relay_runtime.RelayRuntime | None = None
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Mason shared metrics initialization failed", exc_info=True)
...
_PLUGIN_COMPAT_LAZY = {
    'CLIENT_ACTIVE_MARK': ('mason_cli.observability.shared_metrics_contract', 'CLIENT_ACTIVE_MARK'),
    'MODEL_CALL_PROFILE_MODEL': ('mason_cli.observability.shared_metrics_contract', 'MODEL_CALL_PROFILE_MODEL'),
    'SCHEMA_KEY': ('mason_cli.observability.shared_metrics_contract', 'SCHEMA_KEY'),
    'SCHEMA_VERSION': ('mason_cli.observability.shared_metrics_contract', 'SCHEMA_VERSION'),
    'SKILL_LIFECYCLE_MARK': ('mason_cli.observability.shared_metrics_contract', 'SKILL_LIFECYCLE_MARK'),
    'SKILL_LOAD_MARK': ('mason_cli.observability.shared_metrics_contract', 'SKILL_LOAD_MARK'),
    'TOOL_APPROVAL_MARK': ('mason_cli.observability.shared_metrics_contract', 'TOOL_APPROVAL_MARK'),
    'TOOL_CALL_SCOPE': ('mason_cli.observability.shared_metrics_contract', 'TOOL_CALL_SCOPE'),
    'model_call_fields': ('mason_cli.observability.shared_metrics_contract', 'model_call_fields'),
    'skill_lifecycle_fields': ('mason_cli.observability.shared_metrics_contract', 'skill_lifecycle_fields'),
    'skill_load_fields': ('mason_cli.observability.shared_metrics_contract', 'skill_load_fields'),
    'task_start_fields': ('mason_cli.observability.shared_metrics_contract', 'task_start_fields'),
    'task_terminal_fields': ('mason_cli.observability.shared_metrics_contract', 'task_terminal_fields'),
    'task_terminal_state': ('mason_cli.observability.shared_metrics_contract', 'task_terminal_state'),
    'tool_approval_outcome': ('mason_cli.observability.shared_metrics_contract', 'tool_approval_outcome'),
    'tool_category': ('mason_cli.observability.shared_metrics_contract', 'tool_category'),
    'tool_terminal_fields': ('mason_cli.observability.shared_metrics_contract', 'tool_terminal_fields'),
}
