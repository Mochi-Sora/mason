"""Durable aggregation and local export for Mason shared metrics."""
...
_PACKAGE_SCHEMA_VERSION = "mason.shared_metrics.v2"
...
_RESOURCE_COLUMNS = ("mason_version", "os_family", "architecture", "install_method")
...
_CREATE_COUNTER_AGGREGATES_SQL = """
            CREATE TABLE IF NOT EXISTS counter_aggregates (
                ...
                PRIMARY KEY (
                    period_start, metric_name, mason_version, os_family, architecture,
...
_MIGRATE_V1_SQL = (
    ...
    """
            ...
            SELECT
                period_start, metric_name, mason_version, 'unknown', 'unknown', 'unknown',
...
_PENDING_PERIOD_COUNT_SQL = """
                SELECT COUNT(*) AS period_count FROM (
                    SELECT period_start, mason_version, os_family, architecture, install_method
                    FROM counter_aggregates WHERE value > packaged_value
                    GROUP BY period_start, mason_version, os_family, architecture, install_method
                )
                """
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_mason_home() / "telemetry" / "shared_metrics"
    ...
    def counter_snapshot(self) -> list[dict[str, Any]]:
        ...
        with self._connection() as connection:
            rows = connection.execute(
                """
                SELECT period_start, metric_name, mason_version, os_family, architecture,
                    install_method, dimensions_json, value, packaged_value
                FROM counter_aggregates
                ORDER BY period_start, mason_version, os_family, architecture, install_method,
    ...
    def _ensure_schema(self) -> None:
        # Serialize first-run creation and upgrades across Mason processes.
        with self._write(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            ...
            # so a bump would hard-fail an older Mason (second profile, rollback) sharing
    ...
    def _create_package_in_transaction(
        ...
        rows = connection.execute(
            """
                ...
                WHERE period_start = ? AND mason_version = ? AND os_family = ?
        ...
        for row in rows:
            connection.execute(
                """
                    ...
                    WHERE period_start = ? AND metric_name = ? AND mason_version = ?
