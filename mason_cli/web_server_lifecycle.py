...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "mason_cli.gateway",
        "mason_cli.auth",  # provider auth state → copilot_auth → subprocess
        "mason_cli.copilot_auth",
        "mason_cli.runtime_provider",
        "mason_cli.skin_engine",  # resolve_skin() config + skin engine init
        "mason_cli.inventory",  # provider catalogs + models.dev cache
...
def _eager_reconcile_own_session_db() -> None:
    ...
    """
    try:
        from mason_state import SessionDB, _default_db_path
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``MASON_DESKTOP_READY_FILE`` is set.
    ...
    """
    target = os.environ.get("MASON_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``MASON_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``mason.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("MASON_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("MASON_PARENT_START_MARKER") or None
    nonce = os.environ.get("MASON_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable MASON_PARENT_START_MARKER=%r / nonce; "
    ...
    try:
        poll = max(0.5, float(os.environ.get("MASON_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'mason serve' / 'mason dashboard' backend or the Mason gateway. "
