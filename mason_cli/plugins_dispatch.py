...
Mixed into :class:`mason_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from mason_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- mason-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- mason-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- mason-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``mason:<event>``.
MASON_EVENT_NAMESPACE = "mason"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from mason_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"mason-hook-{callback_name}"[:40], daemon=True)
        thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="mason-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from mason_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
