...
Split out of ``mason_cli/config.py``; every name is re-imported there, so
``mason_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``mason_cli.config``
...
logger = logging.getLogger("mason_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but Mason'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by Mason, not hand-curated.
...
def get_custom_provider_model_capability(
    ...
    from mason_cli.config import get_compatible_custom_providers, load_config_readonly
