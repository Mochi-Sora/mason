"""Gateway subcommand for mason CLI.

Handles: mason gateway [run|start|stop|restart|status|install|uninstall|setup]
"""

import asyncio
import contextlib
from mason_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
from mason_cli import setup_platforms
...
# display_mason_home is imported lazily: mason_constants may be a cached pre-update version.
from mason_cli.setup import (
    print_header,
    print_info,
    print_success,
    print_warning,
...
from mason_cli.colors import Colors, color
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``mason-gateway*`` /
    ``ai.mason.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.mason.gateway*`` LaunchAgent — the
    ...
    the ``mason-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "mason-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.mason.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``mason-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``mason-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.mason.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.mason.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `mason update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `mason update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``mason`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``mason gateway status`` from falsely counting the
    ``mason`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``mason gateway
    ...
    current_home = str(get_mason_home().resolve())
    # Forward slashes on both sides of the MASON_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"mason_home={current_home_lc}" in command_lc
            )

        # Default profile: accept unless argv advertises another profile. MASON_HOME may come via
        # env (invisible to wmic/CIM), so only a non-matching explicit MASON_HOME= disqualifies.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        return not ("mason_home=" in command_lc and f"mason_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``mason update`` hung exactly there on
    ...
    from mason_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``mason update``)."""
...
def find_profile_gateway_processes(exclude_pids: set | None = None, *, strict: bool = False) -> list[ProfileGatewayProcess]:
    """Return running gateway PIDs mapped to Mason profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid, get_running_pid_identity_strict
        from mason_cli.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Mason's own PID file + a parent chain ending at a running SCM service
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "mason_cli.main"]
    if profile != "default":
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``mason update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from mason_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (MASON_HOME,
    ...
    if sys.platform == "win32":
        try:
            from mason_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        import sys
        import time
        from mason_cli._subprocess_compat import (
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_mason_home()) / "logs"
        ...
        # MASON_HOME) the windowless base interpreter needs to import mason_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _mason_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``MASON_HOME`` from the on-disk unit file — what refresh/compare already read, and reliable under ``sudo``."""
    ...
    for line in text.splitlines():
        ...
        if body.startswith("Environment="):
            ...
            if body.startswith("MASON_HOME="):
                return body.split("=", 1)[1].strip().strip('"') or None
    return None


def _sync_mason_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``MASON_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_mason_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_mason_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("MASON_HOME", "").strip()
    if unit_home and os.environ.get("MASON_HOME", "").strip() != unit_home:
        os.environ["MASON_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}mason gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}mason gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from mason_cli.service_manager import detect_service_manager, get_service_manager
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from mason_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: mason gateway stop")
    else:
        print("⚠ Gateway process is running for this profile, but the service is not active")
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `mason gateway`")
        print("  can refuse to start another copy until this process stops.")


def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``mason gateway status``."""
    try:
...
def _gateway_list() -> None:
    ...
    try:
        from mason_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from mason_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "Mason_Gateway"
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.mason.gateway* fleet — not just the current profile's label — or a sibling
...
from mason_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    """Lazily import :mod:`mason_cli.gateway_windows` (Windows-only service backend)."""
    from mason_cli import gateway_windows
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("MASON_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``MASON_GATEWAY_DETACHED=1``
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from mason_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "mason-gateway"
SERVICE_DESCRIPTION = "Mason Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    """Service-name suffix for MASON_HOME: "" for the default root, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path."""
    import hashlib
    from mason_constants import get_default_mason_root
    home = get_mason_home().resolve()
    default = get_default_mason_root().resolve()
    if home == default:
        return ""
    # Fallback: short hash for arbitrary MASON_HOME paths
    return _profile_name_from_home(home, default) or hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(mason_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *mason_home*/*default_root* let a
    sudo/root process generate a unit for another user (the defaults would refer to root)."""
    from mason_constants import get_default_mason_root
    home = Path(mason_home or str(get_mason_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_mason_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``mason-gateway`` for default MASON_HOME, ``mason-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("mason.service",)
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "mason_cli.main gateway",
    "mason_cli/main.py gateway",
    "gateway/run.py",
    " mason gateway ",
    "/mason gateway ",
)
...
def _find_legacy_mason_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``mason.service``), which
    ...
    Detects unit files installed by older Mason versions that used a different service name (e.g. When both
    a legacy unit and the current ``mason-gateway.service`` are active, they fight over the same bot token
...
def has_legacy_mason_units() -> bool:
    """Return True when any legacy Mason gateway unit files exist."""
    return bool(_find_legacy_mason_units())


def print_legacy_unit_warning() -> None:
    """Warn about installed legacy gateway units; prints nothing when there are none."""
    legacy = _find_legacy_mason_units()
    if not legacy:
        return
    print_warning("Legacy Mason gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        print_info(f"    {path}  ({_service_scope_label(is_system)} scope)")
    print_info("  These run alongside the current mason-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    mason gateway migrate-legacy")


def remove_legacy_mason_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_mason_units()
    if not legacy:
        print("No legacy Mason gateway units found.")
        return 0, []

    print()
    print("Legacy Mason gateway unit(s) found:")
    for name, path, is_system in legacy:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: mason gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo mason gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    mason gateway uninstall")
    print_info("    sudo mason gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: mason gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``mason setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   mason gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: mason gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: mason gateway install")
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.mason.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
    name = f"ai.mason.gateway-{suffix}" if suffix else "ai.mason.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed MASON_HOME never
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.mason.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.mason.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # Update-boundary: a gateway restarted mid-update can hold a stale mason_constants
            # without this symbol; see _reload_mason_constants() in mason_cli/managed_uv.py.
            from mason_cli.managed_uv import _reload_mason_constants
            venv_python_path = _reload_mason_constants().venv_python_path
...
def _mason_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MASON_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.mason[/profiles/x]`` → ``/home/alice/.mason[/profiles/x]``; custom paths kept as-is."""
    current_mason_raw = os.environ.get("MASON_HOME", "").strip()
    current_mason = Path(current_mason_raw).expanduser() if current_mason_raw else get_mason_home()
    # Keep paths lexical: resolving a non-existent path can bake a different MASON_HOME into the unit.
    current_default = Path.home() / ".mason"
    target_default = Path(target_home_dir) / ".mason"
    try:
        # Default ~/.mason or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_mason.relative_to(current_default))
    except ValueError:
        return str(current_mason)  # Completely custom path (not under ~/.mason) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    extras = (project_root / "node_modules" / ".bin", mason_home / "node" / "bin", mason_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (MASON_HOME, else PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(mason_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = reset_home_override = None
    if mason_home is not None:
        from mason_constants import (reset_mason_home_override, set_mason_home_override)
        override_token = set_mason_home_override(mason_home)
        reset_home_override = reset_mason_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], mason_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<mason_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from mason_constants import (mason_managed_node_tree_present, iter_mason_node_dirs)
    managed_node_present = mason_managed_node_tree_present(mason_root)
    for directory in iter_mason_node_dirs(mason_root) if managed_node_present else ():
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        mason_home = _mason_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.mason when mason_home lives under it.
        target_root = Path(home_dir) / ".mason"
        try:
            Path(mason_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(mason_home, default_root=target_root)
        except ValueError:
            profile_arg = _profile_arg(mason_home)
        # Remap paths under the calling user's home (/root/) to the target user's so the service can read them.
        python_path = _remap_path_for_user(python_path, home_dir)
        working_dir = str(mason_home) if mason_home else _remap_path_for_user(working_dir, home_dir)
        ...
        _append_node_dir_for_service(_target_node_entries, Path(mason_home) if mason_home else None)
        ...
        mason_home = str(get_mason_home().resolve())
        profile_arg = _profile_arg(mason_home)
        user_home = Path.home()
        identity_lines = env_lines = ""
        wanted_by = "default.target"

    watchdog_seconds = _systemd_watchdog_seconds(mason_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m mason_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="MASON_HOME={mason_home}"
Environment="MASON_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__MASON_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # MASON_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.mason, so refresh would rewrite a correct unit and status warn forever.
    # Idempotent; the os.environ mutation persists for later runtime reads (restart's PID/drain).
    _sync_mason_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir MASON_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'MASON_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>MASON_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp MASON_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: MASON_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported MASON_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the MASON_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp MASON_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/mason_test"', "/mason_test/")):
        return False

    # Structural variant: refuse ANY temp-dir MASON_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Mason install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo mason gateway uninstall --system")
    print_info("         mason gateway install")
    print_info("         mason gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("MASON_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("MASON_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "MASON_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin MASON_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_mason_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}mason gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}mason gateway status{scope_flag}             # Check status")
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}mason gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # MASON_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `mason gateway status` or logs for final shutdown state."
        )
        return
...
def systemd_restart(system: bool = False):
    ...
    # MASON_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `mason
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    try:
        ...
        print(
            ...
            "check `mason gateway status` or logs for final state."
        )
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}mason gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}mason gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}mason gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}mason gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.mason.gateway-{suffix}" if suffix else "ai.mason.gateway"
...
# at all and we degrade to a detached background process (the `nohup mason gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_mason_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_mason_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m mason_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "mason_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``mason update`` hands back to
    ...
    ``--external-supervisor`` so ``mason update`` sees the flag on the live grandchild argv and hands the
    ...
    return [get_python_path(), "-m", "mason_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup mason gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
    ...
    """
    from mason_cli._subprocess_compat import windows_detach_popen_kwargs
    log_dir = get_mason_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    from mason_constants import display_mason_home as _dhh
    ...
    print(f"  Try manually: nohup mason gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from mason_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>MASON_HOME</key>
        <string>{mason_home}</string>
        <key>MASON_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Mason install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  mason gateway status             # Check status")
    from mason_constants import display_mason_home as _dhh
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `mason gateway start` re-bootstraps.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    MASON_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `mason update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Mason PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Mason install")
    else:
        print("⚠ Service definition is stale relative to the current Mason install")
        print("  Run: mason gateway start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: mason gateway stop")
        else:
    ...
    if deep:
        log_file = get_mason_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/mason" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 MASON_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from mason_constants import get_default_mason_root
        default_root = get_default_mason_root()
    except Exception:
        return False
    ...
    try:
        ...
        cfg = {}
        if cfg_path.exists():
            from mason_cli.config import read_user_config_raw
        ...
        from mason_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other MASON_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's MASON_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (mason_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("mason_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Use 'mason gateway restart' to replace it,")
    print("  or 'mason gateway stop' first.")
    print("  Or use 'mason gateway run --replace' to auto-replace.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("MASON_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Mason gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'mason' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/mason/docker/entrypoint.sh before the Mason command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$MASON_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set MASON_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from mason_startup_watchdog import (
        ...
        try:
            from mason_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. MASON_GATEWAY_EXIT_DIAG=0 opts out."""
    from datetime import datetime as _dt, timezone as _tz

    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("MASON_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from mason_constants import get_mason_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            line = {
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; MASON_GATEWAY_MAX_STARTS / MASON_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["MASON_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["MASON_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from mason_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (MASON_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `mason gateway restart`.
    if supports_systemd_services():
        try:
...
_PLATFORMS = [
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. mason) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Mason delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "5. Mason connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: mason pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Mason will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    try:
        from mason_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as e:
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'mason pairing approve')",
...
def _prompt_unauthorized_access(*, is_email: bool) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: mason pairing approve <platform> <code>")
    elif is_email:
        print_success("  Unknown email senders will be ignored.")
    else:
        print_info("  Skipped — configure later with 'mason gateway setup'")
...
def _telegram_auto_setup(token_var: str) -> tuple[bool, object]:
    ...
    if prompt("  Choice [1/2]", default="1").strip() != "1":
        return False, None
    try:
        from mason_cli.telegram_managed_bot import (
...
def _setup_standard_platform(platform: dict):
    ...
    from mason_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _running_under_s6() -> bool:
    from mason_cli.service_manager import detect_service_manager
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Mason will open Tencent iLink QR login in this terminal.",
        "  2. Use WeChat to scan and confirm the QR code.",
        "  3. Mason will store the returned account_id/token in ~/.mason/.env.",
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `mason gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_mason_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            print_info(
                "  Unknown DM users can request access and you approve them with `mason pairing approve`."
            )
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `mason pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the mason_cli.setup cycle."""
    from mason_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        print_info(f"  Set these env vars in ~/.mason/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            print_info("Start manually: mason gateway")
    except UserSystemdUnavailableError as e:
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: mason gateway run",
        "  For persistence:   tmux new -s mason 'mason gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: mason gateway run",
        "  Or start it manually in the background (best effort): nohup mason gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: mason gateway run",
    ),
}
...
def _wizard_service_status_block() -> None:
    ...
    if supports_systemd_services() and has_legacy_mason_units():
        print_legacy_unit_warning()
        print()
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        print_info("  You can install later: mason gateway install")
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo mason gateway install --system")
        print_info("  Or run in foreground:  mason gateway run")
        return
    try:
        ...
        print_info("  You can try manually: mason gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_platform_status(p)) for p in _all_platforms()):
        ...
        print()
        print_info("No platforms configured. Run 'mason gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from mason_cli.service_manager import (
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``MASON_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    MASON_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("MASON_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # MASON_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("MASON_S6_SUPERVISED_CHILD"):
        return False
    ...
    print(
        ...
        "  dashboard supervised alongside if MASON_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or MASON_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-mason
        # service's pattern in docker/s6-rc.d/main-mason/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `mason gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  mason gateway run                              # direct foreground",
    "  tmux new -s mason 'mason gateway run'         # persistent via tmux",
    "  nohup mason gateway run > ~/.mason/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: mason gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  mason profile create <name>     # creates the s6 service slot",
        "  mason -p <name> gateway start   # bring it up via s6",
        "  mason status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: mason gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: mason gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: mason gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  mason profile delete <name>     # tears down the s6 service slot",
        "  mason -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: mason gateway run"),
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: mason gateway run",
            "  Or use tmux/screen for persistence: tmux new -s mason 'mason gateway run'",
        )
        print()
...
def _cmd_restart(args):
    ...
    if supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    mason gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: mason gateway start",
        )
        sys.exit(1)
...
# ``mason gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  mason gateway install"),
    "other": (
        "To install as a service:", "  mason gateway install", "  sudo mason gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup mason gateway run > ~/.mason/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s mason 'mason gateway run'         # persistent via tmux",
        "  nohup mason gateway run > ~/.mason/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  mason gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  mason gateway install  # Install as user service",
        "  sudo mason gateway install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: mason gateway status")
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  mason gateway run      # Run in foreground")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Mason gateway unit files (e.g. mason.service)."""
    dry_run = getattr(args, "dry_run", False)
    yes = getattr(args, "yes", False)
    if not supports_systemd_services() and not is_macos():
        print("Legacy unit migration only applies to systemd-based Linux hosts.")
        return
    remove_legacy_mason_units(interactive=not yes, dry_run=dry_run)
