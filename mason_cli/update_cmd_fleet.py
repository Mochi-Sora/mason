"""Gateway fleet restart + post-update verification for ``mason update``.

Split out of ``mason_cli/update_cmd.py``; every name is re-imported there so
``mason_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
# Under MASON_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "mason-gateway*", "mason-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from mason_cli.update_cmd import get_mason_home
    path = get_mason_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """MASON_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from mason_cli.update_cmd import get_mason_home
    return get_mason_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    from mason_cli.update_cmd import _capture_head_sha, _m
    try:
        from mason_cli.build_info import get_code_identity
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from mason_cli.update_cmd import _current_checkout_sha
    try:
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `mason update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `mason update` or `mason gateway restart`.", file=stream)
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every mason-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    with suppress(Exception):
        _m()._purge_stale_mason_modules()
    # Warn if legacy Mason gateway unit files are still installed. When both mason.service (from a
    # pre-rename install) and the current mason-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `mason update` surfaces the issue until the
    # user migrates.
    try:
        from mason_cli.gateway import (
    ...
    failed: list = []
    try:
        # --- Systemd services (Linux) --- Discover all mason-gateway* units (default + profiles) plus
        # mason-serve* units (the Desktop app's backend, #83438).
        if supports_systemd_services():
            _restart_systemd_gateway_units_best_effort(failed)
        # --- Launchd services (macOS) --- Restart EVERY ai.mason.gateway* LaunchAgent, not only the
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``mason update``, finish a skipped restart.

    No-op when nothing is pending; exits 1 on incomplete catch-up so automation
    does not treat the fleet as healthy.
    """
    from mason_cli.update_cmd import _run_pending_fleet_restart
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: mason gateway restart")
    sys.exit(1)
...
def _is_mason_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("mason-serve")``
    would accept ``mason-server.service``."""
    return (
        ...
        unit == "mason-gateway.service"
        or unit.startswith("mason-gateway-")
        or unit == "mason-serve.service"
        or unit.startswith("mason-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each mason-gateway*/mason-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_mason_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``mason-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``mason-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``mason-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "mason-gateway" or svc_name.startswith("mason-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from mason_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    mason gateway status")
    if any(not name.startswith("ai.mason.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.mason.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: mason gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: mason gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(restarted_services: list, failed_or_stale_units: list, drain_budget: float) -> None:
    ...
    The pull is shared across profiles, so every ``ai.mason.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``mason_cli.gateway`` no longer importing
    ...
    """
    try:
        from mason_cli.gateway import find_gateway_pids
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``mason-gateway-foobar.service``.
    """
    name = str(service).removesuffix(".service")
    if profile == "default":
        return name in {"mason-gateway", "ai.mason.gateway", "gateway", "gateway-default"}
    return name in {f"mason-gateway-{profile}", f"ai.mason.gateway-{profile}", f"gateway-{profile}"}
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither MASON_SPAWN
# nor MASON_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a mason-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``mason_cli.gateway`` in the
    ...
    child's ``mason-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``mason_cli.gateway`` from the freshly
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to mason-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. mason-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `mason update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `mason gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active mason-gateway*/mason-serve* systemd unit (user + system).
    ...
    """
    from mason_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: mason gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from mason_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: mason gateway run")
            if unmapped_count > 1:
                print("    (or: mason -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from mason_cli.gateway import find_gateway_pids, _get_service_pids
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from mason_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/mason-serve`` vs ``system/mason-serve`` are different
    ...
    # Purge stale cached Mason modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from mason_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from mason_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.mason.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy mason.service fights mason-gateway.service over the bot token; warn on
    every update until migrated."""
    from mason_cli.gateway import (has_legacy_mason_units, _find_legacy_mason_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_mason_units()):
        return
    print()
    print("⚠ Legacy Mason gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_mason_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (mason.service) fight the current")
    print("  mason-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    mason gateway migrate-legacy")
    print()
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from mason_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `mason serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `mason serve` (no unit) is left running its
    ...
    print("  mason model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from mason_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from mason_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import mason_cli.update_receipt as _ur
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``mason_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
