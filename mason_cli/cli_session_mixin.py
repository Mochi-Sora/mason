...
Mixin bound onto ``MasonCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from mason_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_session_status(self):
        ...
        lines = [
            "Mason CLI Status", "", f"Session ID: {self.session_id}", f"Path: {display_mason_home()}",
        ]
        if title:
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from mason_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from mason_cli.timefmt import relative_time as _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Mason #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _notify_session_boundary(self, event_type: str) -> None:
        ...
        with contextlib.suppress(Exception):
            from mason_cli.lifecycle import finalize_session, invoke_hook
            ...
            if self._session_db:
                with contextlib.suppress(Exception):
                    ...
                    self._session_db.create_session(
                        ...
                        source=os.environ.get("MASON_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        """
        from cli import datetime
        from mason_cli.session_export import (
        ...
        saved_dir = get_mason_home() / "sessions" / "saved"
        try:
        ...
        if filename:
            ...
            path = saved_dir / f"mason_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve MASON_SESSION_ID to the child id after an
            # out-of-place rotation (idempotent when no rotation happened).
            if self.session_id:
                print(f"       Resume the live session with: mason --resume {self.session_id}")
        except Exception as e:
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``mason -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from mason_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``MASON_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / MASON_YOLO_MODE)."
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware: summarize everything except the most recent
        ...
        from mason_cli.partial_compress import (
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if not msg_count:
            try:
                from mason_cli.skin_engine import get_active_goodbye
        ...
        # the hint ("default"/"custom" use the standard MASON_HOME).
        try:
        ...
        print(f"  mason --resume {self.session_id}{profile_flag}")
        if session_title:
            print(f"  mason -c \"{session_title}\"{profile_flag}")
        print()
