...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError("Not logged into Nous Portal. Run `mason auth add nous` first.")
            try:
    ...
    # auth.json access — kept local so mason_cli.auth's public surface does not grow.
