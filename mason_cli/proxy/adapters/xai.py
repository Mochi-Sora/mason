...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Mason-managed OAuth credentials."""

    auth_hint = "mason auth add xai-oauth --type oauth"
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    "No xAI OAuth credentials found. Run `mason auth add xai-oauth --type oauth` first."
                )
            entry = pool.select()
            if entry is None:
                raise RuntimeError(
                    "No available xAI OAuth credentials found. Run "
                    "`mason auth reset xai-oauth` or re-authenticate with `mason auth add xai-oauth --type oauth`."
                )
            self._pool = pool
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `mason auth add xai-oauth --type oauth`."
            )
        base_url = str(
