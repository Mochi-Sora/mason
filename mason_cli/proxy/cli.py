"""CLI handlers for the ``mason proxy`` subcommand."""
...
from mason_cli.proxy.server import (
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("mason proxy requires aiohttp. Run `mason setup` to install it.")
        return 1
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"mason auth add {adapter.name}")
        _err(f"Not logged into {adapter.display_name}. Run `{auth_hint}` first.")
        return 2
    host = getattr(args, "host", None) or DEFAULT_HOST
    port = getattr(args, "port", None) or DEFAULT_PORT
    _err(
        f"Starting Mason proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Mason proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: mason proxy start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``mason proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        "mason proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  mason proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  mason proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  mason proxy providers\n"
