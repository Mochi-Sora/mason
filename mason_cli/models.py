...
``mason_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from mason_cli.urllib_security import open_credentialed_url
from mason_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from mason_cli.models_reasoning_caps import (
...
_MASON_USER_AGENT = f"mason-cli/{_MASON_VERSION}"
...
def _pricing_profile_key() -> str:
    ...
    from mason_constants import mason_home_key

    return mason_home_key()
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_mason_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from mason_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from mason_cli.model_catalog import get_default_model_from_cache
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (mason-agent is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    try:
        from mason_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — mason-agent requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from mason_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from mason_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from mason_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from mason_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``mason model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from mason_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``mason_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Mason should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from mason_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from mason_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from mason_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from mason_cli.codex_models import get_codex_model_ids
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from mason_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Mason release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``mason model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Mason actually ships. (#46850)
    ...
    from mason_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $MASON_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from mason_constants import get_mason_home
    return get_mason_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_mason_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``mason model --refresh``."""
    try:
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from mason_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _MASON_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"mason-agent/{_MASON_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from mason_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=mason"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _MASON_USER_AGENT}
    try:
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('mason_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('mason_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('mason_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('mason_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('mason_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('mason_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('mason_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('mason_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('mason_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('mason_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('mason_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('mason_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('mason_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('mason_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('mason_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('mason_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('mason_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('mason_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('mason_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('mason_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('mason_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('mason_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('mason_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('mason_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('mason_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('mason_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('mason_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('mason_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('mason_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('mason_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('mason_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
