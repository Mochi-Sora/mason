"""CLI subcommand: ``mason send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"mason send: {file_path} is not a text file. --file reads the "
                ...
                f'  mason send --to telegram "MEDIA:{file_path}"\n'
                f'  mason send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  mason send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"mason send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"mason send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"mason send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"mason send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"mason send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `mason gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.mason/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_mason_env() -> None:
    """Populate ``os.environ`` from ``~/.mason/.env`` AND bridge top-level ``config.yaml`` keys into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_mason_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `mason send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "mason send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  mason send --to telegram \"hello\"\n"
            "  mason send --to discord:#ops --file report.md\n"
            "  mason send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "mason send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `mason send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `mason send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Mason "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.mason/.env + ~/.mason/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  mason send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | mason send --to telegram:-1001234567890\n"
            "  mason send --to discord:#ops --file /tmp/report.md\n"
            "  mason send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  mason send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  mason send --list                  # all platforms\n"
            "  mason send --list telegram         # filter by platform\n"
            "\n"
