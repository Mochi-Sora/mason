#!/usr/bin/env python3
"""Skills Hub CLI — Unified interface for the Mason Skills Hub."""
...
# serves everything from "mason-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "mason-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "mason-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _ident_col(style: str) -> tuple:
    ...
    # copy into `mason skills install`.
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.mason/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                "[dim](optional — press Enter to install flat at ~/.mason/skills/<name>/)[/]")
...
def do_search(query: str, source: str = "all", limit: int = 10, console: Optional[Console] = None,
    ...
    c.print(table)
    c.print("[dim]Use: mason skills inspect <identifier> to preview, "
            "mason skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    c.print("[dim]Tip: 'mason skills inspect <identifier>' to preview, "
            "'mason skills install <identifier>' to install, "
            "'mason skills search <query>' to search deeper[/]\n")
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # mason-index AND the external APIs at once would double-count every skill.
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if preview is not None:
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="mason skills install <id> to install"))
    c.print()
...
def _resolve_url_bundle_name(c: Console, bundle, meta, identifier: str,
    ...
    if name_override and _is_valid_installed_skill_name(name_override):
        ...
        c.print(f"[bold red]Cannot install from URL:[/] {url}\n"
                ...
                f"  [bold]mason skills install {url} --name <your-name>[/]\n\n"
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.mason.blueprint`` via /suggestions — never
    ...
    try:
        ...
        if register_blueprint_suggestion(spec) is not None:
            ...
            c.print("[dim]You can still schedule it any time by asking the agent "
                    "or via[/] [bold]mason cron add[/][dim].[/]\n")
...
_INSTALL_PANELS = {
    "official": (
        ...
        "It ships with mason-agent but is not activated by default.\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_mason_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps MASON_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    Mirrors the user-modified protection bundled skills already get from ``mason update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: mason skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None, as_json: bool = False) -> None:
    """List bundled skills the user has edited (which `mason update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `mason update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   mason skills diff <name>[/]")
    c.print("[dim]Resume updates: mason skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: mason skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: mason skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            _print_error(c, f"Repo required. Usage: mason skills tap {action} owner/repo")
            return
    ...
    if target == "github":
        if not repo:
            _print_error(c, "--repo required for GitHub publish.\n"
                            "Usage: mason skills publish <path> --to github --repo owner/repo")
            return
        auth = GitHubAuth()
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_mason_home()}/.env "
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = call("post", f"{target_repo}/pulls", json={
            ...
            "body": f"Submitting the `{skill_name}` skill via Mason Skills Hub.\n\n"
                    f"This skill was scanned by the Mason Skills Guard before submission.",
...
def _snapshot_cli(args) -> None:
    ...
    if snap_action == "export":
        ...
        _console.print("Usage: mason skills snapshot [export|import]\n")


def _tap_cli(args) -> None:
    tap_action = getattr(args, "tap_action", None)
    if not tap_action:
        _console.print("Usage: mason skills tap [list|add|remove]\n")
        return
    do_tap(tap_action, repo=getattr(args, "repo", "") or getattr(args, "name", ""))


# `mason skills <action>` -> handler(args). Lambdas late-bind the do_* names so
# tests that patch("mason_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    """Router for `mason skills <subcommand>` — called from mason_cli/main.py."""
    handler = _CLI_ACTIONS.get(getattr(args, "skills_action", None))
    if handler is None:
        _console.print("Usage: mason skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'mason skills <command> --help' for details.\n")
        return
    handler(args)
