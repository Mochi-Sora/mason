...
from mason_cli.config import load_config, load_env, save_config


def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    """Attach the egress subcommand tree to a parent parser."""
    # dest='egress_command' keeps this tree disjoint from the inbound OAuth ``mason proxy``
    ...
    commands = [
        ...
        ("setup", "Interactive wizard: install + CA + mint tokens + write config", cmd_setup, [
            ...
            ("--no-restart", dict(dest="restart", action="store_false", help=(
                ...
                "`mason egress restart` yourself for changes to take effect."))),
        ]),
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Start:   [cyan]mason egress start[/cyan]\n"
        "  Restart: [cyan]mason egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]mason egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]mason egress status[/cyan]\n"
        "  Stop:    [cyan]mason egress stop[/cyan]\n"
        "  Disable: [cyan]mason egress disable[/cyan]"
    )
    return 0
...
def _setup_mint_tokens(console: Console, args: argparse.Namespace):
    ...
    if args.from_bitwarden:
        ...
        # Operators commonly keep provider keys only in ~/.mason/.env (loaded when the agent
        # runs, NOT exported into an interactive shell); backfill so discovery sees them.
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f"  [dim]Loaded {loaded} provider key name(s) from ~/.mason/.env for discovery.[/dim]")
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Mason sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _setup_restart_daemon(console: Console, args: argparse.Namespace, proxy_cfg: dict) -> None:
    ...
    if do_restart:
        try:
            ...
            console.print("  Run [cyan]mason egress start[/cyan] manually before launching new Docker sandboxes.")
        else:
        ...
        console.print(
            ...
            "changed.  Run [cyan]mason egress restart[/cyan] (or "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print("[yellow]proxy.enabled is false — run `mason egress setup` first.[/yellow]")
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            return _refuse(
                ...
                "back to env credentials with `mason egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            return _refuse(
                ...
                "`mason egress setup --no-bitwarden` to switch back to env-based credentials.",
            )
        if not (bw_cfg or {}).get("project_id"):
            return _refuse(
                console,
                "credential_source=bitwarden but secrets.bitwarden.project_id is empty.",
                "Run `mason secrets bitwarden setup` to configure the "
                "project, or switch back via `mason egress setup --no-bitwarden`.",
            )
    try:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    New upstream SECRETS still need ``mason egress restart``: the daemon reads credentials from
    ...
    console.print(
        ...
        "still need `mason egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `mason egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `mason egress start` before launching Docker sandboxes."])
    return "\n".join(lines)
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            "  iron-proxy is still running — stop it with [cyan]mason egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print("[yellow](no config generated — run `mason egress setup`)[/yellow]")
        return 1
...
def _bitwarden_env_names(console: Console) -> Optional[List[str]]:
    ...
    if not bw_cfg.get("enabled"):
        ...
        console.print("  Run `mason secrets bitwarden setup` first, or omit --from-bitwarden.")
        return None
    try:
...
def _load_env_file_into_environ() -> int:
    """Backfill known provider keys from ``~/.mason/.env`` into ``os.environ``; returns the count.
