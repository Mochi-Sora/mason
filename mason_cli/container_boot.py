...
01-mason-setup (the stage2 hook) has chowned the volume and seeded $MASON_HOME, but
...
def reconcile_profile_gateways(
    *, mason_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$MASON_HOME``): ``mason_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``mason gateway start`` (no ``-p``) targets.

    Without it, bare ``mason gateway start`` inside the container would land on ``s6-svc -u
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        mason_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(mason_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(mason_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", mason_home, default_prior_state, default_should_start))

    profiles_root = mason_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            # SOUL.md (seeded by `mason profile create`) is the "real profile" marker.
    ...
    if not dry_run:
        _write_reconcile_log(mason_home, actions)
    return actions


def _maybe_migrate_legacy_gateway_run_state(
    mason_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = mason_home / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("MASON_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the mason args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "mason":  # the wrapper re-execs `mason <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `mason -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with mason ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the mason user won't EACCES.
...
def _write_reconcile_log(mason_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $MASON_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted MASON_HOME both
    ...
    actions = reconcile_profile_gateways(mason_home=mason_home, scandir=scandir)
    for a in actions:
