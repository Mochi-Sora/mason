"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `mason tools`."""
...
from mason_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from mason_cli.nous_account import format_nous_portal_entitlement_message
from mason_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, NousSubscriptionFeatures
...
def _plugin_registry(module: str):
    ...
    try:
        ...
        from mason_cli.plugins import _ensure_plugins_discovered
...
def _visible_providers(
    ...
    from mason_cli.tools_config import get_nous_subscription_features
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from mason_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    from mason_cli.tools_config import _prompt_choice, _provider_env_ready, get_nous_subscription_features
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from mason_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from mason_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    ...
    from mason_cli.tools_config import _cfg_section
...
def _nous_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    ...
    if managed_feature:
        from mason_cli.nous_subscription import ensure_nous_portal_access
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    ...
    from mason_cli.tools_config import TOOL_CATEGORIES, get_nous_subscription_features
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``mason model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``mason model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from mason_cli.tools_config import _prompt_choice
    ...
    if not providers:
        _print_warning("  No authenticated providers found. Configure a provider first "
                       "with `mason model`, then re-run this.")
        return
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from mason_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
