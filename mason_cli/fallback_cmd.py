"""mason fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `mason model`), ``remove``, ``clear``.
"""
...
def _restore_auth_active_provider(value: Any) -> None:
    """Write back a snapshotted ``active_provider``; best-effort (user re-runs `mason model`), never fails the add."""
    try:
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  mason fallback add\n")
        return
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `mason model`, then append the selection to the chain."""
    from mason_cli.main import _require_tty, select_provider_and_model
    from mason_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `mason model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `mason fallback list` to view, or `mason fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``mason fallback [subcommand]``."""
