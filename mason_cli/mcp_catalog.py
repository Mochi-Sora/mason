...
Entries are added only by merging a PR into mason-agent; presence in ``optional-mcps/`` = Nous
...
from mason_constants import get_mason_home, get_optional_mcps_dir
from mason_cli._subprocess_compat import noninteractive_git_env
from mason_cli.colors import Colors, color
from mason_cli.config import load_config, save_config, get_env_value, save_env_value
from mason_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.mason/.env.
...
def _parse_auth(path: Path, raw: Any, name: str, http: bool) -> AuthSpec:
    ...
    if http and a_type == "api_key":
        ...
        from mason_cli.mcp_config import _env_key_for_server
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Mason understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Mason".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Mason) or ``invalid`` (malformed)."""
...
def _install_root() -> Path:
    ...
    root = get_mason_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.mason/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.mason/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by mason_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from mason_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from mason_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``mason mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`mason mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `mason mcp configure <name>` and unselect a tool (switches to include-mode).
        _write_tools_filter(name, "include", None)
        _say(
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        # Provider-mediated OAuth relies on the existing `mason auth <provider>` flow; surface
        ...
        _say(
            ...
            f"`mason auth {entry.auth.provider}` if you have not "
    ...
    from mason_cli.mcp_config import _save_mcp_server
    ...
    _say(
        ...
        f"Start a new Mason session to load its tools."
    )
