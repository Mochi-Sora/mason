"""`mason checkpoints` CLI subcommand.
...
from mason_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: mason checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``mason checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `mason checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('mason_cli.plugins_cmd', 'cmd_list'),
}
