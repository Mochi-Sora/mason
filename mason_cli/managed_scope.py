...
DISTINCT from ``mason_cli.config.is_managed()`` / ``MASON_MANAGED`` (a coarse package-manager
...
_DEFAULT_MANAGED_DIR = Path("/etc/mason")
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/mason`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``MASON_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$MASON_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/mason`` when it exists.
    A missing directory resolves to None — the common case, so it must be cheap + side-effect-free.
    """
    override = os.environ.get("MASON_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
    elif _under_pytest():
        return None
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    try:
        ...
        from mason_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
