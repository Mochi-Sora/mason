"""External-tool checks for mason doctor: terminal backends, git/rg, Node + agent-browser, npm audit, tool availability.
...
import subprocess
import sys
from mason_cli.doctor_platform import _system_package_install_cmd
from mason_cli.doctor_report import Finding, _fail_and_issue, check_bool, check_info, check_ok, check_warn, doctor_check
from mason_cli.vercel_auth import describe_vercel_auth
from mason_constants import agent_browser_runnable, is_termux as _is_termux
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("MASON_KANBAN_TASK")
            and bool(tools) and all(str(tool).startswith("kanban_") for tool in tools))


def _doctor_tool_availability_detail(toolset: str) -> str:
    """Optional explanatory suffix for toolsets whose doctor status needs context."""
    if toolset == "kanban" and not os.environ.get("MASON_KANBAN_TASK"):
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from mason_cli.doctor_state import _honcho_is_configured_for_doctor
...
def _check_vercel_backend(issues: list[str]) -> None:
    ...
    _require(importlib.util.find_spec("vercel") is not None, ("vercel SDK", "(installed)"),
             ("vercel SDK not installed", "(pip install 'mason-agent[vercel]')"),
             "Install the Vercel optional dependency: pip install 'mason-agent[vercel]'", issues)
...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    try:
        from mason_constants import is_container as _is_container
...
def _check_agent_browser(should_fix: bool) -> bool:
    ...
    Mirrors ``tools.browser_tool_install._find_agent_browser``'s own cascade (lazy npx or a global/Mason-managed
    ...
    try:
        ...
        # (or a global/Mason-managed install) at first use.
    ...
    if resolved and _is_npx_agent_browser_sentinel(resolved):
        ...
        if should_fix:
            # Can't tell whether npx's cache is warm — fire the same warm-up `mason update` does.
    ...
    if resolved and agent_browser_runnable(resolved):
        ...
        return True
    if resolved:
        # Almost always a dangling global symlink left by npm postinstall after `mason update` wiped node_modules.
...
def _check_lightpanda() -> None:
    ...
    if not used:
        ...
        check_info("Fix: pick Lightpanda in `mason tools` → Browser Automation, or set browser.engine: auto")
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    MASON_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    from mason_cli.doctor import PROJECT_ROOT
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable MASON_HOME mirror instead of the
...
def _check_tool_availability(should_fix: bool, f: Finding) -> None:
    ...
    if api_disabled or any(status != "ok" for status, _, _ in web_rows):
        f.issues.append("Run 'mason setup' to configure missing API keys for full tool access")
