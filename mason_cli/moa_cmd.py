...
from mason_cli.moa_config import DEFAULT_MOA_PRESET_NAME, normalize_moa_config
...
def _pick_slot(current: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if not providers:
        raise RuntimeError("No configured model providers found. Run `mason model` first.")
...
def _cmd_delete(cfg: dict, args) -> None:
    ...
    if not preset_name:
        raise SystemExit("Usage: mason moa delete <name>")
