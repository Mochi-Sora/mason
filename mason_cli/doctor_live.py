"""``mason doctor --live`` — opt-in bounded real-call tool-backend probes.

Opt-in only: these probes make real (cheap, metadata/read-only) network calls and may spend a
trivial amount of quota. They run ONLY when the user passes ``mason doctor --live``.
"""
...
from mason_cli.doctor import _section, check_info
from mason_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from mason_cli.doctor import MASON_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (MASON_HOME / "node" / "bin", MASON_HOME / "node", MASON_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung mason_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    from mason_cli.mcp_config import _probe_single_server
