"""CLI handlers for ``mason secrets bitwarden ...``."""
...
# module makes the ``mason update`` self-lock preflight defer. This module is registered
# parse-time from ``mason_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
from mason_cli._secrets_common import (
...
from mason_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``mason_cli.secrets_cli.bw`` directly.
    ...
    so ``import mason_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from mason_cli.main ──
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("[green]✓ Bitwarden Secrets Manager is enabled.[/green]  "
                  "Secrets will be pulled at the start of every Mason process.")
    console.print("  Status:  [cyan]mason secrets bitwarden status[/cyan]\n"
                  "  Refresh: [cyan]mason secrets bitwarden sync[/cyan]\n"
                  "  Disable: [cyan]mason secrets bitwarden disable[/cyan]")
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]mason secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token:
        console.print(f"\n  [yellow]Enabled but {token_env} is not set — Mason will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `mason secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `mason secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print("\n  This was a dry-run — secrets are picked up automatically on the "
                      "next [cyan]mason[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
...
_PROJECT_LIST_HINTS = (
    (("invalid_client", "400 bad request"),
     ...
     "[cyan]mason secrets bitwarden setup[/cyan] and pick EU or "
