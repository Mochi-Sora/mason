"""Dependency-light venv recovery that runs BEFORE mason_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``mason_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``mason_cli.main`` to become importable again (force-reinstall of
...
# ``mason update`` renames the live ``mason*.exe`` shims aside (``mason.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``mason`` on PATH, and the command that would repair it IS ``mason update``. The
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which Mason
    vendors (``~/.mason/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".mason", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``mason acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``mason_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``mason_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls. Only acts when a recovery marker from a
    prior ``mason update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `mason update`: CLI and Desktop retries keep
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``mason update`` (or
    ...
    venv extension mapped; this process, running before ``mason_cli.main``'s third-party imports, maps
    ...
    try:
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `mason update` was interrupted mid-install — "
            ...
            print("  The next launch will retry; mason will keep working from "
