"""Pydantic request/response models for the Mason dashboard web server."""
...
class ImportRequest(BaseModel):
    archive: str
    # --force: the spawned `mason import` has stdin=DEVNULL, so its "Continue? [y/N]" prompt would
...
class ProfileCreate(BaseModel):
    ...
    # Installed async via `mason -p <name> skills install` (skills_hub.SKILLS_DIR is import-time-bound,
    # so MASON_HOME can't redirect it); PIDs go back for the UI to poll.
    hub_skills: List[str] = []
...
class ProfileExport(BaseModel):
    ...
    output: str = ""  # archive path; empty → a staging path under MASON_HOME
