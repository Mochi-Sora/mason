...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from mason_cli.main import _resolve_last_session
    target = (
    ...
    print(f"\nResume this session with:\n  mason --tui --resume {target}")
    if title:
        print(f'  mason --tui -c "{title}"')
    print(f"\nSession:        {target}")
    if title:
        print(f"Title:          {title}")
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @mason/ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@mason" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/mason-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/mason-ink/package.json",
    "packages/mason-ink/index.js",
    "packages/mason-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``MASON_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("MASON_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``MASON_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from mason_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("MASON_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MASON_HOME": mason_home},
    ...
    extras += [Path(mason_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(mason_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if mason_cli_dir is None:
        mason_cli_dir = Path(__file__).parent
    bundled = mason_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``mason update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``mason update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("MASON_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Mason checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `mason update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Mason checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `mason --tui`\n"
        "If the checkout is still inconsistent, run `mason update --force`.",
        file=sys.stderr)
    sys.exit(1)
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``MASON_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$MASON_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("MASON_NODE")
    ...
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from mason_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        from mason_constants import with_mason_node_path
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_mason_node_path()))
    ...
    if result.returncode != 0:
        ...
        from mason_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (MASON_TUI_DIR prebuilt or esbuild)."""
    from mason_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("MASON_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MASON_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MASON_TUI_DIR (e.g. `unset MASON_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    # mason_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   mason_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @mason/ink resolves through
        # packages/mason-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "mason-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from mason_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from mason_cli.main import PROJECT_ROOT
    src_root = str(env.get("MASON_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["MASON_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("MASON_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["MASON_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("MASON_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["MASON_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["MASON_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a mason process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="mason-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["MASON_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["MASON_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("MASON_MODEL", model), ("MASON_INFERENCE_MODEL", model),
        ("MASON_TUI_PROVIDER", provider), ("MASON_INFERENCE_PROVIDER", provider),
        ("MASON_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("MASON_TUI_SKILLS", skills_value),
        ("MASON_TUI_QUERY", query), ("MASON_TUI_IMAGE", image),
        ("MASON_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("MASON_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("MASON_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("MASON_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("MASON_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # MASON_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `mason --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("MASON_TUI_RESUME", None)
    if resume_session_id:
        env["MASON_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `mason update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MASON_KANBAN_BOARD`` so in-process tools and shelled-out
    ``mason kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``mason kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``mason kanban boards switch`` from another session can flip the file mid-turn, so the same
    ...
    """
    if os.environ.get("MASON_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from mason_cli.kanban_db import get_current_board
        os.environ["MASON_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.mason/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``MASON_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``mason chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
