...
- **gateway profiles**, relaunched through the existing per-profile
  ``mason_cli.main -p <profile> gateway restart`` command; and
- **``mason-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``mason serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``mason serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/mason-serve``,
``system/mason-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "MASON_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_MASON_GATEWAY", "MASON_GATEWAY", "MASON_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
_PROFILE_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^mason-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "mason-serve*"
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "mason_cli.main", "-p", profile, "gateway", "restart"]
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return ("mason-gateway.service", "gateway.service", "gateway-default.service")
    return (f"mason-gateway-{profile}.service", f"gateway-{profile}.service")
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    ``systemctl`` comes from ``shutil.which`` so this module never imports a Mason platform helper —
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    if result is None:
        return []
    # The glob is a systemd pattern, not a name gate (`mason-serve*` also matches
    # `mason-server.service`): require the exact base unit or the profile family.
    units: list[str] = []
...
def _normalized_skips(skip_units: Iterable[Any]) -> tuple[set[tuple[str, str]], set[str]]:
    """Split already-settled units into scope-qualified and legacy entries (a bare ``"mason-serve"``
...
def restart_serve_units(
    *, skip_units: Iterable[Any] = (), run: Callable[..., Any] = subprocess.run, sleep: Callable[[float], Any] = time.sleep
    ...
    """Restart every active ``mason-serve*`` systemd unit from this process.

    Units are enumerated from systemd, never from the update inventory, so a manually launched or
    Desktop-owned ``mason serve`` (no unit) structurally cannot be touched here.
    """
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # scope (`mason-serve.service` can exist in BOTH managers; settling one says nothing about
