...
def _strip_mason_path_lines(content: str) -> str:
    """Drop the ``# Mason Agent`` marker (+ its PATH line) and any mason PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Mason Agent' in line or '# mason-agent' in line:
            skip_next = True
            continue
        if skip_next and ('mason' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'mason' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Mason PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_mason_path_lines(content)
...
def remove_wrapper_script():
    """Remove the mason wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a mason_cli / mason-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('mason_cli' in content or 'mason-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("mason", "mason-acp", "mason-agent"))
...
def remove_node_symlinks(mason_home: Path) -> list:
    ...
    node_dir = (mason_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from mason_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from mason_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from mason_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from mason_cli import gateway_windows as gw
...
# MASON_HOME / MASON_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\mason\git\{cmd,bin,usr\bin}, ...\mason\node), PortableGit + Node copies
...
def _mason_path_markers(mason_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Mason-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(mason_home).rstrip("\\/")
    subs = ("mason-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(mason_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Mason-owned entries from User-scope PATH in the registry (see ``_mason_path_markers``)."""
    markers = tuple(m.lower() for m in _mason_path_markers(mason_home, include_managed_bin=include_managed_bin))
...
def remove_mason_env_vars_windows() -> list[str]:
    """Delete MASON_HOME and MASON_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("MASON_HOME", "MASON_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(mason_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``mason_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (mason_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``mason`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from mason_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from mason_constants import get_default_mason_root
        bin_dir = get_default_mason_root() / "bin"
    except Exception as e:
...
def _is_default_mason_home(mason_home: Path) -> bool:
    """Return True when ``mason_home`` points at the default (non-profile) root."""
    try:
        from mason_constants import get_default_mason_root
        return mason_home.resolve() == get_default_mason_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``mason -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current MASON_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m mason_cli.main` (the `mason` wrapper may be half-gone).
    mason_invocation = [sys.executable, "-m", "mason_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                mason_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's MASON_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``mason uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from mason_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    mason_home = get_mason_home()
    summary = gui_install_summary(mason_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Mason Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Mason Chat GUI installation was found.")
        print(f"  Checked: {mason_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Mason agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(mason_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Mason agent at {mason_home / 'mason-agent'}")
        print(f"  • Your config, sessions, and secrets under {mason_home}")
        print()
    ...
    print()
    uninstall_gui(mason_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Mason agent is still installed. Run 'mason' to use the CLI,")
    print("or 'mason uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.mason/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, mason_home=mason_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie MASON_HOMEs and systemd units behind.
    is_default_profile = _is_default_mason_home(mason_home)
    ...
    print()
    _print_box("│            ⚕ Mason Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {mason_home / 'config.yaml'}")
    print(f"  Secrets: {mason_home / '.env'}")
    print(f"  Data:    {mason_home / 'cron/'}, {mason_home / 'sessions/'}, {mason_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, MASON_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Mason data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Mason code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, mason_home=mason_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, mason_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Mason PATH entries from shell configs / Windows User PATH")
    print("  • Mason wrapper scripts and Mason-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Mason config/data: {mason_home}")
    else:
        print(f"  • Mason config/data: {mason_home}")
        profiles = _discover_named_profiles() if _is_default_mason_home(mason_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$MASON_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: mason_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; mason\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_mason_home(mason_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(mason_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Mason-owned PATH entries in User environment"),
        (windows, "Removing MASON_HOME / MASON_GIT_BASH_PATH User env vars...",
         remove_mason_env_vars_windows, "Removed User env var: {}", "No Mason-set User env vars to remove"),
        (True, "Removing mason command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows mason launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows mason launchers found"),
        (True, "Removing Mason-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(mason_home), "Removed {}", "No Mason-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE MASON_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from mason_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(mason_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(mason_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.mason/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(mason_home)
    else:
        log_info(f"Keeping configuration and data in {mason_home}")
    ...
    print()
    print("Thank you for using Mason Agent! ⚕")
    print()


_REINSTALL_HINT = {
    True: "  reinstall from your Mason repo (see README.md)",
    False: "  reinstall from your Mason repo (see README.md)"}
...
def main(argv=None) -> int:
    """``python -m mason_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``mason_constants`` + ``mason_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m mason_cli.uninstall")
    parser.add_argument(
