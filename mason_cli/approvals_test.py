"""``mason approvals test`` — dry-run approval verdict for a command.
...
def approvals_test_command(args) -> int:
    """Handle ``mason approvals test <command...>``. Returns the exit code."""
    ...
    if words and words[0] == "--":
        words = words[1:]
    if not words:
        print("usage: mason approvals test [--env-type TYPE] [--json] -- <command...>")
        return EXIT_USAGE
