...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("MASON_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``mason gateway run --replace`` / ``gateway restart`` invoked from a shell
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from mason_cli.sqlite_safe_read import read_header_bytes_preopen
...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    ``mason kanban repair`` can pick its exit code. ``OperationalError``
...
def _open_configured(path: Path, under_lock) -> tuple[sqlite3.Connection, Any]:
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # DELETE with one ERROR log (see mason_state_wal._WAL_INCOMPAT_MARKERS).
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    :func:`kanban_db_path` (``MASON_KANBAN_DB`` -> ``MASON_KANBAN_BOARD`` ->
    ...
    # holder (e.g. an external `mason kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        # See #12508.
        from mason_state import preflight_db_writability
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"mason_nested_{secrets.token_hex(8)}"
