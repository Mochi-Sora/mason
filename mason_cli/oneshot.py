...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `mason tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(MASON_YOLO_MODE=1). Model/provider mirror `mason chat`: both optional; only --model → auto-detect
...
def _configured_mcp_servers() -> tuple[set[str], set[str]]:
    ...
    try:
        ...
        from mason_cli.tools_config import _parse_enabled_flag
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"mason -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in _ALL_TOOLSETS for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "mason -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"mason -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "mason -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )
    if not valid:
        return None, "mason -z: --toolsets did not contain any valid toolsets.\n"
    return valid, None
...
def run_oneshot(
    ...
    Model/provider fall back to ``MASON_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    env_model_early = os.getenv("MASON_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "mason -z: --provider requires --model (or MASON_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"mason -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        if result.get("failed") or result.get("partial"):
            return 2
        real_stderr.write("mason -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
    return 0
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``MasonCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("MASON_INFERENCE_MODEL", "").strip()
    ...
    try:
        from mason_cli import model_switch as _ms
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("MASON_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
def _run_agent(
    ...
    from mason_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        agent = AIAgent(
            ...
            # MASON_INTERACTIVE (never set), hook approval via MASON_ACCEPT_HOOKS=1, dangerous
            # commands via MASON_YOLO_MODE=1, skill secret capture degrades gracefully.
