...
:mod:`mason_cli.runtime_provider`; origin-internal collaborators
...
from mason_cli.providers import custom_provider_aliases, custom_provider_slug
from utils import base_url_hostname

logger = logging.getLogger("mason_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``mason_cli.runtime_provider.*`` apply."""
    import mason_cli.runtime_provider as origin
    return origin
...
def canonical_custom_identity(*, base_url: Optional[str] = None, config_provider: Optional[str] = None,
    ...
    (3) the configured provider (arg, ``model.provider``, ``MASON_INFERENCE_PROVIDER``) when it
    names a real entry."""
    rp = _rp()
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("MASON_INFERENCE_PROVIDER", "").strip()
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from mason_cli.models import normalize_opencode_base_url, opencode_model_api_mode
