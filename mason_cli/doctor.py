"""``mason doctor`` — diagnose (and with --fix, repair) a Mason install.
...
from mason_cli.config import get_env_path, get_mason_home, get_project_root
...
_DHH = display_mason_home()  # user-facing display path (e.g. ~/.mason or ~/.mason/profiles/coder)

# Load environment variables from ~/.mason/.env so API key checks work
_env_path = get_env_path()
load_mason_dotenv(mason_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from mason_cli.colors import Colors, color
from mason_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from mason_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from mason_cli.doctor_tools import _safe_which

from mason_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from mason_cli.doctor_platform import (
    _check_certificates,
...
from mason_cli.doctor_tools import (
    _check_git_and_rg,
...
from mason_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from mason_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nous Portal auth", get_nous_auth_status_local())
        # Native OAuth is Mason' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from mason_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    """`mason doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from mason_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.mason/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'mason doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("MASON_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from mason_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('mason_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('mason_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('mason_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('mason_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('mason_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('mason_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('mason_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('mason_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('mason_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('mason_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('mason_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('mason_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('mason_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('mason_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('mason_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('mason_cli.doctor_config', 'report_deprecated_config_and_env'),
}
