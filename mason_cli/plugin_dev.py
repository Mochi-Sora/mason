"""Runtime-backed validation behind ``mason plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    MASON_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a mason-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="mason-plugin-doctor-")))
        ...
        stack.enter_context(patch.dict(os.environ, {
            "MASON_HOME": str(home),
            "MASON_BUNDLED_PLUGINS": str(bundled),
            "MASON_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"Mason discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
...
def _is_plugin_module(name: str) -> bool:
    return name == "mason_plugins" or name.startswith("mason_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_mason_home() / "plugins" / raw)
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".mason" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from mason_cli.plugins import SUPPORTED_MANIFEST_VERSION
    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Mason supports "
    ...
    if missing:
        report.warning(
            ...
            + " — Mason never auto-installs plugin dependencies; install manually: pip install "
            + " ".join(f"'{m}'" for m in missing))

    schema = getattr(manifest, "config_schema", {}) or {}
    if schema:
        from mason_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Mason' real scanner and registration path."""
    try:
