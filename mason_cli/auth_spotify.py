...
Re-exported from ``mason_cli/auth.py`` (patch targets unchanged); origin helpers are imported
lazily per function so ``mason_cli.auth.<helper>`` patches still intercept and no cycle forms.
"""
...
def _spotify_client_id(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    client_id = _spotify_setting(
        state, "client_id", ("MASON_SPOTIFY_CLIENT_ID", "SPOTIFY_CLIENT_ID"), "", explicit=explicit,
    )
    if client_id:
        return client_id
    raise _spotify_err(
        "Spotify client_id is required. Set MASON_SPOTIFY_CLIENT_ID or pass --client-id.",
...
def _spotify_redirect_uri(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "redirect_uri", ("MASON_SPOTIFY_REDIRECT_URI", "SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "api_base_url", ("MASON_SPOTIFY_API_BASE_URL",), DEFAULT_SPOTIFY_API_BASE_URL, strip_slash=True,
    )


def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "accounts_base_url", ("MASON_SPOTIFY_ACCOUNTS_BASE_URL",), DEFAULT_SPOTIFY_ACCOUNTS_BASE_URL,
        strip_slash=True,
    )
...
def _refresh_spotify_oauth_state(state: Dict[str, Any], *, timeout_seconds: float = 20.0) -> Dict[str, Any]:
    ...
    if not refresh_token:
        raise _spotify_err(
            "Spotify refresh token missing. Run `mason auth spotify` again.",
    ...
    payload = _spotify_token_post(
        ...
        failed_suffix=" Run `mason auth spotify` again.", relogin_required=True,
    )
...
def resolve_spotify_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from mason_cli.auth import _auth_store_lock, _is_expiring, _load_auth_store, _load_provider_state, _quarantine_flat_oauth_state, _refresh_spotify_oauth_state, _save_auth_store, _store_provider_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "spotify")
        if not state:
            raise _spotify_err(
                "Spotify is not authenticated. Run `mason auth spotify` first.", "spotify_auth_missing", relogin=True,
            )
    ...
    if not access_token:
        raise _spotify_err(
            "Spotify access token missing. Run `mason auth spotify` again.",
...
def get_spotify_auth_status() -> Dict[str, Any]:
    from mason_cli.auth import _is_expiring, get_provider_auth_state
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    """Walk the user through creating a Spotify developer app; persist the client_id to ~/.mason/.env."""
    from mason_cli.auth import _is_remote_session
    ...
    print(
        ...
        "       App name:     anything (e.g. mason-agent)\n"
    ...
    save_env_value("MASON_SPOTIFY_CLIENT_ID", raw)
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("MASON_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print("\nSaved MASON_SPOTIFY_CLIENT_ID to ~/.mason/.env\n")
    return raw


def login_spotify_command(args) -> None:
    from mason_cli.auth import _auth_store_lock, _can_open_graphical_browser, _is_remote_session, _load_auth_store, _print_loopback_ssh_hint, _save_auth_store, _store_provider_state, get_provider_auth_state
    existing_state = get_provider_auth_state("spotify") or {}

    # No client_id anywhere -> wizard instead of "MASON_SPOTIFY_CLIENT_ID is required".
    try:
    ...
    print(
        ...
        f"Open this URL to authorize Mason:\n{authorize_url}\n\nFull setup guide: {SPOTIFY_DOCS_URL}\n"
    )
