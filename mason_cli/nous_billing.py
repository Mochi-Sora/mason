...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    env = os.getenv("MASON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into Nous Portal — run `mason portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
