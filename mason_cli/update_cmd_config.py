"""Post-``mason update`` config-schema migration for the active profile and every sibling.
Names are re-imported by ``update_cmd`` (``mason_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _reload_config_modules() -> None:
    ...
    for mod_name in (
        "mason_cli.config_defaults", "mason_cli.config", "mason_cli.config_migrations",
...
def _run_config_check_fresh() -> tuple:
    ...
    from mason_cli.update_cmd import _reload_config_modules
    _reload_config_modules()
    from mason_cli.config import check_config_version
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local MASON_HOME override (never ``os.environ``)
    ...
    profile, but ``mason update`` historically migrated only the active profile's config — siblings drifted
    versions until their gateway hit a config the new code couldn't read.
    """
    from mason_cli.update_cmd import _run_config_check_fresh, _run_migrate_config_fresh
    migrated: list[tuple[str, int, int]] = []
    with _best_effort('Sibling profile enumeration failed: %s'):
        from mason_constants import (
            get_process_mason_home, reset_mason_home_override, set_mason_home_override)
        from mason_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
        active_home = get_process_mason_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_mason_home_override(entry)
            try:
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling cron auto-restore check failed: %s'):
        from mason_cli.backup import restore_cron_jobs_all_profiles
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from mason_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from mason_cli.update_cmd import _gateway_prompt
    if assume_yes:
    ...
    try:
        ...
        print(
            ...
            "Run 'mason config migrate' manually to configure.")
        return "n"
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    _reload_config_modules()
    from mason_cli.config import get_missing_env_vars, get_missing_config_fields
    ...
    try:
        ...
        print("     Run 'mason config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'mason config migrate' to retry.")
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if unattended and missing_env:
                print("  ℹ API keys require manual entry: mason config migrate")
        else:
            print()
            print("Skipped. Run 'mason config migrate' later to configure.")
    else:
        print("  ✓ Configuration is up to date")

    # The migration above touched only the active profile; run the same NON-INTERACTIVE
    # migration per sibling home via the context-local MASON_HOME override (never os.environ).
