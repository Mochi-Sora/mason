...
from mason_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "mason_cli.commands").
logger = logging.getLogger("mason_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``mason skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/mason <command>`` on Slack only: the registry
...
_SLACK_VIA_MASON_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_MASON_ONLY are skipped. ``/mason`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://mason-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/mason`` handler, plugin commands included."""
