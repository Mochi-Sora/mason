"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``mason setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on mason_cli.setup, so those are imported lazily per function."""
...
def _profile_name_from_mason_home(mason_home) -> str | None:
    """Return the active profile name when MASON_HOME is a profile dir."""
    return mason_home.name if mason_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from mason_cli.setup import get_mason_home
    try:
        from mason_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_mason_home(Path(get_mason_home()))
...
def _declines_reconfigure(env_var: str, label: str, question: str) -> bool:
    ...
    from mason_cli.setup import get_env_value, print_info, prompt_yes_no
...
def _save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from mason_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from mason_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from mason_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from mason_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from mason_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from mason_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from mason_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from mason_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where Mason delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from mason_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if _declines_reconfigure("BLUEBUBBLES_SERVER_URL", "BlueBubbles", "Reconfigure BlueBubbles?"):
        return
    _info("Connects Mason to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from mason_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    print()
    _info("   Full guide: README.md in your Mason checkout", None)
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   Open config in your editor:  mason config edit",
          "   Open config in your editor:  mason config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from mason_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     mason config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from mason_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from mason_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from mason_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Mason from anywhere.",
    ...
    if not selected:
        print_info("No platforms selected. Run 'mason setup gateway' later to configure.")
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `mason import` /
    # `mason setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from mason_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
