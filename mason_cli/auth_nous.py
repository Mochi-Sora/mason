...
Split out of ``mason_cli/auth.py``; origin helpers are imported lazily inside each function
so ``mason_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from mason_cli.auth_codex import _pool_entries
from mason_cli.auth_constants import (
...
if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from mason_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "mason_cli.auth").
logger = logging.getLogger("mason_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: mason auth add nous"
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("MASON_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from mason_cli.nous_account import (
...
def _nous_inference_env_override() -> Optional[str]:
    ...
    """
    from mason_cli.auth import _optional_base_url
    return _optional_base_url(os.getenv("NOUS_INFERENCE_BASE_URL"))


def _nous_portal_env_override() -> Optional[str]:
    """``MASON_PORTAL_BASE_URL`` / ``NOUS_PORTAL_BASE_URL`` override or None.
    ...
    return _optional_base_url(
        os.getenv("MASON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL"))
...
def _nous_invoke_jwt_status(
    ...
    from mason_cli.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from mason_cli.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from mason_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from mason_cli.auth import _nonempty_str
...
def _nous_shared_auth_dir() -> Path:
    """Directory of the shared Nous token store: ``MASON_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``mason --profile X auth add nous --type
    ...
    """
    override = os.getenv("MASON_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from mason_constants import get_default_mason_root
    return get_default_mason_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt (mirrors the _auth_file_path() guard): under pytest, refuse a path under the real
    # user's Mason root so a test that forgot MASON_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from mason_constants import get_default_mason_root
        real_home_shared = (
            get_default_mason_root() / "shared" / NOUS_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set MASON_SHARED_AUTH_DIR to a tmp_path in your test fixture.")
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    """
    from mason_cli.auth import _file_lock
    try:
        lock_path = _nous_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No MASON_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from mason_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    """
    from mason_cli.auth import _nonempty_str, _write_private_file_atomic
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    """
    from mason_cli.auth import _auth_file_path
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    # self-heal hook) redeemed Mason's refresh_token without persisting the rotated token, so the
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        description = (
            ...
            "custom self-heal hook, or another Mason install sharing "
            "~/.mason/auth.json) called POST /api/oauth/token with Mason's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Mason may call the "
            "refresh endpoint. For health checks, use `mason auth status` "
            "instead.\n"
            "Re-authenticate with: mason auth add nous")
        relogin = True
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from mason_cli.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from mason_cli.auth import _coerce_ttl_seconds
...
def fetch_nous_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    for item in data:
        ...
        # Mason models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "mason" not in model_id.lower():
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    ...
    it.
    """
    from mason_cli.auth import _save_active_provider_state, _write_shared_nous_state
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from mason_cli.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (
        _optional_base_url(state.get("portal_base_url")) or os.getenv("MASON_PORTAL_BASE_URL")
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from mason_cli.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    """
    from mason_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("nous")
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from mason_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _pool_first_oauth_status(
    ...
    Pool first (where `mason auth` / `mason model` store device_code tokens), then
...
def _nous_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    portal_base_url = (
        portal_base_url or os.getenv("MASON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    ...
    if _is_remote_session():
        open_browser = False
    print(f"Starting Mason login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `mason model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from mason_cli.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    from mason_cli.models_pricing import (
...
def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    ...
    """
    from mason_cli.auth import _prompt_yes_no, _read_shared_nous_state
    shared = _read_shared_nous_state()
    if not shared:
        return None
    try:
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    ...
    from mason_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("MASON_CA_BUNDLE")
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `mason model` again to switch to Nous Portal.")
            return
