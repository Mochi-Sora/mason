"""mason memory setup|status — configure memory provider plugins."""
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``mason memory setup`` — if the update-time refresh only reinstalled the declared ``hindsight-client``,
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_mason_home() / "hindsight" / "config.json"
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    no-ops when nothing drifted) — how ``mason update`` heals a provider after a venv rebuild.
    ...
    satisfied ranges. This is how ``mason update`` heals the active memory provider after a venv
    ...
    # install_specs routes to the durable target on sealed hosted images (MASON_LAZY_INSTALL_TARGET)
...
def _post_setup_hook(provider, config: dict) -> bool:
    ...
    if hasattr(provider, "post_setup"):
        provider.post_setup(str(get_mason_home()), config)
        return True
    return False
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'mason memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.mason/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, str(get_mason_home()))
        except Exception as e:
...
def _write_env_vars(
    env_writes: dict, mason_home: str | os.PathLike[str] | None = None) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``MASON_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``mason_home`` is applied via the
    ...
    token = set_mason_home_override(mason_home) if mason_home is not None else None
    try:
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        if url and not is_set:
                            line += f"  → {url}"
                        print(line)
                print("  Note: systemd/gateway services do not inherit ~/.mason/.env —")
                print("        set any variables above in the service environment.")
        else:
            print("\n  Plugin:    NOT installed ✗")
            print(f"  Install the '{provider_name}' memory plugin to ~/.mason/plugins/")
