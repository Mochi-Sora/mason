"""CLI subcommand: ``mason pets <subcommand>``."""
...
def _cmd_list(args) -> int:
    ...
    if args.installed:
        ...
        if not pets:
            print("No pets installed. Try: mason pets install boba")
            return 0
    ...
    print("\nInstall one with: mason pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if args.select or not _has_active_pet():
        ...
        print(f"  Make it active with: mason pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: mason pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: mason pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    if pet is None:
        _err("✗ no pet to show — run: mason pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        print("  → no pets installed. Run: mason pets install boba")
        ok = False
    elif active is None:
        print("  → active pet unresolved. Run: mason pets select <slug>")
        ok = False
    elif not enabled:
        print("  → pet display is disabled. Run: mason pets select " + active.slug)
