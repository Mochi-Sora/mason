"""Interactive setup wizard for Mason Agent (config lives in ~/.mason/).
...
``mason_cli.setup.<name>`` keep working.
"""
...
from mason_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_mason_home is
# imported lazily at call sites (stale-module safety during mason update).
from mason_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_mason_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_mason_home,
)
from mason_cli.colors import Colors, color
from mason_cli.cli_output import print_error, print_info, print_success, print_warning
...
_DOCS_BASE = "https://mason-agent.nousresearch.com/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Mason Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    _info("The interactive wizard cannot be used here.", None,
          "Configure Mason using environment variables or config commands:",
          "  mason config set model.provider custom",
          "  mason config set model.base_url http://localhost:8080/v1",
          "  mason config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'mason setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("mason_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from mason_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``mason model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from mason_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``MASON_NONINTERACTIVE=1`` (``mason_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("MASON_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``MASON_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from mason_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'mason setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``mason model`` flow (one code
    ...
    from mason_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: mason model")
    ...
    # keep safe defaults (configure via `mason auth add` / `mason setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into MASON_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("MASON_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          ...
          "  Run `mason setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.mason/logs/tool_calls.log (gateway only)",
)
_SESSION_RESET_HELP = (
...
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives MASON_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`mason setup tools` == `mason tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from mason_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from mason_cli.observability.shared_metrics import SharedMetricsStore
        from mason_cli.observability.shared_metrics_sender import reconcile_send_consent
        from mason_cli.sqlite_util import write_txn
...
# mason_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from mason_cli.setup_tts import setup_tts  # noqa: E402
from mason_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from mason_cli.setup_platforms import setup_gateway  # noqa: E402
from mason_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from mason_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from mason_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``mason setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚕ Mason Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, mason_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {mason_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'mason config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from mason_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``mason setup <section>`` from SETUP_SECTIONS."""
    ...
    from mason_cli.auth import get_active_provider
    is_existing = bool(get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_BASE_URL")
                       or get_active_provider() is not None)
    _print_banner("│             ⚕ Mason Agent Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Mason Agent installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, mason_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have Mason configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'mason setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(mason_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Mason?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from mason_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, mason_home, is_existing))])
            return
    _run_full_setup(config, mason_home, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, mason_home)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('mason_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('mason_constants', 'get_optional_skills_dir'),
