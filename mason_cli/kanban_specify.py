...
``mason kanban specify [task_id | --all]`` asks the auxiliary LLM for a
tightened title + concrete body for a Triage task, then flips it
``triage -> todo`` via ``kanban_db.specify_triage_task``.

Mirrors ``mason_cli/goals.py``: same aux-client pattern, same "empty config
...
MASON_KANBAN_SPECIFY_MAX_TOKENS = max(1500, env_int("MASON_KANBAN_SPECIFY_MAX_TOKENS", 6000))

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Mason Agent board.
...
def _profile_author(default: str = "specifier") -> str:
    """Mirror of ``mason_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return os.environ.get("MASON_PROFILE") or os.environ.get("USER") or default
...
def specify_task(
    ...
    *,
    ...
    raw, reason = _call_aux(
        ...
        max_tokens=MASON_KANBAN_SPECIFY_MAX_TOKENS, timeout=timeout or 120,
    )
    if raw is None:
        return SpecifyOutcome(task_id, False, reason)
    raw = raw.strip()
