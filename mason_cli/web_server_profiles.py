...
from mason_cli.config import DEFAULT_CONFIG, get_process_mason_home
from mason_cli.web_models import MCPServerCreate
from mason_cli.web_server_gateway import _ACTION_LOG_FILES
from mason_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _mason_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_mason_home()`` at call
    time) to ``path`` for the block via the context-local MASON_HOME override."""
    from mason_constants import set_mason_home_override, reset_mason_home_override
    token = set_mason_home_override(str(path))
    try:
        yield
    finally:
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_mason_home().resolve()
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_mason_home()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (MASON_HOME-scoped).
    ...
    from mason_cli.mcp_config import _save_bearer_auth_token
    written = 0
    with _mason_home_scope(profile_dir):
        cfg = load_config()
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_mason_home()`` at call
    ...
    ``get_mason_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late MASON_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_mason_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.MASON_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.MASON_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.MASON_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local MASON_HOME
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``mason`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_mason_action``
