...
# the CLI's copy/exit/clear bindings also claim ``a-c``/``a-d``/``a-l`` (mason-ink reports Alt as
...
# ``a-l`` via the action-modifier lookup, and mason-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    mason-ink rejects them; a bare key is refused by the TUI parser.
...
def _debug(msg: str) -> None:
    """MASON_VOICE_DEBUG=1 breadcrumb on stderr (the TUI gateway shows it as a gateway.stderr
...
def _speak_whole_file(text: str) -> None:
    ...
    os.makedirs(os.path.join(tempfile.gettempdir(), "mason_voice"), exist_ok=True)
    mp3_path = os.path.join(tempfile.gettempdir(), "mason_voice", f"tts_{time.strftime('%Y%m%d_%H%M%S')}.mp3")
