"""``mason kanban …`` — dispatch (``kanban_command``), task-verb handlers, ``run_slash`` for ``/kanban``.
...
from mason_cli import kanban_swarm as ks
from mason_cli.kanban_output import (
...
from mason_cli.kanban_boards import _dispatch_boards
from mason_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from mason_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: mason_cli.main, run_slash)
...
def _check_dispatcher_presence(mason_home: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this MASON_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``mason_home`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different MASON_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=mason_home, use_cache=False)
    except Exception:
    ...
    if pid:
        return (False, "Gateway is running but kanban.dispatch_in_gateway=false in "
                ...
                "standalone daemon (`mason kanban daemon --force`).")
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``mason kanban …``; returns a shell-style exit code."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: mason kanban <action> [options]\n"
                  "Run 'mason kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    # `--board <slug>` pins MASON_KANBAN_BOARD for the duration of this call so it inherits the
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            return _err(f"kanban: board {normed!r} does not exist. "
                        f"Create it with `mason kanban boards create {normed}`.")
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh MASON_HOME.
        try:
            kb.init_db()
...
def _profile_author() -> str:
    ...
    for env in ("MASON_PROFILE_NAME", "MASON_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("MASON_DELEGATED_CHILD_CONTEXT"))
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.mason/profiles/.\n"
              "Create one with `mason -p <name> setup` before assigning tasks.")
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, data):
        return 0
    if not data:
        print("(no assignees — create a profile with `mason -p <name> setup`)")
        return 0
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    raw = os.environ.get("MASON_KANBAN_RUN_ID")
    if os.environ.get("MASON_KANBAN_TASK") != task_id or not raw:
        return None
    try:
        return int(raw)
    except ValueError:
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from mason_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from mason_cli import kanban_decompose as decomp
