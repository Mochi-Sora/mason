"""Post-update maintenance for ``mason update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``mason_cli.update_cmd.<name>``
...
_UPDATE_RUNTIME_RELOAD_MODULES = "mason_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "mason_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"mason_cli", "mason_cli.main", "mason_cli.mason_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "mason_cli.update_"
...
def _purge_stale_mason_modules() -> None:
    """Evict every cached Mason module after the checkout changed in-place. Never raises.
    ...
    with _best_effort('Could not purge stale Mason modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Mason module(s) after checkout update", len(purged))
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  mason curator run --dry-run")
    print("  Pause it:     mason curator pause")
    print("  Docs:         README.md in your Mason checkout")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print(
            "  A previous `mason sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    mason sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  mason sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: mason curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from mason_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("mason_cli._subprocess_compat", "mason_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from mason_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
    ...
    print("    mason dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from mason_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("MASON_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== mason-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from mason_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Mason runtime."""
    from mason_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Mason venv with a uv-managed Python, restart Mason, then verify with `mason doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from mason_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `mason desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `mason "
                "update` again; if SQLite is unchanged, rebuild the Mason venv "
                "with a uv-managed Python, restart Mason, then verify with "
                "`mason doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from mason_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from mason_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (mason gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Mason) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from mason_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from mason_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: mason skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v mason'`` resolves."""
    from mason_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/mason).
    fhs_link = Path("/usr/local/bin/mason")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `mason gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``mason-acp`` launcher next to ``mason`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``mason acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$MasonHome\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        mason_cmd = bin_dir / "mason"
        acp_cmd = bin_dir / "mason-acp"
        try:
            if not (mason_cmd.is_file() or mason_cmd.is_symlink()):
                continue
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_mason_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_mason_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from mason_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from mason_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import mason_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip MASON_HOME under ``backups/`` (restorable via ``mason import``). Never raises."""
    try:
    ...
    from mason_cli.sizefmt import format_bytes
    # display_mason_home so the user sees ~/.mason/...
    try:
        from mason_constants import get_mason_home, display_mason_home
        display_path = f"{display_mason_home()}/{out_path.relative_to(get_mason_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  mason import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of MASON_HOME under ``backups/`` (``mason import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in mason_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # MASON_HOME, so sync_skills()'s module-level MASON_HOME cache can't skew it.
    with suppress(Exception):
        from mason_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from mason_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from mason_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from mason_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from mason_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from mason_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('mason-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from mason_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Mason re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.mason` (repeat "
    ...
    try:
        # See #95596.
        from mason_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
