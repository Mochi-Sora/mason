"""Install and remove the Linux desktop entry (``mason.desktop``).
...
DESKTOP_ENTRY_NAME = "mason.desktop"
...
def _can_import_mason_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``mason_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``mason_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``mason desktop``.

    Prefer the real ``mason`` launcher; fall back to ``<python> -m mason_cli.main desktop``.
    """
    from mason_cli.relaunch import resolve_mason_bin

    bin_path = _resolve_mason_bin_for_desktop_entry(resolve_mason_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_mason_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import mason_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "mason_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `mason` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `mason` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m mason_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_mason_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`mason_cli.relaunch.resolve_mason_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from mason_cli.relaunch import resolve_mason_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/mason, a venv console script) wins
    ...
    # /opt/.../bin/mason from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no `mason` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``mason``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "mason")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/mason"))
    candidates.append(Path.home() / ".local" / "bin" / "mason")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Mason\n"
        "GenericName=Mason Desktop\n"
        "Comment=Launch Mason Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Mason\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "mason.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=mason`` resolves without an
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Mason desktop entry and return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "mason"
