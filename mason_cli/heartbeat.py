...
``mason cron``. Invariants (mirrors goals.py): injection is a plain user message — no system-prompt
...
def _get_session_db() -> Optional[Any]:
    """Persistence goes through the goals module's per-MASON_HOME cached SessionDB (one shared connection)."""
    try:
        from mason_cli.goals import _get_session_db as _goals_db
        return _goals_db()
...
def save_heartbeat(session_id: str, state: HeartbeatState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from mason_cli.goals import _warn_dropped_write
