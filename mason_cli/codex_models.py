...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # not in the public API, so it stays out of the "openai" catalog in mason_cli/models.py.
...
# OpenAI API, so it intentionally stays out of the "openai" provider catalog in mason_cli/models.py — only
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # Spark surfaces whenever a compatible template is present; the backend (not Mason)
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is Mason-side only — stripped before the id hits the wire (agent/transports/codex.py,
