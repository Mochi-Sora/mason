"""Stable opaque identity shared by every profile in one Mason install."""
...
def read_or_create_install_id(root: Path | None = None) -> Optional[str]:
    ...
    """
    root = get_default_mason_root() if root is None else root
...
def get_install_id(*, cache: dict[str, Optional[str]] | None = None) -> Optional[str]:
    """Return the process-cached stable id for the active Mason root."""
