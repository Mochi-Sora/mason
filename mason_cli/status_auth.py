"""Credential sections of `mason status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``mason_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "mason model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "mason auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "mason auth add xai-oauth", _FILE_REFRESH_ROWS))
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `mason status` never performs an OAuth
    ...
    _status._row("Nous Portal", logged_in,
         ...
         else "not logged in (run: mason portal)")
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: mason model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from mason_cli.models_local import probe_lmstudio_models
...
import mason_cli.status as _status  # noqa: E402  (bottom: mason_cli.status imports this module)
