"""Provider-specific runtime builders for :mod:`mason_cli.runtime_provider`: Azure Foundry, the
...
``mason_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _resolve_azure_foundry_runtime(*, requested_provider: str, model_cfg: Dict[str, Any],
    ...
    if not base_url:
        raise rp.AuthError(
            "Azure Foundry requires a base URL. Set it via 'mason model' or "
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from mason_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
