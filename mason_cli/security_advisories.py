"""Security advisory checker for Mason Agent.
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including mason when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.mason/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: mason doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'mason doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# Once the banner is seen we cache that in ``~/.mason/cache/advisory_banner_seen`` (one
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_mason_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active (IDs: {', '.join(h.advisory.id for h in fresh)}). "
            "Run `mason doctor` on the gateway host for details.")
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``mason doctor``.
