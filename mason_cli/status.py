...
from mason_cli.auth import AuthError, resolve_provider
from mason_cli.colors import Colors, color
from mason_cli.config import get_env_path, get_env_value, get_mason_home, load_config
from mason_cli.models import provider_label
from mason_cli.runtime_provider import resolve_requested_provider
from mason_cli.vercel_auth import describe_vercel_auth
from mason_cli.status_auth import (  # renderers wired into _SECTIONS below
...
def _estop_status_line():
    """One-line pause banner for `mason status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `mason resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'mason-agent[vercel]')")
        ...
        # (fail-soft — never break `mason status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        from mason_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_mason_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from mason_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_mason_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from mason_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'mason doctor' for detailed diagnostics", "  Run 'mason setup' to configure"),
            Colors.DIM)
    print()


# Print order of `mason status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Mason Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('mason_cli.nous_account', 'get_nous_portal_account_info'),
