...
``~/.mason/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
...
DEFAULT_CATALOG_URL = (
    "https://mason-agent.nousresearch.com/docs/api/model-catalog.json")
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/mason-agent/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_mason_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _MASON_USER_AGENT})
...
def _builtin_catalog() -> dict[str, Any]:
    """Checked-in manifest (scripts/gen_model_catalog.py) — the detached
    fork's final fallback when remote + disk cache both miss. Never raises."""
    try:
        data = json.loads((Path(__file__).resolve().parent / "model_catalog_builtin.json").read_text())
    except Exception:
        return {}
    return data if _validate_manifest(data) else {}
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    if disk_data is not None:
        return _remember(disk_data, disk_mtime)
    # Detached fork: dead remote + cold cache still yields the builtin manifest.
    builtin = _builtin_catalog()
    if builtin:
        return _remember(builtin, now)
    return {}
...
def refresh_catalogs() -> bool:
    ...
    try:
        from mason_cli.models import fetch_nous_recommended_models, fetch_openrouter_models
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Mason silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``mason update`` that file IS the newest catalog, so the picker stays current even when
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``mason model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
