...
from mason_cli.web_server_memory import _normalize_memory_provider_name
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `mason egress setup` "
            "and `mason egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": _select(
        "When the chat app / gateway updates Mason (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during mason update. "
...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from mason_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → Mason-provider: when the provider is not a known provider/alias but the
    ...
    from mason_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    from mason_cli.model_normalize import normalize_model_for_provider
    from mason_cli.providers import resolve_custom_provider, resolve_user_provider
...
# in mason_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    Long-lived dashboard / Desktop-owned ``mason serve`` processes freeze ``sys.modules``
    at boot; after ``mason update`` replaces the checkout, a first-time lazy import can
...
def _dashboard_skew_restart_hint() -> str:
    ...
    dashboard and Desktop-owned ``mason serve``; naming a systemd unit would mislead
    ...
    if os.environ.get("MASON_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Mason Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Mason process to load the new code "
        "(mason dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def _apply_nous_gateway_defaults(cfg: dict) -> list:
    ...
    *unconfigured* tools through the Nous Tool Gateway. Purely additive — tools with a direct
    ...
    try:
        from mason_cli.nous_subscription import apply_nous_managed_defaults
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    ``mason model`` custom flow) so the picker gets a proper ready row instead of a "needs
    setup" dead-end. Dedups by base_url; never blocks the already-persisted assignment."""
    try:
        from mason_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from mason_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
