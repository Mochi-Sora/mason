"""Implementation of the ``mason bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  mason bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``mason bundles`` argparse tree (called from mason_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `mason bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``mason bundles <subcommand>`` to the right handler."""
