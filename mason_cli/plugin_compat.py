...
The decomposition (PR #102117) moved most of Mason's internals into ``<stem>_<topic>`` sibling modules.
...
Surfaces that read from here: the CLI banner, ``mason plugins compat``, ``mason doctor``, the post-update
...
class MasonPluginCompatWarning(FutureWarning):
...
def report_file_path() -> Path:
    ...
    return get_mason_home() / REPORT_FILE
...
def disable_reason(manifest, *, today: Optional[_dt.date] = None) -> Optional[str]:
    ...
    if not hits:
        return None
    return (f"uses {len(hits)} import path(s) removed on {COMPAT_REMOVAL}; run `mason plugins compat` "
...
def summary_lines(report: Dict[str, List[Hit]], *, today: Optional[_dt.date] = None) -> List[str]:
    ...
    if removal_in_effect(today) and allow_deprecated_imports():
        ...
        tail = "Update the plugin(s); the old paths no longer exist. Details: mason plugins compat"
    elif removal_in_effect(today):
        head = (f"{n} plugin{'s' if n != 1 else ''} DISABLED: they import paths removed on {COMPAT_REMOVAL}: {names}")
        tail = f"Update the plugin(s) or set plugins.{ALLOW_KEY}: true to force-load. Details: mason plugins compat"
    else:
        ...
        tail = "Check for plugin updates or notify the author before then. Details: mason plugins compat"
    return [head, tail]
...
def warn_once(facade: str, name: str, target_module: str, target_name: str) -> None:
    """Per-name record that a moved name was resolved through its old path: a ``MasonPluginCompatWarning``
    ...
    msg = (f"mason plugin compat: `{facade}.{name}` moved to `{new}`. The old path is kept only for external "
           f"plugins and is removed on {COMPAT_REMOVAL}; update your import.")
    _log.warning(msg)
    warnings.warn(msg, MasonPluginCompatWarning, stacklevel=3)
...
def quiet_for_interactive() -> None:
    ...
    Appends (does not override) so an explicit ``-W error::...MasonPluginCompatWarning`` still wins."""
    if not any(a == "error" and c is not None and issubclass(MasonPluginCompatWarning, c)
               for a, _m, c, _mod, _l in warnings.filters):
        warnings.filterwarnings("ignore", category=MasonPluginCompatWarning, append=True)
