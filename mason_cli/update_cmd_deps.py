"""Post-``mason update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``mason_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``mason.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "mason_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from mason_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from mason_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__MASON_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # only our own packages count. Roots come from mason_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('mason_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``mason tools``."""
    try:
        from mason_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Mason Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``mason tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("Mason Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Mason Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `mason update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `mason update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Mason extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from mason_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from mason_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _npm_lockfile_changed(mason_root: Path) -> bool:
    from mason_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(mason_root)
...
def _npm_lock_cache_file(mason_root: Path) -> Path:
...
def _record_npm_lockfile_hash(mason_root: Path) -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(mason_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run mason
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `mason update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from mason_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `mason update`.")
            has_workspace = any(
    ...
    shared_mason_root = get_default_mason_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `mason update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_mason_root):
    ...
    nixos_env = with_mason_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `mason update` comes from the *desktop* build, not this step;
    ...
    print("    deps). Fix npm and re-run `mason update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".mason-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``mason_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``mason.exe`` shim
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `mason` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "mason_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put Mason-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from mason_constants import with_mason_node_path
    build_env = with_mason_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `mason desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo mason`` dies mid-update with ``venv/bin/mason`` already
    ...
    See #83529.
    A later normal ``mason update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/mason`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from mason_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Mason broken.")
    print("  This usually happens after running mason or pip with sudo. Offending paths:")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `mason` again "
    ...
    if not import_ok:
        ...
        print("    Run `mason update` again — if it persists, reinstall:")
        print("    https://mason-agent.nousresearch.com")
