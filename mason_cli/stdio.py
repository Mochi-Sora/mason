...
def configure_windows_stdio() -> bool:
    ...
    ``MASON_DISABLE_WINDOWS_UTF8=1`` to opt out (forces the old cp1252 path for diagnosing
    ...
    if _CONFIGURED:
        return False
    if not is_windows() or os.environ.get("MASON_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
...
def _augment_path_with_known_tools() -> None:
    r"""Prepend Mason-managed tool directories to ``PATH`` (no-op on POSIX / missing dirs).

    install.ps1 adds entries like ``%LOCALAPPDATA%\mason\git\bin`` to the User PATH via
    ``SetEnvironmentVariable``, but already-running shells never see that broadcast, so a mason
    ...
    # dir hosts mason.exe + pip console scripts; WinGet\Links is where ``winget install`` drops
    # CLI shims (ripgrep lands there as rg.exe).
    candidate_dirs = [
        os.path.join(local_appdata, "mason", "git", "cmd"),
        os.path.join(local_appdata, "mason", "git", "bin"),
        os.path.join(local_appdata, "mason", "git", "usr", "bin"),
        os.path.join(local_appdata, "mason", "mason-agent", "venv", "Scripts"),
