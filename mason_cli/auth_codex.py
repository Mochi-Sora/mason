...
Tokens live in ~/.mason/auth.json, NOT ~/.codex/: Mason keeps its own Codex OAuth session
...
_MISSING_ACCESS_TOKEN_MSG = (
    "Codex auth is missing access_token. Run `mason auth` to re-authenticate.")
_MISSING_REFRESH_TOKEN_MSG = (
    "Codex auth is missing refresh_token. Run `mason auth` to re-authenticate.")
_NO_CREDENTIALS_MSG = "No Codex credentials stored. Run `mason auth` to authenticate."
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from mason_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("MASON_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from mason_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Mason auth store (~/.mason/auth.json)."""
    from mason_cli.auth import _load_provider_state, _nonempty_str
    ...
    if not isinstance(tokens, dict):
        raise _codex_err(
            "Codex auth state is missing tokens. Run `mason auth` to re-authenticate.",
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``mason setup`` / the model picker) is always
    synced. ``manual:device_code`` (``mason auth add openai-codex``) is synced only when its
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Mason auth store (~/.mason/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Mason auth, if available."""
    from mason_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def _codex_refresh_failure_error(response: "httpx.Response") -> AuthError:
    ...
    if code == "refresh_token_reused":
        message = (
            ...
            "then run `mason auth` to re-authenticate.")
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating Mason auth state."""
    from mason_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    from mason_cli.auth import _save_codex_tokens, refresh_codex_oauth_pure
    try:
        ...
        # another Mason process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from mason_cli.auth import _codex_access_token_is_expiring
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from Mason's own Codex token store.
    ...
    refresh_timeout_seconds = env_float("MASON_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    if _should_refresh(access_token):
        # Re-read under lock to avoid racing with other Mason processes
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="mason-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from mason_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.mason/auth.json."""
    from mason_cli.auth import (
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Mason will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("Mason will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Mason gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Mason creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _codex_login_rate_limited_error(response: "httpx.Response", *, during: str = "") -> AuthError:
    ...
    # callers surface a "retry later" notice instead of a misleading "run mason auth" prompt (see issue
    # #32790).
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from mason_cli.auth import _utc_now_z
