...
Wraps the same config layer the CLI uses (mason_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from mason_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.mason_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    from mason_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    try:
        from agent.secret_scope import build_profile_secret_scope, reset_secret_scope, set_secret_scope
        from mason_constants import reset_mason_home_override, set_mason_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import MasonTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_mason_home_override(flow.mason_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.mason_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = MasonTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, mason_home=flow.mason_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, mason_home=flow.mason_home)
                    raise
        finally:
