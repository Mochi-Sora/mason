...
ICON_MASON = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22d3ee" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>'
...
<body>
    <div class="layout {layout_class}">
        ...
        <div class="main-content">
            ...
            <footer>
                Built with ☤ Mason Agent • Generated on {generated_at}
            </footer>
        </div>
    </div>
    ...
    return f"""
        <div class="{"session-view" if is_multi else "session-view active"}" id="view-{escaped_sid}">
            <header class="fade-in">
                <h1>{_escape_html(s.get("title") or "Mason Session")}</h1>
                <div class="meta">
...
def generate_multi_session_html_export(sessions: List[Dict[str, Any]]) -> str:
    ...
    return HTML_TEMPLATE.format(
        page_title="Mason Session Export" if is_multi else _escape_html(sessions[0].get("title") or "Mason Session"),
