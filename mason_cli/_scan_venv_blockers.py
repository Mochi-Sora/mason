"""Standalone venv-process scan for JSON consumption (``python -m mason_cli._scan_venv_blockers``).
...
def _is_updater_owned_backend(pid: int, cmdline: str) -> bool:
    """True when *pid* is a Mason backend the CLI updater can stop (positive ledger identity).

    The gateway exemption above keeps ``gateway run`` holders out of the blocker list because the updater's
    own pause machinery stops and resumes them. ``mason serve`` / ``mason dashboard`` backends had no such
...
def _updater_owned_backend_entry(pid: int, cmdline: str) -> dict | None:
    ...
    """
    try:
        from mason_cli.update_cmd import _mason_holder_subcommand  # noqa: PLC0415

        purpose = _mason_holder_subcommand(cmdline)
    except Exception:
        return None
    if purpose not in _UPDATER_STOPPABLE_PURPOSES:
        return None
    try:
        from mason_cli.process_identity import ledger_entries, spawner_is_dead  # noqa: PLC0415
