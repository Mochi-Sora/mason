...
def _managed_scratch_path_info(p: Path) -> tuple[bool, Optional[str]]:
    ...
    override = os.environ.get("MASON_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    root (``MASON_KANBAN_WORKSPACES_ROOT``, ``<kanban_home>/kanban/workspaces``,
    ...
    Mason' own DB and metadata. :func:`_cleanup_workspace` refuses
...
def _cleanup_worktree_workspace(
    ...
    try:
        from mason_cli.worktree_ops import _worktree_has_unpushed_commits, _worktree_is_dirty
    except Exception:
