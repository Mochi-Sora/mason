...
from mason_cli.config import get_mason_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Env-var names assigned a non-empty value in ~/.mason/.env — what the managed backends (launchd /
    systemd / desktop ``serve``) load, as opposed to the shell exports ``os.getenv`` reflects here.

    ``mason debug share`` runs in a terminal, so ``os.getenv`` reflects the shell's environment, which can
...
def _get_git_commit(project_root: Path) -> str:
    ...
    the Dockerfile bakes into ``<project_root>/.mason_build_sha``."""
    value = _git_output(project_root, "rev-parse", "--short=8", "HEAD")
    if value:
        return value
    try:
        from mason_cli.build_info import get_build_sha  # deferred: keeps dump cheap on non-dump paths
...
def _gateway_status() -> str:
    ...
    try:
        from mason_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(mason_home: Path) -> int:
...
def _cron_summary(mason_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = mason_home / "cron" / "jobs.json"
...
def _api_key_lines(show_keys: bool) -> list[str]:
    ...
    for env_var, label in _API_KEYS:
        ...
        # Set in this shell but absent from ~/.mason/.env: a managed backend loads .env, not the login
        ...
        # `mason auth add openrouter` credentials live in the pool, not env — don't read "not set".
        # A credential added via `mason auth add openrouter` lives in the credential pool, not as an env
        # var — surface it so the dump doesn't misleadingly read "not set" while `mason auth list` shows it
        # (#42130).
...
def run_dump(args):
    ...
    load_mason_dotenv(mason_home=get_env_path().parent, project_env=get_project_root() / ".env")
    project_root, mason_home = get_project_root(), get_mason_home()
    try:
        config = load_config()
    except Exception:
        config = {}
    ...
    try:
        ...
        profile = "(default)"
    toolsets = config.get("toolsets", ["mason-cli"])
    ...
    lines = [
        ...
        f"profile:          {profile}",
        f"mason_home:      {display_mason_home()}",
        f"model:            {model}",
        f"provider:         {provider}",
        ...
        f"  cron_jobs:          {_cron_summary(mason_home)}",
        f"  skills:             {_count_skills(mason_home)}",
    ]
