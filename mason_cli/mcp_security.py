...
for the June 2026 mason-0day campaign. Runs BOTH at save time (``_save_mcp_server`` — dashboard API +
...
# OS persistence surfaces an MCP server has no legitimate reason to write to (the mason-0day
...
# Indicators of compromise, June 2026 mason-0day campaign: exact attacker artifacts observed on
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a shell interpreter whose inline script writes to an OS persistence surface (June 2026 mason-0day
    ...
    for ioc in _IOC_SUBSTRINGS:
        if ioc in flat:
            ...
            issues.append(
                f"MCP server '{name}' contains a known mason-0day "
    ...
    if _PERSISTENCE_PATTERN.search(script):
        issues.append(
            ...
            f"shell rc) — this is the mason-0day backdoor shape, not a real "
            f"MCP server"
        )
    return issues
