"""Mason Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.mason/plugins/<name>/``, project
``./.mason/plugins/<name>/`` (opt-in via ``MASON_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``mason_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from mason_constants import get_mason_home, mason_home_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from mason_cli.config import load_config_readonly
from mason_cli.middleware import VALID_MIDDLEWARE
from mason_cli.plugin_capabilities import plugin_capability_granted
from mason_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from mason_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from mason_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from mason_cli.plugins_loader import (
...
from mason_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, MASON_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from mason_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from mason_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``MASON_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
    store paths) first, else the in-repo path."""
    env_override = os.getenv("MASON_BUNDLED_PLUGINS")
    if env_override:
...
# ``MASON_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("MASON_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When MASON_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("MASON_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (mason_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: mason_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        segments = self._segments(key)
        from mason_cli import config as config_mod
    ...
    def platform_actions(self):
        ...
        from mason_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.mason/profiles/<name>`` id, or ``"custom"``),
        derived from ``MASON_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from mason_cli.approval_transport import RegisteredApprovalTransport
        ...
        entry = RegisteredApprovalTransport(
            ...
            profile_home=str(get_mason_home().resolve()),
        )
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Mason core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``mason <name> ...``). *setup_fn* receives the argparse
    ...
    def register_command(
        ...
        with suppress(Exception):  # reject if it conflicts with a built-in command
            from mason_cli.commands import resolve_command
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`mason_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        auth gate (non-loopback bind without ``--insecure``). Wrong type / duplicate name warn and
        are ignored, never raised."""
        from mason_cli.dashboard_auth import DashboardAuthProvider
        from mason_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from mason_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``mason:x`` is reserved for core,
        ...
        if ":" in event:
            logger.warning("Plugin '%s' tried to emit namespaced/reserved event '%s' — a plugin may only emit "
                           ...
                           plugin_key, event, plugin_key, MASON_EVENT_NAMESPACE)
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{MASON_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        Not in ``~/.mason/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_mason_dotenv()`` "
     "(after ``~/.mason/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        self.scope_key = scope_key or mason_home_key()
        ...
        # is keyed per (mason_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `mason plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("MASON_SAFE_MODE"):
                logger.info("MASON_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_mason_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_mason_dotenv() already
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Mason plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh MASON_HOME/.plugin-compat-report.json from this discovery pass (mason_cli.plugin_compat).
        ...
        """
        try:
            from mason_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_mason_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_mason_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        if _env_enabled("MASON_SAFE_MODE"):
            return False
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from mason_cli.agent_plugins import _discover_mcp
                if _discover_mcp(Path(manifest.path), get_mason_home() / "plugin-data"
...
# Resolved Mason home -> PluginManager. A process can switch profiles via
# ``set_mason_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    """Resolved active Mason home — the key for per-profile plugin managers (plugins capture the
    home at registration, so a process serving several profiles cannot share one manager)."""
    try:
        return get_mason_home().expanduser().resolve()
    except Exception:
        return get_mason_home().expanduser()
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Mason profile/home (cached per resolved home; a
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=mason_home_key(current_home))
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from mason_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _plugin_toolset_keys_cache_path() -> Path:
    return get_mason_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from mason_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    threading.Thread(target=_runner, name="mason-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``mason tools`` TUI."""
    manager = get_plugin_manager()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``mason:<event>``) to subscriber callbacks in
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('mason_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('mason_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('mason_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('mason_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('mason_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('mason_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('mason_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('mason_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_mason_home_override': ('mason_constants', 'reset_mason_home_override'),
    'set_mason_home_override': ('mason_constants', 'set_mason_home_override'),
}
