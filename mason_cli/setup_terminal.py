...
setup.py names are resolved through the module object so test patches on ``mason_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Mason has one ($MASON_HOME/bin is never on PATH, so which() misses it and
...
def _ensure_sdk(package: str, manual_hint: str, *, show_stderr: bool = False, install=None) -> None:
    ...
    try:
        ...
        if install is None:
            from mason_cli.tools_config import _pip_install
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `mason setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    # Image and resource limits use defaults; tune via `mason setup terminal`.
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `mason egress setup` then `mason egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `mason egress setup`.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'mason-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'mason-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Mason runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.mason/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
