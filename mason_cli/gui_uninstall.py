"""Mason Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Mason" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Mason"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Mason"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Mason"


def source_built_gui_artifacts(mason_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``mason desktop`` inside the checkout (same ``mason-agent/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Mason" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Mason.app"), home / "Applications" / "Mason.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Mason", local_base / "mason-desktop"] + (
            [Path(program_files) / "Mason"] if program_files else [])
    ...
    from mason_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Mason.desktop"] + [
        icons / size / "apps" / "mason.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(mason_home: Path) -> bool:
    """True when a usable Python agent install exists under MASON_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((mason_home / "mason-agent" / sub).is_dir() for sub in ("mason_cli", "venv", ".venv"))


def gui_is_installed(mason_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(mason_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(mason_home: "Path | None" = None) -> dict:
    ...
    return {"mason_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(mason_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Mason
        # disappears from the launcher.
        try:
            from mason_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove mason' or 'sudo dnf remove mason'). AppImage builds are a single "
