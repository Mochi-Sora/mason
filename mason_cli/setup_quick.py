"""Streamlined setup flows: the Nous Portal one-shot (`mason portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``mason_cli.setup`` take effect."""
...
logger = logging.getLogger("mason_cli.setup")
...
def _blank_slate_done(config: dict, mason_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from mason_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         mason skills opt-in --sync",
          "  Add MCP servers:     mason mcp add", *extra, "  Tune agent settings: mason setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from mason_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``mason model``. False when cancelled or failed (the
    message is already printed)."""
    from mason_cli.setup import _info
    try:
        from mason_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``mason setup --portal`` / ``mason portal``)."""
    from mason_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ⚕ Mason Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("  You can retry later with `mason portal`.")

    if not _run_nous_flow(config, context="`mason portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `mason portal`."),
    ...
    _info("  Run `mason portal info` to inspect routing.", "  Run `mason` to start chatting.")


def _run_first_time_quick_setup(config: dict, mason_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'mason setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `mason import`).
        from mason_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    mason setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  mason setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Mason.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Mason update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``mason-agent`` skill
    ...
    try:
        ...
        from mason_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "mason-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("mason-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``mason setup agent``."""
...
def _run_blank_slate_setup(config: dict, mason_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `mason-agent` skill is always kept so the agent",
          "can help you drive and configure Mason itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, mason_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `mason update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, mason_home, "  Enable tools:        mason tools", "  Enable plugins:      mason plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, mason_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `mason-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`mason update` runs from re-injecting them. Opt back in any",
              "time with `mason skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `mason tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `mason plugins list` / `mason plugins install`.",
         "No plugins enabled. Add later with `mason plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `mason mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `mason mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, mason_home, "  Enable more tools:   mason tools")


def _run_quick_setup(config: dict, mason_home):
    ...
    from mason_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'mason setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Mason to messaging apps to chat from anywhere.",
              "You can configure these later with 'mason setup gateway'.")
