...
The provider list shown by ``mason model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`mason_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `mason model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``mason model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("mason_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("mason_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    MASON_OVERLAYS = _safe_import("mason_cli.providers", "MASON_OVERLAYS", {})
    try:
