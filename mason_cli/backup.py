"""Backup and import commands for mason CLI."""
...
from mason_constants import (
    _get_platform_default_mason_home, get_default_mason_root, get_mason_home, display_mason_home,
)
from utils import (
...
# Where ``mason backup --quick`` / ``/snapshot`` / the pre-update safety net write state
# snapshots (see ``create_quick_snapshot``); defined here because the exclusion set needs it.
_QUICK_SNAPSHOTS_DIR = "state-snapshots"

# Directory names to skip (matched against each path component). ``mason-agent`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../mason-agent/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under MASON_HOME walked
...
_EXCLUDED_DIRS = {
    "mason-agent",     # the codebase repo — re-clone instead
...
# Mason-managed runtime downloads (GGUF models, llama.cpp runtimes, managed Node): re-downloaded
# on demand and routinely tens to hundreds of GB. Matched ONLY at the root of MASON_HOME and at
...
# Files ``mason import`` must never overwrite, matched by basename so root and named profiles are
...
# Reserved archive subtree for memory-provider state OUTSIDE MASON_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Mason backup slot."""
...
@contextmanager
def _backup_operation_lock(mason_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = mason_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another Mason backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to mason root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``mason-agent`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "mason-agent" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(mason_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``mason-agent`` carve-out, root runtime trees, per-file rules),
    shared by ``mason backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(mason_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(mason_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "mason-agent" and not is_root))
        ...
        for fname in filenames:
            rel = rel_dir / fname
            fpath = mason_root / rel
            # zipfile.write() follows file symlinks, so skip links before any archive write can
            # copy data from outside MASON_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``mason update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Mason) and retry.", dst, exc2)
        return False
...
def _resolve_backup_output_path(output: Optional[str]) -> Path:
    ...
    out_path = None
    default_name = f"mason-backup-{datetime.now().strftime('%Y-%m-%d-%H%M%S')}.zip"
    try:
        if output:
...
def run_backup(args) -> None:
    """Create a zip backup of the Mason home directory."""
    mason_root = get_default_mason_root()

    if not mason_root.is_dir():
        print(f"Error: Mason home directory not found at {mason_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(mason_root):
            _run_backup_locked(args, mason_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, mason_root: Path) -> None:
    ...
    print(f"Scanning {display_mason_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(mason_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_mason_home()}.")
    if skipped_external:
    ...
    if errors:
        ...
        print(f"\nRestore with: mason import {out_path.name}")


# --- Import ---

def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Mason backup."""
    names = zf.namelist()
    if not names:
        return False, "zip archive is empty"
    # Telltale files a mason home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Mason backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".mason", "mason"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Mason session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(mason_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((mason_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Mason configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, mason_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under MASON_HOME; provider configs commonly hold credentials, so tighten to 0600.
        ...
        if external:
            ...
            if rel.endswith(_SQLITE_SIDECAR_SUFFIXES):
                skipped_runtime.append(rel)
                continue
            target = mason_root / rel
            root = mason_root.resolve()
...
def run_import(args) -> None:
    """Restore a Mason backup from a zip file."""
    ...
    # ``get_default_mason_root()`` would silently retarget a profile restore at the live root.
    mason_root = get_mason_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_mason_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(mason_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, mason_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_mason_home()}.")
        if errors:
            _print_capped(f"\n  Warnings ({len(errors)} files skipped):", errors, "  ")
        if db_shrunk:
            ...
            print("    Anything recorded after the backup was taken is not in it. "
                  "Recover from a newer backup or snapshot: mason snapshot list")
        if skipped_runtime:
            _print_capped(f"\n  Preserved {len(skipped_runtime)} runtime state "
                          f"file(s) (kept this machine's, not the backup's):",
                          sorted(skipped_runtime), "    ")
        restored_profiles = _restore_profile_wrappers(mason_root)
        print()
        if not (mason_root / "mason-agent").is_dir():
            print("Note: The mason-agent codebase was not included in the backup.\n"
                  "  If this is a fresh install, run: mason update")
        if restored_profiles:
            print("\nTo re-enable gateway services for profiles:")
            for pname in restored_profiles:
                print(f"  mason -p {pname} gateway install")
        _revive_gateway_after_import(mason_root)
        print("Done. Your Mason configuration has been restored.")


def _restore_profile_wrappers(mason_root: Path) -> List[str]:
    """Re-create shell wrapper scripts for restored named profiles; return the profile names seen."""
    profiles_dir = mason_root / "profiles"
    ...
    except ImportError:  # mason_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: mason profile list  (after installing mason)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(mason_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_mason_home()
    if mason_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  mason gateway install")
        return
    try:
        from mason_cli.gateway import ensure_gateway_service, _is_service_running
...
# --- Quick state snapshots (used by /snapshot slash command and mason backup --quick) ---

# Critical state files (relative to MASON_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``mason update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside MASON_HOME.
...
def _quick_snapshot_root(mason_home: Optional[Path] = None) -> Path:
    home = mason_home or get_mason_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, mason_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``mason update`` while the small files are always captured.
    """
    ...
    if failed_dbs:
        ...
        print(f"  ⚠ CRITICAL: could not snapshot DB file(s): {', '.join(failed_dbs)}\n"
              f"  ⚠ If sessions disappear after update, check {root} and run: mason snapshot list")
...
def list_quick_snapshots(limit: int = 20, mason_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(mason_home)):
...
def restore_quick_snapshot(snapshot_id: str, mason_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, mason_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent cron-job loss across ``mason update``.
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_mason_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", mason_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, mason_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``mason update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, mason_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, mason_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, mason_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(mason_home)), keep, shutil.rmtree, "snapshot")


def run_quick_backup(args) -> None:
    """CLI entry point for mason backup --quick."""
    snap_id = create_quick_snapshot(label=getattr(args, "label", None))
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_mason_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, mason_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``mason_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(mason_root):
            return _write_full_zip_backup_locked(out_path, mason_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, mason_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(mason_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    mason_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<MASON_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    mason_root = mason_home or get_default_mason_root()
    if not mason_root.is_dir():
        return None
    backup_dir = mason_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, mason_root) is None:
        return None
...
def create_pre_update_backup(
    mason_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``mason update`` continues anyway."""
    return _create_prefixed_full_backup(mason_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    mason_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``mason claw migrate``
    (same dir as update backups so listings/``mason import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        mason_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
