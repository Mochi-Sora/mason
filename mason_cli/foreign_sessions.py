...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Mason SessionDB; returns the new Mason session id.
...
def pick_foreign_session(source: Optional[str] = None, *, limit: int = 25) -> Optional[ForeignSession]:
    ...
    if not sys.stdin.isatty():
        print("Non-interactive terminal — pass the file path directly:\n"
              "  mason sessions import --from claude|codex <path>")
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`mason sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  mason --resume {session_id}")
    return session_id
