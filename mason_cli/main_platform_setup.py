...
Split out of ``mason_cli/main.py``. Names that still live in main are imported lazily at call time.
"""
...
from mason_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``mason whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Mason?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``mason whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``mason whatsapp``: ``npm install`` the bridge when needed. False = stop."""
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", env=with_mason_node_path())
...
def cmd_whatsapp(args):
    ...
    from mason_cli.main import _require_tty, get_mason_home
    ...
    # every `mason gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    session_dir = get_mason_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: mason gateway")
            return
    ...
    _say("", "  Or install as a service: mason gateway install")


def cmd_whatsapp_cloud(args):
    """Set up WhatsApp Business Cloud API (official Meta integration) — complementary to the
    ``mason whatsapp`` Baileys bridge wizard. See ``mason_cli/setup_whatsapp_cloud.py``."""
    from mason_cli.main import _require_tty
    _require_tty("whatsapp-cloud")
    from mason_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def _sync_toggle(args, sub: str) -> int:
    ...
    if not is_curation_eligible(skill):
        _err(f"'{skill}' is not sync-eligible (bundled, hub-installed, "
             ...
             f"skills under ~/.mason/skills/ can sync.")
        return 1
...
def _sync_status(ssc) -> int:
    ...
    if status.get("org_available"):
        ...
        if modified:
            _err(f"  {len(modified)} with local edits not yet shared: "
                 f"{', '.join(modified)}\n"
                 f"  Share them back with `mason sync propose <skill>`. "
    ...
    if not status.get("logged_in"):
        ...
        _err("\nSync feature is off for this instance (set MASON_SYNC_ENABLED=1 "
             "or config.yaml sync.enabled: true). Sync is inert.")
    elif not status.get("base_url"):
        _err("\nNo sync base URL configured (config.yaml sync.base_url or MASON_SYNC_BASE_URL). Sync is inert.")
    return 0
...
_SYNC_GATED = {
    "pull": _sync_pull,
    "push": lambda ssc, identity: ssc.push_skills(identity=identity, message="mason sync push"),
    "now": lambda ssc, identity: {"pull": ssc.pull_skills(identity=identity),
                                  "push": ssc.push_skills(identity=identity, message="mason sync now")}}
...
def cmd_sync(args):
    ...
    if not ssc.resolve_sync_base_url():
        _err("sync inert: no sync base URL configured (config.yaml sync.base_url or MASON_SYNC_BASE_URL).")
        return 1
...
def cmd_slack(args):
    """``mason slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: mason slack <subcommand>\n"
             "\n"
             "subcommands:\n"
             "  manifest   Generate a Slack app manifest with every gateway\n"
             "             command registered as a native slash\n"
             "\n"
             "Run `mason slack manifest -h` for details.")
        return 1

    if sub == "manifest":
        from mason_cli.slack_cli import slack_manifest_command
