...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_MASON_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Mason's plumbing calls. ``GIT_ASKPASS``/``SSH_ASKPASS`` are deliberately left alone: a
    *working* askpass helper or ssh-agent should still succeed non-interactively. Pair with
    ...
    Mason shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_mason(text: str) -> bool:
    r"""True when *text* names Mason at a path-segment / token boundary.

    A bare ``"mason" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\smasona\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("mason", ".mason"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_mason(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Mason code."""
    try:
        import psutil
        ...
        return _text_names_mason(f"{command} {executable}")
    except Exception:
        return False


def pid_is_mason(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Mason process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_mason(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_mason`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``mason update`` on slow-WMI
