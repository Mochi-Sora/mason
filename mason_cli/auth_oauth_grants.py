...
Split out of ``mason_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``mason_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from mason_cli.auth_constants import _decode_jwt_claims
...
def _is_pkce_row(row: Dict[str, Any]) -> bool:
    return str(row.get("source") or "").endswith("mason_pkce")
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``mason profile create
    ...
    """
    from mason_cli.auth import _save_auth_store
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``mason auth list`` / ``mason auth status`` print them so the user sees the consolidation.
    """
    from mason_cli.auth import _oauth_heal_notices
...
def _find_root_counterpart(
    ...
    ``mason_pkce`` grant uses one client id and carries no claims, so two Anthropic OAuth rows
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``mason -p <p> auth add`` grant, or the only
...
class _HealPass:
    ...
    def heal_pool_rows(self) -> None:
        ...
        for row in self.p_rows:
            ...
            # (the ``mason auth`` PKCE shape); a profile mason_pkce-family row is its copy.
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from mason_cli.auth import _is_same_auth_store
    ...
    def sync_root_singleton_with_pkce_row(self) -> None:
        """Keep root's singleton and its ``mason_pkce``-seeded pool row in step.
        ...
        pkce_idx = next(
            (i for i, r in enumerate(self.r_rows)
             if _is_oauth_pool_payload(r) and r.get("source") == "mason_pkce"), None)
        if pkce_idx is None:
            return
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # ~/.mason/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".mason" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            if summary["adopted"] and root_singleton is not None and singleton_row is not None:
                from agent.anthropic_credentials import _write_mason_oauth_credentials
                _write_mason_oauth_credentials(
