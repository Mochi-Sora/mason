...
from mason_cli.config import OPTIONAL_ENV_VARS, get_env_path, redact_key
from mason_cli.web_deps import LateState, late
from mason_cli.web_server_gateway import _restart_gateway_after
from mason_cli.web_server_messaging import (
...
from mason_cli.web_models import (
...
_profile_scope = late("_profile_scope", "mason_cli.web_server_profiles")
_resolve_profile_dir = late("_resolve_profile_dir", "mason_cli.web_server_profiles")
_restart_gateway_after_whatsapp_onboarding = late("_restart_gateway_after_whatsapp_onboarding", "mason_cli.web_server_messaging")
_telegram_onboarding_request_sync = late("_telegram_onboarding_request_sync", "mason_cli.web_server_messaging")
_whatsapp_session_path = late("_whatsapp_session_path", "mason_cli.web_server_messaging")
_write_platform_enabled = late("_write_platform_enabled", "mason_cli.web_server_messaging")
load_env = late("load_env", "mason_cli.config")
load_config = late("load_config", "mason_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
remove_env_value = late("remove_env_value", "mason_cli.config")
save_env_value = late("save_env_value", "mason_cli.config")
_gateway_subcommand = late("_gateway_subcommand", "mason_cli.web_server_gateway")
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    for key, description, prompt, extra in (
        ...
        ("WEIXIN_ACCOUNT_ID", "iLink Bot account ID obtained through QR login in mason gateway setup", "iLink Bot account ID", {}),
        ("WEIXIN_TOKEN", "iLink Bot token obtained through QR login in mason gateway setup", "iLink Bot token", {"password": True}),
...
def _messaging_platform_payload(
    ...
    # and do NOT follow the MASON_HOME contextvar override, so without it messaging
...
def _platform_payloads(scoped_dir: Optional[Path], entries) -> list[dict[str, Any]]:
    ...
    MASON_HOME contextvar; the gateway status readers do not, hence the explicit path)."""
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env=with_mason_node_path(), creationflags=windows_hide_flags(),
        )
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Mason Agent").strip() or "Mason Agent"
...
async def apply_telegram_onboarding(pairing_id: str, body: TelegramOnboardingApply, profile: Optional[str] = None):
    ...
    # saved token waiting on a manual restart click reads as "Mason is broken" from the
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    def _run():
        ...
        # credentials/state, not the root install's. load_env() honors the MASON_HOME contextvar override;
        ...
        with _profile_scope(profile) as scoped_dir:
            return {
                ...
                "gateway_start_command": " ".join(["mason", *_gateway_subcommand(profile, "start")]),
...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (unrecognized MASON_HOME) is outside
