...
Shared helpers are reached via the late-binding seam in :mod:`mason_cli.web_deps`
...
from mason_cli.web_server_config import _apply_main_model_assignment, _normalize_main_model_assignment
from mason_cli.web_server_gateway import _strip_session_list_rows
from mason_cli.web_server_profiles import (
    _fallback_profile_dicts, _hub_action_name, _write_profile_mcp_servers,
)
from mason_cli.web_server_sessions import _open_session_db_at_path
...
from mason_cli.web_models import (
    ...
    SessionPrScanBody)
from mason_cli.web_server_profiles import _mason_home_scope
...
_cron_profile_home = late("_cron_profile_home", "mason_cli.web_server_cron")
...
def _profile_setup_command(name: str) -> str:
    ...
    return "mason setup" if name == "default" else f"{name} setup"


def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (MASON_HOME-scoped);
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from mason_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map mason_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _spawn_install(ident: str):
        return _spawn_mason_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``mason profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``mason profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the MASON_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``mason profile
...
# ── Export / Import ── wraps mason_cli.profiles.export_profile / import_profile. Paths are
