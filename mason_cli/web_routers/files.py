...
from mason_cli.web_server_files import (
...
# points the managed root at MASON_HOME. Mirrors the two canonical guards
...
# trees are blocked wherever they sit under the root, no MASON_HOME resolution.
...
def _decode_data_url(data_url: str) -> tuple[bytes, str]:
    from mason_cli.web_server import _MANAGED_FILE_MAX_BYTES
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    the returned gateway-visible path under ``MASON_HOME/images/`` (the same
    ...
    def _run():
        ...
        with _profile_scope(profile) as scoped_home:
            img_dir = Path(scoped_home or get_mason_home()) / "images"
...
async def stream_upload_to_path(
    ...
    *,
    ...
    """
    from mason_cli.web_server import _MANAGED_FILE_MAX_BYTES, _UPLOAD_CHUNK_BYTES
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the Electron ``mason:fs:writeText`` hardening: path validated by
    ...
    tmp = target.with_name(f".{target.name}.mason-tmp-{os.getpid()}")
    try:
...
async def fs_read_data_url(path: str):
    from mason_cli.web_server import _FS_DATA_URL_MAX_BYTES
    target, st = _fs_regular_file(_fs_path(path))
