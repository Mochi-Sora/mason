...
from mason_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_mason_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from mason_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from mason_cli.tools_config import _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from mason_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
async def get_toolsets(profile: Optional[str] = None):
    from mason_cli.tools_config import (
        ...
        gui_toolset_label)
    from mason_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `mason tools` runs — otherwise the toggle "saves" but the tool
...
async def select_toolset_provider(
    ...
    ``mason tools``, so both write identical keys).
    ...
    """
    from mason_cli.tools_config import apply_provider_selection, web_provider_capabilities
...
async def run_toolset_post_setup(
    ...
    """Spawn ``mason tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's MASON_HOME."""
    from mason_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``mason computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('mason_cli.web_deps', 'LateState'),
}
