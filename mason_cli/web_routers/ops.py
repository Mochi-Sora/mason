...
from mason_cli.config import redact_key
from mason_cli.web_deps import late
from mason_cli.web_server_files import _path_is_under
from mason_cli.web_server_gateway import _restart_gateway_after
from mason_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from mason_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from mason_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("mason_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "mason_cli.web_server_memory")
...
save_config = late("save_config", "mason_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``mason <argv>`` action; spawn failure -> 500."""
...
# --- Webhooks: same JSON store as the CLI (mason_cli.webhook); the adapter
...
async def list_webhooks():
    import mason_cli.webhook as wh
...
# --- Gateway lifecycle: spawn the real `mason gateway <verb>` so behaviour
...
async def start_gateway(profile: Optional[str] = None):
    with http_failure("Failed to spawn gateway start", 500, "Failed to start gateway"):
        proc = _spawn_mason_action(_gateway_subcommand(profile, "start"), "gateway-start")
...
async def stop_gateway(profile: Optional[str] = None):
    with http_failure("Failed to spawn gateway stop", 500, "Failed to stop gateway"):
        proc = _spawn_mason_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``mason auth add openai-codex``
            ...
            # (mirrors `mason auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from mason_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    the same RemovalStep registry as ``mason auth remove``: each source cleans
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"mason-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def list_hooks():
    ...
    def _run():
        from mason_cli.config import load_config as _load_config
        from agent import shell_hooks

        valid_events = []
        with contextlib.suppress(Exception):
            from mason_cli.plugins import VALID_HOOKS
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from mason_cli.plugins import VALID_HOOKS as valid_hooks
...
async def list_checkpoints():
    ...
    cp_dir = get_mason_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
