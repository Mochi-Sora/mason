...
from mason_cli.pty_session import RegistryFull
from mason_cli.web_deps import LateState, late
from mason_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "mason_cli.web_server_chat")
_profile_scope = late("_profile_scope", "mason_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "mason_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "mason_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "mason_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "mason_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "mason_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from mason_cli.web_server_chat import _LOOPBACK_HOSTS
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from mason_cli.console_engine import MasonConsoleEngine

        engine = MasonConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Mason Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from mason_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Mason inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("MASON_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_mason_auth_identity", None),
        subprotocol=getattr(ws, "_mason_ws_subprotocol", None),
    )


# --- /api/pub + /api/events: the PTY-side tui_gateway.entry opens /api/pub
# (MASON_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
