...
from mason_cli.web_routers._common import http_failure, scoped_to_thread
from mason_cli.web_deps import LateState, late
from mason_cli.web_server_config import (
...
from mason_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, redact_key, _deep_merge
from mason_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "mason_cli.web_server_messaging")
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different MASON_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``mason model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `mason model` picker uses.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level MASON_HOME (mirrors ``/api/config``).
    """
