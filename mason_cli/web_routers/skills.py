...
from mason_cli.web_server_profiles import _hub_action_name, _installed_hub_identifiers
...
# Labels per hub source id (matches `mason skills search` provenance); keep in
# sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Nous)",
    "mason-index": "Mason Index",
    "skills-sh": "skills.sh",
...
def _resolve_hub_skill(ident: str, profile: Optional[str]):
    ...
    from mason_cli.skills_hub import _resolve_source_meta_and_bundle
    from tools.skills_hub_search import create_source_router
...
# Sources subsumed by an available mason-index (progressive per-source fan-out
