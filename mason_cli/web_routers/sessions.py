...
from mason_cli.web_server_sessions import _maybe_auto_archive_for_profile, _session_latest_descendant
from mason_cli.web_models import (
    BulkDeleteSessions, SessionImport, SessionOwnerBackfill, SessionPrune, SessionRename)
from mason_cli.web_routers._common import log as _log, http_failure
from mason_state import is_malformed_db_error
from mason_state_errors import is_transient_sqlite_error

list_router = APIRouter()
search_router = APIRouter()
manage_router = APIRouter()

_cron_default_profile = late("_cron_default_profile", "mason_cli.web_server_cron")
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `mason sessions prune`)."""
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_mason_home()
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics (mirrors `mason sessions stats`)."""
    def _stats(db):
        out = {
