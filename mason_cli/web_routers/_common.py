...
late-binding seam in :mod:`mason_cli.web_deps` (web_server owns helpers/state;
...
log = logging.getLogger("mason_cli.web_server")
...
def spawn_profile_action(
    ...
    """Spawn a background ``mason -p <profile> <argv>`` action; a spawn
    failure is logged and becomes ``500 "<prefix>: <exc>"``."""
    with http_failure(log_msg, 500, prefix):
        proc = _spawn_mason_action(_profile_cli_args(profile) + argv, name)
