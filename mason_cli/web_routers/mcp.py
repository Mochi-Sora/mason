...
from mason_cli.web_server_mcp import _mcp_oauth_flows, _mcp_server_summary, _normalize_mcp_server_create
from mason_cli.web_models import MCPCatalogInstall, MCPEnabledToggle, MCPServerCreate, MCPServersReplace
from mason_cli.web_routers._common import (
    _profile_cli_args, _profile_scope, _spawn_mason_action, config_write_scope, http_failure,
...
_run_dashboard_mcp_oauth = late("_run_dashboard_mcp_oauth", "mason_cli.web_server_mcp")
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from mason_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from mason_cli.mcp_config import _get_mcp_servers, _save_bearer_auth_token, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    from mason_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from mason_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from mason_cli.mcp_config import _get_mcp_servers, _oauth_tokens_present, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # endpoint. The probe only needs MASON_HOME for .env + token resolution.
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    def _home() -> str:
        return str(get_mason_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(f.server_name == name and f.mason_home == flow_home for f in live):
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Mason and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Mason for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Mason.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    with http_failure("mcp_catalog import failed", 500, "Catalog unavailable"):
        from mason_cli import mcp_catalog
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from mason_cli import mcp_catalog
    from mason_cli.config import validate_env_var_name_for_write
    ...
    if entry.install is not None:
        ...
        try:
            _spawn_mason_action(_profile_cli_args(effective_profile) + ["mcp", "install", name], action)
        except HTTPException:
            raise
        except Exception as exc:
