...
from mason_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
...
def _nous_recommended_default() -> dict:
    from mason_cli import models as m
    from mason_cli import models_pricing as mp
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``mason model``'s curation so GUI onboarding lands on a sensible default.
    ...
    try:
        ...
        from mason_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from mason_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.mason/config.yaml`` — applies to **new** sessions only; a running chat
