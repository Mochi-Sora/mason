...
Extracted from ``mason_cli.web_server``; app state and helpers are late-bound through
:mod:`mason_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from mason_cli.web_server_gateway import _display_system_platform
...
from mason_cli.config import get_config_path, get_env_path
from mason_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from mason_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "mason_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "mason_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "mason_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "mason_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "mason_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "mason_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "mason_cli.web_server_lifecycle")
...
check_config_version = late("check_config_version", "mason_cli.config")
...
load_config = late("load_config", "mason_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "mason_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Mason process uses it."""
    from mason_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from mason_cli.web_server import _SSH_OWNER_NONCE
...
# Profile segment mirrors mason_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level MASON_HOME (see issue #69143). Plain
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from mason_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from mason_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from mason_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_mason_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from mason_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from mason_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of mason_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_mason": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_mason_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Mason Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "mason_home": str(get_mason_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond mason home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "mason_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_mason_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``mason <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_mason_action(argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from mason_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from mason_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from mason_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_mason_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from mason_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
