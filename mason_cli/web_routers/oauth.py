...
from mason_cli.web_server_oauth import (
...
from mason_cli.web_models import OAuthSubmitBody
...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "mason_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Mason "
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from mason_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from mason_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from mason_cli.auth import _save_codex_tokens
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from mason_cli import auth as hauth
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from mason_cli.auth import PROVIDER_REGISTRY, _request_device_code
    pconfig = PROVIDER_REGISTRY["nous"]
    portal_base_url = (
        os.getenv("MASON_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from mason_cli.auth import _xai_oauth_request_device_code
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Mason still
    # OWNS its credential (the PKCE file ~/.mason/.anthropic_oauth.json and its
    # credential-pool entry, written by `mason auth add anthropic`), so it is
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Mason — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``mason model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        from mason_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"mason auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Mason-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_mason_oauth_file
        oauth_file = _get_mason_oauth_file()
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str] = None):
    ...
    def _run():
        ...
        try:
            from mason_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
