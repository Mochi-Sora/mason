"""Windows gateway lifecycle for ``mason update``: pause/resume/cold-start the service, sweep venv holders, reap orphaned backends.

Split out of ``update_cmd.py``; names are re-imported there so ``mason_cli.update_cmd.<name>`` still resolves/monkeypatches.
...
logger = logging.getLogger("mason_cli.update_cmd")  # log-record parity with the origin module
...
def _self_and_non_gateway_ancestor_pids(psutil) -> set[int]:
    ...
    with suppress(Exception):
        # Never return ourselves or our own ancestry: a CLI ``mason update`` runs from the venv python and
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    The mason.exe shim guard misses the Desktop backend and anything off ``venv\\Scripts\\python(w).exe``;
    ...
    for proc in proc_iter:
        ...
        name_low = name.lower()
        if not (in_venv or name_low.startswith(("python", "pypy")) or name_low in {"uv.exe", "uvx.exe", "mason.exe"}):
            continue
        ...
        # its .pyd files — match cmdline (venv path, or `-m mason_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "mason_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _mason_holder_subcommand(cmdline: str) -> str | None:
    """The actual Mason SUBCOMMAND a venv-holder argv runs, or None (callers must NOT guess a label).

    Token-based, never substring (``kanban --preserve-cache`` contains "serve"): find the ``mason_cli.main`` /
    ``mason(.exe)`` entry token, return the first following token that isn't a flag or a flag's value.
    ...
    def _is_entry(i: int, token: str) -> bool:
        ...
        return (low.endswith("mason_cli.main") and i > 0 and tokens[i - 1] == "-m") or (
            low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("mason", "mason.exe"))
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    Labels come from the parsed SUBCOMMAND, never substring: a standalone ``mason dashboard`` must not be
    called the Desktop backend, ``--preserve-cache`` must not match "serve". Unknown argv gets no hint.

    See #90778.
    """
    hint_by_subcommand = {
        "serve": "  ← Mason backend (if the Desktop app is open, close it)",
        "dashboard": "  ← mason dashboard (stop it: mason dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    lines = ["✗ Other Mason processes are running from this install's venv:"]
    for pid, name, cmdline in matches[:6]:
        hint = hint_by_subcommand.get(_mason_holder_subcommand(cmdline) or "", "")
    ...
    lines.append(
        ...
        "  Close the Mason desktop app / other Mason terminals, then re-run:\n    mason update\n"
        "  (or use `mason update --force-venv` to proceed anyway at your own risk)"
    )
    return "\n".join(lines)
...
def _leftover_pausable_gateway_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    from mason_cli._scan_venv_blockers import _is_pausable_gateway
    psutil = _psutil()
    pids: list[int] = []
...
def _refuse_gateway_ancestor_tree_kill(pids: list[int], *, gateway_mode: bool) -> bool:
    """Refuse a plain Windows update that would tree-kill its own ancestry (a chat agent's ``mason update`` is
    ...
    if gateway_mode or not pids:
        return False
    def _ancestors():
        from mason_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print(
        ...
        "  From a chat platform, use `/update` instead.\n  Otherwise, run `mason update` from a separate terminal."
    )
    return True
...
def _ledger_manual_serve_holders(matches: list[tuple[int, str, str]]) -> list[dict]:
    ...
    try:
        from mason_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    with suppress(Exception):
        ...
        if scripts_dir is not None:
            mason = next((str(scripts_dir / n) for n in ("mason.exe", "mason") if (scripts_dir / n).is_file()), mason)
    ...
    for entry in entries:
        ...
        commands.append(
            [mason] + (["--profile", profile] if profile and profile != "default" else [])
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    from mason_cli.update_cmd import _m, _record_update_step
    ...
    if skipped or failed:
        print("  ⚠ Some stopped backends could not be relaunched automatically; restart them manually (mason serve --host <ip> --port <port>).")
    _record_update_step(
...
def _is_backend_argv(argv_low: str) -> bool:
    """Whether a lower-cased argv is a Desktop backend (``mason_cli.main`` running ``serve``/``dashboard``)."""
    return "mason_cli.main" in argv_low and (" serve" in argv_low or " dashboard" in argv_low)
...
def _orphaned_desktop_backend_pids(matches: list[tuple[int, str, str]]) -> list[tuple[int, int]] | None:
    ...
    would dead-end the update with "Mason is still running" and zero open windows. Qualifies only if cmdline
    is a Mason backend AND the parent is demonstrably gone (PID missing or reused). Tree-aware: holders inside
    ...
    it tree-kills its backends and waits for the venv shim before spawning mason-setup, and the
    ...
    dead-ends the update with "Mason is still running" while the user stares at zero open windows (ryanc's
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # gateway.status.get_process_start_time so pid_is_mason round-trips at kill time.
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from mason_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _handoff_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    hand-off. Inside the hand-off gate (marker + ``--gateway`` + no live ``mason.exe`` shim) nothing legitimate
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    ``taskkill /T /F``: stopping only the parent can leave a ``.mason-runtime`` child holding the install open.

    See #70026.
    """
    from gateway.status import get_process_start_time
    from mason_cli._subprocess_compat import pid_is_mason, windows_hide_flags
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_mason(pid, expected_start_time=expected_start_time):
                logger.debug("Skipping taskkill of non-Mason or changed PID %s", pid)
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``mason serve`` / ``mason dashboard`` argv (Desktop control plane).
    ...
    """
    return "mason_cli.main" in (cmdline or "").lower() and _mason_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _restore_windows_gateway_service(name: str, *, timeout: float = 60.0) -> None:
    ...
    from mason_cli.update_cmd import _start_windows_gateway_service
    service = _win_service(name)[1]
...
def _windows_cold_start_plan() -> dict | None:
    ...
    from mason_cli.update_cmd import _desktop_owns_gateway_lifecycle
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from mason_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from mason_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    Scheduled/startup gateways run via pythonw.exe, invisible to the mason.exe instance guard, yet keep files
    ...
    with _abort_on_error("Could not prepare Windows gateway pause for update"):
        ...
        from mason_cli.gateway import _capture_gateway_argv
    ...
    print("→ Stopping Windows gateway process(es) before updating Mason...")
    ...
    if unmapped_pids:
        ...
        if any(not u.get("argv") for u in unmapped):  # no recoverable cmdline (psutil missing, denied, gone)
            print("    Restart manually after update: mason gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    PID.
    """
    from mason_cli.update_cmd import _desktop_owns_gateway_lifecycle, _m
    if not _m()._is_windows():
        return True
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    artifacts written once at install time — ``mason update`` never touched them, so installs created
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    """Overwrite ``$MASON_HOME/bootstrap-cache/install-<ref>.{ps1,sh}`` for *branch* from the fresh checkout.

    Old ``mason-setup.exe`` builds NEVER re-download a cached branch-ref script, so a stale one runs
    ...
    so the poisoned cache outlives every ``mason update``.
    ...
    with _best_effort('Could not refresh bootstrap-cache scripts after update: %s'):
        cache_dir = Path(_m().get_mason_home()) / "bootstrap-cache"
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from mason_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
...
def _verify_relaunched_gateways_alive(token: dict, profiles: dict, unmapped: list) -> None:
    ...
    if not ready_pids:
        ...
        print(
            ...
            "    Recover with: mason gateway restart"
        )
...
def _in_handoff_without_live_shim(args) -> bool:
    """GUI hand-off gate: ``--gateway`` + update-incomplete marker AND no live ``mason.exe`` shim.
    ...
    try:
        if not (bool(getattr(args, "gateway", False)) and _m()._update_marker_path().exists()):
            return False
        scripts_dir = _m()._venv_scripts_dir()
        return scripts_dir is not None and not _m()._detect_concurrent_mason_instances(scripts_dir)
    except Exception:
        return False
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    from mason_cli.update_cmd import _m, _record_update_step, _refuse_gateway_ancestor_tree_kill
    ...
    for classifier, message in (
        (_m()._ledger_reapable_backend_pids, "ledger-identified orphaned Mason backend process(es) hold the venv"),
    ...
    # Manual serve/dashboard rung (e.g. `mason serve --host <ip>` for a REMOTE Desktop): ledger identity
    ...
    if holders and _in_handoff_without_live_shim(args) and (handoff_backends := _m()._handoff_reapable_backend_pids(holders)):
        holders = _reap_and_rescan(
            f"  ⚠ {len(handoff_backends)} Mason backend process(es) "
