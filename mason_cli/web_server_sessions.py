...
def _session_db_read_probe_statements() -> tuple:
    ...
    from mason_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from mason_state import SessionDB, is_malformed_schema_error
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    """
    from mason_cli.web_server_cron import _cron_profile_home
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    ``mason serve`` runs neither CLI nor gateway startup hooks, so this
