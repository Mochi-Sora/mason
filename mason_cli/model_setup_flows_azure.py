...
Imports of mason_cli.config / azure_detect stay lazy (tests patch them at call time).
...
from mason_cli.model_setup_flows_common import _HTTP, _ask, _commit_model_config, _load_config_model_section, _say
...
def _azure_entra_preflight(current_entra: dict):
    ...
    if not has_azure_identity_installed():
        _say("◐ The 'azure-identity' package is not installed yet.",
             "  Mason will install it now (the preflight below triggers the lazy-install). "
...
def _azure_detect_transport(effective_url: str, effective_key: str, token_provider, current_api_mode: str):
    ...
    from mason_cli import azure_detect
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    _say("", "Azure Foundry Configuration", "=" * 50, "",
         ...
         "Anthropic-style API endpoints.  Mason will probe your",
