...
maps. Split out of ``mason_cli.models``.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``mason model`` flow share one source."""
    from mason_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $MASON_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Mason-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $MASON_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see mason_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (mason model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `mason model` picker.
# ---------------------------------------------------------------------------
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``mason model``, setup wizard,
...
# Offline/fresh-install fallback for the model Mason silently lands on when the user never picked
...
# (GUI onboarding / ``mason model``) uses the tier-aware ``get_recommended_default_model`` in
# mason_cli/web_server.py + ``partition_nous_models_by_tier``, which may hit the Portal.
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Mason' default
