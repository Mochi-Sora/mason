...
Mixin split out of ``cli.py``; bound onto ``MasonCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``MasonCLI.<name>(self, ...)``.
"""
...
def _heal_bare_custom_provider(provider, *, base_url, model):
    ...
    try:
        ...
        from mason_cli.runtime_provider import canonical_custom_identity
...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from mason_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from mason_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _commit_model_switch(
    ...
    from cli import MasonCLI, _cprint
    old_model = cli.model
    ...
    if not one_turn:
        MasonCLI._persist_model_switch_to_session(cli, result)
...
def _persist_global_switch(cli, result) -> None:
    ...
    from cli import MasonCLI, save_config_value
    MasonCLI._clear_persisted_context_for_model_switch(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from mason_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from mason_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        from mason_cli.models import opencode_provider_family
        if opencode_provider_family(resolved_provider) is not None:
            from mason_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from mason_state import SessionDB as _SessionDB
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current Mason session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `mason model` / gateway pickers); live catalog only when
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from mason_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from mason_cli.moa_config import moa_usage, normalize_moa_config
