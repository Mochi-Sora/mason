...
Split out of :mod:`mason_cli.plugins`. Names that tests patch on the origin
...
from mason_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from mason_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from mason_cli.plugins_manifest import (
...
ENTRY_POINTS_GROUP = "mason_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "mason_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``mason_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from mason_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    user_dir = get_mason_home() / "plugins"
    logger.debug("Scanning user plugins: %s", user_dir)
    _scan("user", user_dir, "user")
    if _origin._env_enabled("MASON_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".mason" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set MASON_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        error = (
            "removed — Relay lifecycle is owned by Mason core; configure "
            f"{RELAY_PLUGINS_CONFIG_ENV} instead"
        )
        return _placeholder(error, logging.WARNING, "Refusing to load removed Mason Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `mason`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `mason plugins enable {lookup_key}` to activate)", logging.DEBUG,
