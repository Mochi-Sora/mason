"""mason hooks — inspect and manage shell-script hooks."""
...
def hooks_command(args) -> None:
    """Entry point for ``mason hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)
    if not sub:
        print("Usage: mason hooks {list|test|revoke|doctor}")
        print("Run 'mason hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.mason/config.yaml.")
        print("See `mason hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.mason/config.yaml.")
    else:
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if drift is True:
                        print(
                            ...
                            f"run `mason hooks doctor` to re-validate"
                        )
            print()
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Mason wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if drift is True:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `mason hooks revoke` + re-approve to refresh")
        elif drift is False:
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `mason hooks doctor`.")
