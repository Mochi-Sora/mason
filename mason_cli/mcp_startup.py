...
from mason_constants import get_mason_home_override, reset_mason_home_override, set_mason_home_override
...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(raw_config.get("mcp_servers"), dict) and raw_config["mcp_servers"]:
            return True
        from mason_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Re-install the caller's context-local MASON_HOME override (multi-profile dashboard/desktop
        ...
        # #67605.
        home_override = get_mason_home_override()

        def _discover() -> None:
            token = set_mason_home_override(home_override)
            try:
...
def _resolve_discovery_timeout(explicit: "float | None", *, single_query: bool = False) -> float:
    ...
    try:
        from mason_cli.config import load_config, DEFAULT_CONFIG
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``mason -z``) can construct ``AIAgent`` before any
