...
def parse_bang_command(text: str) -> str:
    ...
    (history expansion), not to Mason.
    """
...
def bang_shell_enabled() -> bool:
    ...
    return not (env_var_enabled("MASON_GATEWAY_SESSION") or env_var_enabled("MASON_CRON_SESSION")
                or (os.getenv("MASON_SESSION_PLATFORM") or "").strip())
...
def _bang_env() -> dict:
    """Environment for a bang command with Mason-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str] = None, timeout: int = DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Mason session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
