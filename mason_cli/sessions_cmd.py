...
``_DB_HANDLERS`` (everything else, sharing one ``SessionDB``). ``get_mason_home`` is resolved through
``mason_cli.main`` at call time so monkeypatches keep working. Picker: :mod:`mason_cli.sessions_cmd_browse`.
"""
...
from mason_cli.sessions_cmd_browse import _relative_time, _session_browse_picker


def get_mason_home():
    from mason_cli import main
    return main.get_mason_home()


def _sessions_dir() -> Path:
    return get_mason_home() / "sessions"
...
def _print_dry_run_preview(candidates, filters) -> None:
    from mason_cli.session_filters import describe_filters
...
def _export_dir(output) -> Path:
    """``--output`` dir for multi-file exports; ``~/.mason/session-exports`` when empty or ``-``."""
    return Path(output).expanduser() if output and output != "-" else get_mason_home() / "session-exports"
...
def _cmd_repair(args):
    from mason_state import DEFAULT_DB_PATH as db_path, SessionDB
    ...
    print(
        ...
        f"    mason sessions recover --source {source_hint} \\\n"
        "        --inspect-only\n"
...
def _cmd_recover(args):
    ...
    import sqlite3
    from mason_cli.session_recovery import (
...
def _cmd_import(args):
    from mason_cli.foreign_sessions import run_sessions_import
...
def _cmd_list(db, args):
    from mason_state_sessions import workspace_key as _ws_key
...
def _cmd_export(db, args):
    from mason_cli.session_filters import build_prune_filters
    filters = None
    ...
    def _redact(data):
        if not args.redact or data is None:
            return data
        from mason_cli.session_export_md import redact_session_data
...
def _render_only(args, sessions):
    ...
    from mason_cli.session_export import export_record_count, render_sessions_export
...
def _render_html(args, sessions):
    ...
    from mason_cli.session_export_html import generate_html_export, generate_multi_session_html_export
...
def _export_markdown(db, args, filters, redact):
    ...
    from mason_cli.session_export_md import append_manifest_entry, write_session_markdown
    if args.output == "-":
...
def _export_markdown_single(db, args, export_one, output_dir, lineage_is_logical):
    ...
    from mason_cli.session_export_md import verify_export_file
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from mason_cli.session_filters import format_epoch, parse_duration_seconds
...
def _note_pinned_skipped(db, filters, action):
    ...
    if action == "prune":
        verb = "deleted"
        optin = "Pass --include-pinned to delete them anyway, or unpin first with `mason sessions unpin <id>`."
    else:
        verb, optin = "archived", "Unpin first with `mason sessions unpin <id>` to include them."
...
def _cmd_prune_or_archive(db, args, action):
    ...
    from mason_cli.session_filters import build_prune_filters, describe_filters, format_epoch
    ...
    if skipped_open:
        print(f"Note: {skipped_open} open session{'' if skipped_open == 1 else 's'} also match these filters but "
              "will be skipped because prune only deletes ended sessions. Use `mason sessions delete <id>` "
...
def _cmd_pinned(db, args):
    ...
    if getattr(args, "json", False):
        ...
        return
    if not pinned_rows:
        print("No pinned sessions. Pin one with: mason sessions pin <session_id>")
        return
...
def _cmd_optimize_storage(db, args):
    ...
    if result.get("vacuumed") is False:
        print("  (VACUUM was skipped or failed — run `mason sessions optimize` later to reclaim freed space.)")
