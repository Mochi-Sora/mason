"""CLI subcommand: `mason curator <subcommand>`."""
...
def _print_unmanaged_summary() -> None:
    ...
    print("  never auto-staled or archived — `mason curator adopt <name>` hands one over")
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `mason curator status` later")
    if dry:
        print(
            "dry-run: no changes applied. Read the report with "
            "`mason curator status` and run `mason curator run` (no flag) to apply."
            if synchronous else
...
_PIN_MESSAGES = {
    True: (
        ...
        "external). `mason curator list-unmanaged` shows which skills the curator tracks.",
        ...
        "it. Run `mason curator adopt {skill}` to put it under curator management)",
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `mason curator adopt <name>`, "
          "or all with `mason curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`mason curator unpin {args.skill}`")
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.mason/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    for r in rows:
        ...
        print(
            ...
            f"{r.get('skill', '?')}{extra}")
    print(
        "\nRoll back a single mutation with `mason curator rollback <id>`; "
        "whole-tree snapshots remain available via `mason curator rollback --list`.")
    return 0
...
def _rollback_ledger_entry(args, entry_id: str) -> int:
    ...
    if entry is None:
        print(
            ...
            "See `mason curator ledger` for entry ids, or use "
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        if not curator_backup.list_backups():
            print(
                ...
                "`mason curator backup` or wait for the next curator run.")
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.mason/skills/ tree (a safety "
...
_SUBCOMMANDS = (
    ...
    (
        "backup",
        "Take a manual tar.gz snapshot of ~/.mason/skills/ "
    ...
    (
        "rollback",
        "Restore ~/.mason/skills/ from a curator snapshot, or a single "
        "mutation by ledger entry id (see `mason curator ledger`)",
        _cmd_rollback,
        _arg("entry_id", nargs="?", default=None,
             help="Ledger entry id for single-mutation rollback (from "
                  "`mason curator ledger`). Omit for whole-tree snapshot rollback."),
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by mason_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="mason curator")
    register_cli(parser)
