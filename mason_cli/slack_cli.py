"""``mason slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Mason defaults, tweakable in the Slack
    UI after pasting.
    """
    from mason_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Mason in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with Mason in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Mason agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``mason_cli/main.py``)."""
    name = getattr(args, "name", None) or "Mason"
    description = getattr(args, "description", None) or "Your Mason agent on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"mason slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_mason_home()) / "slack-manifest.json"
    else:
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your Mason app\n"
