"""Autostash handling for ``mason update``: stash before the pull, restore/park/discard afterwards, warn about orphans.
...
_AUTOSTASH_NAME_PREFIX = "mason-update-autostash-"
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    if push.returncode != 0:
        if not stash_created:
            ...
            print("  Commit, stash, or clean up your local changes manually, then re-run `mason update`.")
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Deliberately NOT a GC: a stash may be the only copy of the user's work, so Mason never drops one.
...
def _restored_python_paths(git_cmd: list[str], cwd: Path) -> tuple[str, ...] | None:
    ...
    from mason_cli.update_cmd import _git_untracked_paths
...
def _reject_unsafe_stash_restore(
    ...
    from mason_cli.update_cmd import _git_untracked_paths
    print()
    print("✗ Restored local changes made the Mason agent unexecutable.")
...
def _confirm_restore(stash_ref: str, input_fn) -> bool:
    ...
    print("  Review the result afterward if Mason behaves unexpectedly.")
...
def _apply_stash(git_cmd: list[str], cwd: Path, stash_ref: str) -> bool:
    ...
    print(f"  Stash ref: {stash_ref}")
    _reset_hard(git_cmd, cwd)  # conflict markers make mason unrunnable; changes stay in the stash
...
def _drop_restored_stash(git_cmd: list[str], cwd: Path, stash_ref: str) -> None:
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Mason couldn't find the stash entry to drop.")
        print(_STASH_LEFT_IN_PLACE)
        _print_stash_cleanup_guidance(stash_ref)
        return
    drop = _git_run(git_cmd, ["stash", "drop", stash_selector], cwd)
    if drop.returncode != 0:
        print("⚠ Local changes were restored, but Mason couldn't drop the saved stash entry.")
...
def _restore_stashed_changes(
    ...
    print("  Review `git diff` / `git status` if Mason behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(git_cmd: list[str], cwd: Path, stash_ref: str) -> bool:
    ...
    if stash_selector is None:
        print(
            ...
            "but Mason couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    drop = _git_run(git_cmd, ["stash", "drop", stash_selector], cwd)
    if drop.returncode != 0:
        print("⚠ Configured to discard local changes, but Mason couldn't drop the saved stash entry.")
