...
from mason_cli.providers import (
...
# Re-exported: callers/tests patch mason_cli.model_switch.<name>.
from mason_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Mason wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Mason.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Mason itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
_MASON_MODEL_WARNING = (
    "Nous Research Mason 3 & 4 models are NOT agentic and are not designed "
    "for use with Mason Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.).")

# Match only the real Nous Research Mason 3 / 4 chat families; a bare substring check
# false-positived on tool-capable local Modelfiles like ``mason-brain:qwen3-14b-ctx16k``.
#   match:    NousResearch/Mason-3-Llama-3.1-70B, mason-4-405b, openrouter/mason3:70b
#   no match: mason-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_MASON_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])mason[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nous_mason_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Nous Mason 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _NOUS_MASON_NON_AGENTIC_RE.search(model_name))


def _check_mason_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nous Mason 3/4 chat model, else ""."""
    return _MASON_MODEL_WARNING if is_nous_mason_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from mason_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``MasonCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from mason_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
    ...
    try:
        from mason_cli.models import normalize_provider
...
def _ollama_configured_base() -> tuple[dict, str]:
    from mason_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'mason model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'mason doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from mason_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from mason_cli.config import load_config
        from mason_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from mason_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from mason_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from mason_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from mason_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from mason_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_mason_model_warning(st.new_model)) if w]
    ...
    try:
        from mason_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('mason_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('mason_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
