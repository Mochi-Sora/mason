"""Git worktree isolation for ``mason -w`` sessions: create, classify, prune.
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True,
    ...
    *sync_base* branches from the fetched remote tip (``_resolve_worktree_base``), else local
    HEAD. *name* replaces the random ``mason-<id>``; named trees lack the ``mason-`` prefix so
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run mason -w")
        return None

    wt_name = ((name and re.sub(r"[^A-Za-z0-9._-]+", "-", name).strip("-._")[:40])
               or f"mason-{uuid.uuid4().hex[:8]}")
    ...
    try:
        _git(["worktree", "lock", "--reason", f"mason pid={os.getpid()}", str(wt_path)], repo_root)
...
def _worktree_merge_cache_path() -> Path:
    ...
    return get_mason_home() / "cache" / "worktree_merge_verdicts.json"
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    """Lock state: ``"live"`` (owning pid runs), ``"dead"`` (pid gone / non-mason reason), None (unlocked).

    ``mason -w`` locks with reason ``mason pid=<pid>``; ``worktree remove --force`` refuses
    ...
    for line in listing.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"mason pid=(\d+)", reason)
            if not m:
...
def _prune_candidates(worktrees_dir: Path, max_age_hours: int, now: float) -> list:
    ...
    Kanban trees (``t_<hex>``) belong to the kanban gc. ``mason-*`` trees age on *max_age_hours*,
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        if not entry.is_dir() or kanban_re.match(entry.name):
            continue
        tier_hours = max_age_hours if entry.name.startswith("mason-") else max_age_hours * 3
        try:
...
def _classify_prune_candidates(repo_root: str, candidates: list) -> list:
    ...
    def _classify(item):
        ...
        # Live lock = running mason; a dead lock is unlocked in phase 3.
    ...
    try:
        if workers > 1:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="mason-wt-prune"
            ) as pool:
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    if preserved_stale:
        logger.warning("Preserving %d worktree(s) older than 7 days with unmerged work "
                       "(run `mason worktree prune` to review and reclaim): %s",
    ...
    try:
        from mason_cli.worktree_gc import worktrees_summary
        ...
        if count >= 10 or (size_mb or 0) >= 5120:
            ...
            logger.warning(".worktrees/ holds %d tree(s) (%s) — run `mason worktree list` "
                           "to audit and `mason worktree prune` to reclaim safely.", count, size_txt)
    except Exception:
        pass


def _prune_orphaned_branches(repo_root: str, protect: Optional[set] = None) -> None:
    """Delete local ``mason/mason-*`` and ``pr-*`` branches with no worktree, except *protect*."""
    try:
