...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from mason_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"mason {candidate}", f"mason_cli.main {candidate}", f"mason_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Mason installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``mason-serve.service``), or None.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``mason update``, detached, logging to
    ...
    log_path = get_mason_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``mason update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # mason_cli.config.get_mason_home to simulate setup failure.
        from mason_cli.config import get_mason_home as _get_mason_home
        logs_dir = _get_mason_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        log_file.write(f"\n=== mason update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No mason dashboard or serve processes running.")
        return 0

    print(f"{len(live)} mason dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from mason_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OAuth via Nous Portal (run `mason dashboard register`)\n    [3] Cancel\n")
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    mason dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "MASON_DASHBOARD_OAUTH_CLIENT_ID into ~/.mason/.env for you.\n"
            "  Docs: README.md in your Mason checkout"
    ...
    try:
        ...
        from mason_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from mason_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.mason/
    # desktop-ssh, independent of MASON_HOME and the active profile. Anchor
    # validation there, NOT get_mason_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_mason_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".mason" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``mason dashboard`` inheriting that ``MASON_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (MASON_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("MASON_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "mason_cli.main",
        "-p", "default",
    ...
    # MASON_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping MASON_HOME: in the Docker layout the root is /opt/data, and an
    # unset MASON_HOME would fall back to $HOME/.mason = /opt/data/.mason — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_mason_root() strips a trailing profiles/<name> for both layouts.
    try:
        from mason_constants import get_default_mason_root
        env["MASON_HOME"] = str(get_default_mason_root())
    except Exception:
        env.pop("MASON_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets MASON_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless MASON_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed MASON_WEB_DIST can't be populated.
    """
    from mason_cli.main import PROJECT_ROOT
    from mason_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["MASON_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "MASON_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["MASON_WEB_DIST"])
            if "MASON_WEB_DIST" in os.environ
            else PROJECT_ROOT / "mason_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom MASON_WEB_DIST
            ...
            _recoverable = "MASON_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # MASON_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # MASON_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["MASON_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ MASON_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset MASON_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads MASON_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["MASON_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from MASON_WEB_DIST: {_dist_root}")
