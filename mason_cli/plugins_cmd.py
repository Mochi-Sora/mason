"""``mason plugins`` CLI subcommand — install, update, remove, and list plugins."""
...
from mason_cli.plugin_capabilities import _child_dict
...
def _plugins_dir() -> Path:
    ...
    plugins = get_mason_home() / "plugins"
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from mason_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.mason/.env``."""
    env_specs = [
...
def _print_python_dependencies(manifest: dict, console) -> None:
    """Print declared ``python_dependencies`` with an install hint — Mason never auto-installs
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_mason_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_mason_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_mason_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_mason_home() / "plugins" / ".install-metadata.json"
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    if mv_int > _SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"Run {recommended_update_command()} to update Mason.",
        ) from None
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `mason plugins update {plugin_name}`.")
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    from mason_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        ...
        _fail(console, (
            ...
            f"community index ({source}). Use `mason plugins search <term>` to "
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Mason plugin.")
    ...
    if enable:
        ...
        console.print(
            ...
            f"Run `mason plugins enable {installed_name}` to activate.[/dim]")
    ...
    console.print("[dim]  mason gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`mason plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from mason_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `mason plugins enable {name}` "
            f"if you trust them.")
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``mason setup`` / ``mason plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from mason_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Mason core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from mason_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from mason_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from mason_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`mason plugins capabilities {plugin_id}` to review and "
            f"`mason plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`mason plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``mason plugins capabilities [<id>]`` — declared vs granted."""
    from mason_cli.plugin_capabilities import (
        CAPABILITY_REGISTRY,
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`mason plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from mason_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``mason plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] mason plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] mason plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] mason plugins")
    console.print("[dim]Enable/disable:[/dim] mason plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from mason_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] mason plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from mason_cli.curses_ui import _addnstr, flush_stdin
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    def _from_manifest_on_disk() -> Optional[str]:
        from mason_cli.plugins import get_bundled_plugins_dir
        return next((
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.mason/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.mason/plugins/<name>``."""
    ...
    try:
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `mason plugins install {rec.get('source', '<source>')} --force "
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[bool, str]:
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "mason-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``mason
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.mason/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from mason_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from mason_cli.plugin_index import SECURITY_FOOTER, load_index, search_index
    console = _console()
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: mason plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")
...
def _action_pack(args):
    from mason_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``mason plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from mason_cli.plugin_compat import (
...
def plugins_command(args) -> None:
    """Dispatch mason plugins subcommands."""
