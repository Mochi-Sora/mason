"""CLI handlers for ``mason secrets onepassword ...``.

Unlike Bitwarden, the ``op`` binary is NOT auto-installed: 1Password publishes the CLI through OS
package managers and signed installers, so Mason expects an already-installed, already-
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Mason resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]mason secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]mason secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]mason secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]mason secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Mason will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]mason secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]mason secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    return rotate_token(
        ...
        disabled_note=None if op_cfg.get("enabled") else (
            ...
            "run `mason secrets onepassword setup` to turn it on.[/yellow]"
        ),
    )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not references:
        console.print(
            ...
            "`mason secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
    ...
    console.print(
        ...
        "[cyan]mason[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    return disable_secret_source(
        ...
        "next Mason invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]mason secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them.",
    )
...
_PLUGIN_COMPAT_LAZY = {
    'masked_secret_prompt': ('mason_cli.secret_prompt', 'masked_secret_prompt'),
}
