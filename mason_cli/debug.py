"""``mason debug`` debug tools for Mason Agent."""
...
_REDACTION_BANNER = (
    "[mason debug share: log content redacted at upload time. "
...
_USER_AGENT = "mason-agent/debug-share"
...
# ``mason debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_mason_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        pass  # non-fatal — worst case the user runs ``mason debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `mason debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    boundary = "----MasonDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from mason_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_mason_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Mason Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``mason dump`` and return its stdout as a string."""
    from mason_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``mason dump`` is run internally.
    """
    buf = io.StringIO()
...
_NOUS_BUNDLE_FORMAT = "mason-debug-share/1"
...
def build_debug_share(
    *, log_lines: int = 200, expiry: int = 7, redact: bool = True) -> DebugShareResult:
    ...
    Shared by ``mason debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "mason debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `mason debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
          "To delete now:  mason debug delete <url>\n"
          "\nShare these links with the Mason team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Mason version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``mason debug share --nous``: gzip the same bundle into the Nous envelope → Nous-S3."""
    from mason_cli.diagnostics_upload import share_to_nous
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("mason debug share --nous: applied force-mode redaction before upload")
    ...
    try:
        ...
        print(f"\nNous upload failed: {exc}\n"
              ...
              "Run `mason debug share --local` to print the report instead, "
              "or `mason debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nShare this private link with the Nous team — only Nous staff "
          ...
          "  GitHub Issues        https://github.com/NousResearch/mason-agent/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: mason debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'mason debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
