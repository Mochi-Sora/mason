"""Per-provider model-selection wizard flows for ``mason setup`` / ``mason model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``mason_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`mason_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from mason_cli.model_setup_flows_common import (
...
from mason_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from mason_cli.model_setup_flows_azure import _model_flow_azure_foundry
from mason_cli.model_setup_flows_bedrock import _model_flow_bedrock


def _env_base_url(base_url_env: str) -> str:
    """Base-URL override from ``.env`` then the process environment ('' when unset)."""
    from mason_cli.config import get_env_value
    if not base_url_env:
        return ""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from mason_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from mason_cli.models import model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from mason_cli.main_provider_setup import _prompt_api_key
    from mason_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from mason_cli.config import get_env_value
    ...
    from mason_cli.models import ai_gateway_model_ids
    from mason_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from mason_cli.auth import _save_model_choice
    ...
    if not presets:
        print("No MoA presets configured. Run `mason moa configure <name>` first.")
        return
...
def _nous_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    ...
    if free_tier:
        try:
            from mason_cli.nous_account import format_nous_portal_entitlement_message, get_nous_portal_account_info
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from mason_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _nous_persist_selection(selected: str, creds: dict) -> dict:
    ...
    from mason_cli.auth import _save_model_choice, _update_config_for_provider
    from mason_cli.config import get_env_value, load_config, save_config, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from mason_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_nous, PROVIDER_REGISTRY
    from mason_cli.config import load_config
    from mason_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from mason_cli.models import check_nous_free_tier, get_curated_nous_model_ids
    from mason_cli.models_pricing import get_pricing_for_provider
    ...
    from mason_cli.nous_account import nous_policy_notice
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `mason auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from mason_cli.models import provider_model_ids
    ...
    # credentials may live only in the pool (``mason auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from mason_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from mason_cli.auth import (
        get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, DEFAULT_QWEN_BASE_URL)
    from mason_cli.models import fetch_api_models
...
def _copilot_catalog(api_key: str):
    ...
    from mason_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from mason_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from mason_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from mason_cli.main_provider_setup import _prompt_reasoning_effort_selection
    from mason_cli.setup import _current_reasoning_effort, _set_reasoning_effort
    from mason_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from mason_cli.config import load_config
    from mason_cli.models import fetch_api_models, github_model_reasoning_efforts, copilot_model_api_mode
    provider_id = "copilot"
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from mason_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Mason turns to `copilot --acp`.",
         "  Mason currently starts its own ACP subprocess for each request.",
         "  Mason uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set MASON_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from mason_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
    from mason_cli.config import get_env_value, save_env_value
    from mason_cli.models import _PROVIDER_MODELS
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from mason_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from mason_cli.auth import PROVIDER_REGISTRY
    from mason_cli.config import save_env_value
    from mason_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from mason_cli.auth import _prompt_model_selection
    from mason_cli.config import load_config, get_env_value
    from mason_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.mason/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``mason_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from mason_cli.main_provider_setup import _prompt_provider_choice
    from mason_cli.auth import ZAI_ENDPOINTS
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Mason typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Mason, enable billing on your Google Cloud project and regenerate",
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from mason_cli.auth import PROVIDER_REGISTRY
    from mason_cli.config import save_env_value, load_config
    from mason_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from mason_cli.main_provider_setup import _run_anthropic_oauth_flow
    from mason_cli.config import save_env_value, save_anthropic_api_key
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('mason_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('mason_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('mason_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('mason_cli.providers', 'custom_provider_slug'),
