"""Slash command registry for the Mason CLI and gateway.
...
from mason_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``mason_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Mason config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Mason works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .mason/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Mason' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Mason is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.mason/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Mason Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Mason Agent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``mason skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('mason_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('mason_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('mason_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('mason_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('mason_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('mason_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('mason_cli.commands_platforms', 'telegram_menu_max_commands'),
}
