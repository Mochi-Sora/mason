"""Baked-in build metadata for Mason Agent.

Source installs report their git revision live via ``git rev-parse`` (``mason_cli/dump.py``,
``mason_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.mason_build_sha`` file written by the Dockerfile.
"""
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".mason_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.mason_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``MASON_GIT_SHA`` build-arg, full 40-char hash on one
