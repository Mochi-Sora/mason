...
Mixin split out of ``cli.py``; bound onto ``MasonCLI`` via the MRO. cli.py-internal symbols are
...
from mason_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from mason_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from mason_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Mason needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from mason_cli.model_switch import is_nous_mason_non_agentic
        if is_nous_mason_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Mason 3 & 4 models are NOT agentic and are not "
                "designed for use with Mason Agent.[/]")
            self._console_print(
        ...
        # `mason skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `mason skills trust` to enable them.[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from mason_cli.models import model_supports_fast_mode
        except Exception:
            return False
    ...
    def show_help(self, arg: str = ""):
        ...
        from mason_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from mason_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        from mason_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_mason_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_mason_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from mason_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from mason_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `mason auth` first.")
            return
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.mason/skills/ and queue a note for the next user turn.
