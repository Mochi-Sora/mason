...
from mason_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import mason_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $MASON_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_mason_agent_browser() or _has_npx_agent_browser()),
...
def _has_mason_agent_browser() -> bool:
    from mason_constants import get_mason_home  # late import: tests patch mason_constants.get_mason_home
    home = get_mason_home()
    if _IS_WINDOWS:  # npm -g --prefix puts .cmd shims directly in the prefix dir
        return (home / "node" / "agent-browser.cmd").is_file()
    # install.sh installs into $MASON_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-MasonHome", str(get_mason_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**mason_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
