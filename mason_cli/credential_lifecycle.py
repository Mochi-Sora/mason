"""Unified provider-credential lifecycle across every store Mason reads.
...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    from mason_cli.config import get_config_path, require_readable_config_before_write
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `mason auth remove` does: a lingering shell export (or
    ...
    _for_each_provider(providers, "mason_cli.auth.suppress_credential_source", f"env:{env_var}")
    if clear_models_cache and providers:
        # Best-effort — a cache failure must not block the credential removal itself.
        _for_each_provider(providers, "mason_cli.models.clear_provider_models_cache")
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    kept 401'ing until the user ran ``mason auth add <provider> --type api-key`` separately. This makes the
    Desktop save's effect on disk match what ``mason auth add`` does.
    """
    from mason_cli.config import load_env, save_env_value
    ...
    _for_each_provider(providers, "mason_cli.auth.unsuppress_credential_source", f"env:{env_var}")
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    from mason_cli.config import load_env, remove_env_value
