"""Configuration management for Mason Agent: config.yaml / .env loading, saving,
validation, migration, and the ``mason config`` command."""
...
from mason_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from mason_cli.secret_prompt import masked_secret_prompt
# Re-export from mason_constants — canonical definition lives there.
from mason_constants import get_mason_home, get_process_mason_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (mason config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  mason config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Mason restarts through them), PATH (fix tool lookup
...
# and Mason runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``MASON_*`` overall is NOT blocked — many integration credentials use that prefix
# (MASON_LANGFUSE_PUBLIC_KEY, MASON_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Mason runtime location
    "MASON_HOME", "MASON_PROFILE", "MASON_CONFIG", "MASON_ENV",
    "MASON_CONFIG_PATH", "MASON_ENV_PATH",
    ...
    "MASON_COPILOT_ACP_COMMAND", "MASON_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "MASON_YOLO_MODE", "MASON_ACCEPT_HOOKS", "MASON_REDACT_SECRETS",
    "MASON_INTERACTIVE", "MASON_EXEC_ASK", "MASON_GATEWAY_SESSION",
    "MASON_CRON_SESSION", "MASON_SINGLE_QUERY_SESSION",
    "MASON_SESSION_KEY", "MASON_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Mason runtime location and security policy (MASON_HOME, MASON_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.mason/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean MASON_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "MASON_TOOL_PROGRESS_MODE",
    ...
    "MASON_LANGFUSE_ENV", "MASON_LANGFUSE_RELEASE", "MASON_LANGFUSE_SAMPLE_RATE",
    "MASON_LANGFUSE_MAX_CHARS", "MASON_LANGFUSE_CAPTURE", "MASON_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "MASON_ACP_AUTH_METHOD", "MASON_ACP_AUTO_APPROVE", "MASON_COPILOT_ACP_COMMAND",
    "MASON_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# MASON_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: MASON_MANAGED env var (systemd service) or a ``.managed`` marker file in
    MASON_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("MASON_MANAGED", "").strip().lower() or None
    managed_marker = get_mason_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Mason is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Mason through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Mason was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$MASON_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because MASON_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``mason update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``nousresearch/mason-agent`` image bakes a ``docker`` stamp into
    ``/opt/mason`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of mason_cli/), NOT of $MASON_HOME,
    ...
    method = _install_method_stamp(get_mason_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``mason_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/mason-agent:latest",
    "apt": "pkg upgrade mason-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "mason update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (mason_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Mason.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``mason update`` doesn't apply inside the Docker container.

Mason Agent runs as a published image (nousresearch/mason-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/mason-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate mason-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/mason-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/mason-agent/tags
  • Your config and session history live under ``$MASON_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``mason update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Mason installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Mason installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Mason.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MASON_HOME/.container-mode.
    ...
    already inside the container, or when MASON_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("MASON_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_mason_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "mason-agent"),
        "exec_user": info.get("exec_user", "mason"),
        "mason_bin": info.get("mason_bin", "/data/current-package/bin/mason")}


# ---- Config paths / MASON_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_mason_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / MASON_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("MASON_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_mason_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read MASON_UID / MASON_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns MASON_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Mason commonly set these to map the in-container user to a host user so
    ...
    return _env_int("MASON_UID"), _env_int("MASON_GID")


def _chown_to_mason_uid(path) -> None:
    """Chown ``path`` to ``MASON_UID:MASON_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_mason_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_mason_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply MASON_UID/GID ownership. No-op when managed.
    MASON_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse MASON_HOME to
    a served subdirectory without directory listings.

    Also applies ``MASON_UID``/``MASON_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("MASON_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_mason_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or MASON_CONTAINER / MASON_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("MASON_CONTAINER") or os.environ.get("MASON_SKIP_CHMOD")
...
_MASON_HOME_ENSURED: set = set()
_MASON_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_mason_home():
    """Ensure the ~/.mason directory skeleton exists with secure permissions.
    ...
    from mason_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _MASON_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"MASON_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _MASON_HOME_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _MASON_HOME_ENSURED.add(key)


# ---- Config loading/saving ----

from mason_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from mason_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use mason_cli.config.<name>)
...
# Back-compat re-exports — :mod:`mason_cli.personality` owns personality/overlay semantics.
from mason_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``mason config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `mason config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `mason update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (mason_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # mason_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'mason tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'mason setup' to create a valid config")]
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Mason won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'mason doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        from mason_constants import display_mason_home

        hint_path = display_mason_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from mason_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ mason config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from mason_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from mason_cli.toolset_validation import validate_platform_toolsets
        from mason_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: mason config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.mason/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_mason_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``mason config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``mason status``/``dump``) read ``name`` and
...
# the MASON_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Mason host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_mason_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_mason_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — mason auth) — OpenAI Codex
#   nous         (OAuth — mason auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.mason/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Mason writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.mason/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.mason/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.mason/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``MASON_CUSTOM_`` prefix keeps the
    ...
    return f"MASON_CUSTOM_{slug}_API_KEY" if slug else "MASON_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.mason/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from mason_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.mason/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Mason (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.mason/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Mason-managed credential preferring ``~/.mason/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale MASON_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("MASON_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale MASON_MAX_ITERATIONS={env_ghost} "
                    f"(run 'mason doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from mason_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ⚕ Mason Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from mason_cli.auth import get_anthropic_key
    ...
    print(color("  mason config edit     # Edit config file", Colors.DIM))
    print(color("  mason config set <key> <value>", Colors.DIM))
    print(color("  mason setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``MASON_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("MASON_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `mason cron list`, then pin the intended values with "
        "`mason cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``mason config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``mason config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    mason config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    mason config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_mason_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Mason may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `mason config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from mason_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: mason config get <key> [--json]", [
    "mason config get model", "mason config get terminal.backend",
    "mason config get skills.config --json"], None)
_USAGE_SET = ("Usage: mason config set [--force] <key> <value>", [
    "mason config set model anthropic/claude-sonnet-4", "mason config set terminal.backend docker",
    "mason config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: mason config unset <key>", [
    "mason config unset model", "mason config unset terminal.backend",
    "mason config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'mason config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  mason config           Show current configuration
  mason config edit      Open config in editor
  mason config get <key>          Print a resolved config value
  mason config set <key> <value>   Set a config value
  mason config unset <key>        Remove a config value
  mason config check     Check for missing/outdated config
  mason config migrate   Update config with new options
  mason config path      Show config file path
  mason config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``mason config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``mason_cli/`` — i.e. the git checkout for source
    installs, ``/opt/mason`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MASON_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MASON_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/mason`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('mason_cli.route_identity', 'normalize_route_base_url'),
}
