"""Helpers shared by the Bitwarden and 1Password ``mason secrets`` CLIs.

Import-light on purpose: ``mason_cli.secrets_cli`` must stay free of the Bitwarden backend
...
def require_enabled(console: Console, cfg: dict, product: str, command: str) -> bool:
    ...
    console.print(f"[yellow]{product} integration is disabled.  Run "
                  f"`mason secrets {command} setup` first.[/yellow]")
    return False
...
def rotate_token(
    ...
    console.print(f"[green]✓[/green] stored in {env_path()} as {token_env}.  "
                  "Takes effect on the next Mason invocation.")
    if disabled_note:
