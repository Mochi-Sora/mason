...
Scope is deliberately narrow: Mason only upgrades an npm inside its **own** managed Node tree
(``$MASON_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Mason-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$MASON_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$MASON_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
    the "upgrade" would install a second npm elsewhere while the managed one stayed stale.
    """
    if not quiet:
        print(f"→ Upgrading Mason-managed npm to satisfy {npm_range}…", flush=True)
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Mason-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="mason-npm-upgrade-") as tmp:
...
def _probe_version(npm: str) -> str | None:
    try:
        result = subprocess.run([npm, "--version"], timeout=30, env=with_mason_node_path(), **_RUN_KW)
    except (OSError, subprocess.SubprocessError):
        return None
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Mason could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$MASON_HOME/node`` and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Mason-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_mason_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool = False) -> str | None:
    ...
    if prefix is not None:
        # Mason owns this npm — upgrade in place. Only an npm-range failure is fixable this way.
