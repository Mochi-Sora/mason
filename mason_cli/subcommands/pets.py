...
def build_pets_parser(subparsers) -> None:
    ...
    pets_parser = subparsers.add_parser(
        ...
        description="Petdex (https://github.com/crafter-station/petdex) is a public "
            ...
            "and Mason shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app.")
    try:
        from mason_cli.pets import register_cli as _register_pets_cli

        _register_pets_cli(pets_parser)
    except Exception as _exc:
        logging.getLogger("mason_cli.main").debug("pets CLI wiring failed: %s", _exc)
