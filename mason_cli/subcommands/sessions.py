"""``mason sessions`` subcommand parser."""
...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_export.add_argument("output", nargs="?",
        help="Output path. JSONL: file path (use - for stdout, required). "
            "md/qmd: output directory (default: <mason home>/session-exports)")
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import", help="Import a Claude Code or Codex CLI session into Mason",
        description="Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Mason session store "
            "so it can be resumed with 'mason --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in mason_cli/sessions_cmd.py; the parser is threaded
