"""``mason import-agent`` subcommand parser (logic lives in ``mason_cli/agent_import.py``)."""

from __future__ import annotations

from typing import Callable

from mason_cli.subcommands._shared import add_yes_flag
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent", help="Import a Claude Code or Codex CLI setup into Mason",
        description="One-command import of another coding agent's setup into Mason. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Mason equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'mason setup' for those.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Overwrite existing Mason items on name conflicts (default: skip)")
