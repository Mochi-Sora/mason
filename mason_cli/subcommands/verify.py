"""``mason verify`` subcommand parser.

Follows the pattern of ``mason_cli/subcommands/doctor.py``: parser built
...
# avoid paying an extra module import on every `mason` invocation.
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description="Detect how the current project is built, tested, and started "
            "(or load the saved manifest at .mason/environment.json), then "
    ...
    verify_parser.add_argument(
        ...
        help="Save the recipe as .mason/environment.json in the project")
    verify_parser.add_argument(
