"""``mason computer-use`` subcommand parser."""

from __future__ import annotations

import sys

from mason_cli.subcommands._shared import add_json_flag


def _cu_install(args) -> int:
    from mason_cli.tools_config import _cua_driver_contract_status, install_cua_driver
...
def _cu_status(args) -> int:
    import os as _os
    import subprocess
    from mason_cli.tools_config import _cua_driver_contract_status
    ...
    override = _os.environ.get("MASON_CUA_DRIVER_CMD", "").strip()
    if not path:
        print("cua-driver: not installed")
        print("  Run: mason computer-use install")
        return 1
    version = ""
    try:
        from mason_cli.tools_config import _cua_driver_env
        ...
        pass
    from mason_cli.tools_config import _cua_version_summary
    version = _cua_version_summary(version)
    # Name the override here too. Without it the operator is told to repair an
    # install that `mason computer-use install` will (correctly) refuse to touch,
    # with nothing pointing at the env var that actually selected the binary.
    origin = " [custom binary from MASON_CUA_DRIVER_CMD]" if override else ""
    ...
    if not contract.get("ready"):
        ...
        if override:
            print(
                "    Update the binary selected by MASON_CUA_DRIVER_CMD, or unset "
                "the override and run: mason computer-use install --upgrade")
        else:
            print("    Run: mason computer-use install")
        return 1
    try:
        ...
        if st and st.get("update_available"):
            ...
            print("    Run: mason computer-use install --upgrade")
        elif st:
            print("  ✓ Up to date.")
        else:
            # Older driver (no check-update verb) or offline.
            print("  Refresh to latest: mason computer-use install --upgrade")
    except Exception:
        print("  Refresh to latest: mason computer-use install --upgrade")
    return 0
...
def _cu_perms_status(args) -> None:
    ...
    if not st["platform_supported"]:
        ...
        sys.exit(1)
    if not st["installed"]:
        print("cua-driver: not installed. Run: mason computer-use install")
        sys.exit(1)
    ...
    if st["can_grant"]:  # macOS TCC permissions
        ...
        if not st["ready"]:
            print("  Grant: mason computer-use permissions grant")
...
def build_computer_use_parser(subparsers) -> None:
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description="Install or check the cua-driver binary used by the\n"
            ...
            "Linux.\n\n"
            "Use `mason computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `mason tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `mason computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description="Computer Use drives the Mac through cua-driver, whose TCC grants\n"
            ...
            "terminal or the Mason app. `status` reports the driver's grant\n"
