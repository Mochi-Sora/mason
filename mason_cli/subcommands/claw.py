...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Mason")
    claw_subparsers = claw_parser.add_subparsers(dest="claw_action")

    # claw migrate
    claw_migrate = claw_subparsers.add_parser(
        "migrate", help="Migrate from OpenClaw to Mason",
    ...
    claw_migrate.add_argument(
        ...
        help="Skip the pre-migration zip snapshot of ~/.mason/ (by default a "
        "single restore-point archive is written to ~/.mason/backups/ "
        "before apply; restorable with 'mason import').")
    claw_migrate.add_argument(
