"""``mason skills`` subcommand parser."""

from __future__ import annotations

from typing import Callable

from mason_cli.subcommands._shared import add_json_flag, add_yes_flag
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser("trust",
        help="Trust a project so its repo-local skills (./.mason/skills, ./.agents/skills) load")
    ...
    skills_reset = skills_subparsers.add_parser("reset",
        ...
        description="Clear a bundled skill's entry from the sync manifest (~/.mason/skills/.bundled_manifest) "
            "so future 'mason update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified", help="List bundled skills you've edited (which `mason update` keeps)",
        description="Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `mason update` reports as user-modified and skips. "
            "Use `mason skills diff <name>` to see changes and `mason skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description="Write the .no-bundled-skills marker so the installer, "
            "`mason update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description="Remove the .no-bundled-skills marker so bundled skills are seeded "
            "again on the next `mason update`. Pass --sync to re-seed now.")
