"""``mason pause`` / ``mason resume`` — the global emergency stop.

``pause`` writes the ESTOP sentinel at ``$MASON_HOME/ESTOP``; cron, kanban and new gateway
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Mason paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `mason resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Mason resumed — dispatch picks up on the next tick.")
    else:
        print(f"Mason is not paused (no sentinel at {sentinel_path()}).")
    return 0
