"""``mason dashboard`` / ``mason serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `mason dashboard|serve`
    # cmdlines and SIGTERM them — the same path `mason update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Mason web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Mason web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="mason serve",
        description="Run the Mason backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Mason Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically")
    # Compat shim: desktop shells <= 0.15.x spawn `mason dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Mason backend server (headless; powers the desktop app and remote backends)",
        description="Run the Mason backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    _configure_serve_parser(serve_parser, cmd_dashboard=cmd_dashboard)

    # `register` is nested so bare `mason dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description="Register this install as a self-hosted dashboard with your Nous "
            ...
            "MASON_DASHBOARD_OAUTH_CLIENT_ID into ~/.mason/.env, and prints "
            "how to engage the login gate. Requires being logged in (mason setup).")
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Optional public HTTPS OAuth redirect URI for the dashboard, e.g. "
            "https://mason.example.com/auth/callback. Omit for localhost-only use.")
    dashboard_register_parser.add_argument(
        ...
        help="Override the Nous Portal base URL for registration (default: the "
            ...
            "portal. Also settable via MASON_DASHBOARD_PORTAL_URL. Mainly for "
