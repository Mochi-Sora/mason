"""``mason egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `mason proxy` is the separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description="Manage iron-proxy, the optional TLS-intercepting egress firewall "
            ...
            "https://mason-agent.nousresearch.com/docs/user-guide/egress/iron-proxy")

    from mason_cli import proxy_cli as _proxy_cli
    _proxy_cli.register_cli(egress_parser)

    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``mason proxy`` (dest='proxy_command').
