"""``mason peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Mason gateway running the ``api_server`` platform; its stock
...
cross-machine twin of ``mason -p <bot> chat --in ~ -c "Bot Chat"``. ``run``/``status``
/``stop`` do the same turn through the async Runs API. Peer labels/URLs live in
config.yaml (``bot_peers``); the key lives in ``~/.mason/.env`` as
``MASON_PEER_<NAME>_KEY``. ``<peer>/<profile>`` targets the ``/p/<profile>/`` mirror.
"""
...
def _peer_key_env(name: str) -> str:
    return f"MASON_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    request_headers = {
        ...
        "User-Agent": "mason-peer-dm"}
    if headers:
        request_headers.update(headers)
    req = urllib.request.Request(url, data=data, method=method, headers=request_headers)
    # The peer URL is user-registered (``mason peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's mason-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}.") from exc
        raise
    # Real api_server wraps the row: {"object": "mason.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (mason peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: mason peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: mason peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.mason/.env)")
...
def _peer_add(args) -> int:
    ...
    if key:
        ...
        print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.mason/.env")
    else:
        print(
            f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
            f"  mason peer add {name} --url {url} --key <key>\n"
            f"  (or add {_peer_key_env(name)}=<key> to ~/.mason/.env)")
    return 0
...
def _peer_list(args) -> int:
    peers = _load_peers()
    if not peers:
        print("No peers registered. Add one: mason peer add <name> --url http://host:port --key <API_SERVER_KEY>")
        return 0
...
def cmd_peer(args) -> int:
    ...
    if action not in {"dm", "run", "status", "stop"}:
        print("Unknown peer action. See: mason peer --help", file=sys.stderr)
        return 2
    try:
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer", help="Bot-to-bot DMs across machines (peer Mason gateways)",
        description="Register other Mason gateways as peers and message their agents. "
            "'mason peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'mason -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.mason/.env.",
        epilog=(
            "Examples:\n"
            "  mason peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  mason peer list\n"
            '  mason peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  mason peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  mason peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  mason peer status spark run_abc123\n"
            "  mason peer stop spark run_abc123\n"
            "  mason peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.mason/.env)")
    ...
    def _remote(name: str, help: str, *, run_id: bool):
        ...
        if run_id:
            sp.add_argument("run_id", help="Run ID returned by 'mason peer run'")
        else:
