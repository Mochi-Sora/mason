...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Mason log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    mason logs                    Show last 50 lines of agent.log
    mason logs -f                 Follow agent.log in real time
    mason logs errors             Show last 50 lines of errors.log
    mason logs gateway -n 100     Show last 100 lines of gateway.log
    mason logs gui -f             Follow gui.log in real time
    mason logs desktop -f         Follow desktop.log (Electron app boot/backend)
    mason logs --level WARNING    Only show WARNING and above
    mason logs --session abc123   Filter by session ID
    mason logs --component tools  Only show tool-related lines
    mason logs --since 1h         Lines from the last hour
    mason logs --since 30m -f     Follow, starting from 30 min ago
    mason logs list               List available log files with sizes
""")
