...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Mason Agent. Use 'mason debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    mason debug share              Upload debug report (asks for confirmation)
    mason debug share --yes        Skip confirmation (for scripts/CI)
    mason debug share --lines 500  Include more log lines
    mason debug share --expire 30  Keep paste for 30 days
    mason debug share --local      Print report locally (no upload)
    mason debug share --no-redact  Disable upload-time secret redaction
    mason debug share --nous       Upload to Nous-internal storage (private)
    mason debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'mason debug share'")
