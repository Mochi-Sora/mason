"""``mason gateway`` and ``mason proxy`` subcommand parsers."""
...
from mason_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if MASON_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy mason.service units from pre-rename installs",
        description="Stop, disable, and remove legacy Mason gateway unit files "
            "(e.g. mason.service) left over from older installs. Profile "
            "units (mason-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.mason/.env. "
            "Requires being logged in (mason setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.mason/.env and forwarded at provision. "
    ...
    proxy_start.add_argument("--provider", default="nous",
        help="Upstream provider: nous or xai (default: nous). See `mason proxy providers`.")
