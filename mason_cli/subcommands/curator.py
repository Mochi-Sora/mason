"""``mason curator`` subcommand parser."""
...
def build_curator_parser(subparsers) -> None:
    ...
    try:
        from mason_cli.curator import register_cli as _register_curator_cli

        _register_curator_cli(curator_parser)
    except Exception as _exc:
        logging.getLogger("mason_cli.main").debug("curator CLI wiring failed: %s", _exc)
