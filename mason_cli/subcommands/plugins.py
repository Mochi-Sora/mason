"""``mason plugins`` subcommand parser."""
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native Mason plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL, owner/repo shorthand (e.g. anpicasso/mason-plugin-chrome-profiles), "
            "or a bare plugin name resolved through the community index "
            "(see `mason plugins search`)")
    plugins_install.add_argument(
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `mason plugins enable <name>`",
    )
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (mason-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a mason-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-mason-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a mason-pack.yaml file, or an https:// URL")
