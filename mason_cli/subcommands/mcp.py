...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp", help="Manage MCP servers and run Mason as an MCP server",
        description="Manage MCP server connections and run Mason as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'mason mcp add' to connect to a new server, or\n"
            "'mason mcp serve' to expose Mason conversations over MCP.")
    mcp_sub = mcp_parser.add_subparsers(dest="mcp_action")

    mcp_serve_p = mcp_sub.add_parser(
        "serve", help="Run Mason as an MCP server (expose conversations to other agents)")
    ...
    mcp_sub.add_parser(
        "picker", help="Interactive catalog picker (also the default for `mason mcp`)")
    mcp_sub.add_parser("catalog", help="List Nous-approved MCPs available for one-click install")
    mcp_install_p = mcp_sub.add_parser(
        "install", help="Install a catalog MCP by name (e.g. `mason mcp install n8n`)")
