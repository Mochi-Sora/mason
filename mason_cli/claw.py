"""mason claw — OpenClaw migration commands."""
...
from mason_cli.config import get_mason_home, get_config_path, load_config, save_config
from mason_constants import get_optional_skills_dir
from mason_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
_SCRIPT_REL = Path("migration", "openclaw-migration", "scripts", "openclaw_to_mason.py")
...
_OPENCLAW_SCRIPT_INSTALLED = get_mason_home() / "skills" / _SCRIPT_REL

# Known OpenClaw directory names (current + legacy)
_OPENCLAW_DIR_NAMES = (".openclaw", ".clawdbot", ".moltbot")

# `mason claw migrate` flags/defaults. Secrets are never included implicitly: --migrate-secrets
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ⚕ Mason — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    if running and _warn_running(
        ...
        ("Messaging platforms (Telegram, Discord, Slack) only allow one "
         ...
         "Mason may try to use the same token, causing disconnects.",
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Mason gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Mason gateway is running with active connections: " + ", ".join(connected), [],
        ("Migrating bot tokens while the gateway is active will cause "
         "conflicts (Telegram, Discord, and Slack only allow one active "
         "session per token).",
         "Recommendation: stop the gateway first with 'mason gateway stop'."),
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_mason.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_mason", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: mason claw <command> [options]\n\nCommands:\n"
              "  migrate          Migrate settings from OpenClaw to Mason\n"
              "  cleanup          Archive leftover OpenClaw directories after migration\n\n"
              "Run 'mason claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Mason migration: preflight, preview, confirm, back up, apply."""
    ...
    if not opts.source_dir.is_dir():
        return _error_block(
            ...
            "You can specify a custom path: mason claw migrate --source /path/to/.openclaw")
    ...
    opts.mason_home = get_mason_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.mason_home}",
    ...
    if _confirm(opts.yes, "Proceed with migration?", default=True, declined="Migration cancelled.",
                non_tty=("Non-interactive session — preview only.",
                         "To execute, re-run with: mason claw migrate --yes")):
        _apply_migration(run_migrator, opts)
    # Source directory is left untouched — archiving is `mason claw cleanup`'s job.
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.mason_home.resolve(),
...
def _preview_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> bool:
    ...
    if conflicts > 0 and not opts.overwrite:
        _error_block(
            ...
            "Each conflict is an item whose target already exists in ~/.mason/. "
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    restorable with `mason import`: one restore point before any mutation, pruned to the last 5."""
    backup_archive: Optional[Path] = None
    if not opts.no_backup:
        try:
            from mason_cli.backup import create_pre_migration_backup
            from mason_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(mason_home=opts.mason_home)
            if backup_archive:
                print()
                print_success(f"Pre-migration backup: {backup_archive} "
                              f"({_format_size(backup_archive.stat().st_size)})")
                print_info(f"Restore with: mason import {backup_archive.name}")
        except Exception as e:
            return _error_block(
                f"Could not create pre-migration backup: {e}",
                "Re-run with --no-backup to skip, or free up disk space under the Mason home.",
    ...
    try:
        ...
        if backup_archive:
            _info(f"A pre-migration backup is available at: {backup_archive}",
                  f"Restore with: mason import {backup_archive.name}")
        return
...
def _cmd_cleanup(args):
    ...
    if running and not _warn_running(
        ...
        "Proceed anyway?",
        declined="Aborted. Stop OpenClaw first, then re-run: mason claw cleanup",
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} → {archive_path}")
                      ...
                      non_tty=(f"Non-interactive session — would archive: {source_dir}",
                               "To execute, re-run with: mason claw cleanup --yes")):
            try:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        _info("", "To execute the migration, run without --dry-run:",
              f"  mason claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if any(i.get("kind") == "provider-keys" and i.get("status") == "skipped" for i in items):
            ...
            _info("", "To migrate API keys, re-run with:",
                  "  mason claw migrate --migrate-secrets", "", "Or add your key manually:",
                  "  mason config set OPENROUTER_API_KEY sk-or-v1-...")
