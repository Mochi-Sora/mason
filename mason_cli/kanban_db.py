...
Lives under the shared Mason root: ``default`` board DB at ``<root>/kanban.db`` (pre-boards
back-compat), other boards at ``<root>/kanban/boards/<slug>/``; a worker on one board never sees
another. Board resolution: ``board=`` arg > ``MASON_KANBAN_BOARD`` > ``MASON_KANBAN_DB`` (pins the
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    from mason_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("MASON_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    try:
        from mason_cli.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    try:
        from mason_cli.lifecycle import has_hook
...
# ``heartbeat_claim`` or raise it via MASON_KANBAN_CLAIM_TTL_SECONDS.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    """Explicit ``ttl_seconds`` > ``MASON_KANBAN_CLAIM_TTL_SECONDS`` > default."""
    if ttl_seconds is not None:
        return max(1, int(ttl_seconds))

    return _env_int("MASON_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    """``MASON_KANBAN_CRASH_GRACE_SECONDS`` (0 = immediate, for tests) else default."""
    return _env_int("MASON_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``MASON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("MASON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "mason_kanban_current_board_override", default=None,
)
...
def kanban_home() -> Path:
    """``MASON_KANBAN_HOME`` else ``get_default_mason_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's MASON_HOME would
    fork the board per profile and break the dispatcher/worker handoff."""
    override = os.environ.get("MASON_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    """Active slug: context override -> ``MASON_KANBAN_BOARD`` -> ``<root>/kanban/current``
    ...
    for candidate in (
        ...
        os.environ.get("MASON_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """``kanban.db`` path: ``MASON_KANBAN_DB`` pins it (injected into workers);
    ``default`` -> ``<root>/kanban.db`` (back-compat), else the board dir."""
    return _board_path("MASON_KANBAN_DB", board, ("kanban.db",), "kanban.db")


def workspaces_root(board: Optional[str] = None) -> Path:
    """Per-board scratch workspace root (``MASON_KANBAN_WORKSPACES_ROOT`` wins);
    ``default`` keeps the legacy ``<root>/kanban/workspaces/``."""
    return _board_path("MASON_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")


def attachments_root(board: Optional[str] = None) -> Path:
    """Per-board attachments root (``MASON_KANBAN_ATTACHMENTS_ROOT`` wins). Workers
    read attachments by absolute path, so remote terminal backends must mount it."""
    return _board_path("MASON_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``mason kanban log``
...
class Task:
    ...
    session_id: Optional[str] = None         # originating MASON_SESSION_ID; NULL from CLI/dashboard
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (mason_cli/projects_db). When set,
    ...
    -- inside an agent loop that propagated ``MASON_SESSION_ID``. NULL
...
def _resolve_project_link(
    ...
    from mason_cli import projects_db as _pdb
...
# Canonical sort-order mappings for ``mason kanban list --sort``.
...
# dispatcher tick or a manual `mason kanban recompute` (issue #22459).
...
def _ctx_comments(lines: list[str], comments: list[Comment], now: int) -> None:
    ...
    worker" framing stops an operator-controlled MASON_PROFILE like
    "mason-system" being read as a system directive above an
    ...
    if omitted_note:
        lines.append(omitted_note)
    for c in shown:
        # Render author with explicit "comment from worker" framing so operator-controlled MASON_PROFILE
        # values like "mason-system" or "operator" can't be misread by the next worker as a system
...
def list_profiles_on_disk() -> list[str]:
    ...
    directly to avoid importing ``mason_cli.profiles`` at startup."""
    try:
        from mason_constants import get_default_mason_root
...
from mason_cli.kanban_db_connect import (  # noqa: E402
    _INITIALIZED_PATHS,
    init_db,
    write_txn,
)
from mason_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from mason_cli.kanban_db_dispatch import (  # noqa: E402
...
_PLUGIN_COMPAT_LAZY = {
    'DEFAULT_BUSY_TIMEOUT_MS': ('mason_cli.kanban_db_connect', 'DEFAULT_BUSY_TIMEOUT_MS'),
    'DEFAULT_LOG_BACKUP_COUNT': ('mason_cli.kanban_db_dispatch', 'DEFAULT_LOG_BACKUP_COUNT'),
    'DEFAULT_LOG_ROTATE_BYTES': ('mason_cli.kanban_db_dispatch', 'DEFAULT_LOG_ROTATE_BYTES'),
    'DERIVED_MAX_IN_PROGRESS_CEILING': ('mason_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_CEILING'),
    'DERIVED_MAX_IN_PROGRESS_FLOOR': ('mason_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_FLOOR'),
    'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS': ('mason_cli.kanban_db_dispatch', 'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS'),
    'KanbanDbCorruptError': ('mason_cli.kanban_db_connect', 'KanbanDbCorruptError'),
    'MEMORY_GUARD_MB_PER_WORKER': ('mason_cli.kanban_db_dispatch', 'MEMORY_GUARD_MB_PER_WORKER'),
    'RepairResult': ('mason_cli.kanban_db_connect', 'RepairResult'),
    'add_notify_sub': ('mason_cli.kanban_db_notify', 'add_notify_sub'),
    'advance_notify_cursor': ('mason_cli.kanban_db_notify', 'advance_notify_cursor'),
    'check_respawn_guard': ('mason_cli.kanban_db_dispatch', 'check_respawn_guard'),
    'claim_unseen_events_for_sub': ('mason_cli.kanban_db_notify', 'claim_unseen_events_for_sub'),
    'configured_max_in_progress': ('mason_cli.kanban_db_dispatch', 'configured_max_in_progress'),
    'connect': ('mason_cli.kanban_db_connect', 'connect'),
    'connect_closing': ('mason_cli.kanban_db_connect', 'connect_closing'),
    'count_notify_subs': ('mason_cli.kanban_db_notify', 'count_notify_subs'),
    'count_running_tasks': ('mason_cli.kanban_db_dispatch', 'count_running_tasks'),
    'count_running_tasks_other_boards': ('mason_cli.kanban_db_dispatch', 'count_running_tasks_other_boards'),
    'derive_default_max_in_progress': ('mason_cli.kanban_db_dispatch', 'derive_default_max_in_progress'),
    'detect_crashed_workers': ('mason_cli.kanban_db_dispatch', 'detect_crashed_workers'),
    'detect_stale_running': ('mason_cli.kanban_db_dispatch', 'detect_stale_running'),
    'dispatch_once': ('mason_cli.kanban_db_dispatch', 'dispatch_once'),
    'enforce_max_runtime': ('mason_cli.kanban_db_dispatch', 'enforce_max_runtime'),
    'has_spawnable_ready': ('mason_cli.kanban_db_dispatch', 'has_spawnable_ready'),
    'has_spawnable_review': ('mason_cli.kanban_db_dispatch', 'has_spawnable_review'),
    'heartbeat_worker': ('mason_cli.kanban_db_dispatch', 'heartbeat_worker'),
    'list_notify_subs': ('mason_cli.kanban_db_notify', 'list_notify_subs'),
    'purge_stale_done_notify_subs': ('mason_cli.kanban_db_notify', 'purge_stale_done_notify_subs'),
    'reap_worker_zombies': ('mason_cli.kanban_db_dispatch', 'reap_worker_zombies'),
    'reconcile_orphaned_running': ('mason_cli.kanban_db_dispatch', 'reconcile_orphaned_running'),
    'remove_notify_sub': ('mason_cli.kanban_db_notify', 'remove_notify_sub'),
    'repair_db': ('mason_cli.kanban_db_connect', 'repair_db'),
    'resolve_max_in_progress': ('mason_cli.kanban_db_dispatch', 'resolve_max_in_progress'),
    'resolve_workspace': ('mason_cli.kanban_db_workspace', 'resolve_workspace'),
    'review_dispatch_enabled': ('mason_cli.kanban_db_dispatch', 'review_dispatch_enabled'),
    'rewind_notify_cursor': ('mason_cli.kanban_db_notify', 'rewind_notify_cursor'),
    'run_daemon': ('mason_cli.kanban_db_dispatch', 'run_daemon'),
    'set_branch_name': ('mason_cli.kanban_db_workspace', 'set_branch_name'),
    'set_workspace_path': ('mason_cli.kanban_db_workspace', 'set_workspace_path'),
    'unseen_events_for_sub': ('mason_cli.kanban_db_notify', 'unseen_events_for_sub'),
    'worker_log_rotation_config': ('mason_cli.kanban_db_dispatch', 'worker_log_rotation_config'),
}
