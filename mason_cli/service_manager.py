...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``mason gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from mason_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `mason gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged mason user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Mason runtime call
    inside the container drops to mason via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``mason_cli.gateway`` (systemd/launchd) and
# ``mason_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``mason -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``mason_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import mason_cli
        importlib.import_module(f"mason_cli.{self._backend}")
        return getattr(mason_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in mason_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in mason_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``mason_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `mason profile create` inside the container. Static services (main-mason, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's MASON_HOME blindly: derive the shared profile root and map the suffix to the root
    default profile or ``<root>/profiles/<profile>``.
    """
    profile = _profile_from_service(name)
    validate_profile_name(profile)
    mason_home = Path(os.environ.get("MASON_HOME", "/opt/data"))
    root = mason_home.parent.parent if mason_home.parent.name == "profiles" else mason_home
...
# UID/GID of the in-image ``mason`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_MASON_UID = 10000
_MASON_GID = 10000


def _chown_mason(path: Path) -> None:
    try:
        os.chown(path, _MASON_UID, _MASON_GID)
    except PermissionError:
        # Already running as mason → the dir is mason-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create mason-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    mason user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`mason profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources MASON_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to mason.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root MASON_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``MASON_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``mason_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        lines.append("export MASON_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker — same meaning for the profile-redirect guard in
        # mason_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export MASON_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its MASON_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``mason gateway run``, an agent action, the Open
        ...
        if profile == "default":
            gateway_cmd = "mason gateway run --replace"
        else:
            gateway_cmd = f"mason -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID → s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid mason {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{MASON_HOME:=/opt/data}}"\n'
            f'log_dir="$MASON_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock AS MASON when starting as root. Never
            # chown/unlink mason-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded mason-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid mason mkdir -p "$log_dir"\n'
            f'  s6-setuidgid mason rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid mason s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed mason-owned supervise/ BEFORE publishing so the mason-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is mason-owned (see ``_seed_supervise_skeleton``)
