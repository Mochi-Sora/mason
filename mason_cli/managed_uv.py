"""Mason-managed uv and Python runtime repair.

The Python backing the install is shared by every Mason profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.mason-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
from mason_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".mason-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.mason.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Mason' own uv binary (``$MASON_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_mason_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def managed_python_env(
    ...
    """Return a sanitized environment for Mason-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Mason keeps databases "
              "out of WAL mode on this SQLite build. The next `mason update` "
              "will retry.")
        return
...
def _uv_self_update_stamp() -> Path:
    from mason_constants import get_mason_home
    return get_mason_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``mason update`` daily; a blocking network
...
def _reload_mason_constants():
    """Re-execute ``mason_constants`` from disk (the imported one may predate venv_python_path)."""
    import mason_constants
    return importlib.reload(mason_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Mason generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `mason update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("mason_cli.main")
    ...
    if holders:
        ...
        return True, f"other Mason processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``mason update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``mason.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\mason.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `mason update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m mason_cli.main update",
            "    Sessions stay protected meanwhile: Mason keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$MASON_HOME/bin/`` instead of ``~/.local/bin/``.
    """
