...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_mason_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from mason_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``mason auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:mason_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> None:
    """``mason auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nous credential at <mason-root>/shared/nous_auth.json
    # (written by any previous login) makes `mason --profile <name> auth add nous --type oauth`
...
def auth_add_command(args) -> None:
    ...
    if spec is None:
        raise SystemExit(f"`mason auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Mason reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `mason auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `mason auth` is called bare."""
