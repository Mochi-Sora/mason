"""Dashboard process-hygiene helpers — extracted from ``mason_cli/main.py``.

Helpers defined in ``mason_cli.main_dashboard`` / ``mason_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("mason", "mason_cli.main", "mason_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "mason.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``mason update`` (every API call 401s). *exclude_pids* (Desktop's MASON_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Mason
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``mason serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``MASON_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`mason --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from mason_cli.process_identity import ledger_entries
...
def _mason_home_for_pid(pid: int) -> str | None:
    """Best-effort ``MASON_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("MASON_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"MASON_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``mason update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], mason_home: str | None = None) -> str:
    """Stable owner key: ``MASON_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if mason_home:
        parts = _resolved_home(mason_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(mason_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, mason_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``MASON_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``MASON_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``mason update`` respawn detaches
    ...
    for _pid, argv, mason_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and mason_home and _normalized_home_for_compare(mason_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, mason_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``MASON_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("MASON_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_mason(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not mason-owned or process identity changed"))
            else:
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``mason dashboard`` / ``mason serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``mason update``'s systemd fleet-restart loop, which restarts ``mason-serve*`` units before this
    ...
    """
    from mason_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. mason-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "mason-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + MASON_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot MASON_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _mason_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (mason serve) under Restart=on-failure never comes
...
def _detect_concurrent_mason_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``mason.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from mason_cli.main_install_repair import _mason_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _mason_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``mason serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("mason" not in cmd and "mason_cli" not in cmd):
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``mason serve`` it spawns. Such a backend
...
def _mason_home_dir() -> Path:
    """Resolved Mason home (MASON_HOME override or ~/.mason)."""
    override = os.environ.get("MASON_HOME", "").strip()
    return Path(override).expanduser() if override else Path.home() / ".mason"
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "masonPath", "masonHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated MASON_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{mason_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _mason_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local mason serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``mason serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / MASON_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after MASON_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
