#!/usr/bin/env python3
"""Mason CLI - Main entry point.

Usage:
    mason                     # Interactive chat (default)
    mason chat / gateway / setup / status / cron / doctor / update / ...
    mason --version           # Show version and update status
    mason <cmd> --help        # Per-command help
"""

# mason_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``mason update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``mason update``.
try:
    import mason_bootstrap  # noqa: F401
...
from mason_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python mason_cli/main.py`` (script mode: sys.path[0]
# is mason_cli/, not the repo root) can import mason_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from mason_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``mason update`` may have left a recovery marker with a core package wiped;
# the mason_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``mason_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from mason_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from mason_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Cosmetic: show 'mason' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``mason.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("mason")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"mason", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"mason")
    except Exception:
        pass
...
# mason_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("MASON_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".mason", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``MASON_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("MASON_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``MASON_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MASON_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``mason --version`` is answered before config/logging imports.
...
from mason_cli.subcommands.cron import build_cron_parser
from mason_cli.subcommands.sync import build_sync_parser
from mason_cli.subcommands.gateway import build_gateway_parser
from mason_cli.subcommands.profile import build_profile_parser
from mason_cli.subcommands.model import build_model_parser
from mason_cli.subcommands.setup import build_setup_parser

from mason_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from mason_cli.subcommands.slack import build_slack_parser
from mason_cli.subcommands.login import build_login_parser
from mason_cli.subcommands.logout import build_logout_parser
from mason_cli.subcommands.auth import build_auth_parser
from mason_cli.subcommands.status import build_status_parser
from mason_cli.subcommands.pause import build_pause_parser
from mason_cli.subcommands.webhook import build_webhook_parser
from mason_cli.subcommands.hooks import build_hooks_parser
from mason_cli.subcommands.doctor import build_doctor_parser
from mason_cli.subcommands.verify import build_verify_parser
from mason_cli.subcommands.security import build_security_parser
from mason_cli.subcommands.approvals import build_approvals_parser
from mason_cli.subcommands.dump import build_dump_parser
from mason_cli.subcommands.debug import build_debug_parser
from mason_cli.subcommands.backup import build_backup_parser
from mason_cli.subcommands.import_cmd import build_import_cmd_parser
from mason_cli.subcommands.import_agent import build_import_agent_parser
from mason_cli.subcommands.config import build_config_parser
from mason_cli.subcommands.skin import build_skin_parser
from mason_cli.subcommands.console import build_console_parser
from mason_cli.subcommands.update import build_update_parser
from mason_cli.subcommands.uninstall import build_uninstall_parser
from mason_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from mason_cli.subcommands.gui import build_gui_parser
from mason_cli.subcommands.logs import build_logs_parser
from mason_cli.subcommands.prompt_size import build_prompt_size_parser
from mason_cli.subcommands.memory import build_memory_parser
from mason_cli.subcommands.acp import build_acp_parser
from mason_cli.subcommands.tools import build_tools_parser
from mason_cli.subcommands.insights import build_insights_parser
from mason_cli.subcommands.monitoring import build_monitoring_parser
from mason_cli.subcommands.skills import build_skills_parser
from mason_cli.subcommands.pairing import build_pairing_parser
from mason_cli.subcommands.plugins import build_plugins_parser
from mason_cli.subcommands.mcp import build_mcp_parser
from mason_cli.subcommands.claw import build_claw_parser
from mason_cli.subcommands.moa import build_moa_parser
from mason_cli.subcommands.fallback import build_fallback_parser
from mason_cli.subcommands.worktree import build_worktree_parser
from mason_cli.subcommands.browser import build_browser_parser
from mason_cli.subcommands.secrets import build_secrets_parser
from mason_cli.subcommands.egress import build_egress_parser
from mason_cli.subcommands.migrate import build_migrate_parser
from mason_cli.subcommands.checkpoints import build_checkpoints_parser
from mason_cli.subcommands.bundles import build_bundles_parser
from mason_cli.subcommands.curator import build_curator_parser
from mason_cli.subcommands.pets import build_pets_parser
from mason_cli.subcommands.journey import build_journey_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'mason {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any mason module import: many modules
# cache MASON_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# MASON_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.mason/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors mason_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `mason mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Mason' own profile selector.
    """
    try:
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`mason chat -p
    ...
    """
    from mason_cli._parser import top_level_value_flag_sets
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo mason -p <name>` against the invoking user's home.
    ...
    try:
        ...
        candidate = Path(pwd.getpwnam(sudo_user).pw_dir) / ".mason" / "profiles" / name
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin MASON_HOME to the profile dir; a bare invocation
    means "the root MASON_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``mason profile use``) would silently redirect the default gateway into
    ...
    MASON_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    MASON_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), MASON_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("MASON_SUPERVISED_CHILD") or os.environ.get("MASON_S6_SUPERVISED_CHILD"):
        return True
    ...
    return os.environ.get(
        "MASON_GATEWAY_EXTERNAL_SUPERVISOR", ""
    ).strip().lower() in {"1", "true", "yes", "on"}


def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MASON_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # MASON_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the mason root (systemd hardcodes MASON_HOME=/root/.mason)
    # we must still read active_profile — the user may have run
    # `mason profile use` and the gateway should honour it (#22502).
    mason_home_env = os.environ.get("MASON_HOME", "")
    if profile_name is None and mason_home_env and Path(mason_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and not _under_gateway_supervisor(argv):
        try:
            ...
            active_path = get_default_mason_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from mason_cli.profiles import resolve_profile_env

        mason_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        mason_home = _resolve_sudo_user_profile_env(profile_name)
        if not mason_home:
        ...
        # A bug in profiles.py must NEVER prevent mason from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["MASON_HOME"] = mason_home
...
# Windows launcher self-heal — the ``mason`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``mason update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m mason_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no mason module may import before
...
# ``<checkout>\bin``, where ``mason update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``mason`` stopped
...
if sys.platform == "win32":
    try:
        from mason_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.mason/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from mason_cli.config import get_mason_home
from mason_cli.env_loader import load_mason_dotenv
...
# Bridge security.redact_secrets → MASON_REDACT_SECRETS BEFORE mason_logging
...
try:
    ...
    # mason_logging and later raw reads: 3-4 config.yaml parses become one.
    from mason_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_mason_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from mason_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "MASON_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["MASON_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from mason_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from mason_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if mason_constants not importable yet

import logging
import threading
from datetime import datetime

from mason_cli import __version__, __release_date__

from mason_cli.model_setup_flows import (
...
from mason_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from mason_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from mason_cli.main_dashboard import (
...
from mason_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from mason_cli.main_provider_setup import (
...
from mason_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from mason_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from mason_cli.main_desktop import (
    cmd_gui,
)
from mason_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from mason_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from mason_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from mason_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_mason_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MASON_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MASON_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.mason/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from mason_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    """
    from mason_cli.config import DEFAULT_CONFIG, get_env_path, get_mason_home, load_config
    from mason_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from mason_cli.config import split_model_config_default
    ...
    _has_mason_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_mason_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Mason is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_mason_config and not strict_profile_scope:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from mason_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``mason_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``mason -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, mason_bin
        cli_args: the original CLI arguments (everything after 'mason')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    mason_bin = container_info["mason_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo mason {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo mason {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'mason sessions list' to see available sessions, or "
            ...
            try:
                from mason_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from mason_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: mason --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'mason sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from mason_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from mason_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'mason doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.mason/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from mason_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from mason_cli.config import get_mason_home
            skills_dir = Path(get_mason_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import mason_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `mason setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Mason isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  mason setup")
    print()
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'mason setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["MASON_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["MASON_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older mason_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: MasonCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `mason` / `mason chat`.
        from mason_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from mason_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from mason_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``mason <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "mason_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "mason_cli.auth", "login_command", doc='Authenticate Mason CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "mason_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "mason_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "mason_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "mason_cli.cron", "cron_command", doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "mason_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "mason_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "mason_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "mason_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "mason_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "mason_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "mason_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "mason_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "mason_cli.backup", "run_import", doc='Restore a Mason backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "mason_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "mason_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "mason_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "mason_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "mason_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "mason_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "mason_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "mason_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from mason_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from mason_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from mason_cli.auth import AuthError, format_auth_error, resolve_provider
    from mason_cli.config import get_compatible_custom_providers, get_env_value
    from mason_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'mason model' for "
                "available providers, or run 'mason doctor' to diagnose config "
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``mason model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("MASON_INFERENCE_PROVIDER") or "auto"
    ...
    from mason_cli.models import _PROVIDER_LABELS
    ...
    # mason_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.mason/.env that poisons
...
# ``mason_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on mason_cli.main; importing update_cmd eagerly would cost every ``mason``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "mason_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_mason_modules",
    ...
    "mason_cli.dashboard_procs": (
        "_detect_concurrent_mason_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from mason_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `mason security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from mason_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `mason approvals <subcmd>`."""
    from mason_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from mason_cli.config import config_command
...
def cmd_backup(args):
    """Back up Mason home directory to a zip file."""
    from mason_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `mason --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Mason Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from mason_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from mason_cli.uninstall import run_gui_uninstall
    ...
    from mason_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from mason_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from mason_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Mason Agent")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Mason runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from mason_cli.update_inventory import (
    ...
    from mason_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from mason_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Mason Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from mason_cli.update_lock import (
    ...
    from mason_cli.update_cmd import _cmd_update_impl
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``mason -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from mason_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No mason dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `mason update`;
        # it prints outcomes itself. Exit 1 only if every pid was unkillable.
        from mason_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `mason serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with mason serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with MASON_DESKTOP=1 plus
    MASON_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    MASON_SERVE_HEADLESS=1). A shell inheriting those then running
    `mason dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (MASON_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    MASON_SERVE_HEADLESS itself.
    """
    if os.environ.get("MASON_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("MASON_WEB_DIST", "")):
            os.environ.pop("MASON_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("MASON_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Mason surface.
    try:
        from mason_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from mason_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("MASON_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from mason_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from mason_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from mason_cli import completion
...
def cmd_logs(args):
    """View and filter Mason log files."""
    from mason_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Mason command console."""
    from mason_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``mason`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``mason photon`` failing
    ...
    not import it, so the CLI registration never happens and ``mason photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("MASON_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, MASON_TUI env, display.interface config). Bare
    # `mason`/`mason chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: MASON_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from mason_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from mason_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from mason_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["MASON_SAFE_MODE"] = "1"
    os.environ["MASON_IGNORE_USER_CONFIG"] = "1"
    os.environ["MASON_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from mason_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("MASON_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from mason_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("MASON_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("MASON_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MASON_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MASON_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MASON_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `mason --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``mason-agent``) — matching is exact. ``MASON_AGENT`` is the
    Mason-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Mason running inside another agent's terminal).
    """
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from mason_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``mason sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from mason_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``mason`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``mason --help`` order; keep it stable. Groups
    live in ``mason_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``mason proxy`` (gateway group) is the INBOUND one.
    ...
    from mason_cli.send_cmd import register_send_subparser
    ...
    from mason_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from mason_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from mason_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from mason_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Mason-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``mason -c model`` — 'model' is the session name)
...
def main():
    """Main entry point for mason CLI."""
    _set_process_title()
    _advertise_agent_env()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from mason_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``mason.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_mason_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``mason update``, and
    ...
    # substring match is deliberately loose: over-matching (``mason skills
    # install update``) only defers recovery one launch; under-matching
    # (``mason -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from mason_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('mason_cli.cli_output', 'line_input'),
}
