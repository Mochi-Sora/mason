"""Helpers for loading Mason .env files consistently across entrypoints."""
...
# Once-per-process guards: load_mason_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `mason
...
# MASON_HOME paths already pulled external secrets for: load_mason_dotenv() runs at import time from
...
# Behavioral routing keys a parent Mason process injects into child env that silently redirect a profile
...
def get_secret_source_values(mason_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``mason_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(mason_home).resolve()), {}))


def hydrate_profile_secret_sources(mason_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(mason_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_mason_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `mason setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from mason_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_mason_dotenv(
    *,
    mason_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Mason env files: ``~/.mason/.env`` overrides stale shell exports; project ``.env`` is a dev
    fallback that only fills gaps when the user env exists (and overrides shell vars when it does not)."""
    home_path = Path(mason_home or os.getenv("MASON_HOME", Path.home() / ".mason"))
    ...
    if is_multiplex_active() and get_mason_home_override() is not None:
        ...
        if load_external_secrets:
            from mason_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``mason update`` retry just completed a deferred dependency
    ...
    # ~/.mason/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.mason/.env (e.g. written by an older
    # `mason setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_mason_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_mason_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Mason reads credentials; failures never block startup. Precedence/conflicts/provenance
    ...
    # load_mason_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and mason_logging reuse one parse instead of 3-4.
    if home_path == _process_mason_home():
        try:
...
def _process_mason_home() -> Path:
    """The MASON_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_mason_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``mason_constants.get_process_mason_home()`` is the override-immune
