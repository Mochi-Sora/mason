"""On-demand supply-chain audit for Mason Agent installs.
...
def _discover_plugins(mason_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.mason/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, mason_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = mason_home or Path(get_mason_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `mason security audit`."""
    home = Path(get_mason_home())
    ...
    components = _discover_components(mason_home=home, **skips)
    ...
    try:
        findings = run_audit(mason_home=home, components=components)
