"""Dispatcher and maintenance verbs for ``mason kanban``: ``dispatch``,
...
from mason_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "mason kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    mason gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"successfully. Check profile health (venv, PATH, credentials) and `mason "
                    f"kanban list --status ready` / `mason kanban list --status blocked` for "
