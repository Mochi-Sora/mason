...
from mason_cli.banner import cprint, _DIM, _RST
from mason_cli.config import save_env_value_secure
...
def _secret_result(var_name: str, value: str) -> dict:
    ...
    cprint(f"\n{_DIM}  ✓ Stored secret in {display_mason_home()}/.env as {var_name}{_RST}")
    return {
        **stored,
        "skipped": False,
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    in ~/.mason/.env and never exposed to the model.
    """
