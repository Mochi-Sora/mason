...
from mason_cli._subprocess_compat import harden_git_argv, noninteractive_git_env
...
def _ensure_repo(cwd: str) -> None:
    ...
    if needs_root:
        _git_ok(cwd, ["-c", "user.email=mason@localhost", "-c", "user.name=Mason",
...
def worktree_add(cwd: str, options: dict) -> dict:
    ...
    branch = _sanitize_branch(options.get("branch") or "") or f"mason/{slug}"
