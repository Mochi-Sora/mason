...
Mixin on ``MasonCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from mason_cli.main import _print_version_info
        _print_version_info(check_updates=True)

    def _cmd_reload(self, cmd_original: str):
        from mason_cli.config import reload_env
        count = reload_env()
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `mason plugins list`, so
            ...
            # on one line (full catalog behind `mason plugins list`).
            user_entries = [e for e in entries if e[3] != "bundled"]
            bundled_count = len(entries) - len(user_entries)
            if not user_entries:
                print("No user plugins installed.")
                print("  Install one: mason plugins install owner/repo")
                print(f"  Or drop a plugin directory into {display_mason_home()}/plugins/")
                if bundled_count:
                    print(f"  ({bundled_count} bundled plugins available — see: mason plugins list)")
                return
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: mason plugins list)")
            print("  Enable/disable: mason plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from mason_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from mason_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        if not last_response.strip():
            return
        try:
            from mason_cli.goals import gather_background_processes as _gather_bg
