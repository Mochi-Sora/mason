"""Unified self-relaunch for Mason CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``mason sessions browse`` / post-setup relaunch keep the user's mode."""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched mason."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_mason_bin() -> Optional[str]:
    """Mason entry point: ``sys.argv[0]`` if a real executable, else ``which mason``, else ``None``
    (caller falls back to ``python -m mason_cli.main``)."""
    argv0 = sys.argv[0]
    ...
    if not argv0.startswith("-") and os.path.isfile(argv0):
        ...
        if os.access(abs_path, os.X_OK) and not (_is_windows and _is_python_script(abs_path)):
            return abs_path
    return shutil.which("mason") or None


def build_relaunch_argv(
    extra_args: Sequence[str], *, preserve_inherited: bool = True, original_argv: Optional[Sequence[str]] = None
) -> list[str]:
    """Construct an argv list for replacing the current process with mason."""
    bin_path = resolve_mason_bin()
    argv = [bin_path] if bin_path else [sys.executable, "-m", "mason_cli.main"]
...
def relaunch(
    ...
    """Replace the current process with a fresh mason invocation.
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``mason`` not on
            # PATH yet (install hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nMason relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run mason.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
