"""Migrate Mason MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_MARKER = (
    "# managed by mason-agent — `mason codex-runtime migrate` regenerates this section")
MIGRATION_END_MARKER = "# end mason-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Mason config.")
...
# Mason MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Mason keys.
_KNOWN_MASON_KEYS = {
...
# (mason key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, mason_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Mason MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Mason' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(mason_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if mason_cfg.get("command"):
        if mason_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(mason_cfg["command"])
        if mason_cfg.get("args"):
            out["args"] = [str(a) for a in mason_cfg["args"]]
        if mason_cfg.get("env"):
            out["env"] = _str_map(mason_cfg["env"])
        if mason_cfg.get("cwd"):
            out["cwd"] = str(mason_cfg["cwd"])
    elif mason_cfg.get("url"):
        out["url"] = str(mason_cfg["url"])
        if mason_cfg.get("headers"):
            out["http_headers"] = _str_map(mason_cfg["headers"])
        if mason_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for mason_key, codex_key, note in _TIMEOUT_KEYS:
        if mason_key in mason_cfg:
            try:
                out[codex_key] = float(mason_cfg[mason_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if mason_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in mason_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_MASON_KEYS:
            skipped.append(f"{key} (unknown Mason key)")
    return out, skipped
...
# env-var passthrough (MASON_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Mason)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="mason-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a MASON_HOME pointing there burned into
...
def _build_mason_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Mason' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    MASON_HOME passes through only IF SET, read from os.environ (not get_mason_home()): when
    unset the codex subprocess must inherit its launcher's runtime MASON_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched MASON_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched mason finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # MASON_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_mason_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever MASON_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's MASON_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("MASON_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    mason_home = os.environ.get("MASON_HOME") or ""
    if mason_home and not _looks_like_test_tempdir(mason_home):
        env["MASON_HOME"] = mason_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["MASON_QUIET"] = "1"
    env["MASON_REDACT_SECRETS"] = env.get("MASON_REDACT_SECRETS", "true")
    return {
        "command": sys.executable,
        "args": ["-m", "agent.transports.mason_tools_mcp_server"],
        "env": env,
...
def migrate(
    mason_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_mason_tools: bool = True) -> MigrationReport:
    """Translate Mason mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_mason_tools`` registers Mason' own tool surface
    (agent/transports/mason_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    mason_servers = (mason_config or {}).get("mcp_servers") or {}
    if not isinstance(mason_servers, dict):
        report.errors.append("mcp_servers in Mason config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in mason_servers.items():
        name = str(raw_name)
    ...
    if expose_mason_tools:
        translated["mason-tools"] = _build_mason_tools_mcp_entry()
        if "mason-tools" not in report.migrated:
            report.migrated.append("mason-tools")
