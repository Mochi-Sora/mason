"""PTY bridge for `mason dashboard` chat tab.
...
``mason_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/mason-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "MASON_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Mason Agent supports Windows only via WSL.")
