...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "mason-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from mason_constants import agent_browser_runnable
    ...
    try:
        ...
        # Mason-managed Node dirs ($MASON_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_mason_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("MASON_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from mason_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# Inline Nous Portal login for the Tool Gateway picker (`mason tools`)
