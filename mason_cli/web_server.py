"""Mason Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m mason_cli.main web [--port 8080]``.
"""
...
from mason_cli.install_identity import get_install_id as _shared_get_install_id
from mason_cli.pty_session import run_reaper
...
try:
    ...
    # running `mason dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["MASON_WEB_DIST"]) if "MASON_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from mason_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``mason dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from mason_cli.profiles import profiles_to_serve
            ...
            if len(profile_homes) > 1:
                ...
                from mason_cli.profiles import _check_gateway_running

                start_kwargs["profile_gate"] = lambda _name, home: not _check_gateway_running(Path(home))
                from mason_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `mason update` otherwise 500s
    ...
    # Import mason_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `mason update` replaced the code
    ...
    # Desktop-spawned backends (MASON_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `mason
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("MASON_DESKTOP") == "1":
        ...
        try:
            from mason_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    try:
        ...
        try:
            from mason_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Mason Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from mason_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# MASON_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("MASON_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Mason-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".mason-ssh-runtime-{nonce}")
...
from mason_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from mason_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 mason-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``MASON_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("MASON_DESKTOP") == "1"
        and bool(os.environ.get("MASON_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from mason_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from mason_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from mason_cli.dashboard_auth.token_auth import token_auth_middleware
...
from mason_cli import web_server_gateway as _gateway_mod  # noqa: E402
from mason_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from mason_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from mason_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from mason_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Mason home (not the profile MASON_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``mason gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_mason_action(subcommand, "gateway-restart")
...
from mason_cli.web_routers import (  # noqa: E402
...
from mason_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset MASON_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: run `mason dashboard register` (Nous Portal) or "
    ...
    try:
        from mason_cli.config import load_config as _load_cfg
        from mason_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`mason plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 mason-0day
    ...
    if app.state.auth_required:
        ...
        from mason_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from mason_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from mason_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from mason_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `mason serve` (no MASON_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from mason_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `mason update` relaunch a manually-started serve
    ...
    ready_token = "MASON_BACKEND_READY" if headless else "MASON_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  Mason backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  Mason Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from mason_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``MASON_SERVE_HEADLESS``).
    ...
    from mason_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from mason_cli.nous_auth_keepalive import start_nous_auth_keepalive
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('mason_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('mason_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('mason_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('mason_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('mason_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('mason_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('mason_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('mason_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('mason_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('mason_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('mason_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('mason_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('mason_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('mason_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('mason_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('mason_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('mason_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('mason_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('mason_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('mason_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('mason_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('mason_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('mason_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('mason_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('mason_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('mason_cli.web_models', 'HookCreate'),
    'HookDelete': ('mason_cli.web_models', 'HookDelete'),
    'ImportRequest': ('mason_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('mason_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('mason_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('mason_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('mason_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('mason_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('mason_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('mason_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('mason_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('mason_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('mason_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('mason_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('mason_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('mason_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('mason_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('mason_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('mason_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('mason_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('mason_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('mason_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('mason_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('mason_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('mason_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('mason_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('mason_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('mason_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('mason_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('mason_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('mason_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('mason_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('mason_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('mason_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('mason_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('mason_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('mason_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('mason_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('mason_cli.web_models', 'SessionImport'),
    'SessionPrune': ('mason_cli.web_models', 'SessionPrune'),
    'SessionRename': ('mason_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('mason_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('mason_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('mason_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('mason_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('mason_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('mason_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('mason_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('mason_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('mason_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('mason_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('mason_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('mason_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('mason_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('mason_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('mason_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('mason_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('mason_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('mason_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('mason_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('mason_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('mason_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('mason_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('mason_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('mason_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('mason_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('mason_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('mason_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('mason_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('mason_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('mason_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('mason_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('mason_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('mason_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('mason_cli.config', 'cfg_get'),
    'check_config_version': ('mason_cli.config', 'check_config_version'),
    'check_mason_update': ('mason_cli.web_routers.actions', 'check_mason_update'),
    'clear_model_endpoint_credentials': ('mason_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('mason_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('mason_cli.config', 'coerce_provider_id'),
    'console_ws': ('mason_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('mason_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('mason_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('mason_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('mason_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('mason_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('mason_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('mason_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('mason_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('mason_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('mason_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('mason_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('mason_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('mason_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('mason_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('mason_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('mason_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('mason_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('mason_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('mason_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('mason_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('mason_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('mason_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('mason_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('mason_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('mason_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('mason_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('mason_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('mason_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('mason_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('mason_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('mason_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('mason_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('mason_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('mason_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('mason_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('mason_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('mason_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('mason_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('mason_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('mason_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('mason_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('mason_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('mason_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('mason_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('mason_cli.config', 'get_config_path'),
    'get_config_raw': ('mason_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('mason_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('mason_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('mason_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('mason_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('mason_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('mason_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('mason_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('mason_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('mason_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('mason_cli.config', 'get_env_path'),
    'get_env_vars': ('mason_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('mason_cli.web_routers.status', 'get_health'),
    'get_mason_home': ('mason_cli.config', 'get_mason_home'),
    'get_learning_graph': ('mason_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('mason_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('mason_cli.web_routers.status', 'get_logs'),
    'get_media': ('mason_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('mason_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('mason_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('mason_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('mason_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('mason_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('mason_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('mason_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('mason_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('mason_cli.web_routers.status', 'get_portal_status'),
    'get_process_mason_home': ('mason_cli.config', 'get_process_mason_home'),
    'get_profile_setup_command': ('mason_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('mason_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('mason_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('mason_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('mason_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('mason_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('mason_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('mason_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('mason_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('mason_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('mason_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('mason_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('mason_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('mason_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('mason_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('mason_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('mason_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('mason_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('mason_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('mason_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('mason_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('mason_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('mason_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('mason_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('mason_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('mason_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('mason_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('mason_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('mason_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('mason_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('mason_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('mason_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('mason_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('mason_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('mason_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('mason_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('mason_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('mason_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('mason_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('mason_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('mason_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('mason_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('mason_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('mason_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('mason_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('mason_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('mason_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('mason_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('mason_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('mason_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('mason_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('mason_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('mason_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('mason_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('mason_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('mason_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('mason_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('mason_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('mason_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('mason_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('mason_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('mason_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('mason_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('mason_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('mason_cli.config', 'load_env'),
    'mcp_oauth_callback': ('mason_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('mason_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('mason_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('mason_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('mason_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('mason_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('mason_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('mason_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('mason_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('mason_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('mason_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('mason_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('mason_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('mason_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('mason_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('mason_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('mason_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('mason_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('mason_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('mason_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('mason_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('mason_cli.config', 'remove_env_value'),
    'remove_env_var': ('mason_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('mason_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('mason_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('mason_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('mason_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('mason_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('mason_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('mason_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('mason_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('mason_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('mason_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('mason_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('mason_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('mason_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('mason_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('mason_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('mason_cli.doctor', 'run_doctor'),
    'run_dump': ('mason_cli.dump', 'run_dump'),
    'run_import': ('mason_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('mason_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('mason_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('mason_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('mason_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('mason_cli.config', 'save_config'),
    'save_env_value': ('mason_cli.config', 'save_env_value'),
    'save_toolset_env': ('mason_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('mason_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('mason_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('mason_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('mason_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('mason_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('mason_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('mason_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('mason_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('mason_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('mason_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('mason_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('mason_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('mason_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('mason_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('mason_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('mason_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('mason_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('mason_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('mason_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('mason_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('mason_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('mason_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('mason_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('mason_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('mason_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('mason_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('mason_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('mason_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('mason_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('mason_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('mason_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('mason_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('mason_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('mason_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('mason_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('mason_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('mason_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('mason_cli.web_routers.cron', 'update_cron_job'),
    'update_mason': ('mason_cli.web_routers.actions', 'update_mason'),
    'update_learning_node': ('mason_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('mason_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('mason_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('mason_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('mason_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('mason_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('mason_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('mason_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('mason_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('mason_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('mason_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('mason_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('mason_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('mason_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('mason_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('mason_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('mason_cli.config', 'write_platform_config_field'),
}
