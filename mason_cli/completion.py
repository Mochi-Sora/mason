"""Shell completion script generation for mason CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_mason_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Mason Agent bash completion
# Add to ~/.bashrc:
#   eval "$(mason completion bash)"

_mason_profiles() {{
    local profiles_dir="$HOME/.mason/profiles"
...
_mason_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_mason_profiles)" -- "$cur"))
        return
    fi
...
complete -F _mason_completion mason
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef mason
# Mason Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(mason completion zsh)"

_mason_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.mason/profiles" ]]; then
        profiles+=($HOME/.mason/profiles/*(N/:t))
    fi
...
_mason() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_mason_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'mason command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Mason Agent fish completion",
        "# Add to your config:",
        "#   mason completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __mason_profiles",
        "    echo default",
        "    if test -d $HOME/.mason/profiles",
        "        for d in $HOME/.mason/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c mason -f -s p -l profile"
        " -d 'Profile name' -xa '(__mason_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c mason -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__mason_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
