...
Split out of ``mason_cli/model_switch.py``; every moved name is re-imported there so
``mason_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from mason_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("mason_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from mason_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Mason itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from mason_cli.models import _get_ollama_native_headers, cached_fetch_api_models
    from mason_cli.models_local import (
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    from mason_cli.model_switch import list_authenticated_providers
    ...
    def _warm() -> None:
        try:
            from mason_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from mason_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(mason_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from mason_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from mason_cli.models import _AGGREGATOR_PROVIDERS
    from mason_cli.providers import ALIASES
    for mason_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(mason_id)
        if alias_target and alias_target != mason_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(mason_id)
            if prof is not None and prof.name != mason_id:
                continue
        except Exception:
            pass
        if mason_id.lower() in seen:
            continue
        if mason_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(mason_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(mason_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield mason_id, mdev_id, pconfig, env_vars


def _auth_store_has_provider(*keys: str) -> bool:
    """True when ``auth.json`` has a ``providers`` entry under any of *keys*."""
    try:
        from mason_cli.auth import _load_auth_store
...
def _raw_pool_usable(mason_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from mason_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(mason_id):
            return _credential_pool_is_usable(mason_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, mason_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``mason model`` builds), falling
    back to the curated list (merged with models.dev for preferred providers) when live is empty."""
    from mason_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    ...
    try:
        from mason_cli.models_pricing import get_pricing_for_provider
        from mason_cli.models import (
            check_nous_free_tier,
            union_with_portal_free_recommendations,
            union_with_portal_paid_recommendations,
        )
        from mason_cli.auth import get_provider_auth_state
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from mason_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Mason's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from mason_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from mason_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from mason_cli.auth import PROVIDER_REGISTRY
    from mason_cli.providers import MASON_OVERLAYS
    from mason_cli.models import CANONICAL_PROVIDERS
    ...
    for mason_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(mason_id):
            _emit(mason_id, mason_id)

    mdev_to_mason = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in MASON_OVERLAYS.items():
        mason_slug = mdev_to_mason.get(pid, pid)
        if _skip(seen, excluded_set, pid, mason_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, mason_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, mason_slug) or _pool_usable(mason_slug)):
            _emit(mason_slug, pid, mason_slug)
...
class _PickerBuild:
    ...
    models.dev, 2 Mason-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``mason_cli.auth/models`` stay lazy so tests can patch those modules."""
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from mason_cli.model_switch import _declared_model_ids
    from agent.models_dev import get_provider_info
    for mason_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_any_env(env_vars) or _raw_pool_usable(mason_id)):
            continue
        model_ids = _live_or_curated_ids(mason_id, b.curated)
        ...
        configured = user_providers.get(mason_id) if isinstance(user_providers, dict) else None
        ...
        b.add_builtin_row(
            mason_id, display_name, b.current_provider in (mason_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, mason_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(mason_slug, b.current_provider)
    else:
        has_creds = _overlay_has_env_creds(pid, mason_slug, overlay, os.environ.get)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(mason_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, mason_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(mason_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(mason_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
            logger.debug("Credential pool check failed for %s: %s", mason_slug, exc)
    if not has_creds and mason_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_mason_oauth_credentials
            mason_creds = read_mason_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (mason_creds and mason_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Mason-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from mason_cli.providers import MASON_OVERLAYS

    # MASON_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Mason IDs ("copilot").
    mdev_to_mason = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in MASON_OVERLAYS.items():
        mason_slug = mdev_to_mason.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, mason_slug):
            continue
        if not _overlay_has_creds(b, pid, mason_slug, overlay):
            continue
        if mason_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from mason_cli.models import cached_provider_model_ids
            model_ids = cached_provider_model_ids(mason_slug)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(mason_slug, b.curated, mason_slug, pid)
        elif mason_slug == "nous":
            model_ids = _nous_picker_model_ids(b.curated, b.force_fresh_nous_tier)
        else:
            model_ids = _live_or_curated_ids(mason_slug, b.curated, mason_slug, pid)
        b.add_builtin_row(
            mason_slug, get_label(mason_slug), b.current_provider in (mason_slug, pid), model_ids, "mason")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from mason_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from mason_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from mason_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from mason_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    """Curated model lists keyed by mason provider id, plus the dynamic ones (nous manifest,
    Ollama Cloud, LM Studio live probe)."""
    from mason_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Remote manifest so new Portal models surface without a release; in-repo snapshot fallback.
    curated["nous"] = get_curated_nous_model_ids()
    if "ollama-cloud" not in curated:
        from mason_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from mason_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    ("built-in", "mason", "canonical", "user-config", "model-config").
    ...
    from mason_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (mason_id / mdev_id / canonical slug).
    b = _PickerBuild(
...
def _finalize_picker_rows(results: list, user_providers, current_model: str) -> list:
    ...
    try:
        from mason_cli.config import is_provider_enabled
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from mason_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`mason_cli.models.fetch_openrouter_models` (curated
    ...
    from mason_cli.model_switch import list_authenticated_providers
    from mason_cli.models import fetch_openrouter_models
