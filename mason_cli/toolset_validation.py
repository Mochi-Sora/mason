...
from mason_cli.platforms import PLATFORMS
from mason_cli.toolset_scope import toolset_allowed_for_platform

_NO_TOOLS = "the agent will have no tools on this platform. Run `mason tools` to reconfigure."


def _platform_default_toolset(platform: object) -> str:
    info = PLATFORMS.get(platform)
    return info.default_toolset if info is not None else f"mason-{platform}"
...
def _platform_default_is_valid(
    ...
    # mason-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    Reports: a toolset name ``is_valid_toolset`` rejects (suggesting ``mason-<platform>`` when that
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `mason tools` to configure explicitly.")
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `mason tools` to reconfigure.")
    return warnings
