...
def pairing_command(args):
    """Handle mason pairing subcommands."""
    ...
    if handler is None:
        print("Usage: mason pairing {list|approve|revoke|clear-pending}")
        print("Run 'mason pairing --help' for details.")
    else:
        handler()
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: mason pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  To reset sooner, delete the '_lockout:{platform}' entry from ~/.mason/platforms/pairing/_rate_limits.json\n")
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'mason pairing list' to see pending requests.\n")
