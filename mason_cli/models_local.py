...
disk cache). Split out of ``mason_cli.models``; helpers still defined there are looked up on
``mason_cli.models`` at call time so ``patch("mason_cli.models.<name>")`` mocks keep intercepting.
"""
...
from mason_cli.urllib_security import url_origin

# Log-record parity with the origin module.
logger = logging.getLogger("mason_cli.models")
...
def _get_ollama_base_url() -> str:
    ...
    from mason_cli.models import _get_model_config_dict
...
def probe_ollama_local_models(
    ...
    from mason_cli.models import _MASON_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _MASON_USER_AGENT, **(headers or {})}
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from mason_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    from mason_cli.models import _MASON_USER_AGENT
    token = str(api_key or "").strip()
    return {"User-Agent": _MASON_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
...
def _lmstudio_fetch_raw_models(
    ...
    from mason_cli.models import _urlopen_model_catalog_request
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_mason_home() / "ollama_cloud_models_cache.json"
...
def _load_ollama_cloud_cache(*, ignore_ttl: bool = False) -> Optional[dict]:
    ...
    from mason_cli.models import _read_json_cache
...
def _save_ollama_cloud_cache(models: list[str]) -> None:
    ...
    from mason_cli.models import _write_json_cache
