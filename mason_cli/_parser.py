"""Top-level argparse construction for the mason CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``mason_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``MASON_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/mason_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``mason --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``mason_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    mason                        Start interactive chat
    mason chat -q "Hello"        Single query mode
    mason --tui                  Launch the modern TUI (or set display.interface: tui)
    mason --cli                  Force the classic REPL (overrides display.interface: tui)
    mason -c                     Resume the most recent session
    mason -c "my project"        Resume a session by name (latest in lineage)
    mason --resume <session_id>  Resume a specific session by ID
    mason --resume latest        Resume the most recent session (same as -c)
    mason --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    mason setup                  Run setup wizard
    mason logout                 Clear stored authentication
    mason auth add <provider>    Add a pooled credential
    mason auth list              List pooled credentials
    mason auth remove <p> <t>    Remove pooled credential by index, id, or label
    mason auth reset <provider>  Clear exhaustion status for a provider
    mason model                  Select default model
    mason fallback [list]        Show fallback provider chain
    mason fallback add           Add a fallback provider (same picker as `mason model`)
    mason fallback remove        Remove a fallback provider from the chain
    mason config                 View configuration
    mason config edit            Edit config in $EDITOR
    mason config set model gpt-4 Set a config value
    mason gateway                Run messaging gateway
    mason -s mason-agent-dev,github-auth
    mason -w                     Start in isolated git worktree
    mason gateway install        Install gateway background service
    mason sessions list          List past sessions
    mason sessions browse        Interactive session picker
    mason sessions rename ID T   Rename/title a session
    mason logs                   View agent.log (last 50 lines)
    mason logs -f                Follow agent.log in real time
    mason logs errors            View errors.log
    mason logs --since 1h        Lines from the last hour
    mason debug share             Upload debug report for support
    mason console                Open the safe Mason command console
    mason update                 Update to latest version
    mason dashboard              Start web UI dashboard (port 9119)
    mason dashboard --stop       Stop running dashboard processes
    mason dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via MASON_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `mason setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to MASON_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.mason/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``mason -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/mason_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Mason Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also MASON_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.mason/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Mason itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="mason", description="Mason Agent - AI assistant with tool-calling capabilities",
