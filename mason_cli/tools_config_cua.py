"""cua-driver installer, lock hygiene and pip-install helper for `mason tools` /
`mason computer-use install`."""
...
# Quiet ``mason update`` refreshes stay bounded even when upstream waits on Read-Host / a consent
...
_UPGRADE_CMD = "mason computer-use install --upgrade"
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("MASON_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Mason telemetry policy applied; falls back to the current
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Mason' runtime contract (30s cache keyed on the
...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    # Managed uv first: $MASON_HOME/bin is never on PATH, so a bare which() misses the uv Mason
...
def _confirmed_update_check(driver_cmd: str, require_confirmed_update: bool) -> tuple:
    ...
    None: `mason update` then keeps the installed version — an indeterminate check must never
...
def install_cua_driver(upgrade: bool = False, require_confirmed_update: bool = False,
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if not upgrade:  # silent under `mason update`, which calls this for every user
            _print_warning(
    ...
    override = os.environ.get("MASON_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        return _fail(f"    MASON_CUA_DRIVER_CMD does not resolve to an executable: {override}",
    ...
    # A driver failing Mason' runtime contract (version floor, missing manifest verbs) is repaired
    # regardless of mode. Mason' minimum requirement IS the confirmation an upgrade is needed, so
    ...
    if repair_existing:
        _print_warning(f"    Found cua-driver {contract.get('version') or 'unknown version'}, but "
                       "Mason cannot use its current runtime contract: "
                       f"{contract.get('reason') or 'required runtime features are missing'}.")
        if override:
            return _fail("    Update the binary selected by MASON_CUA_DRIVER_CMD, or unset the "
...
def _print_cua_platform_notes(is_windows: bool, is_linux: bool, *, fresh_install: bool) -> None:
    ...
    if is_windows:
        ...
        if fresh_install:
            _print_info("    Both must allow the terminal / Mason process.")
...
def _installer_popen_kwargs(is_windows: bool, verbose: bool, env: dict) -> dict:
    ...
    holding the install lock and wedge every later run. Non-verbose (`mason update` refresh):
...
def _record_installer_output(out: str, returncode: int) -> None:
    ...
    During `mason update`, sys.stdout is the mirroring _UpdateOutputStream whose `_log` handle is
    ~/.mason/logs/update.log — write straight to it so the full output is kept (success AND
    failure)."""
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True,
    ...
    # Unattended refreshes (installer_timeout set by `mason update`) preflight and may skip.
    # Unattended refreshes (installer_timeout set by `mason update`) fail FAST on the two conditions that
