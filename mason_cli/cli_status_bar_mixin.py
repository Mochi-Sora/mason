...
from mason_cli.banner import _format_context_length
...
class CLIStatusBarMixin:
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            from mason_cli.focus_view import focus_statusbar_segment
    ...
    def _pet_resolve_config(self) -> None:
        """(Re)resolve the active pet from config so ``/pet`` / ``mason pets`` changes apply
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Mason'}"
