...
DEFAULT_API_URL = "https://setup.mason-agent.nousresearch.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Mason Agent"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "MasonSetupBot",
    ...
    print(f"\n  Contacting Mason Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Mason Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "MasonSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``mason_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``mason_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"mason_{generate_username_slug()}_bot"
