"""``mason approvals suggest`` — mine approval history into allowlist proposals.

Mason has no dedicated approval-decision ledger: ``always`` answers land in ``command_allowlist``
...
def default_db_path() -> Path:
    ...
    return get_mason_home() / "state.db"
...
def normalize_command(command: str) -> str:
    """Fold user/mason home prefixes and collapse whitespace."""
    from tools.approval_detection import _rewrite_resolved_mason_home, _rewrite_resolved_user_home
    return " ".join(_rewrite_resolved_user_home(_rewrite_resolved_mason_home(command)).split())
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  mason approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.mason/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``mason approvals suggest``."""
    ...
    if apply_spec:
        ...
        if as_json:
            ...
            for pattern in applied:
                print(f"  + {pattern}")
            print(f"\ncommand_allowlist now has {len(merged)} entries (~/.mason/config.yaml).")
        return 0
...
def approvals_command(args) -> int:
    """Dispatch ``mason approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from mason_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: mason approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `mason approvals <subcommand> -h` for details."
    )
    return 1
