...
logger = logging.getLogger("mason.security_audit")
...
def _running_as_root() -> Optional[str]:
    return None if not _is_root() else (
        ...
        "prompt-injection or exposed endpoint is a full host compromise. Run Mason as an unprivileged user "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("MASON_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(mason_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    if mason_home is None:
        from mason_constants import get_mason_home

        mason_home = get_mason_home()
    if _path_is_mounted(mason_home):  # any error propagates to run_security_audit (= no finding)
        return None
    return (f"Running in a container but the data dir ({mason_home}) is NOT on a persistent volume mount — "
            "sessions, memory, skills, and API keys are ephemeral and lost on container restart. Mount a host "
            "volume over the MASON_HOME data directory.")
...
def run_security_audit(*, mason_home: Optional[Path] = None, config: Optional[dict] = None) -> list[str]:
    ...
    """
    findings: list[str] = []
    for check in (_running_as_root, _ssh_password_auth_enabled, lambda: _container_no_volume_mount(mason_home),
...
def log_startup_security_warnings(
    *, mason_home: Optional[Path] = None, config: Optional[dict] = None, force: bool = False
) -> list[str]:
    ...
    if _AUDIT_RAN and not force:
        return []
    _AUDIT_RAN = True
    findings = run_security_audit(mason_home=mason_home, config=config)
    if findings:
