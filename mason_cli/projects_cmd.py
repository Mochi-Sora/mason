"""``mason project`` CLI — manage first-class, multi-folder Projects."""
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Mason Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``mason project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: mason project <action> [options]\nRun 'mason project --help' for the full list.", file=sys.stderr)
        return 0
...
def _cmd_list(args, conn):
    ...
    if not projs:
        return "No projects yet. Create one with `mason project create <name>`."
    for p in projs:
...
def _cmd_set_primary(args, conn, proj):
    if not pdb.set_primary(conn, proj.id, args.path):
        return _err(f"'{args.path}' is not a folder of {proj.slug}; add it first with `mason project add-folder`.")
