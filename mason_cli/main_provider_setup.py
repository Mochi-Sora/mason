...
from mason_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.mason/.env unless the active provider is 'custom' — a
    leftover value routes provider:auto auxiliary clients to the old custom endpoint."""
    from mason_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`mason_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Mason only falls back to a lightweight backend (OpenRouter,",
...
def _aux_select_for_task(task: str) -> None:
    ...
    from mason_cli.config import load_config
    from mason_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    with contextlib.suppress(Exception):
        from mason_cli.setup import _curses_prompt_choice
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Mason URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    from mason_cli.runtime_provider import _detect_api_mode_for_url
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "", existing_source: str = "") -> tuple:
    """API-key entry for ``mason setup`` / ``mason model``: first-time entry, or [K]eep / [R]eplace /
    ...
    from mason_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `mason setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from mason_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from mason_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            from mason_constants import display_mason_home as _dhh_fn
            print(f"    Mason will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from mason_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``mason model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from mason_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from mason_cli.models_catalog_static import group_providers, provider_group_for_slug
