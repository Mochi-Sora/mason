...
Imported by ``mason_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
mason_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(mason_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MASON_HOME?"""
    active = (_read_text(os.path.join(mason_root, "active_profile")) or "").strip()
...
def _resolved_home() -> str:
    return os.environ.get("MASON_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("MASON_DEV") == "1" or is_container_startup_environment():
        return False
    mason_home = os.environ.get("MASON_HOME", "").strip()
    if mason_home:
        if os.path.exists(os.path.join(mason_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(mason_home)))
        return parent_name != "profiles" and active_profile_may_override_home(mason_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``mason --version`` output (also used by /version).
    ...
    try:
        ...
        from mason_cli import __release_date__, __version__

        print(f"Mason Agent v{__version__} ({__release_date__})")
    ...
    try:
        from mason_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``mason --version`` before the heavy import wall.
    ...
    MASON_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("MASON_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
