"""Billing and subscription handlers for the interactive CLI (mixed into ``MasonCLI``).
...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Mason Agent page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Mason Agent page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Mason Agent page before adding funds."
...
class CLIBillingMixin:
    ...
    def _print_logged_out(self, state, load_failed, cmd) -> None:
        ...
        if state.error:
            ...
            print(f"  Run `mason portal` to log in, then {cmd}.")
    ...
    def _step_up_remote_spending(self, *, explain, noninteractive_msg, declined_msg, not_granted_msg) -> bool:
        ...
        try:
            from mason_cli.auth import step_up_nous_billing_scope
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from mason_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from cli import _cprint
        from mason_cli.nous_billing import (
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `mason portal` and allow Remote Spending, then re-run /subscription.",
        ...
        try:
            from mason_cli import nous_billing as _nb
    ...
    def _billing_submit_and_poll(self, state, amount, key, *, missing_msg, status_msg, on_scope=None):
        ...
        from cli import _cprint, _d
        from mason_cli.nous_billing import BillingError, BillingScopeRequired, post_charge
        try:
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        from mason_cli.nous_billing import BillingError, BillingTransient, get_charge_status
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from mason_cli.nous_billing import BillingTransient, BillingRemoteSpendingRevoked, BillingSessionRevoked
        code = exc.error
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `mason portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `mason portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `mason portal` and allow Remote Spending, then retry.",
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Mason Agent page, then run /topup again.")
    ...
    def _billing_patch_auto_top_up(self, state, **kwargs) -> bool:
        ...
        from mason_cli.nous_billing import BillingError, BillingScopeRequired, patch_auto_top_up
        try:
