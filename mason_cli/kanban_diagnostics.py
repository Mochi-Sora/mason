...
def _log_hint_action(task_id: str) -> DiagnosticAction:
    cmd = f"mason kanban log {task_id}"
...
_TRIAGE_SLOTS = {
    ...
    False: (
        ...
        "`mason kanban specify`, which uses auxiliary.triage_specifier.",
    ),
}
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [_cli_hint(
        f"Configure {primary_slot}", f"mason config set {primary_slot}.provider auto", suggested=True,
    )]
    if not fallback_explicit and not main_visible:
        actions.append(_cli_hint(
            f"Or configure fallback {fallback_slot}", f"mason config set {fallback_slot}.provider auto",
        ))
    if not auto_decompose:
        cmd = f"mason kanban specify {task_id}"
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        doctor, auth = f"mason -p {assignee} doctor", f"mason -p {assignee} auth"
...
def _rule_review_dependency_deadlock(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(_cli_hint(
            "Complete the finished implementation phase", f"mason kanban complete {task_id}",
            suggested=True,
        ))
    if task_id and child_ids:
        actions.append(_cli_hint(
            "Or unlink the incorrectly gated reviewer", f"mason kanban unlink {task_id} {child_ids[0]}",
        ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions: list[DiagnosticAction] = []
    if task_id:
        cmd = f"mason kanban events {task_id}"
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [
        ...
        _cli_hint("Check dispatcher status", "mason kanban diagnostics"),
    ]
    return [Diagnostic(
