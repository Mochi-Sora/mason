"""Offline, non-destructive recovery for a damaged Mason session database.
...
from mason_state_common import FTS_STORAGE_VERSION, SCHEMA_VERSION
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    guard keeps this path consistent with ``mason_state_repair._backup_db_file``.
    ...
    ``mason_cli.sqlite_safe_read`` exists to prevent (see #71724). Holding the lock means no connection can
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="mason-session-recovery-", dir=str(work_root))
    try:
        snapshot_source, copied = _copy_source_bundle(source, Path(temp_dir.name))
        if _source_fingerprint(source) != before:
            raise SessionRecoverySafetyError(
                "The source database bundle changed while it was being copied. Stop every Mason process using this "
                "profile and retry. This includes the interactive `mason` CLI session this command may have been "
                "launched from: a running parent CLI writes session bookkeeping (compression ticks, context "
                "tracking) to state.db in the background and counts as a Mason process even after the gateway is "
                "stopped. Run the recovery from a fresh shell with no `mason` session open, or point --source at an "
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from mason_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
