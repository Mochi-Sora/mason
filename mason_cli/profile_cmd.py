"""``mason profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``mason_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from mason_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.mason_requires:
        print(f"  Requires: Mason {mf.mason_requires}")
...
def _profile_status(args):
    """Bare ``mason profile`` — show current profile status."""
    from mason_constants import display_mason_home
    from mason_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_mason_home()
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → mason -p {p.name}")
    print()
...
def _profile_use(args):
    from mason_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.mason)" if name == "default" else f"Switched to: {name}")
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  mason profile alias {name} --name <custom>")
            print(f"  Or access via flag:     mason -p {name} chat")
        else:
...
def _profile_delete(args):
    from mason_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_mason_home),
    everything else to its named directory."""
    from mason_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from mason_constants import get_mason_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from mason_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `mason profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → mason -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from mason_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from mason_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from mason_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="mason_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  mason -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      mason -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from mason_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from mason_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `mason profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  mason -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
def _profile_info(args):
    from mason_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``mason profile``.
PROFILE_ACTIONS = {
