"""MCP picker — interactive `mason mcp picker` (also the default `mason mcp`)."""
...
from mason_cli.colors import Colors
from mason_cli.cli_output import prompt_yes_no
from mason_cli.curses_ui import curses_single_select
from mason_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Mason session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from mason_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `mason mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: mason mcp install <name>    Picker: mason mcp", Colors.DIM)
    # Manifest-version warnings: the user's Mason is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Mason — run `mason update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`mason mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`mason mcp picker` (and default `mason mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`mason mcp install <name>` — non-interactive entry-point."""
    from mason_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `mason mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
