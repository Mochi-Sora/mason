"""Profile management for multiple isolated Mason instances."""
...
from mason_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from mason_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See mason_constants.get_subprocess_home().
...
# (``~/.mason``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "mason-agent", ".worktrees", "profiles", "bin", "node_modules",
})

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `mason backup` archives, quick-backup
...
# Marker written by `mason profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `mason update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this Mason profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_mason_home().resolve():
...
# Allow-list for ``export_profile("default")``: when MASON_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Mason profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``mason_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"mason", "default", "test", "tmp", "root", "sudo"})

# Mason subcommands that cannot be used as profile names/aliases
_MASON_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the mason root (NOT the current MASON_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_mason_home() / "profiles"


def _get_default_mason_home() -> Path:
    """Default (pre-profile) MASON_HOME: ``~/.mason``, or MASON_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from mason_constants import get_default_mason_root
    return get_default_mason_root()


def _get_active_profile_path() -> Path:
    return _get_default_mason_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Mason-generated wrapper (contains ``mason -p``)."""
    try:
        return "mason -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: mason profile create {canon}")
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MASON_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_mason_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _MASON_SUBCOMMANDS:
        return f"'{canon}' conflicts with a mason subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            mason_exe = shutil.which("mason") or "mason"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(mason_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `mason doctor --fix` surfaces
    # the detailed error in the target profile.
    with contextlib.suppress(Exception):
        from mason_constants import reset_mason_home_override, set_mason_home_override
        from mason_cli.config import check_config_version, migrate_config
        token = set_mason_home_override(str(profile_dir))
        try:
...
# wrappers are a few hundred bytes with the ``mason -p X`` needle near the top; the wrapper
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `mason model` sets it later
        import yaml
        from mason_constants import get_mason_home
        from mason_cli.config import read_user_config_raw
        source = get_mason_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating MASON_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from mason_cli.gateway import named_profile_served_by_running_multiplexer
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing Mason config, ~5000
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_mason_home()
...
def profiles_to_serve(multiplex: bool, profile_allowlist: Optional[List[str]] = None) -> List[Tuple[str, Path]]:
    """``(profile_name, mason_home)`` pairs a gateway should serve — the single chokepoint
    ...
    *profile_allowlist* (invalid entries skipped, missing ones warned once)."""
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_mason_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from mason_cli.auth import strip_cloned_single_use_oauth_grants
...
def create_profile(
    ...
    ``no_skills`` creates an empty profile and writes a marker so ``mason update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.mason).")
    profile_dir = get_profile_dir(canon)
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `mason -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from mason_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `mason update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`mason profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `mason profile describe` works later
    ...
    # `mason -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches MASON_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MASON_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_mason_home() / ".env"
    for entry in _iter_named_profile_dirs():
        env_path = entry / ".env"
        if env_path.exists():
            continue
...
_MASON_ARGV_MARKERS = ("mason_cli.main", "mason-gateway", "tui_gateway")
...
# `findOnPath('mason')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "mason".
...
# argv[1] is matched against exact names, not ``startswith("mason")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("mason-notes.py").
_MASON_CONSOLE_SCRIPT_NAMES = frozenset({"mason", "mason-agent", "mason-acp"})


def _is_mason_argv(argv: list) -> bool:
    """True for a Mason process: entrypoint marker in argv, executable named ``mason*``,
    or a python interpreter directly exec'ing a known ``mason`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _MASON_ARGV_MARKERS) or exe_name.startswith("mason"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _MASON_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Mason *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`mason -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_mason_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by MASON_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("MASON_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.mason).\nTo remove everything, use: mason uninstall")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``MASON_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # MASON_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("MASON_HOME")
    try:
        os.environ["MASON_HOME"] = str(profile_dir)
        from mason_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("MASON_HOME", None)
        if old_home is not None:
            os.environ["MASON_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's mason_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded MASON_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from MASON_HOME: ``"default"`` when unset or ``~/.mason``, the
    name under ``~/.mason/profiles/<name>``, ``"custom"`` for any other path."""
    from mason_constants import get_mason_home
    resolved = get_mason_home().resolve()
    if resolved == _get_default_mason_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when MASON_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_mason_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point MASON_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/mason-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".mason-profile-exports", Path(tempfile.gettempdir()) / f"mason-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    MASON_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Mason file the user may have lying alongside MASON_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``mason
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.mason (dir name ".mason"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: mason profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.mason). "
            "Specify a different name: mason profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise FileExistsError(f"Profile '{canon}' already exists at {profile_dir}")
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="mason_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"mason_{old_name}"
    legacy_old_host = f"mason.{old_name}"
    new_host = f"mason_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_mason_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``mason_<old>`` or legacy ``mason.<old>``
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MASON_HOME path string. Called early in the CLI entry
    point, before mason modules are imported, to set MASON_HOME.

    When MASON_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Mason-owned PYTHONPATH
