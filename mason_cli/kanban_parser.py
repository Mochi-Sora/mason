"""Argparse tree for ``mason kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $MASON_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `mason kanban boards export`. The board always lands as a "
...
# Top-level ``mason kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `mason project list`."),
        _TENANT,
        _PRIORITY,
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $MASON_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $MASON_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $MASON_PROFILE or 'user')"),
    ...
    ], help="DEPRECATED — dispatcher now runs in the gateway. Use `mason gateway start`."),
    _cmd("watch", [
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.mason/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Mason profiles. "
                    "Tasks are claimed atomically, can depend on other tasks, and "
                    "are executed by a named profile in an isolated workspace. "
                    "See https://mason-agent.nousresearch.com/docs/user-guide/features/kanban "
                    "or docs/mason-kanban-v1-spec.pdf for the full design.",
    )
    # --board scopes every subcommand to one board's DB; when omitted the
    # resolution is MASON_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `mason kanban boards switch <slug>` or the "
                                    "MASON_KANBAN_BOARD env var). Use `mason kanban boards "
