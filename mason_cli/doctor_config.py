"""Configuration-file checks for mason doctor: .env, config.yaml validation, drift, deprecations.
Split out of ``mason_cli/doctor.py``."""
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.mason/.env contains provider auth/base URL settings."""
    from mason_cli.doctor import _PROVIDER_ENV_HINTS
...
# TERMINAL_CWD do not false-positive). MASON_TOOL_PROGRESS is silently ignored since the v12 config floor
# removed its only consumer; MASON_TOOL_PROGRESS_MODE is still read by the gateway as a back-compat fallback.
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ("MASON_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("MASON_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    """Return actionable findings for the removed Mason Relay plugin."""
    from mason_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
...
def managed_scope_check() -> None:
    """Report the active managed scope (resolved dir + pinned key counts); silent when none. A MASON_MANAGED_DIR
    ...
    if os.environ.get("MASON_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via MASON_MANAGED_DIR={managed_dir}")
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    """Managed scope plus ~/.mason/.env presence and provider credentials."""
    from mason_cli.doctor import MASON_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
    env_path = MASON_HOME / '.env'
    if env_path.exists():
        ...
        if not check_bool(_has_provider_env_config(content), "API key or custom endpoint configured", f"No API key found in {_DHH}/.env"):
            f.issues.append("Run 'mason setup' to configure API keys")
        ...
        if should_fix:
            ...
            check_info("Run 'mason setup' to configure API keys")
            f.fixed += 1
        else:
            check_info("Run 'mason setup' to create one")
            f.issues.append("Run 'mason setup' to create .env")
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from mason_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
        known = set(PROVIDER_REGISTRY.keys()) | {"openrouter", "custom", "auto", "moa"}
    with warn_on_error(""):
        from mason_cli.config import get_compatible_custom_providers
        from mason_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if provider and provider != "auto" and (catalog_provider is None or (known_providers and not (accept & valid_provider_ids))):
        ...
        _fail_and_issue(f"model.provider '{provider_raw}' is not a recognised provider", f"(known: {known_list})",
                        ...
                        f"Fix: run 'mason config set model.provider <valid_provider>'", issues)
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from mason_cli.doctor import _DHH
        with warn_on_error(""):
            if not _provider_has_credentials(runtime_provider):
                _fail_and_issue(f"model.provider '{runtime_provider}' is set but no API key is configured",
                                "(check ~/.mason/.env or run 'mason setup')",
                                f"No credentials found for provider '{runtime_provider}'. Run 'mason setup' or set the provider's "
                                f"API key in {_DHH}/.env, or switch providers with 'mason config set model.provider <name>'", issues)
...
def _check_config_file(should_fix: bool, f: Finding) -> None:
    ...
    from mason_cli.doctor import MASON_HOME, PROJECT_ROOT, _DHH
    config_path = MASON_HOME / 'config.yaml'
    if config_path.exists():
        ...
        if example_config.exists():
            ...
            from mason_cli.config import DEFAULT_CONFIG, save_config
...
def _drift_config_version(f: Finding, should_fix: bool, config_path) -> None:
    ...
    if check_bool(current_ver >= latest_ver, f"Config version up to date (v{current_ver})", outdated):
        return
    if not should_fix:
        f.issues.append("Run 'mason doctor --fix' or 'mason setup' to migrate config")
        return
    try:
        ...
        f.issues.append("Run 'mason setup' to migrate config")
...
def _drift_stale_root_keys(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from mason_cli.config import atomic_config_write, read_user_config_raw
    ...
    if not should_fix:
        f.issues.append("Stale root-level provider/base_url in config.yaml — run 'mason doctor --fix'")
        return
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    """A stale MASON_MAX_ITERATIONS in .env shadows agent.max_turns in config.yaml.

    The setup wizard used to dual-write the budget. The gateway bridge derives MASON_MAX_ITERATIONS from
    ...
    """
    from mason_cli.doctor import _DHH
    # Detect stale MASON_MAX_ITERATIONS ghost in .env shadowing agent.max_turns in config.yaml (issue
    # #17534). The setup wizard used to dual-write the iteration budget to both stores; users who later edit
    # only config.yaml are left with a .env ghost. The gateway bridge normally derives MASON_MAX_ITERATIONS
    ...
    from mason_cli.config import load_env, read_user_config_raw, remove_env_value
    raw_config = read_user_config_raw(config_path)
    ...
    if cfg_max_turns is None or env_ghost is None or str(cfg_max_turns).strip() == str(env_ghost).strip():
        return
    check_warn(f"MASON_MAX_ITERATIONS={env_ghost} in .env shadows agent.max_turns={cfg_max_turns} in config.yaml",
               "(stale ghost from an earlier `mason setup` run)")
    if not should_fix:
        f.issues.append("Stale MASON_MAX_ITERATIONS in .env shadows config.yaml — run 'mason doctor --fix'")
    elif remove_env_value("MASON_MAX_ITERATIONS"):
        check_ok(f"Removed stale MASON_MAX_ITERATIONS from .env (config.yaml agent.max_turns={cfg_max_turns} is now authoritative)")
        f.fixed += 1
    else:
        check_warn("Could not remove MASON_MAX_ITERATIONS from .env")
        f.manual_issues.append(f"Manually delete the MASON_MAX_ITERATIONS line from {_DHH}/.env — config.yaml agent.max_turns is authoritative.")
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from mason_cli.config import load_env, read_user_config_raw
...
def _check_config_drift(should_fix: bool, f: Finding) -> None:
    """Config version, stale root keys, MASON_MAX_ITERATIONS ghost, deprecations, structure.
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from mason_cli.config import load_config
    from mason_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from mason_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
    ...
    check_info("Details: mason plugins compat")
    f.manual_issues.append(
        f"Update {len(report)} plugin(s) still importing pre-decomposition paths (mason plugins compat) — "
