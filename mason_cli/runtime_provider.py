...
Custom-provider lookup lives in :mod:`mason_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`mason_cli.runtime_provider_backends` — both re-exported here so
``mason_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from mason_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from mason_cli import config as _config_mod
from mason_cli import models as _models  # attribute access keeps ``mason_cli.models.<name>`` patches effective
from mason_constants import OPENROUTER_BASE_URL
from mason_cli.providers import determine_api_mode, is_official_openai_host, nous_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``mason_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in mason_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider in ("xai", "actual"):
        ...
        # the host_mandated_api_mode clause in mason_cli/providers.py so the runtime resolver stays in
...
def _nous_min_key_ttl() -> int:
    return max(60, env_int("MASON_NOUS_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nous_creds() -> Dict[str, Any]:
    return resolve_nous_runtime_credentials(timeout_seconds=float(_getenv("MASON_NOUS_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``MASON_INFERENCE_PROVIDER``, else
    ...
    return _getenv("MASON_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from mason_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from mason_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _refresh_nous_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    ...
    selection (avoids network calls in `mason auth list`); refresh here before falling back to
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    ...
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "mason-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "mason-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.mason/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `mason setup` to install Vertex support.")
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('mason_cli.providers', 'custom_provider_aliases'),
