...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or mason -c.",
    ...
    "Ctrl+Z suspends Mason to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "mason -c resumes your most recent CLI session. mason -c \"project name\" resumes by title.",
    "mason -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "mason -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "mason chat -t web,terminal enables only specific toolsets for a focused session.",
    "mason chat -s github-pr-workflow preloads a skill at launch.",
    "mason chat -q \"query\" runs a single non-interactive query and exits.",
    "mason chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "mason chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "mason --yolo bypasses all dangerous command approval prompts for the entire session.",
    "mason chat --source telegram tags the session for filtering in mason sessions list.",
    "mason -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "mason doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "mason dump outputs a compact setup summary — great for bug reports.",
    "mason config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "mason config edit opens config.yaml in your default editor.",
    "mason config check scans for missing or stale configuration options.",
    "mason sessions browse opens an interactive session picker with search.",
    "mason sessions stats shows session counts by platform and database size.",
    "mason sessions prune --older-than 30 cleans up old sessions.",
    "mason skills search react --source skills-sh searches the skills.sh public directory.",
    "mason skills check scans installed hub skills for upstream updates.",
    "mason skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "mason skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "mason mcp add github --command npx adds MCP servers from the command line.",
    "mason mcp serve runs Mason itself as an MCP server for other agents.",
    "mason auth add lets you add multiple API keys for credential pool rotation.",
    "mason completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "mason logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "mason backup creates a zip backup of your entire Mason home directory.",
    "mason profile create coder creates an isolated profile that becomes its own command.",
    "mason profile create work --clone copies your current config and keys to a new profile.",
    "mason update syncs new bundled skills to ALL profiles automatically.",
    "mason gateway install sets up Mason as a system service (systemd/launchd).",
    "mason memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "mason webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: mason tools disables unused tools, mason skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "mason models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as mason -w).",
    ...
    "MASON_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Mason tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'mason profile create coder' creates the 'coder' command.",
    "mason profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Mason prints a resume command with session ID and stats.",
    "mason sessions export backup.jsonl exports all sessions for backup or analysis.",
    "mason -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "mason skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .mason/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: mason cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Mason runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "mason gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Mason via a local macOS server.",
    ...
    "Context files (.mason.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Mason loads project context from .mason.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "mason mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "mason mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "mason mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — mason tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.mason/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "mason chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "mason plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "mason acp runs Mason as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MASON_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "mason auth add nous or mason auth add openai-codex sets up OAuth-based providers.",
    ...
    "mason status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.mason/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Mason your own.",
    ...
    "Before a gateway session reset, Mason auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.mason/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Mason state via MASON_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "MASON_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MASON_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.mason/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'mason profile create ls' would be rejected.",
    "mason profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "mason claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MASON_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via mason tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.mason/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.mason/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Mason auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Mason config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Mason is working.',
    ...
    '/reload reloads ~/.mason/.env into the running session — pick up new API keys without restarting.',
    ...
    "MASON_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.mason/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.mason/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'mason curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "mason curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'mason curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'mason auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'MASON_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MASON_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'mason dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'mason dashboard embeds the full Mason TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.mason/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.mason/dashboard-plugins/ — no npm build required.',
    ...
    'MASON_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'MASON_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'MASON_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MASON_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.mason/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.mason/skills/ so drop-in skills appear without restarting the session.',
    ...
    'mason -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'mason chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'mason chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'mason chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "mason chat --source tool tags programmatic chats so they don't clutter mason sessions list.",
    'mason dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'mason sessions rename <ID> "new title" renames any past session; mason sessions delete <ID> removes one.',
    'mason import restores a full Mason backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'mason fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'mason pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'mason setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'mason status --deep runs the full health sweep across every component; plain mason status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'MASON_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MASON_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.mason/plugins/ — trust-gated by design.',
    "MASON_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MASON_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MASON_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MASON_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MASON_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MASON_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MASON_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'MASON_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MASON_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MASON_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MASON_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Mason block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from mason sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.mason/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Mason is
