"""Cross-process mutual exclusion for in-flight Mason updates.
...
MARKER_NAME = ".mason-update-in-progress"

# Set by an orchestrating updater (Tauri `mason-setup --update`) to its own pid before
# spawning `mason update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "MASON_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Mason is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Mason home (never the context-local profile override): the Rust
    updater resolves ``$MASON_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_mason_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``mason update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Mason update is already running (PID {holder.pid}, "
