"""``mason dashboard register`` — register a self-hosted dashboard OAuth client.
...
write ``MASON_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
    ...
    try:
        ...
        if exc.code == 401:
            message = ("Nous Portal rejected the access token (401). "
                       "Try `mason auth add nous` to re-authenticate.")
        elif exc.code == 403:
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    from mason_cli.config import get_env_path
    print(f"\n  Wrote to {get_env_path()}:\n    MASON_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print("    MASON_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    MASON_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print(
        "\n  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `mason dashboard` (localhost) leaves the gate off and serves locally\n"
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to MASON_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    from mason_cli.auth import AuthError, resolve_nous_access_token
    from mason_cli.config import is_managed, save_env_value
    # Managed installs get the client id stamped in by the orchestrator (save_env_value refuses).
    if is_managed():
        print("✗ `mason dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nous Portal.\n"
                  "  Run `mason setup` (or `mason auth add nous`) first, then retry.")
        else:
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("MASON_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("MASON_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        save_env_value("MASON_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write MASON_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  MASON_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("MASON_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("MASON_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("MASON_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("MASON_DASHBOARD_PUBLIC_URL", public_url))
