...
resolved through the module object so test patches on ``mason_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from mason_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'mason setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'mason setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'mason tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("mason setup", "          Re-run the full wizard"), ("mason setup model", "    Change model/provider"),
    ("mason setup terminal", " Change terminal backend"), ("mason setup gateway", "  Configure messaging"),
    ("mason setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("mason config", "         View current settings"), ("mason config edit", "    Open config in your editor"),
    ("mason config set <key> <value>", ""))
_READY_ROWS = (
    ("mason", "              Start chatting"), ("mason gateway", "      Start messaging gateway"),
    ("mason doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'mason setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `mason tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'mason setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via mason auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, mason_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Mason cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    mason model            (pick any provider/model)",
              "    mason setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'mason setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {mason_home}/cron/, sessions/, logs/")):
...
import mason_cli.setup as _setup  # noqa: E402  (bottom: mason_cli.setup imports this module)
