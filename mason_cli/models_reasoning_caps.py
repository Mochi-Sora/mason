...
Split out of ``mason_cli.models``. OpenRouter and
...
``mason_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `mason -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_mason_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._MASON_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``mason_cli.models`` plus how to name its URL.
