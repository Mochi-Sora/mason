...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Mason auth store (~/.mason/auth.json)."""
    from mason_cli.auth import _save_active_provider_state
...
def _minimax_oauth_login(*, region: str = "global", open_browser: bool = True, timeout_seconds: float = 15.0) -> Dict[str, Any]:
    ...
    from mason_cli.auth import PROVIDER_REGISTRY, _can_open_graphical_browser, _is_remote_session, _minimax_pkce_pair, _minimax_request_user_code, _minimax_save_auth_state, _print_device_code_instructions
    ...
    print(f"Starting Mason login via MiniMax ({region}) OAuth...")
...
def _refresh_minimax_oauth_state(state: Dict[str, Any], *, timeout_seconds: float = 15.0, force: bool = False) -> Dict[str, Any]:
    ...
    from mason_cli.auth import _minimax_save_auth_state
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    from mason_cli.auth import _minimax_save_auth_state, _quarantine_flat_oauth_state
...
def _minimax_fresh_state() -> Dict[str, Any]:
    ...
    from mason_cli.auth import _refresh_minimax_oauth_state, get_provider_auth_state
    state = get_provider_auth_state("minimax-oauth")
    if not state or not state.get("access_token"):
        raise _minimax_err(
            "Not logged into MiniMax OAuth. Run `mason model` and select MiniMax (OAuth).", "not_logged_in", relogin=True,
        )
    try:
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    """Return {provider, api_key, base_url, source}; string ``api_key`` by default (``mason status`` contract)."""
...
def _login_minimax_oauth(args, pconfig: ProviderConfig) -> None:
    ...
    from mason_cli.auth import format_auth_error
    try:
