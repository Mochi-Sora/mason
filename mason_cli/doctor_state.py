"""MASON_HOME state checks for mason doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from mason_cli.sizefmt import format_bytes as _human_bytes
from mason_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(mason_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (mason_home if mason_home is not None else MASON_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                      ...
                      "(stop the listed processes, then run 'mason sessions optimize-storage' with the gateway stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'mason sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(mason_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(mason_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """MASON_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from mason_cli.doctor import MASON_HOME, _DHH
    mason_home = MASON_HOME
    ensure_dir(f, should_fix, mason_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(mason_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, mason_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    # SOUL.md persona file
    soul_path = mason_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Mason a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text("# Mason Agent Persona\n\n<!-- Edit this file to customize how Mason communicates. -->\n\n"
                                 "You are Mason, a helpful AI assistant.\n", encoding="utf-8")
    ...
    memories_dir = mason_home / "memories"
    if not memory_on:
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'mason doctor --fix' (or 'mason sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'mason doctor --fix' (or 'mason sessions repair') to recover hidden sessions"),
}
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    from mason_state_repair import repair_state_db_schema
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from mason_state_repair import _db_opens_cleanly
        ...
        from mason_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from mason_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'mason sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run 'mason doctor --fix' to checkpoint")
            import sqlite3
            conn = sqlite3.connect(str(state_db_path))
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from mason_cli.doctor import MASON_HOME, _DHH
    state_db_path = MASON_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from mason_cli.doctor import MASON_HOME, _DHH
    hub_dir = MASON_HOME / "skills" / ".hub"
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: mason skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: mason memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: mason memory setup",
                        "No Honcho API key — run 'mason memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run mason memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: mason memory status")
    else:
        check_warn(f"{name} plugin not found", "run: mason memory setup")


@doctor_check()
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from mason_cli.doctor import MASON_HOME
    name = _doctor_memory_config(MASON_HOME).get("provider", "")
    if not name:
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from mason_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"mason -p (\S+)", wrapper.read_text(encoding="utf-8"))
