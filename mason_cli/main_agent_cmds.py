...
def _cmd_memory_reset(args):
    ...
    if not existing:
        print(f"\n  Nothing to reset — no memory files found in {display_mason_home()}/memories/\n")
        return
    ...
    print(f"  Files were in: {display_mason_home()}/memories/\n")
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from mason_cli.memory_setup import memory_command
        memory_command(args)
...
def cmd_acp(args):
    """Launch Mason Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from mason_cli.tools_config import run_post_setup_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: mason-agent[otlp])")
...
def cmd_skills(args):
    ...
    if action == "config":
        ...
        from mason_cli.skills_config import skills_command as skills_config_command
        skills_config_command(args)
    elif action in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from mason_cli.skills_hub import skills_command
        skills_command(args)


def _cmd_skills_trust(args):
    """``mason skills trust|untrust [path]`` — manage ``skills.trusted_project_dirs``.
