...
tests/mason_cli/test_commands_execute.py).
"""
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    home_display = str(ctx.options.get("home_display") or "").strip() or display_mason_home()
    # Presentation-only display name (profile.yaml); `data.profile` stays the canonical id.
    label = profile_name
    try:
        from mason_cli.profiles import format_profile_label, get_profile_dir, read_profile_meta
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: mason bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from mason_cli.commands import gateway_help_lines
