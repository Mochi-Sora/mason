...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``mason-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``mason-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``mason_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "mason_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["MASON_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_MASON_GATEWAY", "MASON_GATEWAY", "MASON_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``mason-serve.service`` can exist in both the user and the system
    ...
    """
    from mason_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    """Name the serve/dashboard processes still on pre-update code: ``mason serve`` hosts
    ...
    print(
        "    Restart them before using Mason again, e.g. `systemctl --user restart mason-serve.service`"
        " or by relaunching `mason serve` / the Desktop app.")
