"""MCP Server Management CLI — ``mason mcp`` subcommand."""
...
from mason_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_mason_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.mason/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return MasonTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see mason_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  mason mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  mason mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  mason mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: mason mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_mason_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `mason mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  mason mcp add <name> --url <endpoint>')
        _info('  mason mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `mason mcp remove` + `mason mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `mason mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `mason mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: mason mcp reauth <name>   |   mason mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'mason mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "mason mcp                                    Open the catalog picker (default)",
    "mason mcp catalog                            List Nous-approved MCPs",
    "mason mcp install <name>                     Install a catalog MCP",
    "mason mcp serve                              Run as MCP server",
    "mason mcp add <name> --url <endpoint>        Add a custom MCP server",
    "mason mcp add <name> --command <cmd>         Add a stdio server",
    "mason mcp add <name> --preset <preset>       Add from a known preset",
    "mason mcp remove <name>                      Remove a server",
    "mason mcp list                               List configured servers",
    "mason mcp test <name>                        Test connection",
    "mason mcp configure <name>                   Toggle tools",
    "mason mcp login <name>                       Re-authenticate OAuth",
    "mason mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``mason mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from mason_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `mason plugin`).
    from mason_cli.mcp_picker import run_picker
    run_picker()
