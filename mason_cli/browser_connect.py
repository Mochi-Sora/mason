"""Shared helpers for attaching Mason to a local Chromium-family CDP port.
...
# the real profile into ``~/.mason/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the mason-owned snapshot dir for ``browser``'s real profile."""
    return str(get_mason_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, MASON_UID/GID) as every Mason secret dir — via ``_secure_dir``/
    ...
    ``MASON_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from mason_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
_SNAPSHOT_DONE_MARKER = ".mason-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Mason can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Mason "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the mason copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_mason_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_mason_home() / "chrome-debug")
