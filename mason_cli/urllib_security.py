...
_CA_BUNDLE_ENV_VARS = ("MASON_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
...
def _resolved_https_context() -> ssl.SSLContext | None:
    """Return the explicit CA context for Mason-owned urllib openers."""
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    (``ssl_ca_cert``/``ssl_verify``) apply; with None a Mason-owned opener gets the explicit CA
    ...
    secured._mason_initial_addheaders = list(getattr(installed, "addheaders", ()))
    secured.addheaders = []
    return secured
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_mason_initial_addheaders", ()):
