...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: mason gateway install\n"
                "                    sudo mason gateway install --system  # Linux servers\n"
                "     Check status:  mason cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'mason cron create ...' "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `mason cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: mason cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: mason cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec mason mason cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `mason cron status`.\n"
              "  If heartbeat never appears, restart: mason gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: mason gateway restart")
    ...
    if provider != "builtin":
        ...
        if pids or gateway_alive_via_lock:
            ...
            print("\n  To enable automatic execution:\n"
                  "    mason gateway install    # Install as a user service\n"
                  "    sudo mason gateway install --system  "
                  "# Linux servers: boot-time system service\n"
                  "    mason gateway            # Or run in foreground")
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside MASON_HOME/scripts: {script!r}"
    if not path.exists():
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `mason cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits MASON_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (MASON_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _mason_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _mason_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``mason cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: mason cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: mason cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
