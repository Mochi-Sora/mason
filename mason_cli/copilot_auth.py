...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `mason model` to authenticate via OAuth\n"
...
def _post_form(url: str, fields: dict, timeout: float) -> dict:
    req = urllib.request.Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "MasonAgent/1.0",
...
def _jwt_disk_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_mason_home()) / _JWT_DISK_FILENAME
    except Exception:
        return None
...
def copilot_request_headers(
    *, is_agent_turn: bool = True, is_vision: bool = False) -> dict[str, str]:
    ...
    headers: dict[str, str] = {"Editor-Version": _EDITOR_VERSION, "User-Agent": "MasonAgent/1.0",
