"""Post-setup install hooks and installed-state predicates for `mason tools` provider rows."""
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Mason-managed ``$MASON_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Mason under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Mason-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung mason_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Mason-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Mason-managed node -> npx) rather than
    # a bare shutil.which — Mason-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
    ...
    if _running_in_docker():
        ...
        _info_lines("Pull the latest image to get the bundled Chromium:",
                    "  docker pull ghcr.io/nousresearch/mason-agent:latest")
        return
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.mason/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.mason/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `mason auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.mason/.env)
    # then PKCE; existing app → OAuth only.
    from types import SimpleNamespace
    try:
        from mason_cli.auth import login_spotify_command
    except Exception as exc:
        _print_warning(f"    Could not load Spotify auth: {exc}")
        _info_lines("Run manually: mason auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: mason auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from mason_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _info_lines("Run manually: mason plugins enable observability/langfuse")
    _info_lines("Restart Mason for tracing to take effect.", "Verify: mason plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from mason_cli.setup import prompt_choice, prompt as _setup_prompt
        from mason_cli.setup_tts import _run_xai_oauth_login_from_setup
        from mason_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: mason auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `mason auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: mason auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: mason auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``mason tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: mason tools post-setup <key>")
        return 2
...
# Python deps installed via ``mason tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``mason tools`` Python dependencies present in this runtime."""
    return [
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from mason_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from mason_cli.nous_subscription import _has_agent_browser
