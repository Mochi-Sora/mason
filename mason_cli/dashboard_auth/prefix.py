...
Proxies mounting the dashboard at a path inject ``X-Forwarded-Prefix: /mason``
so the backend can build prefixed URLs (Location headers, OAuth redirect_uri,
cookie Path, SPA asset URLs). An operator-declared ``MASON_DASHBOARD_PUBLIC_URL``
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    _warn_once(
        ...
        source, cleaned, cleaned.split("://")[-1] or "mason.example.com")
...
def normalise_prefix(raw: Optional[str]) -> str:
    """``"/mason"`` form (no trailing slash) or ``""`` when unset/malformed. ``..``, ``//`` and
...
# --- MASON_DASHBOARD_PUBLIC_URL / dashboard.public_url --------------------
...
def resolve_public_url() -> str:
    ...
    ``MASON_DASHBOARD_PUBLIC_URL`` env (blank counts as unset so a provisioned-but-blank secret
    cannot shadow config.yaml), then ``dashboard.public_url``; malformed warns and falls through."""
    env_raw = os.environ.get("MASON_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("MASON_DASHBOARD_PUBLIC_URL env var", env_raw)
