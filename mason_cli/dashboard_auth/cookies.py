...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``mason_session_at``
(access token; Max-Age = token TTL), ``mason_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``mason_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``mason_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``mason_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "mason_session_at"
SESSION_RT_COOKIE = "mason_session_rt"
SESSION_PROVIDER_COOKIE = "mason_session_provider"
PKCE_COOKIE = "mason_session_pkce"
SSO_ATTEMPT_COOKIE = "mason_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/mason`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (mason #37247); a provider that omits it returns
