"""Audit log for dashboard-auth events: ``$MASON_HOME/logs/dashboard-auth.log``, one JSON object
per line. Token-like fields are stripped before serialisation so refresh tokens / JWTs never
reach disk. Minimal import surface (no ``mason_constants`` at import time) so early-loading
...
def _resolve_log_path() -> Path:
    ...
    return get_mason_home() / "logs" / "dashboard-auth.log"
