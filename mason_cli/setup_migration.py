...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    from mason_cli.setup import get_env_value
    try:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    detection), else None. ``get_env_value`` is reached through mason_cli.setup so test patches
...
def _skip_configured_section(config: dict, section_key: str, label: str) -> bool:
    ...
    from mason_cli.setup import print_success, prompt_yes_no
...
_OPENCLAW_SCRIPT = (
    ...
    / "migration" / "openclaw-migration" / "scripts" / "openclaw_to_mason.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_mason migration script as a module; None if it can't be loaded."""
    if not _OPENCLAW_SCRIPT.exists():
        return None
    spec = importlib.util.spec_from_file_location("openclaw_to_mason", _OPENCLAW_SCRIPT)
...
# platforms; config values and instruction/context .md files may not map 1:1 to Mason.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Mason to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Mason at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Mason at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Mason at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Mason at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Mason equivalents",
...
_MIGRATION_WARNING_NOTES = (
    "  Note: OpenClaw config values may have different semantics in Mason.",
    "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Mason's yolo mode.",
...
def _print_migration_preview(report: dict):
    ...
    config semantics)."""
    from mason_cli.setup import color, Colors, print_info
    ...
    groups = (
        ...
        ("conflict", "  Would overwrite (conflicts with existing Mason config):", Colors.YELLOW,
...
def _run_migrator(mod, openclaw_dir: Path, mason_home: Path, selected, *, execute: bool, overwrite: bool):
    """Run a Migrator with the fixed first-time-setup options and return its report."""
    return mod.Migrator(
        source_root=openclaw_dir.resolve(), target_root=mason_home.resolve(), execute=execute,
...
def _migration_step(label: str, log_label: str, fn):
    ...
    from mason_cli.setup import print_warning
    try:
        return fn()
...
def _offer_openclaw_migration(mason_home: Path) -> bool:
    ...
    _info(f"Found OpenClaw data at {openclaw_dir}",
          "Mason can preview what would be imported before making any changes.", None)
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info("Skipping migration. You can run it later with: mason claw migrate --dry-run")
        return False
    ...
    def _preview():
        ...
        return selected, _run_migrator(mod, openclaw_dir, mason_home, selected, execute=False, overwrite=True)
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        _info("Migration cancelled. You can run it later with: mason claw migrate",
              "Use --dry-run to preview again, or --preset minimal for a lighter import.")
        return False

    # overwrite=False so existing Mason configs are preserved. The user saw the preview;
    # conflicts are skipped by default.
    report = _migration_step("Migration failed", "OpenClaw migration error", lambda: _run_migrator(
        mod, openclaw_dir, mason_home, selected, execute=True, overwrite=False))
    ...
    for key, printer, text in (
        ...
        ("conflict", print_info,
         "Skipped {n} item(s) that already exist in Mason (use mason claw migrate --overwrite to force)."),
