...
_STATE_DB_GUARD_BYPASS_ENV = "MASON_STATE_DB_GUARD_BYPASS"


def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Mason root. Avoids ``Path.home()`` /
    ``mason_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
    reads HOME/passwd, which the conftest never rewrites."""
    try:
        home = Path(os.path.expanduser("~"))
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", "").strip()
            root = Path(base) / "mason" if base else home / "AppData" / "Local" / "mason"
        else:
            root = home / ".mason"
        return root.resolve()
    except Exception:
...
#: Exported by the hermetic conftest alongside the MASON_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``MASON_HOME`` to
...
# under Mason test : isolation", and it inherits into subprocess children by default — so a : child that
# received the patched ``MASON_HOME`` also received the marker, : and a child that resolves a production DB
# while carrying it is, by : definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "MASON_TEST_ISOLATION"
...
def _ancestry_check_available() -> bool:
    """True when the ancestry fallback can actually run (psutil importable).

    psutil is a pinned core dependency — when it is missing the install is
    broken AND the guard is blind, so callers must fail closed, never open.
    """
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the MASON_HOME redirect together, ancestry
    survives that. FAILS CLOSED without psutil: a blind guard assumes test.

    ``_running_under_pytest`` reads ``PYTEST_*`` env vars, which a child spawned with a rebuilt environment
    loses at the same moment it loses the ``MASON_HOME`` redirect: that child aims at the production DB
    *and* disarms the guard in one step (#82770). Ancestry is the one test-context signal that survives an
    ...
    if not _ancestry_check_available():
        # Fail closed: without psutil the guard cannot see the process tree,
        # so it must assume the worst (a scrubbed test child) and refuse.
        # psutil is a pinned core dep — this path means a broken install.
        _PYTEST_ANCESTOR = True
        return True
    assert psutil is not None  # narrowed by _ancestry_check_available(); satisfies checkers
    try:
