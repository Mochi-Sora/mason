...
Mason Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``MASON_NIX_BUILD=1`` on the
Mason package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("MASON_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for mason-agent is not supported.\n"
    "Mason is distributed via the shell installer, Docker image, or Nix.\n"
    "See README.md (Quick Install) in your Mason checkout\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Mason Nix derivation sets MASON_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``mason_state``, ``toolsets``...)
