...
# caplog tests pin the "mason_state" logger name.
...
def _cwd_prefix_filter(value: str) -> Tuple[List[str], list]:
    from mason_state_sessions import _cwd_prefix_clause
...
class SessionMaintenanceMixin:
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            # PASSIVE, not TRUNCATE: a manual `mason sessions vacuum` runs in a transient CLI
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        """
        from mason_state_repair import _release_auto_maintenance_lock, _try_acquire_auto_maintenance_lock
