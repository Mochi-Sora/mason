...
from mason_constants import OPENROUTER_BASE_URL, get_mason_home
from agent.retry_utils import jittered_backoff
from mason_cli.env_loader import load_mason_dotenv

# Load .env from MASON_HOME first, then project root as a dev fallback.
load_mason_dotenv(mason_home=get_mason_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: mason setup")
