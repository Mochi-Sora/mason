# Mason Agent - Development Guide

Instructions for AI coding assistants and developers working on the mason-agent codebase.
...
Mason is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Mason ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`mason tools`, `mason setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `MASON_HOME`. Mocks hide integration bugs.
...
- **New `MASON_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `mason tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.mason/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `mason <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`mason webhook`, `mason cron`, `mason tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.mason/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Mason Cloud, and
only the first two carry `MASON_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Mason desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_MASON_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `MASON_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`mason --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.mason/mason-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _MASON_CORE_TOOLS
├── cli.py                # MasonCLI (REPL, slash dispatch) + mason_cli/cli_*_mixin.py
├── mason_state.py       # SessionDB facade; mason_state_*.py siblings
├── mason_constants.py   # get_mason_home(), display_mason_home() — profile-aware paths
├── mason_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── mason_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `mason --tui`
...
**User state:** `~/.mason/config.yaml` (settings), `~/.mason/.env` (secrets only),
`~/.mason/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_mason_home()`. Browse logs with `mason logs [--follow] [--level] [--session]`.
...
families: `mason_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`mason_cli/kanban.py` (14), `mason_cli/web_server.py` (13 + 24 routers), `mason_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `mason_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::mason_cli.plugin_compat.MasonPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `mason_cli.update_cmd._mason_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `mason_cli/AGENTS.md`.
- **Never hardcode `~/.mason`.** `get_mason_home()` for code paths, `display_mason_home()`
  for user-facing text (both from `mason_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `MASON_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.mason/profiles`) so `mason -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `mason webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `MASON_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `MASON_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `MASON_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Tests must not write to `~/.mason/`.** The autouse `_isolate_mason_home` fixture in
  `tests/conftest.py` redirects `MASON_HOME`; never hardcode `~/.mason/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_mason_home()` stay
  in the temp dir (pattern: `tests/mason_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".mason"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("MASON_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `MASON_HOME` — code reads the
  env var, not `Path.home()/.mason`.
...
`tests/mason_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `mason_cli/`, `main.py` | `mason_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `mason update` pipeline, profiles / multiplex |
...
| `plugins/`, `mason_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `mason_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`mason_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`mason-agent-dev` skill, not here.
