...
def _get_script_timeout() -> int:
    ...
    resolved = _timeout_from_env_or_config(
        "MASON_CRON_SCRIPT_TIMEOUT", "script_timeout_seconds", _positive_int,
        "cron script timeout",
    )
...
def _get_media_send_timeout() -> int:
    """Per-attachment media-send timeout: MASON_CRON_MEDIA_SEND_TIMEOUT env, then
    ``cron.media_send_timeout_seconds``, then 300s (long TTS audio can exceed a 30s window)."""
    resolved = _timeout_from_env_or_config(
        "MASON_CRON_MEDIA_SEND_TIMEOUT", "media_send_timeout_seconds", _positive_int,
...
def _get_session_db_timeout() -> float:
    """Bound on run_job's SessionDB init: MASON_CRON_SESSION_DB_TIMEOUT env, then
    ...
    resolved = _timeout_from_env_or_config(
        "MASON_CRON_SESSION_DB_TIMEOUT", "session_db_timeout_seconds", float,
...
def _resolve_script_path(script_path: str) -> tuple[Optional[Path], Optional[str]]:
    ...
    inside MASON_HOME/scripts/ (relative, absolute and ``~`` paths are all validated — path
    traversal / absolute-path injection); contract of lifecycle_guard._expand_candidate_path."""
    scripts_dir = _sched._get_mason_home() / "scripts"
    ...
    # Traversal / absolute-path / symlink escape guard — MUST stay inside MASON_HOME/scripts/.
    try:
...
def _run_job_script(
    ...
    Args: script_path: Path to the script. Relative paths are resolved against MASON_HOME/scripts/.
