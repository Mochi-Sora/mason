...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Mason — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from mason_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    ``mason [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file`` — the
    ...
    if mason_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("mason_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: mason CLI not resolvable"
        argv = [sys.executable, "-m", "mason_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's MASON_HOME must not shadow it.
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="mason-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`mason cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_MASON_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `mason cron list` shows it.
    ...
    # at boot; standalone runs (`mason cron run`) did not, silently dropping files. Idempotent.
