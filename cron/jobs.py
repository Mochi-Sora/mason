"""Cron job storage: ~/.mason/cron/jobs.json; output in
~/.mason/cron/output/{job_id}/{timestamp}.md"""
...
# Cron is per-profile by design: anchor at get_mason_home() (active profile home), NOT
# get_default_mason_root() — the shared root would funnel every profile's jobs into one jobs.json
# and run them under the ticker's MASON_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own MASON_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same MASON_HOME — so a job authored in profile `coder` lives in
# `~/.mason/profiles/coder/cron/jobs.json` and executes with `coder`'s `.env`, `config.yaml`, and skills.
# Do NOT change this to the default root: that re-breaks per-profile isolation. See also the dynamic
# `_get_mason_home()` / `_get_lock_paths()` resolution in cron/scheduler.py. See #4707.
MASON_DIR = get_mason_home().resolve()
...
CRON_DIR = MASON_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat: touched every ticker loop so `mason cron status` can tell the ticker THREAD is alive,
# not just the gateway PROCESS; success = last tick that completed WITHOUT raising.
# The gateway process and the (separate) ``mason cron status`` process share it so status can tell whether
...
# threshold in `mason cron status` (mason_cli/cron.py), so they never drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_mason_home(), so re-pointing MASON_HOME after import uses ITS OWN store rather than the
...
# Fallback stale-recovery window for a one-shot's running-claim when MASON_CRON_TIMEOUT=0
...
def _oneshot_run_claim_ttl_seconds() -> float:
    """One-shot running-claim TTL from ``MASON_CRON_TIMEOUT``: unset/invalid → 600s → 1800s;
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<mason_home>/profiles/<name>/`` (default/custom homes are not).
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # Naive timestamps become aware in the CONFIGURED Mason timezone (not server-local):
            # the due-check compares against mason_time.now().
            ...
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=_mason_now().tzinfo)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    if schedule_lower.startswith("in "):
        ...
        run_at = _mason_now() + timedelta(minutes=minutes)
...
def _ensure_aware(dt: datetime) -> datetime:
    """Aware datetime in the configured Mason timezone. Legacy naive values are read as
...
def _record_persisted_error_recovery(job: Dict[str, Any], previous_next_run: str) -> None:
    ...
    entry = {
        ...
        "rearmed_at": _mason_now().isoformat(),
    }
    ...
    if kind == "cron":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall mason-agent or run 'pip install croniter' in your runtime env.",
                expr)
            return None
...
# --- Ticker heartbeat (liveness signal for `mason cron status`) ---
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    tick* marker. We track two distinct signals so `mason cron status` can tell a thread that is merely
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    critical under multiplex_profiles where ``mason cron status`` must report per-profile liveness
    (#69377).
    """
...
def _stage_jobs_payload(jobs_file: Path, jobs: List[Dict[str, Any]]) -> str:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"jobs": jobs, "updated_at": _mason_now().isoformat()},
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_mason_home() / "config.yaml"
    ...
    *, provider: Any, model: Any, base_url: Any, no_agent: Any,
    ...
    if normalized_provider is None:
        with contextlib.suppress(Exception):
            ...
            # Delegate all rate-limit / 5xx retry to mason's outer conversation loop, which honors
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(job["id"], {
        "enabled": False,
        "state": "paused",
        "paused_at": _mason_now().isoformat(),
...
def trigger_job(job_id: str, extra_prompt: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'mason cron resume {name} "
            "--run-now' or '--at <ISO-8601>'.")
    manual_run_at = _mason_now().isoformat()
    return update_job(job["id"], {
        "enabled": True,
        "state": "scheduled",
        "paused_at": None,
...
def note_fire_forward_failure(job_id: str, detail: str) -> bool:
    ...
    def apply(jobs, _i, job):
        job["last_fire_error"] = {
            "at": _mason_now().isoformat(), "detail": str(detail or "")[:500]}
        save_jobs(jobs)
        return True
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    written = _write_oneshot_diagnostic(
        ...
        f"- removed at: {_mason_now().isoformat()}\n\n"
...
def _machine_id() -> str:
    ...
    fresh-claim check): ``MASON_MACHINE_ID`` if set, else hostname:pid."""
    explicit = os.getenv("MASON_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _oneshot_dispatch_limit_reached(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if job.get("last_run_at") is not None:
        ...
        logger.warning(
            ...
            "'mason cron resume <job> --run-now' (#93524).",
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same MASON_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    raw_jobs = load_jobs()
    scan = _DueScan(raw_jobs, _mason_now())
...
# the disk.
# Unlike the quick-snapshot store (`mason_cli.backup`, capped at 20) it had no retention, so a
...
def save_job_output(job_id: str, output: str):
    ...
    output_file = job_output_dir / f"{_mason_now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
