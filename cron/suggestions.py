...
# Per-profile by design (anchored on get_mason_home(), see cron/jobs.py). Optional test override;
# production resolves the path at CALL time so multiplexed profile ticks (set_mason_home_override)
...
# get_mason_home() (profile home), not the shared default root. Same pattern as cron/executions.py.
...
def _current_suggestions_file() -> Path:
    return SUGGESTIONS_FILE or (get_mason_home().resolve() / "cron" / "suggestions.json")
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            payload = {"suggestions": suggestions, "updated_at": _mason_now().isoformat()}
    ...
    *, title: str, description: str, source: str, job_spec: Dict[str, Any], dedup_key: str,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _mason_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _mason_now().isoformat()
                _save_raw(suggestions)
                return True
        return False
