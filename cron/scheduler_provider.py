...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``mason profile list`` (#47368). Filtering on directory existence leaves a deleted
...
def _profile_cron_scope(home):
    ...
    # unsuccessful (#32612).
    home_token = set_mason_home_override(str(home))
    try:
...
def fire_overdue_jobs(
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _mason_now, is_job_runnable, load_jobs,
    )
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global MASON_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `mason cron status` reflects liveness for every profile independently.
        if profile_homes:
        ...
        # Heartbeat before the first sleep so `mason cron status` sees a live ticker immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `mason cron status` (separate process) shows WHY.
