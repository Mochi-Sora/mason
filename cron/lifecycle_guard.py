...
A cron job that restarts/stops the gateway from inside the gateway (``mason gateway restart``,
``launchctl kickstart ai.mason.gateway``, ``systemctl restart mason-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `mason gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `mason` from being a path component or word tail (`/docs/mason gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])mason\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a mason-gateway label so unrelated mason services stay
    ...
    # -l ai.mason.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a mason-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.mason.gateway-... \` / `  -- ...`,
...
# Branch A2: `mason -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `mason -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the mason-gateway label anchor
...
_MASON_GATEWAY_LABEL_RE = re.compile(r"(?i)\bmason[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``MASON_PROFILE_NAME``/``MASON_PROFILE`` env first, then
    ``mason_cli.profiles.get_active_profile_name`` (from ``MASON_HOME``); ``None`` if neither."""
    for env_name in ("MASON_PROFILE_NAME", "MASON_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _MASON_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``mason``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: mason gateway
    ...
    # Profile-flag form (#78028): `mason -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `mason` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<MASON_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_mason_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_mason_home() falls back to Path.home(), which raises when neither MASON_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.mason/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `mason gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `mason gateway restart` from a shell outside "
