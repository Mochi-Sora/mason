...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `mason setup`), or pin a "
            "working provider via `mason cron edit "
...
def _primary_profile_routes_for_current_home() -> list:
    ...
    try:
        ...
        primary_home = get_default_mason_root()
        current_home = _sched.Path(get_mason_home())
        if (
    ...
    for platform_name in platform_parts:
        ...
        if (
            ...
            return (
                ...
                "`mason setup` or change the job's `deliver` target."
            )
    return None
