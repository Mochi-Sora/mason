...
(``mason cron notepad <job_id> set ...``) via the terminal tool; no model tool is added.
"""
...
# ticks (set_mason_home_override) cannot leak one profile's notepad rows into the import-time home
...
def _current_notepad_file() -> Path:
    return NOTEPAD_FILE or (get_mason_home().resolve() / "cron" / "notepad.db")
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`mason cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `mason cron notepad {job_id} delete "
