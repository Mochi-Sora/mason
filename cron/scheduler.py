...
A file lock (~/.mason/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
# `mason update`) otherwise fail with ModuleNotFoundError for mason_time et al.
...
from mason_time import now as _mason_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `mason fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `mason cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``mason cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Mason, pin it explicitly: "
                f"`mason cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # (interrupted `mason update`, manual git pull) serves MIXED modules — old entries frozen in
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`mason gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy mason_cli import at module load; shares MCP-membership with gateway/CLI
    from mason_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from mason_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    from mason_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = os.getenv("MASON_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid MASON_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _mason_now().isoformat()}
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_mason_home() / "cron" / "inflight_forced_releases.jsonl"
...
def mark_running_jobs_interrupted(
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_mason_home())
                for job_id in (
...
def _cron_inactivity_seconds() -> float:
    """Parse MASON_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the
    ...
    raw = os.getenv("MASON_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid MASON_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Per-fire usage audit log; resolves via _get_mason_home() so profile-scoped paths work.
def _usage_audit_path() -> Path:
    return _get_mason_home() / "cron" / "usage_audit.jsonl"
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``mason update`` / ``mason
...
_mason_home: Path | None = None


def _get_mason_home() -> Path:
    """Mason home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active MASON_HOME at call time means a profile's jobs are stored AND executed under that
    profile's home (its .env, config.yaml, scripts, skills).
    """
    return _mason_home or get_mason_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    mason_home = _get_mason_home()
    lock_dir = mason_home / "cron"
...
def _run_no_agent_job(
    ...
    try:
        ...
        load_mason_dotenv(mason_home=_get_mason_home())
    except Exception:
    ...
    now_iso = _mason_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _apply_monitor_gate(
    ...
    _mon_now = _mason_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    MASON_MODEL > config ``model:``. Re-read every tick (no cache) so ``mason cron edit --model``
    applies next tick. An axis resolved from cron.model/model_provider is explicit (no drift guard)."""
    model = job.get("model") or os.getenv("MASON_MODEL") or ""
    _cron_default_provider = ""
    _cfg: dict = {}
    _model_cfg: Any = {}
    try:
        from mason_cli.config import read_user_config_raw
        _cfg_path = str(_get_mason_home() / "config.yaml")
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"MASON_MODEL={os.getenv('MASON_MODEL', '')!r}, "
            "config.yaml model.default missing or empty). "
            f"Set a per-job model via "
            f"`mason cron edit {job_id} --model <name>` or set a "
            "default with `mason model <name>`."
        )
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        os.getenv("MASON_PREFILL_MESSAGES_FILE", "")
...
def _preflight_or_block(job: dict, job_id: str, job_name: str, cfg: dict) -> Optional[tuple]:
    ...
    blocked_doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {_mason_now().strftime('%Y-%m-%d %H:%M:%S')}\n"
...
def _resolve_job_runtime(
    ...
    try:
        # Do NOT pass MASON_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _check_model_drift(
    ...
    if _finite_oneshot:
        ...
        _remediation = (
            "To run on the new config, on the host running Mason pin it explicitly: "
            f"`mason cron edit {job_id} --provider <provider> "
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (MASON_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from MASON_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override MASON_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from mason_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # MASON_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    try:
        ...
        _cron_title = f"{_title_base} · {_mason_now().strftime('%b %d %H:%M')}"
    ...
    # vocabulary in mason_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: MASON_IGNORE_USER_CONFIG=1.
...
_CRON_DELIVERY_VARS = (
    "MASON_CRON_AUTO_DELIVER_PLATFORM",
    "MASON_CRON_AUTO_DELIVER_CHAT_ID",
    "MASON_CRON_AUTO_DELIVER_THREAD_ID")
...
class _CronRunScope:
    ...
    MASON_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / MASON_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the MASON_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # MASON_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["MASON_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # MASON_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    from mason_cli.env_loader import load_mason_dotenv, reset_secret_source_cache
    ...
    if delivery_target:
        _VAR_MAP["MASON_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
        _VAR_MAP["MASON_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
        _VAR_MAP["MASON_CRON_AUTO_DELIVER_THREAD_ID"].set(
...
def run_job(
    ...
    _cron_session_id = f"cron_{job_id}_{_mason_now().strftime('%Y%m%d_%H%M%S')}"
...
def run_one_job(
    ...
    external_owner = os.environ.get("_MASON_CRON_EXTERNAL_WORKER") == execution_id
    ...
    profile_home = _get_mason_home().resolve()
    with _running_lock:
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_mason_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_mason_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_mason_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_mason_home().resolve()),
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"MASON_HOME": str(_get_mason_home().resolve())},
    )
    try:
        process = subprocess.Popen(
            scoped_command,
...
def _run_external_worker_payload(payload_path: Path, ack_path: Path) -> bool:
    ...
    home_token = set_mason_home_override(profile_home)
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_MASON_CRON_EXTERNAL_WORKER")
            os.environ["_MASON_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_MASON_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_MASON_CRON_EXTERNAL_WORKER"] = old_external_execution
    finally:
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the mason checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # Mason source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``mason -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `mason cron run` that claimed a job and
...
def _resolve_max_parallel_workers() -> Optional[int]:
    """Max workers: env > config.yaml > unbounded (MASON_CRON_MAX_PARALLEL=1 restores serial)."""
    try:
        _env_par = os.getenv("MASON_CRON_MAX_PARALLEL", "").strip()
        if _env_par:
            return int(_env_par) or None
    except (ValueError, TypeError):
        logger.warning("Invalid MASON_CRON_MAX_PARALLEL value; defaulting to unbounded")
    with contextlib.suppress(Exception):
        _ucfg = load_config() or {}
...
def tick(
    ...
    try:
        # `mason pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
        ...
        if not due_jobs:
            ...
            if verbose:
                ...
                # due.
                logger.info("%s - No jobs due", _mason_now().strftime('%H:%M:%S'))
            _sweep_mcp_orphans()
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _mason_now().strftime('%H:%M:%S'), len(due_jobs))
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from mason_logging import setup_logging

            setup_logging(mason_home=_get_mason_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
