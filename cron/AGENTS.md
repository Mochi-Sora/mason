...
schedule via the `cronjob` tool; users via `mason cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.mason/cron/.tick.lock` prevents duplicate ticks across processes.
...
- The cron ticker runs in the desktop-spawned backend when `MASON_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `mason kanban
...
- **CLI:** `mason_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/mason-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `MASON_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/mason_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
