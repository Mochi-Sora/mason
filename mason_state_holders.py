...
an unlinked SQLite DB/WAL/SHM generation.  ``mason_state`` supplies only the
...
_MASON_EXECUTABLES = frozenset({"mason", "mason-agent", "mason-acp"})
_MASON_PYTHON_MODULES = frozenset({"acp_adapter", "mason_cli.main"})
_MASON_PYTHON_SCRIPTS = frozenset({"mason_cli/main.py", "run_agent.py"})
...
def _looks_like_mason(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Mason execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _MASON_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _MASON_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _MASON_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_mason(argv):
                        cmdline = " ".join(argv)
