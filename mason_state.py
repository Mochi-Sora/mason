#!/usr/bin/env python3
"""SQLite state store for Mason Agent: session metadata, message history, model
...
from mason_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from mason_state_errors import (
...
from mason_state_guard import (
    _STATE_DB_GUARD_BYPASS_ENV, _ancestry_check_available, _in_test_context, _is_production_state_db,
    _real_platform_state_root, _set_last_init_error, get_last_init_error,
)
from mason_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from mason_state_sessions import SessionSessionsMixin
from mason_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from mason_state_portability import SessionPortabilityMixin
from mason_state_telegram import SessionTelegramTopicsMixin
from mason_state_schema import SessionSchemaMixin
import mason_state_holders as _state_holders
from mason_state_dbfile import (
...
from mason_state_messages import SessionMessagesMixin
from mason_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from mason_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from mason_state_titles import SessionTitlesMixin
from mason_state_usage import SessionUsageMixin
from mason_state_maintenance import SessionMaintenanceMixin
from mason_state_gateway import SessionGatewayMixin
from mason_state_compression import SessionCompressionMixin
from mason_state_search import SessionSearchMixin
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_mason_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_mason_home()`` is resolved fresh (a runtime MASON_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_mason_home() / "state.db"


# Live-DB guard knobs live HERE (not in mason_state_guard): the hermetic conftest monkeypatches
# ``mason_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-MASON_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``MASON_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            blind = "" if _ancestry_check_available() else (
                " (ancestry check unavailable — psutil is not installed, so the guard "
                "refused rather than risk it: pip install psutil==7.2.2)"
            )
            raise RuntimeError(
                "live-system guard: test attempted to open production "
                f"state.db at {resolved} (under real Mason root {root}){blind}. "
                "Tests must run against a temporary MASON_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "MASON_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to MASON_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_mason_home() / "sessions"
...
# connection per resolved path via mason_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in mason_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via mason_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many mason processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via mason_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via mason_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`mason db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_zeroed(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `mason snapshot list` / "
            f"`mason snapshot restore <id>` if available. "
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Mason process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `mason sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see mason_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``mason_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from mason_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                if self._db_corrupt:  # quarantined: no checkpoint over a damaged image
                    logger.warning(
                        ...
                        "then run `mason sessions recover --source %s --inspect-only`.", self.db_path,
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('mason_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('mason_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('mason_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('mason_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('mason_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('mason_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('mason_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('mason_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('mason_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('mason_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('mason_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('mason_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('mason_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('mason_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('mason_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('mason_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('mason_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('mason_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('mason_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('mason_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('mason_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('mason_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('mason_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('mason_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('mason_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('mason_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('mason_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('mason_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('mason_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('mason_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('mason_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('mason_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('mason_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('mason_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('mason_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('mason_state_registry', 'release_or_close'),
    'report_startup_progress': ('mason_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('mason_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('mason_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('mason_state_wal', 'sqlite_source_id'),
    'workspace_key': ('mason_state_sessions', 'workspace_key'),
}
