"""SQLite journal-mode and PRAGMA policy for state.db (split from mason_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``mason_state_wal.<name>``.
"""
...
from mason_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``mason_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    Returns the mode actually set. Shared by :class:`SessionDB` and ``mason_cli.kanban_db_connect.connect``.
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``mason update`` can actually do for this install type.

    See #75153.
    """
    try:
        from mason_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if method in {"git", "unknown"}:
            return f"Mason-managed installs can repair the embedded runtime with `{cmd}`"
        return f"update the container image with `{cmd}`" if method == "docker" else cmd  # else nix/nixos
    except Exception:
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Mason"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `mason doctor`. This warning fires once per process per database."),
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from mason_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
