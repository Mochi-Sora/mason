...
_MASON_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `mason-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _MASON_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_MASON_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _MASON_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `mason tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``mason tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _MASON_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (MASON_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _MASON_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in mason_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Mason toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. mason-acp is the
    ...
    "mason-api-server": _ts(
        "OpenAI-compatible API server — full agent tools accessible via HTTP (no "
        "interactive UI tools like clarify or send_message)",
        _core_without("text_to_speech", "clarify", "computer_use",
    "read_state", "write_state", "recall_backup", kanban=False),
    ),
    "mason-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors mason-cli; `mason tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "mason-cron": _bundle("Default cron toolset - same core tools as mason-cli; gated by `mason tools`"),
    "mason-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "mason-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "mason-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "mason-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "mason-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "mason-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "mason-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "mason-email": _bundle("Email bot toolset - interact with Mason via email (IMAP/SMTP)"),
    "mason-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "mason-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "mason-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "mason-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "mason-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "mason-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "mason-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "mason-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "mason-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _MASON_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "mason-sms": _bundle("SMS bot toolset - interact with Mason via SMS (Twilio)"),
    "mason-webhook": _ts("Webhook toolset - receive and process external webhook events", _MASON_WEBHOOK_SAFE_TOOLS),
    "mason-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "mason-telegram", "mason-discord", "mason-whatsapp", "mason-slack",
            "mason-signal", "mason-bluebubbles", "mason-homeassistant", "mason-email",
            "mason-sms", "mason-mattermost", "mason-matrix", "mason-dingtalk",
            "mason-feishu", "mason-wecom", "mason-wecom-callback", "mason-weixin",
            "mason-qqbot", "mason-webhook", "mason-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _MASON_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only mason-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_MASON_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `mason-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("mason-"):
        return []
    platform_name = name[len("mason-"):]
    try:
