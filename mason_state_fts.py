...
from mason_state_common import FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("MASON_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_mason_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("MASON_CJK_FTS", "1").strip().lower() not in ("0", "false", "off", "no")
...
class SessionFtsSetupMixin:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `mason update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `mason sessions optimize-storage` on a host "
