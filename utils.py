"""Shared utility functions for mason-agent."""
...
def warn_if_credential_file_broadly_readable(path: Union[str, Path], *, label: str = "", log: logging.Logger | None = None) -> bool:
    ...
    Hand-made secret files (or ones older Mason wrote without an explicit mode) commonly end up
...
def atomic_roundtrip_yaml_update(path: Union[str, Path], key_path: str, value: Any) -> None:
    ...
    # blind splitting — same navigation as ``mason config set``'s ``_set_nested``; otherwise
    # /model + TUI persistence wrote ``glm-5: {'3': ...}`` phantom siblings.
    # See #91607.
    from mason_cli.config import _greedy_literal_match, _split_key_path
...
def atomic_roundtrip_yaml_save(path: Union[str, Path], new_state: dict) -> None:
    ...
    from mason_cli.config import require_readable_config_before_write
