"""
Mason MCP Server — expose messaging conversations as MCP tools (`mason mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Mason-specific
channels_list. Client config: {"mcpServers": {"mason": {"command": "mason", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("mason.mcp_serve")
...
def _mason_home() -> Path:
    try:
        ...
        return Path(os.environ.get("MASON_HOME", Path.home() / ".mason"))


def _get_sessions_dir() -> Path:
    return _mason_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_mason_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_mason_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Mason analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Mason MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("mason", instructions=(
        "Mason Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Mason MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
