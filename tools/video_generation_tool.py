...
Ships **no in-tree provider**: enable a plugin and select it in ``mason tools`` → Video
    ...
    if configured:
        return json.dumps(error_response(
            error=(f"video_gen.provider='{configured}' is set but no plugin registered that name. "
                   f"Run `mason plugins list` to see installed video gen backends, or "
                   f"`mason tools` → Video Generation to pick one."),
            error_type="provider_not_registered", provider=configured))
    return json.dumps(error_response(
        error=("No video generation backend is configured. Run `mason tools` → "
...
_GENERIC_DESCRIPTION = (
    ...
    "`mason tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `mason tools` → Video Generation.")
