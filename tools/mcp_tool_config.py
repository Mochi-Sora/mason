...
def _node_fallback(command: str) -> str:
    ...
    mason_home = os.path.expanduser(os.getenv("MASON_HOME", os.path.join(home, ".mason")))
    # /usr/local/bin: canonical Node location (from-source Linux, Mason Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(mason_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``mason_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Mason already supervises the
