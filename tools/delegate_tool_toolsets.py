...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    """Add every toolset whose tools are a subset of the parent's tools: a parent on a composite like ``mason-cli``
...
def _resolve_child_toolsets(
    ...
    ``disabled_toolsets`` so blocked names inside mixed bundles (mason-cli) are subtracted AFTER composite
