...
refresh; this module supplies ``MasonTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Mason' Client ID Metadata
...
from mason_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__mason_user_skipped__"


def _get_token_dir(mason_home: str | Path | None = None) -> Path:
    """``MASON_HOME/mcp-tokens/`` — per-profile token directory."""
    from mason_constants import get_mason_home

    return Path(mason_home if mason_home is not None else get_mason_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "MasonTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "MasonTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``mason
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `mason mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # secure_parent_dir refuses to chmod /, top-level dirs, or the mason-agent install tree (#25821,
    # #93050).
    ...
    # mason-agent install tree (#25821, #93050).
...
class MasonTokenStorage:
    """Persist OAuth state as ``MASON_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, mason_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._mason_home = Path(mason_home) if mason_home is not None else None

    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._mason_home) / f"{self._server_name}{suffix}"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._mason_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Mason.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `mason mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: README.md in your Mason checkout\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Mason' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
MasonOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, mason_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    MasonTokenStorage(server_name, mason_home=mason_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Mason only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate — servers MUST NOT follow redirects when fetching it, and mason-agent.nousresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/mason-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "MasonTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Mason must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "MasonTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "MasonTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Mason Agent"),
...
def _invalidate_tokens_on_client_change(
    storage: "MasonTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: mason mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "MasonTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Mason is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global MasonOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `mason mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if MasonOAuthClientProvider is None:
        from tools.mcp_oauth_provider import MasonProviderMixin

        MasonOAuthClientProvider = type("MasonOAuthClientProvider", (MasonProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Mason' token-endpoint fixes (see ``MasonProviderMixin``).",
            "__module__": __name__, "_mason_logger": logger})
    return MasonOAuthClientProvider(server_url=server_url, **kwargs)
