...
processes survive cleanup, idle reaping, or Mason exit.
...
# display_mason_home imported lazily at call site (stale-module safety during mason update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("MASON_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's Mason profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("MASON_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``mason-agent=1`` for the current
    profile — leftovers of Mason processes that died without firing
    ...
    Mason processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``mason serve``, Desktop
    ...
    env values (possibly stale from ``mason setup``); env values for omitted keys
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from mason_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from mason_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Mason-owned control-plane
