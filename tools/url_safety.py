...
(TOCTOU) is closed for Mason-owned httpx paths by ``create_ssrf_safe_[async_]client()``, which
...
def normalize_url_for_request(url: str) -> str:
    """ASCII-safe HTTP URL for Mason-owned URL tools (IRI -> URI, e.g. ``https://wttr.in/Köln``).
...
def _global_allow_private_urls() -> bool:
    """True when the user has opted out of private-IP blocking. Priority: ``MASON_ALLOW_PRIVATE_URLS``
    env, ``security.allow_private_urls``, legacy ``browser.allow_private_urls``. Profile-scoped turns
    (``get_mason_home_override()`` set) bypass the process-global cache — a multiplex gateway serves
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("MASON_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def _install_ssrf_guard_on_transport(transport: Any, schemes_by_origin_var: Any, *, is_async: bool = False) -> None:
    ...
    if transport is None or state.get("_mason_ssrf_guarded", False):
        return
    ...
    transport._mason_ssrf_guarded = True
...
def _install_ssrf_guard_on_client(client: Any, *, is_async: bool = False) -> None:
    ...
    import contextvars
    var_name = "mason_ssrf_async_origin_schemes" if is_async else "mason_ssrf_origin_schemes"
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("mason_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("mason_ssrf_origin_schemes")
