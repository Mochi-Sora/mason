...
def _transcribe_openai(
    ...
    def _run(client):
        ...
        with tempfile.TemporaryDirectory(prefix="mason-stt-") as work_dir:
            try:
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `mason model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        from tools.xai_http import mason_xai_user_agent
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": mason_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``mason_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
    ...
    from mason_cli.models import deepinfra_base_url, deepinfra_model_ids
