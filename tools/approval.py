...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MASON_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Mason YOLO. Lazy import so approval-only
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``mason chat -q``
    exports MASON_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports MASON_INTERACTIVE=1 but nobody answers prompts, and MASON_EXEC_ASK has no
    ...
    is_ask = env_var_enabled("MASON_EXEC_ASK")
    if _is_single_query_approval_context():
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set MASON_INTERACTIVE or "
                               "MASON_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set MASON_INTERACTIVE or "
                       "MASON_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_mason_interactive_context': ('tools.approval_context', 'reset_mason_interactive_context'),
    ...
    'set_mason_interactive_context': ('tools.approval_context', 'set_mason_interactive_context'),
}
