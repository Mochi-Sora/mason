#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Mason desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Mason window as a JSON string."""
    return read_pane(callback, (), (
        "read_window_below is only available in the Mason desktop app.",
        "",
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        "Identify the app window directly behind the Mason desktop window "
