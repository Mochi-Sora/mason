...
from mason_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Mason secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = mason_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in mason_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "mason-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-mason_…``
...
# Owner Mason home per session: the janitor is one process-global thread, so each
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = mason_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_mason_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_mason_home_override': ('mason_constants', 'get_mason_home_override'),
    'mason_home_key': ('mason_constants', 'mason_home_key'),
    ...
    'node_tool_runnable': ('mason_constants', 'node_tool_runnable'),
