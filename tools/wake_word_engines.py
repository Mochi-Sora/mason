...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_mason") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under MASON_HOME. Keywords are tokenized at RUNTIME.
...
def _sherpa_model_root() -> Path:
    ...
    return get_mason_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey mason").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="mason-kws-", delete=False,
                                         encoding="utf-8")
