...
from tools.bot_mode_probe import _default_home, _mason_root
...
# One turn in a profile's canonical Bot Chat: ``mason -p <profile> *BOT_CHAT_TURN_ARGS``.
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import mason_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("mason" if profile == "default" else profile)
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_mason_root(Path(_default_home())))
...
def _mason_cli() -> str:
    """mason CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "mason" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``mason`` entrypoint. A bare ``"mason"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("mason.exe" if sys.platform == "win32" else "mason")
    return str(sibling) if sibling.is_file() else shutil.which("mason") or "mason"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_mason_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]


# Two deliveries into the SAME profile must never run Bot Chat turns concurrently.
# Deliveries are separate ``mason`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``mason``
