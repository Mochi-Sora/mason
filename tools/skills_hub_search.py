"""Skills Hub discovery: the centralized Mason index fetch (cached, stale-
...
from tools.skills_hub_official import MasonIndexSource, OptionalSkillSource
...
MASON_INDEX_URL = "https://mason-agent.nousresearch.com/docs/api/skills-index.json"
MASON_INDEX_TTL = 6 * 3600  # 6 hours


def _mason_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "mason-index.json"


def _load_mason_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for MASON_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _mason_index_cache_file()
    cached = _read_json_if_fresh(cache_file, MASON_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(MASON_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("Mason index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("Mason index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Mason index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    cached = _read_json_if_fresh(_mason_index_cache_file(), float("inf"))
    if isinstance(cached, dict) and cached.get("skills"):
        return cached
    return _builtin_skills_index()


def _builtin_skills_index() -> Optional[dict]:
    """Checked-in first-party index (scripts/gen_skills_index.py) — the detached
    fork's final fallback. Hub search works offline over shipped skills."""
    try:
        from pathlib import Path
        data = json.loads((Path(__file__).resolve().parent / "skills_index_builtin.json").read_text())
    except Exception:
        return None
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    return [
        ...
        MasonIndexSource(auth=auth),     # centralized index (search + resolved install paths)
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "mason-index" and getattr(src, "is_available", False) for src in sources
    )
