...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from mason_state_repair import apply_durability_barriers
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"MASON_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``MASON_SESSION_ID``
    ...
    request-scoped ``MASON_SESSION_CHAT_ID`` (raw X-Mason-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("MASON_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("MASON_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    try:
        # Propagate the dispatching profile so the detached child resolves get_mason_home() correctly.
