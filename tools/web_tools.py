...
Backend is selected during ``mason tools`` (``web.backend`` in config.yaml; per
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``mason config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Mason' config/.env layer
    (``mason config set``, ``mason tools``) are honored here too — not just raw process-env exports.
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `mason tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``mason doctor``)
