...
Registers when either xAI credential path is available (``XAI_API_KEY`` or ``mason auth add
...
from tools.xai_http import DEFAULT_XAI_BASE_URL, mason_xai_user_agent, resolve_xai_http_credentials
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `mason auth add xai-oauth` "
