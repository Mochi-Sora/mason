...
_PROBE_INITIALIZE_BODY = (  # JSON-RPC ``initialize`` body for the content-type preflight POST
    ...
    '"capabilities":{},"clientInfo":{"name":"mason-probe","version":"0.1"}}}')
...
class MCPServerTransportMixin:
    ...
    def _track_spawned_children(self, new_pids: Set[int]) -> None:
        ...
        for _pid in new_pids:
            try:
                from mason_cli.process_identity import register_child
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _core._ensure_mcp_sdk():
            raise ImportError(f"MCP server '{self.name}' requires the 'mcp' Python SDK, but "
                              "it is not installed. Run `mason setup` to install MCP support, then retry.")
        ...
        new_pids: set = set()
        # Subprocess stderr goes to ~/.mason/logs/mcp-stderr.log so banners can't corrupt the TUI.
