...
# ladders, parked-server self-probes and repeated `mason mcp test` runs re-run the preflight
...
# The cache is also persisted under the Mason home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``mason_constants.get_mason_home()`` so the cache follows the
    active profile and is isolated across Mason homes. The cache directory
    is created on demand. Returns ``None`` when Mason home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "mason-agent-osv-check/1.0"},
        method="POST")
