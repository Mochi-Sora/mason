...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if _origin._MCP_DISCOVERY_LOCK_PATH is None:
            _origin._MCP_DISCOVERY_LOCK_PATH = str(get_mason_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local MASON_HOME override into ``coro`` (task-local on the MCP
