...
# prefix, the exact-name MASON_ allowlist, or (Windows) an OS-essential name.
# The broad "MASON_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (MASON_BASE_URL, MASON_KANBAN_DB, *_WEBHOOK).
# MASON_RPC_SOCKET / MASON_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. MASON_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Mason code loses the Kanban mutation guard
# while still inheriting MASON_HOME.
_MASON_CHILD_ALLOWED = frozenset({
    "MASON_HOME", "MASON_PROFILE", "MASON_CONFIG", "MASON_ENV", "MASON_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational MASON_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret MASON_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _MASON_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("MASON_"):
            _dropped_mason.append(k)
    if _dropped_mason:
        logger.debug(
            "execute_code: dropped %d non-allowlisted MASON_* var(s) from the "
            ...
            len(_dropped_mason), ", ".join(sorted(_dropped_mason)),
        )
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from mason_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["MASON_RPC_SOCKET"] = rpc_endpoint
    child_env["MASON_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; MASON_TIMEZONE is an internal setting.
    _tz_name = os.getenv("MASON_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("MASON_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (mason_tools.py) must always be importable even when project
    # mode changes CWD. Mason's root is added ONLY when the child runs in Mason's Python env —
    # exposing Mason's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Mason-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Mason-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Mason-
    ...
    _strip_mason_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_mason_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import mason_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Mason "
                    "environment; mason root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Mason environment.
...
def _uses_mason_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Mason's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the mason root."""
