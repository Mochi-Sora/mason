"""Mason-managed Camofox state helpers.

With managed persistence enabled, Mason sends a deterministic userId derived from the
...
def get_camofox_state_dir() -> Path:
    ...
    return get_mason_home() / "browser_auth" / "camofox"


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Stable Mason-managed Camofox identity: userId is profile-scoped, session key is
    ...
    return {"user_id": f"mason_{user_digest}", "session_key": f"task_{session_digest}"}
