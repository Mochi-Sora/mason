...
is ALWAYS host-side ``$MASON_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $MASON_HOME/cache/spillover as a Path (not created)."""
    from mason_constants import get_mason_home
    return get_mason_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $MASON_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/mason-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "MASON_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
