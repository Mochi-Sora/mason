...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._MASON_CORE_TOOLS``)
...
class ToolSearchConfig:
    ...
    @property
    def effective_defer_tools(self) -> Optional[frozenset]:
        # None = Mason default rule inside is_deferrable_tool_name (defer-all
        # but eager). NOT the curated set: None must stay distinguishable from
        # an explicit ``defer: []`` (everything eager).
        return self.defer_tools
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``mason_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_MASON_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_MASON_CORE_TOOLS`` so non-GUI clients never pay
...
# Mason cold start: tools ALWAYS injected as native schemas (never deferred).
# ``clarify`` stays ambient — A/B showed deferring it collapsed structured-clarify
# usage (18/18 -> 7/18): the ask-the-user affordance must be present, not a stub.
# Everything else (core, MCP, plugin) defers behind the bridge by default; the
# name:summary manifest + tool_describe/tool_call make them one turn away, and
# the session working set (tools/working_set.py) re-injects whatever gets used.
_EAGER_TOOL_NAMES = frozenset({"clarify"})

# Legacy curated set, kept for reference/tests: with defer=None the Mason rule
# above applies instead of this list. An explicit ``defer: [...]`` still works
# as a membership allowlist on top of the MCP/plugin auto-deferral below.
...
def is_deferrable_tool_name(name: str, defer_tools: Optional[frozenset] = None) -> bool:
    ...
    Mason default (``defer_tools`` None, i.e. user set no explicit ``defer`` list):
    EVERYTHING defers except the eager allowlist — new tools auto-defer the moment
    they register, no config edit needed. An explicit ``defer: [...]`` keeps the
    legacy semantics (membership allowlist; [] = everything eager) on top of the
    MCP/plugin auto-deferral. Bridge names never defer.
    """
    if name in BRIDGE_TOOL_NAMES:
        return False
    if defer_tools is None:
        return name not in _EAGER_TOOL_NAMES
    if name in defer_tools:
        return True
