#!/usr/bin/env python3
"""One parent-death supervisor per Mason process, shared by all stdio MCP servers.

Why this exists
---------------
When Mason dies without running its cleanup path (SIGKILL, OOM killer, a hard
crash), stdio MCP servers it spawned are reparented to init and keep running
forever.  macOS has no ``PR_SET_PDEATHSIG``, so something has to outlive Mason
and reap them.

This module is deliberately standard-library-only and must not import anything
from ``tools/``: it runs after Mason may already be dead, and pulling in
...
with a single supervisor per Mason process:

* **Death detection is a blocking read on a pipe.**  Mason holds the only write
  end.  When Mason dies -- by any means, including SIGKILL -- the write end
...
Mason never unregistered *is* the orphan set, so a clean Mason shutdown --
...
an unrelated process owned by the same user.  If Mason then dies ungracefully
...
Two things narrow the window.  Mason prunes registrations whose group has no
...
exposure already exists in Mason's own killpg-based orphan cleanup, which this
...
def _is_safe_target(pgid: int, *, own_pgid: int, parent_pgid: int) -> bool:
    ...
    Defensive only -- Mason already filters non-MCP children before it
    ...
    Mason process tree or the session.
    """
    if pgid <= 1:
        return False
...
def _serve(stream, *, own_pgid: int, parent_pgid: int) -> set[int]:
    ...
    stopped. Nothing in Mason can produce that today, but this process is the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Process group of the spawning Mason process; never signalled.",
    )
