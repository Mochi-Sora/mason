...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(mason_home).expanduser() if mason_home else Path.home() / ".mason") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Mason's live source checkout "
        ...
        "is pushed. To change this checkout, stop Mason, run the command externally, then restart "
        "Mason.")
