...
~/.mason/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from mason_constants import get_bundled_skills_dir, get_mason_home, get_optional_skills_dir
...
# retarget MASON_HOME after import, and frozen constants would resolve (and for
...
# once under the launch MASON_HOME and later scope requests to a different profile via
# set_mason_home_override(). See #65828.
_MASON_HOME_AT_IMPORT = MASON_HOME
...
def _mason_home() -> Path:
    return _live(MASON_HOME, _MASON_HOME_AT_IMPORT, get_mason_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _mason_home() / "skills")
...
# Written by `mason profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors mason_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # MASON_BUNDLED_SKILLS env first, then repo-relative
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if _dir_hash(candidate) != origin_hash:  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `mason skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `mason skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.mason/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``mason-agent``)."""
    essential_only = (_mason_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing MASON_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.mason/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_mason_home() / NO_BUNDLED_SKILLS_MARKER).exists()
