...
def _capture_origin() -> tuple[str, str, Any, Any]:
    ...
    clobbers the MASON_SESSION_ID ContextVar/os.environ with the subagent's id."""
    from tools.async_delegation import _current_origin_session_id
    _origin_wake_sid = _current_origin_session_id()
    _origin_ui_session_id = ""
    with _quiet(None):
        from gateway.session_context import get_session_env
        _origin_ui_session_id = get_session_env("MASON_UI_SESSION_ID", "")
...
_SYNC_FALLBACK_NOTES = {
    "no_async": (
        ...
        "one-shot runner such as `mason -z`, a cron job, a Kanban "
...
def _resolve_async_wake_sid(origin_wake_sid: str) -> Optional[str]:
    ...
    wake. Uses the origin captured BEFORE child construction — MASON_SESSION_ID here would be the subagent's internal
    id.
    """
    try:
...
def _resolve_async_session_key(parent_agent: Any, origin_ui_session_id: str) -> tuple[str, str]:
    ...
    platform conversation key (agent:main:...). The CLI has no bound approval contextvar and no MASON_SESSION_KEY, so
    ...
    with _quiet(None):
        ...
        origin_ui_session_id = get_session_env("MASON_UI_SESSION_ID", "") or origin_ui_session_id
