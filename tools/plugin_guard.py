#!/usr/bin/env python3
"""Plugin Guard — ``skills_guard`` engine applied to ``mason plugins install``/``update``.
...
# ``~/.mason/.env`` mention is how READMEs say where keys go (READING it still trips
# ``read_secrets_file``, critical); ``curl | sh`` in READMEs is caution, not a hard block.
SEVERITY_REMAP = {
    "binary_file": "high", "mason_env_access": "medium", "curl_pipe_shell": "high"}
