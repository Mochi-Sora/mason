...
~/.mason/config.yaml (stdio, Streamable HTTP or SSE), discovers their tools and registers them
into the mason tool registry. The ``mcp`` package is optional (no-op without it).
...
async def _preflight_stdio_command(server_name: str, command: str, args: list) -> tuple[str, list]:
    ...
    # nothing (~48 MB per server, measured). Mason already supervises the child (shared death
...
class MCPServerTask(MCPServerRunMixin, MCPServerTransportMixin, MCPServerHealthMixin):
    ...
    def __init__(self, name: str):
        ...
        # task that does not inherit MASON_SESSION_PLATFORM, so the callback replays this.
