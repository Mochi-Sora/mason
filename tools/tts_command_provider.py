...
def render_command_template(command_template: str, placeholders: Dict[str, str]) -> str:
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__MASON_CMD_PLACEHOLDER_{len(replacements)}__"
...
def run_command_provider(
    ...
    provider survives, a silently stalled one is killed. Child env is scrubbed of Mason secrets
    while propagating delegated-child lineage markers."""
    from agent.delegation_context import delegated_child_subprocess_env
    from tools.environments.local import mason_subprocess_env
    scrubbed = mason_subprocess_env(inherit_credentials=False)
