...
mason-owned copy, launch the real browser binary on it, and attach agent-browser.
...
def _real_profile_unsupported_reason(browser) -> Optional[str]:
    ...
    """
    from mason_cli.browser_connect import UNSUPPORTED_CHANNEL
    if browser is None:
...
def _real_profile_snapshot_error(err: str) -> str:
    ...
    from mason_cli.browser_connect import _PROFILE_LOCKED_PREFIX
    if err and err.startswith(_PROFILE_LOCKED_PREFIX):
        return (err[len(_PROFILE_LOCKED_PREFIX):] + " To close it (only after the user approves — it "
                "quits their browser and loses unsaved tabs), run: `mason browser close-profile`, then retry.")
    return f"{_RP}{err}"
...
def _real_profile_cdp() -> tuple:
    ...
    if not _cloud._use_real_profile():
        ...
        try:
            from mason_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    from mason_cli.browser_connect import (chromium_executable, detect_default_chromium,
    ...
    with _bt._real_profile_cdp_lock:
        ...
        # mason process) holds the user-data-dir open — that corrupts the databases.
