"""Wake-word ("Hey Mason") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey mason" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "sherpa", "phrase": "hey mason", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Legacy openWakeWord weights (tools/wakewords/hey_mason.*) — trained on the
# previous hotword, NOT true "hey mason" detectors. Kept for backward compat
# when provider=openwakeword is set explicitly; the sherpa default below is the
# genuine hey-mason path. Retrain true weights with scripts/train_wakeword.py.
_BUNDLED_MODEL_NAME = "hey_mason"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_mason", "hey mason", "mason"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_mason model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey mason"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from mason_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Mason backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `mason tools` "
...
# ── Process-wide singleton (mirrors mason_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_mason_root() / "runtime" / "wake-word.lock"
