...
Independent of local skill state (no ``~/.mason`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Mason speaks "
                        f"{WIRE_VERSION} — update Mason to sync with it")
