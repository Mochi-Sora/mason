...
def _emergency_cleanup_all_sessions():
    ...
    mason processes — every clean exit reaps accumulated orphans, not only
    ...
    # owned by other live mason processes.
    _best_effort("Orphan reap on exit", _reap_orphaned_browser_sessions)


@contextlib.contextmanager
def _session_owner_scope(task_id: str):
    """Run under the Mason home + secret scope owning ``task_id``'s session (no-op if unrecorded).
    ...
    home_token = set_mason_home_override(owner_home)
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record this mason PID in ``<socket_dir>/<session>.owner_pid`` so the orphan
...
def _reap_socket_dir(socket_dir: str, session_name: str, tracked_names: set) -> bool:
    ...
    A live ``owner_pid`` means another mason process owns it — leave it UNLESS untracked
...
def _reap_orphaned_browser_sessions():
    """Kill agent-browser daemons whose owning mason process is gone (an unclean exit loses
    ...
    socket_dirs = []
    for prefix in ("agent-browser-h_*", "agent-browser-cdp_*", "agent-browser-mason_*"):
...
def _update_session_activity(task_id: str):
    """Touch the activity timestamp and record the owning Mason home on first sight (the
    ...
    with _bt._cleanup_lock:
        ...
        _bt._session_owner_homes.setdefault(task_id, str(get_mason_home()))
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        recordings_dir = get_mason_home() / "browser_recordings"
    except Exception as e:
