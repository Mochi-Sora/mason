...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_mason_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.mason/.env`` first, then ``os.environ``.

    Wraps :func:`mason_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
        from mason_cli.config import get_env_value as _mason_get_env_value
    except ImportError:
        return os.environ.get(name, default)
    value = _mason_get_env_value(name)
    return value if value is not None else default


def mason_xai_user_agent() -> str:
    """Return a stable Mason-specific User-Agent for xAI HTTP calls."""
    try:
        from mason_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Mason-Agent/{__version__}"


def mason_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": mason_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Mason home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from mason_constants import get_mason_home
        marker_dir = get_mason_home() / "state"
...
def _xai_base_url_override() -> str:
    """``MASON_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    return str(get_env_value("MASON_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Mason-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.mason/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``MASON_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Mason-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``mason_cli.config.get_env_value`` so keys stored in ``~/.mason/.env`` (the standard Mason
    ...
    scoping is identical to the fallback branch, and the base URL honors ``MASON_XAI_BASE_URL`` /
