...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("MASON_SESSION_PLATFORM")
    origin_chat_id = get_session_env("MASON_SESSION_CHAT_ID")
    if not (origin_platform and origin_chat_id):
        return None
    thread_id = get_session_env("MASON_SESSION_THREAD_ID") or None
    ...
    if thread_id and origin_platform == "slack":
        message_id = get_session_env("MASON_SESSION_MESSAGE_ID") or None
    ...
    return {
        ...
        "chat_name": get_session_env("MASON_SESSION_CHAT_NAME") or None, "thread_id": thread_id,
        # Lets a delivery mirror resolve the participant's session in per-user-isolated groups.
        "user_id": get_session_env("MASON_SESSION_USER_ID") or None,
        ...
        "scope_id": get_session_env("MASON_SESSION_SCOPE_ID") or None,
    }
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    TUI/CLI sessions cannot be captured as a cron ``origin`` (no ``MASON_SESSION_PLATFORM``/``CHAT_ID`` is
...
def _validate_bot_chat_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        from mason_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (mason profile list), or omit the "
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    from MASON_CRON_AUTO_DELIVER_*, or ``local`` when it has no concrete target; other
    elements pass through. Otherwise the scheduler would guess a home channel."""
    from gateway.session_context import get_session_env
    from utils import is_truthy_value
    if not is_truthy_value(get_session_env("MASON_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("MASON_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("MASON_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("MASON_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    """Scripts must be relative paths within MASON_HOME/scripts/ (absolute / ~ / drive-letter
    ...
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.mason/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.mason/scripts/ and use just the filename.")

    from tools.path_security import validate_within_dir
    scripts_dir = get_mason_home() / "scripts"
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``mason_cli.cron._builtin_gateway_liveness`` (#87033) so the
    ...
    """
    try:
        from mason_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        ...
        return {
            ...
            "warning": (
                f"The Mason gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(mason gateway install / mason gateway start). "
