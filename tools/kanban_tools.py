...
Registered only under the dispatcher (``MASON_KANBAN_TASK`` set) or when the profile
enables the ``kanban`` toolset. Tools rather than ``mason kanban`` shell-outs: they run
...
from mason_cli.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever MASON_KANBAN_* is present but not ours."""
...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    (MASON_KANBAN_TASK) per flag; else the profile toolset decides."""
    if _is_delegated_child_context():
        return False
    if os.environ.get("MASON_KANBAN_TASK") and _is_dispatcher_owned_worker():
...
def _reject_delegated_child_mutation(tool_name: str) -> None:
    """A delegate_task child shares the parent's process, so inherited MASON_KANBAN_*
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if _is_delegated_child_context() or not _is_dispatcher_owned_worker():
        return None
    return os.environ.get("MASON_KANBAN_TASK") or None


def _require_task_id(args: dict) -> str:
    tid = _default_task_id(args.get("task_id"))
    _check(tid, "task_id is required (or set MASON_KANBAN_TASK in the env)")
    return tid


def _own_task_env(task_id: str, var: str) -> Optional[str]:
    """``$var`` only when this worker is scoped to ``task_id``; else None."""
    return os.environ.get(var) if os.environ.get("MASON_KANBAN_TASK") == task_id else None


def _worker_run_id(task_id: str) -> Optional[int]:
    """This worker's dispatcher run id when it is scoped to task_id."""
    raw = _own_task_env(task_id, "MASON_KANBAN_RUN_ID")
    try:
...
def _stamp_worker_session_metadata(task_id: str, metadata: Optional[dict]) -> Optional[dict]:
    ...
    session_id = _own_task_env(task_id, "MASON_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> None:
    """A dispatcher-spawned worker may only mutate its own MASON_KANBAN_TASK; a
    ...
    """
    env_tid = os.environ.get("MASON_KANBAN_TASK")
...
# worker context (no ``MASON_KANBAN_TASK``). - No durable note on these auto-heartbeats; that's reserved
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    attempted. ``MASON_KANBAN_RUN_ID`` pins the run row so a reclaimed stale run is not
    heartbeated; ``MASON_KANBAN_CLAIM_LOCK`` absent -> default claimer (local workers)."""
    global _auto_heartbeat_last_attempt
    tid = os.environ.get("MASON_KANBAN_TASK")
    now = time.monotonic()
    ...
    try:
        ...
        with _board(None, quiet_close=True) as (kb, conn):
            ops = ((kb.heartbeat_claim, {"claimer": os.environ.get("MASON_KANBAN_CLAIM_LOCK")}),
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    steer was injected; never raises. Own comments (``MASON_PROFILE``) are skipped."""
    ...
    own = (os.environ.get("MASON_PROFILE") or "").strip()
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    from mason_cli import kanban_db_dispatch as kbd
    with _board(args.get("board")) as (kb, conn):
        # The dispatcher pins MASON_KANBAN_CLAIM_LOCK at spawn; the default
        # claimer covers locally-driven workers that bypassed the dispatcher.
        kb.heartbeat_claim(conn, tid, claimer=os.environ.get("MASON_KANBAN_CLAIM_LOCK"))
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # forge a directive from ``mason-system``. Cross-task commenting stays unrestricted —
    ...
    # comment from an authoritative-looking name like ``mason-system`` and poison the future-worker context
    # with what reads as a system directive. See #19713.
    author = os.environ.get("MASON_PROFILE") or "worker"
    with _board(args.get("board")) as (kb, conn):
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        chunks: list[bytes] = []
        total = 0
        with httpx.stream("GET", current_url, headers={"User-Agent": "mason-kanban/attach"},
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # Prefer the request-scoped api_server origin binding over MASON_SESSION_ID: the env
    ...
    session_id = (args.get("session_id") or _current_origin_session_id()
                  or os.environ.get("MASON_SESSION_ID"))
    ...
    with _board(args.get("board")) as (kb, conn):
        if project_id is None and workspace_kind is None and workspace_path is None:
            self_tid = os.environ.get("MASON_KANBAN_TASK")
        ...
        new_tid = kb.create_task(
            ...
            parents=tuple(parents), tenant=args.get("tenant") or os.environ.get("MASON_TENANT"),
            ...
            created_by=os.environ.get("MASON_PROFILE") or "worker", session_id=session_id)
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    Gateway sessions: ``MASON_SESSION_PLATFORM``/``CHAT_ID`` ContextVars. TUI/desktop:
    those are cleared but the subprocess inherits ``MASON_SESSION_KEY`` -> ``platform="tui"``
    for the TUI poller. ``MASON_SESSION_ID`` is deliberately NOT a fallback: it is set for
    ...
    platform, chat_id = env("MASON_SESSION_PLATFORM", ""), env("MASON_SESSION_CHAT_ID", "")
    if not platform or not chat_id:
        session_key = env("MASON_SESSION_KEY", "") or os.environ.get("MASON_SESSION_KEY", "")
        if not session_key:
            return None
        platform, chat_id = "tui", session_key
    chat_type = env("MASON_SESSION_CHAT_TYPE", "") or None
    thread_id = env("MASON_SESSION_THREAD_ID", "") or None
    message_id = env("MASON_SESSION_MESSAGE_ID", "") or ""
    notifier_profile = env("MASON_SESSION_PROFILE", "") or os.environ.get("MASON_PROFILE")
    ...
    return dict(
        ...
        user_id=env("MASON_SESSION_USER_ID", "") or None,
        user_id_alt=env("MASON_SESSION_USER_ID_ALT", "") or None,
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        from mason_cli import kanban_db_notify as _kbn
