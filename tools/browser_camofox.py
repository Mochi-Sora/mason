...
``~/.mason/.env`` (Docker: see ``CAMOFOX_REWRITE_LOOPBACK_URLS`` below).
"""
...
from mason_cli.config import cfg_get, load_config, read_raw_config
...
def _config_cdp_url() -> str:
    ...
    try:
        from mason_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    share a user ID so Mason uses the same profile), or None."""
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity is None:
            identity = {"user_id": f"mason_{uuid.uuid4().hex[:10]}", "session_key": f"task_{task_id[:16]}"}
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$MASON_HOME/browser_screenshots`` and return the path."""
    from mason_constants import get_mason_home
    screenshots_dir = get_mason_home() / "browser_screenshots"
