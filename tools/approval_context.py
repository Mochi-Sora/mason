...
# Mason session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["MASON_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_mason_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("mason_interactive", None)


def set_mason_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context instead of mutating os.environ."""
    return _mason_interactive_ctx.set("1" if interactive else "")


def reset_mason_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_mason_interactive_context`."""
    _mason_interactive_ctx.reset(token)


def _is_interactive_cli() -> bool:
    """True for an interactive CLI/ACP session (contextvar first, env fallback)."""
    ctx_val = _mason_interactive_ctx.get()
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("MASON_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("MASON_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    return _session_env("MASON_SESSION_PLATFORM")


def _is_cron_approval_context() -> bool:
    """True when the current approval decision is running inside cron."""
    return is_truthy_value(_session_env("MASON_CRON_SESSION"))
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``MASON_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``mason chat -q`` exports
    ``MASON_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("MASON_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set MASON_GATEWAY_SESSION; concurrent paths bind
    MASON_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("MASON_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``MASON_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
