...
# Sensitive write targets, matched via ~ / $HOME / $MASON_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once MASON_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_MASON_ENV_PATH = (
    r'(?:~\/\.mason/|(?:\$home|\$\{home\})/\.mason/|(?:\$mason_home|\$\{mason_home\})/)' r'\.env\b'
)
# ~/.mason/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_MASON_CONFIG_PATH = (
    r'(?:~\/\.mason/|(?:\$home|\$\{home\})/\.mason/|(?:\$mason_home|\$\{mason_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_MASON_ENV_PATH}|{_MASON_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]mason[^\n]*\.env\b', "access to Mason secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # between `mason` and `gateway` (`mason -p ade gateway restart`) are allowed so a profile flag can't slip past.
    (r'\bmason\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart mason gateway (kills running agents)"),
    (r'\bmason\s+update\b', "mason update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart mason-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart mason-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(mason|gateway|cli\.py)\b', "kill mason/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.mason.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.mason...'; do
    # launchctl bootout "$label"`) never has "mason" after the verb, and that slipped past and restarted 4 gateways
    # with zero approval. Erring broad is correct for an approval gate: an extra prompt is cheap.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:mason|ai\.mason)\b)', "stop/restart mason launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Mason file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.mason/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Mason config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Mason-managed security file (~/.mason/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_MASON_CONFIG_PATH}|{_MASON_ENV_PATH})', "in-place edit of Mason config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_MASON_CONFIG_PATH}|{_MASON_ENV_PATH})', "in-place edit of Mason config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_MASON_CONFIG_PATH}|{_MASON_ENV_PATH})', "in-place edit of Mason config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Mason home prefixes to ~/ and ~/.mason/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/MASON_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Mason home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_mason_home(command)
...
def _rewrite_resolved_mason_home(command: str) -> str:
    """Resolved MASON_HOME (and its realpath) -> ``~/.mason/`` so the _MASON_CONFIG_PATH /
    _MASON_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.mason")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart mason gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Mason ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"mason-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.mason.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a mason-gateway
