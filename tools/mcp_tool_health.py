...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Mason logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Mason' logging
        (agent.log via mason_logging), tagged with the server name. Without this, the SDK's default
