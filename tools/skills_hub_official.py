"""Skills Hub official sources: repo-shipped optional skills and the centralized Mason index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _mason_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.mason/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "NousResearch/mason-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.mason.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``mason update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.mason.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        mason_meta = meta_block.get("mason") if isinstance(meta_block, dict) else None
        upstream = mason_meta.get("upstream") if isinstance(mason_meta, dict) else None
...
class MasonIndexSource(SkillSource):
    """Skill source backed by the centralized Mason Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_mason_index
            self._index, self._loaded = _load_mason_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "mason-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "mason-index"), identifier=entry.get("identifier", ""),
