...
# match mason_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _runtime_provider_credentials(v: dict, explicit_request_overrides) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'mason auth'."
        )
...
def _load_config() -> dict:
    ...
    (follows MASON_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``MASON_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("MASON_IGNORE_USER_CONFIG") != "1":
        try:
...
def _resolve_child_runtime(
    ...
    # would pin a child Mason/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
