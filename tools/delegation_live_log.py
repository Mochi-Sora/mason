...
One append-only log per child under ``<mason_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.mason)."""
    from mason_constants import get_mason_dir
    return get_mason_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        with _best_effort(f"init ({delegation_id} task {task_index})"):
            ...
            path.write_text(
                "=== Mason subagent live transcript ===\n"
