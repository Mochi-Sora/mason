"""Apply a layout preset in the Mason desktop GUI (``layout.apply`` via ``desktop_ui``).
...
def apply_layout_tool(preset: str) -> str:
    ...
    return desktop_ui.emit_or_error(
        ...
        "Layout apply is only available in the Mason desktop app.", {"success": True, "preset": name},
    )
...
APPLY_LAYOUT_SCHEMA = {
    "name": "apply_layout",
    "description": (
        "Apply a saved layout preset to the Mason desktop app when the user "
