...
"""Auto-generated Mason REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["MASON_KERNEL_DIR"]
...
class RemoteKernel:
    ...
    # poll loop (same guard as tools.code_kernel, mason-agent#101861).
    attached: int = 0
...
def _kernel_key(owner: str, env_type: str, task_env_id: str, sandbox_tools: frozenset) -> Tuple:
    """The mason_tools stub module is generated from ``sandbox_tools`` once, at spawn, so a kernel
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    from tools.code_execution_tool import (
        MAX_STDOUT_BYTES, _ship_file_to_remote, _env_temp_dir, generate_mason_tools_module,
    )
    kernel_dir = f"{_env_temp_dir(env)}/mason_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/mason_tools.py",
                             generate_mason_tools_module(list(sandbox_tools), transport="file"))
        env_prefix = (f"MASON_KERNEL_DIR={q_dir} MASON_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
                      f"MASON_RPC_TOKEN={shlex.quote(rpc_token)} PYTHONDONTWRITEBYTECODE=1 PYTHONPATH={q_dir}")
