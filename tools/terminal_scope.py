...
_terminal_scope_var: ContextVar = ContextVar("mason_terminal_scope", default=None)
...
def build_profile_terminal_scope(mason_home: "Any") -> Dict[str, str]:
    ...
    """
    from mason_cli.config import TERMINAL_CONFIG_ENV_MAP, _terminal_env_value
    from mason_cli.config_defaults import DEFAULT_CONFIG

    home = Path(mason_home)
    scope: Dict[str, str] = {}
    ...
    if config_exists:
        from mason_cli.config import fast_safe_load
...
def install_profile_terminal_scope(mason_home: "Any") -> Token:
    """Build AND install a profile's policy; on failure install the refusal scope. Never raises."""
    try:
        return set_terminal_scope(build_profile_terminal_scope(mason_home))
...
@contextmanager
def install_and_reset_profile_terminal_scope(mason_home: "Any") -> Iterator[None]:
    """Install the profile's terminal policy for a bounded turn/fire. Never raises."""
    token = install_profile_terminal_scope(mason_home)
    try:
        yield
    finally:
