...
NOT_REGULAR_SENTINEL = "__mason_not_regular__"
...
MISSING_SENTINEL = "__mason_missing__"

_READ_SENTINEL_PREFIX = "__MASON_RF_"
_WRITE_SENTINEL_PREFIX = "__MASON_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and Mason disables MSYS argument
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        tmpl = self._escape_shell_arg(".mason-tmp.XXXXXX")
        script = (
            "set -e; "
            ...
            '|| mktemp "$d/.mason-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.mason-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('MASON_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``MASON_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("MASON_NATIVE_FILE_READ", "1").strip().lower()
