...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """User modification ``mason update`` keeps: a recorded origin hash (un-baselined v1 entries
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return _fail("not_in_manifest", f"'{name}' is not a tracked bundled skill. Nothing to reset. "
                     f"(Hub-installed skills use `mason skills uninstall`.)")
    ...
    if not restore:
        action, message = "manifest_cleared", (f"Cleared manifest entry for '{name}'. Future `mason update` runs "
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Bundled skills ``mason update`` keeps because the user edited them (same test the sync
...
def diff_bundled_skill(name: str) -> dict:
    ...
    if (bundled_src := bundled_by_name.get(name)) is None:
        return _fail(False, f"'{name}' is not a tracked bundled skill (no stock version to "
                     f"diff against). Hub-installed skills use `mason skills inspect`.")
...
_OPT_OUT_MESSAGES = {  # (enabled, changed) -> message
    ...
    (False, True): "Opted back in. The next `mason update` (or `mason skills opt-in --sync`) will re-seed bundled skills.",
    (False, False): "Not opted out — no marker to remove."}


def set_bundled_skills_opt_out(enabled: bool) -> dict:
    """Toggle the .no-bundled-skills marker (on-disk half of ``mason skills opt-out`` / ``opt-in``;
    removing present skills is ``remove_pristine_bundled_skills``)."""
    ss = _ss()
    marker = ss._mason_home() / ss.NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            ss._mason_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding (`mason skills opt-out`).\n"
