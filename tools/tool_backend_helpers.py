...
def nous_tool_gateway_unavailable_message(capability: str = "the Nous Tool Gateway", *,
    ...
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `mason model` to refresh your "
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``mason_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``mason_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``mason auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``mason
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `mason tools` selection: ``"nous"`` (managed
...
# (mason_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via mason "
            f"tools), but {failure}. Run 'mason tools' to change it.")
