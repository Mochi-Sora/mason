...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    precedence with config-only gates (no network I/O) for ``/browser status`` / ``mason doctor``.
    """
    _bt = _origin()
    ...
    return True, "Browser Use mode: Mason spawns `lightpanda serve` per session"
    ...
    if not _install._chromium_installed():
        if _install._running_in_docker():
            hint = ("Chrome fallback requires Chromium, but it is missing. You're running in Docker — "
                    "pull the latest image: docker pull ghcr.io/nousresearch/mason-agent:latest")
        else:
