...
elsewhere the agent falls back to ``mason mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(server: str = "", action: str = "install", reason: str = "", callback: Optional[Callable] = None) -> str:
    ...
    if callback is None:
        ...
        return tool_error(
            "setup_mcp is only available in the Mason desktop app. Use the "
            "terminal instead: `mason mcp install <name>` for catalog entries, "
            "`mason mcp login <name>` for OAuth.")
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "unanswered, continue without it. Catalog names: `mason mcp "
