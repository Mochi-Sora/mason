"""Central registry for all mason-agent tools: each tool file calls ``registry.register()``
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_mason_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales, so results are cached ~30 s: env-var flips via ``mason tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "MASON_SESSION_ID",
    "MASON_BROWSER_CONTROL_PRINCIPAL",
    "MASON_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Mason-home override
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``mason tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved MASON_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("mason_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``mason_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``mason_plugins.pkg.handlers`` is still owned by the
                # ``mason_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``mason tools enable`` lands quickly."""
        result = []
