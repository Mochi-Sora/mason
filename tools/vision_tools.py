...
_VISION_DOWNLOAD_TIMEOUT = _read_vision_setting("MASON_VISION_DOWNLOAD_TIMEOUT", "download_timeout", float)
...
def _resolve_vision_cpu_workers() -> int:
    """MASON_VISION_MAX_CONCURRENCY → ``auxiliary.vision.max_concurrency`` → host cores (< 1 ignored)."""
    val = _read_vision_setting("MASON_VISION_MAX_CONCURRENCY", "max_concurrency", int, minimum=1)
...
async def _prepare_image(
    ...
    temp_dir = get_mason_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    is pre-formatted by the caller. Temp images live under $MASON_HOME/cache/vision/."""
...
async def _materialize_video(video_url: str, task_id: Optional[str], temp_paths: list) -> Path:
    ...
    if not _is_local_terminal_backend() and path_like:
        ...
        temp_dir = get_mason_dir("cache/video", "temp_video_files")
        temp_dir.mkdir(parents=True, exist_ok=True)
    ...
    if await _validate_image_url_async(video_url):
        ...
        if blocked:
            raise PermissionError(blocked["message"])
        path = get_mason_dir("cache/video", "temp_video_files") / f"temp_video_{uuid.uuid4()}.mp4"
        temp_paths.append(path)
