"""Shared ``OAuthClientProvider`` customizations for Mason MCP OAuth.
...
class MasonProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_mason_logger`` to keep their own logger name).
    ...
    _mason_logger: logging.Logger = logger
    ...
    def __init__(self, *args: Any, token_user_agent: str | None = None, **kwargs: Any):
        ...
        self._mason_token_user_agent = token_user_agent

    def _prepare_token_request(self, request):
        """Stamp the configured User-Agent onto a token/refresh request."""
        ua = getattr(self, "_mason_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return request

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``MasonTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if MasonTokenStorage._coerce_secret_auth_method(data):
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        if not (200 <= response.status_code < 300):
            self._mason_logger.warning("Token refresh failed: %s", response.status_code)
        ...
        try:
            ...
            self._mason_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
        await self._store_tokens(token_response)
        return True
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "MasonTokenStorage"]:
    ...
    return cfg, mo.MasonTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "MasonTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
