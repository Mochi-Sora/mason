...
def _is_mason_provider_credential(name: str) -> bool:
    """True if ``name`` is a Mason-managed provider credential per
    ``_MASON_PROVIDER_ENV_BLOCKLIST`` or a dynamic Mason-internal secret
    ...
    in the ``execute_code`` child); non-Mason keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _MASON_PROVIDER_ENV_BLOCKLIST, _is_mason_internal_secret)
        ...
        return True
    return _is_mason_internal_secret(name) or name in _MASON_PROVIDER_ENV_BLOCKLIST


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). Mason-managed provider credentials
    ...
    for name in _accepted((n.strip() for n in var_names), (
        "env passthrough: refusing to register Mason provider "
        "credential %r (blocked by _MASON_PROVIDER_ENV_BLOCKLIST). "
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Mason provider credentials; refused
    names are logged with *refusal_msg* (``%r`` = name)."""
    for name in names:
        if not name:
            continue
        if _is_mason_provider_credential(name):
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    try:
        ...
        result.update(_accepted((i.strip() for i in items if isinstance(i, str)), (
            "env passthrough: refusing to register Mason "
            "provider credential %r from config.yaml (blocked "
            "by _MASON_PROVIDER_ENV_BLOCKLIST). Operator "
