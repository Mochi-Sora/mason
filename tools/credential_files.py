...
from mason_constants import get_mason_dir, get_mason_home
...
def _contained_host_path(rel: str, mason_home: Path, abs_msg: str, traversal_msg: str) -> Optional[Path]:
    """Resolve *rel* under MASON_HOME, refusing absolute paths and escapes."""
    if os.path.isabs(rel):
        logger.warning(abs_msg, rel)
        return None
    host_path = mason_home / rel
    from tools.path_security import validate_within_dir  # resolves symlinks and ``..`` before checking

    if containment_error := validate_within_dir(host_path, mason_home):
        logger.warning(traversal_msg, rel, containment_error)
        return None
    return host_path.resolve()


def register_credential_file(relative_path: str, container_base: str = "/root/.mason") -> bool:
    """Register a MASON_HOME-relative credential file for mounting; True if it exists and was registered.

    Rejects absolute paths and traversal out of MASON_HOME. Containment alone is not
    enough: MASON_HOME holds the MASTER stores (``.env``, ``auth.json``, ``mcp-tokens/``),
    ...
    resolved = _contained_host_path(
        relative_path, get_mason_home(),
        "credential_files: rejected absolute path %r (must be relative to MASON_HOME)",
    ...
    if not resolved.is_file():
        ...
        return False
    # Master credential stores are never mountable, even though they sit inside MASON_HOME and therefore
...
def register_credential_files(entries: list, container_base: str = "/root/.mason") -> List[str]:
    ...
    try:
        ...
        for item in cred_files if isinstance(cred_files, list) else []:
            ...
            if resolved_path is not None and resolved_path.is_file():
                result.append(_mount(resolved_path, f"/root/.mason/{rel}"))
...
def _skill_dir_roots(container_base: str) -> Iterator[Tuple[Path, str]]:
    ...
    skills_dir = get_mason_home() / "skills"
...
def get_skills_directory_mount(container_base: str = "/root/.mason") -> list[Dict[str, str]]:
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = _safe_skills_tempdir = Path(tempfile.mkdtemp(prefix="mason-skills-safe-"))
...
def iter_skills_files(container_base: str = "/root/.mason") -> List[Dict[str, str]]:
...
# (new_subpath, old_name) pairs matching mason_constants.get_mason_dir().
...
def _cache_dir_roots(container_base: str, *, create_missing: bool) -> Iterator[Tuple[Path, str]]:
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_mason_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # get_mason_dir already picked new-vs-legacy, so this can't shadow a legacy dir.
            try:
                ...
                # bind-mounted dir costs nothing; a missing mount costs the feature. get_mason_dir()
...
def get_cache_directory_mounts(container_base: str = "/root/.mason") -> List[Dict[str, str]]:
    """Bind-mount entries for each cache directory (host layout via ``get_mason_dir``)."""
...
def map_cache_path_to_container(host_path: str, container_base: str = "/root/.mason") -> Optional[str]:
...
def from_agent_visible_cache_path(container_path: str, container_base: str = "/root/.mason") -> str:
...
# Backends whose file-sync lands under the remote home: ``~/.mason`` is
...
def to_agent_visible_cache_path(host_path: str, container_base: str = "/root/.mason") -> str:
    ...
    ``/root/.mason``; ssh/daytona/vercel_sandbox under ``~/.mason``; plugin backends declare
    ...
    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at ``/root/.mason`` (the
    *container_base* default). * ssh / daytona / vercel_sandbox — file-synced under the remote user's home;
    ``~/.mason`` is shell-expanded by the remote shell, so tool commands resolve it regardless of the
...
def iter_cache_files(container_base: str = "/root/.mason") -> List[Dict[str, str]]:
