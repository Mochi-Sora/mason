...
def _dump_subagent_timeout_diagnostic(
    *, child: Any, task_index: int, timeout_seconds: float, duration_seconds: float,
    ...
    response", 0 API calls, nothing to inspect): ``~/.mason/logs/subagent-timeout-<sid>-<ts>.log`` with the
...
def _register_child(
    ...
    if owner_session_id is None:
        with _quiet(None):
            ...
            owner_session_id = get_session_env("MASON_UI_SESSION_ID", "") or None
