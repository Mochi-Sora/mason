...
# "No STT provider available". The single-env-var MASON_LOCAL_STT_COMMAND escape
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"mason-cmd-stt-{provider_name}-") as tmpdir:
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return _error_result(
        ...
        "provider registered that name. Run `mason plugins list` to see "
...
def _apply_pre_transcription_hook(
    *, file_path: str, provider: str, model: Optional[str], language: Optional[str],
    ...
    try:
        from mason_cli.plugins import has_hook, invoke_hook
