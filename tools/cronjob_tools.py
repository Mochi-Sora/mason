...
# `cronjob(action="run")` executes in-process (comfortably below MASON_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default MASON_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with MASON_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (MASON_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with MASON_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `mason cron run` that died mid-run). The ticker does this at startup; one-shot
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `mason cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    # completion, so don't claim-and-dispatch (direct callers like `mason cron run` exit right after).
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("MASON_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_mason_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("MASON_INTERACTIVE")
        or env_var_enabled("MASON_GATEWAY_SESSION")
        or env_var_enabled("MASON_EXEC_ASK")
    )


# Agent-facing arguments forwarded verbatim to cronjob(). model / provider / base_url are
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `mason cron
