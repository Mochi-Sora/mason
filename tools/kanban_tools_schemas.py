...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to MASON_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: MASON_KANBAN_DB env → "
    "MASON_KANBAN_BOARD env → the 'current' symlink under the kanban "
...
KANBAN_ATTACH_URL_SCHEMA = _schema(
    "kanban_attach_url",
    (
        "Attach a file to a task by URL — Mason downloads it server-side "
...
KANBAN_CREATE_SCHEMA = _schema(
    ...
    ),
    {
        ...
        "tenant": _prop("string", (
                ...
                "Defaults to MASON_TENANT env if set."
        )),
