...
MASON_HOME = get_mason_home()  # all skills live in ~/.mason/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped MASON_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_mason_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from MASON_HOME/.env."""
    env_path = get_mason_home() / ".env"
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.mason/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``MASON_PLATFORM``, session
    ``MASON_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("MASON_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("MASON_SESSION_PLATFORM") or ""
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `mason sync propose` (or automatically, if your org enables it).\n\n")
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`mason skills untrust`."), None, None
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.mason/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `mason skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.mason.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        mason_meta = (metadata.get("mason", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(mason_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
