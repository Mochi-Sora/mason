...
``metadata.mason.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.mason.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.mason.blueprint`` is a mapping with a
    ...
    mason = meta.get("mason") if isinstance(meta, dict) else None
    blueprint = mason.get("blueprint") if isinstance(mason, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.mason.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.mason.blueprint`` block) ready for ``mason skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"mason": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
