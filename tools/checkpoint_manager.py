...
config / ``--checkpoints`` flag.  One store under ``~/.mason/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/mason/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
CHECKPOINT_BASE = get_mason_home() / "checkpoints"

_STORE_DIRNAME, _INDEXES_DIRNAME, _PROJECTS_DIRNAME, _LEDGERS_DIRNAME = "store", "indexes", "projects", "ledgers"
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/mason", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (Mason convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("MASON_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "mason@local"), ("user.name", "Mason Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell Mason' writes from later user edits."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    users may want to recover; the archive falls under retention and ``mason checkpoints
    ...
    logger.info("Migrated pre-v2 checkpoint repos to %s. "
                "Clear with `mason checkpoints clear-legacy` when safe.", legacy_root)
    return legacy_root
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "MASON_WORKDIR").exists():
                workdir = (child / "MASON_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Mason just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what Mason
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        Mason.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, (line.strip() for line in names_out.splitlines())):
            ...
            mason_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if mason_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after Mason' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is Mason-created (delete to restore) — unless ``max_file_size_mb`` kept
