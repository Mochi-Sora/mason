...
For custom/local-only tools do NOT edit core: create `~/.mason/plugins/<name>/plugin.yaml` +
...
1. `tools/your_tool.py`:
   ...
   ```
2. `toolsets.py`: add the name to `_MASON_CORE_TOOLS` (all platforms) or a new toolset. **Required**
   — discovery registers the schema, but a tool is only exposed if a toolset names it.
   `_MASON_CORE_TOOLS` is the default bundle every platform's base toolset inherits, not dead code.
...
- **Paths in schema descriptions use `display_mason_home()`** (schema is built at import, after
  `_apply_profile_override()` set `MASON_HOME`). **State files use `get_mason_home()`**, never
  `Path.home()/.mason`, so each profile gets its own state.
...
- New tools integrate with existing setup UX (`mason tools`, `mason setup`, auto-install) rather
  than a raw env var; secrets go in `OPTIONAL_ENV_VARS` (`mason_cli/AGENTS.md`).
...
`mason tools` (curses) or `tools.<platform>.enabled/disabled` in config.yaml. `browser_exec`
...
imports against a temp `MASON_HOME` (see `tests/tools/test_approval_config_readonly.py`).
