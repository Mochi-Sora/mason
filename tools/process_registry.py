...
CHECKPOINT_PATH = get_mason_home() / "processes.json"
        ...
        if _IS_LINUX:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"mason-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Whether this process is the live, supervised Mason gateway itself.
    Supervisor markers and ``_MASON_GATEWAY`` are inherited by every descendant (and
    ...
    if os.environ.get("_MASON_GATEWAY") != "1":
        return False
    try:
...
def _build_systemd_scope_argv(shell_argv: List[str], unit_suffix: str) -> List[str]:
    ...
    if binary is None:
        ...
        return shell_argv
    return _systemd_scope_argv(binary, f"mason-worker-{unit_suffix}", *shell_argv)
...
class ProcessRegistry:
    ...
    def _config_value(section: str, key: str, fallback):
        ...
        from mason_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config
    ...
    def _scope_argv(self, session: ProcessSession, safe_command: str, unit_suffix: str, label: str) -> List[str]:
        ...
        if in_supervised_gateway and _systemd_run_user_scope_available():
            session.systemd_unit = f"mason-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path, pid_path, exit_path = (f"{temp_dir}/mason_bg_{session.id}.{ext}" for ext in ("log", "pid", "exit"))
        q = shlex.quote
        bg_command = (
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        """Whether a goal loop (``mason_cli.goals`` wait barrier) should stay parked on
    ...
    def wait_for_pending_completions(
        ...
        A one-shot CLI run (``mason -q/-Q/-z``) exits when its turn ends; a background
        ...
        Bot Mode handoff REPLIES are the visible casualty (#90879): a recipient invoked as ``mason -p <bot>
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        descendant (e.g. a daemon from ``mason update``) holds the pipe open, poll()
        ...
        process (e.g. a daemon spawned by `mason update` restarting the gateway) is still holding the
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if self.on_close is None:
            return {"status": "error", "error": "close_terminal is only available in the Mason desktop app."}
        ...
        try:
            with self._lock:
                ...
                for s in self._running.values():
                    ...
                    # Redact inline credentials before persisting (~/.mason/processes.json).
