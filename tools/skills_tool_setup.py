...
def _capture_required_environment_variables(
    ...
    # hint. Interactive gateway surfaces (desktop app / TUI) set MASON_INTERACTIVE (same flag
    # tools/approval.py uses) and register a callback routing to a secure secret.request overlay.
    if _is_gateway_surface() and not env_var_enabled("MASON_INTERACTIVE"):
        try:
            ...
            hint = (f"Secure secret entry is not available. Load this skill in the local CLI to be "
                    f"prompted, or add the key to {display_mason_home()}/.env manually.")
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("MASON_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("MASON_SESSION_PLATFORM"))
