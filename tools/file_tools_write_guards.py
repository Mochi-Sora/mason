...
_mason_config_resolved: str | None = None
_mason_config_resolved_loaded = False
_real_mason_home_cached: str | None = None
_real_mason_home_loaded = False
...
def _mason_home_real() -> str:
    from mason_constants import get_mason_home
    return os.path.realpath(str(get_mason_home()))


def _get_mason_config_resolved() -> str | None:
    """Return the resolved absolute path of the Mason config file (cached)."""
    return _cached_lookup("_mason_config_resolved", "_mason_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.mason/config.yaml")).resolve()))


def _get_real_mason_home() -> str | None:
    """Return the realpath of the authoritative Mason home (cached)."""
    return _cached_lookup("_real_mason_home_cached", "_real_mason_home_loaded", _mason_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.mason")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    mason_config = _get_mason_config_resolved()
    if mason_config and mason_config in candidates:
        return (
            f"Refusing to write to Mason config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.mason/config.yaml directly or use 'mason config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .mason tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from mason_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.mason itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.mason`` component rule, which would match the home.
    real_home = _get_real_mason_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .mason config dirs (<repo>/.mason/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.mason.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".mason":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Mason mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.mason" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Mason state (a write the host never reads). Not profile isolation — that guard was
