...
``~/.mason/skills/.curator_ledger.jsonl`` with before/after file manifests whose
contents are stored content-addressed (sha256-deduped) under
``~/.mason/.curator_backups/blobs/``. JSONL, not the state DB: durable, greppable,
...
def blobs_dir() -> Path:
    return get_mason_home() / ".curator_backups" / "blobs"
...
def fill_snapshot_from_curator_backup(
    ...
    for rel, data in extra.items():
        ...
        if not _is_within(skills, dest) or not _is_within(get_mason_home(), dest):
            continue
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """Every entry path must be under MASON_HOME — a hand-edited ledger must not
