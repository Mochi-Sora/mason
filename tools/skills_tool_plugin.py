...
def _serve_plugin_skill(
    ...
    from mason_cli.plugins import _get_disabled_plugins, get_plugin_manager
    from tools import skills_tool as _st
    if namespace in _get_disabled_plugins():
        return _fail(f"Plugin '{namespace}' is disabled. Re-enable with: mason plugins enable {namespace}")
