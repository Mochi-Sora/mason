...
GitHub releases to $MASON_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_mason_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _mason_bin_dir() -> str:
    """$MASON_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_mason_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MASON_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_mason_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $MASON_HOME/bin."""
    mason_bin = os.path.join(_mason_bin_dir(), "tirith")
    return shutil.which("tirith") or (mason_bin if _is_executable(mason_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $MASON_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
