...
def get_env_value(name, default=None):
    ...
    try:
        from mason_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    ...
    return str(get_mason_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    MASON_HOME and later scope requests to a different profile via
    ``mason_constants.set_mason_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped MASON_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("mason_cli.config not available, using default TTS config")
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `mason setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run mason setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'mason setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'mason tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("MASON_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_mason_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'mason_xai_user_agent': ('tools.xai_http', 'mason_xai_user_agent'),
