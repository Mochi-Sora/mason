...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Mason (the agent's own "
    ...
    if _is_pinned(name, "pinned-guard"):
        return (
            ...
            f"run `mason curator unpin {name}` if they want to delete it. Patches and edits "
...
def _background_review_write_guard(
    ...
    if _is_pinned(name, "pinned skill guard"):
        return _refusal(
            ...
            f"are off-limits to autonomous maintenance. Ask the user to run `mason curator "
    ...
    try:
        ...
        # closed for both shapes; `mason curator adopt <name>` is the supported way in. See #67140.
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return _refusal(
                ...
                f"curation. Run `mason curator adopt {name}` to opt it in.")
    except Exception:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `mason sync propose {name}` to share it back.")
        ...
        return (
            ...
            f"right now — run `mason sync propose {name}` to retry.")
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    via `mason sync propose`. Deletion stays refused: the mirror is a view of org HEAD, so a
    ...
    try:
        if _is_org_mirror(skill_path):
            return _refusal(
                ...
                f"back with `mason sync propose {name}`.)")
    except Exception:
