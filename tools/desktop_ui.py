#!/usr/bin/env python3
"""Bridge desktop-only tools to Mason-desktop renderer events.

The desktop ``tui_gateway`` installs an emitter via :func:`set_emitter`; elsewhere it
stays ``None`` and tools report "desktop only". Routing keys off ``MASON_UI_SESSION_ID``
...
def emit(event: str, payload: dict) -> bool:
    ...
    if _emit is None:
        return False
    _emit(get_session_env("MASON_UI_SESSION_ID", ""), event, payload)
    return True
