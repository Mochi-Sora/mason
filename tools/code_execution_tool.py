...
The LLM writes a Python script that calls Mason tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Mason does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_mason_dir("cache/exec", "exec_spill")
...
def _missing_mason_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
...
# these as the top execute_code failure classes: mason_tools import misuse, importing the built-in
# helpers, treating tool results as strings, importing third-party packages absent from the sandbox.
_FAILURE_HINT_RULES = (
    (r"cannot import name '(\w+)' from 'mason_tools'", _missing_mason_tools_import_hint),
...
def generate_mason_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the mason_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated Mason tools RPC stubs."""
...
def _connect():
    ...
    MASON_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["MASON_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("MASON_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (MASON_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("MASON_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Mason tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("MASON_RPC_DIR") or os.path.join(tempfile.gettempdir(), "mason_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage mason_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/mason_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/mason_tools.py",
                             generate_mason_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"MASON_RPC_DIR={quoted_rpc_dir} MASON_RPC_TOKEN={shlex.quote(rpc_token)} "
                      "PYTHONDONTWRITEBYTECODE=1")
        tz = os.getenv("MASON_TIMEZONE", "").strip()
        if tz:
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (mason-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Mason tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.mason.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.mason/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Mason's own python (the common case — stdlib plus Mason's "
    ...
    description = (
        "Run Python that calls Mason tools programmatically. Use when you "
        ...
        f"Available via `from mason_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from mason_tools import {import_str}` "
