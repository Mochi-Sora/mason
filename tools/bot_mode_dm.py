...
local → ``mason -p <name> chat --in ~ -c "Bot Chat" --create-if-missing -Q
--query-file <tmp>``; peer → ``mason peer dm <peer>[/<name>] < <tmp>``; both via
...
# delivery runner (``python bot_mode_dm.py --run-delivery …``); Mason helpers import lazily.
...
_DM_DIR_NAME = "mason-dm"
_DM_STALE_SECONDS = 24 * 60 * 60

# '<peer>/<agent>' — peer names are lowercase (``mason peer`` normalizes them).
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'mason' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('mason' → 'default')."""
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    try:
        from tools.bot_mode_probe import (
            BOT_CHAT_TITLE, _handle, _mason_root, _peers, _profile_name as _self_profile_name, _roster,
            is_bot_mode_managed,
        )
    ...
    root, me = _mason_root(Path(home)), _self_profile_name(Path(home))
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        # Pin the registry-owning profile: `mason peer` resolves bot_peers via the profile-scoped
        ...
        return _start_delivery(["mason", "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery(["mason", "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
...
def cleanup_bot_dm_cache(max_age_hours: float = _DM_STALE_SECONDS / 3600, *, now: float | None = None) -> int:
    ...
    locations = [(temp_root, "mason-dm-*.txt"), (temp_root, "mason-relay-dm-*.txt")]
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    if stdin_file or len(argv) < 3 or cli not in ("mason", "mason.exe") or argv[1] != "-p":
        return contextlib.nullcontext()
    from tools.bot_mode_probe import _mason_root
    from tools.bot_relay import acquire_turn_lock

    return acquire_turn_lock(_mason_root(Path(_default_home())), argv[2])
