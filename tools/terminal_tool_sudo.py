...
def _in_delegated_child_context() -> bool:
    ...
    threads and inherit process-wide ``MASON_INTERACTIVE=1``, which does NOT mean they can reach
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    is_gateway = env_var_enabled("MASON_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    try:
        os.environ["MASON_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        print("\n".join((
            "",
            "┌" + "─" * 58 + "┐",
        ...
        os.environ.pop("MASON_SPINNER_PAUSE", None)
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # delegate_task children inherit MASON_INTERACTIVE=1 (and possibly a stale thread-local
    ...
    should_prompt_for_sudo = (
        env_var_enabled("MASON_INTERACTIVE") or _get_sudo_password_callback() is not None
