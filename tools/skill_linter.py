...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    mason_meta = meta.get("mason") if isinstance(meta, dict) else None
    if not isinstance(mason_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.mason.{tags, related_skills}.")
    elif "tags" not in mason_meta:
        yield _warn("missing-metadata", "metadata.mason.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("mason", "agent", "mason agent") and (
        author != "Mason Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Mason Agent' (proper caps) "
