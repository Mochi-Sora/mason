"""yuanbao_tools.py - 元宝平台工具集 (the "mason-yuanbao" toolset).
...
async def send_sticker(args) -> dict:
    """向 chat_id（缺省取当前会话 MASON_SESSION_CHAT_ID）发送一张内置贴纸（TIMFaceElem）。
    ...
    target = (args.get("chat_id", "") or "").strip() or _session_env("MASON_SESSION_CHAT_ID")
    if not target:
...
async def send_dm(args) -> dict:
    ...
    group_code = args.get("group_code", "")
    if not group_code and (chat_id := _session_env("MASON_SESSION_CHAT_ID")).startswith("group:"):
...
def _check_yuanbao():
    ...
    return _session_env("MASON_SESSION_PLATFORM") == "yuanbao" or _get_active_adapter() is not None
...
for _schema, _handler, _emoji in _TOOLS:
    registry.register(
        name=_schema["name"], toolset="mason-yuanbao", schema=_schema, handler=_handler,
