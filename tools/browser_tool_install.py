...
from mason_constants import agent_browser_runnable, get_mason_home, is_termux as _is_termux_environment, node_tool_runnable
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx, extended (Mason-managed/Homebrew) PATH first.
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    local node_modules binary, which vanishes on the next ``mason update`` and leaves a dangling link ``which``
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    """Best-effort pre-fetch of the agent-browser npm package via npx (``mason update`` / ``doctor --fix``).
    ...
    registry-lookup/fetch cost. Calling this during ``mason update`` (or ``mason doctor --fix``) warms
