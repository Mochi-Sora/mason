...
When any profile carries ``ui_meta['mason-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient MASON_HOME (env, else ~/.mason) as a string."""
    return os.getenv("MASON_HOME") or os.path.expanduser("~/.mason")
...
def _mason_root(home: Path) -> Path:
    """Root ~/.mason for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @mason.
    return "mason" if name == "default" else name
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['mason-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("mason-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "mason-bots")) is not None
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    return _swallow(lambda: _any_managed(_mason_root(_resolve_home(home))), False)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``mason peer``) from config.yaml, read
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `mason peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _mason_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Mason profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient MASON_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_mason_home_override(str(resolved))
        try:
