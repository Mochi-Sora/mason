...
_MASON_CONFIG_FILES = r'\.mason/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.mason/\.env|\~/\.mason/\.env',
     "mason_env_access", "critical", "exfiltration", "directly references Mason secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Mason/other config (setup docs routinely say
    ...
    (_prose_modify_re(_MASON_CONFIG_FILES),
     "mason_config_mod", "high", "persistence", "modification language aimed at Mason configuration files (verify intent)"),
    (_shell_write_re(_MASON_CONFIG_FILES),
     "mason_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Mason configuration files"),
    (r'\.mason/config\.yaml|\.mason/SOUL\.md',
     "mason_config_ref", "low", "persistence", "references Mason configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Mason-native; `.clawhubignore` is honored for skills published through ClawHub.
