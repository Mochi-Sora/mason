...
broad, declarative). New skills land in ~/.mason/skills/ (or ``skills.create_dir``);
...
# All skills live in ~/.mason/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live MASON_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch MASON_HOME and later bind a different profile per session (#40677).
    """
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    if len(others) == 1:
        ...
        base += (
            ...
            f"it, switch profiles (`mason -p {other_profile}`) or edit the file directly "
            f"(file tools / terminal).")
    elif others:
        names = ", ".join(f"'{p}'" for p, _ in others)
        base += (
            f" Skills by that name exist in other profiles: {names}. Switch profiles (`mason -p "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match Mason skill standards.")
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`mason curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`mason curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # ~/.mason/skills/.archive/ is documented as the maximum destructive action the curator may take,
        # and `mason curator restore` promises the skill can be brought back. Route through the recoverable
