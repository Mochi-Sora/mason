...
tick ``maybe_pull_skills`` and ``mason sync``. Lives under tools/ so it never imports the CLI at
...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.mason/skills/ skills qualify."""
...
# Configuration -- env-first so Mason Cloud can enable sync via environment alone. Every knob:
# MASON_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
...
def resolve_sync_base_url() -> Optional[str]:
    """MASON_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
    (``/v1/sync/`` is appended by the client). None only if the default is blanked out."""
    env = os.getenv("MASON_SYNC_BASE_URL")
...
def sync_feature_enabled() -> bool:
    ...
    return _sync_config_bool("MASON_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """False (default): edits to an org skill stay LOCAL until ``mason sync propose``. True: every
    edit is proposed right away (an admin still approves unless the editor is one)."""
    return _sync_config_bool("MASON_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)


def sync_default_opt_in() -> bool:
    """False (default): opt-IN -- a skill syncs only after ``mason sync enable`` or a plane manifest
    opting it in. True: opt-OUT -- every eligible skill syncs unless disabled (Mason Cloud default)."""
    return _sync_config_bool("MASON_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.mason/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.mason/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.mason/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.mason/skills/.sync_device_id. An existing file always wins; else seeded
    from MASON_SYNC_DEVICE_NAME (first use only, for Mason Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("MASON_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.mason/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "mason skill sync") -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {"ok": False, "conflict": True, "conflict_ref": conflict_ref, "overlapping_skills": sorted(overlaps),
                "actual_head": actual_head, "message": (f"{len(overlaps)} skill(s) changed on both sides; wrote "
                                                        f"{conflict_ref}. Resolve out-of-band (mason sync / NAS UI).")}
...
def maybe_push_skills(*, message: str = "mason skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Snapshot for ``mason sync status``; never raises. ``org_available`` False = not in a shared org."""
