...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Mason tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``mason gateway restart|stop|uninstall``
    targeting mason-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_MASON_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (mason
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``mason gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "Use Mason cron for one-shot delayed work, or install an "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `mason gateway restart` from a "
