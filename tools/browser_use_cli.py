...
_PRIVATE_BROWSER_SENTINEL = "_MASON_BU_PRIVATE_BROWSER"
...
# BU_NAME + daemon pid).
_OWN_TAB_PREAMBLE = """\
# mason: pin this named session to its own tab (once per daemon process)
def _mason_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "mason-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_mason_ensure_own_tab()
del _mason_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # (Mason's venv) wins over its site-packages → wrong-ABI C-extensions and a crash.
    # PYTHONPATH/PYTHONHOME inherited from the agent process point at Mason's venv site-packages, and a
    ...
    # #84841, #86006, #86104). Strip both — the CLI manages its own environment and never needs Mason's
    # import path.
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_mason_home()) / "cache" / ".browser_use_default_notice"
        ...
        with contextlib.suppress(OSError):
            ...
            stamp.touch()
        return ("Browser Use CLI not found — using the built-in browser tools. Run `mason tools` "
...
def _managed_bin_dir() -> str:
    """$MASON_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_mason_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: Mason' own ``$MASON_HOME/bin``
    copy always wins so every session drives one Mason-controlled binary; PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$MASON_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_mason_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at a Mason-spawned ``lightpanda serve`` (``browser.engine: lightpanda`` and
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Lightpanda could not be started: {e} Set browser.engine to auto "
                   "to use local Chrome, or switch backends via `mason tools` → Browser Automation."),
...
def _resolve_managed_chromium_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at Mason' packaged Chromium, launched through agent-browser for this cache key —
    ...
    if not cdp:
        return (f"The local browser could not be started: {(res or {}).get('error') or 'agent-browser returned no CDP endpoint'} "
                "Run `mason tools` → Browser Automation to (re)install Chromium, or switch backends.")
    _set_cdp_env(env, cdp)
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    (4) the local engine — ``browser.engine: lightpanda`` or Mason' packaged Chromium via agent-browser
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Cloud browser provider {provider_name} failed to provide a session: {e}. "
                   "Fix the provider configuration or switch backends via `mason tools` → Browser Automation."),
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    profile, mason_cli.browser_connect) when consented. Two ways in: the effective backend is already local
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            ...
            "description": ("Drive the user's own local browser (a Mason-managed copy of their real "
