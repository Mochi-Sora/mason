#!/usr/bin/env python3
"""Reveal/focus a pane in the Mason desktop GUI (``pane.reveal`` via ``desktop_ui``).
...
def focus_pane_tool(pane: str) -> str:
    ...
    return desktop_ui.emit_or_error(
        ...
        "Pane focus is only available in the Mason desktop app.", {"success": True, "pane": name})
...
registry.register(
    ...
    schema={
        "name": "focus_pane",
        "description": (
            "Reveal and focus a Mason desktop pane when the user asks to see it: "
