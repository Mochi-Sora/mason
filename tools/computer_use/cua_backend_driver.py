...
# var would only LOOK like it pinned. Point MASON_CUA_DRIVER_CMD at a specific binary instead.
_CUA_DRIVER_CMD_ENV = "MASON_CUA_DRIVER_CMD"
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command to its DrvFS ``/mnt/<drive>/...`` form when Mason runs in WSL
    (a Windows cua-driver manifest can report ``C:\\...`` while Mason spawns via POSIX). Non-Windows paths and
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    """Candidate commands in resolution order. ``override`` / a non-empty ``MASON_CUA_DRIVER_CMD`` is authoritative
...
def cua_driver_install_hint() -> str:
    ...
    return ("cua-driver is not installed. Install with one of:\n  mason computer-use install\n"
            ...
            "Or run `mason tools` and enable the Computer Use toolset to install it automatically.")
...
def _resolve_mcp_invocation(driver_cmd: str, *, timeout: float = 6.0) -> Tuple[str, List[str]]:
    ...
    Surface 8 of NousResearch/mason-agent#47072: instead of hardcoding ``["mcp"]`` we ask the driver itself
    ...
    keeps working without a Mason patch.
...
def _manifest_contract_reason(manifest: Optional[Dict[str, Any]]) -> str:
    ...
    if tuple(int(part) for part in match.groups()) < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return "Mason computer use requires cua-driver 0.20.0 or newer"
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Mason' 0.20 integration."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (f"cua-driver {state.get('latest_version') or '?'} is available "
            f"(you have {state.get('current_version') or '?'}); update with `mason computer-use install --upgrade`.")
