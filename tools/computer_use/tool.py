...
# Hard-blocked regardless of approval level (e.g. logout kills the session Mason runs in). Alt is
...
# Per-Mason-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
# -> set of (action, delivery_mode) scope keys See NousResearch/mason-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Mason's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("MASON_COMPUTER_USE_BACKEND", "cua").lower()
    if backend_name in {"cua", "cua-driver", ""}:
        from tools.computer_use.cua_backend import CuaDriverBackend
        return CuaDriverBackend(permission_mode=permission_mode)
    if backend_name != "noop":
        raise RuntimeError(f"Unknown MASON_COMPUTER_USE_BACKEND={backend_name!r}")
...
def _shutdown_backend_atexit() -> None:
    ...
    the Mason process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (MASON_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `mason computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$MASON_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from mason_constants import get_mason_dir  # lazy so tests can patch get_mason_dir
    cache_dir = get_mason_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Mason' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$MASON_HOME/cache/vision/``) and merge
...
def check_computer_use_requirements() -> bool:
    """macOS/Windows/Linux + cua-driver binary (or env override). `mason computer-use doctor` names blocked checks."""
