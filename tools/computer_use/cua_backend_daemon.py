...
def _embedded_daemon_spawn_command(driver_cmd: str, serve_args: List[str], *, platform: str,
    ...
    if not resolved_app:
        raise RuntimeError("CuaDriver.app is required for private computer-use sessions on macOS. Run `mason computer-use install` to restore it.")
...
class _EmbeddedCuaDaemon:
    ...
    startup, so reusing the machine-wide daemon would let one Mason session's YOLO choice affect another. A
    ...
    Mason YOLO (``--dangerously-bypass-approvals``); ``bounded`` = a user-reviewed capability manifest approved
    ...
    def __init__(self, driver_cmd: str, permission_mode: str, capability_manifest: Optional[str] = None) -> None:
        ...
        self.socket_path = (rf"\\.\pipe\mason-cua-{token}" if sys.platform == "win32"
    ...
    def start(self) -> None:
        ...
        threading.Thread(target=self._drain_stderr, args=(self._process,), name="mason-cua-daemon-stderr", daemon=True).start()
