"""`mason computer-use doctor` — thin client for cua-driver's `health_report` MCP tool. cua-driver owns the health
...
def _build_identity(binary: str, report: Report) -> Report:
    """Mason-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under mason_identity (and environment under
        # mason_environment when present) so overall/checks parsers keep working.
        payload = {**report, "mason_identity": identity}
        if environment:
            payload["mason_environment"] = environment
