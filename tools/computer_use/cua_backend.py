...
(no DISPLAY, missing AT-SPI, TCC) surface via `mason computer-use doctor` instead of failing silently. Install
with `mason computer-use install`. The macOS path uses private SkyLight SPIs that can break on OS updates.
...
def sanitized_cua_driver_env() -> Dict[str, str]:
    """``cua_driver_child_env()`` with Mason provider secrets stripped — cua-driver is a third-party binary and must
...
def _empty_discovery_reason() -> str:
    ...
    if sys.platform == "darwin":  # headless Mac / asleep panel: ScreenCaptureKit has 0 shareable displays while TCC looks fine
        return ("window discovery returned no windows; on macOS this usually means no shareable display (headless Mac or "
                "panel asleep) — wake the display or attach a monitor/HDMI dummy, then run `mason computer-use doctor`")
    return "window discovery returned no windows; run `mason computer-use doctor` (display reachability, AX capability)"
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    attempted / it failed). Never raises. An explicit ``MASON_CUA_DRIVER_CMD`` override is authoritative even
...
class CuaDriverBackend(_CaptureMixin, _InputMixin, ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Public session label (one per Mason run) sent as `session` on every call: owns the cursor color and
        # gives config/recording state a stable owner across transport restarts. Part of the 0.20 runtime contract.
        self._session_id: str = f"mason-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            raise RuntimeError(f"cua-driver is not ready: {contract.get('reason') or 'runtime contract is incomplete'}. "
                               + ("Update the binary selected by MASON_CUA_DRIVER_CMD or remove that override."
                                  if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip() else "Run `mason computer-use install` to repair it."))
        _maybe_nudge_update()
    ...
    def _clear_active_target(self) -> None:
        ...
        # Surface 6 of NousResearch/mason-agent#47072: per-snapshot `element_index -> element_token` map
