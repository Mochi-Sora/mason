...
_UNKNOWN_OUTCOME_MESSAGES = {
    "transport_outcome_unknown": (
        "cua-driver transport failed during {name}; the action outcome is unknown, so Mason "
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            # Telemetry policy first (default: disabled), then strip Mason secrets.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                f"{getattr(self, '_startup_phase', 'unknown')}). Run `mason computer-use doctor` and check "
                f"{display_mason_home()}/logs/agent.log for the phase timings.")
