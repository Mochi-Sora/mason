...
_NEEDS_REAUTH_MSG = (
    "MCP server '{s}' requires re-authentication. Run `mason mcp login {s}` (or delete the tokens file under "
    "~/.mason/mcp-tokens/ and restart). Do NOT retry this tool — ask the user to re-authenticate.")
...
def _render_content_blocks(result, server_name: str) -> Tuple[str, int]:
    ...
    # cache those via the gateway's image-cache helper so they flow through Mason' MEDIA: tag convention
    ...
    # (gnanirahulnutakki), both too stale to cherry-pick. #10848's approach (integrate with Mason' MEDIA
