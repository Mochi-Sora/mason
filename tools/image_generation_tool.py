#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``mason tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `mason tools` selection, or ``None`` for direct FAL.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `mason tools` → Image Generation."
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.mason", "singularity": "/root/.mason",
                          "modal": "/root/.mason", "ssh": "~/.mason"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.mason"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `mason tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nous account that has the managed FAL gateway enabled "
                     "(`mason setup`)")
    lines.append("  3. Configure a different image_gen provider via `mason tools` → Image Generation "
                 "(run `mason plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `mason plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `mason tools` → "
