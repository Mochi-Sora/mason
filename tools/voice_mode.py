...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "mason_voice")
...
def _wsl_powershell_player_cmd(file_path: str) -> Optional[List[str]]:
    ...
    try:
        ...
        if not win_tmp_wsl:
            return None
        wsl_wav = os.path.join(win_tmp_wsl, f"mason-tts-{uuid.uuid4().hex[:8]}.wav")
...
def _vad_log(msg: str) -> None:
    """VAD diagnostic: logger.debug, plus stderr when MASON_VOICE_DEBUG=1 (live tuning)."""
    logger.debug(msg)
    if os.environ.get("MASON_VOICE_DEBUG", "").strip() == "1":
        with suppress(Exception):
