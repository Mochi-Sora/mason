...
from tools.environments.remote_common import (
    bash_argv, client_env_with, load_mason_env_vars, prepend_unset, resolve_passthrough_env, run_capture)
...
# Module-level binding: tests patch ``docker._load_mason_env_vars`` to fake the .env file.
_load_mason_env_vars = load_mason_env_vars
...
def _get_active_profile_name() -> str:
    """Active Mason profile name, or ``"default"`` on any error. Resolved at container-create
...
def reap_orphan_containers(
    *, max_age_seconds: int = 600, profile_filter: str | None = None, docker_exe: str | None = None,
    ...
    """Remove stale mason-tagged containers left behind by prior processes (SIGKILL/OOM
    ...
    filters = ["--filter", "label=mason-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=mason-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    """Locate the docker/podman CLI (cached): ``MASON_DOCKER_BINARY`` override, ``docker``
    on PATH, ``podman`` on PATH, then macOS Docker Desktop locations; ``None`` if absent."""
    global _docker_executable
    if _docker_executable is not None:
        return _docker_executable

    override = os.getenv("MASON_DOCKER_BINARY")
    if override and _is_executable(override):
        logger.info("Using MASON_DOCKER_BINARY override: %s", override)
        found = override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # s6-overlay images (e.g. mason-agent:latest) already use /init as PID 1 and exec
        ...
        # Labels identify mason containers to the orphan reaper (mason-agent=1),
        ...
        self._labels = {
            "mason-agent": "1",
            "mason-task-id": task_label,
            "mason-profile": profile_name,
    ...
    def _mount_args(self, volumes, host_cwd, auto_mount_cwd, task_id) -> tuple[list[str], list[str]]:
        ...
        Persistent mode bind-mounts from TERMINAL_SANDBOX_DIR (default ~/.mason/sandboxes/)."""
    ...
    def _docker_run(self, cwd: str) -> str:
        ...
        container_name = f"mason-{uuid.uuid4().hex[:8]}"
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        return resolve_passthrough_env(self._forward_env, mason_env_loader=_load_mason_env_vars)
    ...
    def _recreate_container(self) -> bool:
        ...
        existing = self._find_reusable_container(
            self._labels.get("mason-task-id", ""),
            self._labels.get("mason-profile", ""),
        ...
        if not self._container_id:
            if not self._image:
                ...
                return False
            try:
                new_name = f"mason-{uuid.uuid4().hex[:8]}"
                result = run_capture(
    ...
    def _find_reusable_container(
        ...
        reused after ``mason egress disable`` (baked-in proxy env and CA mounts), and every
        ...
        filters = [
            "--filter", "label=mason-agent=1",
            "--filter", f"label=mason-task-id={task_label}",
            "--filter", f"label=mason-profile={profile_label}",
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"mason-cleanup-{log_id}")
        with _TEARDOWN_LOCK:
