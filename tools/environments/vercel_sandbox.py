...
under ``MASON_HOME`` and new sandboxes are restored from them on task reuse.
"""
...
def _ensure_vercel_sdk() -> None:
    ...
    # The SDK (>=0.7) ships default-on telemetry; Mason policy is opt-in only, so disable it
...
def _snapshot_store() -> Path:
    return get_mason_home() / "vercel_sandbox_snapshots.json"
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _remote_mason_dir(self) -> str:
        return f"{self._remote_home.rstrip('/')}/.mason"
    ...
    def _attach_fresh_sandbox(self, requested_cwd: str) -> None:
        ...
        container_base = self._remote_mason_dir()
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        archive_member = self._remote_mason_dir().lstrip("/")
        remote_tar = f"/tmp/.mason_sync.{os.getpid()}.tar"
        sandbox = self._require_sandbox()
        try:
