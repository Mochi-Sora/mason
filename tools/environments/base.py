"""Base class for all Mason execution environment backends.
...
# (MASON_DEBUG_INTERRUPT=1). Off by default to avoid flooding gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("MASON_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    overlays/SIF cache). ``TERMINAL_SANDBOX_DIR`` overrides ``{MASON_HOME}/sandboxes``."""
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    p = Path(custom) if custom else get_mason_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Mason backends. Subclasses
    ...
    # True only when commands execute on the SAME host as the Mason process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/mason-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/mason-cwd-{self._session_id}.txt"
    ...
    def fetch_file(self, remote_path: str, local_dest: Path, *, max_bytes: int) -> None:
        ...
        import base64
        import binascii
        marker = f"__MASON_FETCH_{uuid.uuid4().hex[:12]}__"
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"MASON_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        stdin_thread = getattr(proc, "_mason_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        if stdin_errors := getattr(proc, "_mason_stdin_errors", None):
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the ``__MASON_CWD_{session}__`` marker from ``result["output"]``, update
