...
from tools.environments.local_env_policy import _MASON_PROVIDER_ENV_BLOCKLIST, _is_mason_internal_secret


def load_mason_env_vars() -> dict[str, str]:
    """``~/.mason/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            mason_env_loader: Callable[[], dict[str, str]] = load_mason_env_vars,
    ...
    is filtered through the Mason provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_mason_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _MASON_PROVIDER_ENV_BLOCKLIST)
    mason_env = mason_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or mason_env.get(key)
