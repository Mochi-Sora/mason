...
from mason_constants import get_process_mason_home
...
from tools.environments.local_env_policy import (
    _ALWAYS_STRIP_KEYS, _MASON_PROVIDER_ENV_BLOCKLIST, _MASON_PROVIDER_ENV_FORCE_PREFIX,
    _is_mason_internal_secret, _is_terminal_first_party_env,
...
from tools.environments.local_pythonpath import (
    _build_mason_repo_root_aliases, _strip_mason_owned_pythonpath_and_runtime_markers)
...
# get_temp_dir() defaults to MASON_HOME/cache/terminal (real storage, not tmpfs), so
...
# Background artifacts come in triplets (mason_bg_<id>.log/.pid/.exit). A live
...
_BG_GROUP_RE = re.compile(r"^(mason_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return MASON_HOME/cache/terminal, or None if unresolvable."""
    try:
        from mason_constants import get_mason_home
        return get_mason_home() / "cache" / "terminal"
    except Exception:
        return None
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local MASON_HOME override, then the subprocess HOME contract."""
    from mason_constants import apply_subprocess_home_env, get_mason_home_override
    try:
        if value := get_mason_home_override():
            env["MASON_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (MASON_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping Mason-managed secrets. ``_MASON_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_MASON_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_MASON_PROVIDER_ENV_FORCE_PREFIX):]
            if not _is_mason_internal_secret(key):
                out[key] = value
            continue
        if _is_mason_internal_secret(key) or key in plugin_strip:
            continue
        first_party = _is_terminal_first_party_env(key)
        passthrough = is_env_passthrough(key)
        if key in _MASON_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
            continue
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, Mason-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
    Kanban scrub. Returns the (possibly new) dict."""
    _apply_profile_home(env)
    _inject_session_context_env(env)
    _strip_mason_owned_pythonpath_and_runtime_markers(env)
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the mason install dir so bare ``mason`` resolves
    ...
    # Keep bare ``mason`` invocations available to child jobs even when the gateway was launched by a
    ...
    if path_key is not None:
        out[path_key] = _prepend_mason_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Mason-managed secrets from a subprocess environment (background/PTY
...
def mason_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    if not inherit_credentials:
        strip |= _MASON_PROVIDER_ENV_BLOCKLIST
    for key in list(env):
        if (key in strip or key.startswith(_MASON_PROVIDER_ENV_FORCE_PREFIX)
                or _is_mason_internal_secret(key)):
            del env[key]
...
def build_subprocess_env(
    ...
    bridges MASON_HOME + HOME and ``extra`` is applied last so caller overrides win."""
...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    """Ordered bash.exe candidates on Windows: MASON_GIT_BASH_PATH, our portable Git
    under %LOCALAPPDATA%\\mason\\git (PortableGit ``bin`` and MinGit ``usr\\bin``),
    ...
    roots = [
        lad and os.path.join(lad, "mason", "git", "bin"),
        lad and os.path.join(lad, "mason", "git", "usr", "bin"),
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("MASON_GIT_BASH_PATH")
    candidates = _windows_bash_candidates(custom)
    # First candidate that can actually start wins: a stale MASON_GIT_BASH_PATH
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "MASON_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
        ...
        return candidates[0]
    raise RuntimeError(
        "Git Bash not found. Mason Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set MASON_GIT_BASH_PATH to your bash.exe location.")
...
# Cached directory containing the ``mason`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_MASON_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_mason_bin_dir() -> str | None:
    """Directory holding the ``mason`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``mason`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    mason executable; ``sys.executable``'s dir if it holds the shim."""
    global _MASON_BIN_DIR
    if _MASON_BIN_DIR is not _SENTINEL:
        return _MASON_BIN_DIR  # type: ignore[return-value]
    which = shutil.which("mason")
    argv0 = sys.argv[0] if sys.argv else ""
    base = os.path.basename(argv0).lower()
    exe_dir = os.path.dirname(sys.executable) if sys.executable else ""
    shim = "mason.exe" if _IS_WINDOWS else "mason"
    if which:
        candidate = os.path.dirname(which)
    elif (os.path.isabs(argv0) and (base == "mason" or base.startswith("mason."))
    ...
    _MASON_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _MASON_BIN_DIR


def _prepend_mason_bin_dir(existing_path: str) -> str:
    """Prepend the mason install dir to ``existing_path`` if missing."""
    bin_dir = _resolve_mason_bin_dir()
    return _prepend_missing_path_entries(existing_path, [bin_dir] if bin_dir else [])


def _managed_runtime_path_entries() -> list[str]:
    """Existing Mason-managed runtime dirs: ``$MASON_HOME/node`` (+``/bin``) and
    ``$MASON_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    profile-scoped and a managed tree can appear mid-process."""
    try:
        from mason_constants import get_mason_home, iter_mason_node_dirs
        return [str(d) for d in (*iter_mason_node_dirs(), get_mason_home() / "bin") if d.is_dir()]
    except Exception:
        return []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Mason runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# --- Mason venv / repo-root detection (module-level, computed once) ---
...
# spellings — the Windows gateway launcher renders Mason-owned paths under the
# configured MASON_HOME spelling (possibly a junction to another drive).
_mason_repo_root: Path = Path(__file__).resolve().parents[2]
_mason_repo_root_aliases: tuple[Path, ...] = _build_mason_repo_root_aliases(
    _mason_repo_root, Path(__file__).absolute().parents[2], get_process_mason_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_mason_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
def _kill_process_group_posix(proc) -> None:
    ...
    try:
        ...
        if (pgid := getattr(proc, "_mason_pgid", None)) is None:
            raise
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Mason host itself — controller-side platform behavior
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no /tmp), ``MASON_HOME/cache/terminal`` (real storage: tmpfs /tmp
        fills under Mason load; pruned by ``cleanup_terminal_temp_cache``), /tmp,
        ...
        so always the MASON_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "mason_terminal")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        if not _IS_WINDOWS:
            with contextlib.suppress(ProcessLookupError):
                proc._mason_pgid = os.getpgid(proc.pid)
