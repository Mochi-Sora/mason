...
_SNAPSHOT_STORE = get_mason_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        def _create(image_spec: Any) -> None:
            async def _create_sandbox():
                app = await _modal.App.lookup.aio("mason-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .mason/ as a tar archive (sandboxes run as root, so /root/.mason)."""
        data = self._exec("tar cf - -C / root/.mason", timeout=120, fail_label="bulk download", capture=True)
