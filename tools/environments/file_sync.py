...
_FORCE_SYNC_ENV = "MASON_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.mason") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.mason to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.mason", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``MASON_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def sync_back(self, mason_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.mason/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(mason_home=mason_home)

    def _sync_back_transaction(self, mason_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .mason/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (mason_home or get_mason_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="mason-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.mason/skills/b.md`` -> ``~/.mason/skills/b.md``)."""
