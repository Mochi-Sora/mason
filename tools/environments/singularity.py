...
_SNAPSHOT_STORE = get_mason_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    if custom_scratch:
        ...
        if scratch.exists() and os.access(scratch, os.W_OK):
            scratch_path = scratch / os.getenv("USER", "mason") / "mason-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "~", timeout: int = 60, cpu: float = 0,
        ...
        self.instance_id = f"mason_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            ...
            self._overlay_dir = (
                _get_scratch_dir() / "mason-overlays" / f"overlay-{sanitize_task_id_for_path(task_id)}")
