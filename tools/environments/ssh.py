...
# Module-level binding: tests patch ``ssh._load_mason_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "mason-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.mason"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.mason directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.mason"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="mason-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .mason/ as a tar archive."""
        # Tar from / with the full path so archive entries keep absolute paths
        # (home/user/.mason/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.mason".lstrip("/")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(mason_env_loader=_load_mason_env_vars)
