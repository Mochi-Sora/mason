...
# backend serves many sessions, so a snapshot carrying the FIRST session's MASON_SESSION_ID
# would make every LATER session source a foreign identity. Every bridged name starts with
# one of these prefixes (or is MASON_UI_SESSION_ID); unit tests use this regex as the
...
# "default" environment), so ``export -p`` dumping the FIRST session's MASON_SESSION_ID into the snapshot
# makes every LATER session ``source`` that stale value and see a FOREIGN session's identity — overriding
# the correct per-command Popen env (issue: cross-session MASON_SESSION_ID leak via the shared snapshot).
# Stripping them from the snapshot is safe because they are re-injected on every command; a snapshot should
# only carry the user's own shell state (PATH, functions, exports they set), not Mason' per-turn session
# identity. Used by unit tests as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (MASON_SESSION_|MASON_UI_SESSION_ID|MASON_CRON_AUTO_DELIVER_|"
    "MASON_CRON_SESSION|MASON_BROWSER_CONTROL_)")
_SHELL_ENV_NAME_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")

# mktemp template suffix + the shell variable holding the allocated temp path.
_SNAP_TMP_SUFFIX = ".tmp.XXXXXXXXXX"
_SNAP_TMP = '"$__mason_snap_tmp"'


def _cwd_marker(session_id: str) -> str:
    return f"__MASON_CWD_{session_id}__"
...
def _export_dump_excluding_session_vars(tmp_path: str, excluded_names: Iterable[str] = ()) -> str:
    ...
    ``curl … | bash #`` smuggled into a Matrix room/display name via ``MASON_SESSION_CHAT_NAME``) land in
    ...
    return (
        "{ ( unset ${!MASON_SESSION_*} ${!MASON_CRON_AUTO_DELIVER_*} "
        "${!MASON_BROWSER_CONTROL_*} "
        # AI_AGENT / MASON_AGENT are per-command attribution markers re-exported
        ...
        "AI_AGENT MASON_AGENT "
        f"MASON_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; ) || true; } "
        f"> {tmp_path}")
...
def _snapshot_bootstrap_script(
    *, quoted_cwd: str, quoted_snap: str, snap_tmp_template: str, excluded_names: Iterable[str], cwd_marker: str,
    ...
    return (
        "umask 077\n"
        f"__mason_snap_tmp=$(mktemp {snap_tmp_template}) || exit 1\n"
        f"{_export_dump_excluding_session_vars(_SNAP_TMP, excluded_names)}\n"
        "__mason_fns=$(declare -F | awk '{print $3}' | grep -vE '^_[^_]') || true\n"
        f"[ -n \"$__mason_fns\" ] && declare -f $__mason_fns >> {_SNAP_TMP} 2>/dev/null || true\n"
        f"alias -p >> {_SNAP_TMP}\n"
...
def _passthrough_save_restore(names: Iterable[str]) -> tuple[list[str], list[str]]:
    ...
    restore: list[str] = []
    for name in names:
        marker = f"_MASON_RUNTIME_PASSTHROUGH_{name}"
...
def _wrap_command_script(
    ...
    ``declare -x`` lines when sourcing. AI_AGENT/MASON_AGENT advertise the harness to remote
    ...
    parts += restore
    parts += [
        'export AI_AGENT="${AI_AGENT:-mason-agent}" MASON_AGENT="${MASON_AGENT:-true}"',
