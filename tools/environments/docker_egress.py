...
_EGRESS_LABEL_KEY = "mason-egress"
_CONTAINER_CA = "/etc/ssl/certs/mason-egress-ca.crt"
_NODE_OPTIONS_SENTINEL = "_MASON_EGRESS_NODE_OPTIONS_APPEND"
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        return _degraded(
            ...
            "Re-run `mason egress setup` to regenerate it.")
    ...
    if not mappings:
        return _degraded(
            ...
            "corrupt.  Re-run `mason egress setup` to mint provider "
    ...
    env_overrides: dict[str, str] = {
        ...
        "MASON_EGRESS_PROXY": "1",  # lets the in-sandbox agent know it is proxy-aware
        _NODE_OPTIONS_SENTINEL: "--use-openssl-ca"}

    # Proxy tokens under the standard provider env names (and their aliases) so
    # SDKs work unchanged; MASON_PROXY_TOKEN_* copies are for diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"MASON_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
