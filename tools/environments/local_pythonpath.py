"""Mason-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the Mason venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven Mason-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_mason_repo_root_aliases``, ``_in_venv``,
``_mason_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_mason_repo_root_aliases(
    resolved_root: Path, lexical_root: Path, configured_home: Path,
) -> tuple[Path, ...]:
    """Exact repo-root spellings emitted by Mason launchers. Mirrors
    ``gateway_windows._preserve_mason_home_path`` (physical path under the resolved
    MASON_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary MASON_HOME descendants as Mason-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_mason_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._mason_repo_root_aliases):
        return None
    try:
...
def _get_mason_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the Mason runtime (cached):
    ...
    local = _state()
    if local._mason_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._mason_site_packages = list(result)
    result = list(local._mason_site_packages)
...
def _strip_mason_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Mason-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_mason_owned_pythonpath(env: dict) -> None:
    """Remove Mason-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_mason_site_packages(env), *_state()._mason_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped Mason-owned entries from PYTHONPATH: %s", stripped)
