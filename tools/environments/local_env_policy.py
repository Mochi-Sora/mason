"""Secret-scrub policy for Mason child processes: pure data + predicates for which env
names are Mason-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``mason_subprocess_env``) live in ``tools.environments.local``."""

import os

# Prefix a caller uses in ``extra_env`` to force a blocklisted var through.
_MASON_PROVIDER_ENV_FORCE_PREFIX = "_MASON_FORCE_"

# Mason-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "MASON_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Mason: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Mason-managed
    # inference credential — Claude subscription auth is not a working Mason provider path. It arrives via
...
_MASON_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# ``_sanitize_subprocess_env``); execute_code, mason_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``MASON_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`mason_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Mason credential; these ARE Mason-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("MASON_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Mason venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Mason venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Mason-owned entries only).
...
# dependencies into the Mason venv path instead of the project's own ``.venv`` — silently clobbering the
# Mason environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Mason venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Mason itself
...
# _strip_mason_owned_pythonpath() which removes only Mason-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_mason_internal_secret(key: str) -> bool:
    """True for Mason-internal secrets injected under *dynamic* names the static
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_mason_internal_secret, but _ID has no secret suffix, so it must be
    # enumerated here to stay stripped on the inherit_credentials=True path.
    "GATEWAY_RELAY_ID", "GATEWAY_RELAY_SECRET", "GATEWAY_RELAY_DELIVERY_KEY",
    "HASS_TOKEN", "EMAIL_PASSWORD", "MASON_DASHBOARD_SESSION_TOKEN",
