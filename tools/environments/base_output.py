...
def _new_output_collector(proc, bounded_capture: bool) -> _BoundedOutputCollector:
    ...
    spill file under ``$MASON_HOME/cache/terminal-output`` (created only on actual overflow;
    ...
    spill_path = None
    try:
        spill_dir = get_mason_home() / "cache" / "terminal-output"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    ``proc._mason_stdin_errors`` (surfaced by ``_wait_for_process`` as ``stdin_error``) and
    stdin is still closed in ``finally`` so the child sees EOF instead of hanging.
    """
    errors: list[BaseException] = []
    proc._mason_stdin_errors = errors
    ...
    proc._mason_stdin_thread = thread
    thread.start()
