...
def _mason_tags(fm: dict) -> Any:
    """``metadata.mason.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    mason_meta = metadata.get("mason", {}) if isinstance(metadata, dict) else None
    return mason_meta.get("tags", []) if isinstance(mason_meta, dict) else []
