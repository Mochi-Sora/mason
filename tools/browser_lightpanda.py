...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_mason_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("mason home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$MASON_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _state_dir() -> Path:
    from mason_constants import get_mason_home
    path = Path(get_mason_home()) / "cache" / "browser-use" / "lightpanda"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Mason spawns.
    ...
    bounds it without Mason managing eviction.
    """
...
def _cdp_ready(url: str, timeout: float = 0.2) -> bool:
    try:
        from mason_cli.browser_connect import is_browser_debug_ready
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Mason under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Mason is gone; return the count. A live owner is
