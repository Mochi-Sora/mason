...
class MCPServerRunMixin:
    ...
    async def _prepare_run(self, config: dict) -> bool:
        ...
        # routes it through Mason' approval system.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # exit the reconnect loop and the MCP server would stay dead until Mason is fully
                # restarted. See #9930.
                self.session = None
                raise
            except Exception as exc:
    ...
    async def _on_initial_connect_error(self, exc: Exception, root: BaseException,
        ...
        if failure_class == "permanent":
            ...
            detail = (f"authentication, parking until credentials change; re-authenticate with "
                      f"`mason mcp login {self.name}`" if _errors._is_auth_error(root)
