...
# ``mason send`` CLI, the kanban notifier and the opt-in MCP server import the helpers.
...
def _resolve_platform_config(platform_name, config):
    ...
    if pconfig is None:
        return None, None, None, (f"Platform '{platform_name}' is not configured. Set up credentials in "
                                  "~/.mason/config.yaml or environment variables.")
...
def _home_chat_id(config, platform, platform_name):
    ...
    return None, (f"No home channel set for {platform_name} to determine where to send the message. "
                  ...
                  f"or set a home channel via: mason config set {home_env} <channel_id>")
...
def _mirror_sent_message(platform_name, chat_id, mirror_text, thread_id):
    ...
    try:
        ...
        return bool(mirror_to_session(
            ...
            source_label=get_session_env("MASON_SESSION_PLATFORM", "cli"),
...
def _maybe_skip_cron_duplicate_send(platform_name: str, chat_id: str, thread_id: str | None):
    ...
    from gateway.session_context import get_session_env
    auto_platform = get_session_env("MASON_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    auto_chat_id = get_session_env("MASON_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not (auto_platform and auto_chat_id and auto_platform == platform_name and auto_chat_id == str(chat_id)
            and (get_session_env("MASON_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None) == thread_id):
        return None
