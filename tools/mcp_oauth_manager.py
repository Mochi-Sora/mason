...
class MasonMCPOAuthProvider(MasonProviderMixin, *_SDK_BASES):
    ...
    fixes come from ``MasonProviderMixin``. Only usable when the SDK's OAuth module imported.

    Reference: Claude Code's ``invalidateOAuthCacheIfDiskChanged`` (``src/utils/auth.ts:1320``, CC-1096 /
    GH#24317).
    """

    _mason_logger = logger
    ...
    def __init__(self, *args: Any, server_name: str = "", preregistered: bool = False, **kwargs: Any):
        ...
        self._mason_server_name = server_name
        self._mason_home = ""
        # A config-supplied client_id rejected as invalid_client means the *config* is wrong — only DCR clients auto-heal.
        self._mason_preregistered = preregistered

    def _mason_storage(self):
        """The context storage when it is a ``MasonTokenStorage``, else None."""
        from tools.mcp_oauth import MasonTokenStorage
        return self.context.storage if isinstance(self.context.storage, MasonTokenStorage) else None

    def _log_nonfatal(self, what: str, exc: BaseException) -> None:
        logger.debug("MCP OAuth '%s': %s failed (non-fatal): %s", self._mason_server_name, what, exc)
    ...
    async def _initialize(self) -> None:
        ...
        Bearer tokens as "valid"; seeding the expiry (``MasonTokenStorage`` persists absolute
        ...
        if storage is not None and self.context.oauth_metadata is None:
            ...
            if meta is not None:
                ...
                logger.debug("MCP OAuth '%s': restored metadata from disk (token_endpoint=%s)",
                             self._mason_server_name, meta.token_endpoint)
    ...
    async def _prefetch_oauth_metadata(self) -> None:
        ...
        # The SDK's httpx flavour, not Mason': `create_oauth_metadata_request` returns *its* (httpx2) Request objects.
        ...
        async def _send(client, url: str, label: str):
            try:
                ...
                logger.debug("MCP OAuth '%s': %s discovery to %s failed: %s", self._mason_server_name, label, url, exc)
                return None
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            for url in build_oauth_authorization_server_metadata_discovery_urls(self.context.auth_server_url, server_url):
                ...
                if asm:
                    ...
                    storage = self._mason_storage()  # persist now so a later cold-load skips discovery
                    if storage is not None:
                        storage.save_oauth_metadata(asm)
                    logger.debug("MCP OAuth '%s': pre-flight ASM discovered token_endpoint=%s",
                                 self._mason_server_name, asm.token_endpoint)
                    break
    ...
    async def _maybe_flag_poisoned_client(self, response: Any) -> None:
        ...
        is swallowed. The browser-side "Redirect URI Mismatch" case has no HTTP signal (``mason mcp reauth``).

        See #36767.
        """
        try:
            if (self._mason_preregistered or getattr(response, "status_code", None) not in (400, 401)
            ...
            # back into the same refusal (`mason mcp login` clears the marker).
            ...
            if cimd_url and getattr(self.context.client_info, "client_id", None) == cimd_url:
                logger.warning("MCP OAuth '%s': authorization server rejected our Client ID Metadata Document (%s) "
                               ...
                               self._mason_server_name, cimd_url)
    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        try:  # pre-flow hook: reload from disk if it changed (non-fatal on error)
            await get_manager().invalidate_if_disk_changed(self._mason_server_name, mason_home=self._mason_home)
...
_MASON_PROVIDER_CLS: Optional[type] = MasonMCPOAuthProvider if _SDK_BASES else None
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._mason_home = key[0]
            return entry.provider

    @staticmethod
    def _key(server_name: str, mason_home: str | Path | None = None) -> tuple[str, str]:
        from mason_constants import get_mason_home
        home = Path(mason_home) if mason_home is not None else get_mason_home()
    ...
    def _build_provider(self, server_name: str, entry: _ProviderEntry) -> Optional[Any]:
        """Build a ``MasonMCPOAuthProvider``; None if the SDK's OAuth support is unavailable."""
        if _MASON_PROVIDER_CLS is None:
        ...
        if get_dashboard_oauth_flow() is None and not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                f"Run `mason mcp login {server_name}` interactively first to complete initial authorization.")
        return _MASON_PROVIDER_CLS(
    ...
    def remove(self, server_name: str, *, mason_home: str | Path | None = None) -> _ProviderEntry | None:
        """Evict the provider from cache AND delete tokens from disk (``mason mcp remove`` / forced re-auth)."""
        entry = self.evict(server_name, mason_home=mason_home)
        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, mason_home=mason_home)
        logger.info("MCP OAuth '%s': evicted from cache and removed from disk", server_name)
        return entry

    def restore_entry(self, server_name: str, entry: _ProviderEntry | None, *, mason_home: str | Path | None = None) -> None:
        """Restore a provider entry removed for a failed reauthorization."""
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, mason_home), entry)

    def evict(self, server_name: str, *, mason_home: str | Path | None = None) -> _ProviderEntry | None:
        """Drop only the in-process provider, preserving persisted OAuth state."""
        with self._entries_lock:
            return self._entries.pop(self._key(server_name, mason_home), None)

    async def invalidate_if_disk_changed(self, server_name: str, *, mason_home: str | Path | None = None) -> bool:
        """Force the SDK provider to reload when the tokens file mtime changed (e.g. a cron refresh); True if so."""
        from tools.mcp_oauth import _get_token_dir, _safe_filename
        entry = self._entries.get(self._key(server_name, mason_home))
        if entry is None or entry.provider is None:
            return False
        async with entry.lock:
            try:
                mtime_ns = (_get_token_dir(mason_home) / f"{_safe_filename(server_name)}.json").stat().st_mtime_ns
            except OSError:
                return False
