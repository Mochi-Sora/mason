...
def _resolve_stt_language(
    ...
    (plus *extra_keys* aliases, e.g. ``language_code``) > ``stt.language`` > ``MASON_LOCAL_STT_LANGUAGE``
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Whether *provider* is exempt from Mason's remote upload cap."""
...
def _resolve_explicit_local() -> str:
    ...
    if not backend:
        logger.warning("STT provider 'local' configured but unavailable "
                       "(install faster-whisper or set MASON_LOCAL_STT_COMMAND)")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(target=_watch, name="mason-stt-idle-unload", daemon=True)
...
def _no_provider_error(provider: str, stt_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Surface it — with its `mason tools` remediation — instead of the all-provider setup hint (#93045).
