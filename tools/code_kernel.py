...
"""Auto-generated Mason session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["MASON_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("MASON_KERNEL_SPILL_DIR", "")
_SPILL_CAP = {spill_cap}
_PARENT_PROCESS_HANDLE = os.environ.pop("MASON_KERNEL_PARENT_PROCESS_HANDLE", "")
_PARENT_DEATH_FD = os.environ.pop("MASON_KERNEL_PARENT_DEATH_FD", "")
...
def _start_parent_death_pipe_watchdog():
    ...
    threading.Thread(target=_wait, name="mason-parent-watchdog", daemon=True).start()
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("MASON_SESSION_ID", "") or (task_id or "")
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    (the client stub reconnects: MASON_RPC_PERSISTENT). The serving thread carries NO frozen
...
def _bind_rpc_socket(kernel: SessionKernel) -> str:
    ...
    if _IS_WINDOWS:
        ...
        rpc_endpoint = kernel.sock_path = os.path.join(sock_tmpdir, f"mason_rpc_{uuid.uuid4().hex}.sock")
...
def _parent_process_handle(child_env: Dict[str, str]):
    ...
    try:
        ...
        if handle:
            child_env["MASON_KERNEL_PARENT_PROCESS_HANDLE"] = str(int(handle))
            startupinfo = subprocess.STARTUPINFO()
            startupinfo.lpAttributeList = {"handle_list": [int(handle)]}
    except (AttributeError, ImportError, OSError, TypeError, ValueError):
        if handle and close is not None:
            close(handle)
        child_env.pop("MASON_KERNEL_PARENT_PROCESS_HANDLE", None)
        handle = close = startupinfo = None
...
def _spawn(kernel: SessionKernel, *, child_python: str, child_cwd: str,
    ...
    from tools.code_execution_tool import generate_mason_tools_module
    kernel.tmpdir = tempfile.mkdtemp(prefix="mason_kernel_")
    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@MASON-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    rpc_endpoint = _bind_rpc_socket(kernel)
    for name, src in (("mason_tools.py", generate_mason_tools_module(list(sandbox_tools))),
                      ("mason_kernel_runner.py", KERNEL_RUNNER_SOURCE)):
    ...
    if not _IS_WINDOWS:
        ...
        child_env["MASON_KERNEL_PARENT_DEATH_FD"] = str(death_r)
        pass_fds = (death_r,)
    try:
        kernel.proc = subprocess.Popen(
            [child_python, os.path.join(kernel.tmpdir, "mason_kernel_runner.py")],
