...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    Ported in spirit from nearai/ironclaw#5029 (dual line/byte cap on ``read_file``). Where mason
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    guard (host only) → document extraction → binary-extension guard → Mason
    ...
    try:
        ...
        # Mason internal denylist (prompt injection via catalog metadata,
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with mason_tools.write_file() for very "
            "large files."
        )
