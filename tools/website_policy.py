...
Loads a user-managed website blocklist (``security.website_blocklist`` in ~/.mason/config.yaml plus
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    default_path = get_mason_home() / "config.yaml"
    ...
    for shared_file in shared_files:
        ...
        path = path if path.is_absolute() else (get_mason_home() / path).resolve()
