...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with mason-agent)
2. User plugins: ``$MASON_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``mason_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MASON_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MASON_HOME/plugins/model-providers/`` if it exists."""
    try:
        from mason_constants import get_mason_home

        d = get_mason_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$MASON_HOME/plugins/`` if it exists.

    This is where ``mason plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MASON_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_mason_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``mason_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."mason_agent.plugins"]
        acme-inference = "acme_mason_plugin:register"
    ...
    * **Provider targets only.** The ``mason_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MASON_HOME``) keep their
    ...
    try:
        from mason_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MASON_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``mason plugins install`` at
          ``$MASON_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``mason_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$MASON_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with mason-agent.
    ...
    # 2. User plugins — under $MASON_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``mason plugins install`` / the plugin index.
    #     Those clone into $MASON_HOME/plugins/<name>/ — flat, NOT under
