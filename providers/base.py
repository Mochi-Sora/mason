...
def _profile_user_agent() -> str:
    """Return a ``mason-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from mason_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"mason-cli/{_ver}"
    except Exception:
        return "mason-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; mason_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.mason/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # mason-cli UA so the catalog endpoint is reachable.
