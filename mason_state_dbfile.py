"""state.db file-level health helpers, split out of ``mason_state.py``.
...
holder-process classification.  Helpers that mason_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``mason_state`` at
call time, so tests that monkeypatch ``mason_state.<name>`` keep intercepting.
"""
...
# shared lock (see mason_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from mason_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from mason_state import DeletedWalGenerationError
    from mason_state_errors import _DELETED_WAL_GENERATION_MSG
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without mason_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from mason_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("mason_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching mason_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    snapshots).  Prefers ``mason_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "state-snapshots via `mason snapshot list` / `mason snapshot restore <id>`.",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from mason_state import _connect_tracked_db
