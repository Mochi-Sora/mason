...
#
# Wired into the image as /etc/cont-init.d/01-mason-setup by the
...
MASON_HOME="${MASON_HOME:-/opt/data}"
INSTALL_DIR="/opt/mason"

# Drop to mason via s6-setuidgid, but skip it when already non-root.
as_mason() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid mason "$@"; }
...
# starts non-root. The baked install tree under /opt/mason is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $MASON_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass MASON_UID/MASON_GID — or the PUID/PGID aliases — which the
...
# `id -u mason` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# mason UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u mason)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-mason UID).
...
and the baked /opt/mason install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e MASON_UID=\$(id -u) -e MASON_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the mason user to that UID/GID at boot and chowns the data
...
# --- Bootstrap MASON_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid mason mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom MASON_HOME paths whose parents only
# root can create (e.g. `MASON_HOME=/home/mason/.mason` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for MASON_UID/MASON_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  MASON_UID/
# MASON_GID still win when both are set.  See #15290, salvages #25872.
MASON_UID="${MASON_UID:-${PUID:-}}"
MASON_GID="${MASON_GID:-${PGID:-}}"

if [ -n "${MASON_UID:-}" ] && validate_uid_gid "$MASON_UID" && [ "$MASON_UID" != "$(id -u mason)" ]; then
    echo "[stage2] Changing mason UID to $MASON_UID"
    usermod -u "$MASON_UID" mason
fi
if [ -n "${MASON_GID:-}" ] && validate_uid_gid "$MASON_GID" && [ "$MASON_GID" != "$(id -g mason)" ]; then
    echo "[stage2] Changing mason GID to $MASON_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$MASON_GID" mason 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised mason user (UID 10000)
...
# mason process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes mason. Idempotent across container
...
#   - mason is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G mason 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] mason already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" mason 2>/dev/null; then
        echo "[stage2] Added mason to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group mason failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When MASON_UID is remapped or the top-level $MASON_HOME isn't owned by
# the runtime mason UID, restore ownership to mason — but ONLY for the
# directories mason actually writes to. The full $MASON_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of mason-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_mason_uid=$(id -u mason)
...
chown_mason_tree() {
    ...
    chown -R mason:mason "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_mason_owner() {
    target="$1"
    find "$target" \( ! -user mason -o ! -group mason \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$MASON_HOME" 2>/dev/null)" != "$actual_mason_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $MASON_HOME (targeted) to mason ($actual_mason_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $MASON_HOME: chown the directory itself (not its contents)
    # so mason can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$MASON_HOME"; then
        :
    else
        chown mason:mason "$MASON_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $MASON_HOME failed (rootless container?) — continuing"
    fi
    # Mason-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by mason (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$MASON_HOME/$sub" ] && tree_has_non_mason_owner "$MASON_HOME/$sub"; then
            chown_mason_tree "$MASON_HOME/$sub"
        fi
    done
fi
...
# mason user. Hosted/container instances keep mutable state under
# $MASON_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# MASON_DISABLE_LAZY_INSTALLS=1. Keeping /opt/mason root-owned and
...
# $MASON_HOME/lazy-packages dir on the data volume (Dockerfile sets
# MASON_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to mason
...
# Always reset ownership of $MASON_HOME/profiles to mason on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> mason …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as mason and walks
...
if [ -d "$MASON_HOME/profiles" ] && tree_has_non_mason_owner "$MASON_HOME/profiles"; then
    chown_mason_tree "$MASON_HOME/profiles"
fi

# Always reset ownership of $MASON_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged mason runtime even
...
# gate as profiles/).
if [ -d "$MASON_HOME/cron" ] && tree_has_non_mason_owner "$MASON_HOME/cron"; then
    chown_mason_tree "$MASON_HOME/cron"
fi

# Always ensure logs/gateways is mason-owned (#45258). Formerly healed by
...
# top-level $MASON_HOME is mis-owned, so a warm volume with mason-owned
# MASON_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid mason mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as mason.
if [ -d "$MASON_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$MASON_HOME/logs/gateways"; then
        :
    else
        chown mason:mason "$MASON_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# mason pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged mason gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $MASON_HOME is
...
# and cron/.
if [ -d "$MASON_HOME/platforms/pairing" ] && tree_has_non_mason_owner "$MASON_HOME/platforms/pairing"; then
    chown_mason_tree "$MASON_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$MASON_HOME/pairing" ] && tree_has_non_mason_owner "$MASON_HOME/pairing"; then
    chown_mason_tree "$MASON_HOME/pairing"
fi

# Reset ownership of mason-owned top-level state files on every boot.
# The targeted data-volume chown above only covers mason-owned
# *subdirectories*; loose state files living directly under $MASON_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> mason …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged mason runtime then hits
...
# sweep so host-owned files in a bind-mounted $MASON_HOME are never
...
# entries of mason_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$MASON_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$MASON_HOME/$f"; then
            :
        else
            chown mason:mason "$MASON_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the mason runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$MASON_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$MASON_HOME/config.yaml"; then
        :
    else
        chown mason:mason "$MASON_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$MASON_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as mason user ---
# Run as mason via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $MASON_HOME values
...
as_mason mkdir -p \
    ...
    "$MASON_HOME/sessions" \
    "$MASON_HOME/logs" \
    "$MASON_HOME/logs/gateways" \
    "$MASON_HOME/hooks" \
    "$MASON_HOME/memories" \
    "$MASON_HOME/skills" \
    "$MASON_HOME/skins" \
    "$MASON_HOME/plans" \
    "$MASON_HOME/workspace" \
    "$MASON_HOME/home" \
    "$MASON_HOME/pairing" \
    "$MASON_HOME/platforms/pairing" \
    "$MASON_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/mason/.install_method (see Dockerfile), NOT written here into
# $MASON_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $MASON_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.mason:/opt/data) and sometimes shared with a
...
# 'mason update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $MASON_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/mason stamp, so deleting the data-dir copy is safe).
if [ -f "$MASON_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$MASON_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$MASON_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$MASON_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$MASON_HOME/$dest"; then
            :
        else
            as_mason cp "$INSTALL_DIR/$src" "$MASON_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/mason/.env.example being present
...
# not generate one. Mason loads $MASON_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$MASON_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$MASON_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $MASON_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$MASON_HOME/.env" ] && ! refuse_symlinked_path "clean" "$MASON_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$MASON_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$MASON_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$MASON_HOME/.env"; then
        :
    else
        if [ ! -f "$MASON_HOME/.env" ]; then
            ...
            (umask 077 && as_mason touch "$MASON_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$MASON_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$MASON_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$MASON_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $MASON_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $MASON_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$MASON_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$MASON_HOME/.env"; then
        :
    else
        chown mason:mason "$MASON_HOME/.env" 2>/dev/null || true
        chmod 600 "$MASON_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `mason` user — without this it gets EACCES on every
...
# mason group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp mason /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted mason group access to the Fly Machines API socket"
    else
...
# $MASON_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `mason update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set MASON_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$MASON_HOME/config.yaml" ]; then
    s6-setuidgid mason "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$MASON_HOME/auth.json" ] && [ -n "${MASON_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$MASON_HOME/auth.json"; then
        :
    else
        printf '%s' "$MASON_AUTH_JSON_BOOTSTRAP" > "$MASON_HOME/auth.json"
        chown mason:mason "$MASON_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$MASON_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via MASON_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$MASON_HOME/auth.json" ] && [ -n "${MASON_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$MASON_HOME/auth.json"; then
        :
    else
        s6-setuidgid mason "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$MASON_HOME/auth.json" \
...
# can set MASON_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# MASON_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$MASON_HOME/gateway_state.json" ] && \
        [ "${MASON_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$MASON_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$MASON_HOME/gateway_state.json"
        chown mason:mason "$MASON_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$MASON_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_mason "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/mason/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Mason spawns for the
...
# main-wrapper.sh propagates it into the supervised ``mason`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-mason, dashboard, etc.).
