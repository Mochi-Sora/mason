#!/bin/sh
# shellcheck shell=sh
# /opt/mason/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching MASON_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `mason` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `mason <args>` (subcommand passthrough)
#
# Drop to mason via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${MASON_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${MASON_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export MASON_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset MASON_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid mason "$@"; exec "$@"; }
...
# an arbitrary non-root, non-mason UID, the bootstrap was skipped and the
# baked image dirs (owned by the mason build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u mason)" ]; then
    cat >&2 <<EOF
[mason] ERROR: container started with --user $cur_uid (an arbitrary, non-mason UID) — not supported.
...
# to the mason user's home before dropping privileges so libraries that
...
_mason_orig_cwd="${MASON_ORIG_CWD:-$PWD}"
...
# the user's command so `mason chat` starts in the Docker -w
