...
# by /etc/cont-init.d/01-mason-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs mason"
...
echo "[mason] WARNING: docker/entrypoint.sh is a deprecated shim under " \
