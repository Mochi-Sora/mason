...
# Mason WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: MASON_TINI_SHIM_TARGET overrides the `/init` path (and
# MASON_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${MASON_TINI_SHIM_TARGET:-/init}"
WRAPPER="${MASON_TINI_SHIM_WRAPPER:-/opt/mason/docker/main-wrapper.sh}"
