...
try:
    ...
    pass  # partial `mason update` — only skips the Windows UTF-8 stdio setup
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``mason -c`` / ``--resume``), or None.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("MASON_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("MASON_SESSION_SOURCE", "")
...
_mason_home = get_mason_home()  # read by agent_init via _ra()._mason_home
_loaded_env_paths = load_mason_dotenv(mason_home=_mason_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[mason-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from mason_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``MASON_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("MASON_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``MASON_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("MASON_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("MASON_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See mason-agent#21444 for symptom history."
        )
