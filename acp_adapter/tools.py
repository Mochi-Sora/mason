"""ACP tool-call helpers for mapping mason tools to ACP ToolKind and building content."""
...
# Mason tool name -> ACP ToolKind (anything unlisted is "other").
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a mason tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Mason tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Mason tool result clearly failed.
...
def build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    """Create a ToolCallStart event for the given mason tool invocation.
