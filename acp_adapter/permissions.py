"""ACP permission bridging for Mason dangerous-command approvals."""
...
# ACP permission option id -> Mason approval result. Ids are stable across the
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    """Return ACP options that match Mason approval semantics."""
    ...
    # DENY override — offering a scope Mason discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, Mason semantics in the id.
...
def _map_outcome_to_mason(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Mason approval strings."""
    ...
    if outcome.option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_MASON.get(outcome.option_id, "deny")
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    """Return a Mason approval callback (``command, description, **kw`` as used by
    ...
    def _callback(command: str, description: str, *, allow_permanent: bool = True,
        ...
        if response is None:
            return "deny"
        return _map_outcome_to_mason(response.outcome, allowed_option_ids={option.option_id for option in options})
