"""CLI entry point for the mason-agent ACP adapter.

Loads ``~/.mason/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: mason acp / mason-acp
"""

# IMPORTANT: mason_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See mason_bootstrap.py for full rationale.
try:
    import mason_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``mason update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Mason modules.
    mason_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from MASON_HOME (default ``~/.mason``)."""
    from mason_cli.env_loader import load_mason_dotenv

    mason_home = get_mason_home()
    loaded = load_mason_dotenv(mason_home=mason_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", mason_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="mason-acp", description="Run Mason Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Mason version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Mason provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.mason/node/ "
...
def _print_version() -> None:
    from mason_cli import __version__ as mason_version
...
def _run_check() -> None:
    ...
    from acp_adapter.server import MasonACPAgent  # noqa: F401

    print("Mason ACP check OK")


def _run_setup() -> None:
    from mason_cli.main import main as mason_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "mason", "model"]
        mason_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting mason-agent ACP adapter")
    ...
    import acp
    from .server import MasonACPAgent
    ...
    if os.environ.get("MASON_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
