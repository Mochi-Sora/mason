"""ACP agent server — exposes Mason Agent via the Agent Client Protocol."""
...
from acp_adapter.commands import MASON_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_mason_interactive_context, set_mason_interactive_context
...
def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
    ...
    if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
        ...
        kind = "standalone"
    if kind == "standalone":
        return {"mason": {"compactionSummary": True}}
    if kind == "merged":
        return {"mason": {"containsCompactionSummary": True}}
    return None
...
class MasonACPAgent(SlashCommandsMixin, acp.Agent):
    """ACP Agent implementation wrapping Mason AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Mason inventory (same substrate
        as ``mason model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from mason_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_mason_session_id: str, previous_mason_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.mason.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_mason_session_id,
                previous_mason_session_id=previous_mason_session_id,
            )
        except Exception:
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_mason_session_id: Optional[str] = None, previous_mason_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_mason_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_mason_session_id or session_id, previous_mason_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["mason-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from mason_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from mason_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="mason-agent", version=MASON_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``MASON_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # MASON_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Mason workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global MASON_CRON_SESSION.
            ...
            stack.callback(reset_mason_interactive_context, set_mason_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "MASON_SESSION_ID", os.environ.get("MASON_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        """Run Mason on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        try:
            ...
            pre_turn_mason_id = getattr(state.agent, "session_id", None)
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_mason_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_mason_id: Any,
        ...
        post_turn_mason_id = getattr(state.agent, "session_id", None)
        if conn and post_turn_mason_id and pre_turn_mason_id and post_turn_mason_id != pre_turn_mason_id:
            try:
                await self._send_session_info_update(
                    session_id, current_mason_session_id=post_turn_mason_id,
                    previous_mason_session_id=pre_turn_mason_id,
                )
            except Exception:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Mason has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
