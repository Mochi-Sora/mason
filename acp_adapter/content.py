"""ACP prompt content blocks -> Mason/OpenAI user-content payloads (text, images, resources)."""
...
def _resource_link_to_parts(block: ResourceContentBlock) -> list[dict[str, Any]]:
    ...
    if path is None:
        return _text_parts(
            **ident, body="[Resource link only; Mason cannot read non-file ACP resource URIs directly.]"
        )
...
def _content_blocks_to_openai_user_content(prompt: list[PromptBlock]) -> str | list[dict[str, Any]]:
    """Convert ACP prompt blocks into a Mason/OpenAI-compatible user content payload."""
