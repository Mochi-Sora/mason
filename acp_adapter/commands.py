...
class SlashCommandsMixin:
    """Slash-command surface for ``MasonACPAgent``; relies on ``_conn``, ``_send``, ``_schedule_soon``,
    ...
    _COMMANDS: dict[str, tuple[str, str, str | None]] = {
        ...
        "version": ("Show Mason version", "Show Mason version", None),
    }
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # and ``/model`` REBUILD the system prompt, so unpinned they'd bake the Mason install tree
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = _expand_acp_enabled_toolsets(getattr(state.agent, "enabled_toolsets", None) or ["mason-acp"])
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Mason Agent v{MASON_VERSION}"
