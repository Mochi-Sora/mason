"""ACP session manager — maps ACP sessions to Mason AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.mason/state.db``) so they
...
from mason_constants import get_mason_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Mason runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["mason-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Mason agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Mason AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.mason/state.db``."""
    ...
    def _get_db(self):
        ...
        minimal test env). ``MASON_HOME`` is resolved here, not via the import-time
        ``DEFAULT_DB_PATH``, so test fixtures that change the env var later are honoured."""
        if self._db_instance is None:
            try:
                from mason_state import SessionDB
                self._db_instance = SessionDB(db_path=get_mason_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["mason-acp"], mcp_server_names=configured_mcp_servers),
        ...
        # picked up by MasonACPAgent._schedule_mcp_late_refresh.
        try:
