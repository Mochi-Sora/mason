"""ACP auth helpers — detect and advertise Mason authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "mason-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Mason runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Mason.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Mason credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Mason using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Mason provider", type="terminal", args=["--setup"],
        description=("Open Mason' interactive model/provider setup in a terminal. "
                     "Use this when Mason has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Mason can resolve any runtime provider credentials."""
