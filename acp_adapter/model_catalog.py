"""ACP model picker: deduplicated ``provider:model`` rows from the Mason inventory + named endpoints."""
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        from mason_cli.config import (get_compatible_custom_providers, is_provider_enabled, load_config)
        from mason_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
        from mason_cli.model_switch_providers import _NativePickerModelList, _fetch_picker_live_models
        from mason_cli.model_switch_providers import _discover_flag
...
def _choice_provider(model_id: str) -> str:
    ...
    if parts[:1] == ["custom"] and len(parts) > 1:
        from mason_cli.models import _configured_custom_provider_ids
...
def build_model_state(model: str, provider: str, base_url: str) -> SessionModelState | None:
    ...
    from mason_cli.models import normalize_provider, provider_label
