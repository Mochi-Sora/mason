...
Additive Mason extension under ACP ``_meta.mason`` (unknown to other clients,
...
rotates the internal Mason head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    db: Any, acp_session_id: str, current_mason_session_id: str, *, previous_mason_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.mason.sessionProvenance`` for an ACP session.

    ``db`` must expose ``get_session``. ``current_mason_session_id`` is the live
    internal id (``state.agent.session_id``); ``previous_mason_session_id`` is
    the id before the most recent turn, supplied by ``prompt()`` to flag a
    rotation. Returns ``None`` if the session can't be read."""
    row = _get_row(db, current_mason_session_id)
    if not row:
        return None
    ...
    root_id, compression_depth, cursor_parent = current_mason_session_id, 0, parent_id
    seen = {current_mason_session_id}
    ...
    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id, "currentMasonSessionId": current_mason_session_id,
        "rootMasonSessionId": root_id, "parentMasonSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root", "compressionDepth": compression_depth,
    }
    if previous_mason_session_id:
        provenance["previousMasonSessionId"] = previous_mason_session_id
        if previous_mason_session_id != current_mason_session_id:
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"mason": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(db, acp_session_id, current_mason_session_id,
                                    previous_mason_session_id=previous_mason_session_id)
    return None if prov is None else {"mason": {"sessionProvenance": prov}}
