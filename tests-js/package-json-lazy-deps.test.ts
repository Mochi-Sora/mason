 ...
 *
 * The root ``package.json`` is installed by ``mason update`` on every user,
 ...
 *   workspace graph entirely. ``mason update`` and ``mason doctor --fix``
 ...
 *   ``mason tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``mason update`` for
 ...
 * fails — read the lazy-install guidance in the ``mason-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in mason_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`mason tools` → Browser Automation → Camofox.'
  )
})
