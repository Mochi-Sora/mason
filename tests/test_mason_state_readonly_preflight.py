...
- files inside the Mason home tree are repaired with ``chmod u+rw``
...
import mason_state
from mason_state import SessionDB, preflight_db_writability
...
def mason_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".mason"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, mason_home):
        db = mason_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, mason_home):
        db = mason_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, mason_home):
        sub = mason_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, mason_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, mason_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, mason_home):
        db = mason_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, mason_home):
        db_path = mason_home / "state.db"
    ...
    def test_sessiondb_actionable_error_outside_home(
        ...
        mason_state._set_last_init_error(None)
        try:
            with pytest.raises(sqlite3.OperationalError) as exc_info:
                SessionDB(db_path)
            msg = str(exc_info.value)
