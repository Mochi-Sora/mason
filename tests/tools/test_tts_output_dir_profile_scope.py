...
``MASON_HOME`` or ``set_mason_home_override()``. The call-time accessor
...
def _reload_tts_tool(import_home: Path, monkeypatch):
    monkeypatch.setenv("MASON_HOME", str(import_home))
...
def test_default_output_dir_follows_contextvar_profile_override(tmp_path, monkeypatch):
    """The web server scopes profiles via set_mason_home_override() rather
