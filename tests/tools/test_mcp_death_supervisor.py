...
def test_leaves_an_unregistered_group_alone_at_eof():
    # The other failure direction, and the more damaging one: a clean Mason
...
# A stand-in for Mason: registers a real child, then blocks forever holding the
...
def test_reaps_the_server_when_the_registering_parent_is_sigkilled(tmp_path):
    ...
    try:
        ...
        assert _wait_gone(victim_pid), (
            "stdio MCP server survived kill -9 of its Mason parent"
        )
    finally:
