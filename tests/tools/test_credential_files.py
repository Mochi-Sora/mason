...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        mason_home.mkdir()
        (mason_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"MASON_HOME": str(mason_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(mason_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.mason/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        (mason_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.mason/skills"


    def test_custom_container_base(self, tmp_path):
        mason_home = tmp_path / ".mason"
        (mason_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"MASON_HOME": str(mason_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.mason")

        assert mounts[0]["container_path"] == "/home/user/.mason/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.mason/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.mason/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.mason/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.mason/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MASON_HOME."""
        monkeypatch.setenv("MASON_HOME", str(tmp_path / ".mason"))
        (tmp_path / ".mason").mkdir()

        # Create a sensitive file one level above mason_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside mason_home
    ...
    def test_nested_subdir_inside_mason_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MASON_HOME must be allowed."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        subdir = mason_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MASON_HOME pointing outside must be rejected."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        monkeypatch.setenv("MASON_HOME", str(mason_home))

        # Create a sensitive file outside mason_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside mason_home pointing outside
        symlink = mason_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes MASON_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, mason_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MASON_HOME."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        ...
        sensitive.write_text("{}")
        self._write_config(mason_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(mason_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MASON_HOME via config must still mount."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        monkeypatch.setenv("MASON_HOME", str(mason_home))

        (mason_home / "oauth.json").write_text("{}")
        self._write_config(mason_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        (mason_home / "cache" / "documents").mkdir(parents=True)
        (mason_home / "cache" / "audio").mkdir(parents=True)
        (mason_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("MASON_HOME", str(mason_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.mason/cache/documents" in paths
        assert "/root/.mason/cache/audio" in paths
        assert "/root/.mason/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_mason_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        # Use legacy dir name with content — get_mason_dir prefers
        # populated old over new.
        legacy_doc = mason_home / "document_cache"
        legacy_img = mason_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(mason_home / "document_cache") in host_paths
        assert str(mason_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.mason/cache/documents" in container_paths
        assert "/root/.mason/cache/images" in container_paths
    ...
    def test_empty_mason_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.mason/attachments" in container_paths
        assert "/root/.mason/images" in container_paths
        assert "/root/.mason/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``MASON_HOME/images``, not
        ...
        """
        mason_home = tmp_path / ".mason"
        (mason_home / "images").mkdir(parents=True)
        monkeypatch.setenv("MASON_HOME", str(mason_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.mason/images" in by_container
        assert by_container["/root/.mason/images"] == str(mason_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        (mason_home / "images").mkdir(parents=True)
        upload = mason_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("MASON_HOME", str(mason_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.mason/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        mason_home = tmp_path / ".mason"
        img_dir = mason_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.mason/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(mason_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.mason/cache/images/x.png"
        assert (mason_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Mason cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        mason_home = tmp_path / ".mason"
        (mason_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        return str(mason_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_mason(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.mason/attachments/drop.zip"

    def test_ssh_maps_to_tilde_mason(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.mason/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        doc_dir = mason_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — MASON_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping MASON_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"MASON_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.mason/google_token.json" in paths
        assert "/root/.mason/.env" not in paths
