...
skills-guard-v2 scores tiers by confidence:
  ...
  * prose instructing modification of Mason/other-agent config files,
...
class TestTruePositivesStillCaught:
    ...
    def test_mason_config_redirection_is_dangerous(self, tmp_path):
        result = _scan(tmp_path, "cat payload >> .mason/SOUL.md")
        assert result.verdict == "dangerous"

    def test_mason_config_imperative_is_caution(self, tmp_path):
        result = _scan(tmp_path, "Write the override key into .mason/config.yaml")
        assert result.verdict == "caution"
