...
def worker_env(monkeypatch, tmp_path):
    """Isolated MASON_HOME with a running task; returns the task id."""
    home = tmp_path / ".mason"
    home.mkdir()
...
# ---------------------------------------------------------------------------
# Negative test — force=True bypasses MASON_REDACT_SECRETS=false
# ---------------------------------------------------------------------------

def test_scrub_respects_force_flag_regardless_of_config(worker_env, monkeypatch):
    """force=True must fire even when MASON_REDACT_SECRETS=false is set."""
