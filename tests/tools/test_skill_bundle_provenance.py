...
def test_install_with_junctioned_skills_dir(served_repo, monkeypatch, tmp_path):
    ...
    entry without a content_hash (which then poisons 'mason skills check').
    """
    from mason_cli.skills_hub import do_install
...
def test_optional_source_upstream_stub_fetches_from_external_repo(tmp_path, monkeypatch):
    """A catalog stub with metadata.mason.upstream installs the upstream repo's
    ...
    assert fetched_ids == ["acme/design-skill/.mason/skills/design"]
    assert bundle is not None
