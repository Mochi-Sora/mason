...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_mason_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["MASON_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_mason_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see MASON_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('MASON_HOME'), "
    ...
    assert result["home"] == str(mason_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `mason` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_mason_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no mason console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``mason``.
    ...
    bin_dir = local_mod._resolve_mason_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "mason.exe" if os.name == "nt" else "mason")
    ):
        pytest.skip("no real mason console-script install available")

    # Simulate the service-manager minimal PATH: mason dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("mason", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("mason", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'mason' must resolve from the child PATH {env.get('PATH')!r}"
    )
