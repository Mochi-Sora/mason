...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("MASON_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("MASON_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Mason secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("MASON_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.mason_xai_user_agent = MagicMock(return_value="mason-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("MASON_LOCAL_STT_LANGUAGE", "fr")
...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``mason tools`` remediation for managed-Nous users), the old
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its mason tools remediation instead of the all-provider install
        hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_nous_tools_enabled", lambda: True
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             ...
             patch(
                 ...
                 lambda what: f"managed route down for {what}; run `mason tools`",
             ):
        ...
        assert "mason tools" in result["error"]
