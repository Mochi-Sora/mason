...
and the shared slash-command handlers (mason_cli/write_approval_commands.py).
...
@pytest.fixture
def mason_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="mason_wa_test_")
    home = os.path.join(d, ".mason")
    os.makedirs(home)
    monkeypatch.setenv("MASON_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import mason_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(mason_home):
...
def test_invalid_subsystem_is_off(mason_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(mason_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(mason_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(mason_home, monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.config.load_config", _boom)
...
def test_handle_approve_all(mason_home):
...
def test_handle_approval_on(mason_home):
    from mason_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(mason_home):
    from mason_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(mason_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(mason_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(mason_home):
