...
Real config files in a temp MASON_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".mason"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "MASON_LOCAL_STT_LANGUAGE",
    ):
