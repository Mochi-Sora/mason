...
A package-manager install (NixOS, and anything else shipping Mason from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.mason/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("mason_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets MASON_MANAGED *and* a writable target.

    Dockerfile sets MASON_LAZY_INSTALL_TARGET and the NixOS container module
    passes MASON_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("mason_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.config.get_managed_system", _raise)
