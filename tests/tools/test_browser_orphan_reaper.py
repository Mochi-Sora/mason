...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning mason PID to write to <session>.owner_pid
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which mason process owns a daemon so that
    concurrent mason processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived mason process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a mason process that stays up
    for days.
    """
