...
the active terminal backend: under docker the mounted cache is at /root/.mason, so a host path in the
...
def test_truncation_footers_render_the_sandbox_visible_cache_path(tmp_path, monkeypatch):
    ...
    for text, subdir in ((web_out, "cache/web"), (snap_out, "cache/web"), (deleg_out, "cache/delegation")):
        ...
        assert path.startswith(f"/root/.mason/{subdir}/"), path
        # The bytes still live on the host under MASON_HOME; only the rendered path is translated.
