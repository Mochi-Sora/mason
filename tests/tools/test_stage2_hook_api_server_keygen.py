...
was gated on ``[ -f "$MASON_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'MASON_HOME="{home}"\n'
        # In tests we run unprivileged; as_mason is a passthrough then.
        'as_mason() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Mason loads $MASON_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/mason/.env.example -> $MASON_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $MASON_HOME/.env from it on first boot"
    )
