...
class TestWrapCommand:
    def test_basic_shape(self):
        ...
        assert "exit $__mason_ec" in wrapped
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        import shlex
        snap = str(tmp_path / "mason-snap-x.sh")
        _q = shlex.quote
        ...
        writer = (
            ...
            f"__mason_snap_tmp=$(mktemp {_tmpl}) && "
            f"{{ export -p > \"$__mason_snap_tmp\" && mv -f \"$__mason_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__mason_snap_tmp\" 2>/dev/null || true; "
            "done"
        )
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"__mason_snap_tmp=$(mktemp {bad_tmp}) && "
            f"{{ export -p > \"$__mason_snap_tmp\" && mv -f \"$__mason_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__mason_snap_tmp\" 2>/dev/null || true"
        )
        self._run(script)
