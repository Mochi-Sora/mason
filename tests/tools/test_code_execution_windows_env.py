...
``mason_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``MASON_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # MASON_* handling (#27303): only the operational allowlist passes;
        # every other MASON_* is dropped (the broad prefix was removed).
        "MASON_HOME": "/home/alice/.mason",        # allowlisted → kept
        "MASON_PROFILE": "default",                 # allowlisted → kept
        "MASON_INTERACTIVE": "1",                   # not allowlisted → dropped
        "MASON_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "MASON_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``mason_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``mason_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_mason_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_mason_tools_module(["terminal"], transport="uds")
...
# let the child import mason_tools, a user script that printed non-ASCII
