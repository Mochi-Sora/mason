...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("mason_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_SESSION_KEY", session_key)
        monkeypatch.setenv("MASON_EXEC_ASK", "1")
        monkeypatch.delenv("MASON_CRON_SESSION", raising=False)
        monkeypatch.setattr(
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("mason-verify-", "mason-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "mason-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/mason-verify-example.py",
            "rm -f /tmp/mason-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../mason-verify-example.py",
            "rm -f /tmp/a/../../tmp/mason-verify-example.py",
            "rm -f /var/tmp/mason-verify-example.py",
            "rm -f /tmp/mason-verify-*",
            "rm -f /tmp/mason-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/mason-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/mason-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\mason-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\mason-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\mason-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"MASON_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.mason/.env",
            "echo x | tee $MASON_HOME/.env",
            'echo x | tee "$MASON_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestMasonConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.mason/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.mason/config.yaml",
            "echo '  mode: off' >> ~/.mason/config.yaml",
            "echo x | tee ~/.mason/config.yaml",
            "echo x | tee $MASON_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.mason/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Mason absolute config.yaml is
        # handled by the project patterns, not the Mason-home rule.
        for cmd in (
            "cat ~/.mason/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.mason/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Mason-home prefixes must fold to ~/ and
    ~/.mason/ in dangerous-command detection.
    ...
    SSH, and Mason config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_mason_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/MASON_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("MASON_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.mason/mason-agent && source venv/bin/activate && python -m mason_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m mason_cli.main gateway run --replace &",
            "nohup python -m mason_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart mason-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("MASON_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("MASON_CRON_SESSION", raising=False)
        monkeypatch.setenv("MASON_GATEWAY_SESSION", "1")
        monkeypatch.setenv("MASON_SESSION_PLATFORM", "telegram")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_CRON_SESSION", "1")
        monkeypatch.setenv("MASON_SESSION_PLATFORM", "telegram")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("MASON_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("MASON_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("MASON_INTERACTIVE", raising=False)
        monkeypatch.setenv("MASON_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("MASON_SESSION_KEY", "test-api-session")
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Mason security config via IFS.
            "sed${IFS}-i ~/.mason/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep mason) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof mason)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof mason_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof mason`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Mason service
    label achieves the same effect as `mason gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_mason_label_detected(self):
        for cmd in (
            "launchctl stop ai.mason.gateway",
            "launchctl kickstart -k system/ai.mason.gateway",
            "launchctl bootout system/ai.mason.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.mason.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Mason labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a mason-gateway identifier, so quoted prose and non-gateway mason
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "mason"/"ai.mason" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("MASON_GATEWAY_SESSION", "MASON_CRON_SESSION",
                      "MASON_YOLO_MODE",
                      "MASON_SESSION_KEY", "MASON_INTERACTIVE")
        }
        os.environ.pop("MASON_YOLO_MODE", None)
        os.environ.pop("MASON_INTERACTIVE", None)
        # MASON_CRON_SESSION takes priority over MASON_GATEWAY_SESSION in
        ...
        os.environ.pop("MASON_CRON_SESSION", None)
        os.environ["MASON_GATEWAY_SESSION"] = "1"
        os.environ["MASON_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("mason_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"MASON_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.mason.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.mason.gateway.plist",
    "launchctl stop ai.mason.gateway",
    "launchctl restart ai.mason.gateway",
    "launchctl bootout gui/501/ai.mason.gateway",
    "launchctl remove ai.mason.gateway",
    "launchctl disable gui/501/ai.mason.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_MASON_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Mason ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.mason.update-checker",
