...
PROMPT = "Mason, Teknium, Nous Research, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("mason_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("mason_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.mason_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    mason_home = Path(os.environ["MASON_HOME"])
    plugin_dir = mason_home / "plugins" / "stt_vocab"
