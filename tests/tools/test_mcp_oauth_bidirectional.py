"""Regression test for the ``MasonMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_mason_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _MASON_PROVIDER_CLS, reset_manager_for_tests

    assert _MASON_PROVIDER_CLS is not None, "SDK OAuth types must be available"
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Mason Agent",
    )
    provider = _MASON_PROVIDER_CLS(
        server_name="srv",
...
@pytest.mark.asyncio
async def test_mason_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _MASON_PROVIDER_CLS is not None
...
async def test_long_lived_resource_request_does_not_block_concurrent_post(
    ...
    acquiring its Bearer token.  Mason narrows that lock around resource I/O
