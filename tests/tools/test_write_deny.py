...
class TestWriteDenyExactPaths:
    ...
    def test_mason_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.mason/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "mason_root"
        ...
        # Sanity check: MASON_HOME does point to the profile dir, not the root.
        from mason_constants import get_mason_home, get_default_mason_root
...
class TestWriteAllowed:
    ...
    def test_mason_control_files_requested_writable(self):
