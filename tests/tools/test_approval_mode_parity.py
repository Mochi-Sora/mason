...
def test_mode_and_timeout_parity_across_surfaces(
    mason_home, tui_server, yaml_text, expected_mode, expected_timeout
):
    approval_mod = _approval_module()

    _write_config(mason_home, yaml_text)
...
def test_tui_loader_delegates_to_core(mason_home, tui_server):
