...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "mason", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@mason", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("mason@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("mason@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "mason" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "mason", "connection_id": "cloud-1",
         "connection_label": "Mason Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="mason", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "Mason Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 mason (@mason): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@mason` — on Mason Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="mason",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("MASON_HOME", str(tmp_path / "nope"))
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "mason", "connection_id": "cloud-1",
         "connection_label": "Mason Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
