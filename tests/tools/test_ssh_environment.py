...
class TestBuildSSHCommand:
    ...
    def test_run_bash_forwards_passthrough_by_sendenv_never_in_remote_argv(self, monkeypatch):
        ...
        monkeypatch.setattr(ssh_env, "_load_mason_env_vars", lambda: {"NEXTCLOUD_PASS": "from-dotenv"})
...
class TestOneShotSSH:
    ...
    def test_state_does_not_persist(self):
        _run("export MASON_ONESHOT_TEST=yes")
        r = _run("echo $MASON_ONESHOT_TEST")
...
class TestPersistentSSH:
    ...
    def test_env_var_persists(self):
        _run("export MASON_PERSIST_TEST=works")
        r = _run("echo $MASON_PERSIST_TEST")
