"""Tests for tools.wake_word — the "Hey Mason" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey mason"}) == "hey mason"
    assert ww.wake_phrase({}) == "hey mason"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_mason.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "mason_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey mason"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey mason"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_mason": object()}

        def predict(self, frame):
            return {"hey_mason": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_mason.onnx"}}
    )
    assert calls["download"] == [["/models/hey_mason.onnx"]]
    assert eng._labels == ["hey_mason"]


def test_bundled_hey_mason_model_ships_on_disk():
    # The "hey mason" wake word works out of the box only if the model is
