...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("mason_cli.config.load_config", side_effect=_boom):
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in mason_cli.config must expose tool_output so
    that ``mason setup`` and default installs stay in sync with the
    helpers here."""
