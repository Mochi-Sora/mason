...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad MASON_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MASON_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_mason_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "MASON_HOME": "/h", "MASON_PROFILE": "p",
        "MASON_CONFIG": "/c.yaml", "MASON_ENV": "/e",
        "MASON_DELEGATED_CHILD_CONTEXT": "1",
        # other MASON_* → dropped (broad prefix removed)
        "MASON_BASE_URL": "https://x", "MASON_INTERACTIVE": "1",
        "MASON_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "MASON_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "MASON_BASE_URL", "MASON_INTERACTIVE", "MASON_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MASON_* vars."""
    import logging
