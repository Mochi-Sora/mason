...
def test_output_path_rejects_mason_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    mason_home = tmp_path / "mason-home"
    mason_home.mkdir()
    monkeypatch.setattr(file_safety, "_mason_home_path", lambda: mason_home)
    monkeypatch.setattr(file_safety, "_mason_root_path", lambda: mason_home)

    target = mason_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    mason_home = tmp_path / "mason-home"
    token_dir = mason_home / "mcp-tokens"
