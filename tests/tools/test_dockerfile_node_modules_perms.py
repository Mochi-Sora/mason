...
Older images made /opt/mason/ui-tui and /opt/mason/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV MASON_TUI_DIR=/opt/mason/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R mason:mason /opt/mason/ui-tui" not in text
    assert "chown -R mason:mason /opt/mason/node_modules" not in text
