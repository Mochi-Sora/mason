...
Verifies:
  - Tools are gated on MASON_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `mason chat` sessions (no MASON_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("mason-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: MASON_HOME isolated, MASON_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".mason"
    home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(home))
    monkeypatch.setenv("MASON_PROFILE", "test-worker")
    monkeypatch.delenv("MASON_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("MASON_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("MASON_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated MASON_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to MASON_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``MASON_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``mason-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "mason-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from MASON_PROFILE in the fixture, not the
        # caller-supplied "mason-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(home))
    monkeypatch.setenv("MASON_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has MASON_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no MASON_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # MASON_PROFILE — never to a caller-controlled string.
    conn = kbc.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no MASON_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via MASON_KANBAN_BOARD env var,
...
# restarting Mason. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Mason home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    MASON_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".mason"
    home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(home))
    # Make sure neither MASON_KANBAN_DB nor MASON_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("MASON_KANBAN_DB", raising=False)
    monkeypatch.delenv("MASON_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("MASON_KANBAN_TASK", raising=False)
    monkeypatch.setenv("MASON_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: MASON_SESSION_PLATFORM + MASON_SESSION_CHAT_ID set.
# - TUI sessions: MASON_SESSION_KEY (or MASON_SESSION_ID) set, with
...
def _list_subs_for_task(task_id):
    from mason_cli import kanban_db as kb
    from mason_cli import kanban_db_connect as kbc
    from mason_cli import kanban_db_notify as kbn
    conn = kbc.connect()
    try:
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("MASON_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("MASON_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("MASON_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("MASON_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("MASON_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports MASON_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("MASON_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("MASON_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("MASON_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("MASON_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("MASON_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("MASON_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("MASON_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("MASON_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("MASON_SESSION_KEY", raising=False)
    monkeypatch.delenv("MASON_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.mason; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".mason"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("MASON_HOME", str(home))
    monkeypatch.setenv("MASON_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("MASON_SESSION_CHAT_ID", "channel-1")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting MASON_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("MASON_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears MASON_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("MASON_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
