...
UDS socket lifecycle, mason_tools generation, timeout enforcement,
...
class TestMasonToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_mason_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"mason_rpc\")", src)
        self.assertNotIn('os.environ.get("MASON_RPC_DIR", "/tmp/mason_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_mason_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "mason_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "mason_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/mason_exec_", mkdir_cmd)
        self.assertIn("MASON_RPC_DIR=/data/data/com.termux/files/usr/tmp/mason_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/mason_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/mason_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """MASON_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"MASON_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import mason_constants; print(mason_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("mason_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from mason_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from mason_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_mason_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("MASON_RPC_SOCKET", child_env)
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching MASON_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated mason_tools module reads MASON_RPC_TOKEN and sends it."""
        src = generate_mason_tools_module(["terminal"], transport="uds")
        self.assertIn("MASON_RPC_TOKEN", src)
