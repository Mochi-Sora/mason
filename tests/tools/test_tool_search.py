...
class TestClassification:
    def test_core_tools_defer_by_default(self):
        """Mason cold start: core tools defer (working set re-injects on use).
        Only the eager allowlist (clarify) stays native. Inverts the pre-Mason
        OpenClaw invariant on purpose — see tools/tool_search._EAGER_TOOL_NAMES."""
        from tools.tool_search import is_deferrable_tool_name
        # Sample of core tools from _MASON_CORE_TOOLS.
        for core_name in ["terminal", "read_file", "write_file", "patch",
            ...
            assert is_deferrable_tool_name(core_name), (
                f"Core tool '{core_name}' must defer by default"
            )
        assert not is_deferrable_tool_name("clarify")
    ...
    def test_gui_surface_tools_defer_by_default(self):
        """Session-gated GUI tools defer like everything else, and stay off
        ...
        for name in ("read_window_below", "apply_layout", "project_list"):
            ...
            assert name not in _MASON_CORE_TOOLS
    ...
    def test_gui_surface_defers_by_default(self):
        ...
        # Mason default: project_list defers too (served via the bridge).
    ...
    def test_core_tools_defer_even_with_mcp_active(self):
        """The bridge activates for MCP, and working-set core tools (terminal,
        files, memory...) defer with it — serving happens via describe/call,
        and the session working set re-injects whatever gets used."""
        ...
        # Mason default: terminal/memory/computer_use all defer (bridge-served).
    ...
    def test_unknown_tool_deferrable_but_undispatchable(self):
        """Mason default cannot distinguish unregistered names, so they read as
        deferrable — the DROP protection moved to dispatch: scoped membership
        still refuses them (never silently served). Explicit defer=[] keeps
        the legacy eager answer."""
        from tools.tool_search import is_deferrable_tool_name
        assert is_deferrable_tool_name("xx_definitely_not_a_tool_xx")
        assert not is_deferrable_tool_name("xx_definitely_not_a_tool_xx", frozenset())

    def test_classify_never_drops_unknown(self):
        """A tool we can't classify is bridge-served, never silently dropped.

        OpenClaw #84141 guard, Mason edition: cron lost ``exec`` because it
        wasn't in the catalog. Now unclassified defs land in the deferrable
        bucket (reachable via describe/call), not in the void.
        """
        ...
        kept = {(td.get("function") or {}).get("name") for td in visible + deferrable}
        assert "xx_unknown_tool" in kept
        dnames = {(td.get("function") or {}).get("name") for td in deferrable}
        assert "xx_unknown_tool" in dnames
...
class TestAssembly:
    def test_no_deferrable_returns_unchanged(self):
        """Only the eager allowlist passes through untouched: clarify-only in,
        clarify-only out, no bridge tools added."""
        ...
        assert {t["function"]["name"] for t in result.tool_defs} == {"clarify"}
...
class TestBridgeDispatch:
    ...
    def test_resolve_underlying_call_parses_object_args(self):
        ...
        # Mason default: unknown names read as deferrable (parsed fine here);
        # the session scope gate refuses them at dispatch, not here.
        assert err is None
...
class TestHandleFunctionCallIntegration:
    ...
    def test_tool_search_emits_one_terminal_hook(self, monkeypatch):
        ...
        import model_tools
        from mason_cli import lifecycle
...
class TestRegression_OpenClawCron84141:
    ...
    def test_core_tool_survives_alongside_many_mcp_tools(self):
        ...
        # 1 core tool: Mason default puts terminal behind the bridge (served,
        # not dropped) — the working set re-injects it natively on first use.
        defs = [_td("terminal", "Run shell commands")]
        visible, deferrable = classify_tools(defs)
        assert any(
            (td.get("function") or {}).get("name") == "terminal"
            for td in deferrable
        ), "Core tool 'terminal' must be bridge-served, never dropped"
        ...
        # terminal rides the bridge; bridge tools are present to serve it.
        assert "terminal" not in names
        assert "tool_describe" in names and "tool_call" in names

    def test_unwrap_accepts_core_tool_attempt(self):
        """Mason cold start: invoking a core tool through tool_call is the
        NORMAL path (it defers by default) — it resolves, then joins the
        session working set for native use afterwards."""
        from tools.tool_search import resolve_underlying_call
        name, args, err = resolve_underlying_call({
            "name": "terminal",
