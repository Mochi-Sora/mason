"""Tests for mason_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_MASON_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
from tools.environments.local_env_policy import _ALWAYS_STRIP_KEYS, _MASON_PROVIDER_ENV_FORCE_PREFIX
...
_TIER1_SAMPLE = {
    ...
    "MASON_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return mason_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_buzz_platform_vars_stripped_by_default(self):
        ...
        for var in buzz_sample:
            assert var not in result, f"{var} leaked via mason_subprocess_env"
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "MASON_KANBAN_TASK": "t_parent",
                "MASON_KANBAN_RUN_ID": "123",
                "MASON_KANBAN_DB": "/tmp/parent-kanban.db",
                "MASON_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
