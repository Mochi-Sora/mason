...
Verifies that Mason-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Mason secrets.

See: https://github.com/NousResearch/mason-agent/issues/1002
See: https://github.com/NousResearch/mason-agent/issues/1264
"""
...
from tools.environments.local_env_policy import (
    _MASON_PROVIDER_ENV_BLOCKLIST,
    _MASON_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.mason/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Mason inference secret
        ...
        models`` run inside a Mason terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _MASON_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Mason-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "MASON_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_mason_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Mason even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_mason_bin_dir",
    ...
    def test_bare_mason_path_does_not_duplicate_existing_install_dir(self):
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _MASON_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, mason_subprocess_env,
    ...
    def test_buzz_vars_stripped_without_buzz_context(self, monkeypatch):
        ...
        monkeypatch.delenv("MASON_SESSION_PLATFORM", raising=False)
        buzz_vars = {
            "BUZZ_PRIVATE_KEY": "nsec1faketestkey",
            "BUZZ_AUTH_TAG": '["tag","data","kind","sig"]',
            "BUZZ_RELAY_URL": "https://mycommunity.communities.buzz.xyz",
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        mason_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in mason-snap-<id>.sh
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _MASON_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_MASON_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_MASON_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_MASON_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_MASON_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Mason venv path instead of the project's own ``.venv`` —
    silently clobbering the Mason environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Mason venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.mason/mason-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/mason",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "mason-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Mason-owned entry stripping (#74817).

    The Desktop Electron app injects the Mason repo root and the Mason
    ...
    ``_strip_mason_owned_pythonpath`` surgically removes only the
    entries Mason itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Mason-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _mason_repo_root
        cannot silently pass), duplicate Mason entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Mason ordering with an
        empty component preserved.
        """
        from tools.environments.local_pythonpath import _strip_mason_owned_pythonpath
        ...
        for input_entries, expected in cases:
            ...
            _strip_mason_owned_pythonpath(env)
            if expected is None:
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Mason-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Mason venv
        ...
        off-by-one in _mason_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Mason-owned entries.
        ...
        Mason-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Mason venv site-packages entries are stripped
        by the same Mason-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local_pythonpath import _strip_mason_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        mason_win = f"C:\\\\Users\\\\u\\\\.mason\\\\mason-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([mason_win, user_win]),
        }
        ...
        # the Mason-owned check cannot match a backslash path here.
        assert mason_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_mason_owned_paths_stripped(self):
        """On Windows, a Mason venv site-packages entry written with
        backslashes is stripped by the same Mason-owned check, while a
        ...
        _strip_mason_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert mason_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Mason venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Mason runtime producer contract.
        """
        import tools.environments.local as local
        from tools.environments import local_pythonpath

        repo_root = tmp_path / "mason-agent"
        ...
        monkeypatch.setattr(local, "_mason_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_mason_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local_pythonpath._strip_mason_owned_pythonpath(env)
    ...
    @pytest.mark.parametrize("builder", [
        ...
        "mason_subprocess_env",
    ])
    def test_builders_strip_mason_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        Mason venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.mason_subprocess_env()
    ...
    def test_scrub_child_env_strips_mason_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Mason venv
        site-packages entries should be stripped when
        _strip_mason_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_mason_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_mason_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (Mason repo root + Mason
        ...
        * inherited Mason venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        mason_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_env._uses_mason_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [mason_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(mason_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Mason venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from tools.environments import local_pythonpath
        from mason_cli.gateway_windows import _preserve_mason_home_path
        ...
        monkeypatch.setenv("MASON_HOME", str(configured_home))

        launcher_entry = Path(_preserve_mason_home_path(physical_root))
        aliases = local_pythonpath._build_mason_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "mason-agent"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_mason_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with MASON_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home MASON_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "mason-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("MASON_HOME", str(configured_home))
        lexical_root = configured_home / "mason-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) MASON_HOME.
        aliases = local_pythonpath._build_mason_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\mason\\mason-agent -> C:\\...\\mason-agent) while the
        ...
        # repo-level link: <configured-home>/mason-agent -> physical repo
        try:
            _make_directory_link(configured_home / "mason-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "mason-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "mason-agent"
        ...
        for lookalike in (configured_home / "mason-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/mason-agent.
        ...
        try:
            _make_directory_link(configured_root / "mason-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "mason-agent"
        aliases = local_pythonpath._build_mason_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local_pythonpath._same_path(a, lexical_root) for a in aliases)
        assert not any(local_pythonpath._same_path(a, configured_home / "mason-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._mason_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Mason runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Mason venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "mason_subprocess_env":
                result = local_mod.mason_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/mason-venv",
            "VIRTUAL_ENV": "/opt/mason-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/mason-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/mason-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_MASON_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Mason (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _MASON_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _MASON_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Mason-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _MASON_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Mason, and blocklisting
        ...
        leaked_block = general_chain & _MASON_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _MASON_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        from mason_cli.config import OPTIONAL_ENV_VARS

        for name, metadata in OPTIONAL_ENV_VARS.items():
            category = metadata.get("category")
            if category in {"tool", "messaging"}:
                assert name in _MASON_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_mason_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        mason-install-dir prepend (a separate concern, covered by
        TestMasonBinDirOnPath) so a real ``mason`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._MASON_BIN_DIR
        local_mod._MASON_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._MASON_BIN_DIR = saved
...
class TestMasonBinDirOnPath:
    """The mason install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``mason`` via the terminal tool must work
    even when the gateway was launched without the mason install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_mason_bin_dir / _prepend_mason_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._MASON_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/mason/bin/mason" if name == "mason" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/mason/bin")
        assert local_mod._resolve_mason_bin_dir() == "/opt/mason/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._MASON_BIN_DIR = None
        assert local_mod._prepend_mason_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_mason_bin_dir(self):
        """A gateway env missing the mason dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_mason_bin_dir`` uses ``os.pathsep`` on
        every host, so no platform flag is faked here."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._MASON_BIN_DIR = "/opt/mason/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/mason/bin"
        assert "/usr/bin" in entries


class TestMasonInternalDynamicSecrets:
    """Dynamically-named Mason secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_MASON_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_mason_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``mason_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local_env_policy import _is_mason_internal_secret
        assert _is_mason_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_mason_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_mason_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_mason_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local_env_policy import _is_mason_internal_secret
        assert _is_mason_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_mason_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local_env_policy import _is_mason_internal_secret
        assert _is_mason_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_mason_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_mason_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local_env_policy import _is_mason_internal_secret
        assert not _is_mason_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_mason_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_mason_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_mason_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_mason_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_mason_internal_secret("PATH")
        assert not _is_mason_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _MASON_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _MASON_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _MASON_PROVIDER_ENV_BLOCKLIST
