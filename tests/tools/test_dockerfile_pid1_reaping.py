...
- Signal forwarding runs through the init so ``docker stop`` triggers
  mason's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, mason accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, mason accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and mason would run without zombie reaping.
    """
    ...
    assert "/opt/mason/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/mason/docker/main-wrapper.sh" in dispatcher, (
