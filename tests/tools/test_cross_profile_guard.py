...
@pytest.fixture
def fake_mason(tmp_path, monkeypatch):
    """Build a two-profile Mason layout and point MASON_HOME at
    the mason-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-mason"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "mason-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("MASON_HOME", str(sec_home))

    import mason_constants
    monkeypatch.setattr(mason_constants, "get_default_mason_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_mason_home_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_mason_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_mason):
        from tools.file_tools import write_file_tool
        target = fake_mason["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_mason):
        ...
        from tools.file_tools import write_file_tool
        target = fake_mason["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_mason_path_unaffected(self, fake_mason, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_mason):
    ...
    def test_cross_profile_patch_bypass(self, fake_mason):
        from tools.file_tools import patch_tool
        target = fake_mason["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_mason):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_mason, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_mason["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up MASON_HOME (set in
        ...
        assert "not found in active profile 'mason-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.mason/profiles/<name>/."""
        # Don't set MASON_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_mason_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_mason_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_mason):
        """When active profile is 'mason-security', the prompt warns
        ...
        assert "Active Mason profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.mason/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Mason profile: default" in src
        assert "Active Mason profile: {active_profile}" in src
