...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects MASON_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("MASON_INTERACTIVE", "MASON_GATEWAY_SESSION", "MASON_EXEC_ASK", "MASON_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithAllowSafeCommand:
    ...
    def test_both_allow(self, mock_tirith):
        os.environ["MASON_INTERACTIVE"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["MASON_INTERACTIVE"] = "1"
        session_key = os.getenv("MASON_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No MASON_INTERACTIVE or MASON_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
