...
def test_resolve_stdio_command_falls_back_to_mason_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool_config.shutil.which", return_value=None), \
         patch.dict("os.environ", {"MASON_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$MASON_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Mason Docker image
    ...
    # the other candidates ($MASON_HOME/node/bin/npx and ~/.local/bin/npx)
