...
Covers:
- :class:`MasonTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.MasonMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _MASON_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# MasonTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = MasonTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MASON_HOME", str(tmp_path))
        storage = MasonTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (MasonMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: MasonTokenStorage, **context_attrs):
    ...
    provider = _MASON_PROVIDER_CLS.__new__(_MASON_PROVIDER_CLS)
    provider._mason_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(tmp_path))
        storage = MasonTokenStorage("mgr-srv")
        ...
        with patch.object(
            _MASON_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(tmp_path))
        storage = MasonTokenStorage("flow-srv")
        ...
        with patch.object(
            _MASON_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
