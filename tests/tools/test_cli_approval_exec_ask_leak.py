...
When ``MASON_EXEC_ASK`` leaks into a classic CLI process (historically via
...
class TestExecuteCodeGuardCliApprovalSurvivesExecAskLeak:
    ...
    def test_cli_callback_used_for_platform_marker_leak_without_exec_ask(
        ...
        ``MASON_SESSION_PLATFORM`` is set, so the whole-script gate is
        reached with ``MASON_EXEC_ASK`` entirely absent. That path must
...
print("EXEC_ASK=" + repr(os.environ.get("MASON_EXEC_ASK")))
""" % (str(REPO_ROOT),)
        mason_home = tmp_path / "import-test-home"
        proc = subprocess.run(
