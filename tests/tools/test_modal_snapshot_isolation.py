...
def _restore_tool_modules():
    original_mason_home = os.environ.get("MASON_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_mason_home is None:
            os.environ.pop("MASON_HOME", None)
        else:
            os.environ["MASON_HOME"] = original_mason_home
        _reset_modules(("tools", "mason_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    mason_cli = types.ModuleType("mason_cli")
    mason_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["mason_cli"] = mason_cli
    mason_home = tmp_path / "mason-home"
    os.environ["MASON_HOME"] = str(mason_home)
    sys.modules["mason_cli.config"] = types.SimpleNamespace(
        get_mason_home=lambda: mason_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="mason-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": mason_home / "modal_snapshots.json",
