...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `mason-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.mason/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.mason/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because mason uses an internal scheduler, not system crontab.
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["MASON_INTERACTIVE", "MASON_GATEWAY_SESSION", "MASON_EXEC_ASK"],
    )
    ...
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("MASON_INTERACTIVE", "MASON_GATEWAY_SESSION", "MASON_EXEC_ASK"):
...
class TestAgentCannotSetModelPin:
    """Per-job inference pins are user-owned (dashboard / `mason cron`
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.mason/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.mason/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.mason/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.mason/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
