...
Mason exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_mason_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    MASON_HOME (cache/terminal), NOT tmpfs /tmp."""
    import mason_constants  # noqa: F401 — resolves MASON_HOME per call
    ...
    assert result == str(tmp_path / ".mason" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "mason_bg_dead1.log").exists()
