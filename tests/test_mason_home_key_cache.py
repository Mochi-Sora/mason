"""Tests for the remembered results in `mason_home_key`.

`Path.resolve()` is a filesystem call. `mason_home_key` sits under
...
class TestSameAnswerAsBefore:
    ...
    def test_matches_the_uncached_calculation(self, tmp_path, case):
        ...
        assert hc.mason_home_key(target) == _uncached(target)

    def test_matches_for_the_default_home(self):
        assert hc.mason_home_key() == _uncached()
    ...
    def test_accepts_a_path_object(self, tmp_path):
        (tmp_path / "real").mkdir()
        assert hc.mason_home_key(tmp_path / "real") == _uncached(tmp_path / "real")

    def test_second_call_returns_the_same_string(self, tmp_path):
        (tmp_path / "real").mkdir()
        first = hc.mason_home_key(str(tmp_path / "real"))
        second = hc.mason_home_key(str(tmp_path / "real"))
...
class TestWhatGetsRemembered:
    def test_an_existing_path_is_remembered(self, tmp_path):
        (tmp_path / "real").mkdir()
        hc.mason_home_key(str(tmp_path / "real"))
    ...
    def test_a_missing_path_is_not_remembered(self, tmp_path):
        ...
        hc.mason_home_key(str(tmp_path / "not_there"))
        assert hc._HOME_KEY_CACHE == {}

    def test_a_path_created_later_picks_up_the_real_answer(self, tmp_path):
        later = tmp_path / "later"
        before = hc.mason_home_key(str(later))
        later.mkdir()
        after = hc.mason_home_key(str(later))
    ...
    def test_different_paths_get_their_own_entries(self, tmp_path):
        for name in ("a", "b", "c"):
            (tmp_path / name).mkdir()
            hc.mason_home_key(str(tmp_path / name))
...
class TestHomeChanges:
    def test_pointing_mason_home_somewhere_else_gives_a_new_key(
        ...
        monkeypatch.setenv("MASON_HOME", str(first))
        key_one = hc.mason_home_key()
        monkeypatch.setenv("MASON_HOME", str(second))
        key_two = hc.mason_home_key()
...
class TestSymlinks:
    def test_a_link_resolves_to_its_target(self, tmp_path):
        ...
        assert hc.mason_home_key(str(link)) == _uncached(str(link))
        assert hc.mason_home_key(str(link)) == hc.mason_home_key(str(target))
