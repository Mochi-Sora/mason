"""display_mason_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom MASON_HOME under
the user profile (e.g. AppData/Local/mason), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\mason`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\mason/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayMasonHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "mason"
        with patch.object(hc, "get_mason_home", return_value=nested):
            out = hc.display_mason_home()
        self.assertEqual(out, "~/AppData/Local/mason")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import mason_constants as hc

        with patch.object(hc, "get_mason_home",
                          return_value=Path.home() / ".mason"):
            out = hc.display_mason_home()
        self.assertEqual(out, "~/.mason")

    def test_outside_home_falls_back_to_absolute(self):
        import mason_constants as hc

        outside = Path("/opt/mason-custom") if os.name != "nt" else Path("C:/opt/mason-custom")
        with patch.object(hc, "get_mason_home", return_value=outside):
            out = hc.display_mason_home()
