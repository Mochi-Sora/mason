...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_mason_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "mason_constants.get_default_mason_root", lambda: root
    )
    token = set_mason_home_override(str(grant_home))
...
class TestRoutedSatellitePreflight:
    ...
    def test_primary_home_itself_skips_route_lookup(self, multiplex_homes):
        ...
        token = set_mason_home_override(str(root))
        try:
            assert _delivery_platform_routed_from_primary_gateway("telegram") is False
        finally:
            reset_mason_home_override(token)
