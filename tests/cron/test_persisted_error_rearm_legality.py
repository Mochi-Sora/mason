...
def cron_store(tmp_path, monkeypatch):
    ...
    (mason_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    monkeypatch.setattr(J, "MASON_DIR", mason_home)
    monkeypatch.setattr(J, "CRON_DIR", mason_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", mason_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", mason_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_mason_now", lambda: now):
            due = J.get_due_jobs()
