...
def mason_env(tmp_path, monkeypatch):
    """Isolate MASON_HOME for each test so jobs/scripts/snapshots don't leak."""
    home = tmp_path / ".mason"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
    ...
    # Reload modules that cache get_mason_home() at import time.
    import importlib
...
def _install_agent_stubs(monkeypatch, observed: dict):
    ...
    from mason_cli import runtime_provider as _rtp
    monkeypatch.setattr(
        _rtp,
    ...
    monkeypatch.setenv("MASON_CRON_TIMEOUT", "0")
...
def test_create_job_stores_monitor_script(mason_env):
    from cron.jobs import create_job, get_job

    _write_script(mason_env, "mon.sh", "echo stable\n")
    job = create_job(
...
def test_create_job_monitor_script_and_url_mutually_exclusive(mason_env):
...
def test_create_job_monitor_rejected_with_no_agent(mason_env):
    from cron.jobs import create_job

    _write_script(mason_env, "w.sh", "echo hi\n")
...
def test_update_job_rejects_no_agent_on_monitor_job(mason_env):
...
def test_update_job_rejects_adding_monitor_to_no_agent_job(mason_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_rejects_second_monitor_source(mason_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_allows_clearing_monitor_then_no_agent(mason_env):
...
def test_update_job_unrelated_fields_skip_mode_validation(mason_env):
...
def test_hash_is_exact_bytes(mason_env):
...
def test_unified_diff_is_capped(mason_env):
...
def _make_monitor_job(mason_env, script_body: str):
    from cron.jobs import create_job

    _write_script(mason_env, "mon.sh", script_body)
    return create_job(
...
def test_first_run_always_runs_agent(mason_env, monkeypatch):
    from cron.scheduler import run_job

    job = _make_monitor_job(mason_env, "echo 'state A'\n")
    observed: dict = {}
...
def test_unchanged_output_suppresses_agent_run(mason_env, monkeypatch):
...
def test_changed_output_injects_diff(mason_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(mason_env, "mon.sh", "echo 'state B'\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
    assert success is True
...
def test_hash_persists_across_scheduler_restart(mason_env, monkeypatch):
...
def test_monitor_script_failure_is_error_not_change(mason_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(mason_env, "mon.sh", "echo boom >&2\nexit 3\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
...
def test_cronjob_tool_create_with_monitor_script(mason_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob

    _write_script(mason_env, "mon.sh", "echo hi\n")
    result = json.loads(
        cronjob(
            action="create",
            prompt="React",
...
def test_cronjob_tool_rejects_monitor_script_path_escape(mason_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_clears_monitor_script(mason_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob
