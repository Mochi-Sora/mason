...
``mason_time`` under the process's own startup profile, then ticks OTHER
profiles' cron stores via ``set_mason_home_override()`` + ``use_cron_store()``.
...
def test_foreign_process_tick_persists_owning_profile_offset(
    ...
    monkeypatch.setenv("MASON_HOME", str(backend_home))

    # Backend process resolves its own timezone first (process startup).
    assert mason_time.now().utcoffset() == datetime.now(
        ZoneInfo("UTC")
    ).utcoffset()
    ...
    token = set_mason_home_override(str(profile_a))
    try:
