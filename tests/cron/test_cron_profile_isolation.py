...
profile's MASON_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.mason/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active MASON_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever MASON_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    MASON_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import mason_constants

    monkeypatch.setattr(
        mason_constants, "_get_platform_default_mason_home", lambda: root
    )
    monkeypatch.setenv("MASON_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile MASON_HOME (<root>/profiles/<name>), the cron store
    ...
    assert mason_constants.get_mason_home().resolve() == profile_home.resolve()
    assert mason_constants.get_default_mason_root().resolve() == root.resolve()

    # cron/jobs.py computes MASON_DIR from get_mason_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.MASON_DIR.resolve() == profile_home.resolve()
        assert (
