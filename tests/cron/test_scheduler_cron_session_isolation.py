...
def test_run_job_cron_execute_code_deny_does_not_pollute_later_gateway_execute_code(
    ...
    monkeypatch.setenv("MASON_MODEL", "test-model")
    monkeypatch.setattr(approval_module, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setattr(approval_context, "_get_approval_mode", lambda: "manual")
    monkeypatch.setattr(approval_context, "_get_cron_approval_mode", lambda: "deny")
    monkeypatch.setattr("mason_state_registry.acquire", _DummySessionDB)
    monkeypatch.setattr("run_agent.AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "mason_constants.resolve_reasoning_config", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_scheduler, "_get_mason_home", lambda: tmp_path)
    ...
    assert os.environ.get("MASON_CRON_SESSION") is None
    ...
    assert get_session_env("MASON_CRON_SESSION") == "1"
    monkeypatch.delenv("MASON_CRON_SESSION")
