...
- Path resolution (absolute, relative to MASON_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MASON_HOME."""
    mason_home = tmp_path / ".mason"
    mason_home.mkdir()
    (mason_home / "cron").mkdir()
    (mason_home / "cron" / "output").mkdir()
    (mason_home / "scripts").mkdir()
    monkeypatch.setenv("MASON_HOME", str(mason_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MASON_DIR", mason_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", mason_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", mason_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", mason_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Mason provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _MASON_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_MASON_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.mason/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MASON_SESSION_PLATFORM") is None
        assert os.environ.get("MASON_SESSION_CHAT_ID") is None
        assert os.environ.get("MASON_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("MASON_CRON_SCRIPT_TIMEOUT", "2")
