...
kind) on every recurring dispatch so `mason cron list` and
`mason cron status` can surface late catch-ups.
"""
...
def cron_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cron.jobs._mason_now", lambda: FIXED_NOW)
    return tmp_path
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `mason cron list` process can read it.
