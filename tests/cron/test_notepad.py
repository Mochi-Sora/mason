...
use the notepad, and the `mason cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp MASON_HOME (mirrors test_cron_context_from)."""
...
class TestNotepadProfileIsolation:
    def test_profile_override_routes_writes_to_current_home(self, tmp_path):
        ...
        import_token = set_mason_home_override(profile_a)
        try:
            importlib.reload(notepad_mod)
        finally:
            reset_mason_home_override(import_token)

        runtime_token = set_mason_home_override(profile_b)
        try:
            notepad_mod.set_note("job-1", "cursor", "page=7")
        finally:
            reset_mason_home_override(runtime_token)
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"mason cron notepad {job['id']} set" in prompt
...
class TestNotepadCli:
    ...
    def test_cli_set_get_list_delete(self, notepad, capsys):
        from mason_cli.cron import cron_notepad
