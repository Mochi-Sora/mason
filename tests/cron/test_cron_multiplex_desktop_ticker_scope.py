...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from mason_constants import get_mason_home, set_mason_home_override
    ...
    async def _inside_running_loop():
        ...
        set_mason_home_override(str(sec_home))
...
def test_multiplex_ticker_profile_gate_skips_rejected_profile(tmp_path):
    ...
    def _tick(*args, **kwargs):
        ticked.append(str(get_mason_home()))
        if len(ticked) >= 3:
            stop.set()
        return 0
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.profiles.profiles_to_serve", lambda multiplex=False: list(homes)
    )
    monkeypatch.setattr(
        "mason_cli.profiles._check_gateway_running", lambda home: home.name == "ops"
    )
    captured = {}
    ...
    monkeypatch.setattr("mason_logging.enable_profile_log_routing", lambda homes: None)
