...
the shared /blueprint command handler (mason_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated MASON_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("mason://blueprint/morning-brief?")
