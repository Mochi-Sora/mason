...
CronScheduler plan, .mason/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - mason_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from mason_cli.web_server import _start_desktop_cron_ticker
...
def test_multiplex_recovery_isolates_profile_failures(tmp_path):
    ...
    def _tick(*args, **kwargs):
        tick_homes.append(str(get_mason_home()))
        if len(tick_homes) >= 2:
            stop.set()
        return 0
