...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mason_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "mason_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("MASON_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("MASON_TEST_RUNTIME_KEY", ""),
            "base_url": None,
    ...
    monkeypatch.setattr(
        "mason_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
