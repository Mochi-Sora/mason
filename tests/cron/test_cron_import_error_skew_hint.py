...
Field incident: an interrupted `mason update` (pull done, restart never ran)
...
had no way to know the fix was one `mason gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "mason gateway restart" not in msg
