...
- Unlimited timeout (MASON_CRON_TIMEOUT=0)
- Backward compat: MASON_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """MASON_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """MASON_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("MASON_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_mason_time_importable(self):
        """mason_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _mason_now
        assert callable(_mason_now)
