...
(``mason cron list`` / ``mason cron status``); the agent path did not.
...
def mason_env(tmp_path, monkeypatch):
    """Isolate MASON_HOME for each test so jobs don't leak."""
    home = tmp_path / ".mason"
    home.mkdir()
    (home / "cron").mkdir()
...
class TestCreateSurfacesGatewayLiveness:
    def test_create_with_gateway_running_has_no_warning(self, mason_env):
    ...
    def test_create_without_gateway_warns_not_scheduled(self, mason_env):
        with (
    ...
    def test_non_builtin_provider_is_exempt(self, mason_env):
    ...
    def test_failed_probe_stays_neutral(self, mason_env):
...
class TestListSurfacesGatewayLiveness:
    ...
    def test_list_with_gateway_running_has_no_warning(self, mason_env):
    ...
    def test_list_without_gateway_warns_jobs_inert(self, mason_env):
        _create_job()
    ...
    def test_list_empty_without_gateway_stays_quiet(self, mason_env):
    ...
    def test_list_non_builtin_provider_is_exempt(self, mason_env):
        _create_job()
        with patch_liveness(provider="chronos", pids=[]):
            result = self._list_jobs()
...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "mason_cli.cron._active_cron_provider_name",
        ...
        self._stack.enter_context(
            patch(
                "mason_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            )
        )
        self._stack.enter_context(
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_active_reports_alive_despite_empty_pid_scan(self, mason_env):
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        import mason_cli.cron as cron_cli

        with (
            patch("mason_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("mason_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
    ...
    def test_no_lock_no_pids_is_false(self):
        ...
        with (
            patch("mason_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("mason_cli.gateway.find_gateway_pids", return_value=[]),
            patch(
                "mason_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            ),
...
class TestCronStatusLockFirst:
    """`mason cron status` shares the lock-first false-alarm fix (#95947).
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("mason_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("mason_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
    ...
    def test_lock_active_suppresses_not_running_false_alarm(self, mason_env):
    ...
    def test_no_lock_no_pids_still_warns(self, mason_env):
