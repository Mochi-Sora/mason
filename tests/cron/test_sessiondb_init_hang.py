...
which is already bounded by MASON_CRON_TIMEOUT — the worker thread submitted
...
timeout (MASON_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        profile_token = set_mason_home_override(profile_home)
        try:
            with patch("cron.scheduler._mason_home", None), \
                 patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
                 patch("mason_cli.env_loader.load_mason_dotenv"), \
                 patch("mason_cli.env_loader.reset_secret_source_cache"), \
                 patch("mason_state_registry.acquire", side_effect=make_session_db), \
                 patch(
            ...
            reset_mason_home_override(profile_token)
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._mason_home", tmp_path), \
             ...
             patch("mason_cli.env_loader.load_mason_dotenv"), \
             patch("mason_cli.env_loader.reset_secret_source_cache"), \
             patch("mason_state_registry.acquire"), \
             patch(
                 "mason_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed MASON_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors MASON_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("MASON_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "MASON_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("MASON_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._mason_home", tmp_path), \
                 ...
                 patch("mason_cli.env_loader.load_mason_dotenv"), \
                 patch("mason_cli.env_loader.reset_secret_source_cache"), \
                 patch("mason_state_registry.acquire", side_effect=_hanging_then_capture), \
                 patch(
                     "mason_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._mason_home", tmp_path), \
             ...
             patch("mason_cli.env_loader.load_mason_dotenv"), \
             patch("mason_cli.env_loader.reset_secret_source_cache"), \
             patch("mason_state_registry.acquire") as mock_db_cls, \
             patch(
