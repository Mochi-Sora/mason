...
def test_external_worker_adopts_execution_and_runs_payload_once(
    ...
    adopted = Mock(
        side_effect=lambda execution_id: (
            observed_homes.append(get_mason_home().resolve())
            or {"id": execution_id, "status": "running"}
        )
    )
    run = Mock(
        side_effect=lambda *_args, **_kwargs: (
            observed_homes.append(get_mason_home().resolve()) or True
        )
    )
...
def test_launch_external_worker_uses_restart_safe_scope_and_acknowledges(
    ...
    monkeypatch.setattr(scheduler, "_get_mason_home", lambda: tmp_path)
    wrapped_commands = []
...
def test_worker_delivery_queue_is_keyed_by_the_delivering_jobs_own_execution(
    ...
    ``mason cron run <other>``) must not queue under the OUTER execution id."""
    import cron.scheduler as scheduler
    ...
    monkeypatch.setenv("_MASON_CRON_EXTERNAL_WORKER", "exec-outer")
...
def test_managed_gateway_restart_preserves_active_worker_and_single_side_effect(
    ...
    harness = (
        ...
        f"os.environ['MASON_HOME'] = {str(home)!r}\n"
