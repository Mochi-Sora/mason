...
def test_create_job_no_agent_requires_script(mason_env):
...
def test_update_job_roundtrips_no_agent_flag(mason_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = mason_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(mason_env):
...
def test_run_job_no_agent_success_returns_script_stdout(mason_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = mason_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(mason_env, monkeypatch):
    ...
    def fake_load(*, mason_home=None, project_env=None):
        loaded_homes.append(mason_home)
        return []

    monkeypatch.setattr(env_loader, "load_mason_dotenv", fake_load)

    script_path = mason_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_mason_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(mason_env)


def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    mason_env, monkeypatch,
):
    ...
    (mason_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(mason_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(mason_env):
...
def test_run_job_script_nul_path_fails_cleanly(mason_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(mason_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(mason_env):
    ...
    script = mason_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.mason/scripts/nightly.sh",
