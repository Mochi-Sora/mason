...
This file drives the REAL `tick()` end-to-end against a throwaway MASON_HOME:
...
def wedge_env(tmp_path, monkeypatch):
    ...
    script = mason_home / "scripts" / "probe.py"
    script.write_text("print('ok')\n")

    return {"home": mason_home, "job_id": job["id"]}
...
class TestEAGAINRecurringRedispatches:
    ...
    def test_eagain_then_redispatched_on_next_tick(self, wedge_env, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(S, "_mason_home", env["home"])
