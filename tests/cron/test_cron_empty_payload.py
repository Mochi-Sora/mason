...
@pytest.fixture
def mason_env(tmp_path, monkeypatch):
    """Isolate MASON_HOME for each test so jobs/scripts don't leak."""
    home = tmp_path / ".mason"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
...
def test_create_job_rejects_empty_payload(mason_env, prompt):
...
def test_create_job_rejects_blank_script_and_blank_skills(mason_env):
    from cron.jobs import create_job
...
def test_create_job_no_agent_error_still_wins(mason_env):
...
def test_valid_shapes_are_accepted(mason_env):
    from cron.jobs import create_job

    (mason_env / "scripts" / "w.sh").write_text("echo hi\n")
...
def test_update_job_rejects_clearing_the_only_payload(mason_env):
...
def test_update_job_rejects_dropping_last_skill_from_promptless_job(mason_env):
...
def test_update_job_rejects_clearing_script_from_promptless_job(mason_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_rejects_toggling_no_agent_on_without_a_script(mason_env):
...
def test_update_job_rejects_removing_script_from_a_no_agent_job(mason_env, blank):
    from cron.jobs import create_job, get_job, update_job
...
def test_update_job_rejects_swapping_script_for_prompt_on_a_no_agent_job(mason_env):
...
def test_update_job_allows_dropping_script_when_no_agent_is_turned_off(mason_env):
...
def test_update_job_allows_swapping_the_script_of_a_no_agent_job(mason_env):
    from cron.jobs import create_job, get_job, update_job
...
def test_update_job_allows_clearing_prompt_when_script_remains(mason_env):
...
def test_update_job_allows_swapping_prompt_for_skill(mason_env):
    from cron.jobs import create_job, get_job, update_job
...
def _legacy_empty_job(mason_env):
...
def test_run_job_fails_closed_and_never_builds_an_agent(mason_env):
...
def test_run_job_pauses_the_job_on_disk(mason_env):
...
def test_run_one_job_does_not_resurrect_the_paused_job(mason_env):
...
def _legacy_no_agent_scriptless_job(mason_env, script_value=None):
...
def test_run_job_pauses_a_legacy_no_agent_job_without_a_script(mason_env, script_value):
    """Erroring alone left it enabled, so it re-fired every tick."""
    import cron.scheduler as scheduler
    from cron.jobs import get_job

    job = _legacy_no_agent_scriptless_job(mason_env, script_value)
...
def test_run_one_job_does_not_resurrect_the_paused_no_agent_job(mason_env):
    """Post-run bookkeeping must not put it back in the due queue."""
    import cron.scheduler as scheduler
    from cron.jobs import get_due_jobs, get_job

    job = _legacy_no_agent_scriptless_job(mason_env)
...
def test_run_job_does_not_block_a_valid_no_agent_job(mason_env):
    """The guard sits after the no_agent short-circuit, which must still run."""
    import cron.scheduler as scheduler

    script = mason_env / "scripts" / "w.sh"
    script.write_text("echo hello\n")

    job = dict(_legacy_empty_job(mason_env), script="w.sh", no_agent=True)
    success, doc, final, error = scheduler.run_job(job)
...
def test_tool_update_rejects_the_2026_08_03_destructive_shape(mason_env):
...
def test_tool_update_rejects_the_destructive_shape_on_a_script_job(mason_env):
...
def test_tool_update_rejects_the_destructive_shape_on_a_no_agent_job(mason_env):
...
def test_tool_update_blank_name_is_a_no_op(mason_env):
...
def test_tool_update_still_renames_when_a_real_name_is_given(mason_env):
    from cron.jobs import create_job, get_job
...
def test_tool_update_blank_scalars_still_clear_workdir_and_context_from(mason_env):
    ...
    job = create_job(
        ...
        enabled_toolsets=["file"], workdir=str(mason_env / "wd"),
        deliver="local",
    )
    assert get_job(job["id"])["workdir"] == str(mason_env / "wd")
