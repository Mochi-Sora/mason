...
- Job store (cron/jobs.py): the field is validated at the storage choke
  point against the canonical Mason effort grammar (parse_reasoning_effort
  in mason_constants — the SAME parser every other effort surface uses).
...
class TestCronjobToolReasoningEffort:
    ...
    via `mason cron create/edit --reasoning-effort` only."""
    ...
    def test_schema_does_not_expose_reasoning_effort(self):
        ...
        keeps the parameter for the CLI lane (mason_cli/cron.py), but the
