...
def test_resolver_keeps_facade_defined_names_on_facade(mods):
    cache: dict = {}
    src = (ROOT / "mason_state.py").read_text(encoding="utf-8")
    own = [n for n, (_, _, node) in bench.top_level_defs(src).items() if not bench._is_alias(node)]
    assert own, "mason_state.py should still define something at top level"
    for name in own[:20]:
        assert bench.resolve_definer(mods, name, "mason_state", cache) == "mason_state", name
