...
($INSTALL_DIR), not into the shared $MASON_HOME.

Background (shared-$MASON_HOME bug)
------------------------------------
$MASON_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.mason:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$MASON_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``mason update``).
...
``~/.mason/mason-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$MASON_HOME/\.install_method"', text), (
        "install.sh must not stamp $MASON_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `mason update`"
    )
