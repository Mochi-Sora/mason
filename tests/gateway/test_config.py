...
class TestLoadGatewayConfig:
    def test_shipped_template_does_not_enable_auto_reset(self, tmp_path, monkeypatch):
        ...
        docker/stage2-hook.sh, mason doctor) copy the template verbatim to
        ~/.mason/config.yaml, so whatever ``session_reset.mode`` the template
    ...
    def test_multiplex_profiles_from_nested_gateway_section(self, tmp_path, monkeypatch):
        ...
        ``mason config set gateway.multiplex_profiles true``) must enable
    ...
    def test_discord_websocket_health_settings_seed_platform_extra(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        for key in (
            "MASON_DISCORD_LIVENESS_INTERVAL_SECONDS",
            "MASON_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ):
    ...
    def test_api_server_port_bridged_into_extra(self, tmp_path, monkeypatch):
        ...
        assert extra["model_name"] == "my-mason"
    ...
    def test_room_link_url_from_nested_gateway_section(self, tmp_path, monkeypatch):
        ...
        (mason_home / "config.yaml").write_text(
            "gateway:\n"
            "  room_link_url: https://peer.example.test/mason\n",
            encoding="utf-8",
        )
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        ...
        assert config.room_link_url == "https://peer.example.test/mason"
        assert GatewayConfig.from_dict(config.to_dict()).room_link_url == (
            "https://peer.example.test/mason"
        )
    ...
    def test_relay_exclusive_reads_profile_scoped_env(self, tmp_path, monkeypatch):
        ...
        load_mason_dotenv exports that file into os.environ at startup.)"""
    ...
    def test_profile_scoped_env_overrides_do_not_fall_back_to_default_profile_env(
        ...
        home_token = set_mason_home_override(str(secondary_home))
...
class TestMultiplexProfilesEnvOverride:
    ...
    def _load(self, tmp_path, monkeypatch, config_text=None):
        ...
        if config_text is not None:
            (mason_home / "config.yaml").write_text(config_text, encoding="utf-8")
        monkeypatch.setenv("MASON_HOME", str(mason_home))
...
class TestMultiplexProfilesConfig:
    ...
    def test_multiplex_profiles_nested_under_gateway(self, tmp_path, monkeypatch):
        """gateway.multiplex_profiles (the form written by `mason config set
