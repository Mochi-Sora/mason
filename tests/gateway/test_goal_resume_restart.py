...
def mason_home(tmp_path, monkeypatch):
    ...
    # get_mason_home() prefers the context-local override over the env
    # var, so a set_mason_home_override() leaked by ANY earlier test in
...
class TestCliResumeRestartsWork:
    def test_resume_after_budget_exhaustion_queues_continuation(self, mason_home):
    ...
    def test_resume_without_goal_queues_nothing(self, mason_home):
...
class TestGatewayResumeRestartsWork:
    ...
    @pytest.mark.asyncio
    async def test_resume_without_goal_enqueues_nothing(self, mason_home):
        runner, adapter = _make_runner()
