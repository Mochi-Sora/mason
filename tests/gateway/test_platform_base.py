...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    mason_home.mkdir()
    monkeypatch.setattr(base, "_MASON_HOME", mason_home)
    monkeypatch.setattr(base, "_MASON_ROOT", mason_home)
    path = mason_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.mason/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("MASON_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("MASON_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("MASON_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("MASON_KANBAN_HOME", str(tmp_path / "mason"))
        monkeypatch.setenv("MASON_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "mason" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("MASON_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("MASON_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("MASON_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.mason/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        mason_dir = fake_home / ".mason"
        (mason_dir / "mcp-tokens").mkdir(parents=True)
        secret = mason_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._MASON_HOME",
            mason_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._MASON_ROOT",
            mason_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the MASON_HOME root (google_token.json)
        ...
        token = mason_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._MASON_HOME", mason_dir)
        monkeypatch.setattr("gateway.platforms.base._MASON_ROOT", mason_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_mason_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.mason
        ...
        mason_dir = fake_home / ".mason"
        mason_dir.mkdir(parents=True)
        artifact = mason_dir / "adhoc_report.pdf"
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.mason/state.db`` must not exfiltrate them (#41071). Named
        ...
        monkeypatch.setattr("gateway.platforms.base._MASON_HOME", mason_dir)
        monkeypatch.setattr("gateway.platforms.base._MASON_ROOT", mason_dir)
        board = mason_dir / "kanban" / "boards" / "team-a"
        ...
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(mason_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(mason_dir / rel))] == []

    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting MASON_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose MASON_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.mason) emits
        ...
        mason_root = denied_root / ".mason"
        prof_cache = mason_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._MASON_ROOT", mason_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.mason/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        mason_home = tmp_path / ".mason"
        cache = mason_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.mason/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "MASON_HUMAN_DELAY_MODE": "natural",
            "MASON_HUMAN_DELAY_MIN_MS": "oops",
            "MASON_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"MASON_MEDIA_DELIVERY_STRICT": "1",
                                     "MASON_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Mason
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../mason/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.mason/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.mason exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".mason"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.mason/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
