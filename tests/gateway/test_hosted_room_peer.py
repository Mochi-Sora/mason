...
def test_room_link_endpoint_reads_supported_config_with_env_override(
    ...
    (home / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://configured.example.test/mason\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("MASON_HOME", str(home))
    monkeypatch.delenv("MASON_ROOM_LINK_URL", raising=False)
    assert local_room_link_endpoint() == {
        "available": True,
        "url": "https://configured.example.test/mason",
        "transport_security": "tls",
    }

    monkeypatch.setenv(
        "MASON_ROOM_LINK_URL", "https://override.example.test/mason"
    )
    assert local_room_link_endpoint()["url"] == (
        "https://override.example.test/mason"
    )
...
def test_named_profile_inherits_gateway_room_link_endpoint(tmp_path, monkeypatch):
    ...
    (root / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://gateway.example.test/mason\n",
        encoding="utf-8",
    )
    ...
    try:
        assert local_room_link_endpoint() == {
            "available": True,
            "url": "https://gateway.example.test/mason",
            "transport_security": "tls",
        }
    finally:
        reset_mason_home_override(token)
...
def test_named_profile_room_link_override_wins_over_gateway_root(
    ...
    (profile / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://profile.example.test/mason\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("MASON_HOME", str(root))
    monkeypatch.delenv("MASON_ROOM_LINK_URL", raising=False)
    ...
    try:
        assert local_room_link_endpoint()["url"] == (
            "https://profile.example.test/mason"
        )
    finally:
        reset_mason_home_override(token)
...
def test_self_advertised_endpoint_is_explicit_and_validated(
    ...
    if configured is None:
        ...
        monkeypatch.setenv("MASON_ROOM_LINK_URL", configured)
