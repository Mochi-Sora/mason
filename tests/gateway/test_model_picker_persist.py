...
closure the PR changed, against a real temp ``MASON_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_mason_home() / config.yaml`` — point it here.
    monkeypatch.setattr("mason_constants.get_mason_home", lambda: mason_home)
    monkeypatch.setattr("mason_cli.config.get_mason_home", lambda: mason_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_mason_home", default_home)
    _stub_picker_dependencies(monkeypatch)
