...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Mason"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^mason"]}
        )
        assert adapter._message_matches_mention_patterns("mason please help") is True
        assert adapter._message_matches_mention_patterns("please mason help") is False
