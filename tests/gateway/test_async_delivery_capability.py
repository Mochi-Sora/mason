...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("MASON_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``mason -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``MASON_SESSION_*`` routing keys, so an async delegation's
