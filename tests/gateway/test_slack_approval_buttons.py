...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "mason_approve_once" in action_ids
        assert "mason_approve_session" in action_ids
        assert "mason_approve_always" in action_ids
        assert "mason_deny" in action_ids
...
class TestSlackApprovalAction:
    ...
    async def test_truncates_inflated_original_text(self):
        ...
        action = {"action_id": "mason_approve_once", "value": "session-key"}
...
class TestSlackReactionForwarding:
    ...
    async def test_reaction_synthesizes_message_in_thread(self):
        ...
        assert synth["_mason_reaction"]["name"] == "thumbsup"
        assert synth["_mason_reaction"]["action"] == "added"
        assert synth["_mason_reaction"]["reacted_to_ts"] == "2000.0"
        # Pre-authorized as addressed-to-the-bot (skips mention gate only).
        assert synth["_mason_force_process"] is True
    ...
    async def test_target_channel_handoff_routes_top_level(self):
        ...
        assert synth["_mason_no_thread_response"] is True
        assert synth["_mason_reaction_source_channel"] == "C1"
    ...
    async def test_target_channel_with_thread_routes_to_thread(self):
        ...
        assert "_mason_no_thread_response" not in forwarded[0]
