...
class TestRuntimeProviderUsesScope:
    """mason_cli.runtime_provider._getenv resolves through the secret scope."""


    def test_getenv_two_profiles_isolated(self, monkeypatch):
        from mason_cli.runtime_provider import _getenv
...
def test_turn_scoped_dotenv_reload_does_not_pollute_process_env(tmp_path, monkeypatch):
    ...
    ``load_mason_dotenv`` has several lazy-import and cron call sites beyond
    ...
    with _profile_runtime_scope(profile_a):
        ...
        assert load_mason_dotenv(mason_home=get_mason_home()) == []
