...
def test_routed_profile_flows_into_prune_via_send_metadata(tmp_path: Path):
    ...
    assert meta["mason_profile"] == "coder"
    assert "mason_profile" not in runner._thread_metadata_for_source(_source(None, "99"))
    ...
    adapter._mason_profile_name = "default"  # transport = primary bot
