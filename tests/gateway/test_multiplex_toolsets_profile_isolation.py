...
E2E-style: real profile homes under a temp MASON root, real config.yaml
...
def mason_root(tmp_path, monkeypatch):
    ...
    # get_default_mason_root memoizes per (native_home, env_home) pair, so
...
class TestMultiplexOnToolsetIsolation:
    @pytest.mark.asyncio
    async def test_each_profile_sees_its_own_toolsets(self, mason_root):
    ...
    @pytest.mark.asyncio
    async def test_owner_key_does_not_open_secondary_profile(self, mason_root):
    ...
    @pytest.mark.asyncio
    async def test_unknown_profile_is_404(self, mason_root):
        adapter = _make_adapter(multiplex=True)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/ghost/v1/toolsets",
...
class TestMultiplexOffPrefixFailsClosed:
    ...
    def test_foreign_prefix_rejected(self, mason_root):
    ...
    def test_self_referential_prefix_falls_through(self, mason_root):
    ...
    def test_own_named_profile_prefix_falls_through(self, mason_root, monkeypatch):
        """A gateway launched FOR profile lokaj accepts /p/lokaj/…"""
        monkeypatch.setenv(
            "MASON_HOME", str(mason_root / "profiles" / "lokaj")
        )
        adapter = _make_adapter(multiplex=False)
    ...
    @pytest.mark.asyncio
    async def test_foreign_prefix_is_404_end_to_end(self, mason_root):
        adapter = _make_adapter(multiplex=False)
        async with TestClient(TestServer(_make_app(adapter))) as cli:
            resp = await cli.get(
                "/p/lokaj/v1/toolsets",
