...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        mason_home = tmp_path / "mason"
        mason_home.mkdir()
        (mason_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_includes_enabled_mcp_servers_in_gateway_toolsets(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_mason_home", mason_home)
        monkeypatch.setattr(gateway_run, "_env_path", mason_home / ".env")
        monkeypatch.setattr(
            gateway_run,
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        mason_home = tmp_path / "mason"
        mason_home.mkdir()
        (mason_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_mason_home", mason_home)
