...
``gateway run --replace`` only kills same-MASON_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "mason_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(mason --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_mason_home(adapter):
    """Locks written before the profile field existed still attribute via mason_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "mason_home": "/opt/data/profiles/lead-gen-outreach",
