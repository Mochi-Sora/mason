...
* ``compression.codex_app_server_auto`` semantics hold: only ``mason``
  lets Mason' threshold start a compaction; ``native``/``off`` skip
...
class LiveCodexAgent:
    ...
    def __init__(self, mode="mason", session=None):
...
# ---------------------------------------------------------------------------
# Core no-op regression: mason mode + live thread => thread/compact runs
# ---------------------------------------------------------------------------

def test_mason_mode_compacts_live_thread_at_rpc_boundary(tmp_path):
    agent = LiveCodexAgent(mode="mason")
    key = "tg:123"
...
def test_mason_mode_without_cached_agent_skips_without_local_compression(tmp_path):
...
@pytest.mark.parametrize("mode", ["native", "mason", "off"])
...
def test_force_without_live_thread_does_not_run_local_compressor(mode):
    # The #73715 branch let force=True fall through to the local Mason
...
def test_timeout_records_persistent_cooldown(tmp_path):
    ...
    agent = LiveCodexAgent(mode="mason", session=HangingSession())
    key = "tg:123"
    gw, db = _gateway(tmp_path, key, agent)
...
# (profile secret scope / MASON_HOME override). A bare run_in_executor worker
...
def test_hygiene_worker_inherits_caller_contextvars(tmp_path):
    ...
    agent = ScopeProbeAgent(mode="mason")
    key = "tg:ctx"
    ...
    async def _scoped():
        ...
        try:
            return await run_codex_hygiene_compaction(
                gw, key, agent.session_id, auto_mode="mason",
