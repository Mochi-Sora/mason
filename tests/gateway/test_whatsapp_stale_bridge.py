...
``mason update``, serving pre-update bridge.js behavior forever (e.g.
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["MASON_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["MASON_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["MASON_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
