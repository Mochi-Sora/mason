...
override and the ``mason_model_provider`` / ``mason_model_model`` /
``mason_model_back`` / ``mason_model_cancel`` action dispatch.
"""
...
class TestSlackModelPickerSend:
    ...
    async def test_sends_provider_dropdown(self):
        ...
        assert elements[0]["action_id"] == "mason_model_provider"
        ...
        assert elements[1]["action_id"] == "mason_model_cancel"
...
class TestSlackModelPickerAction:
    ...
    async def test_provider_select_shows_models(self):
        ...
        ack = AsyncMock()
        action = {
            "action_id": "mason_model_provider",
        ...
        assert elements[0]["action_id"] == "mason_model_model"
        # Two models for openrouter, values are indices
        assert [o["value"] for o in elements[0]["options"]] == ["0", "1"]
        # Back + Cancel buttons present
        assert elements[1]["action_id"] == "mason_model_back"
        assert elements[2]["action_id"] == "mason_model_cancel"
    ...
    async def test_model_select_callback_error_is_reported(self):
        ...
        ack = AsyncMock()
        action = {"action_id": "mason_model_model", "selected_option": {"value": "1"}}
    ...
    async def test_model_select_gateway_error_return_uses_failure_header(self):
        ...
        ack = AsyncMock()
        action = {"action_id": "mason_model_model", "selected_option": {"value": "0"}}
    ...
    async def test_cancel_clears_state(self):
        ...
        ack = AsyncMock()
        action = {"action_id": "mason_model_cancel", "value": "cancel"}
    ...
    async def test_back_returns_to_provider(self):
        ...
        ack = AsyncMock()
        action = {"action_id": "mason_model_back", "value": "openrouter"}
    ...
    async def test_state_not_found_shows_expiry(self):
        ...
        mock_client.chat_update = AsyncMock()
        ack = AsyncMock()
        action = {"action_id": "mason_model_provider", "selected_option": {"value": "0"}}
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        ack = AsyncMock()
        action = {"action_id": "mason_model_provider", "selected_option": {"value": "openrouter"}}
...
class TestSlackModelPickerGatewayIntegration:
    ...
    async def test_bare_model_triggers_picker(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_mason_home", mason_home)
        monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
        monkeypatch.setattr(
            "mason_cli.model_switch_providers.list_picker_providers",
    ...
    async def test_text_fallback_when_no_picker(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
        monkeypatch.setattr(
            "mason_cli.model_switch_providers.list_authenticated_providers",
            lambda **kw: [],
        )
