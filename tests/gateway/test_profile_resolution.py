...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("mason_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("mason_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("mason_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/mason/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/mason/profiles/from-source")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("mason_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("mason_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/mason/profiles/nonexistent")
                with patch("mason_cli.profiles.profile_exists", return_value=False):
                    with patch("mason_constants.get_mason_home", return_value=Path("/mason")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global MASON_HOME
                            assert result == Path("/mason")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("mason_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("mason_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("mason_constants.get_mason_home", return_value=Path("/mason")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
...
class TestRoutingConsultation:
    ...
    def test_routing_consulted_when_source_profile_empty(self, mock_runner, discord_source):
        ...
        with patch("mason_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("mason_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/mason/profiles/routed")
...
class TestNonDiscordProfileRouting:
    ...
    def test_telegram_route_resolves(self, mock_runner, telegram_source):
        ...
        with patch(
            "mason_cli.profiles.profiles_to_serve",
