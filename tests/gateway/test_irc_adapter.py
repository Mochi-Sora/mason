...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "mason"
        assert adapter.channel == "#mason-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :mason: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG mason :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :mason: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        assert any(line.startswith("NICK masonbot-cron") for line in sent_lines)
        assert any(line.startswith("USER masonbot-cron 0 * :Mason Agent (cron)")
...
class TestMultiplexProfileScope:
    ...
    def test_secondary_missing_keys_fail_closed(
        ...
        assert adapter.nickname == "mason-bot"
