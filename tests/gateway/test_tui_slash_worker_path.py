...
that omits the Mason venv bin dir and ~/.local/bin. The spawned
tui_gateway.slash_worker then cannot resolve Mason-managed CLIs such as
...
class TestPrependToolPaths:
    def test_prepends_managed_venv_and_user_bin(self, monkeypatch, tmp_path):
        monkeypatch.setenv("MASON_HOME", str(tmp_path / "hh"))
        env = {"PATH": "/usr/bin"}
