...
top-level module ``mason_startup_watchdog``), disarmed once the gateway's asyncio loop is confirmed
...
import mason_startup_watchdog as sw
...
def _isolate(tmp_path, monkeypatch):
    """Every test gets a fresh singleton and its own MASON_HOME."""
...
class TestContracts:
    ...
    def test_implementation_module_is_stdlib_only(self):
        ...
        must not import the gateway/agent/mason_cli graphs at module level."""
        import ast
        import inspect
    ...
    def test_all_documented_entry_points_arm_the_watchdog(self):
        ...
        arm_sites = {
            ...
            "mason_cli/main.py": "argv fast-path (standard `mason gateway run`)",
            "mason_cli/gateway.py": "run_gateway() config-bridge re-arm",
        ...
        for rel, reason in arm_sites.items():
            ...
            aliases = set()
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module == "mason_startup_watchdog":
...
class TestProgressLease:
    ...
    def test_schema_init_declares_lease(self):
        """mason_state_schema._init_schema must hold a progress lease so
        multi-GB migrations aren't misread as deadlocks (wiring contract)."""
        import inspect

        import mason_state_schema

        src = inspect.getsource(mason_state_schema.SessionSchemaMixin._init_schema)
    ...
    def test_repair_declares_lease(self):
        ...
        src = inspect.getsource(mason_state_repair.repair_state_db_schema)
        assert "report_startup_progress" in src
