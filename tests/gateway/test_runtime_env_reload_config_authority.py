...
MASON_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env MASON_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``mason config get terminal.backend`` still says local.
    """
    mason_home = tmp_path / ".mason"
    mason_home.mkdir()
    (mason_home / "config.yaml").write_text(
    ...
    (mason_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
