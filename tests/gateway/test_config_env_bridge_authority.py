...
Guards against the 60-vs-500 bug where a stale `.env MASON_MAX_ITERATIONS=60`
...
def _run_gateway_import(mason_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "MASON_AGENT_TIMEOUT",
            "MASON_AGENT_TIMEOUT_WARNING",
            "MASON_TURN_LEASE_TIMEOUT",
            "MASON_SESSION_STALL_TIMEOUT",
            "MASON_GATEWAY_BUSY_INPUT_MODE",
            "MASON_GATEWAY_BUSY_TEXT_MODE",
            "MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "MASON_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Mason settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def mason_home(tmp_path: Path) -> Path:
    home = tmp_path / ".mason"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(mason_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(mason_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(mason_home, {
        "MASON_AGENT_TIMEOUT": "60",
        "MASON_AGENT_TIMEOUT_WARNING": "30",
        "MASON_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(mason_home, initial_env={})

    assert env.get("MASON_AGENT_TIMEOUT") == "1800"
    assert env.get("MASON_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("MASON_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(mason_home: Path) -> None:
    ...
    _write_env(
        mason_home,
        {"MASON_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(mason_home, initial_env={})

    assert env.get("MASON_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("MASON_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(mason_home: Path) -> None:
    ...
    _write_config(mason_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(mason_home, initial_env={})

    assert env.get("MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(mason_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        mason_home,
        initial_env={"MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
