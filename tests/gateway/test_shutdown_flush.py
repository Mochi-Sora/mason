...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("mason_state_registry.acquire", lambda: db)
    monkeypatch.setattr(
        "mason_state_registry.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_mason_home(tmp_path, monkeypatch):
    """Flush dir must use get_mason_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_mason_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "mason_constants.get_mason_home", fake_get_mason_home
    )
