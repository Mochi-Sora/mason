...
def _clear_env(monkeypatch):
    monkeypatch.delenv("MASON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("MASON_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("MASON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("MASON_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
