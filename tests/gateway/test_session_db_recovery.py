...
def test_session_store_and_runner_reopen_after_failed_construction(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(mason_state_registry, "acquire", fail_once_session_db)
    monkeypatch.setattr(mason_state, "_default_db_path", lambda: db_path)
    ...
    monkeypatch.setattr(mason_state_registry, "acquire", runner_fail_once)
    monkeypatch.setattr(mason_state, "AsyncSessionDB", lambda db: ("async", db))
...
def test_recovered_db_rows_survive_fallback_structural_save(monkeypatch, tmp_path) -> None:
    ...
    database = mason_state.SessionDB(db_path=db_path)
    ...
    real_session_db = mason_state.SessionDB
    calls = 0
    ...
    monkeypatch.setattr(mason_state, "SessionDB", fail_once_session_db)
    monkeypatch.setattr(mason_state, "_default_db_path", lambda: db_path)
    store = SessionStore(
        sessions_dir,
