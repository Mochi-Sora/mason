...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/mason-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Mason-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/NousResearch/mason-agent/issues/87051:
    ...
    response in a different chat lane than the Mason topic the session runs
    in. The no-anchor branch must prefer the Mason topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_mason_topic_thread_id(self):
