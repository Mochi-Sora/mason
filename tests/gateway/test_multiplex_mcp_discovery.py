...
async def test_gateway_boot_discovers_mcp_under_every_profile_home(
    ...
    def fake_discover() -> list[str]:
        seen.append((get_mason_home(), threading.current_thread().name))
        return []
...
async def test_reload_mcp_only_touches_requesting_profile(
    ...
    worker_scope = mason_home_key(worker_home)
    ...
    monkeypatch.setattr(
        ...
        {"default-srv": mason_home_key(tmp_path), "worker-srv": worker_scope},
    )
    seen: list[tuple] = []

    def fake_shutdown(*, scope=None) -> None:
        seen.append(("shutdown", scope, get_mason_home()))

    def fake_discover() -> list[str]:
        seen.append(("discover", get_mason_home()))
        return []
...
def test_deregister_scope_kwarg_targets_overlay_and_keeps_plugin_confinement() -> None:
    ...
    reg._plugin_module_scopes["mason_plugins.p"] = {"/home/p1"}
    reg._caller_module = staticmethod(lambda: "mason_plugins.p")
