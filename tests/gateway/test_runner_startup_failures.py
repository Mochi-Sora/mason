...
async def test_start_gateway_verbosity_imports_redacting_formatter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mason_logging.setup_logging", lambda mason_home, mode: tmp_path)
    monkeypatch.setattr("mason_logging._add_rotating_handler", lambda *args, **kwargs: None)
    ...
    *mid-run* reconnect can recover once the holder exits.  The startup router
    ...
    monkeypatch.setattr(
        ...
        lambda scope, identity, metadata=None: (
            False,
            {"pid": 424242, "start_time": 1, "mason_home": "/other/home", "profile": "other"},
        ),
    )
    config = GatewayConfig(
