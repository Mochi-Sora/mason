...
``_profile_runtime_scope``, which installs the routed profile's ``MASON_HOME``
override and its secret scope as **contextvars**. A bare
``loop.run_in_executor(None, ...)`` starts the worker with an EMPTY context, so
``SessionDB()`` / ``get_mason_home()`` inside the worker resolve the LAUNCH
...
async def test_insights_opens_session_db_under_the_routed_home(
    ...
    monkeypatch.setattr(mason_state, "SessionDB", _RecordingDB)
