...
(/opt/mason/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable MASON_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_mason_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable MASON_HOME."""
    ...
    monkeypatch.setattr(
        "mason_constants.get_mason_home", lambda: mason_home
    )
    ...
    expected = mason_home / "scripts" / "whatsapp-bridge"
