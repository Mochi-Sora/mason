...
MASON_HOME, GatewayRunner._scale_to_zero_is_idle's composition, and
...
def test_heartbeat_path_lives_under_mason_home_state(mason_home):
    p = s2z.dashboard_client_heartbeat_path()
    assert p == mason_home / "state" / "dashboard_clients.heartbeat"


def test_last_seen_missing_marker_is_none_not_fail_awake(mason_home):
...
def test_touch_creates_state_dir_and_marker(mason_home):
...
def test_last_seen_returns_raw_mtime_without_staleness_cutoff(mason_home):
...
def test_last_seen_future_mtime_is_clamped_to_now(mason_home):
...
def test_last_seen_unreadable_marker_fails_awake(mason_home, monkeypatch):
    s2z.touch_dashboard_client_heartbeat()
...
def test_touch_never_raises(mason_home, monkeypatch):
...
def test_idle_without_dashboard_client_unchanged(mason_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    assert r._scale_to_zero_is_idle() is True


def test_attached_dashboard_client_blocks_idle(mason_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    assert r._scale_to_zero_is_idle() is False


def test_client_gets_the_same_idle_grace_as_a_message(mason_home, monkeypatch):
...
def test_client_gone_longer_than_idle_timeout_is_idle(mason_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    p = s2z.dashboard_client_heartbeat_path()
    old = time.time() - 121
...
def test_marker_predating_gateway_inbound_does_not_matter(mason_home, monkeypatch):
...
def test_dashboard_client_seen_recently_extends_inbound_clock(mason_home, monkeypatch):
...
def test_newer_gateway_inbound_wins_over_older_marker(mason_home, monkeypatch):
...
def test_unreadable_marker_keeps_gateway_awake(mason_home, monkeypatch):
    r = _runner(monkeypatch, last_inbound_at=time.time() - 600)
    s2z.touch_dashboard_client_heartbeat()
    monkeypatch.setattr(s2z.os, "stat", _stat_denying(s2z.dashboard_client_heartbeat_path()))
    assert r._scale_to_zero_is_idle() is False
...
def test_handle_ws_connect_touches_marker(mason_home, monkeypatch):
...
def test_handle_ws_inbound_frames_refresh_marker(mason_home, monkeypatch):
    from tui_gateway import server, ws as ws_mod
...
def test_note_activity_is_throttled(mason_home, monkeypatch):
