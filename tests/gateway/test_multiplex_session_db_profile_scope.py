...
redirected ``get_mason_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with MASON_HOME on the root.
    ...
    # The suite-wide fixture in conftest re-points ``mason_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_mason_home()``,
    # which is what production does.  ``MASON_HOME`` above still keeps that
...
def test_primary_handler_enters_routed_profile_scope_before_dispatch(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append(Path(get_mason_home()))
    ...
    assert seen == [profile]
    assert Path(get_mason_home()) == root
...
def test_primary_handler_rejected_route_falls_back_and_marks_sentinel(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append((Path(get_mason_home()), event.source.profile_route_rejected))
...
def test_db_handle_follows_the_active_profile_scope(multiplex_homes):
    ...
    token = set_mason_home_override(str(profile))
    try:
...
def test_crash_marker_from_a_secondary_profile_survives_restart(multiplex_homes):
    ...
    scope = set_mason_home_override(str(profile))
    try:
        entry = store.get_or_create_session(_profile_source())
        assert store.mark_turn_active(entry.session_key) is not None
    finally:
        reset_mason_home_override(scope)
