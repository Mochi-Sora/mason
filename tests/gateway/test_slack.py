...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.mason."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /mason, /btw, /stop,
        ...
        for expected in ("/mason", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``mason gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "mason_feedback" in created_apps[0].registered_actions
        assert "mason_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("mason_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("mason_clarify_choice")
            for pattern in clarify_choice_patterns
        )
    ...
    async def test_channel_delivery_honors_unfurl_config(self):
        ...
        with patch.object(_slack_mod.aiohttp, "ClientSession", return_value=session):
            result = await _slack_mod._standalone_send(
                config,
                "C123",
                "[Mason](https://example.com/mason)",
            )

        assert result["success"] is True
        payload = session.post.call_args.kwargs["json"]
        assert payload["text"] == "<https://example.com/mason|Mason>"
...
class TestSlashCommands:
    ...
    # instead of as /mason subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_mason_prefix_still_works(self, adapter):
        """Backward compat: /mason btw foo must still route to /btw foo.

        Old workspace manifests only declared /mason as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`mason config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "mason gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_mason_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/mason",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/mason-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/mason-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_MASON_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_mason_slack_user_agent_prefix_format(self):
        """Module constant matches the MasonAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._MASON_SLACK_USER_AGENT_PREFIX.startswith("MasonAgent/")
