"""E2E: ``mason peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real MASON homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp MASON_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``mason peer dm`` client code (``mason_cli.subcommands.peer``),
...
from mason_cli.subcommands import peer as peer_cmd
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own MASON_HOME + state.db) on a real socket."""
