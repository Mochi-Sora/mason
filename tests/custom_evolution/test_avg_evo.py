...
from custom_evolution.avg_evo import analyzer
from custom_evolution.avg_evo import patcher
...
class TestDedupe:
    ...
    def test_repeat_collapses(self, base):
        ...
        assert len(list((base / "custom_evolution" / "patches" / "pending").glob("*.json"))) == 1

    def test_fixes_log_rotates(self, base):
        log = base / "custom_evolution" / "queue" / "fixes.jsonl"
...
class TestPatcher:
    def test_no_llm_rejects(self, base):
        p = base / "custom_evolution" / "patches" / "pending" / "t.json"
        ...
        assert not p.exists()
        mem = (base / "custom_evolution" / "memory.jsonl").read_text()
    ...
    def test_valid_summary_applies(self, base):
        ...
        assert not p.exists()
        assert (base / "custom_evolution" / "patches" / "applied" / "t.json").exists()
...
class TestHook:
    def test_disabled_config_fires_nothing(self, tmp_path):
        (tmp_path / "custom_evolution").mkdir()
        (tmp_path / "custom_evolution" / "config.yaml").write_text("avg_evo:\n  enabled: false\n")
        from custom_evolution import hook
        hook._CONFIG_CACHE.clear()
        hook.on_response(tmp_path, "u", "a")
        import time
        time.sleep(0.2)
        assert not (tmp_path / "custom_evolution" / "queue").exists()

    def test_trace_builder(self):
        from custom_evolution.hook import _trace_from_messages
...
class TestDeadLetter:
    def test_three_strikes_parks(self, base):
        from custom_evolution.avg_evo import patcher as P
