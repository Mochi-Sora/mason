"""Shared fixtures for the mason-agent test suite.
...
2. **Isolated MASON_HOME.** MASON_HOME points to a per-test tempdir so
   code reading ``~/.mason/*`` via ``get_mason_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".mason"`` instead of the canonical
   ``get_mason_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MASON_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox MASON_HOME before ANY test module is imported ──────────────────
# `mason_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_mason_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.mason/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes MASON_HOME, but fixtures run
...
# the REAL Mason root — capture it BEFORE the sandbox rewires MASON_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.mason (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("MASON_KANBAN_HOME", "").strip()
_PRE_SANDBOX_MASON_HOME = os.environ.get("MASON_HOME", "")


def _mason_home_points_at_production(value: str) -> bool:
    """True when a pre-set MASON_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MASON_HOME=~/.mason``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``mason_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.mason`` — the escape vector that landed pytest
    ...
    MASON_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".mason").resolve()
    except Exception:
        return True
...
if _mason_home_points_at_production(os.environ.get("MASON_HOME", "")):
    _SESSION_MASON_HOME = tempfile.mkdtemp(prefix="mason-test-home-")
    os.environ["MASON_HOME"] = _SESSION_MASON_HOME
    atexit.register(shutil.rmtree, _SESSION_MASON_HOME, True)
...
# like a real CLI") — which used to disarm mason_state's live-DB guard in
# the child at the same moment the child lost the MASON_HOME redirect.
# MASON_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# mason_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export MASON_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["MASON_TEST_ISOLATION"] = os.environ.get("MASON_HOME", "") or "1"

#: MASON_HOME as it stood when conftest was imported - i.e. before any test
...
MASON_HOME_AT_CONFTEST_IMPORT = os.environ.get("MASON_HOME", "")
...
# MASON_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MASON_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``mason_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Mason agent session inherits them and
    # mason_constants home-resolution helpers prefer them over monkeypatched
    ...
    "MASON_MAX_ITERATIONS",
    "MASON_SESSION_PLATFORM",
    "MASON_SESSION_CHAT_ID",
    "MASON_SESSION_CHAT_NAME",
    "MASON_SESSION_CHAT_TYPE",
    "MASON_SESSION_THREAD_ID",
    "MASON_SESSION_SOURCE",
    "MASON_SESSION_KEY",
    "MASON_GATEWAY_SESSION",
    "MASON_CRON_SESSION",
    "_MASON_GATEWAY",
    "MASON_PLATFORM",
    "MASON_MODEL",
    "MASON_INFERENCE_MODEL",
    "MASON_INFERENCE_PROVIDER",
    "MASON_TUI_PROVIDER",
    "MASON_MANAGED",
    "MASON_MANAGED_DIR",
    "MASON_DEV",
    "MASON_CONTAINER",
    "MASON_EPHEMERAL_SYSTEM_PROMPT",
    "MASON_TIMEZONE",
    "MASON_REDACT_SECRETS",
    "MASON_BACKGROUND_NOTIFICATIONS",
    "MASON_EXEC_ASK",
    "MASON_HOME_MODE",
    "MASON_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.mason/kanban.db instead of the per-test MASON_HOME.
    "MASON_KANBAN_DB",
    "MASON_KANBAN_BOARD",
    "MASON_KANBAN_HOME",
    "MASON_KANBAN_WORKSPACES_ROOT",
    "MASON_KANBAN_LOGS_ROOT",
    "MASON_KANBAN_TASK",
    "MASON_KANBAN_WORKSPACE",
    "MASON_KANBAN_RUN_ID",
    "MASON_KANBAN_CLAIM_LOCK",
    "MASON_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "mason" host.
    "MASON_HONCHO_HOST",
    ...
    "MASON_DASHBOARD_OAUTH_CLIENT_ID",
    "MASON_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MASON_HOME to per-test tempdirs so code that
    reads ``~/.mason/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MASON_* vars that could change test semantics.
    for name in _MASON_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("MASON_HONCHO_HOST", "mason")

    # 3. Redirect MASON_HOME to a per-test tempdir. Code that reads
    #    ``~/.mason/*`` via ``get_mason_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.mason/*`` via
    #    ``Path.home() / ".mason"`` instead of ``get_mason_home()``
    #    is a bug to fix at the callsite.
    fake_mason_home = tmp_path / "mason_test"
    fake_mason_home.mkdir()
    (fake_mason_home / "sessions").mkdir()
    (fake_mason_home / "cron").mkdir()
    (fake_mason_home / "memories").mkdir()
    (fake_mason_home / "skills").mkdir()
    monkeypatch.setenv("MASON_HOME", str(fake_mason_home))
    ...
    # mason_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("MASON_TEST_ISOLATION", str(fake_mason_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("MASON_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. mason_state computes ``DEFAULT_DB_PATH = get_mason_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from mason_state import ...``) that
    ...
    mason_state_mod = sys.modules.get("mason_state")
    if mason_state_mod is not None and hasattr(mason_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            mason_state_mod, "DEFAULT_DB_PATH", fake_mason_home / "state.db"
        )
    ...
    monkeypatch.setenv("MASON_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.mason/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import mason_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MASON_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_mason_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from mason_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.mason. This autouse
...
# allow-list because test-level fixtures legitimately move MASON_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MASON_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``MASON_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Mason root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_MASON_HOME and not _mason_home_points_at_production(
        _PRE_SANDBOX_MASON_HOME
    ):
        # MASON_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_mason_root().resolve()
    # No pre-existing MASON_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".mason").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.mason`` captured at import time. Hermetic tests that legitimately
    move MASON_HOME to sibling tempdirs are unaffected.

    Only patches when ``mason_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("mason_cli.kanban_db")
    _kdbc = sys.modules.get("mason_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import mason_cli.kanban_db`
...
# ``mason_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Mason root. This fixture wires the
# test-side knobs:
...
#     installs where MASON_HOME is not ~/.mason) into the guard's
...
# so subprocess children that import mason_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("mason_state")
    ...
    if _PRE_SANDBOX_MASON_HOME and not _mason_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_MASON_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Mason home override redirects every later
    # ``get_mason_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from mason_constants import get_mason_home_override, set_mason_home_override

        if get_mason_home_override() is not None:
            set_mason_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal mason config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``mason-gateway`` process
...
# correlated with a ``tests/mason_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> mason-gateway``
...
def _wal_is_usable() -> bool:
    """True when Mason will actually put a database into WAL mode here.

    Mason refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Mason can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Mason managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``mason_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_mason_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_mason_home`` fixture
    redirects ``MASON_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.mason/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``mason_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["MASON_VOICE_TTS"] = "1"``. The
...
#      ``mason_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``mason_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``mason_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``mason_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Mason runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.mason``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Mason falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Mason uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart mason-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting mason/python patterns are also blocked.
    """
    ...
    def _matches_mason_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MASON_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_mason_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch mason-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m mason_cli.main".
                if (
                    "mason" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live mason-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting mason/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `mason update` (or the
        # equivalent `python -m mason_cli.main update`).  These commands
        ...
        if "update" in low and (
            # mason update / mason update --gateway / setsid bash -c ... mason update
            ("mason" in low and "update" in low.split())
            or
            # python -m mason_cli.main update --gateway
            ("mason_cli" in low and "update" in low.split())
            or
            # venv/bin/mason update  (absolute path variant used in tests)
            (".venv/bin/mason" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`mason update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_MASON_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import mason_cli.voice as _voice
    except Exception:
