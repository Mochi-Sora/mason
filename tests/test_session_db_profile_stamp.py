...
def mason_root(tmp_path, monkeypatch):
    ...
    # get_default_mason_root memoizes on (native_home, env) — the env change
    ...
    monkeypatch.setattr(mason_state, "DEFAULT_DB_PATH", root / "state.db")
    return root
...
def test_default_store_stamps_default(mason_root):
    db = SessionDB(db_path=mason_root / "state.db")
    try:
        db.create_session("s_default", source="cli")
    finally:
        db.close()
    assert _profile_of(mason_root / "state.db", "s_default") == "default"


def test_named_profile_store_stamps_own_name(mason_root):
    db_path = mason_root / "profiles" / "workprof" / "state.db"
...
def test_explicit_profile_name_wins(mason_root):
    db = SessionDB(db_path=mason_root / "state.db")
    try:
        db.create_session("s_explicit", source="cli", profile_name="llm-wiki")
    finally:
        db.close()
    assert _profile_of(mason_root / "state.db", "s_explicit") == "llm-wiki"


def test_store_outside_profile_tree_never_guesses(mason_root, tmp_path):
...
def test_compression_child_of_null_parent_is_stamped(mason_root):
    db_path = mason_root / "state.db"
    db = SessionDB(db_path=db_path)
    try:
...
def test_peer_self_heal_insert_is_stamped(mason_root):
    db_path = mason_root / "state.db"
    db = SessionDB(db_path=db_path)
    try:
...
def test_legacy_backfill_still_targets_only_null(mason_root):
