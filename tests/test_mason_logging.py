"""Tests for mason_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import mason_logging
# Use whatever RotatingFileHandler class mason_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. mason_logging
...
from mason_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    mason_logging._logging_initialized = False
    ...
    mason_logging._reset_queued_handlers()
    ...
    mason_logging._install_session_record_factory()
    yield
    ...
    mason_logging._logging_initialized = False
    mason_logging.clear_session_context()


@pytest.fixture
def mason_home(tmp_path, monkeypatch):
    """Provide an isolated MASON_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_mason_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["MASON_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, mason_home):
        log_dir = mason_logging.setup_logging(mason_home=mason_home)
        assert log_dir == mason_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in mason_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home)
        mason_logging.setup_logging(mason_home=mason_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home)

        test_logger = logging.getLogger("test_mason_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        mason_logging.flush_log_queue()

        agent_log = mason_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, mason_home, tmp_path):
        ...
        profile_home.mkdir()
        mason_logging.setup_logging(mason_home=mason_home)
        assert mason_logging.enable_profile_log_routing(
            [mason_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            mason_home / "logs" / "agent.log"
        ).read_text()




    def test_explicit_params_override_config(self, mason_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (mason_home / "config.yaml").write_text(yaml.dump(config))

        mason_logging.setup_logging(mason_home=mason_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, mason_home):
        ...
        gw_log = mason_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, mason_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home, mode="gui")

        logging.getLogger("mason_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        mason_logging.flush_log_queue()

        gui_log = mason_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, mason_home):
        """When session context is set, log lines include [session_id]."""
        mason_logging.setup_logging(mason_home=mason_home)
        mason_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = mason_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home)
        mason_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_mason_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        mason_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in mason_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        mason_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("mason_cli.config.is_managed", return_value=True):
                mason_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = mason_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert mason_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not mason_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, mason_home):
        level, max_size, backup = mason_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, mason_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> mason_logging._ManagedRotatingFileHandler:
        handler = mason_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that mason_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        mason_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        mason_logging.setup_logging(mason_home=mason_home, mode="gateway")
        gw_path = mason_home / "logs" / "gateway.log"
        rotated = mason_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = mason_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, mason_home):
        mason_logging.setup_logging(mason_home=mason_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_mason_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in mason_logging._queued_file_handlers
        )
