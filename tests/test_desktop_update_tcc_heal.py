...
``venv/bin/mason`` execs ``venv/bin/python3``, so the desktop update
...
class TestAnchorHeal:
    ...
    def test_alias_brick_healed_to_real_files(self, tmp_path):
        ...
        assert "invoke=" in out and out.endswith("/venv/bin/mason")
...
class TestUpdateInvokeFallback:
    def test_dead_alias_no_marker_falls_back_to_module_invocation(self, tmp_path):
        ...
        launchd-gateway shape): drive the update as python -m mason_cli.main."""
        root = make_venv(tmp_path, python=GOOD_STUB, python3=BAD_STUB,
                         marker=None)
        out = run_selftest(root)
        assert "state=no-marker" in out
        assert out.endswith("/venv/bin/python -m mason_cli.main")

    def test_healthy_venv_keeps_mason_entrypoint(self, tmp_path):
        root = make_venv(tmp_path, python=GOOD_STUB, python3=GOOD_STUB,
                         marker=None)
        out = run_selftest(root)
        assert out.endswith("/venv/bin/mason")


@requires_bash
class TestHandoffSurvivesBrickAB:
    """A/B analog of the reported loop: `venv/bin/mason` execs python3."""

    def _make_mason(self, root: Path) -> Path:
        mason = root / "venv/bin/mason"
        _write_exe(
            mason,
            "#!/bin/bash\n"
    ...
    def test_bricked_entrypoint_fails_before_and_boots_after_heal(self, tmp_path):
        root = make_venv(tmp_path, python=GOOD_STUB, python3=BAD_STUB,
                         marker="STORE")
        mason = self._make_mason(root)
        # BEFORE the heal: the entrypoint dies exactly like the field logs.
        before = subprocess.run([str(mason)], capture_output=True,
        ...
        after = subprocess.run([str(mason)], capture_output=True,
                               text=True, encoding="utf-8", errors="replace")
