...
``[diff "x"] command=/textconv=`` driver. Mason gathers workspace context by
...
context-gathering git path Mason runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from mason_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
