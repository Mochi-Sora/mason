...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    mason_state = ModuleType("mason_state")
    mason_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "mason_state", mason_state)
    mason_state_registry = ModuleType("mason_state_registry")
    mason_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "mason_state_registry", mason_state_registry)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Mason", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Mason"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Mason", "detail": "full"}),
        ),
    )
