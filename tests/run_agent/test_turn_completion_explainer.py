...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("mason_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "mason doctor" not in lower
...
def test_classify_persistence_error_categories():
    ...
    from mason_state import classify_persistence_error
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to mason_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from mason_state import SessionCompressionInProgressError
    from mason_state_errors import CompressionSessionBusyError
    from mason_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from mason_state import classify_persistence_error
    from mason_state_errors import SessionTurnLeaseLostError
...
def test_persistence_error_causes_tuple_matches_classifier():
    ...
    from mason_state import classify_persistence_error
    from mason_state_errors import PERSISTENCE_ERROR_CAUSES
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("MASON_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"MASON_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("MASON_TURN_COMPLETION_EXPLAINER", None)
        with patch("mason_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from mason_state import StateDbCorruptError, classify_persistence_error
