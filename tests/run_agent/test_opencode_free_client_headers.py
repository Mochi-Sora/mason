...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_mason_attribution(mock_openai):
    """Keyless requests still identify as Mason (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Mason Agent"
    assert str(headers.get("User-Agent", "")).startswith("MasonAgent/")
