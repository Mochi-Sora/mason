...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        mason_home.mkdir()
        (mason_home / "google_token.json").write_text("{}")
        (mason_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.mason/google_token.json" in container_paths
            assert "/root/.mason/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
