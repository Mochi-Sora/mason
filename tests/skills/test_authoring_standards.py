...
def test_required_frontmatter_fields(p):
    ...
    mason = (fm.get("metadata") or {}).get("mason") or {}
    if not (mason.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.mason.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    mason = (fm.get("metadata") or {}).get("mason") or {}
    dangling = [
        rs for rs in (mason.get("related_skills") or []) if rs not in _all_names()
    ]
