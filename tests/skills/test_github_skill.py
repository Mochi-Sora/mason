...
def test_frontmatter_required_fields():
    ...
    mason = fm["metadata"]["mason"]
    assert mason["tags"]
    assert "related_skills" in mason
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Mason Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["mason"]["related_skills"]:
        hits = (
