...
`scripts/desktop-update/windows.ps1` drives `mason update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\mason.exe` console-script launcher. On Windows that launcher is
a real process that keeps `mason.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `mason.exe` the launcher still has
...
Start Menu shortcut points at a `Mason.exe` that no longer exists.

Driving the update as `python.exe -m mason_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-MasonStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$masonExe` shim. `mason.exe` may still be *named* in the file for the
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-MasonStep`` to
...
def test_invoke_mason_step_calls_drive_python_not_the_shim() -> None:
    source = _handoff_source()

    invocations = re.findall(r"Invoke-MasonStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-MasonStep call in "
    ...
    assert not offenders, (
        "Every Invoke-MasonStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the mason.exe shim. Driving the update "
        "through the shim keeps mason.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "mason_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m mason_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "mason_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m mason_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_mason_exe_shim() -> None:
    source = _read()

    assert "Invoke-MasonStep $masonExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the mason.exe shim (`Invoke-MasonStep $masonExe`). That is the "
