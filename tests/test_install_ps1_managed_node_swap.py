...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$MasonHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$MasonHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$MasonHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$MasonHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_mason_home() -> None:
    swap = _swap_block()
