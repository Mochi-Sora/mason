...
from mason_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(mason_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <mason_home>/plugins/<name> and opt it in."""
    plugin_dir = mason_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MASON_HOME", str(tmp_path / "mason_empty"))
