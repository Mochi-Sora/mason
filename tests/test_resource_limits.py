...
from mason_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "mason_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import mason_cli.plugins
    import mason_cli.web_server

    # cmd_dashboard(headless_backend=True) exports MASON_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("MASON_SERVE_HEADLESS", "0")
    ...
    monkeypatch.setattr(mason_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "mason_cli.main", "-p", "default"]
    assert exec_call["env"]["MASON_HOME"] == str(tmp_path)
...
def test_dashboard_lifecycle_flags_skip_limit_adjustment(monkeypatch, lifecycle_flag):
    ...
    from mason_cli import main as cli_main
    import mason_cli.main_dashboard as mason_cli_main_dashboard
    ...
    monkeypatch.setattr(mason_cli_main_dashboard, "_find_stale_dashboard_pids", lambda: [])
