...
1. `mason secrets bitwarden setup --help` works (dispatch path)
2. `mason update --check` works (update path)
3. `mason secrets bitwarden disable` works (handler execution)
4. `mason secrets onepassword status` works (lazy backend loads on demand)
...
def _run_mason(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run mason CLI as a subprocess from repo root.
    ...
    connectivity — but ``mason update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-mason-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-mason-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "mason_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`mason secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_mason(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`mason secrets bitwarden disable` must exit 0."""
        result = _run_mason(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`mason secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_mason(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`mason secrets onepassword setup --help` must exit 0."""
        result = _run_mason(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `mason update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`mason update --check` must not load cryptography._rust."""
        result = _run_mason(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['mason','update','--check']
...
sys.argv = ['mason', 'update', '--check']

import mason_cli.main as m
...
# resolves the mason_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('mason_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
