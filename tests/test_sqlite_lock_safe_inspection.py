"""POSIX advisory locks must survive Mason' own database inspection.
...
Mason used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `mason sessions
...
out across Mason' inspection calls.
"""
...
from mason_cli.sqlite_safe_read import (
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_mason_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import mason_cli.sqlite_safe_read as ssr
