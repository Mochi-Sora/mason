...
Older versions of ``install.sh`` created ``$command_link_dir/mason`` as a
symlink to the pip-generated entry point at ``$MASON_BIN`` (i.e.
``venv/bin/mason``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/mason" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$MASON_BIN" "$@"`` then self-recursed and ``mason`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/mason``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/mason\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/mason"')
    cat_idx = block.find('cat > "$command_link_dir/mason" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/mason before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/mason        <- pip entry point (the one we must preserve)
          local_bin/mason       <- symlink → ../venv/bin/mason  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``MASON_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/mason`` still contains its original pip-script body
      * ``local_bin/mason`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "mason"
    ...
    shim_path = command_link_dir / "mason"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/mason was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/mason must be replaced with a regular file, not "
