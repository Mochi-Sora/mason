...
from mason_state_wal import resolve_synchronous_level
...
def _config(monkeypatch, database_section):
    ...
    It imports `mason_cli.config` lazily inside the function body, so the
...
class TestMacOSFloor:
    ...
    def test_lowering_below_full_is_refused_on_darwin(
        ...
        try:
            ...
            mason_state_wal._enforce_macos_synchronous_full(conn)
            assert _level(conn) == 2
            with caplog.at_level("WARNING"):
                apply_database_pragmas(conn, db_label="state.db")
