"""Windows installer regression for Mason children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Mason process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".mason-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Mason launchers
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -MasonHome '{mason_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
