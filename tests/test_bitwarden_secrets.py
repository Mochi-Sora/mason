...
binary download are exercised manually by `mason secrets bitwarden
...
def mason_home(tmp_path, monkeypatch):
    """Point Mason at an isolated home directory."""
    home = tmp_path / ".mason"
    home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(home))
    # Some modules cache get_mason_home; clear if needed.
    import mason_constants
    if hasattr(mason_constants, "_MASON_HOME_CACHE"):
        mason_constants._MASON_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(mason_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from mason_cli.env_loader import _apply_external_secret_sources
