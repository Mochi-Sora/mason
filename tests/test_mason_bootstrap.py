"""Tests for mason_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Mason entry point
(mason, mason-agent, mason-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Mason entry point imports mason_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported mason_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("mason_bootstrap", None)
    import mason_bootstrap  # noqa: WPS433
    return mason_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Mason CLI
...
class TestEntryPointsImportBootstrap:
    """Every Mason entry point must import mason_bootstrap as its
    ...
    # Entry points that invoke Mason as a process.  Each one must
    # import mason_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "mason_cli/main.py",   # mason CLI (console_script)
        "run_agent.py",          # mason-agent (console_script)
        "acp_adapter/entry.py",  # mason-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import mason_bootstrap' and that
        ...
        half-finished ``mason update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``mason_bootstrap`` as a top-level module) leaves mason
        ...
        """
        # Resolve relative to the mason-agent repo root.  Tests live
        # at tests/test_mason_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # mason start even when mason_bootstrap hasn't been
        ...
        assert first_import_name == "mason_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'mason_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import mason_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Mason's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("MASON_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["MASON_PYTHON_SRC_ROOT"] = env
            elif "MASON_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["MASON_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/mason")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MASON_PYTHON_SRC_ROOT", None)
            else:
                os.environ["MASON_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/mason", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Mason root.  The guard must relocate Mason to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/mason"])
        assert result[0] == "/opt/mason"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Mason root.
        assert result.index("/opt/mason") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["MASON_PYTHON_SRC_ROOT"] = "/env/mason"
            hb.harden_import_path()
            assert sys.path[0] == "/env/mason"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MASON_PYTHON_SRC_ROOT", None)
