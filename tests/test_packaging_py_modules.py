...
(``run_agent``, ``mason_state``, ``toolsets``...) reach the wheel through
...
``ModuleNotFoundError`` on ``import mason_state``. This test pins the two halves of that
...
PACKAGES = ("agent", "tools", "mason_cli", "gateway", "tui_gateway", "cron", "acp_adapter", "plugins", "providers")


def _root_py_modules() -> set[str]:
    spec = importlib.util.spec_from_file_location("_mason_setup_py", REPO_ROOT / "setup.py")
...
def test_every_root_module_imported_by_packaged_code_is_shipped():
    ...
    assert "mason_state" in shipped and "setup" not in shipped
