...
from cli import MasonCLI, _build_compact_banner, _rich_text_from_ansi
from mason_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCompactBannerSkinIntegration:
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_mason(self):
        set_active_skin("poseidon")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Mason v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "Poseidon Agent" in banner
        assert "NOUS MASON" not in banner
