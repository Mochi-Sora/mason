...
Under a gateway (Telegram, Discord, Slack, ...) Mason passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Mason via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Mason profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `mason -p partner` pinned to a different person's peer).
