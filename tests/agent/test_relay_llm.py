...
def test_relay_metadata_preserves_provider_name():
    metadata = {"api_mode": "chat_completions", "mason.provider": "explicit"}
...
def test_stream_uses_rewritten_request_and_post_intercept_chunks(relay_turn):
    ...
    try:
        ...
        relay.intercepts.deregister_llm_stream_execution("mason-test-stream")
        relay.intercepts.deregister_llm_request("mason-test-request")
...
def test_live_stream_defers_runtime_shutdown_until_exhaustion(
    ...
    monkeypatch.setenv("MASON_HOME", str(tmp_path / "stream-shutdown-profile"))
    relay_runtime._reset_for_tests()
...
def test_stream_current_primes_lazy_completed_response(relay_turn, monkeypatch):
    """A lazy Relay stream must run once before Mason decides its shape."""
    _relay, _turn = relay_turn
