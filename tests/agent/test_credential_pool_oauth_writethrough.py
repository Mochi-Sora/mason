...
Companion to ``tests/mason_cli/test_xai_oauth_writethrough.py``. That file
...
from mason_cli import auth as A
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.mason/auth.json``; a tmp_path
...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two Mason processes both read the
...
def test_mason_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    """A successful mason_pkce refresh must update
    ~/.mason/.anthropic_oauth.json, or ``_seed_from_singletons()`` on the
    ...
    oauth_file = mason_home / ".anthropic_oauth.json"
    oauth_file.write_text(
        json.dumps({"accessToken": "sk-ant-oat-rt0", "refreshToken": "rt0", "expiresAt": 0}),
        encoding="utf-8",
    )
    _write_store(mason_home / "auth.json", {"version": 1, "providers": {}})
    ...
    assert on_disk["refreshToken"] == "rt1", (
        "successful mason_pkce refresh must write back to "
        "~/.mason/.anthropic_oauth.json, or _seed_from_singletons() will "
        "revert the pool entry to the pre-refresh (spent) token on next load"
    )

    reloaded = load_pool("anthropic")
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("mason_pkce")]
    assert reloaded_entries, "mason_pkce entry should still be present after reload"
...
def test_manual_mason_pkce_refresh_does_not_create_duplicate_singleton(
    tmp_path, monkeypatch
):
    """A pool-owned manual:mason_pkce entry must not create a second source."""
    mason_home = tmp_path / "mason"
    mason_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    monkeypatch.setattr("mason_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    assert not oauth_file.exists(), (
        "manual:mason_pkce is already pool-owned; refreshing it must not "
        "create a second mason_pkce singleton source"
    )

    reloaded = load_pool("anthropic")
    matching = [e for e in reloaded.entries() if e.id == "manual-entry"]
    assert len(matching) == 1
    assert matching[0].source == "manual:mason_pkce"
