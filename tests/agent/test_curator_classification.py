...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"mason-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "mason-agent-dev", "declared": True},
        },
    )
...
# `mason curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `mason curator pin <umbrella>` in the rename block so
