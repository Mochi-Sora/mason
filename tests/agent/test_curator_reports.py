...
Reports live under ``~/.mason/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated MASON_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".mason"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "MASON_DIR", home)
