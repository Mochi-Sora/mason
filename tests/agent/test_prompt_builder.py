...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.config.load_config", default_load_config)
        monkeypatch.setattr("mason_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("mason_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    ...
    def test_mason_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".mason.md").write_text("Mason-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Mason-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("MASON_HOME", str(tmp_path / "mason_home"))
        mason_home = tmp_path / "mason_home"
        mason_home.mkdir()
        (mason_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
...
# =========================================================================
# .mason.md helper functions
# =========================================================================


class TestFindMasonMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".mason.md").write_text("rules")
        assert _find_mason_md(tmp_path) == tmp_path / ".mason.md"
    ...
    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".mason.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_mason_md(sub) == tmp_path / ".mason.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .mason.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """MASON_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("MASON_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._BACKEND_PROBE_CACHE.clear()
        result = _pb.build_environment_hints()
...
class TestContextFileReadTimeout:
    def test_slow_mason_md_is_skipped_and_agents_md_still_loads(self, tmp_path, monkeypatch, caplog):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".mason.md").write_text("Mason project rules.")
        ...
        assert "Mason project rules" not in result
