...
Mason *processes* via the cross-process ``_auth_store_lock`` flock,
...
Mason's own PKCE login (``manual:mason_pkce`` / ``mason_pkce``) got no
...
def _fake_pool_store(monkeypatch):
    ...
    This stands in for ``~/.mason/auth.json`` so the two "process-local"
...
def test_anthropic_refresh_is_protected_by_cross_process_lock(monkeypatch):
    ...
    pool = CredentialPool(
        "anthropic",
        [_entry(id="a1", access_token="stale-at", refresh_token="stale-rt", source="manual:mason_pkce")],
    )
...
def test_concurrent_mason_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'Mason processes' race to refresh the same stale mason_pkce
    ...
    assert loser_result is not None, (
        ...
        "disk. mason_pkce-sourced entries must adopt the winner's token "
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        "regression: the losing process marked its Anthropic mason_pkce "
        "credential STATUS_EXHAUSTED after a lost refresh race, even "
        "though the account is not actually exhausted -- a sibling Mason "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent Mason usage (fleet workers, cron jobs, "
...
def test_concurrent_claude_code_refresh_recovers_via_credentials_file(monkeypatch):
    ...
    evidence that mason_pkce/dashboard-sourced credentials were simply
