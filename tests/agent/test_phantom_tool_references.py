...
Covers:
  * MASON_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestMasonAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import MASON_AGENT_HELP_GUIDANCE
        assert "skill_view(name='mason-agent')" in MASON_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import MASON_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in MASON_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "mason-agent.nousresearch.com/docs" in MASON_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the mason-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "mason-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_mason_home", lambda: home)
        ...
        assert result["copied"] == ["mason-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "mason-agent" / "SKILL.md").exists()
