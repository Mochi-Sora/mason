...
def mason_home(tmp_path, monkeypatch):
    """Real on-disk MASON_HOME so ``load_pool()`` re-reads what it persisted."""
    home = tmp_path / "mason"
...
def test_persisted_claude_code_row_carries_no_token_material(
    ...
    rows = [r for r in _persisted_rows(mason_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(mason_home, claude_credentials):
...
def test_empty_oauth_entry_is_never_leased(mason_home, claude_credentials):
...
def test_mason_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    """The borrowed-source refusal must not disable pool-owned adoption.

    ``mason_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
