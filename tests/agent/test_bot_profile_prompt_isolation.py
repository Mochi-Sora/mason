...
thread that did not bind the MASON_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_mason_home()``,
so an unbound build thread fell back to ``~/.mason`` (default) and leaked
default's full skills index + "Active Mason profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient MASON_HOME, on a
    ...
    def build():
        # No set_mason_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_mason_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    mason ROOT (get_default_mason_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: MASON_HOME env points at the profile dir itself.
    monkeypatch.setenv("MASON_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MASON_HOME", str(other_home))
