...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated MASON_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="mason_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".mason"))
    prev_home = os.environ.get("MASON_HOME")
    os.environ["MASON_HOME"] = os.path.join(test_home, ".mason")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("mason_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into mason_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["MASON_HOME"] = prev_home
