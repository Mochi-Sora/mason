...
A skill whose metadata.mason.session_platforms names gateway channels is
...
def _conds(platforms):
    return extract_skill_conditions(
        {"metadata": {"mason": {"session_platforms": platforms}}}
    )
...
class TestSessionPlatformGate:
    ...
    def test_teams_meeting_pipeline_carries_the_gate(self):
        ...
        assert fm["metadata"]["mason"]["session_platforms"] == ["teams", "cron"]
