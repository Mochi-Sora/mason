...
def mason_home(monkeypatch):
    """Isolate MASON_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``MASON_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="mason-reload-skills-")
    monkeypatch.setenv("MASON_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "MASON_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, mason_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(mason_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, mason_home):
