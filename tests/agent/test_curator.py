...
def curator_env(tmp_path, monkeypatch):
    """Isolated MASON_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".mason"
    ...
    # save_state() resolves the state path from MASON_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.mason once monkeypatch
    ...
    # thread here, while MASON_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from mason_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `mason model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from mason_cli.main_provider_setup import _AUX_TASKS
    from mason_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. mason_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. mason_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``mason curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``mason curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.mason/skills/<umbrella>/references/ && mv ...`` its
