...
def test_fast_auto_and_cold_parse_and_slash_command(monkeypatch):
    ...
    from mason_cli.commands import COMMAND_REGISTRY
    ...
    cli_mod.MasonCLI._handle_fast_command(stub, "/fast auto")
    assert stub.service_tier == "auto"
    assert stub.agent is None
    cli_mod.MasonCLI._handle_fast_command(stub, "/fast status")
    assert any("auto" in line for line in printed)
    cli_mod.MasonCLI._handle_fast_command(stub, "/fast cold")
    ...
    assert cli_mod.MasonCLI._resolve_turn_agent_config(route_stub, "hi")["request_overrides"] is None
    route_stub.service_tier = "priority"
    assert cli_mod.MasonCLI._resolve_turn_agent_config(route_stub, "hi")["request_overrides"] == {
        "service_tier": "priority"
    }
