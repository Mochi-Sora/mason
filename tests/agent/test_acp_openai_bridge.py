"""The ACP text bridge is what makes Mason' own tools reachable on an ACP provider.
...
def test_allowlist_forwards_only_the_named_tools():
    ...
    make Mason re-run finished work, so those clients forward an allowlist."""
...
def test_response_level_extras_survive_the_stream_conversion():
    """Mason reads provider extras off the returned object; a plain list would
    drop them and silently disable the projection on stream=True."""
    chunks = completion_to_stream_chunks(
        _completion(mason_projected_messages=[{"role": "tool", "content": "x"}])
    )
    assert isinstance(chunks, StreamChunks)
    assert isinstance(chunks, list)
    assert chunks.mason_projected_messages == [{"role": "tool", "content": "x"}]
