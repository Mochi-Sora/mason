...
class TestDesktopHintBlockRemoved:
    ...
    def test_build_environment_hints_has_no_embedded_pane_clarifier(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_DESKTOP", "1")
        monkeypatch.setenv("MASON_DESKTOP_TERMINAL", "1")
        from agent.prompt_builder import _BACKEND_PROBE_CACHE
        _BACKEND_PROBE_CACHE.clear()
        hints = build_environment_hints()
...
class TestEmbeddedTuiPaneClarifier:
    """When ``MASON_DESKTOP_TERMINAL=1``, a standalone ``mason --tui`` is
    ...
    def test_falsy_env_does_not_trigger_clarifier(self, monkeypatch, val):
        monkeypatch.setenv("MASON_DESKTOP_TERMINAL", val)
        out = _tui_embedded_pane_clarifier(PLATFORM_HINTS["tui"])
        assert out == PLATFORM_HINTS["tui"], (
            f"MASON_DESKTOP_TERMINAL={val!r} should not trigger clarifier"
        )


class TestContradictionGone:
    """The original contradiction: a single assembled system prompt
    contained both ``You are running in the Mason terminal UI (TUI).`` and
    ``Runtime surface: you're running inside the Mason desktop GUI app.``.
