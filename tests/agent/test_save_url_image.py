...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate MASON_HOME under tmp_path."""
    monkeypatch.setenv("MASON_HOME", str(tmp_path / ".mason"))
    (tmp_path / ".mason").mkdir()

    # Force the constants/image cache helpers to re-read MASON_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("mason_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_mason_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under MASON_HOME — gateway cleanup
