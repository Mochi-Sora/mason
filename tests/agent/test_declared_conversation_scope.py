...
conversation-affinity hint Mason sends — ``prompt_cache_key`` on both
...
bucket it warmed. Mason cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Mason-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Mason-internal token; a session KEY embeds the
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Mason-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``MASON_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("MASON_SESSION_SOURCE", "override-src")
