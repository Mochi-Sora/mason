"""Tests for the core Relay-managed Mason tool adapter."""
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "mason-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "mason-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("mason-test-tool-execution")
        relay.intercepts.deregister_tool_request("mason-test-tool-request")
