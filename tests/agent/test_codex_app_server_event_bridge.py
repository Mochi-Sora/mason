"""Regression tests for the codex_app_server → Mason UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_mason_tools_mcp_server_emits_bare_tool_name(self):
        """The mason-tools MCP server wraps Mason' own tools for codex;
        ...
        mcp.mason-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "mason-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "mason-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "mason agent docs"
        assert calls[0].args[3] == {"query": "mason agent docs"}
