...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_mason_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for mason_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": mason_level}
            )
            assert kw["reasoning_effort"] == wire_level, mason_level
