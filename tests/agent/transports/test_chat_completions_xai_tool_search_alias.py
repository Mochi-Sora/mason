...
``mason_tool_search`` for xAI targets and map the alias back to
...
named ``mason_tool_search`` is never silently dispatched as the bridge.
"""
...
class TestRenameToolSearchBridgeForXai:
    def test_tool_search_renamed_alias_value(self):
        ...
        assert out[0]["function"]["name"] == "mason_tool_search"
        assert alias_map == {"mason_tool_search": "tool_search"}
    ...
    def test_collision_with_real_mason_tool_search_takes_suffix(self):
        ...
        # takes mason_tool_search_2 instead.
        tools = [
            {"type": "function", "function": {"name": "mason_tool_search"}},
        ...
        assert alias_map == {"mason_tool_search_2": "tool_search"}
...
class TestNormalizeResponseMapsAliasBack:
    def test_alias_call_maps_back_to_bridge_name(self, transport):
        transport._last_wire_aliases = {"mason_tool_search": "tool_search"}
    ...
    def test_no_alias_emitted_means_no_reverse_rewrite(self, transport):
        ...
        # call named mason_tool_search is a REAL tool (user/plugin/MCP)
        # and must dispatch under its own name.
        transport._last_wire_aliases = {}
        resp = transport.normalize_response(_fake_response("mason_tool_search"))
        assert resp.tool_calls[0].name == "mason_tool_search"

    def test_suffixed_alias_maps_back(self, transport):
        transport._last_wire_aliases = {"mason_tool_search_2": "tool_search"}
        resp = transport.normalize_response(_fake_response("mason_tool_search_2"))
        assert resp.tool_calls[0].name == "tool_search"
        # And the real tool occupying the plain alias name is untouched.
        resp2 = transport.normalize_response(_fake_response("mason_tool_search"))
        assert resp2.tool_calls[0].name == "mason_tool_search"
