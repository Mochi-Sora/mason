"""Tests for MASON_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/mason-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``MASON_HOME`` itself,
...
for the credential stores while leaving arbitrary ``MASON_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_mason_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "mason_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_mason_home_path", lambda: home)
    return home
...
def test_arbitrary_mason_home_file_not_blocked(fake_home):
    """Non-credential files inside MASON_HOME stay readable."""
...
def test_identically_named_mason_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Mason-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside MASON_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside MASON_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside MASON_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, MASON_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_mason_home_path", lambda: profile)
