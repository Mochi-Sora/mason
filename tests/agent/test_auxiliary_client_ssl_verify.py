...
``MASON_CA_BUNDLE`` via ``agent.ssl_verify.resolve_httpx_verify``. Auxiliary calls
...
_CA_ENV_VARS = ("MASON_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "HTTPS_PROXY")
...
def test_resolve_aux_verify_ssl_verify_false(clean_tls_env, monkeypatch):
    ...
    import mason_cli.config_providers as config_providers
