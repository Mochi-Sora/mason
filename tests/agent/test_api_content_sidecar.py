...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("mason_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated MASON_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="mason_api_content_")
