"""Affinity contract on Mason Studio's group-chat bridge shape (#96811).

Studio's group chat is the reproduction reported on #96811, and it reaches
Mason as a LIBRARY rather than through the gateway: its Python bridge
...
class TestStudioBridgeAffinity:
    def test_the_undeclared_bridge_shape_rekeys_every_reply(self, db):
        ...
        Mason must not infer the conversation from the id's syntax — that is
    ...
    def test_a_new_session_seed_rotates_the_affinity(self, db):
        ...
        without Mason having to observe a reset of its own.
        """
