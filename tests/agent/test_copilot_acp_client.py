...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_read_text_file_redacts_sensitive_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # agent.redact snapshots MASON_REDACT_SECRETS at import time into
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"MASON_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
        ...
        self.assertIn("MASON_WRITE_SAFE_ROOT", str(response["error"]))
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    (mason_home / "home").mkdir(parents=True)
    real_home = tmp_path / "real-home"
    real_home.mkdir()

    monkeypatch.setenv("HOME", str(real_home))
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    # Hermeticity: an ambient MASON_REAL_HOME (exported by Mason' own
    ...
    assert captured["kwargs"]["env"]["MASON_REAL_HOME"] == str(real_home)
...
# `session/set_model` call. Without it, picking gpt-5.6-terra in Mason
