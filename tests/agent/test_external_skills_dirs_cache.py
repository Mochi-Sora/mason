...
@pytest.fixture
def mason_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.mason/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".mason"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(mason_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = mason_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different MASON_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".mason"
