...
path via ``mason chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp MASON_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MASON_HOME at a temp dir and turn moa.save_traces on."""
    mason_home = tmp_path / ".mason"
    mason_home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(mason_home))

    # save_moa_turn reads config via mason_cli.config.load_config; stub it to
