...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Mason Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = mason_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.mason.
    assert "/root/.mason/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.mason/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.mason/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.mason/auth.json` and read the
    ...
    oauth = mason_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = mason_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.mason/auth.json and @file:.mason/.anthropic_oauth.json "
        "and @file:.mason/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Mason/composer-images/a.png",
