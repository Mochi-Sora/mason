...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Mason root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, mason_home: Path):
    """Point file_safety._mason_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_mason, monkeypatch):
        _set_active_home(monkeypatch, fake_mason["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_mason, monkeypatch):
        """If MASON_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_mason_home_path", _boom)
