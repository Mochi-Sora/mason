...
def _process_claude_code_refresh_worker(
    ...
    os.environ["MASON_HOME"] = profile_home
    ...
    def fake_refresh(refresh_token, *, use_json=False):
        ...
        # what must ensure that the second Mason process never calls this
...
def mason_home(tmp_path, monkeypatch):
    """Real, throwaway MASON_HOME so _auth_store_lock and
...
def test_high_concurrency_anthropic_refresh_no_lost_updates_no_deadlock(
    ...
    """CONCURRENCY 'Mason processes' race the same stale refresh token
    ...
    shared_stale_entry = _entry(
        id="pool-entry", refresh_token="stale-rt", source="manual:mason_pkce"
    )
    pools = [
...
def test_distinct_profiles_share_one_claude_refresh_without_duplicate_post(
    ...
    """
    shared_credentials_path = mason_home / "shared-claude-credentials.json"
    ...
    server_state_path = mason_home / "fake-token-server.json"
    server_state_path.write_text(
        json.dumps({"calls": [], "spent": [], "rotation": 0}),
        encoding="utf-8",
    )

    profile_homes = [mason_home / "profile-a", mason_home / "profile-b"]
