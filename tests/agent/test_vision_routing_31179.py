...
def isolated_home(monkeypatch):
    """Temp MASON_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="mason_test_31179_")
    mason_home = os.path.join(test_home, ".mason")
...
def _fresh_modules():
    """Drop cached mason modules so each test reloads against current env."""
    _drop_reload_targets()
