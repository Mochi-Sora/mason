...
``~/.mason/.anthropic_oauth.json`` for ``mason_pkce`` ones.  Those singletons
...
def mason_home(tmp_path, monkeypatch):
    """Real on-disk MASON_HOME so ``load_pool()`` re-reads what we persisted."""
    home = tmp_path / "mason"
    ...
    monkeypatch.setattr(
        "mason_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_mason_oauth_writer_raises_instead_of_swallowing(mason_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_mason_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    mason_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: mason_pkce
# ---------------------------------------------------------------------------


def test_pool_mason_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "mason_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
