...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_mason_home``,
    ``get_default_mason_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``MASON_HOME`` and no resolver mocks.
    """

    def test_real_mason_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_mason_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Mason profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """MASON_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".mason"
        ...
        assert "Active Mason profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "MASON_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "MASON_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_mason_home", lambda: Path("/mason"))

    # Production renders this as str(get_mason_home()) + "/profiles/<name>/",
    # and str(Path("/mason")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/mason".
    _home_str = str(Path("/mason"))
    expected_profile = (
    ...
    expected = "\n\n".join((
        ...
        "STEER",
        "CODING_STABLE",
        # Live references, not frozen text: this test pins ORDER (cache split),
        # not wording — wording is owned by state_history_prompt + working_set.
        system_prompt.STATE_HISTORY_INSTRUCTION.strip(),
        system_prompt.WORKING_SET_PROTOCOL.strip(),
        "WORKSPACE",
    ...
    with (
        ...
        patch("mason_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("mason_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under MASON_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from mason_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("mason_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from mason_time import now as mason_now
        sid = mason_now().strftime("%Y%m%d_%H%M%S_fresh")
