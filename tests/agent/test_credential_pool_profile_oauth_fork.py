...
Real imports, real temp MASON_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root MASON_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    # against $HOME/.mason/auth.json; our root is elsewhere, so writes go.
    import mason_constants
    mason_constants._default_mason_root_memo = None  # type: ignore[attr-defined]
    ...
    store = {
        ...
        "credential_pool": {
            "anthropic": [{
                ...
                "priority": 0, "source": "manual:mason_pkce",
    ...
    import mason_cli.auth as _auth_mod
...
def _profile(fleet, name, **kw):
    from mason_cli.profiles import create_profile
...
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path):
    """Root's mason_pkce row is seeded from ROOT's .anthropic_oauth.json; a
    ...
    assert [e.source for e in root_rows] == ["mason_pkce"]

    kid = _profile(fleet, "kid")
    fleet["use"](kid)
    pool = load_pool("anthropic")
    assert [e.source for e in pool.entries()] == ["mason_pkce"], "borrowed root grant was pruned"
    assert fleet["rows"](root) and fleet["rows"](root)[0]["source"] == "mason_pkce"
...
def test_profile_auth_add_owns_only_its_own_rows(fleet):
    ...
    pool.add_entry(PooledCredential(
        ...
        priority=0, source="manual:mason_pkce", access_token="sk-ant-oat01-MINE",
...
def test_heal_consolidates_existing_forks_to_the_live_copy(fleet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="mason_cli.auth"):
        fleet["use"](forge)
        sel = load_pool("anthropic").select()
...
def test_heal_is_idempotent_and_logs_once(fleet, caplog):
    import logging
    from agent.credential_pool import load_pool
    from mason_cli.auth import consume_oauth_heal_notices, heal_forked_single_use_oauth_grants
...
def test_heal_never_deletes_the_only_surviving_copy(fleet):
    """Root lost its grant (user ran `mason auth remove` at root); the profile's
...
def test_heal_pkce_singleton_shape_commits_live_pair_to_root_singleton(fleet):
    """`mason auth` PKCE shape: root + profile each have .anthropic_oauth.json +
    a mason_pkce-seeded row; the profile's copy is the rotated (live) one."""
    from agent.credential_pool import load_pool
    ...
    fleet["use"](root)
    load_pool("anthropic")  # seeds root's mason_pkce row from the singleton
...
def test_heal_is_a_noop_in_classic_mode(fleet):
    from mason_cli.auth import heal_forked_single_use_oauth_grants
    fleet["use"](fleet["root"])
...
def test_heal_skips_profile_auth_json_symlinked_to_the_root_store(fleet):
    """#101356: `ln -s ~/.mason/auth.json <profile>/auth.json` shares ONE store.
