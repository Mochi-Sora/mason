...
Relay runs its finalizer as soon as the provider stream ends — concurrently with Mason'
...
def _stream_through_relay(tmp_path, monkeypatch, response_body: bytes, *, finalize_before):
    ...
    Returns ``(mason_result, relay_llm_end_event)``."""
    ...
    monkeypatch.setenv("MASON_STREAM_RETRIES", "0")
...
def test_openai_stream_final_tool_call_delta_reaches_relay_parent_event(tmp_path, monkeypatch):
    ...
    mason_call = result.choices[0].message.tool_calls[0]
    assert (mason_call.function.name, mason_call.function.arguments) == ("read_file", '{"path": "/tmp/x"}')
