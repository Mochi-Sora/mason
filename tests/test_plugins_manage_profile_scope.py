...
``set_mason_home_override`` / ``reset_mason_home_override``. Because
``_plugins_dir()`` keys off ``get_mason_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import mason_cli.profiles as profiles
