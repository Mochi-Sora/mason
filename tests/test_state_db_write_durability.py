...
The defect: mason_state already knows macOS `fsync()` does not guarantee
...
import mason_state
from mason_state import repair_state_db_schema
from mason_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in mason_state_repair; mason_state only
    # re-imports them.
    import mason_state_repair

    source = Path(mason_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(mason_state_repair.__file__))
