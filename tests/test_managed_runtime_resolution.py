"""Guard: Mason-owned subprocesses must not resolve managed runtimes by bare PATH.

Mason installs runtimes for itself — ``uv`` at ``$MASON_HOME/bin/uv``, Node at
``$MASON_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Mason's own
...
* when a system copy also exists, the one Mason does not own wins — which is
...
``iter_mason_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Mason provisions into MASON_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Mason-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("mason_cli/update_cmd_deps.py", "uv"): (
    ...
    ("mason_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (mason_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Mason-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$MASON_HOME/bin (uv) and $MASON_HOME/node are not on an "
        ...
        "  node/npm -> mason_constants.find_node_executable()\n"
        "  PATH env -> mason_constants.iter_mason_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(mason_constants, helper))


def test_managed_uv_helpers_exist():
    from mason_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
