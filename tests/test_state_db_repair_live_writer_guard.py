...
def test_repair_refuses_while_another_connection_holds_the_db(tmp_path):
    ...
    (``mason_state_holders.live_writer_holds_db``) first scans for foreign
    holders — deleted sidecar generations, uninspectable Mason processes —
    ...
    SQLite builds carrying the WAL-reset bug (and on NFS/SMB) Mason runs
...
def test_repair_checks_foreign_holders_before_opening_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mason_state_repair, "_connect_repair_durable", _unexpected_probe)
...
def test_linux_holder_scan_does_not_require_psutil(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mason_state_holders, "psutil", None)

    holders = mason_state_holders.foreign_state_db_holders(
...
def test_incomplete_holder_scan_keeps_unknown_sentinel(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mason_state_holders.os, "readlink", lambda _path: str(db))
    real_stat = mason_state_holders.os.stat
    ...
    holders = mason_state_holders.foreign_state_db_holders(db)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (["python3", "-m", "mason_cli.main", "gateway"], True),
    ),
)
def test_uninspectable_unknown_descriptor_uses_mason_identity_at_repair_boundary(
    tmp_path, monkeypatch, argv, should_block
):
    """An unknown fd target blocks only when argv identifies Mason."""
    db = _make_wal_db(tmp_path)
    ...
    if should_block:
        ...
        def _unexpected_probe(*_args, **_kwargs):
            pytest.fail("repair opened SQLite with an unproven Mason descriptor")

        monkeypatch.setattr(
            mason_state_repair, "_connect_repair_durable",
            _unexpected_probe,
        )
        report = repair_state_db_schema(db, backup=False)
        ...
        real_connect = mason_state_repair._connect_repair_durable
        probe_reached = False
...
def test_uninspectable_watched_identity_blocks_alias_before_sqlite(
    ...
    monkeypatch.setattr(mason_state_holders.os, "listdir", _listdir)
    monkeypatch.setattr(
        mason_state_holders.os, "readlink", lambda _path: str(alias)
    )
    monkeypatch.setattr(mason_state_holders.os, "stat", _stat)
...
@pytest.mark.linux_only
def test_uninspectable_alias_descriptor_for_mason_blocks_before_sqlite(
    tmp_path, monkeypatch
):
    """Mason cannot make an aliased fd safe when its identity is unreadable."""
    db = _make_wal_db(tmp_path)
    alias = tmp_path / "namespace-alias" / "state.db"
    ...
    monkeypatch.setattr(
        mason_state_holders,
        "_read_proc_argv",
        lambda _pid: ["python3", "-m", "mason_cli.main", "gateway"],
    )

    def _unexpected_probe(*_args, **_kwargs):
        pytest.fail("repair opened SQLite with an unproven Mason alias fd")
