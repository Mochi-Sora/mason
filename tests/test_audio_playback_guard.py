...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``mason_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_MASON_BEHAVIORAL_VARS`` now blanks ``MASON_VOICE`` /
    ``MASON_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("MASON_VOICE_TTS") == "1"
    ...
    # calls it (a late `from mason_cli.voice import speak_text`), with the
    # exact fixture string that came out of the speakers.
    from mason_cli.voice import speak_text
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``MASON_VOICE_TTS=1`` set.
    Before ``MASON_VOICE``/``MASON_VOICE_TTS`` were added to
    ``_MASON_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "MASON_VOICE_TTS" not in os.environ
    assert "MASON_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import mason_cli.voice as voice
