...
Mason must not *enable* multi-process WAL on SQLite builds that still contain
...
from mason_state_wal import apply_wal_with_fallback, is_sqlite_wal_reset_vulnerable, sqlite_source_id


@pytest.fixture(autouse=True)
def _reset_wal_reset_bug_warnings():
    mason_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    mason_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            mason_state_wal, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
...
class TestNoDowngradeUnderConcurrentOpeners:
    ...
    def test_configured_delete_refuses_when_probe_blocked(
        ...
        monkeypatch.setattr(mason_state_wal, "resolve_journal_mode", lambda: "delete")
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(mason_state_wal, "sqlite_source_id", lambda: "testid-abc")
