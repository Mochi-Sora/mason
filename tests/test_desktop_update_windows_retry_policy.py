...
def test_retry_policy_distinguishes_self_lock_deferral(tmp_path: Path) -> None:
    ...
    command = f"""
        . '{policy}'
        $withoutMarker = @(
            (Test-MasonUpdateShouldRetry -ExitCode 0 -InstallRoot '{root}'),
            (Test-MasonUpdateShouldRetry -ExitCode 1 -InstallRoot '{root}'),
            (Test-MasonUpdateShouldRetry -ExitCode 2 -InstallRoot '{root}')
        )
        New-Item -ItemType File -Path (Join-Path '{root}' '.update-incomplete') | Out-Null
        $withMarker = Test-MasonUpdateShouldRetry -ExitCode 2 -InstallRoot '{root}'
