...
Regression for the Aug 2026 `mason -w` timeout incident: 39 accumulated packs
...
(lock pid = the live mason process), poisoning every retry.
...
class TestCleanupFailedWorktreeAdd:
    def _simulate_timed_out_add(self, repo):
        ...
        wt = repo / ".worktrees" / "mason-dead00"
        _git(repo, "worktree", "add", str(wt), "-b", "mason/mason-dead00")
        # Live-pid lock, exactly what `mason -w` writes before the checkout.
        _git(repo, "worktree", "lock", str(wt), "--reason", "mason pid=999999")
    ...
    def test_sweeps_dir_admin_entry_and_branch(self, repo):
        from mason_cli.worktree_ops import _cleanup_failed_worktree_add

        wt = self._simulate_timed_out_add(repo)
        admin = repo / ".git" / "worktrees" / "mason-dead00"
        assert admin.exists() and (admin / "locked").exists()

        _cleanup_failed_worktree_add(str(repo), wt, "mason/mason-dead00")

        assert not wt.exists(), "partial worktree dir must be removed"
        assert not admin.exists(), "LOCKED admin entry must be removed"
        branches = _git(repo, "branch", "--list", "mason/mason-dead00").stdout
    ...
    def test_retry_succeeds_after_cleanup(self, repo):
        ...
        result = _git(
            repo, "worktree", "add", str(wt), "-b", "mason/mason-dead00", check=False
        )
    ...
    def test_noop_when_nothing_exists(self, repo):
        ...
        _cleanup_failed_worktree_add(
            str(repo), repo / ".worktrees" / "never-existed", "mason/never-existed"
        )  # must not raise
