...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = MasonCLI.__new__(MasonCLI)
        host.model = "opus"
        ...
        with patch.object(MasonCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(MasonCLI, "_is_session_yolo_active", return_value=False):
            text = MasonCLI._build_status_bar_text(host, width=width)
