...
SessionDB.session_gateway_runtime (mason_state.py) — the round trip that
makes `mason --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare MasonCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.MasonCLI)
