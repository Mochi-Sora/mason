...
Updated for the single-owner unification (mason_cli.personality): built-ins
...
class TestCLIPersonalityNone:
    ...
    def _make_cli(self, personalities=None):
        ...
        from mason_cli.personality import available_personalities
    ...
    def test_set_persists_display_personality_not_system_prompt(self):
        ...
        with patch("mason_cli.personality.persist_personality", side_effect=_persist):
    ...
    def test_neutral_restores_manual_system_prompt_without_wiping_config(self):
        ...
        with (
            patch("mason_cli.personality.persist_personality", side_effect=_persist),
            patch(
    ...
    def test_builtin_personality_works_without_config_entry(self):
        # Built-ins come from mason_cli.personality, not from config.
        cli = self._make_cli(personalities={})
        with patch("mason_cli.personality.persist_personality", return_value=True):
...
class TestGatewayPersonalityNone:
    ...
    def _gateway_env(self, tmp_path):
        ...
        # gateway.run._mason_home) and persists via persist_personality
        # (rooted at MASON_HOME) — point both at the same tmp dir.
        return (
            patch("gateway.run._mason_home", tmp_path),
            patch.dict(os.environ, {"MASON_HOME": str(tmp_path)}),
        )
