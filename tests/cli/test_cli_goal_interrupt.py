...
minimal ``MasonCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def mason_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".mason"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("MASON_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves MASON_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal MasonCLI stub with an active goal wired in."""
    from cli import MasonCLI
    from mason_cli.goals import GoalManager

    cli = MasonCLI.__new__(MasonCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, mason_home):
        ...
        with patch("mason_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, mason_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, mason_home):
        ...
        src = inspect.getsource(MasonCLI.chat)
