...
``mason chat -q`` wires the interactive prompt_toolkit clarify callback
...
inter-agent wakeups all deliver work as ``mason chat -q``, so an agent that
...
def test_no_choices_returns_immediate_headless_answer():
    from mason_cli.cli_agent_setup_mixin import _single_query_clarify_callback
...
def test_agent_construction_gates_clarify_callback_on_single_query_mode():
    ...
    from mason_cli import cli_agent_setup_mixin as mixin_mod
