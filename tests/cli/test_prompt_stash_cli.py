...
def _make_cli(**kwargs):
    """Create a MasonCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports mason_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``MasonCLI.run()``'s local ``KeyBindings`` instance, so they are not
