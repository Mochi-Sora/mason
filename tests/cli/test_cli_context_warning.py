...
def _isolate(tmp_path, monkeypatch):
    """Isolate MASON_HOME so tests don't touch real config."""
    home = tmp_path / ".mason"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal MasonCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("mason_cli.banner.build_welcome_banner"):
        from cli import MasonCLI
        obj = MasonCLI.__new__(MasonCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["mason-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Mason' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Mason' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Mason' minimum context length."""
