...
def session_db(tmp_path):
    ...
    os.environ["MASON_HOME"] = str(tmp_path / ".mason")
    os.makedirs(tmp_path / ".mason", exist_ok=True)
    from mason_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".mason" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal MasonCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        MasonCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        MasonCLI._handle_branch_command(cli_instance, "/branch refactor approach")
