...
The recovery path lives in ``MasonCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A MasonCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(MasonCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise MasonCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("mason_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        bare_cli._force_full_redraw = MagicMock()
        with patch(
            "mason_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "mason_cli.curses_ui.flush_stdin",
    ...
    def test_flush_stdin_is_tty_gated(self):
        ...
        touching termios.
        """
        from mason_cli.curses_ui import flush_stdin
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        # process_loop's post-turn finally block is MasonCLI._tui_after_turn.
        src = inspect.getsource(MasonCLI._tui_after_turn)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(MasonCLI, "_recover_terminal_after_interrupt")
        assert callable(MasonCLI._recover_terminal_after_interrupt)
