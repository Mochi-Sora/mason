...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_mason_home(), so
        # point MASON_HOME at the temp dir (the _mason_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "mason_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in MASON_HOME/config.yaml
    ...
    install whose MASON_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read MASON_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh MASON_HOME with NO config.yaml (managed/desktop first launch).
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        monkeypatch.setenv("MASON_HOME", str(mason_home))
