"""Parity guard for the slash-command dispatch table in cli.MasonCLI.
...
def test_registry_names_resolve_into_the_table():
    from mason_cli.commands import COMMAND_REGISTRY, resolve_command

    for name in MasonCLI._SLASH_DISPATCH:
        cmd = resolve_command(name)
        assert cmd is not None and MasonCLI._slash_handler(cmd.name) is not None, name
    # registry commands the CLI never handled inline must still fall through
    dispatched = {c.name for c in COMMAND_REGISTRY if MasonCLI._slash_handler(c.name)}
    assert dispatched == set(OLD_CHAIN_COMMANDS) - {"exit"} | {"quit"}


def _cli():
    c = MasonCLI.__new__(MasonCLI)
...
def test_dispatch_return_semantics_and_side_effects():
    c = _cli()
    with patch.object(MasonCLI, "_toggle_yolo", return_value=None) as m, \
            patch("mason_cli.plugins.fire_pre_command_hook") as hook:
    ...
    c = _cli()
    with patch.object(MasonCLI, "_handle_resume_command") as m:
    ...
    c = _cli()
    with patch.object(MasonCLI, "_handle_handoff_command", return_value=False):
        assert c.process_command("/handoff telegram") is False
    with patch.object(MasonCLI, "_handle_handoff_command", return_value=True):
        assert c.process_command("/handoff telegram") is True
    with patch.object(MasonCLI, "_handle_update_command", return_value=True):
        assert c.process_command("/update") is False
    with patch.object(MasonCLI, "_handle_update_command", return_value=False):
...
def test_unknown_command_falls_through():
    ...
    with patch.object(MasonCLI, "_process_unregistered_slash", return_value=True) as m:
