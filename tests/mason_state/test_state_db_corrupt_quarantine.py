...
from mason_state import SessionDB, StateDbCorruptError
...
class TestQuarantinedHandleStopsTouchingTheFile:
    ...
    def test_close_skips_wal_checkpoint_when_quarantined(self, tmp_path, caplog):
        ...
        assert any(
            ...
            and "mason sessions recover" in rec.getMessage()
    ...
    def test_reopen_after_close_refused_when_quarantined(self, tmp_path, monkeypatch):
        ...
        db.close()
        reopen = MagicMock()
        monkeypatch.setattr("mason_state._connect_tracked_db", reopen)
...
class TestQuarantineScope:
    ...
    def test_replaced_file_takes_precedence_over_corrupt(self, tmp_path):
        ...
        from mason_state import StateDbReplacedError
    ...
    def test_classify_persistence_error_maps_quarantine_to_corrupt(self):
        from mason_state import _STATE_DB_CORRUPT_MSG, classify_persistence_error
...
@pytest.fixture
def _clean_registry():
    import mason_state_registry as registry
