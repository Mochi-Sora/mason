...
entirely on the process environment: ``MASON_HOME`` says *where* to write and
...
import mason_state
import mason_state_guard

REPO_ROOT = Path(__file__).resolve().parents[2]

# Probe run in the child: resolve the REAL platform state root (not a
# hardcoded ~/.mason — that root is %LOCALAPPDATA%\mason on Windows) and
...
root = mason_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        mason_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``MASON_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("MASON_HOME", "MASON_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no MASON_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``MASON_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{mason_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert mason_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["mason", "gateway", "start"],
            ["/usr/bin/python", "-m", "mason_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["mason", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not mason_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not mason_state_guard._process_looks_like_pytest(_Denied())


class TestBlindGuardFailsClosed:
    def test_missing_psutil_assumes_test_context(self, monkeypatch):
        """No psutil => the guard is blind => it must refuse, never allow.

        psutil is a pinned core dep; its absence means a broken install, and
        a guard that cannot see the process tree must assume the worst (a
        scrubbed test child) rather than wave a production open through.
        """
        monkeypatch.setattr(mason_state_guard, "psutil", None)
        monkeypatch.setattr(mason_state_guard, "_PYTEST_ANCESTOR", None)
        for var in ("PYTEST_CURRENT_TEST", "PYTEST_VERSION", "MASON_TEST_ISOLATION"):
            monkeypatch.delenv(var, raising=False)
        assert mason_state_guard._ancestry_check_available() is False
        assert mason_state_guard._in_test_context() is True
