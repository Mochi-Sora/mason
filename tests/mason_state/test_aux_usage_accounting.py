...
class TestAnalyticsAuxRows:
    def test_aux_usage_rows_and_merge(self, db):
        from mason_cli.web_server_profiles import _aux_task_summary, _aux_usage_rows, _merge_aux_into_by_model
...
class TestInsightsAuxTotals:
    def test_overview_totals_include_aux_usage(self, db):
        """`mason insights` overview must count aux tokens, not just the
