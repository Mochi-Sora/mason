...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- mason-e2e-evidence:start -->\npending\n<!-- mason-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/NousResearch/mason-agent/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- mason-ci-review-bot -->\n<!-- mason-e2e-evidence:start -->\npending\n<!-- mason-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- mason-ci-review-bot --> no evidence"}]) is None
