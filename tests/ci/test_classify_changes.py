...
CASES = {
    ...
    "product python → nix": (["mason_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every mason_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["mason_state.py"], _lanes(python=True, scan=True)),
...
def test_pull_request_changed_files_skips_non_pr_events(monkeypatch):
    ...
    monkeypatch.setenv("REPO", "NousResearch/mason-agent")
