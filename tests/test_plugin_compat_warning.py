...
def test_compat_resolution_warns_once_per_name_with_the_new_location():
    from mason_cli.plugin_compat import MasonPluginCompatWarning, _seen
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        getattr(mod, name)
        getattr(mod, name)
    ours = [w for w in rec if issubclass(w.category, MasonPluginCompatWarning)]
...
def test_importing_the_facade_itself_does_not_warn():
    ...
    from mason_cli.plugin_compat import MasonPluginCompatWarning
    import sys
    sys.modules.pop("tools.web_tools", None)
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        importlib.import_module("tools.web_tools")
    assert not [w for w in rec if issubclass(w.category, MasonPluginCompatWarning)]
