...
(SQLITE_PROTOCOL — typical on NFS/SMB), Mason must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``MASON_HOME`` silently lose
...
from mason_state import SessionDB, format_session_db_unavailable, get_last_init_error
from mason_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see mason_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("mason_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        mason_state._set_last_init_error("OperationalError: locking protocol")
