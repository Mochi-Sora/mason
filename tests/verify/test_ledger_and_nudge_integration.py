...
- ``mason verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``mason verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        mason_dir = project / ".mason"
        mason_dir.mkdir()
        (mason_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(mason_home):
    project = _workspace(mason_home)
    ...
    assert status["evidence"]["canonical_command"] == "mason verify"


def test_record_verify_run_records_failure(mason_home):
    project = _workspace(mason_home)
...
def test_cli_passing_run_writes_ledger_evidence(mason_home, capsys):
    project = _workspace(mason_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(mason_home, capsys):
    project = _workspace(mason_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(mason_home, capsys):
...
def test_cli_run_uses_mason_session_id_env(mason_home, capsys, monkeypatch):
    monkeypatch.setenv("MASON_SESSION_ID", "sess-42")
    project = _workspace(mason_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> mason verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(mason_home, capsys):
    project = _workspace(mason_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `mason verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_mason_verify_when_recipe_has_start(mason_home):
    project = _workspace(mason_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "mason verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_mason_verify_when_manifest_exists(mason_home):
    # No start script, but a saved .mason/environment.json qualifies.
    project = _workspace(
        mason_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(mason_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(mason_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "mason verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(mason_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(mason_home, capsys):
    project = _workspace(mason_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(mason_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(mason_home, capsys):
    project = _workspace(mason_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
