...
The ``mason update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m mason_cli.main gateway run`` — image name
    ``pythonw``, not ``mason.exe`` — so the ``taskkill /IM mason.exe`` guard
    ...
    # The mason.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM mason.exe' in text
