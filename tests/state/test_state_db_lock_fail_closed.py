...
work on a file several Mason processes share (gateway service, the Desktop
app's `mason serve` backend, CLI sessions, the TUI slash worker):

* `mason_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `mason_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
