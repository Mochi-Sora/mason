...
The gate parses ``mason_state.py`` with ``ast`` and flags any method
...
_STATE_PY = _REPO_ROOT / "mason_state.py"

# SessionDB's own class body lives in mason_state.py; the rest of its
...
_ALL_STATE_SOURCES: list[tuple[Path, str]] = [
    (_STATE_PY, "SessionDB"),
    (_REPO_ROOT / "mason_state_search.py", "SessionSearchMixin"),
    (_REPO_ROOT / "mason_state_schema.py", "SessionSchemaMixin"),
    (_REPO_ROOT / "mason_state_portability.py", "SessionPortabilityMixin"),
]
...
def _scan_all_state_sources() -> list[str]:
    ...
    to ``SessionDB`` — the class body in ``mason_state.py`` plus each mixin
...
class TestNoPureReadersUnderWriterLock:
    ...
    def test_scan_all_state_sources_visits_every_mixin_file(self, tmp_path):
        ...
        reader planted in a MIXIN file (not mason_state.py) must still be
