...
import mason_state
import mason_state_holders
import mason_state_schema
from mason_state import SessionDB
from mason_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from mason_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m mason_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("grep", "mason-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m mason_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/mason-agent/.venv/bin/mason gateway"),
            ("python3", "/opt/mason-agent/tools/check_state.py"),
            ("mason-monitor", "gateway"),
            ("masonctl", "serve"),
            ("python3", "worker.py", "mason_cli.main"),
            ("python3", "-m", "other.module", "mason_cli.main"),
            ("python3", "-c", "mason_cli.main"),
            ("python3", "-Icprint('mason_cli.main')", "mason_cli/main.py"),
        ),
    )
    def test_uninspectable_non_mason_process_is_not_a_holder(self, argv):
        assert not mason_state_holders._looks_like_mason(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/mason", "gateway"),
            ("/usr/local/bin/mason-agent", "serve"),
            ("/usr/local/bin/mason-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "mason_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "mason_cli.main", "gateway"),
            ("/usr/bin/python3", "-mmason_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "mason_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "mason_cli.main"),
            (
                "/opt/mason-agent/.venv/bin/python",
                "/opt/mason-agent/mason_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "mason_cli/main.py", "gateway"),
            ("python3", "/opt/mason-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_mason_process_remains_a_holder(self, argv):
        assert mason_state_holders._looks_like_mason(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(mason_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(mason_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(mason_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(mason_state_holders.os, "stat", _stat)

        holders = mason_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert mason_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Mason process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Mason
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00mason_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(mason_state_holders, "_read_proc_argv", _fake_argv)

        holders = mason_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "mason_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from mason_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(mason_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="mason_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
