...
Several independent Mason processes routinely share one state.db (gateway,
Desktop's ``mason serve`` backend, CLI sessions, the TUI slash worker). Two
...
mason_state_common) and FAILS CLOSED: a process that cannot acquire the
...
from mason_state_common import FTS_STALE_KEY, _FTS_TRIGGERS
...
def fast_timeout(monkeypatch):
    monkeypatch.setattr(
        mason_state_common, "_FTS_REBUILD_LOCK_TIMEOUT_SECONDS", 0.5
    )
...
class TestRebuildFtsAdmission:
    ...
    def test_rebuild_waits_out_a_short_holder(self, db, monkeypatch):
        ...
        monkeypatch.setattr(
            mason_state_common, "_FTS_REBUILD_LOCK_TIMEOUT_SECONDS", 10.0
        )
    ...
    def test_admission_yields_true_for_pathless_db(self):
        ...
        with mason_state_common.fts_rebuild_admission(None) as admitted:
...
admission = mason_state_common.fts_rebuild_admission({db!r})
...
def _orphaned_fork_holder(db_path: Path):
    ...
    script = _ORPHANING_HOLDER_SCRIPT.format(
        repo=str(Path(mason_state_common.__file__).parent), db=str(db_path)
    )
    proc = subprocess.Popen(
        [sys.executable, "-c", script], stdout=subprocess.PIPE, text=True
    )
...
class TestOrphanedHolderStalenessBreak:
    ...
    def test_admission_still_fails_closed_for_live_unrecorded_holder(
        ...
        """A live holder that wrote no record (pre-fix build, non-Mason
    ...
    def test_admission_fails_closed_for_live_recorded_holder(
        ...
        with _rebuild_lock_held_by_other_process(db.db_path) as proc:
            record = {
                "pid": proc.pid,
                "start_ticks": mason_state_common._proc_start_ticks(proc.pid),
                "acquired_at": 0,
            }
    ...
    def test_holder_record_cleared_on_normal_release(self, tmp_path):
        ...
        with mason_state_common.fts_rebuild_admission(tmp_path / "x.db") as ok:
            assert ok is True
    ...
    def test_repair_lock_breaks_orphaned_holder(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mason_state, "_REPAIR_LOCK_TIMEOUT_SECONDS", 0.5)
        db_path = tmp_path / "state.db"
        db_path.touch()
...
lock_cm = mason_state_repair._cross_process_repair_lock(Path({db!r}))
...
print("child", pid, flush=True)
os._exit(1)
""".format(repo=str(Path(mason_state_common.__file__).parent), db=str(db_path))
        import os
        import signal
        ...
        try:
            ...
            with mason_state_repair._cross_process_repair_lock(db_path) as holding:
...
class TestNonContentionErrnoFailsFast:
    def test_non_contention_oserror_does_not_wait_out_timeout(
        ...
        monkeypatch.setattr(
            mason_state_common, "_FTS_REBUILD_LOCK_TIMEOUT_SECONDS", 30.0
        )
        ...
        db_path = tmp_path / "state.db"
        t0 = time.monotonic()
        with mason_state_common.fts_rebuild_admission(db_path) as admitted:
    ...
    def test_retry_deferred_fts_recovery_rebuilds_same_instance(
        ...
        monkeypatch.setattr(mason_state_schema, "_FTS_STALE_RETRY_SECONDS", 0.0)
        db_path = tmp_path / "state.db"
        d = SessionDB(db_path=db_path)
        if not d._fts_enabled:
            d.close()
    ...
    def test_non_contention_errno_skips_holder_warning(
        ...
        monkeypatch.setattr(fcntl, "flock", _flock)
        with caplog.at_level(logging.INFO, logger="mason_state"):
            with mason_state_common.fts_rebuild_admission(
                tmp_path / "state.db"
            ) as admitted:
                assert admitted is False
    ...
    def test_repair_lock_non_contention_errno_fails_fast(
        ...
        monkeypatch.setattr(mason_state, "_REPAIR_LOCK_TIMEOUT_SECONDS", 30.0)
        ...
        monkeypatch.setattr(fcntl, "flock", _flock)
        t0 = time.monotonic()
        with mason_state_repair._cross_process_repair_lock(tmp_path / "state.db") as ok:
            assert ok is False
        assert time.monotonic() - t0 < 2.0
    ...
    def test_is_advisory_lock_contention_table(self, exc, expected):
        assert mason_state_common.is_advisory_lock_contention(exc) is expected
...
class TestDeferredFtsRetryInProcess:
        ...
        with _rebuild_lock_held_by_other_process(db_path):
            ...
            try:
                ...
                assert gw._fts_stale_retry_interval == min(
                    2 * mason_state_schema._FTS_STALE_RETRY_SECONDS,
                    mason_state_schema._FTS_STALE_RETRY_MAX_SECONDS,
                )
    ...
    def test_gateway_housekeeping_tick_drives_the_retry(
        ...
        with _rebuild_lock_held_by_other_process(db_path):
            gw = mason_state_registry.acquire(db_path)
        try:
            assert gw._fts_stale is True
            assert gw in mason_state_registry.live_shared_session_dbs()
            ...
            mason_state_registry.release_or_close(gw)
        assert _meta_value(db_path, FTS_STALE_KEY) is None
