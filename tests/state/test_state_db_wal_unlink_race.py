...
def test_wal_restoration_reuses_exclusive_repair_connection(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mason_state_repair, "_connect_repair_durable", fail_if_reopened)

    mason_state_repair._restore_journal_mode_after_repair(db_path, None, conn=conn)
...
def test_repair_never_reopens_after_the_guard_releases(tmp_path, monkeypatch):
    ...
    _make_db(db)
    monkeypatch.setattr(mason_state_repair, "_db_opens_cleanly", lambda path: "forced-unhealthy")
    # The scratch-space pre-flight wants ~10GB headroom; irrelevant here.
    monkeypatch.setattr(mason_state_repair, "_repair_scratch_space_error", lambda path: None)
    ...
    events: list[str] = []
    real_guard = mason_state_repair._exclusive_repair_db_guard
    ...
    monkeypatch.setattr(mason_state_repair, "_exclusive_repair_db_guard", tracing_guard)
    monkeypatch.setattr(mason_state_repair, "_connect_repair_durable", tracing_connect)
