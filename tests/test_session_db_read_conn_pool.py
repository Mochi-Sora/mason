...
``mason_cli.sqlite_safe_read``'s registry permanently over-counted as well.
...
def _live_count(path) -> int:
    ...
    import mason_cli.sqlite_safe_read as mod
...
def test_read_open_failure_backs_off_but_recovers(db):
    ...
    from mason_state import _READ_OPEN_RETRY_SECONDS
...
def test_peak_live_connections_bounded_under_simultaneous_burst(db):
    ...
    """
    from mason_state import _READ_POOL_MAX
...
def test_peak_is_bounded_across_two_SessionDBs_on_one_path(db):
    ...
    from mason_state import SessionDB, _READ_POOL_MAX
...
# And Mason's SQLite descriptors are only ever a share of the fd table: the
...
def test_peak_is_bounded_across_many_database_files(tmp_path):
    ...
    import mason_state
    from mason_state import SessionDB, _READ_POOL_MAX
    from mason_state_readpool import _READ_POOL_PROCESS_MAX
    ...
    try:
        ...
        for d in dbs:
            d.close()
        assert mason_state_readpool._process_read_permits.acquire(blocking=False), (
            "close() stranded a process permit"
        )
        mason_state_readpool._process_read_permits.release()
...
def test_no_read_connection_is_opened_without_descriptor_headroom(db, monkeypatch):
    ...
    """
    import mason_state_readpool as readpool
...
def test_duplicate_handles_on_one_path_are_reported(db, caplog):
    ...
    from mason_state import SessionDB
    from mason_state_readpool import _HANDLES_PER_PATH_WARN
