...
def progress(tmp_path):
    ...
    status = tmp_path / "mason-update-status"
...
# Stands in for `mason update`, and reports the stage that was on screen
...
n="$(cat "$MASON_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$MASON_TEST_CALLS"
for f in "$TMPDIR"/mason-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$MASON_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$MASON_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "mason-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    mason = install_root / "venv" / "bin" / "mason"
    mason.write_text(FAKE_MASON)
    mason.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "MASON_TEST_CAPTURE": str(capture),
        "MASON_TEST_CALLS": str(calls),
        "MASON_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".mason-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`mason update` is the longest wait in the hand-off and the one the
