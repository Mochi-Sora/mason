"""The conftest WAL gate must agree with mason_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``mason_state``. That is deliberate: importing
``mason_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.mason`` before the per-test ``MASON_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_mason_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and mason_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_mason_state_at_collection():
    """The gate must stay import-free of mason_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.mason, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``mason_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "mason_state" or name.startswith("mason_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported mason_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.mason during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("mason_state", None)
