"""
Tests for timezone support (mason_time module + integration points).
...
def _reset_mason_time_cache():
    """Reset the mason_time module cache."""
    mason_time.reset_cache()


# =========================================================================
# mason_time.now() — core helper
# =========================================================================

class TestMasonTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_mason_time_cache()

    def teardown_method(self):
        _reset_mason_time_cache()
        os.environ.pop("MASON_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["MASON_TIMEZONE"] = "Asia/Kolkata"
        result = mason_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["MASON_TIMEZONE"] = "UTC"
        result = mason_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["MASON_TIMEZONE"] = "America/New_York"
        result = mason_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["MASON_TIMEZONE"] = "Europe/London"
        tz = mason_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching MASON_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("MASON_TIMEZONE", raising=False)

        monkeypatch.setenv("MASON_HOME", str(first_home))
        assert str(mason_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch MASON_HOME in one process.
        monkeypatch.setenv("MASON_HOME", str(second_home))
        assert str(mason_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_mason_home_override(str(homes[key]))
                try:
                    tz = mason_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When MASON_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) MASON_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("MASON_TIMEZONE=" + os.environ.get("MASON_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "MASON_TIMEZONE=NOT_SET" in result["output"], (
            "MASON_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When MASON_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("MASON_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=mason_tz) which shifted
        absolute time when system-local tz != Mason tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Mason tz
        ...
        # Use a Mason timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _mason_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["MASON_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_mason_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Mason timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["MASON_TIMEZONE"] = "US/Eastern"
        _reset_mason_time_cache()
