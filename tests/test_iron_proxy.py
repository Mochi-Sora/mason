...
def mason_home(tmp_path, monkeypatch):
    """Point MASON_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(mason_home):
...
def test_verify_checksums_signature_skips_without_gpg(mason_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(mason_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(mason_home, monkeypatch):
...
def test_ca_key_created_with_0o600(mason_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(mason_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(mason_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(mason_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(mason_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(mason_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("mason-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(mason_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(mason_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(mason_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=mason_home / "ca.crt",
        ca_key=mason_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (mason_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = mason_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("mason-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(mason_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_MASON_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_MASON_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(mason_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(mason_home, monkeypatch):
