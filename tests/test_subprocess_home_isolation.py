...
Mason state stays profile-scoped through MASON_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/mason-agent/issues/25114
See: https://github.com/NousResearch/mason-agent/issues/36144
See: https://github.com/NousResearch/mason-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for mason_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(mason_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("MASON_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        mason_home = real_home / ".mason" / "profiles" / "coder"
        profile_home = mason_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        from mason_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_REAL_HOME", str(real_home))

        from mason_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MASON_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["MASON_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(mason_home / "home")
        assert result["MASON_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_mason_home_unset(self, monkeypatch):
