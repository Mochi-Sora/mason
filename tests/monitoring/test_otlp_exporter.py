...
def test_gateway_health_event_maps_to_span_with_attrs():
    ...
    assert spans[0].name == "mason.gateway_health"
    attrs = dict(spans[0].attributes or {})
    assert attrs["mason.old_state"] == "starting"
    assert attrs["mason.new_state"] == "running"
    assert attrs["mason.active_agents"] == 2
...
def test_trace_resource_includes_stable_hashed_instance():
    ...
    assert attrs["service.name"] == "mason-gateway"
...
def test_streamer_receives_events_and_respects_filter(monkeypatch):
    ...
    spans = mem.get_finished_spans()
    assert [s.name for s in spans] == ["mason.gateway_health"]
