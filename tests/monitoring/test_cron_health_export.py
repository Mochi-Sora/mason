...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("mason.gateway.up"), _M("mason.gateway.active_agents"),
        _M("mason.gateway.busy"), _M("mason.gateway.drainable"),
        _M("mason.gateway.restart_requested"),
        _M("mason.platform.up"), _M("mason.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("mason.cron.scheduler.heartbeat_age_seconds"),
        _M("mason.cron.scheduler.last_success_age_seconds"),
        _M("mason.cron.scheduler.catch_up_occurrences"),
        _M("mason.cron.jobs.enabled"), _M("mason.cron.jobs.running"),
        _M("mason.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Mason Agent-owned Relay transport health" in text
