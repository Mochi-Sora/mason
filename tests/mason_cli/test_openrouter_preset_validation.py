...
def test_combined_openrouter_preset_reference_rejects_unknown_base_model():
    with patch("mason_cli.models.fetch_api_models", return_value=["openai/gpt-5.4"]):
        result = validate_requested_model(
...
def test_combined_preset_preserves_suffix_on_catalog_autocorrect():
    with (
        patch("mason_cli.models.fetch_api_models", return_value=None),
        patch(
...
def test_preset_reference_does_not_bypass_other_provider_validation():
    with patch("mason_cli.models.fetch_api_models", return_value=["gpt-5.4"]):
        result = validate_requested_model(
...
def test_preset_reference_does_not_bypass_custom_endpoint_validation():
    ...
    with patch("mason_cli.models.probe_api_models", return_value=probe) as mock_probe:
        result = validate_requested_model(
            "@preset/email-copywriter",
            "openrouter",
            api_key="key",
