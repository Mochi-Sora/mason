"""Tests for ``mason peer`` — cross-machine bot-to-bot DMs."""
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "mason.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``mason peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``mason peer
