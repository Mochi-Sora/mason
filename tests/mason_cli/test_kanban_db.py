"""Tests for the Kanban DB layer (mason_cli.kanban_db)."""
...
import mason_state
import mason_state_wal
from mason_cli import kanban_db as kb
from mason_cli import kanban_db_connect as kbc
from mason_cli import kanban_db_dispatch as kbd
from mason_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME with an empty kanban DB."""
    home = tmp_path / ".mason"
    home.mkdir()
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import mason_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import mason_cli.kanban_db as _kb
    from mason_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("MASON_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("MASON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Mason' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Mason' DB/metadata/logs on task completion.
    """
...
# spawned with `mason -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's MASON_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's MASON_HOME."""

    def _set_home(self, monkeypatch, tmp_path, mason_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        monkeypatch.delenv("MASON_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.mason/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.mason/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.mason/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # MASON_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile MASON_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # MASON_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is MASON_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["MASON_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["MASON_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["MASON_KANBAN_TASK"] == "t_dispatch_env"
        assert env["MASON_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "MASON_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see mason_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import mason_state_wal as _mason_state_wal
    monkeypatch.setattr(
        _mason_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _mason_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    mason_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("mason_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="mason_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        mason_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "mason_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from mason_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'mason kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from mason_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_mason_argv()
#
# Workers spawned by the dispatcher must use a `mason` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["mason", ...]`
...
def test_resolve_mason_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m mason_cli.main`.

    Pins the correct module name (NOT `mason` — there is no top-level
    `mason` package). Regression for #23198: the original PR shipped
    `python -m mason` which fails with `No module named mason` on every
    invocation.
    """
    import shutil
    import sys
    import mason_cli.kanban_db as kb
    from mason_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("MASON_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_mason_argv()
    assert argv == [sys.executable, "-m", "mason_cli.main"]


def test_resolve_mason_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `mason_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m mason_cli.main --version`
    ...
    assert "Mason Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mason_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from mason_cli.kanban_db_connect import connect
    from mason_cli.sqlite_safe_read import file_length_matches_header
