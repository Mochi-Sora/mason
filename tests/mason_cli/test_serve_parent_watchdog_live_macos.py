...
calls ``os._exit(0)`` ~5ms after ``MASON_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            MASON_PARENT_PID=str(parent.pid),
            MASON_PARENT_START_MARKER=drifted_marker,
            MASON_PARENT_NONCE="nonce-95693",
            MASON_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("MASON_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "MASON_BACKEND_READY"), "backend never announced READY"
