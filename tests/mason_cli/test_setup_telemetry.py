...
def test_disabling_collection_closes_the_send_consent_window(monkeypatch, tmp_path):
    """`mason tools` -> disable shared metrics must withdraw send consent.
    ...
    monkeypatch.setattr(
        "mason_cli.setup.prompt_yes_no", lambda _question, default: False
    )
