"""Tests for mason_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME for receipt writes."""
    home = tmp_path / ".mason"
    home.mkdir()
    monkeypatch.setattr(
        "mason_cli.config.get_mason_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["mason-gateway"],
        ...
        assert gr["restarted_services"] == ["mason-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another mason.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "mason.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            mason_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            mason_main,
        ...
        monkeypatch.setattr(
            mason_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import mason_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "mason_cli.profiles._get_default_mason_home", lambda: home
        )
        monkeypatch.setattr(
            "mason_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
