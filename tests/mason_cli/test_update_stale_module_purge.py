"""Tests for _purge_stale_mason_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `mason update`.

Field failure (2026-08-20, Teknium's Linux box): `mason update` pulled a
checkout where mason_cli/gateway.py newly imports `line_input` from
mason_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from mason_cli.gateway
...
the Mason package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Mason modules from the cache; later
...
def test_purge_evicts_mason_prefixed_modules():
    victims = [
        "mason_cli.cli_output",
        "mason_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_mason_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_mason_modules()
    assert sys.modules.get("mason_cli.update_cmd") is update_cmd
    assert sys.modules.get("mason_cli.main") is cli_main
    assert "mason_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import mason_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_mason_modules()
        post_purge_receipt = importlib.import_module("mason_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["mason_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_mason_modules()
    finally:
        sys.modules.pop("mason_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from mason_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from mason_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_mason_modules()
    from mason_cli.update_inventory import RuntimeRecord as after
