...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp MASON_HOME IS the root here).
...
def test_not_ours_returns_none(mason_home):
    from mason_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(mason_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from mason_cli.local_runtime.bootstrap import assets_dir
    from mason_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(mason_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(mason_home):
...
def test_lookup_chain_consults_managed_runtime(mason_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(mason_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(mason_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(mason_home, monkeypatch, tmp_path):
