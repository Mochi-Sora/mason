"""Tests for mason backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import mason_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew mason_cli.backup's datetime forward instead of sleeping.
    ...
    import mason_cli.backup as _backup

    shim = getattr(_backup.datetime, "_mason_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _mason_test_shim = True
...
def _make_mason_tree(root: Path) -> None:
    """Create a realistic ~/.mason directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "mason_state.db"):
    ...
    # mason-agent repo (should be EXCLUDED)
    (root / "mason-agent").mkdir(exist_ok=True)
    (root / "mason-agent" / "run_agent.py").write_text("# big file\n")
    (root / "mason-agent" / ".git").mkdir()
    (root / "mason-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_mason_agent(self):
        from mason_cli.backup import _should_exclude
        assert _should_exclude(Path("mason-agent/run_agent.py"))
        assert _should_exclude(Path("mason-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from mason_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the MASON_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``mason-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/mason-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from mason_cli.backup import _iter_backup_files

        root = tmp_path / ".mason"
        root.mkdir()
        _make_mason_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # mason-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "mason-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/mason-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("mason-agent") for s in selected)
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import mason_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = mason_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, mason_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside MASON_HOME."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        _make_mason_tree(mason_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(mason_home / "skills" / "outside-link.txt", outside)
        ...
        from mason_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        mason_home.mkdir()
        _make_mason_tree(mason_home)
        with sqlite3.connect(mason_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from mason_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(mason_home=mason_home)
        assert snap_id and (mason_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Mason backup."""
        from mason_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from mason_cli.backup import run_import
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Mason configuration has been restored." in out
        assert "mason gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        (mason_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (mason_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (mason_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            mason_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (mason_home / "gateway.pid").write_text("4242")
        (mason_home / "processes.json").write_text('{"live": true}')
        ...
        assert (mason_home / "gateway.pid").read_text() == "4242"
        assert (mason_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (mason_home / "cron.pid").exists()
        assert not (mason_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (mason_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".mason"
        src_home.mkdir(parents=True)
        _make_mason_tree(src_home)

        monkeypatch.setenv("MASON_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from mason_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".mason"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("MASON_HOME", str(dst_home))
        ...
        # mason-agent should NOT be present
        assert not (dst_home / "mason-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from mason_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from mason_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".mason/", "")
            zf.writestr(".mason/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_mason_home(self, tmp_path, monkeypatch):
        """Backup handles empty mason home (no files to back up)."""
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        # Only excluded dirs, no actual files
        (mason_home / "__pycache__").mkdir()
        (mason_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        (mason_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = mason_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        mason_home.mkdir()
        (mason_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (mason_home / "config.yaml").exists()
        assert (mason_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`mason import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the MASON_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (mason_home / "config.yaml").write_text(original)
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (mason_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(mason_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside MASON_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        mason_home = dst_home / ".mason"
        mason_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo mason import`` onto a user-owned (or Docker/NAS volume-owned)
        MASON_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "mason_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``mason import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from mason_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from mason_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from mason_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def mason_home(self, tmp_path):
        """Create a fake MASON_HOME with critical state files."""
        home = tmp_path / ".mason"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, mason_home):
        from mason_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(mason_home=mason_home)
        db_copy = mason_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, mason_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, mason_home, monkeypatch):
        ...
        restore_quick_snapshot(snap_id, mason_home=mason_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(mason_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, mason_home):
        ...
        live_conn = sqlite3.connect(str(mason_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, mason_home=mason_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, mason_home):
        ...
        (mason_home / "platforms" / "pairing").mkdir(parents=True)
        (mason_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (mason_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (mason_home / "pairing").mkdir()
        (mason_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (mason_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = mason_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (mason update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, mason_home, capsys):
        ...
        """
        import json
        from mason_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", mason_home=mason_home)
        assert first_id is not None
        first_dir = mason_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", mason_home=mason_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = mason_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, mason_home=mason_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, mason_home):
        ...
        board_dir = mason_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(mason_home=mason_home)
        copy = (
            mason_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, mason_home=mason_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, mason_home, monkeypatch):
        ...
        import mason_cli.backup as bk
        from mason_cli.backup import create_quick_snapshot

        board = mason_home / "kanban" / "boards" / "work"
        ...
        copy = mason_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``mason update``
    ...
    def test_backup_contents_match_full_backup(self, mason_home):
        """Pre-update backup should include the same user data that
        ``mason backup`` would, and should exclude the same directories."""
        from mason_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(mason_home=mason_home)
        ...
        # mason-agent repo excluded
        assert not any(n.startswith("mason-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, mason_home):
        """``mason update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", mason_home=mason_home)
        assert snap_id and (mason_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, mason_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(mason_home=mason_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (mason_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, mason_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside MASON_HOME."""
        from mason_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def mason_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".mason"
        root.mkdir()
        _make_mason_tree(root)
        # Point MASON_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("MASON_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve MASON_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(mason_home, value):
        import yaml
        (mason_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(mason_home):
        d = mason_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(mason_home):
        d = mason_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, mason_home, capsys):
        ...
        self._set_mode(mason_home, "off")
        from mason_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(mason_home)
        assert not self._zips(mason_home)



    def test_config_full_mode(self, mason_home, capsys):
        self._set_mode(mason_home, "full")
        from mason_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(mason_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (mason claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``mason claw migrate`` runs before mutating ~/.mason/."""
    ...
    def test_restorable_with_mason_import(self, mason_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Mason
        backup — `mason import` should accept it."""
        from mason_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(mason_home=mason_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, mason_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from mason_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(mason_home=mason_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(mason_home=mason_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`mason update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, mason_home: Path, label="pre-update"):
        from mason_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, mason_home=mason_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from mason_cli.backup import restore_cron_jobs_if_emptied
        mason_home = tmp_path / ".mason"
        jobs_path = mason_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(mason_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, mason_home=mason_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, mason_home: Path) -> Path:
    ...
    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from mason_cli.backup import restore_config_model_settings_if_rewritten

        mason_home = tmp_path / ".mason"
        cfg = self._seed(mason_home)
        snap_id = self._make_snapshot(mason_home)
        assert snap_id
...
# location, NOT under MASON_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, mason_home: Path) -> None:
        mason_home.mkdir(parents=True, exist_ok=True)
        (mason_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (mason_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (mason_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        mason_home = tmp_path / ".mason"
        self._make_min_tree(mason_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under MASON_HOME,
        ...
        # External state did NOT leak into MASON_HOME.
        assert not (mason_home / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: MASON_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsMasonHomeOverride:
    """`mason import` must restore into the home the command runs under.

    Resolving the target through get_default_mason_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """MASON_HOME=<root>/profiles/<name> must restore INTO the profile,
        not into <root> (which would clobber the live root config)."""
        root = tmp_path / "mason-root"
        profile = root / "profiles" / "coder"
        profile.mkdir(parents=True)
        # Live root config that must survive untouched.
        (root / "config.yaml").write_text("model:\n  provider: openai\n")

        monkeypatch.setenv("MASON_HOME", str(profile))
        from mason_constants import get_mason_home

        assert get_mason_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("MASON_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_mason_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "mason_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "mason_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("MASON_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Mason-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`mason import` must not swap the inode of a database Mason holds open.
    ...
    def _prepare(self, tmp_path, monkeypatch, live=(3, 4), backup=(2, 2)):
        home = tmp_path / ".mason"
        home.mkdir()
        monkeypatch.setenv("MASON_HOME", str(home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
