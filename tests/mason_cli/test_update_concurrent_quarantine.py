"""Tests for issue #26670 — concurrent mason.exe detection and improved
quarantine retry / reboot-deferred fallback during `mason update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_mason_instances
# helper (and need the autouse stub in tests/mason_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_mason_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "mason.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "mason.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_mason_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "mason.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_mason_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "mason.exe"
    shim.write_bytes(b"old")

    pairs = main_install_repair._quarantine_running_mason_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("mason.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_mason_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "mason_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "mason_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import mason_cli.update_cmd_windows as update_cmd_windows
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["MasonGateway"],
        "expected_services": ["MasonGateway"],
        "restarted_services": [],
        "service_profiles": {"MasonGateway": "default"},
    }
    assert stopped == ["MasonGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["MasonGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="MasonGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="MasonGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    with pytest.raises(RuntimeError, match="MasonGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["MasonGatewayPicasso", "MasonGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: MasonGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("MasonGateway")

    assert restarted == ["MasonGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="MasonGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["MasonGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m mason_cli.main gateway run"
...
# when every concurrent mason.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, mason.exe shim, mason-gateway.exe,
    gateway/run.py, bare `mason gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "mason_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\mason.exe", "gateway", "run"],
        [r"C:\venv\Scripts\mason-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["mason.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["mason.exe", "gateway"],  # bare `mason gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["mason.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\mason.exe"],  # interactive REPL
        [r"C:\venv\Scripts\mason.exe", "dashboard"],
        ["mason.exe", "gateway", "status"],  # management, not runtime
        ["mason.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["mason.exe", "gateway", "run"],
                200: ["mason.exe"],  # REPL — keep
                300: ["mason.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "mason.exe"), (300, "mason.exe"), (400, "mason.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["mason.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\mason-gateway.exe"],
            }
        ),
    )
    matches = [(111, "mason.exe"), (222, "mason-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_mason_instances",
        return_value=[(1000, "mason.exe"), (2000, "mason-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another mason.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_mason_instances",
        return_value=[(1000, "mason.exe"), (3000, "mason.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "mason.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m mason_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "MasonGateway", expected_service_identity=(11, 11.0)
        )
