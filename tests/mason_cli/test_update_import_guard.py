"""Tests for the post-update *import* guard in ``mason update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not mason_main's. Both modules expose
...
def test_import_guard_does_not_accept_forged_static_marker(monkeypatch, tmp_path):
    ...
    (tmp_path / "consumer.py").write_text(
        "import os, sys\n"
        "sys.stdout.write('__MASON_IMPORT_HEALTH__[]')\n"
        "sys.stdout.flush()\n"
        "os._exit(7)\n"
    )
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("consumer",))
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("mason update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``mason update`` can run under a different Python than the install's.
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "masonx"])
...
@pytest.mark.parametrize("modname", ["tools.todo_tool", "agent.context_compressor",
                                     "mason_constants", "mason_cli.config", "cli"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `masonx`
    (third-party) matched the probe's loose `startswith("mason")`. A user
    ...
    """
    from mason_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "masonx"):
