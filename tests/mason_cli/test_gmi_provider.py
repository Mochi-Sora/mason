...
from mason_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from mason_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # mason model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in MASON_OVERLAYS
        overlay = MASON_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a MasonAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("MasonAgent/")

    def test_gmi_profile_declares_mason_user_agent(self):
        """The GMI plugin sets a MasonAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("MasonAgent/"), (
            f"expected GMI profile User-Agent to start with 'MasonAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "mason_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["mason", "chat", "--provider", "gmi"])

        from mason_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("mason_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("mason_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from mason_cli.model_setup_flows import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_mason_home() / "config.yaml").read_text()) or {}
