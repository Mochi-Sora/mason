...
from mason_cli.dashboard_auth import clear_providers, get_provider
...
from mason_cli.dashboard_auth import registry as _auth_registry
...
def _real_ctx() -> tuple[PluginManager, PluginContext]:
    manager = PluginManager(scope_key=mason_home_key())
...
def test_auth_provider_kept_out_of_manager_teardown_order():
    ...
    # Still attributed to the plugin for `mason plugins list`.
...
def test_targeted_unload_disposes_persistent_auth_provider():
    ...
    # `mason plugins disable basic` drives a targeted unload of that plugin.
