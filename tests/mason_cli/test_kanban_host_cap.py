...
1. The standalone daemon path (``mason kanban daemon --force`` /
   :func:`mason_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``mason kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import mason_cli.config as cfgmod
    import mason_cli.profiles as profmod
