"""Tests for the unified provider catalog (mason_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``mason model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from mason_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
