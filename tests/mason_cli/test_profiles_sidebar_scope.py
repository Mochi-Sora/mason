...
@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_mason_home):
...
def _seed_project(home, name, folder):
    from mason_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_mason_home()``. If the fan-out failed to rebind
