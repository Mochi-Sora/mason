...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_mason_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import mason_cli.oneshot as oneshot
        from mason_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """mason -z bypasses MasonCLI, but recall still needs SessionDB."""
    from mason_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "mason_state", mod("mason_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "mason_cli.config",
        mod("mason_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "mason_cli.models",
        mod("mason_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("mason_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["MASON_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/mason-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["MASON_MODEL"] == "nous/mason-test"
    assert env["MASON_INFERENCE_MODEL"] == "nous/mason-test"
    assert env["MASON_TUI_PROVIDER"] == "nous"
    assert env["MASON_INFERENCE_PROVIDER"] == "nous"
    assert env["MASON_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["MASON_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("mason-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_mason_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "mason-ink"
    ...
    monkeypatch.delenv("MASON_TUI_DIR", raising=False)
