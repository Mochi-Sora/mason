...
def test_fast_serve_launch_dispatches_only_unambiguous_serve(monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["mason", "serve", "--host", "127.0.0.1", "--port", "0"])
    ...
    for argv in (["serve", "--future-flag"], ["serve", "--help"], ["chat"]):
        monkeypatch.setattr(sys, "argv", ["mason", *argv])
        assert main_mod._try_fast_serve_launch() is False
    monkeypatch.setenv("MASON_DISABLE_FAST_SERVE_LAUNCH", "1")
    monkeypatch.setattr(sys, "argv", ["mason", "serve"])
    assert main_mod._try_fast_serve_launch() is False
    monkeypatch.delenv("MASON_DISABLE_FAST_SERVE_LAUNCH")
