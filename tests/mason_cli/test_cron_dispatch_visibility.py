...
`mason cron list` shows a Dispatch line (scheduled vs actual, lateness,
disposition) and `mason cron status` calls out jobs whose last dispatch
...
class TestCronListDispatchLine:
    def test_catch_up_dispatch_rendered(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr(
            "mason_cli.cron._warn_if_gateway_not_running", lambda: None
        )
