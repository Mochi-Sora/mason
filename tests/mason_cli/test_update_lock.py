"""Cross-process update mutual exclusion (``mason_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``mason
...
dashboard-spawned ``mason update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".mason-update-in-progress"


def test_marker_path_follows_process_mason_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* MASON_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("MASON_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".mason-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``mason update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Mason is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``mason-setup`` under ``~/.mason`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("mason_cli.update_lock._pid_alive", lambda pid: True)
