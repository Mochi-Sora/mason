"""Tests for mason_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate MASON_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".mason"
    home.mkdir()
    ...
    import importlib
    from mason_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from mason_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://mason-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/NousResearch/mason-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import mason_cli.model_catalog as model_catalog
        from mason_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("mason_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("mason_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("mason_cli.models.fetch_openrouter_models") as orm, \
             patch("mason_cli.models.fetch_nous_recommended_models") as nous:
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in mason_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``mason model`` CLI — not the live
        ...
        # ``MASON_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` MASON_HOME directly instead.
        import importlib
        from mason_cli import model_catalog
        from mason_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from mason_cli.model_switch_providers import list_picker_providers

            active_home = Path(os.environ["MASON_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("mason_cli.models.check_nous_free_tier", return_value=False), patch(
                "mason_cli.models.union_with_portal_free_recommendations",
