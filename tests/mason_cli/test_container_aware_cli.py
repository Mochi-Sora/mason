...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MASON_HOME with .container-mode file."""
    mason_home = tmp_path / ".mason"
    mason_home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    monkeypatch.delenv("MASON_DEV", raising=False)

    container_mode = mason_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=mason-agent\n"
        "exec_user=mason\n"
        "mason_bin=/data/current-package/bin/mason\n"
    )
    return mason_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("mason_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "mason-agent"
    assert info["exec_user"] == "mason"
    assert info["mason_bin"] == "/data/current-package/bin/mason"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from mason_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "mason"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "mason-agent" in cmd
    assert "/data/current-package/bin/mason" in cmd
    assert "chat" in cmd
