...
profile's MASON_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``mason -p <profile>``
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_mason_action", _fake_spawn)
        resp = client.post(
