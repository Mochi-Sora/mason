"""Regression coverage for stale ``mason serve`` generations after an abort.

Issue #92145: ``mason update`` moves the checkout from generation N to N+1
...
``mason serve`` is not a gateway profile.  It hosts ``tui_gateway.server``,
systemd lists ``mason-gateway.service`` before ``mason-serve.service`` (so
...
from mason_cli import update_abort_recovery as abort_recovery
from mason_cli import update_cmd
from mason_cli import update_restart_recovery as recovery
...
def _identity_module():
    return __import__("mason_cli.process_identity", fromlist=["ledger_entries"])
...
def test_failed_serve_unit_blocks_completion():
    ...
    result["serve_units"] = {"verified": [], "failed": ["mason-serve"]}
    assert (
        update_cmd._abort_recovery_is_complete(
            planned_gateway_profiles={"default"},
            covered_gateway_profiles={"default"},
...
class _Systemctl:
    ...
    def __call__(self, argv, **kwargs):
        if "list-units" in argv:
            ...
            body = "\n".join(
                f"{unit} loaded active running Mason" for unit in self.listed
            )
...
def test_serve_unit_verified_when_main_pid_changes(linux_systemctl):
    fake = _Systemctl(
        listed=["mason-serve.service"],
        active={"mason-serve.service": True},
        main_pids={"mason-serve.service": 4242},
    )
    ...
    assert out == {"verified": ["user/mason-serve"], "failed": []}
    assert fake.restarted == ["mason-serve.service"]
...
def test_unchanged_main_pid_is_not_verified(linux_systemctl):
    ...
    assert out == {"verified": [], "failed": ["user/mason-serve"]}
...
def test_unit_that_does_not_come_back_active_is_failed(linux_systemctl):
    ...
    def dies(argv, **kwargs):
        if "restart" in argv:
            fake.restarted.append(argv[-1])
            fake.active["mason-serve.service"] = False
            return _Completed(0)
        return original_call(argv, **kwargs)
...
def test_inactive_serve_unit_is_left_alone(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["mason-serve.service"],
        active={"mason-serve.service": False},
        main_pids={"mason-serve.service": 0},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
...
def test_units_already_restarted_by_the_phase_are_skipped(linux_systemctl):
    fake = _Systemctl(
        listed=["mason-serve.service", "mason-serve-work.service"],
        active={"mason-serve.service": True, "mason-serve-work.service": True},
        main_pids={"mason-serve.service": 1, "mason-serve-work.service": 2},
    )
    out = recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "mason-serve"}],
        run=fake,
        sleep=lambda _: None,
    )
    assert fake.restarted == ["mason-serve-work.service"]
    assert out == {"verified": ["user/mason-serve-work"], "failed": []}


def test_unrelated_mason_server_service_is_never_touched(linux_systemctl):
    """``mason-serve*`` is a systemd glob, not a name gate (review on #83595)."""
    fake = _Systemctl(
        listed=["mason-server.service", "mason-serve.service"],
        active={"mason-server.service": True, "mason-serve.service": True},
        main_pids={"mason-server.service": 7, "mason-serve.service": 8},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == ["mason-serve.service"]
    assert out["verified"] == ["user/mason-serve"]
...
def test_gateway_units_are_not_restarted_by_the_serve_pass(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["mason-gateway.service"],
        active={"mason-gateway.service": True},
        main_pids={"mason-gateway.service": 5},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == []
    assert out == {"verified": [], "failed": []}
...
def test_same_unit_name_in_both_scopes_is_proven_in_both(linux_systemctl):
    ...
    def dual_scope(argv, **kwargs):
        ...
        if "list-units" in argv:
            return _Completed(0, stdout="mason-serve.service loaded active running x")
        if "is-active" in argv:
    ...
    assert out == {
        "verified": ["user/mason-serve"],
        "failed": ["system/mason-serve"],
    }
            ...
            json.dumps(
                {
                    ...
                    "serve_units": {
                        ...
                        "skip": [
                            "mason-serve",
                            "mason-serve-work.service",
                            {"scope": "system", "unit": "mason-serve"},
                            {"scope": "user", "unit": "mason-serve-work.service"},
                            "mason-server",
                            "../../etc/systemd/evil",
                            "mason-serve; rm -rf /",
                            "mason-gateway",
...
def test_serve_only_fleet_still_spawns_the_recovery_child(monkeypatch):
            ...
            stdout=json.dumps(
                {
                    ...
                    "relaunch_attempted": [],
                    "failed": [],
                    "serve_units": {"verified": ["mason-serve"], "failed": []},
                }
            ),
        )
    ...
    assert result["serve_units"] == {"verified": ["mason-serve"], "failed": []}
...
def test_verified_serve_units_do_not_enter_gateway_restart_vocabulary(monkeypatch):
    ...
    assert "mason-serve" not in result["verified"]
...
def test_already_restarted_units_are_forwarded_as_skips(monkeypatch):
    ...
    update_cmd._recover_gateway_restart_after_abort(
        _plan(_runtime("gateway", "default", "systemd", 111)),
        gateway_mode=False,
        skip_units={"user/mason-serve"},
    )
    # The scope travels with the unit: settling `user/mason-serve` must not
    # suppress recovery of a stale system-scope unit of the same name.
    assert captured["payload"]["serve_units"]["skip"] == [
        {"scope": "user", "unit": "mason-serve"}
    ]
...
def test_serve_coverage_reaches_the_persisted_receipt(tmp_path, monkeypatch):
    from mason_cli import update_receipt
    ...
    update_receipt.record_gateway_restart(
        restarted_services=[],
        incomplete=True,
        phase_error="cannot import name 'line_input' from 'mason_cli.cli_output'",
        fresh_recovery={
            "requested": ["default"],
            "verified": ["default"],
            "relaunch_attempted": [],
            ...
            "serve_units": {
                "verified": ["user/mason-serve"],
                "failed": ["system/mason-serve-work"],
            },
            "stale_runtimes": [
                {
                    "pid": 4242,
...
def test_stale_serve_warning_names_the_process_and_the_fix(capsys):
    ...
    assert "4242" in out
    assert "serve" in out
    assert "systemctl --user restart mason-serve.service" in out
...
def test_recovery_module_reports_serve_units_in_a_real_process():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mason_cli.update_restart_recovery", "--stdin"],
        input=json.dumps(
...
def _stub_dashboard_helpers(monkeypatch, **helpers):
    """Stub the ``mason_cli.main_dashboard`` helpers the dashboard-cleanup path reads at call time."""
...
def _dashboard_main_stub(scan_calls, *, restart_result=True):
    return dict(
        _DASHBOARD_SYSTEMD_UNIT="mason-dashboard.service",
...
def test_managed_dashboard_restart_still_scans_for_serve_backends(monkeypatch):
    ...
    The reporter's host runs ``mason-dashboard.service`` AND
    ``mason-serve.service``.  Returning as soon as the dashboard unit was
...
def test_restarted_dashboard_unit_is_not_killed_by_the_continued_scan(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_systemd_service_for_pid=lambda pid: "mason-dashboard.service",
...
def test_serve_backend_survives_selection_when_the_dashboard_unit_restarts(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_pid_cgroup_path=lambda pid: "/user.slice/mason-serve.service",
        _get_systemd_service_for_pid=lambda pid: "mason-serve.service",
        _dashboard_cmdline_for_pid=lambda pid: None,
    ...
    assert restarted == ["mason-serve.service"]
...
def _dual_scope_systemctl(*, user_pid=10, system_pid=20, settle=("user", "system")):
    """Both managers own a ``mason-serve.service``; each is its own process."""
...
def test_settled_user_scope_does_not_suppress_the_stale_system_scope(
    ...
    ``user/mason-serve`` was restarted before the phase aborted;
    ``system/mason-serve`` is a different process still on the pre-update
    generation. A bare ``mason-serve`` skip token suppressed BOTH, leaving
    ...
    assert out == {"verified": ["system/mason-serve"], "failed": []}
...
def test_one_authorized_scope_never_mutates_the_same_name_in_the_other(
    ...
    recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "mason-serve.service"}],
        run=run,
        sleep=lambda _: None,
    )
    user_verbs = [
...
def test_legacy_unqualified_skip_is_honoured_in_both_scopes(linux_systemctl):
    ...
    run, calls = _dual_scope_systemctl()
    out = recovery.restart_serve_units(
        skip_units=["mason-serve"], run=run, sleep=lambda _: None
    )
...
def test_qualified_skip_payload_shape():
    ...
    assert update_cmd._qualified_serve_skips(
        {"user/mason-serve", "system/mason-serve-work"}
    ) == [
        {"scope": "system", "unit": "mason-serve-work"},
        {"scope": "user", "unit": "mason-serve"},
    ]
...
def test_unscoped_bookkeeping_entries_are_forwarded_without_a_scope():
    ...
    assert update_cmd._qualified_serve_skips({"mason-serve"}) == [
        {"unit": "mason-serve"}
    ]
