...
``mason auth add b-ai`` stores keys under ``credential_pool.b-ai``. Runtime
...
def _write_keyed_provider_home(tmp_path, monkeypatch, *, pool_id="b-ai", extra_config=None):
    ...
    (mason_home / "config.yaml").write_text(yaml.safe_dump(config), encoding="utf-8")
...
def test_prune_keeps_active_legacy_pool_for_keyed_provider(tmp_path, monkeypatch):
    ...
    after = json.loads(
        (tmp_path / ".mason" / "auth.json").read_text(encoding="utf-8")
    )
