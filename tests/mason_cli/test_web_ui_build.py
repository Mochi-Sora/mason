...
(mirroring the desktop build), recorded in a stamp file under $MASON_HOME,
NOT mtime comparison — so ``git pull`` / ``mason update`` that rewrite
source mtimes without changing content no longer fool it.

Critical invariant: the dashboard Vite build outputs to mason_cli/web_dist/
(vite.config.ts: outDir: "../../mason_cli/web_dist"), NOT web/dist/.
...
from mason_cli.main_web_build import _build_web_ui, _run_npm_install_deterministic
from mason_cli.main_web_build import _web_ui_build_needed, _compute_web_ui_content_hash, _missing_web_build_tool, _web_ui_stamp_path, _write_web_ui_build_stamp
from mason_cli.update_cmd import _web_build_toolchain_ready, _web_toolchain_roots


@pytest.fixture(autouse=True)
def _isolated_mason_home(tmp_path, monkeypatch):
    """Keep web-build-stamp writes inside the test's tmp dir, never the real home."""
    monkeypatch.setenv("MASON_HOME", str(tmp_path / "_mason_home"))
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "mason_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``mason update`` do) must NOT report stale."""
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_web_install_omits_workspace_and_scrubs_esbuild_override(
        ...
        with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("mason_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("mason_cli.main_web_build._run_with_idle_timeout", return_value=build_cp) as mock_build:
    ...
    def test_workspace_root_install_names_update_closure(self, tmp_path, monkeypatch):
        ...
        closure as `mason update` (ui-tui + web + --include-workspace-root).
        ...
        with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("mason_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("mason_cli.main_web_build._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("mason_cli.main.subprocess.run", return_value=install_cp), \
             patch("mason_cli.main_web_build._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("mason_cli.main_web_build._time.sleep") as mock_sleep, \
             patch("mason_cli.main.subprocess.run", return_value=install_ok), \
             patch("mason_cli.main_web_build._run_with_idle_timeout",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("mason_cli.main_web_build._time.sleep"), \
             patch("mason_cli.main.subprocess.run", return_value=install_ok), \
             patch("mason_cli.main_web_build._run_with_idle_timeout",
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_without_dist_waits_then_skips_fresh_build(self, tmp_path):
        ...
        import fcntl
        import threading
        from mason_cli.main_web_build import _build_web_ui as build
        ...
        try:
            with patch("mason_cli.main.shutil.which", return_value="/usr/bin/npm"), \
                 patch("mason_cli.main.subprocess.run") as mock_run:
...
class TestBuildRecoversFromMissingToolchain:
    def test_reinstalls_and_retries_when_the_build_cannot_resolve_tsc(self, tmp_path):
        ...
        with patch("mason_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             ...
             patch("mason_cli.main_web_build._run_with_idle_timeout", side_effect=[build_fail, build_ok]) as mock_build, \
             patch("mason_cli.main_web_build._web_ui_build_needed", return_value=True), \
             patch("mason_cli.main_web_build._write_web_ui_build_stamp"), \
             patch("mason_cli.main_web_build._time.sleep"):
            result = _build_web_ui(web_dir)
    ...
    def test_healthy_tree_builds_without_an_extra_install(self, tmp_path):
        ...
        with patch("mason_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("mason_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("mason_cli.main_web_build._run_with_idle_timeout", return_value=build_ok) as mock_build, \
             patch("mason_cli.main_web_build._web_ui_build_needed", return_value=True), \
             patch("mason_cli.main_web_build._write_web_ui_build_stamp"):
            result = _build_web_ui(web_dir)
