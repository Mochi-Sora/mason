"""Tests for the top-level `./mason` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./mason` should use `mason_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "mason"
    called = []

    fake_main_module = types.ModuleType("mason_cli.main")

    def fake_main():
        called.append("mason_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "mason_cli.main", fake_main_module)
