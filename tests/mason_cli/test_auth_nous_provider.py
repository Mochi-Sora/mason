...
from mason_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from mason_cli.auth import _resolve_verify
...
def test_resolve_nous_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    payload = json.loads((mason_home / "auth.json").read_text())
...
def test_resolve_nous_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    import mason_cli.auth as auth_mod
    import mason_cli.auth_nous as auth_nous
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    monkeypatch.setenv("MASON_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="mason_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from mason_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `mason model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return mason_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import mason_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from mason_cli.auth import PROVIDER_REGISTRY, _login_nous

        mason_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `mason auth add nous`
    ...
    """
    from mason_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    """
    from mason_cli.auth import persist_nous_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Mason must surface an
    ...
    re-authenticate" string led users to report this as a Mason persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from mason_cli.auth import _refresh_access_token
    ...
    assert "mason auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect MASON_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("MASON_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without MASON_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.mason/shared/`` across CI runs.
    """
    from mason_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("MASON_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `mason auth add nous
    ...
    mason_home = tmp_path / "mason"
    mason_home.mkdir(parents=True, exist_ok=True)
    (mason_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    """
    from mason_cli import auth as auth_mod
    import mason_cli.auth_nous as auth_nous
    ...
    assert result["client_id"] == "mason-cli"
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from mason_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
