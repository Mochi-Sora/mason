"""Dashboard Mason Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_mason_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from mason_cli.console_engine import ConsoleResult, MasonConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(MasonConsoleEngine, "execute", slow_execute)
