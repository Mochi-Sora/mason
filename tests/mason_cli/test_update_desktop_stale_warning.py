...
#88251: ``mason update`` treated a failed desktop pack as non-fatal, printed
...
import mason_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_constants.with_mason_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "mason_constants.display_mason_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_mason_home", lambda: tmp_path)
