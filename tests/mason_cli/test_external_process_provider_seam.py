...
way a standalone package does — before importing ``mason_cli`` — and walks the
...
def test_an_out_of_tree_external_process_provider_resolves_end_to_end(fake_cli, monkeypatch):
    from mason_cli.auth import PROVIDER_REGISTRY, resolve_external_process_provider_credentials, resolve_provider
...
def test_copilot_acp_launch_details_are_unchanged(fake_cli, monkeypatch):
    from mason_cli.auth import resolve_external_process_provider_credentials
    from mason_cli.runtime_provider import resolve_runtime_provider
