"""Tests for mason_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $MASON_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Mason-managed node dir.
"""
...
def _make_mason_node(mason_home: Path) -> Path:
    """Create a fake $MASON_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = mason_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    mason_home = fake_home / ".mason"
    _make_mason_node(mason_home)
    ...
    removed = uninstall.remove_node_symlinks(mason_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    mason_home = fake_home / ".mason"
    node_bin = _make_mason_node(mason_home)
