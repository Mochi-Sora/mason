...
`mason update` migrated only the active profile's config.yaml; sibling
...
context-local MASON_HOME override.
...
import mason_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        mason_constants, "get_process_mason_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_mason_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_mason_home_override() == before
