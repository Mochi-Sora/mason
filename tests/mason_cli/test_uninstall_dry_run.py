...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_mason_home", lambda: mason_home)
    monkeypatch.setattr(uninstall, "_is_default_mason_home", lambda home: False)
    ...
    assert str(mason_home) in output
    assert project_root.exists()
    assert mason_home.exists()
