"""Tests for mason_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("mason_cli.profiles._get_default_mason_home", lambda: default_home)
    monkeypatch.setattr("mason_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("mason_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    monkeypatch.setattr("gateway.status._pid_exists", lambda pid: pid in (100, 200))
    monkeypatch.setattr("mason_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("mason_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("mason_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "mason_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("mason_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("mason_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from mason_cli.update_inventory import describe_restart_mechanism

        assert "mason -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("mason_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "mason_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/mason-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from mason_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "mason_cli.config.detect_install_method",
            "mason_cli.build_info.get_code_identity",
            "mason_cli.profiles._get_default_mason_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("mason_cli.config.get_mason_home", lambda: home, raising=False)
        ur._current = None
