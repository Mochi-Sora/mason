...
import mason_cli.web_models as _web_models
import mason_cli.web_routers.cron as _rt_cron
import mason_cli.web_server_cron as _web_server_cron
...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_mason_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_mason_home()
    ...
    outer_token = set_mason_home_override(default_home)
    try:
        ...
        assert scheduler._get_mason_home() == default_home
    finally:
...
def test_create_registers_scheduler_inside_target_profile(
    ...
    class RecordingProvider(CronScheduler):
        ...
        def register_job(self, job):
            captured["job"] = job
            captured["runtime_home"] = get_mason_home()
            captured["jobs_file"] = cron_jobs._current_cron_store().jobs_file
...
async def test_dashboard_cron_noop_inference_fields_keep_existing_snapshots(
    ...
    from mason_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.mason`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from mason_cli import web_server

    monkeypatch.setenv(
        "MASON_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """MASON_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from mason_cli import web_server

    monkeypatch.setenv("MASON_HOME", str(isolated_profiles["default"]))
