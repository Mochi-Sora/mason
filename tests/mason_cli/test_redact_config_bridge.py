...
`MASON_REDACT_SECRETS` at module-import time. `mason_cli/main.py` at
...
in .env), the toggle was silently ignored in both `mason chat` and
`mason gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `MASON_REDACT_SECRETS`
env var in `mason_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports mason_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("MASON_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import mason_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('MASON_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["MASON_HOME"] = str(mason_home)
    env.pop("MASON_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env MASON_REDACT_SECRETS takes precedence over config.yaml."""
    mason_home = tmp_path / ".mason"
    mason_home.mkdir()
    (mason_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (mason_home / ".env").write_text("MASON_REDACT_SECRETS=true\n")
