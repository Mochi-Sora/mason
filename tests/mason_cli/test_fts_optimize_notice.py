...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            mason_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(mason_constants, "get_mason_home", lambda: tmp_path)
    monkeypatch.setattr(mason_state, "SessionDB", FakeSessionDB)
    ...
    assert "mason sessions optimize-storage" in capsys.readouterr().out
    conn.close()
