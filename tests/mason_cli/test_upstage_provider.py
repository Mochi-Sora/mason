...
Regression guard for the bug where `mason model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`mason_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in MASON_OVERLAYS
        overlay = MASON_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `mason_cli/main.py` (cmd_model): read
