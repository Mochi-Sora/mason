...
1. The venv-repair import probes (early recovery + `mason update`) must
...
2. ``mason doctor`` must fail the certificate check in that state, and
   ``mason doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `mason update`'s venv repair must apply
...
# =========================================================================
# 2. mason doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "mason doctor --fix" in message
