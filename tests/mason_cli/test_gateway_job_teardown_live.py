...
2. ``TestWatcherRespawnLive`` — drives the REAL
   ``mason_cli.gateway._spawn_gateway_restart_watcher`` end to end with a
   real stub gateway process, against a temp MASON_HOME:
   ...
   - the respawn env must carry ``_MASON_GATEWAY_BREAKAWAY=1`` (the stamp
...
class TestWatcherRespawnLive:
    ...
    def _run_watcher_cycle(self, tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
        ...
        stub = (
            ...
            "    os.environ.get('_MASON_GATEWAY_BREAKAWAY', 'MISSING'),\n"
            "    encoding='utf-8')\n"
