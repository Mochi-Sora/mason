...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Mason"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.mason.cli_plugin",
    ...
    monkeypatch.setattr(
        "mason_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
        importlib.metadata,
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Mason",
            "entrypoint",
            "adapters.mason.cli_plugin",
            "wiki",
        )
    ]
...
def test_declared_capabilities_for_entrypoint_uses_distribution_metadata(
    ...
    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "mason_cli.plugins.get_bundled_plugins_dir", lambda: bundled_dir
    )
    monkeypatch.setattr(
        importlib.metadata,
        "entry_points",
