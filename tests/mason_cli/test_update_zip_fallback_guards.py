...
install``. A locked ``mason.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(mason_main, "_is_windows", lambda: True)
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M mason_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(mason_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.mason-update-staging/")
    assert is_artifact("?? cli.py.mason-update-staging")
    assert is_artifact("?? tools.mason-update-old/")
    ...
    assert not is_artifact(" M mason_cli/update_cmd.py")
    assert not is_artifact("?? notes.mason-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.mason-update-staging/\n?? cli.py.mason-update-old\n"),
    )
    assert (
