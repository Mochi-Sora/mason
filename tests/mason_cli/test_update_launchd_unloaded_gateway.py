"""Regression for #74973 — `mason update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``mason gateway restart``.
...
def launchd(monkeypatch):
    """Stub mason_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.mason.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.mason.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "mason gateway restart" in out
...
ai.mason.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.mason.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.mason/bin/mason
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.mason.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from mason_cli.gateway import _parse_launchd_pid_from_print_output
