...
from mason_cli.console_engine import MasonConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/mason-console-test.zip",
    "import /tmp/mason-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_mason_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(mason_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = MasonConsoleEngine().execute(
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_mason_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_mason_home):
    ...
    assert code == 0
    assert "Mason Console" in stdout.getvalue()
    assert "mason>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from mason_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_mason_home):
    result = MasonConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_mason_home, monkeypatch
):
    ...
    result = MasonConsoleEngine().execute("checkpoints prune", confirmed=True)
