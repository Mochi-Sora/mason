"""Regression tests for the parked-branch guard in ``mason update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``mason
...
def _no_config(monkeypatch):
    ...
    import mason_cli.config as mason_config

    monkeypatch.setattr(mason_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && mason update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(mason_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(mason_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(mason_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(mason_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        mason_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(mason_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(mason_main, "_capture_active_tool_dependencies", lambda: [])
