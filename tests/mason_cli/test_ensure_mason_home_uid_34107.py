"""Regression tests for #34107 — Docker UID/GID handling in ensure_mason_home.

When Mason runs in Docker with ``MASON_UID=1000`` / ``MASON_GID=911``,
the entrypoint chowns the top-level ``MASON_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_mason_home()`` — especially
...
The fix is a ``_chown_to_mason_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_mason_uid_gid
# ---------------------------------------------------------------------------


class TestResolveMasonUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("MASON_UID", "1000")
        monkeypatch.setenv("MASON_GID", "911")
        from mason_cli.config import _resolve_mason_uid_gid
...
class TestChownToMasonUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_mason_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("MASON_UID", raising=False)
        monkeypatch.delenv("MASON_GID", raising=False)
        from mason_cli import config as cfg
