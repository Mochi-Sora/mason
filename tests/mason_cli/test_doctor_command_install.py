"""Tests for the Command Installation check in mason doctor."""
...
import mason_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal MASON_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".mason"
    ...
    mason_bin = venv_bin_dir / "mason"
    mason_bin.write_text("#!/usr/bin/env python\n# entry point\n")
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, mason_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "mason"
        wrong_target = tmp_path / "wrong_mason"
        ...
        assert cmd_link.resolve() == mason_bin.resolve()
