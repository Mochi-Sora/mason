...
class TestUnescapedDottedModelKeys:
    def test_set_hits_existing_literal_key_no_phantom(self, _isolated_mason_home):
        _write_config(_isolated_mason_home, PROVIDER_CONFIG)
        set_config_value("providers.myprov.models.grok-4.6.supports_vision", "true")
        saved = _read_config(_isolated_mason_home)
    ...
    def test_get_reads_real_dotted_key(self, _isolated_mason_home, capsys):
        _write_config(_isolated_mason_home, PROVIDER_CONFIG)
    ...
    def test_unset_removes_real_dotted_key_field(self, _isolated_mason_home):
        cfg = {
            "providers": {
                "myprov": {
                    "models": {
        ...
        _write_config(_isolated_mason_home, cfg)
        args = argparse.Namespace(
    ...
    def test_escaped_form_creates_new_dotted_key(self, _isolated_mason_home):
        _write_config(_isolated_mason_home, PROVIDER_CONFIG)
        set_config_value(
...
class TestMatrixRoomIds:
    def test_set_existing_room_id_key(self, _isolated_mason_home):
        _write_config(
    ...
    def test_escaped_creates_new_room_id_key(self, _isolated_mason_home):
        _write_config(_isolated_mason_home, {"matrix": {"channel_prompts": {}}})
        set_config_value(
...
class TestDottedLeafKeys:
    ...
    def test_set_leaf_parent_is_dotted(self, _isolated_mason_home):
        _write_config(_isolated_mason_home, self.CFG)
    ...
    def test_get_dotted_leaf_mapping(self, _isolated_mason_home, capsys):
        _write_config(_isolated_mason_home, self.CFG)
        args = argparse.Namespace(
    ...
    def test_unset_dotted_leaf(self, _isolated_mason_home):
        _write_config(_isolated_mason_home, self.CFG)
        args = argparse.Namespace(
...
class TestBackwardCompatibility:
    def test_plain_nested_set(self, _isolated_mason_home):
    ...
    def test_deep_creation_without_siblings_unchanged(self, _isolated_mason_home):
