...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``mason_cli._subprocess_compat.pid_is_mason`` fails closed on foreign
  processes and identity mismatches.
- ``mason_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsMasonLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Mason process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from mason_cli._subprocess_compat import pid_is_mason

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'mason' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "mason" in sys.executable.lower():
                pytest.skip("interpreter path names mason; probe would match")
            assert pid_is_mason(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_mason_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_mason(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
    ...
    def test_nonexistent_pid_is_refused(self):
        ...
        assert pid_is_mason(2**24) is False
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from mason_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
