...
def _client():
    ...
    # Keep the state DB under the isolated MASON_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_mason_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (mason_home / "config.yaml").read_text()
        env_text = (mason_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from mason_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.mason/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `mason auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `mason auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_mason_home):
        ...
        (get_mason_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_mason_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_mason_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "mason_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("mason-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # mason-index source advertises availability + featured search.
            idx = _Src("mason-index")
        ...
        assert {"official", "github", "mason-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/mason/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/mason/update`` runs ``mason update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/mason/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/mason/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `mason tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_mason_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_mason_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _MASON_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_mason_action stores
    ...
    _web_server_gateway._spawn_mason_action(["gateway", "restart"], "gateway-restart")

    assert "_MASON_GATEWAY" not in captured["env"]
    assert captured["env"]["MASON_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_mason_home
):
    ...
    startup under MASON_DESKTOP=1 so the stale orphan is cleared first.
    """
    import mason_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from mason_cli.gateway, so patch it
    # on that module.
    import mason_cli.gateway as g
