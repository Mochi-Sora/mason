...
def test_copilot_picker_uses_live_catalog_when_available():
    ...
    with patch("agent.models_dev.fetch_models_dev", return_value={}), \
         patch("mason_cli.models._resolve_copilot_catalog_api_key", return_value="gh-token"), \
         patch("mason_cli.models._fetch_github_models", return_value=live_models):
...
def _no_other_copilot_creds(monkeypatch):
    ...
    # the provider configured with no executable at all (mason_cli/auth.py), so
    # a host that sets it would decide the outcome instead of the test.
    for var in ("GH_TOKEN", "GITHUB_TOKEN", "MASON_COPILOT_ACP_COMMAND",
...
def test_copilot_acp_listed_when_executable_resolves(tmp_path, monkeypatch, _no_other_copilot_creds):
    ...
    with patch("agent.models_dev.fetch_models_dev", return_value={}), \
         patch("mason_cli.models._resolve_copilot_catalog_api_key", return_value=None), \
         patch("mason_cli.models._fetch_github_models", return_value=[]):
        providers = list_authenticated_providers(current_provider="openrouter", max_models=50)
