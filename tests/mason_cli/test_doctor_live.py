"""Tests for ``mason doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from mason_cli import doctor_live
from mason_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `mason doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("mason_cli.doctor.MASON_HOME", tmp_path / "home")
        monkeypatch.setattr("mason_cli.doctor.PROJECT_ROOT", tmp_path / "root")
