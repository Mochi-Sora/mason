...
The bug class: on Windows, when `mason.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("mason", "mason-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # mason.exe cannot be renamed; mason-gateway.exe can
    real_rename = Path.rename

    def deny_mason(self, target):
        if self.name == "mason.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_mason)
    monkeypatch.setattr(main_install_repair, "_mason_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "mason.exe" in exc_info.value.failed_shims
    # The successful rename (mason-gateway.exe) was rolled back.
    assert (scripts / "mason-gateway.exe").exists()
    assert not list(scripts.glob("mason-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("mason",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("mason",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["mason.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "mason.exe" in out
