...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $MASON_HOME.

    Models a host git install whose $MASON_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'mason update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("mason_cli.config.get_managed_system", return_value=None), \
         patch("mason_cli.config.get_mason_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``mason update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("mason_cli.config.get_managed_system", return_value=None), \
         patch("mason_cli.config.get_mason_home", return_value=tmp_path), \
         patch("mason_constants.is_container", return_value=True):
        from mason_cli.config import detect_install_method
