...
Covers:
  ...
  - Namespace spoofing (mason:, foreign, already-colon'd) is rejected
  ...
  - `mason plugins show` output includes emits/listens
"""
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "mason:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including mason: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("mason:core_event", lambda **p: got.append("mason"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "mason:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
...
def test_pending_budget_drops_new_event_without_blocking(monkeypatch, caplog):
    from mason_cli import plugins_dispatch
