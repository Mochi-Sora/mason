...
same way the `mason model` CLI picker does.
"""

from threading import Event
from time import monotonic

import mason_cli.inventory as inv
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (mason_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        ...
        assert observed["a"] == (
            mason_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            mason_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    token = set_mason_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
