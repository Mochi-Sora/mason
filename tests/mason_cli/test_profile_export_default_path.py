...
import pytest
from mason_cli import profile_cmd
...
def profiles():
    ...
    Sibling test files reload ``mason_cli.profiles`` / ``mason_cli.main``;
    a top-level ``from mason_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("mason_cli.profiles")
...
def test_custom_mason_home_inside_a_checkout_uses_a_sibling_store(
    tmp_path, monkeypatch, profiles
):
    ...
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: checkout)
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """MASON_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    mason_home = checkout / "data_home"
    mason_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: mason_home)
...
def test_cli_export_rejects_bad_profile_name_without_traceback(
    ...
    main_mod = importlib.import_module("mason_cli.main")
    default_home = tmp_path / ".mason"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: default_home)
...
def test_cli_export_default_does_not_write_into_the_current_checkout(
    ...
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: default_home)
    monkeypatch.setattr(
        "mason_constants.get_default_mason_root", lambda: default_home
    )
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("mason_cli.cli_commands_mixin")
    default_home = tmp_path / ".mason"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from mason_cli.web_models import ProfileExport

    router_mod = importlib.import_module("mason_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a MASON_HOME inside checkout B."""
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: checkout_b)
