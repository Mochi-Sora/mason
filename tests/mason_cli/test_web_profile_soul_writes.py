...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MASON_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test MASON_HOME."""
    monkeypatch.setenv("MASON_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # MASON_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/mason_cli/test_web_profiles_off_loop.py``.
    """
