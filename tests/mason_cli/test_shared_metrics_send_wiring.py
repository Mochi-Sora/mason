...
from mason_cli.observability import relay_shared_metrics as mod
...
def capture_sender(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.observability.shared_metrics_sender.SharedMetricsSender",
        FakeSender,
    )
    return record
...
def _set_config(monkeypatch, config):
    monkeypatch.setattr(
        "mason_cli.config.read_raw_config_readonly", lambda: config, raising=False
    )
...
class TestConsentWindows:
    ...
    def test_reconcile_runs_even_when_collection_is_disabled(
        ...
        # MASON_HOME: the boot reconciler probes the default path (without
        ...
        home = tmp_path / "home"
        monkeypatch.setattr(
            "mason_constants.get_mason_home", lambda: home
        )
        ...
        monkeypatch.setattr(
            "mason_cli.observability.shared_metrics.SharedMetricsStore",
            lambda *a, **k: store,
        )
...
class TestFailureIsolation:
    ...
    def test_an_unreadable_config_does_not_break_export(self, runtime, monkeypatch, capture_sender):
        ...
        monkeypatch.setattr(
            "mason_cli.config.read_raw_config_readonly", explode, raising=False
        )
        runtime._export()
        assert runtime.subscriber.store.exported == 1
        assert capture_sender["passes"] == []
