"""Tests for the per-profile Projects store (mason_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Mason Agent", folders=["/tmp/mason"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "mason-agent"
    assert proj.name == "Mason Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/mason"
    assert [f.path for f in proj.folders] == ["/tmp/mason"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "mason-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' MASON_HOME.
