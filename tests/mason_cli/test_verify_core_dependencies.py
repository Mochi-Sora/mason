...
base deps) during ``mason update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point mason_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("mason_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("mason_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("mason_cli.main_install_repair._run_install_with_heartbeat"):

            from mason_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("mason_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("mason_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from mason_cli.main_install_repair import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `mason update`.
        ...
        with patch("mason_cli.main_install_repair._is_windows", return_value=True):
            from mason_cli.main_install_repair import _resolve_install_target_python
