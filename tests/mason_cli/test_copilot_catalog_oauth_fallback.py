...
``mason auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.mason/.env`` — the picker was silently dropping back to
...
from mason_cli.models import _resolve_copilot_catalog_api_key
