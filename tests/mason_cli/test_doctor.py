...
import mason_cli.doctor as doctor
import mason_constants
from mason_cli import config as config_mod
import mason_cli.gateway as gateway_cli
from mason_cli import doctor as doctor_mod
from mason_cli.doctor_config import _has_provider_env_config
import shutil
from mason_cli import doctor_tools
from mason_cli import doctor_state
from mason_cli import doctor_platform
from mason_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert mason_constants.is_termux() is True
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `mason doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "MASON_HOME", mason_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("MASON_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("MASON_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_mason_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal MASON_HOME with config.yaml."""
        home = tmp_path / ".mason"
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_mason_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "MASON_HOME", home)
        ...
        try:
            from mason_cli import auth as _auth_mod
...
def test_run_doctor_accepts_mason_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (mason_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".mason"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`mason doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp MASON_HOME.

        ``run_doctor`` reads the module-level ``MASON_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("MASON_HOME")`` alone
        leaves doctor probing the REAL ~/.mason. On a dev machine with a
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from mason_cli.doctor import run_doctor
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from mason_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("mason_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "mason_cli.doctor", raising=False)
        from mason_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale MASON_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"MASON_MAX_ITERATIONS={ghost}\n")
        (mason_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "MASON_HOME", mason_home)
        monkeypatch.setattr(doctor_mod, "get_mason_home", lambda: mason_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("MASON_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("MASON_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale MASON_MAX_ITERATIONS" in out
        env_after = (mason_home / ".env").read_text(encoding="utf-8")
        assert "MASON_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_mason_tool_progress_warning_says_unsupported_since_floor(self):
        """MASON_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"MASON_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["MASON_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"MASON_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["MASON_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        mason_home = tmp_path / ".mason"
        mason_home.mkdir(parents=True)
        (mason_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (mason_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"MASON_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: MASON_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mason" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "mason update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mason"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.nousresearch.mason" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (mason desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.mason" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
