"""ensure_mason_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".mason"
    second = tmp_path / "b" / ".mason"

    monkeypatch.setenv("MASON_HOME", str(first))
    cfg.ensure_mason_home()

    # Profile switch: MASON_HOME moves → the new path is ensured too.
