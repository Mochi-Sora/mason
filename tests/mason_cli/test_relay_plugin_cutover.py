"""Regression tests for migration from the removed Mason Relay plugin."""
...
from mason_cli.doctor_config import collect_relay_plugin_cutover_findings
...
def test_doctor_reports_legacy_exporter_env_without_new_config(monkeypatch):
    ...
    findings = dict(
        collect_relay_plugin_cutover_findings(
            {},
            {
                "MASON_NEMO_RELAY_ATIF_ENABLED": "true",
                "MASON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S": "30",
            },
        )
    )

    assert "now ignored" in findings["MASON_NEMO_RELAY_ATIF_ENABLED"]
    assert RELAY_PLUGINS_CONFIG_ENV in findings["MASON_NEMO_RELAY_ATIF_ENABLED"]
    assert "now ignored" in findings["MASON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    assert (
        RELAY_PLUGINS_CONFIG_ENV
        in findings["MASON_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    )
...
def test_enable_rejects_removed_relay_plugin_without_discovery(name, capsys):
    with (
        patch("mason_cli.plugins_cmd._resolve_plugin_key_and_source") as resolve,
        patch("mason_cli.plugins_cmd._save_enabled_set") as save_enabled,
    ):
        from mason_cli.plugins_cmd import cmd_enable
...
def test_enable_rejects_alias_resolving_to_removed_relay_plugin(capsys):
    with (
        patch(
            "mason_cli.plugins_cmd._resolve_plugin_key_and_source",
