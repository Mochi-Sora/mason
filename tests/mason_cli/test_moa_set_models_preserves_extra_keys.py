...
from mason_cli.web_models import MoaConfigPayload, MoaModelSlot, MoaPresetPayload
from mason_cli.web_routers.models import set_moa_models
...
class TestSetMoaModelsPreservesUndeclaredKeys:
    ...
    def test_save_traces_preserved(self, tmp_path):
        ...
        with (
            patch("mason_cli.config.load_config", side_effect=fake_load_config),
            patch("mason_cli.config.save_config", side_effect=fake_save_config),
            patch("mason_cli.web_server_profiles._profile_scope"),
        ):
