...
def test_doctor_restores_global_tool_policy_and_module_state(tmp_path: Path) -> None:
    ...
    before_modules = {
        ...
        if name == "mason_plugins" or name.startswith("mason_plugins.")
    }
...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from mason_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``mason plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary MASON_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from mason_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = mason_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_mason_home", lambda: mason_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a mason-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded mason-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("mason-plugin-doctor-*"))
