...
from mason_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    MASON_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def mason_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("mason_cli.config") or mod == "mason_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, mason_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, mason_home, capsys, monkeypatch):
