...
vulnerable sqlite3 CLI at a live Mason database.
...
Mason' own corruption banners used to instruct exactly that command
(`sqlite3 ~/.mason/state.db ".recover"`). The fix routes operators to
`mason sessions recover --source ...`, whose lane snapshots the damaged
...
from mason_cli.session_lost_and_found import (
    _parse_sqlite3_cli_version,
    _wal_reset_vulnerable,
    find_sqlite3_cli,
    find_sqlite3_cli_refusal,
)
from mason_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable


LIVE_DB_SALVAGE_COMMAND = 'sqlite3 ~/.mason/state.db ".recover"'
...
class TestFindSqlite3CliRefusal:
    def test_missing_binary_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "mason_cli.session_lost_and_found.shutil.which", lambda _: None
        )
    ...
    def test_no_dbpage_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "mason_cli.session_lost_and_found.shutil.which",
            lambda _: "/usr/bin/sqlite3",
        )
        monkeypatch.setattr(
            "mason_cli.session_lost_and_found._cli_supports_recover",
            lambda _: False,
        )
        assert find_sqlite3_cli() is None
    ...
    def test_wal_reset_vulnerable_refusal(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.session_lost_and_found._parse_sqlite3_cli_version",
            lambda _: (3, 45, 1),
        )
        assert find_sqlite3_cli() is None
...
class TestParseSqlite3CliVersion:
    def test_parses_modern_output(self):
        ...
        with patch(
            "mason_cli.session_lost_and_found.subprocess.run",
            return_value=Probe(),
        ):
...
class TestGuidanceNeverNamesLiveDb:
    ...
    def test_run_agent_corrupt_explanation(self):
        ...
        assert "mason sessions recover --source" in explanation
    ...
    def test_repair_budget_error_names_safe_lane(self, tmp_path: Path):
        import mason_state_repair as mason_state  # helper lives in the split-out repair module

        message = mason_state._persistent_repair_exhausted_error(
            tmp_path / "state.db"
        )
    ...
    def test_forensic_backup_refusals_name_safe_lane(self):
        ...
        body = inspect.getsource(mason_state_repair._backup_db_file)
        assert ".recover\"`" not in body
        assert "sessions recover --source" in body
        assert "--inspect-only" in body

    def test_kanban_manual_recovery_warns_about_live_db(self):
        import mason_cli.kanban_ops as kanban  # ``_cmd_repair`` lives here (split from mason_cli.kanban)
...
# printed `mason sessions recover --source <db>` — which cmd_sessions
...
class TestEmittedCommandsSatisfyCliContract:
    ...
    `mason sessions recover --source <db>`, which cmd_sessions rejects
    ...
    the real `cmd_sessions` (the same function `mason` main() hands the
    ...
    def _namespace(source: Path, **overrides) -> "argparse.Namespace":
        """The namespace mason main() produces for `sessions recover`.

        Mirrors the registrations in mason_cli/main.py (sessions_recover
    ...
    def test_old_v1_shape_is_still_rejected(self, tmp_path):
        ...
        import mason_cli.sessions_cmd as sc
