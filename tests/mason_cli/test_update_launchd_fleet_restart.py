"""Regression for #41403 — ``mason update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.mason.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed MASON_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.mason.gateway",
            "ai.mason.gateway-merit-ops",
            "ai.mason.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(mason_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.mason.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.mason.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.mason.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.mason.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.mason.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.mason.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.mason.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.mason.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.mason.gateway", "ai.mason.gateway-a", "ai.mason.gateway-b"],
        )
        located = {
            "ai.mason.gateway": (f"gui/{UID}", 100),
            "ai.mason.gateway-a": (f"gui/{UID}", 200),
            "ai.mason.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through mason_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.mason.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.mason.gateway", current, "ai.mason.gateway-user-scoped"],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.mason.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.mason.gateway",
            f"user/{UID}/ai.mason.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.mason.gateway",
            f"user/{UID}/ai.mason.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.mason.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.mason.gateway-a"],
            located={"ai.mason.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.mason.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mason.gateway",
            labels=["ai.mason.gateway", "ai.mason.gateway-idle"],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.mason.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mason.gateway",
            labels=["ai.mason.gateway", "ai.mason.gateway-dormant"],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.mason.gateway-dormant"]
        assert restarted == ["ai.mason.gateway", "ai.mason.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mason.gateway",
            labels=["ai.mason.gateway", "ai.mason.gateway-a"],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.mason.gateway-a"]
        assert restarted == ["ai.mason.gateway", "ai.mason.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.mason.gateway",
                "ai.mason.gateway-bad",
                "ai.mason.gateway-good",
            ],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-bad": (f"gui/{UID}", 200),
                "ai.mason.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.mason.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.mason.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.mason.gateway-good"]
        assert restarted == ["ai.mason.gateway", "ai.mason.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.mason.gateway",
                "ai.mason.gateway-wedged",
                "ai.mason.gateway-after",
            ],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.mason.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.mason.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.mason.gateway-after"]
        assert restarted == ["ai.mason.gateway", "ai.mason.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-wedged": (f"gui/{UID}", 200),
                "ai.mason.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.mason.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.mason.gateway",
            labels=["ai.mason.gateway", "ai.mason.gateway-zombie"],
            located={
                "ai.mason.gateway": (f"gui/{UID}", 100),
                "ai.mason.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.mason.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.mason.gateway"]
        assert failed == ["ai.mason.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.mason.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
