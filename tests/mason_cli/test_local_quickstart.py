...
def test_quickstart_refuses_when_nothing_fits(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.catalog.select_variant", lambda *a, **k: None)
...
def test_quickstart_runs_all_three_legs(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries.installed_tags", lambda: [])
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "mason_cli.web_routers.local_models.download_file", _fake_download)

    # Leg 3: activation — stub the server start and the model assignment.
    monkeypatch.setattr(
        "mason_cli.local_runtime.bootstrap.ensure_local_runtime",
        lambda config, force=False: calls.append("server") or None)
    monkeypatch.setattr(
        "mason_cli.web_routers.local_models._state_endpoint",
        lambda: {"base_url": "http://127.0.0.1:1/v1", "api_key": "k"})
    monkeypatch.setattr(
        "mason_cli.web_server_config._apply_model_assignment_sync",
...
def test_quickstart_skips_satisfied_legs(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries.ensure_runtime_installed",
        lambda tag, backend, progress=None: calls.append("install"))
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.bootstrap.staged_model_ids", lambda: all_ids)
    monkeypatch.setattr(
        "mason_cli.web_routers.local_models.download_file",
...
def quickstart_ready(monkeypatch):
    ...
    from mason_cli.local_runtime.catalog import VariantChoice

    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "mason_cli.local_runtime.catalog.select_variant",
        lambda entry, budget: VariantChoice(variant=entry.variants[0],
                                            zero_spill=True,
                                            reason_key="best-fits"))
    monkeypatch.setattr(
        "mason_cli.web_routers.local_models._engine_too_old",
...
def test_quickstart_is_single_flight(client, quickstart_ready, monkeypatch):
    ...
    import mason_cli.web_routers.local_models as lm
