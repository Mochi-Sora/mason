"""``mason`` must survive git operations on the checkout (launcher layout).

The Windows ``mason`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``MASON_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``mason-agent\\bin``) was swept by ``mason update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``mason`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``mason update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``mason_constants.venv_bin_dir``), so these
...
from mason_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: MASON_HOME/mason-agent/venv/Scripts + launchers."""
    home = tmp_path / "mason"
    root = home / "mason-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under MASON_HOME gains nothing anywhere."""
    home = tmp_path / "mason-home"
    home.mkdir()
    monkeypatch.setenv("MASON_HOME", str(home))
    root = tmp_path / "src" / "mason-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``mason -p <name>`` MASON_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("MASON_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `mason update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "mason.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "mason.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\mason\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/MASON/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``mason update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/mason.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/mason.exe is not gitignored — mason update's autostash "
