"""Venv ownership preflight for ``mason update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo mason`` contains root-owned files
(e.g. ``site-packages/mason_agent-*.dist-info/INSTALLER``). A later normal
``mason update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/mason`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "mason").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "mason_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "mason_agent-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    mason_bin = str(venv / "bin" / "mason")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == mason_bin else real_uid(p),
    )
    monkeypatch.setattr(
        update_cmd_deps,
        "_path_uid",
