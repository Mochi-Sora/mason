"""Tests for the AI_AGENT / MASON_AGENT harness-attribution env vars.
...
Mason-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Mason' id in the public agent-harness
registry (``mason-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Mason process env, and survives the
cross-session leak guard that strips ``MASON_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__MASON_CWD_testsession0__"
        env._snapshot_path = "/tmp/mason-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'MASON_AGENT="${MASON_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT MASON=$MASON_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "MASON_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} MASON=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", MASON_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi MASON=false" in out.stdout
