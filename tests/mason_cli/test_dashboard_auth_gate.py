...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 mason-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
...
def test_empty_provider_login_page_shows_supported_auth_paths():
    ...
    assert (
        'href="https://mason-agent.nousresearch.com/docs/'
...
def test_trusted_container_proxy_controls_https_detection():
    ...
    from mason_cli.dashboard_auth.cookies import detect_https
...
def test_loopback_public_url_fail_closed_message_is_actionable(monkeypatch):
    ...
    assert "mason dashboard register" in msg
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("MASON_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("MASON_DASHBOARD_PUBLIC_URL", public_url)
