...
This rewrites files Mason does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``MASON_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("MASON_HOME", str(home / ".mason"))
    return home


class TestHappyPath:
    def test_mason_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Mason Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/MASON_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Mason Agent" not in real.read_text(encoding="utf-8")
