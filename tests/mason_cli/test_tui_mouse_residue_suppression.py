...
The Python launcher (`mason --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Mason session
...
from mason_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["mason", "--tui"])
        monkeypatch.delenv("MASON_TUI", raising=False)
        monkeypatch.setenv("MASON_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["mason", "--tui"])
        monkeypatch.delenv("MASON_TUI", raising=False)
        monkeypatch.delenv("MASON_TUI_NO_EARLY_DISABLE", raising=False)
