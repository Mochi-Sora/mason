"""Tests for the Linux XDG desktop entry installed by ``mason desktop``."""
...
from mason_cli import linux_desktop_entry as lde
...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/mason on the dev machine.
...
def _stub_install(tmp_path, monkeypatch) -> None:
    mason_bin = tmp_path / "bin" / "mason"
    mason_bin.parent.mkdir(exist_ok=True)
    mason_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "mason_cli.relaunch.resolve_mason_bin", lambda: str(mason_bin)
    )
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    assert entry == xdg_home / "applications" / "mason.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `mason` would not resolve.
    assert values["Exec"] == f"{mason_bin} desktop"
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "mason.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "mason_cli.relaunch.resolve_mason_bin", lambda: "/usr/bin/mason"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("mason_cli.relaunch.resolve_mason_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m mason_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `mason`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(
    ...
    mason_bin = tmp_path / "bin" / "mason"
    mason_bin.parent.mkdir()
    mason_bin.write_text(
        "#!/usr/bin/env python3\nimport mason_cli\n", encoding="utf-8"
    )
    mason_bin.chmod(0o755)
    monkeypatch.setattr(
        "mason_cli.relaunch.resolve_mason_bin", lambda: str(mason_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(mason_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    mason_bin.parent.mkdir()
    mason_bin.write_text(
        '#!/bin/bash\nexec /opt/mason/venv/bin/python "$@"\n', encoding="utf-8"
    )
    mason_bin.chmod(0o755)
    monkeypatch.setattr(
        "mason_cli.relaunch.resolve_mason_bin", lambda: str(mason_bin)
    ...
    assert exec_line == f"{mason_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    mason_bin.parent.mkdir()
    interpreter = os.path.abspath(sys.executable)
    mason_bin.write_text(f"#!{interpreter}\nimport mason_cli\n", encoding="utf-8")
    mason_bin.chmod(0o755)
    monkeypatch.setattr(
        "mason_cli.relaunch.resolve_mason_bin", lambda: str(mason_bin)
    )
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/mason (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "mason"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport mason_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "mason"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "mason" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m mason_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m mason_cli.main desktop` fallback. Persisting the
    ...
    def fake_resolve():
        # Mirror resolve_mason_bin's chain: argv[0] → relative → PATH → None.
        return sys.argv[0] if sys.argv[0] else None

    monkeypatch.setattr("mason_cli.relaunch.resolve_mason_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's mason script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "mason"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "mason"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/mason may
    ...
    other_root.mkdir()
    foreign_wrapper = tmp_path / "known-home" / ".local" / "bin" / "mason"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/mason",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/mason",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/mason") < candidates.index(
            f"{sentinel_home}/.local/bin/mason"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/mason" not in candidates
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Mason is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "mason"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "mason"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("mason_cli.relaunch.resolve_mason_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_can_import_probe_runs_and_caches(tmp_path):
    ...
    deliberately NOT asserted - a CI host with mason pip-installed
    ...
    try:
        first = lde._can_import_mason_cli(real)
        assert isinstance(first, bool)
        assert str(real) in lde._probe_cache
        t0 = time.monotonic()
        second = lde._can_import_mason_cli(real)
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_mason_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "mason-agent"
    (home_real / "mason-agent").mkdir()

    shim = home_link / ".local" / "bin" / "mason"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/mason "$@"\n',
        encoding="utf-8",
    )
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "mason"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "mason"
...
def test_install_places_1024_png_in_256x256_not_scalable(
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "mason.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "mason.png"
    assert values["Icon"] == "mason"
    assert dest.is_file()
    assert dest.read_bytes() == lde.icon_path(root).read_bytes()
...
def test_install_removes_stale_scalable_png(tmp_path, xdg_home, monkeypatch):
    """v2026.8.31 wrote the PNG into scalable/. A later mason desktop
...
def test_install_exact_48_png_uses_48x48_dir(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "48x48" / "apps" / "mason.png"
    assert dest.is_file()
    assert not (
        xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "mason.png"
    ).exists()
...
def test_install_resizes_decodable_png_to_panel_sizes(
    ...
    dest_24 = xdg_home / "icons" / "hicolor" / "24x24" / "apps" / "mason.png"
    dest_256 = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "mason.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "mason.png"
