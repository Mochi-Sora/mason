...
Issue #98790: `mason cron status` in profile B reports profile A's gateway
PID as proof that B's jobs will fire. Root causes:

1. systemd branch of ``_get_service_pids()`` returns every mason-gateway*
...
class TestCronStatusHeartbeatGuard:
    """Ensure `mason cron status` correctly warns when the heartbeat file is absent.
    ...
    def test_no_heartbeat_triggers_yellow_warning(self, monkeypatch, capsys):
        from mason_cli import cron as cron_mod

        with (
            patch("mason_cli.gateway.find_gateway_pids", return_value={789}),
    ...
    def test_fresh_heartbeat_shows_green_checkmark(self, monkeypatch, capsys):
        ...
        with (
            patch("mason_cli.gateway.find_gateway_pids", return_value={999}),
...
class TestGetServicePidsProfileScope:
    ...
    def test_default_scope_filters_current_profile_systemd_unit(self, monkeypatch):
        from mason_cli import gateway as gateway_mod

        def _run_side_effect(args, **kwargs):
            cmd_str = " ".join(str(a) for a in args)
            if "list-units" in cmd_str:
                # systemctl now filters: return only the unit that matches the pattern
                if "mason-gateway-jarvis" in cmd_str:
                    stdout = "mason-gateway-jarvis.service loaded active running\n"
                elif "mason-gateway-coder" in cmd_str:
                    stdout = "mason-gateway-coder.service loaded active running\n"
                elif "mason-gateway*" in cmd_str:
                    stdout = (
                        "mason-gateway-jarvis.service loaded active running\n"
        ...
        with (
            patch("mason_cli.gateway.is_macos", return_value=False),
            patch("mason_cli.gateway.supports_systemd_services", return_value=True),
            patch("mason_cli.gateway.get_service_name", return_value="mason-gateway-jarvis"),
    ...
    def test_all_profiles_true_enumerates_fleet(self, monkeypatch):
            ...
            if "list-units" in cmd_str:
                return MagicMock(
                    ...
                    stdout=(
                        ...
                        "mason-gateway-profile-b.service loaded active running\n"
                    ),
                    stderr="",
                )
            if "show" in cmd_str and "MainPID" in cmd_str:
                if "mason-gateway.service" in " ".join(args):
                    return MagicMock(returncode=0, stdout="111\n", stderr="")
                elif "mason-gateway-profile-b.service" in " ".join(args):
        ...
        with (
            patch("mason_cli.gateway.is_macos", return_value=False),
            patch("mason_cli.gateway.supports_systemd_services", return_value=True),
            patch("mason_cli.gateway.get_service_name", return_value="mason-gateway"),
            patch("subprocess.run", side_effect=_run_side_effect),
        ):
...
class TestCronStatusMissingHeartbeat:
    ...
    def test_missing_heartbeat_warns_when_gateway_old(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        with (
            ...
            patch("mason_cli.gateway.find_gateway_pids", return_value=[4242]),
