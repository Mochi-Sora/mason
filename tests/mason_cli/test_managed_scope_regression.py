...
def mason_home(tmp_path, monkeypatch):
    ...
    # /etc/mason on the dev/CI box can't influence the test.
    monkeypatch.setenv("MASON_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(mason_home, monkeypatch):
...
def test_env_expansion_in_user_config(mason_home, monkeypatch):
    from mason_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_mason_dotenv(mason_home=str(home))
