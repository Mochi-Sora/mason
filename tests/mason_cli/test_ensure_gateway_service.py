"""Tests for mason_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `mason setup` and `mason import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("mason_constants.is_container", lambda: container)
