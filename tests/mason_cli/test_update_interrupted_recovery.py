...
``mason update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
import mason_cli.main_install_repair as mason_cli_main_install_repair
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr(mason_cli_main_install_repair, "_is_termux_env", lambda *a, **k: False)
    monkeypatch.setattr("mason_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to mason_cli._install_repair (shared between
    ...
    import mason_cli._install_repair as ir
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(mason_cli_main_install_repair, "_is_windows", lambda: True)
    monkeypatch.setattr(m, "_venv_scripts_dir", lambda: scripts_dir)
    monkeypatch.setattr(mason_cli_main_install_repair, "_venv_scripts_dir", lambda: scripts_dir)
    ...
    monkeypatch.setattr(
        mason_cli_main_install_repair, "_repair_venv_via_import_probes", lambda *a, **k: "healthy"
    )
