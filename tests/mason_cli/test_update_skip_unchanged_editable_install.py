"""``mason update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``mason.exe`` gets quarantined,
...
from mason_cli.update_cmd import _editable_install_is_current
