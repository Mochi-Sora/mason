...
``mason update`` can legitimately leave an autostash behind (--keep-stash
parks it; a conflicted restore preserves it), but nothing ever mentioned those
entries again — they persisted invisibly for weeks. ``mason update`` now
warns about ``mason-update-autostash-*`` entries older than the threshold.
...
def _make_repo_with_autostash(tmp_path, age_days: float):
    """Real repo with one mason-update-autostash entry aged ``age_days``."""
    import shutil
    ...
    name = f"mason-update-autostash-{stamp}"
...
def test_non_mason_stash_is_ignored(tmp_path, capsys):
    import shutil
...
def test_unparseable_autostash_timestamp_left_alone(tmp_path, capsys):
    ...
    _git(tmp_path, "stash", "push", "-m", "mason-update-autostash-notadate")
    count = update_cmd._warn_orphaned_update_autostashes(["git"], tmp_path)
    assert count == 0
