...
import mason_cli.local_runtime.catalog as cat
...
def _doc_from(entries):
    ...
    doc = json.loads(files("mason_cli.local_runtime")
...
def test_refresh_swaps_in_memory_only(monkeypatch, tmp_path):
    ...
    packaged_path = files("mason_cli.local_runtime").joinpath("catalog.json")
...
def test_min_engine_gate(monkeypatch):
    from mason_cli.web_routers.local_models import _engine_too_old

    monkeypatch.setattr("mason_cli.local_runtime.binaries.installed_tags",
                        lambda: ["b10362"])
