...
A network-bound `mason serve --host <ip>` powering a remote Desktop used to
be invisible to the update pipeline: not in the inventory, a dead-end at the
venv-holder guard, and never relaunched after `mason update` killed it. The
...
import mason_cli.update_inventory as update_inventory
from mason_cli import main as cli_main
import mason_cli.main_install_repair as main_install_repair
import mason_cli.main_dashboard as main_dashboard
...
def _ledger_entry(**over):
    entry = {
        ...
        "spawner_create": None,
        "registered_at": 222.0,
        "argv": "mason serve --host 100.94.65.93 --port 9119",
        "host": "100.94.65.93",
        "port": 9119,
        "profile": "",
    }
...
def test_register_self_records_structured_detail(tmp_path, monkeypatch):
    from mason_cli import process_identity as pi
...
def test_inventory_includes_manual_serve_from_ledger(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "mason_cli.process_identity", fake_pi)
...
def test_serve_relaunch_commands_built_from_structured_identity(monkeypatch):
    ...
    assert ["mason", "serve", "--host", "100.94.65.93", "--port", "9119"] in cmds
    assert ["mason", "--profile", "work", "serve", "--port", "9200"] in cmds
    assert ["mason", "dashboard", "--host", "0.0.0.0", "--port", "9300"] in cmds
...
def test_scan_dashboard_processes_includes_ledger_only_serves(monkeypatch):
    """A profiled serve (`mason --profile p serve ...`) matches no scan
    pattern; the ledger row must still surface it."""
    import mason_cli.dashboard_procs as dp

    profiled = _ledger_entry(
        pid=8123,
        argv="mason --profile work serve --host 100.94.65.93 --port 9119",
        profile="work",
    )
