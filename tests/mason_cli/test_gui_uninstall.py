"""Tests for mason_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import mason_cli.gui_uninstall as gu


def _make_agent(mason_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = mason_home / "mason-agent"
    (agent_root / "mason_cli").mkdir(parents=True)
    (agent_root / "mason_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(mason_home: Path) -> None:
    """Create the source-built GUI artifacts a `mason desktop` run produces."""
    desktop = mason_home / "mason-agent" / "apps" / "desktop"
    ...
    (mason_home / "mason-agent" / "node_modules").mkdir(parents=True)
    (mason_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(mason_home: Path) -> None:
    (mason_home / "config.yaml").write_text("x: 1\n")
    (mason_home / ".env").write_text("KEY=secret\n")
    (mason_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(mason_home)
    _make_gui_build(mason_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(mason_home)
    ...
    assert summary["mason_home"] == str(mason_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `mason desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    mason_home = tmp_path / ".mason"
    _make_agent(mason_home)
    icon = lde.icon_path(mason_home / "mason-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(mason_home)
    ...
    assert lde.icon_path(mason_home / "mason-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (mason_home / "mason-agent" / "mason_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".mason")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import mason_cli.uninstall as uninstall
