"""Tests for mason_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\mason — see
                # mason_constants._get_platform_default_mason_home.
                ...
                assert home == base / "mason"
            else:
                assert home == Path.home() / ".mason"


class TestEnsureMasonHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from mason_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Mason Agent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``mason logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from mason_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from mason_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MASON_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MASON_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MASON_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MASON_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("mason_cli.config.sanitize_env_file", return_value=2),
            patch(
                "mason_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("mason_cli.config.read_raw_config", return_value={}),
            patch("mason_cli.config.get_missing_env_vars", return_value=[]),
            patch("mason_cli.config.get_missing_config_fields", return_value=[]),
            patch("mason_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from mason_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """MASON_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `mason setup`) lets a
        ...
        via config.yaml; MASON_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from mason_cli.config import OPTIONAL_ENV_VARS
        assert "MASON_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `mason setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `mason setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_mason_home(self):
        from mason_cli.config_migrations import support_floor_message
        from mason_constants import display_mason_home

        msg = support_floor_message()
        assert f"{display_mason_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from mason_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Mason runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Mason
    process on next startup via the dotenv → ``os.environ`` chain in
    ``mason_cli/env_loader.py``.
    ...
    def _mason_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "MASON_LANGFUSE_PUBLIC_KEY",
            "MASON_SPOTIFY_CLIENT_ID",
            "MASON_QWEN_BASE_URL",
            "MASON_MAX_ITERATIONS",
        ],
    )
    def test_mason_integration_keys_still_writable(self, allowed_key):
        """``MASON_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "MASON_ENV_PATH",
            "MASON_OPTIONAL_MCPS",
            "MASON_COPILOT_ACP_COMMAND",
            "MASON_COPILOT_ACP_ARGS",
            "MASON_YOLO_MODE",
            "MASON_ACCEPT_HOOKS",
            ...
            "MASON_CRON_SESSION",
            "MASON_SINGLE_QUERY_SESSION",
            "MASON_SESSION_KEY",
            "MASON_SESSION_PLATFORM",
        ],
    )
    def test_mason_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from mason_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"MASON_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["MASON_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Mason_Yolo_Mode", "MASON_YOLO_MODE"),
            ("Mason_Optional_Mcps", "MASON_OPTIONAL_MCPS"),
            ("Mason_Copilot_Acp_Command", "MASON_COPILOT_ACP_COMMAND"),
            ("Mason_Copilot_Acp_Args", "MASON_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from mason_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from mason_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from mason_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"MASON_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: mason\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "mason"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import mason_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`mason config set/unset` must exit cleanly (no traceback) when the
