...
- ``mason_cli/_subprocess_compat.pid_is_mason`` / ``kill_process_tree``
- ``mason_cli/dashboard_procs._kill_stale_dashboard_processes`` (win32)
- ``mason_cli/update_cmd._stop_process_trees``
...
from mason_cli import _subprocess_compat
...
class TestPidIsMason:
    ...
    def test_non_windows_is_unconditional_pass(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False):
            assert _subprocess_compat.pid_is_mason(1234) is True
    ...
    def test_non_windows_still_rejects_recycled_identity(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False), mock.patch.object(
            ...
            assert _subprocess_compat.pid_is_mason(
                1234, expected_start_time=123
            ) is False

    def test_mason_match_requires_token_boundary(self):
        # "mason" buried inside an unrelated path segment must not match.
        assert _subprocess_compat._text_names_mason(
            r"c:\users\smasona\app.exe"
        ) is False
        assert _subprocess_compat._text_names_mason(
            r"C:\Users\x\.mason-runtime\python.exe -m mason_cli.main"
        ) is True
        assert _subprocess_compat._text_names_mason(
    ...
    def test_invalid_pid_inputs_do_not_crash(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True):
            assert _subprocess_compat.pid_is_mason(-1) is False
            assert _subprocess_compat.pid_is_mason(0) is False
            assert _subprocess_compat.pid_is_mason("not-a-pid") is False
            assert _subprocess_compat.pid_is_mason(True) is False

    def test_probe_matches_mason_like_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_mason", return_value=True
        ):
            assert _subprocess_compat.pid_is_mason(1234) is True
    ...
    def test_probe_rejects_foreign_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_mason", return_value=False
        ):
            assert _subprocess_compat.pid_is_mason(1234) is False
    ...
    def test_missing_pid_real_probe_fails_closed(self):
        # A PID that cannot exist must never be judged Mason-owned.
        assert _subprocess_compat.pid_is_mason(2**24) is False
...
class TestKillProcessTree:
    ...
    def test_retained_handle_is_taskkilled_without_probe(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            _subprocess_compat, "pid_is_mason"
...
class TestStopProcessTrees:
    ...
    def test_foreign_pids_only_probed(self):
        with mock.patch(
            ...
            "mason_cli._subprocess_compat.pid_is_mason", return_value=False
        ), mock.patch.object(update_cmd.subprocess, "run") as run:
            update_cmd._stop_process_trees([1111, 2222])
        run.assert_not_called()

    def test_mason_pid_probed_then_taskkilled(self):
        with mock.patch(
            "gateway.status.get_process_start_time", return_value=123
        ), mock.patch(
            "mason_cli._subprocess_compat.pid_is_mason", return_value=True
        ), mock.patch.object(
...
class TestKillStaleDashboardProcesses:
    ...
    def test_foreign_pid_reported_not_killed(self):
        ...
        assert result["failed"] == [
            (12345, "not mason-owned or process identity changed")
        ]
        run.assert_not_called()

    def test_mason_pid_killed(self):
        with self._patch_find(), mock.patch.object(
            dashboard_procs.sys, "platform", "win32"
        ), mock.patch(
            "gateway.status.get_process_start_time", return_value=123
