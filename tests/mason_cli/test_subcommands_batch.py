...
from mason_cli.subcommands.dashboard import build_dashboard_parser
...
from mason_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`mason config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `mason login` fails gracefully, not with argparse error ────
#
# `mason login` is a removed command; its handler (`login_command` in
# `mason_cli/auth.py`) prints a deprecation notice pointing at `mason auth` /
# `mason model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `mason --help`.
