...
which broke ``mason --tui`` because ``ui-tui/`` had vanished.
...
from mason_cli.update_cmd import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.mason-update-staging").exists()
    assert not (dst.parent / "ui-tui.mason-update-old").exists()
