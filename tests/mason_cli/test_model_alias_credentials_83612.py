...
def _install_config(monkeypatch, alias_entry):
    ...
    monkeypatch.setattr("mason_cli.config.load_config", lambda *a, **k: cfg)
    monkeypatch.setattr("mason_cli.runtime_provider.load_config", lambda *a, **k: cfg)
    return cfg
...
def _switch_to_alias(monkeypatch, alias_entry):
    ...
    monkeypatch.setattr(
        "mason_cli.models_validate.validate_requested_model", _fake_validate
    )

    import mason_cli.model_switch as ms
...
class TestDirectAliasCredentialLoading:
    def test_api_key_and_key_env_are_loaded_from_config(self, monkeypatch):
        ...
        from mason_cli.model_switch import _load_direct_aliases
    ...
    def test_credential_fields_default_to_empty(self, monkeypatch):
        ...
        from mason_cli.model_switch import DirectAlias
...
class TestDirectAliasApiKeyHelper:
    ...
    def test_resolves_literal_env_template_and_key_env(
        ...
        from mason_cli.model_switch import DirectAlias, direct_alias_api_key
...
class TestSessionKeyIsHostScoped:
    ...
    def _switch(self, monkeypatch, session_base_url):
        ...
        monkeypatch.setattr("mason_cli.config.load_config", lambda *a, **k: cfg)
        monkeypatch.setattr(
            "mason_cli.runtime_provider.load_config", lambda *a, **k: cfg
        )
        monkeypatch.setattr(
...
class TestProviderLabelCannotSelectAKeyForAnArbitraryHost:
    ...
    def _switch(self, monkeypatch, alias, session_provider="openrouter",
        ...
        monkeypatch.setattr("mason_cli.models_validate.validate_requested_model", _fake_validate)
        import mason_cli.model_switch as ms
...
class TestSchemelessBaseUrls:
    ...
    def test_schemeless_loopback_alias_keeps_the_session_credential(self, url):
        from mason_cli.model_switch import _may_reuse_session_credential
...
class TestOneShotUsesTheSameHostInvariant:
    """`mason chat -m <alias>` must not trust the alias's provider label.
    ...
    def test_no_key_alias_on_a_foreign_host_gets_no_provider_token(
        ...
        monkeypatch.setenv(env_var, secret)
        monkeypatch.setattr(
            "mason_cli.runtime_provider.load_config",
            lambda *a, **k: {"model": {"default": "m", "provider": "openrouter"}},
        )
        from mason_cli.model_switch import DirectAlias, direct_alias_runtime_request
...
class TestOneshotPassesAliasCredential:
    def test_alias_api_key_is_passed_to_the_resolver(self, monkeypatch):
        ...
        # its source module.
        monkeypatch.setattr(
            "mason_cli.runtime_provider.resolve_runtime_provider", _fake_resolve
        )
        monkeypatch.setattr("mason_cli.config.load_config", lambda *a, **k: {})
...
class TestNoProductionCodeMutatesTheAliasCacheInPlace:
    ...
    OWNER = ("mason_cli/model_switch.py", "_ensure_direct_aliases")
    ...
    def test_the_scan_actually_detects_a_violation(self):
        ...
        offending = (
            "from mason_cli.model_switch import DIRECT_ALIASES\n"
            "def warm():\n"
