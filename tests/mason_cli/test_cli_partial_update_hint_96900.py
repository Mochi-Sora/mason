...
``mason_cli.config`` while the older ``config.py`` does not export them.
Construction of ``MasonCLI`` then dies before the agent-setup mixin can
print ``partial_update_hint``. ``cmd_chat`` is the load-bearing catch:
bare ``mason`` and ``mason chat`` (including the fast-chat launch path)
all go through it.
"""
...
from mason_constants import emit_partial_update_hint, partial_update_hint
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'mason_cli.config'"
    )
    exc.name = "mason_cli.config"
    return exc
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "mason update" in buf.getvalue()
