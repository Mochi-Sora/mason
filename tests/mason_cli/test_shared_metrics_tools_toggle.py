"""Tests for the `mason tools` shared-metrics consent toggle.

AGENTS.md requires outbound telemetry to be reachable from a config gate, the
setup prompt, AND `mason tools`. These cover the third surface.
"""
...
class TestToggle:
    def test_enabling_send_persists(self, monkeypatch):
        ...
        saved = {}
        monkeypatch.setattr(
            "mason_cli.setup.prompt_yes_no", lambda *_a, **_k: True
        )
        monkeypatch.setattr(
            "mason_cli.setup._record_send_consent_change", lambda **_k: None
        )
        monkeypatch.setattr(
            "mason_cli.tools_config.save_config",
    ...
    def test_no_write_when_nothing_changed(self, monkeypatch):
        ...
        saved = []
        monkeypatch.setattr(
            "mason_cli.setup.prompt_yes_no", lambda *_a, **_k: False
        )
        monkeypatch.setattr(
            "mason_cli.tools_config.save_config", lambda cfg: saved.append(cfg)
        )
        _configure_shared_metrics_interactive(config)
        assert saved == []
    ...
    def test_disabling_collection_also_disables_sending(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.tools_config.save_config", lambda cfg: None
        )
        _configure_shared_metrics_interactive(config)
