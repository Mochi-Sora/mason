...
1. ``_spawn_gateway_restart_watcher``'s inlined watcher source must
   ...
   (b) stamp ``_MASON_GATEWAY_BREAKAWAY`` =1/0 on the respawn env exactly
...
from mason_cli.update_cmd import _resume_windows_gateways_after_update
...
def _captured_watcher_source(monkeypatch) -> str:
    ...
    assert gateway._spawn_gateway_restart_watcher(
        999999, ["python", "-m", "mason_cli.main", "gateway", "run"]
    )
    argv = captured["argv"]
    assert argv[1] == "-c"
    return argv[2]
...
class TestWatcherRespawnTemplate:
    ...
    def test_respawn_stamps_breakaway_state_like_spawn_detached(
        ...
        """The respawned gateway must carry _MASON_GATEWAY_BREAKAWAY=1 on
        ...
        assert _WINDOWS_GATEWAY_BREAKAWAY_ENV == "_MASON_GATEWAY_BREAKAWAY"
