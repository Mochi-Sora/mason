...
2. ``cli_main._refresh_windows_gateway_launchers`` — ``mason
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "mason_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("mason_cli.config.get_mason_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: mason update regenerates launchers
# ---------------------------------------------------------------------------
