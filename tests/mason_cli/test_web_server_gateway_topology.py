...
from mason_cli.web_server_gateway import _collect_profile_gateway_topology, _profile_platform_ports
...
class TestStatusEndpointTopology:
    ...
    def _setup_client(self, monkeypatch, _isolate_mason_home):
        try:
    ...
    def test_profile_names_and_mode_public_when_auth_gated(self, monkeypatch):
        ...
        # Mason Cloud Portal reads /api/status over the network (a gated bind)
        ...
        try:
            ...
            # alongside mason_home / gateway_pid.
            assert "gateways" not in data
            assert "mason_home" not in data
