...
The notice prints the most recent curator run summary on `mason update`,
...
session start) so users normally never see the rename map. `mason update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from mason_cli import update_cmd_maint
