"""Tests for mason_cli.web_server and related config utilities."""
...
import mason_cli.config as _cfg_mod
import mason_cli.web_routers.chat_ws as _rt_chat_ws
import mason_cli.web_routers.status as _rt_status
import mason_cli.web_server_chat as _web_server_chat
import mason_cli.web_server_config as _web_server_config
import mason_cli.web_server_dashboard as _web_server_dashboard
import mason_cli.web_server_files as _web_server_files
import mason_cli.web_server_gateway as _web_server_gateway
import mason_cli.web_server_lifecycle as _web_server_lifecycle
import mason_cli.web_server_memory as _web_server_memory
import mason_cli.web_server_messaging as _web_server_messaging
import mason_cli.web_server_sessions as _web_server_sessions
...
@pytest.fixture
def _install_example_plugin(_isolate_mason_home):
    """Drop the example-dashboard fixture into the per-test MASON_HOME
    ...
    ``MASON_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, mason-achievements, model providers
    ...
    from mason_cli import web_server

    user_plugins_dir = get_mason_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `mason plugins
    # enable example`.
    from mason_cli.config import load_config, save_config
    _cfg = load_config()
    ...
    #      and start serving requests against a torn-down MASON_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Mason vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints MASON_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("MASON_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mason_home):
        """Create a TestClient and isolate the state DB under the test MASON_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import mason_state
        from mason_constants import get_mason_home
        from mason_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(mason_state, "DEFAULT_DB_PATH", get_mason_home() / "state.db")
    ...
    def test_get_sessions_poll_preserves_pending_wal(self):
        ...
        db_path = get_mason_home() / "state.db"
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import mason_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `mason update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `mason update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(mason_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = mason_state.SessionDB.__init__
        ...
        # the class on mason_state covers every open the helper makes.
        monkeypatch.setattr(mason_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(mason_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mason_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        MASON_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import mason_cli.web_server as web_server
        from mason_cli import profiles as profiles_mod
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from mason_constants import get_mason_home
        from mason_cli.config import load_env
        ...
        config_path = get_mason_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from mason_constants import get_mason_home
        from mason_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide MASON_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from mason_constants import get_mason_home

        path = get_mason_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_mason_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["mason"]["workspace"] == "myws"
        assert cfg["hosts"]["mason"]["peerName"] == "eri"
        assert cfg["hosts"]["mason"]["environment"] == "local"
        assert cfg["hosts"]["mason"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["mason"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from mason_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_mason_returns_docker_guidance_without_spawning(self, monkeypatch):
        import mason_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn mason update")
        ...
        # method through mason_cli.config directly.
        monkeypatch.setattr(
            "mason_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_mason_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("mason-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("mason-update", None)

        resp = self.client.post("/api/mason/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "mason-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/mason-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/mason-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/mason-agent:latest" in line for line in status_data["lines"])

    def test_update_mason_returns_apt_guidance_without_spawning(self, monkeypatch):
        import mason_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn mason update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through mason_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "mason_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade mason-agent"
        assert spawned is False

        check = self.client.get("/api/mason/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade mason-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "mason-update.log").write_text(
            "=== mason-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== mason update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== mason-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/mason-update/status?lines=2000")
        ...
        assert f"=== mason-update completed {action_id} ===" in data["lines"]

    def test_update_mason_spawns_with_action_id(self, monkeypatch):
        import mason_cli.web_server as web_server
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_mason_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("mason-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("mason-update", None)
        ...
        assert calls == [
            (["update"], "mason-update", {"MASON_ACTION_ID": "a" * 32})
        ]

    def test_update_mason_reuses_running_action(self, monkeypatch):
        import mason_cli.web_server as web_server
        ...
        _web_server_gateway._ACTION_PROCS["mason-update"] = Proc()
        _web_server_gateway._ACTION_IDS["mason-update"] = "b" * 32

        try:
            resp = self.client.post("/api/mason/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("mason-update", None)
            _web_server_gateway._ACTION_IDS.pop("mason-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/mason_cli/test_setup_hidden_env.py.
        """
        from mason_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Mason provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from mason_cli.config import save_env_value
        from mason_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from mason_cli.config import save_env_value
        from mason_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "mason_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/MasonSetupBot/mason_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/MasonSetupBot/mason_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "mason_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_mason_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from mason_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from mason_cli.web_server_profiles import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``mason model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from mason_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from mason_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from mason_cli import profiles as profiles_mod
        from mason_cli.config import custom_endpoint_key_env
        from mason_constants import get_mason_home

        default_home = get_mason_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from mason_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from mason_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from mason_cli.web_server_config import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from mason_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from mason_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_mason_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from mason_constants import get_mason_home
        import mason_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import mason_cli.tools_config as tools_config
        from mason_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import mason_cli.tools_config as tools_config
        from mason_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "mason_cli.nous_subscription.get_nous_portal_account_info",
        ...
        import mason_cli.tools_config_post_setup as tools_config_post_setup
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "mason"
        save_config(config)
        ...
        assert "mason@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_DESKTOP", "1")
        monkeypatch.setenv("MASON_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("MASON_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # MASON_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from mason_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from mason_cli.web_server_config import _denormalize_config_from_web
        from mason_cli.config import save_config
...
class TestStatusInstallId:
    ...
    Mason home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_mason_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """MASON_HOME=<root> and MASON_HOME=<root>/profiles/<name> resolve to
        ...
        monkeypatch.setenv("MASON_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from mason_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.mason/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        from mason_constants import (
            reset_mason_home_override,
            set_mason_home_override,
        )
        from mason_cli import web_server

        token = set_mason_home_override(str(other))
        try:
            results = _web_server_dashboard._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(mason_home, name="ocean"):
        themes_dir = mason_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="mason-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mason_home):
        ...
        monkeypatch.setattr(
            mason_state, "DEFAULT_DB_PATH", get_mason_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``mason_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in mason_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mason_home):
        ...
        # Pin the SessionDB to the isolated MASON_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_mason_home, _install_example_plugin):
        ...
        installs it into the per-test ``MASON_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into MASON_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, mason-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (mason-achievements is loaded by default).
        resp = self.client.get("/api/plugins/mason-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        MASON_HOME override scopes a request to another profile."""
        from mason_constants import (
            reset_mason_home_override,
            set_mason_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets MASON_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the mason root's plugins/ directory."""
        root = tmp_path / "mason-root"
...
# or ``sh -c 'printf …'``) instead of the real ``mason --tui`` binary.  The
# endpoint resolves its argv through ``_resolve_chat_argv``, so tests
# monkeypatch that hook.
# ---------------------------------------------------------------------------

import sys
from mason_cli import main_tui_launch
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"mason-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "MASON_CWD": str(tmp_path),
            "MASON_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_tui_launch._apply_tui_python_env(env)

        assert env["MASON_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from mason_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from mason_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import mason_cli.main_tui_launch as tui_launch
    import mason_cli.web_server as ws
    ...
    gateway_url = env.get("MASON_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mason_home, _install_example_plugin):
        ...
        the per-test ``MASON_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_mason_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under MASON_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("MASON_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        _web_server_dashboard.mount_spa(spa_app)
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__MASON_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `mason serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__MASON_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("MASON_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__MASON_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__MASON_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/mason"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/mason/ds-assets/bg.png)" in prefixed.text
        assert "url(/mason/fonts-terminal/x.woff2)" in prefixed.text
