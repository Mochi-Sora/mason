...
reports the state and prints the one-switch setup; `mason setup` surfaces
...
import pathlib
from mason_cli.setup_quick import _print_macos_fda_tip
...
class TestSetupFdaTip:
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every Mason update" in out
