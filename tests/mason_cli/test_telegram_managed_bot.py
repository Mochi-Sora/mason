"""Tests for mason_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "mason_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/MasonSetupBot/mason_abcdefghijklmnop_bot?name=Mason+Agent",
            "qr_payload": "https://t.me/newbot/MasonSetupBot/mason_abcdefghijklmnop_bot?name=Mason+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "mason_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Mason Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="mason_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/MasonSetupBot/mason_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/MasonSetupBot/mason_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "mason_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "mason_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("mason_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("mason_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("mason_cli.telegram_managed_bot.time.sleep"):
                result = poll_for_setup_result(
...
class TestSetupTelegramAuto:
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from mason_cli import setup

        seen = {}
        profile_home = tmp_path / ".mason" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_mason_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "mason_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from mason_cli.setup_platforms import _profile_name_from_mason_home

        assert (
            _profile_name_from_mason_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\mason\profiles\oracle")
            )
            == "oracle"
        )
