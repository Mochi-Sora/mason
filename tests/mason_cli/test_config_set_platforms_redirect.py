...
`mason config set platforms.<name>.<display_setting> <value>` must write to
...
def _write_config(mason_home: Path, data: dict) -> Path:
...
def _set(monkeypatch, mason_home, key, value, force=False):
    """Isolated call to set_config_value against a temp MASON_HOME."""
    monkeypatch.setenv("MASON_HOME", str(mason_home))
    # set_config_value resolves the home live via get_config_path()/get_mason_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, mason_home, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, mason_home, "platforms.telegram.streaming", "false")

        result = yaml.safe_load((mason_home / "config.yaml").read_text())
    ...
    def test_show_reasoning_redirects(self, mason_home, monkeypatch):
        _set(monkeypatch, mason_home, "platforms.telegram.show_reasoning", "false")
        result = yaml.safe_load((mason_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["telegram"]["show_reasoning"] is False

    def test_tool_progress_redirects(self, mason_home, monkeypatch):
        ...
        # is the bool.
        _set(monkeypatch, mason_home, "platforms.discord.tool_progress", "off")
        result = yaml.safe_load((mason_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["discord"]["tool_progress"] is False

    def test_connection_key_not_redirected(self, mason_home, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, mason_home, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((mason_home / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, mason_home, monkeypatch, capsys):
        from gateway.display_config import resolve_display_setting
        from mason_cli.config import get_config_value
        ...
        raw = yaml.safe_load((mason_home / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, mason_home, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, mason_home, monkeypatch):
        from mason_cli.config import unset_config_value
    ...
    def test_set_prints_redirect_note(self, mason_home, monkeypatch, capsys):
        _set(monkeypatch, mason_home, "platforms.telegram.streaming", "false")
    ...
    def test_redirect_helper_only_touches_known_display_keys(self):
        ...
        from mason_cli.config import _redirect_platform_display_key
