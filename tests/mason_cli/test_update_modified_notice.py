"""Guard: every `mason update` path that reports user-modified skills must
also tell the user how to find them.

`mason update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``mason_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``mason skills list-modified`` so the count is actionable — otherwise,
...
import mason_cli.update_cmd as update_mod
import mason_cli.update_cmd_maint as update_maint_mod
import mason_cli.update_cmd_zip as update_zip_mod


_COUNT_RE = re.compile(r"user-modified \(kept\)")
_HINT_RE = re.compile(r"mason skills list-modified")


def _source_lines() -> list[str]:
    # The update pipeline was extracted to mason_cli/update_cmd.py and then
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`mason skills list-modified` within the following lines — the "
