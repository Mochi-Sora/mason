"""Tests for mason_cli.security_advisories.
...
import mason_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect MASON_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".mason"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full mason_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "mason_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
