...
the SAME provider universe as ``mason model`` (the CLI/TUI picker).
...
def test_every_mason_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `mason model` universe."""
    ...
    assert not missing, (
        "providers shown in `mason model` but not configurable in the desktop "
