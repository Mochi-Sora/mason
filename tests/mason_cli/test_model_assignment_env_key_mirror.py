...
def test_env_ref_key_carries_template_not_plaintext(_mason_home, monkeypatch):
    ...
    _apply("mylocal")
    model_cfg = _raw_model_cfg(_mason_home)
...
def test_key_env_entry_carries_pointer(_mason_home, monkeypatch):
    ...
    _apply("mylocal")
    raw = (_mason_home / "config.yaml").read_text()
    assert "sk-keyenv-secret" not in raw
    assert _raw_model_cfg(_mason_home).get("key_env") == "MYLOCAL_API_KEY"


def test_literal_key_still_mirrored(_mason_home):
    _write_config(
        _mason_home,
        {"mylocal": {"base_url": "http://localhost:1234/v1", "api_key": "literal-key-abc"}},
    )
    _apply("mylocal")
    assert _raw_model_cfg(_mason_home).get("api_key") == "literal-key-abc"
