...
``mason_cli/web_routers/profiles.py`` holds handler bodies that were extracted
...
* a **loop probe** — the stubbed callee records whether an event loop is
  ...
  ``tests/mason_cli/test_cron_dashboard_off_loop.py``; and
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """A real profile directory under a throwaway MASON_HOME."""
...
def _outcome(ok=True, reason="described", description="a demo profile"):
    from mason_cli.profile_describer import DescribeOutcome
...
def test_update_profile_model_runs_off_loop(client, monkeypatch, loop_probe):
    seen, probe = loop_probe
    from mason_cli.web_routers import profiles as router_mod
