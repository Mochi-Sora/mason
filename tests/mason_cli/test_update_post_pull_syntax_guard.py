"""Tests for the post-pull syntax guard in ``mason update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``mason_cli/config.py``), the CLI
becomes unbootable — every ``mason`` invocation imports those files at
...
markers in ``mason_cli/config.py``; users who ran ``mason update`` in
the 7-minute window before #28458 landed could not run any ``mason``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mason_main.subprocess, "run", fake_run)
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except mason_constants.py
    for relpath in update_cmd._UPDATE_CRITICAL_FILES:
        if relpath == "mason_constants.py":
            continue
...
# release; if a future ``mason update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
