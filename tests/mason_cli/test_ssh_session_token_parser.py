...
from mason_cli.main import cmd_dashboard
from mason_cli.main_dashboard import _read_ssh_session_token_file
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``$HOME/.mason/desktop-ssh`` (a literal ``~/.mason/desktop-ssh`` in
    apps/desktop/electron/remote-lifecycle.ts, expanded against the account's
    $HOME). A non-default sticky profile re-homes ``get_mason_home()`` to
    ``<root>/profiles/<name>``, and a Docker-style ``MASON_HOME`` can point
    elsewhere entirely — neither must move the validator off
    ``$HOME/.mason/desktop-ssh``, or the token is wrongly rejected."""
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_dir = home / ".mason" / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"

    # A sticky profile and a custom (Docker) root both point get_mason_home()
    # away from $HOME/.mason; the anchor must ignore both.
    for elsewhere in (home / ".mason" / "profiles" / "coder", tmp_path / "opt" / "data"):
        token_path.write_text("b" * 64)
        token_path.chmod(0o600)
        override = set_mason_home_override(elsewhere)
        try:
            assert _read_ssh_session_token_file(str(token_path)) == "b" * 64
            assert not token_path.exists()
        finally:
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    profile_home = home / ".mason" / "profiles" / "coder"
    token_dir = profile_home / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"
    token_path.write_text("b" * 64)
    token_path.chmod(0o600)
    override = set_mason_home_override(profile_home)
    try:
...
def test_token_file_rejects_symlink(tmp_path, monkeypatch):
    ...
    override = set_mason_home_override(home / ".mason")
    try:
...
def test_token_file_rejects_parent_escape(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_root = home / ".mason" / "desktop-ssh"
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from mason_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-mason-root"
    monkeypatch.setenv("MASON_HOME", str(machine_root / "profiles" / "writer_2"))
