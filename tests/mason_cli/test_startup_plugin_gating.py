...
``mason_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``mason --help``, ``mason --version``,
``mason logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from mason_cli._parser import build_top_level_parser, top_level_value_flag_sets
from mason_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``mason --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from mason_cli import main as _main
...
def test_reasoning_value_is_not_misclassified_as_subcommand(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["mason", "--reasoning", "high", "chat", "hello"],
    )
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``mason photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain mason-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level mason <name> CLI command via PluginContext.
