...
Every aux picker (``mason model`` → Configure auxiliary models, the
``mason tools`` vision picker, and any future one) must route through
``mason_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
import pytest
import yaml
from mason_cli import main_provider_setup
...
def configured_home(tmp_path, monkeypatch):
    """A MASON_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from mason_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("mason_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("mason_cli.model_switch.list_authenticated_providers", _direct),
        patch("mason_cli.inventory.build_aux_picker_rows", _substrate),
        patch("mason_cli.main_provider_setup._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through mason_cli.inventory.build_aux_picker_rows so custom "
