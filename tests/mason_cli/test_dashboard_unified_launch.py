...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # MASON_HOME.  For a standard install (MASON_HOME unset) that root is
        # the platform-native default (~/.mason), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from mason_constants import get_default_mason_root
        assert env.get("MASON_HOME") == str(get_default_mason_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (MASON_DESKTOP=1) must NOT
