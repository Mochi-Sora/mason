...
def test_installed_tags_newest_first(mason_home):
    from mason_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(mason_home, "b10290")
    _install_fake_tag(mason_home, "b10412")
    assert installed_tags() == ["b10412", "b10290"]


def test_default_tag_flows_from_default_config(mason_home):
    """Unpinned users inherit the Mason-release default (deep-merge);
...
def test_update_available_requires_enabled_and_installed(mason_home, monkeypatch):
...
def test_boot_never_downloads_missing_tag(mason_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(mason_home):
    from mason_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(mason_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = mason_home / "runtimes" / "llamacpp" / "downloads"
