...
def _install_pid_sequence(monkeypatch, snapshots):
    ...
    monkeypatch.setattr("mason_cli.gateway.find_gateway_pids", _fake)
    return calls
...
def test_report_gateway_start_failure_is_loud_not_checkmark(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "mason_cli.config.get_mason_home", lambda: str(tmp_path)
    )
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Mason_Gateway_x")

    gateway_windows._report_gateway_start("direct spawn (PID 7)")
    out = capsys.readouterr().out
    assert "✓" not in out
    assert "FAILED" in out
    assert "schtasks /Run /TN Mason_Gateway_x" in out
...
def attest_home(monkeypatch, tmp_path):
    monkeypatch.setattr("mason_cli.config.get_mason_home", lambda: str(tmp_path))
...
def test_attestation_warning_includes_schtasks_recovery(monkeypatch, attest_home):
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(
        gateway_windows, "get_task_name", lambda: "Mason_Gateway_arthur_tutor"
    )
    gateway_windows._write_start_attestation([888], "direct spawn (PID 888)")
    warning = gateway_windows.check_start_attestation(current_pids=[])
    assert "schtasks /Run /TN Mason_Gateway_arthur_tutor" in warning
...
def test_breakaway_fallback_warns_even_on_success(monkeypatch, attest_home, capsys):
    ...
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Mason_Gateway")
    ...
    assert "schtasks /Run /TN Mason_Gateway" in out
