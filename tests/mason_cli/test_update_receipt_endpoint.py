...
The update receipt (written by every `mason update` run since #91283,
`latest.json` pointer) is the durable outcome record. These tests pin:

- GET /api/mason/update/receipt returns the full receipt + summary; 404
  when none exists.
- GET /api/actions/mason-update/status attaches the receipt summary, and
...
def client():
    ...
    from mason_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
...
class TestUpdateReceiptEndpoint:
    ...
    def test_receipt_endpoint_returns_full_receipt_and_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/mason/update/receipt")
...
class TestUpdateStatusReadsReceipt:
    ...
    def test_status_attaches_receipt_summary(self, client, tmp_path, monkeypatch):
        ...
        resp = client.get("/api/actions/mason-update/status")
