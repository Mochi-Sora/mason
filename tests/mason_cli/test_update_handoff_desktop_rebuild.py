...
A Windows git install runs `mason update` from `mason.exe`, which reexecs a
...
def test_current_checkout_repair_rebuilds_desktop_under_project_root():
    ...
    with (
        ...
        m.return_value.PROJECT_ROOT = update_cmd.Path("/fake/mason")
    ...
    assert rebuild.call_args[0][0] == update_cmd.Path("/fake/mason/apps/desktop")
