"""Tests for SIGHUP protection and stdout mirroring in ``mason update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``mason_cli/main.py``.  These exist so
that ``mason update`` survives a terminal disconnect mid-install
...
from mason_cli.main_dashboard import _UpdateOutputStream, _finalize_update_output, _install_hangup_protection
from mason_cli.update_cmd import _log_only_write, _print_update_completion, _run_logged_subprocess


def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("MASON_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("mason_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== mason-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("MASON_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("mason_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import mason_cli.config as _cfg
        if hasattr(_cfg, "_MASON_HOME_CACHE"):
            _cfg._MASON_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "mason update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_mason_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "mason_cli.config.get_mason_home", _boom, raising=True
        )
