"""Regression tests: `mason dashboard` validates MASON_WEB_DIST before serving.

A custom MASON_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "mason_cli.profiles.get_active_profile_name", lambda: "default"
    )
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    ...
    builds = []
    monkeypatch.setattr(
        "mason_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "MASON_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("MASON_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "mason_cli" / "web_dist"
    ...
    monkeypatch.setattr("mason_cli.main_web_build._build_web_ui", fake_build)
