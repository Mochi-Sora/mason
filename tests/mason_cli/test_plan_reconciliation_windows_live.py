...
Real processes with real Mason-shaped argv, real inventory collection
...
reconciliation. No mocks on the components under test.
 ...
 1. Spawn a real process registered as a profile gateway (PID file + state
    file in a temp MASON_HOME) — real collect_runtime_inventory() must find
...
def test_plan_reconciliation_live_windows(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(profiles_mod, "_get_default_mason_home", lambda: home)
