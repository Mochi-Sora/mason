...
``venv\\Scripts\\mason.exe`` is a launcher that runs the interpreter with the
...
The hand-off sits at the sync boundary, not at the top of ``mason update``:
...
SHIM_NAMES = ["mason.exe", "mason-agent.exe", "mason-acp.exe", "mason-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    # update_cmd reads these off mason_cli.main (frozen ``_m()`` surface); the
    ...
    monkeypatch.setattr(sys, "argv", ["mason", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "mason.exe" / "__main__.py")])
    assert main_install_repair._windows_shim_in_process_chain() == venv / "mason.exe"


def test_detects_shim_from_main_module_spec_origin(venv, monkeypatch):
    fake_main = types.SimpleNamespace(
        __file__=None,
        __spec__=types.SimpleNamespace(origin=str(venv / "mason.exe")),
    )
...
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    ...
    _fake_psutil(monkeypatch, [str(venv / "mason.exe")])
    assert main_install_repair._windows_shim_in_process_chain() == venv / "mason.exe"


def test_ignores_mason_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "mason.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "mason.exe")])
...
def test_no_shim_off_windows(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "mason.exe"), "update"])
    assert main_install_repair._windows_shim_in_process_chain() is None
...
def test_reexec_falls_through_when_venv_python_is_missing(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m mason_cli.main update" not in capsys.readouterr().out
...
def test_reexec_falls_through_when_spawn_fails(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_dependency_sync_off_windows_shim() is False
    assert "-m mason_cli.main update" in capsys.readouterr().out
...
def test_up_to_date_run_never_hands_off(venv, monkeypatch, capsys):
    ...
    The hand-off used to run before the fetch, so every ``mason update`` —
    ...
    """
    monkeypatch.setattr(sys, "argv", [str(venv / "mason.exe"), "update", "--check"])
    calls = _capture_popen(monkeypatch)
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\mason\venv\Scripts\mason.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\mason\venv\Scripts\mason.exe",
        r"!\??\C:\mason\venv\Scripts\mason.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\mason\venv\Scripts\mason.exe")]
    entries = [
        r"\??\C:\mason\venv\Scripts\mason.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = main_install_repair._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries
