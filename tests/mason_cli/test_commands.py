...
from mason_cli.commands import COMMAND_REGISTRY, COMMANDS, COMMANDS_BY_CATEGORY, CommandDef, GATEWAY_KNOWN_COMMANDS, SUBCOMMANDS, command_desktop_meta, gateway_help_lines, infer_argument_mode, resolve_command
from mason_cli.commands_completion import SlashCommandAutoSuggest, SlashCommandCompleter
from mason_cli.commands_platforms import _CMD_NAME_LIMIT, _SLACK_RESERVED_COMMANDS, _SLACK_VIA_MASON_ONLY, _clamp_command_names, _sanitize_telegram_name, slack_app_manifest, slack_native_slashes, slack_subcommand_map, telegram_bot_commands, telegram_menu_commands
...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /mason <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_mason_norm = {_norm(n) for n in _SLACK_VIA_MASON_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_mason_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `mason slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.config.load_config", lambda: {})
        monkeypatch.setattr(
            "mason_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
class TestGatewaySkillCollector:
    ...
    def test_long_skill_name_clamped_but_cmd_key_retained(self, tmp_path):
        ...
        from mason_cli.commands_platforms import _collect_gateway_skill_entries
    ...
    def test_cap_trims_skills_only(self, tmp_path):
        ...
        with (
            patch("agent.skill_commands.get_skill_commands", return_value=fake_cmds),
            patch("tools.skills_tool.SKILLS_DIR", skills_dir),
            patch("mason_cli.plugins.get_plugin_commands", return_value={"plug": {"description": "p"}}),
        ):
            entries, hidden = _collect_gateway_skill_entries(
...
class TestTelegramMenuCommands:
    ...
    def test_configured_priority_promotes_skill_into_last_menu_slot(
        ...
        with (
            patch("mason_cli.commands_platforms.telegram_bot_commands", return_value=fake_core),
            patch("agent.skill_commands.get_skill_commands", return_value=fake_cmds),
            patch("mason_cli.plugins.get_plugin_commands", return_value=fake_plugins),
            patch("tools.skills_tool.SKILLS_DIR", local_dir),
            patch("mason_cli.commands_platforms._telegram_command_menu_config", return_value=menu_cfg),
        ):
    ...
    def test_scalar_configured_priority_is_accepted_as_one_command(self):
        ...
        from unittest.mock import patch
        from mason_cli.commands_platforms import _telegram_command_menu_config
        ...
        with patch("mason_cli.config.read_raw_config", return_value=raw_config):
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch mason_cli.plugins.get_plugin_commands() to a fixed dict."""
        from mason_cli import plugins as _plugins_mod
