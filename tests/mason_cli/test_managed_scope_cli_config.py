...
mason_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `mason config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_mason_home = get_mason_home()`` at import time (module
    singleton), so monkeypatching MASON_HOME after import doesn't move it.
    ...
    this only matters for tests that swap MASON_HOME.)
    """
    import cli

    cli._mason_home = home
