"""Tests for mason_cli.profile_distribution — git-based profile installs.
...
from mason_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/mason_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_mason_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Mason"):
                check_mason_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from mason_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_mason_requires(self, profile_env, monkeypatch):
        # Pin current Mason version to something well below the requirement
        import mason_cli
        monkeypatch.setattr(mason_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            mason_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "mason_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from mason_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from mason_cli.profiles import create_profile, list_profiles, get_profile_dir
