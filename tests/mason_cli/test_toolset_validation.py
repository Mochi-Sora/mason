"""Unit tests for mason_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Mason.
"""
...
# A representative set of real toolset names. `mason` is deliberately absent —
# that is the corruption #38798 reported (`mason-cli` rewritten to `mason`).
_KNOWN = {
    "mason-cli",
    "mason-telegram",
    "mason-discord",
...
def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'mason' instead of 'mason-cli'.
    warnings = validate_platform_toolsets({"cli": ["mason"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'mason'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'mason-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["mason-cli"], "discord": ["bogus"]}
...
def test_empty_list_on_a_platform_warns_even_when_others_are_valid():
    ...
    cfg = {
        "cli": [],
        "telegram": ["mason-telegram"],
        "discord": ["mason-discord"],
    }
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_empty_list_warns_for_each_affected_platform():
    cfg = {"cli": [], "discord": [], "telegram": ["mason-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_platform_warns_even_when_others_are_valid():
    ...
    cfg = {"cli": None, "telegram": ["mason-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'cli'" in w
        and "null toolset value" in w
        and "falling back to 'mason-cli'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_null_platform_uses_canonical_default_for_alias():
    cfg = {"whatsapp_cloud": None, "telegram": ["mason-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'whatsapp_cloud'" in w
        and "falling back to 'mason-whatsapp'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_scalar_platform_value_warns_but_uses_platform_default():
    cfg = {"cli": "bogus", "telegram": ["mason-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_platform_restricted_toolset_warns_when_other_platform_is_valid():
    cfg = {"telegram": ["discord"], "cli": ["mason-cli"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_plugin_platform_uses_synthetic_default():
    ...
    try:
        cfg = {platform: None, "telegram": ["mason-telegram"]}
        warnings = validate_platform_toolsets(cfg, _is_valid)

        assert resolve_toolset(f"mason-{platform}")
        assert any(
            f"platform '{platform}'" in w
            and f"falling back to 'mason-{platform}'" in w
            for w in warnings
        )
        assert not any("zero valid toolsets" in w for w in warnings)
    finally:
...
def test_populated_platforms_produce_no_empty_list_warning():
    cfg = {"cli": ["mason-cli"], "telegram": ["mason-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert warnings == []
