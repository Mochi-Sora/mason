"""Unit tests for mason_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/mason_cli/test_pty_bridge.py.  Drives
...
``mason_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from mason_cli import win_pty_bridge
from mason_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from mason_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
    ...
    async def test_leaked_write_worker_is_logged(self, caplog):
        ...
        bridge = WinPtyBridge(proc)
        with patch.object(win_pty_bridge, "_WRITE_SHUTDOWN_GRACE", 0.05), \
                caplog.at_level(logging.WARNING, logger="mason_cli.win_pty_bridge"):
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from mason_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp
    ...
    def test_non_numeric_falls_back_to_min(self):
        from mason_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('MASON_PTY_TEST=' + os.environ.get('MASON_PTY_TEST',''))",
            ],
            env={**os.environ, "MASON_PTY_TEST": "pty-env-works"},
        )
        try:
