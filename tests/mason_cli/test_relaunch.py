"""Tests for mason_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from mason_cli import relaunch as relaunch_mod


class TestResolveMasonBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/mason"
        ...
        assert relaunch_mod.resolve_mason_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'mason' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_mason_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/mason" if name == "mason" else None
        )
        assert relaunch_mod.resolve_mason_bin() == "/usr/bin/mason"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_mason_bin", lambda: "/usr/bin/mason")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/mason"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/mason", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/mason", ["/usr/bin/mason", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        mason).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_mason_bin", lambda: r"C:\Users\test\mason.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\mason.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\mason.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_mason_bin", lambda: r"C:\mason.exe")
...
class TestResolveMasonBinWindowsPyGuard:
    """On Windows, resolve_mason_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a mason.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\mason.exe" if name == "mason" else None,
        )

        bin_path = relaunch_mod.resolve_mason_bin()
        # Must NOT be the .py — must be the mason.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\mason.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "mason"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_mason_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_mason_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND mason.exe
        ...
        assert relaunch_mod.resolve_mason_bin() is None
