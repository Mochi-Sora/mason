...
from mason_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import tools.skills_hub_install as hub_install
    import mason_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`mason skills list` reads the active profile's config via
    MASON_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from MASON_PLATFORM env."""
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `mason skills search`
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    from mason_cli.skills_hub import do_search
