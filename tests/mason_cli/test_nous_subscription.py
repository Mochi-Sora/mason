...
from mason_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import mason_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "mason_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_logged_in_entitled_account_yields_a_state_for_every_feature(monkeypatch):
    ...
    crashing `mason status`, `mason tools`, and the dashboard toolsets API."""
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `mason model` overwrote it to `web.backend: nous`.
    """
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "mason_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `mason tools`
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("mason_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_mason_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool_install cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Mason Node dir even when
    ...
    monkeypatch.setattr(
        "mason_constants.with_mason_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "mason_constants.agent_browser_runnable",
