"""Tests for IRC gateway configuration via `mason setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Mason install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Mason install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#mason")
            monkeypatch.setenv("IRC_NICKNAME", "mason-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `mason setup gateway` experience with IRC present."""
