"""`mason proxy` must resolve upstream credentials off the event loop.
...
blocking I/O:
  ...
  * ``NousPortalAdapter.get_credential`` takes ``_auth_store_lock()``, a
    *cross-process* advisory lock with ``AUTH_LOCK_TIMEOUT_SECONDS = 15.0``
    (``mason_cli/auth.py:110``), reads ``auth.json`` from disk, and may issue a
...
The harness mirrors ``tests/mason_cli/test_proxy.py``: the proxy and a fake
