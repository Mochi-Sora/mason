...
profile switcher can target any profile's MASON_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via MASON_HOME.
"""
import json
...
import mason_cli.web_server_profiles as _web_server_profiles


@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_mason_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_mason_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import mason_cli.mcp_config as mcp_config
...
class TestProfileScopedModel:
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "mason_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "mason_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same MASON_HOME the rest
        ...
        monkeypatch.setattr(
            "mason_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_mason_home()))
            return None
        ...
        assert resp.json()["mason_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_mason_home(self, isolated_profiles, monkeypatch):
        import mason_cli.web_server as web_server

        monkeypatch.setattr(
            "mason_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["MASON_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "MASON_TUI_GATEWAY_URL" not in env
