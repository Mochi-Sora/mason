...
underneath are exercised against temp MASON_HOME (autouse fixture). Network
...
def test_catalog_never_hides_unaffordable_models(client, monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.local_runtime.hardware.probe_budget",
                        lambda **kw: tiny)
    data = client.get("/api/local-models/catalog").json()
...
def test_download_already_downloaded_short_circuits(client, monkeypatch):
    from mason_cli.local_runtime.bootstrap import models_dir
    from mason_cli.local_runtime.catalog import CATALOG, select_variant
    from mason_cli.local_runtime.estimator import HardwareBudget
...
def test_runtime_install_rejects_impossible_combo(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries._host_os_arch", lambda: ("win", "arm64"))
...
def test_eject_without_supervisor_is_not_a_500(client, monkeypatch):
    ...
    server session."""
    monkeypatch.setattr(
        "mason_cli.local_runtime.bootstrap.get_supervisor", lambda: None)
    # No running server either: the route must answer 409 (no server),
    # never a NameError 500.
    monkeypatch.setattr(
        "mason_cli.web_routers.local_models._state_endpoint", lambda: None)
...
def test_download_tolerates_stale_catalog_size(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.bootstrap.refresh_local_runtime",
        lambda: False)
