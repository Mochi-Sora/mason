...
class TestHolderSubcommand:
    @pytest.mark.parametrize(
        ("cmdline", "expected"),
        [
            (r"C:\x\venv\Scripts\python.exe -m mason_cli.main serve --host 127.0.0.1", "serve"),
            (r"C:\x\venv\Scripts\python.exe -m mason_cli.main dashboard", "dashboard"),
            (r"python.exe -m mason_cli.main gateway run", "gateway"),
            # profile selector skipped; its VALUE must not become the subcommand
            (r"python -m mason_cli.main --profile serve gateway run", "gateway"),
            (r"python -m mason_cli.main -p work serve", "serve"),
            # 90778: flags containing subcommand words are not subcommands
            (r"python -m mason_cli.main kanban --preserve-cache", "kanban"),
            # 91869 review: EVERY top-level value flag must be skipped —
            # a flag VALUE equal to a subcommand must not become the label
            (r"python -m mason_cli.main --reasoning high serve", "serve"),
            (r"python -m mason_cli.main -m dashboard serve", "serve"),
            (r"python -m mason_cli.main -t browser,files gateway run", "gateway"),
            (r"python -m mason_cli.main --model=dashboard serve", "serve"),
            # -c consumes ONE value token; later bare tokens are (harmless,
            # unhinted) subcommand candidates — pin that shape honestly
            (r"python -m mason_cli.main -c mysession serve", "serve"),
            (r"C:\bin\mason.exe dashboard", "dashboard"),
            (r"/usr/local/bin/mason serve", "serve"),
            # no mason entry at all
            (r"python -c import time; time.sleep(3)", None),
            # entry but no subcommand
            (r"python -m mason_cli.main", None),
        ],
    )
    def test_parses_subcommand(self, cmdline, expected):
        assert _mason_holder_subcommand(cmdline) == expected
...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        message = self._msg(r"C:\v\Scripts\python.exe -m mason_cli.main dashboard")
        assert "close the desktop app" not in message.lower()
        assert "mason dashboard" in message

    def test_preserve_cache_not_labeled_serve(self):
        message = self._msg(r"python -m mason_cli.main kanban --preserve-cache")
    ...
    def test_serve_gets_backend_hint(self):
        message = self._msg(r"python -m mason_cli.main serve --host 127.0.0.1 --port 0")
        assert "Mason backend" in message

    def test_gateway_hint(self):
        message = self._msg(r"python -m mason_cli.main gateway run")
