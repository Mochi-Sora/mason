"""Tests for Bug #12905 fix — stale OAuth token detection in mason model flow.

Bug 3: `mason model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.mason/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("mason_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from mason_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
