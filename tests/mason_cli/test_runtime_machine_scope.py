...
def profile_home(tmp_path, monkeypatch):
    """A NAMED-profile MASON_HOME under <root>/profiles/<name>."""
    root = tmp_path / ".mason"
    profile = root / "profiles" / "coder"
    profile.mkdir(parents=True)
    monkeypatch.setenv("MASON_HOME", str(profile))
    # mason_constants memoizes root resolution per (native, env) pair;
...
def test_models_and_runtimes_resolve_to_the_shared_root(profile_home):
    ...
    import mason_cli.local_runtime.binaries as binaries
    import mason_cli.local_runtime.bootstrap as bootstrap
...
def test_all_runtime_state_follows_runtimes_root(profile_home):
    ...
    root, profile = profile_home
    from mason_cli.local_runtime.growth import window_overrides_path
...
def test_default_profile_paths_unchanged(tmp_path, monkeypatch):
    """MASON_HOME at the root itself (default profile) resolves exactly
