...
MASON_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
HEADERS = {"X-Mason-Session-Token": _SESSION_TOKEN}
...
@pytest.fixture
def mason_home(monkeypatch, tmp_path):
    """Fresh MASON_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(mason_home):
    _write_env(mason_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(mason_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = mason_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(mason_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(mason_home, OPENAI_API_KEY=old)
    _write_config(
        mason_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = mason_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `mason auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(mason_home):
    ...
    the old cached credential until a separate ``mason auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(mason_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.mason/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(mason_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(mason_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(mason_home):
    old = "sk-kp-" + "p" * 24
    _write_env(mason_home, OPENAI_API_KEY=old)
    _write_config(
        mason_home,
...
def test_scrub_never_touches_providers_base_url_alias(mason_home):
