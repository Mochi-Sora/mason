...
def test_find_install_script_from_checkout(tmp_path):
    ...
    """
    from mason_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    path, shell = _find_install_script(package_dir=tmp_path / "mason_cli", repo_root=tmp_path)
...
def test_has_npx_agent_browser_true_when_npx_resolves():
    ...
    from mason_cli.dep_ensure import _has_npx_agent_browser
...
def test_find_agent_browser_lazy_install_cycle_terminates(monkeypatch):
    ...
    from mason_cli import dep_ensure
    ...
    monkeypatch.setattr(dep_ensure, "_has_mason_agent_browser", lambda: False)
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    with patch("mason_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("mason_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("mason_cli.dep_ensure.shutil") as mock_shutil, \
         patch("mason_constants.get_mason_home", return_value=tmp_path / "fakehome"), \
