...
The desktop self-update chain (Desktop → mason-setup --update →
``mason update`` → ``mason desktop --build-only`` → relaunch) rebuilds
Mason.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors mason_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Mason.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("mason_cli.main_desktop._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Mason.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("mason_cli.main_desktop._purge_electron_build_cache", return_value=[]), \
         patch("mason_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `mason desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Mason.exe cannot launch
    must exit nonzero (so mason-setup's retry-once kicks in) and must leave
    ...
    live_exe = desktop_dir / "release" / "win-unpacked" / "Mason.exe"
    ...
    def pack_into_staging(cmd, *args, **kwargs):
        ...
        make_pe(staging / "win-unpacked" / "Mason.exe", PE_AMD64, truncate_to=0x300)
        return subprocess.CompletedProcess(list(cmd), 0)

    with patch("mason_cli.main_desktop.shutil.which", return_value="/usr/bin/npm"), \
         patch("mason_cli.main_install_repair._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("mason_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("mason_cli.main_desktop._write_desktop_build_stamp") as mock_stamp, \
         patch("mason_cli.main_desktop._windows_native_machine", return_value="AMD64"), \
         patch("mason_cli.main_desktop.subprocess.run", side_effect=pack_into_staging), \
