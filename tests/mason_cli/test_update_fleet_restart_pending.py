...
A ``mason update`` killed after git pull advanced HEAD but before the
...
- next ``mason update`` with git already up to date still runs the
...
import mason_cli.main_web_build as main_web_build
import mason_cli.main_install_repair as main_install_repair
from mason_cli import update_cmd
import mason_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(mason_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(mason_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(mason_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(mason_main, "_is_windows", lambda: False)
    monkeypatch.setattr(main_install_repair, "_is_windows", lambda: False)
    monkeypatch.setattr(
        mason_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/NousResearch/mason-agent.git",
    )
    monkeypatch.setattr(update_cmd, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        mason_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(mason_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        mason_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(mason_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        mason_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        mason_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(mason_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(mason_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(mason_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(update_cmd, "_purge_stale_mason_modules", lambda: None)
    monkeypatch.setattr(mason_main, "_purge_stale_mason_modules", lambda: None)

    import mason_cli.gateway as mason_gateway

    monkeypatch.setattr(
        mason_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(mason_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        mason_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "mason_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_mason_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_mason_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_mason_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "mason_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(mason_main, "_purge_stale_mason_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``mason-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import mason_cli.update_inventory as ui
    ...
    import mason_cli.process_identity as pi
    ...
    latest = get_mason_home() / "logs" / "update_receipts" / "latest.json"
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(mason_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "mason gateway restart" in err
