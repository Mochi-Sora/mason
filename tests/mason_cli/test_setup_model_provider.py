...
from mason_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from mason_cli.config import load_config, save_config, save_env_value
from mason_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from mason_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("mason_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("mason_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside mason setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("mason_cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("mason_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("mason_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("mason_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    monkeypatch.setattr("mason_cli.nous_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
