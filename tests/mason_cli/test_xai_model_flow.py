...
def test_xai_model_flow_reauth_uses_standard_radio_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.auth.get_xai_oauth_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "mason_cli.setup._curses_prompt_choice",
    ...
    monkeypatch.setattr("mason_cli.auth._login_xai_oauth", _fake_login)
    monkeypatch.setattr(
...
def test_xai_model_flow_cancel_skips_reauth(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.auth._login_xai_oauth",
...
def test_auth_credentials_choice_falls_back_to_numbered_prompt(monkeypatch):
    from mason_cli import model_setup_flows_common as main_mod
