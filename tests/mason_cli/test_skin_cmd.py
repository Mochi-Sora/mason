"""`mason skin set` — deterministic single-color tweak of the active skin.
...
from mason_cli import skin_cmd
from mason_constants import get_mason_home


def _skins():
    d = get_mason_home() / "skins"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _activate(name: str) -> None:
    (get_mason_home() / "config.yaml").write_text(f"display:\n  skin: {name}\n", encoding="utf-8")
...
def test_set_forks_a_builtin_without_inventing_a_background():
    ...
    assert (get_mason_home() / "config.yaml").read_text().find("default-custom") != -1
