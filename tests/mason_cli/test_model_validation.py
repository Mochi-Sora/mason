...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("mason_cli.models.fetch_api_models", return_value=api_models), \
         patch("mason_cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("mason_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("nous") == [("m-live", "")]
        with patch("mason_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "mason_cli.models._PROVIDER_MODELS", {"nous": ["m-static"]}
        ):
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("mason_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("mason_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestNormalizeOpencodeBaseUrl:
    ...
    def test_strips_v1_for_anthropic_messages(self):
        from mason_cli.models import normalize_opencode_base_url
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("mason_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("mason_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("mason_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Mason picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("mason_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("mason_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Mason User-Agent.
    ...
    def test_probe_sends_mason_user_agent(self):
        ...
        assert ua.startswith("mason-cli/"), (
            f"User-Agent must advertise mason-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("mason-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("mason_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "mason_cli.models.provider_model_ids",
...
class TestValidateRequestedModelNousPortalRecommendations:
    ...
    hardcoded curated catalog -- even though `mason chat` already accepts
    ...
    def _validate_nous(self, model, api_models=None, portal_payload=None, portal_raises=False):
        ...
        with patch("mason_cli.models.fetch_api_models", return_value=api_models), \
             patch("mason_cli.models.probe_api_models", return_value=probe_payload), \
             patch("mason_cli.models.fetch_nous_recommended_models", side_effect=_fetch_portal), \
             patch("mason_cli.models._resolve_nous_portal_url", return_value="https://portal.nousresearch.com"), \
             patch("mason_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_non_nous_provider_does_not_consult_portal_feed(self):
        ...
        with patch("mason_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("mason_cli.models.probe_api_models", return_value=probe_payload), \
             patch("mason_cli.models.fetch_nous_recommended_models") as mock_portal, \
             patch("mason_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("mason_cli.models.fetch_api_models", return_value=api_models), \
             patch("mason_cli.models.probe_api_models", return_value=probe_payload), \
             patch("mason_cli.models._model_in_provider_catalog", return_value=True), \
             patch("mason_cli.models.fetch_nous_recommended_models") as mock_portal:
