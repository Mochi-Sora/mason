...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from mason_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_mason_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   mason gateway" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.mason/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.mason/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: mason auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """mason status should surface freshest gateway last_active (#72016)."""
    from mason_cli import status as status_mod
    import mason_cli.auth as auth_mod
    import mason_cli.gateway as gateway_mod
    import mason_state
    ...
    monkeypatch.setattr(mason_state, "SessionDB", _FakeDB)
