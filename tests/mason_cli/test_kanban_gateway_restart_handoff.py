...
def worker_setup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, kb.Task]:
    ...
    monkeypatch.setattr(kbd, "_resolve_mason_argv", lambda: ["mason"])
...
def test_managed_gateway_worker_is_spawned_in_restart_safe_scope(
    ...
    assert captured_cmd[unit_index + 1] == "mason-worker-kanban-t_candidate_restart-run-23"
    assert "MemoryMax=536870912" in captured_cmd
    separator = captured_cmd.index("--")
    assert captured_cmd[separator + 1 : separator + 4] == ["mason", "-p", "coder"]
    assert captured_cwd == str(workspace)
    assert captured_env["MASON_KANBAN_TASK"] == task.id
    assert captured_env["MASON_KANBAN_RUN_ID"] == "23"
...
def test_standalone_dispatcher_keeps_direct_worker_spawn(
    ...
    assert captured_cmd[:3] == ["mason", "-p", "coder"]
...
def test_real_user_systemd_scope_preserves_worker_context(
    ...
    script = (
        ...
        "'task': os.environ.get('MASON_KANBAN_TASK'), "
        "'run': os.environ.get('MASON_KANBAN_RUN_ID'), "
        "'cgroup': pathlib.Path('/proc/self/cgroup').read_text()})); time.sleep(0.5)"
    )
    monkeypatch.setattr(kbd, "_resolve_mason_argv", lambda: [sys.executable, "-c", script, str(receipt)])
    monkeypatch.setenv("INVOCATION_ID", "managed-gateway-test")
    ...
    assert "mason-gateway.service" not in payload["cgroup"]
