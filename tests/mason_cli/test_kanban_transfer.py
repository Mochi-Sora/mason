"""Tests for kanban board export / import (``mason_cli.kanban_transfer``).
...
from mason_cli import kanban_transfer as kt
from mason_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("MASON_KANBAN_HOME", str(root))
        for var in ("MASON_KANBAN_DB", "MASON_KANBAN_WORKSPACES_ROOT",
                    "MASON_KANBAN_ATTACHMENTS_ROOT", "MASON_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Mason"):
