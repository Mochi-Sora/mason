...
def test_detect_returns_none_when_probe_subprocess_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        mason_cli_main_install_repair, "_resolve_install_target_python", lambda *a, **k: python
    )
    monkeypatch.setattr(
        m.subprocess,
        "run",
...
def test_capture_active_tool_dependencies_uses_tools_status_probes(monkeypatch):
    from mason_cli import tools_config_post_setup
...
def test_cmd_update_captures_and_propagates_pre_rebuild_snapshot(
    ...
    from mason_cli import managed_uv, update_cmd
    ...
    monkeypatch.setattr(mason_cli_main_install_repair, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(m, "_get_origin_url", lambda *args: "https://github.com/NousResearch/mason-agent.git")
