"""Regression test: `mason dashboard --tui` must not hard-crash.

Older Mason desktop app shells (<= 0.15.x) spawn the backend as::

    mason dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Mason couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real mason_cli.main parser in a subprocess.
