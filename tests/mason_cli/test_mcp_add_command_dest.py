"""Regression test: ``mason mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``mason_cli/_parser.py``).  The dispatcher in ``mason_cli/main.py``
...
result, ``mason mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the mason parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`mason mcp add foo --url ...` must keep args.command == "mcp".
