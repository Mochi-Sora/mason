...
second Mason holder the updater's drain did not stop — survives the copy and is
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(update_cmd, "get_mason_home", lambda: root_home)
    monkeypatch.setattr(
        "mason_cli.backup._sibling_profile_homes",
