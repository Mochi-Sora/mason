...
Bug class: the checkout's ``.py`` files change (``mason update``, manual
...
def _make_pycache(repo: Path, subdir: str = "mason_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    mason_main._sweep_stale_bytecode_if_checkout_changed()
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.mason/plugins/<name>
# ---------------------------------------------------------------------------
