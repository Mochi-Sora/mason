...
When `mason update` runs INSIDE the gateway's own process tree (the
mason-auto-update cron job), waiting for that gateway to exit is a
circular wait:

  gateway waits on all in-flight work units (#77184)
    -> cron agent session waits on the `mason update` process
      -> `mason update` waits on the gateway to exit  [back to A]
...
class TestSelfRestartFireAndForget:
    ...
    def test_refuses_non_ancestor_pid(self):
        from mason_cli.gateway import _request_gateway_self_restart
...
class TestDrainOrSignalTriage:
    ...
    def test_ancestor_gateway_is_signalled_fire_and_forget(self, monkeypatch):
        ...
        from mason_cli.update_cmd import _drain_or_signal_gateway_for_update
