...
process profile, never a per-task ``set_mason_home_override``.
...
a secondary profile (via ``set_mason_home_override``) must NOT cause that
...
``get_process_mason_home()`` so the comparison reflects the launch scope,
...
def test_process_mason_home_ignores_task_override(tmp_path, monkeypatch):
    ...
    token = set_mason_home_override(str(other_home))
    try:
        assert (
            env_loader._process_mason_home().resolve() == launch_home.resolve()
