...
When ``MASON_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.mason/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.mason            (has its own auth.json fixture)
    * Profile     -> tmp_path/.mason/profiles/coder   (active, MASON_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Mason is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (MASON_HOME != ~/.mason, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    mason_home = tmp_path / "classic"
    mason_home.mkdir()
