...
from mason_cli import config as mason_config
...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_mason_modules`` must also be stubbed: it evicts
    ``mason_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from mason_cli.gateway import ...`` then loads an
    ...
    with patch("mason_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("mason_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("mason_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(mason_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(mason_main, "_purge_stale_mason_modules", lambda *a, **kw: None), \
         patch("mason_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(mason_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(mason_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(mason_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(mason_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(mason_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(mason_config, "check_config_version", lambda **_kwargs: (5, 5))
    monkeypatch.setattr(mason_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(mason_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(mason_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "mason_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    mason_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_mason_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import mason_constants

    monkeypatch.delattr(mason_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(mason_constants, "apply_subprocess_home_env")

    mason_main._reload_updated_runtime_modules()

    assert callable(mason_constants.apply_subprocess_home_env)
...
def _make_update_side_effect(
    ...
    ``refs/mason-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mason_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        mason_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(mason_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        mason_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/mason-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/mason-update-backups/orphan-main-")
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    stale_refs = [
        "refs/mason-update-backups/orphan-main-"
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/mason-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(mason_main.subprocess, "run", side_effect=fake_run):
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        mason_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        mason_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "mason_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    mason_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    mason_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``mason update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(mason_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".mason-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by mason update (mason_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "mason-update-autostash")

    assert marker.exists(), (
        ".mason-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".mason-bootstrap-complete" not in status
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    import subprocess
    from mason_cli import update_cmd
    import mason_cli.update_cmd_deps as update_cmd_deps
    ...
    assert "made the Mason agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert mason_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_restore_stays_parked_when_untracked_baseline_is_unknown(
    ...
    from mason_cli import update_cmd
    import mason_cli.update_cmd_stash as update_cmd_stash
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/mason-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    remaining = git("for-each-ref", "refs/mason-update-backups/").stdout
