"""Tests for the stale-dashboard handling run at the end of ``mason update``.

``mason update`` detects ``mason dashboard`` processes left over from the
...
from mason_cli.main_dashboard import _find_stale_dashboard_pids
from mason_cli.dashboard_procs import _kill_stale_dashboard_processes
from mason_cli import dashboard_procs
from mason_cli import main_dashboard
from mason_cli import update_cmd
from mason_cli.update_cmd import _finish_dashboard_update_cleanup
from mason_cli.main_dashboard import _restart_managed_dashboard_service
from mason_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "masonPath": "/opt/mason/bin/mason",
        "masonHome": str(tmp_path),
    ...
    monkeypatch.delenv("MASON_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "mason_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m mason_cli.main dashboard"),
                    _ps_line(12345, "mason dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("mason_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "mason-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="mason-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "mason-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "mason-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "mason-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("mason_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "mason-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "mason-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "mason-dashboard.service"],
            ["systemctl", "--user", "restart", "mason-dashboard.service"],
        ]
        ...
        assert "✓ restarted mason-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("mason_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("mason_cli._subprocess_compat.pid_is_mason", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "mason_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `mason update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("mason_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m mason_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("mason_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/mason-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="mason-serve.service"), \
        ...
        restart.assert_called_once_with(
            "mason-serve.service", "/system.slice/mason-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service mason-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: mason update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"mason-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["mason", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("mason_cli.dashboard_procs._mason_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["mason", "dashboard", "--port", "8300"],
                ["mason", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["mason", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["mason", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["mason", "serve"]])

        assert failed == [["mason", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.mason/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["mason", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "mason_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "mason_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["mason", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["mason", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["mason", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_mason_home(self):
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/mason-home-a"
        a = ["mason", "dashboard", "--port", "8300"]
        b = ["mason", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.mason/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["mason", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["mason", "dashboard", "--port", "8301"]
        home = "/home/u/.mason"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another MASON_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.mason"),
                (2, b, "/work/project/.mason"),
            ],
            own_home="/home/u/.mason",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.mason-sidecar/mason-agent/venv/bin/python",
            "-m", "mason_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.mason-lifeos")],
            own_home="/Users/u/.mason",
        )
        assert out == []

    def test_matching_mason_home_is_kept(self):
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["mason", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.mason")],
            own_home="/Users/u/.mason",
        )
        assert out == [argv]

    def test_symlinked_mason_home_compares_equal(self, tmp_path):
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable MASON_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from mason_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_mason_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mason_constants, "get_mason_home", lambda: Path("/home/u/.mason")
        )
        argv = ["mason", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.mason-lifeos"),
        ])
        assert foreign == []
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["mason", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00mason_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "mason_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``mason update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``mason_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from mason_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("mason_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import mason_cli._subprocess_compat as compat
        from mason_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["mason_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["mason_cli._subprocess_compat"])
            importlib.reload(sys.modules["mason_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "mason_cli._subprocess_compat" in reloaded
        assert "mason_cli.dashboard_procs" in reloaded
