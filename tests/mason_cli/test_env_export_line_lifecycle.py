...
``~/.mason/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(mason_home):
...
def test_remove_export_prefixed_token(mason_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(mason_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
    ...
    env_text = mason_home.joinpath(".env").read_text(encoding="utf-8")
...
def test_update_export_prefixed_token_does_not_duplicate(mason_home):
...
def test_plain_line_save_and_remove_still_work(mason_home):
    """Sanity: the ordinary KEY= path is unchanged."""
    from mason_cli.config import load_env, remove_env_value, save_env_value
...
def test_export_line_with_comment_untouched(mason_home):
...
def test_remove_token_written_with_spaces_around_equals(mason_home):
    ...
    """
    _write_env_raw(mason_home, f"GITHUB_TOKEN = {OLD_PAT}\n")
...
def test_spaced_token_rotate_then_delete_does_not_resurrect(mason_home):
...
def test_writer_matches_exactly_what_load_env_accepts(mason_home):
    """Parity: the writers must recognise a line iff ``load_env()`` does."""
    from mason_cli.config import _env_line_defines_key, load_env
    ...
    for line, expected in [
        ...
        _write_env_raw(mason_home, line + "\n")
