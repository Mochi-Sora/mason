"""Regression tests for Desktop-owned ``mason serve`` lifecycle tracking."""

from mason_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited MASON_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from mason_cli import web_server_lifecycle

    monkeypatch.setenv("MASON_PARENT_PID", "4242")
    monkeypatch.setenv("MASON_PARENT_START_MARKER", "")
    monkeypatch.setenv("MASON_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("MASON_PARENT_PID", "4242")
    monkeypatch.setenv("MASON_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("MASON_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
        web_server_lifecycle.threading, "Thread", lambda *a, **k: started.append(1) or _NoThread()
    )
    with caplog.at_level(logging.WARNING, logger="mason_cli.web_server"):
        web_server_lifecycle._start_parent_death_watchdog()
