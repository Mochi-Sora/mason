...
def test_env_var_names_the_managing_system(mason_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("MASON_MANAGED", raising=False)
    else:
        monkeypatch.setenv("MASON_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    mason_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the MASON_MANAGED of the service."""
    (mason_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("MASON_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(mason_home, monkeypatch):
    (mason_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("MASON_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    mason_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("MASON_MANAGED", managed_value)
    ...
    # `mason update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "mason update"
...
def test_unmanaged_install_offers_no_update_command(mason_home, monkeypatch):
    monkeypatch.delenv("MASON_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(mason_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = mason_home / ".managed"
