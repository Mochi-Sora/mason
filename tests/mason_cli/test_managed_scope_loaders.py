...
mason_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # mason_time checks an env override first; ensure it's unset so config wins.
    ...
    assert mason_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (MASON_TIMEZONE, MASON_REDACT_SECRETS, MASON_MAX_ITERATIONS,
    ...
    # writing MASON_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
