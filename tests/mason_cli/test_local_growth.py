...
def test_overrides_roundtrip_and_clear(mason_home):
    from mason_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(mason_home):
    from mason_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(mason_home):
    ...
    from mason_cli.local_runtime.growth import maybe_grow_window
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from mason_cli.local_runtime.context_policy import growth_decision
    from mason_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Mason' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from mason_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(mason_home, tmp_path, monkeypatch):
    ...
    import mason_cli.local_runtime.presets as presets_mod

    from mason_cli.local_runtime.catalog import CATALOG
    from mason_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(mason_home, tmp_path, monkeypatch):
    ...
    from mason_cli.local_runtime.estimator import HardwareBudget
    from mason_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(mason_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(mason_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import mason_cli.local_runtime.presets as presets_mod

    from mason_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from mason_cli.local_runtime.growth import save_window_override
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(mason_home, tmp_path, monkeypatch):
