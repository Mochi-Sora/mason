...
Covers:
- Defense 1: gateway stop/restart refuse when _MASON_GATEWAY=1
...
from mason_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "mason gateway restart",
        "mason gateway stop",
        "mason gateway uninstall",
        "mason  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "MASON GATEWAY RESTART",           # uppercase
    ])
    def test_mason_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.mason.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.mason/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l mason-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.mason.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.mason.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.mason.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.mason/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.mason.gateway-apollo:/a.plist' "
            "'ai.mason.gateway-cronus:/c.plist' 'ai.mason.gateway-plutus:/p.plist' "
            "'ai.mason.gateway:/Users/botuser/Library/LaunchAgents/ai.mason.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated mason note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.mason.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.mason.gateway',
        "launchctl kick'start' -k gui/501/ai.mason.gateway",
        "launchctl kick\\start -k gui/501/ai.mason.gateway",
        'launchctl "kickstart" -k gui/501/ai.mason.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.mason.gateway',
        "launchctl dis\\able gui/501/ai.mason.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.mason."gateway"',
        # Same class on the systemctl and mason-CLI branches.
        'systemctl re"start" mason-gateway',
        'mason gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.mason.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.mason.update-checker',
        'systemctl re"start" mason-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "mason cron list",
        "mason update",
        "mason config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `mason gateway start` is benign — starting a gateway from inside a
        ...
        "mason gateway start",
        "mason gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway mason
        # services must not be falsely blocked (the old `.*mason` matched any
        # mason token).
        "launchctl unload ai.mason.update-checker.plist",
        "launchctl restart ai.mason.daemon",
        ...
        "systemctl restart mason-meta.service",
        "systemctl restart mason-cron-helper",
        ...
        "echo after the mason gateway restarted cleanly",
        "the mason gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "mason skill view gateway-notes && echo mason gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `mason` is a path component
        # there, not a command.
        "cat '/docs/mason gateway restart-notes.md'",
        "less /home/user/notes/mason gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "mason gateway restart",
        "mason gateway restart; echo done",
        "mason gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;mason gateway restart",
        "true && mason gateway stop",
        "echo $(mason gateway restart)",
        "echo `mason gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.mason.gateway"])',
        'subprocess.run(["mason", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.mason.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["mason", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: mason gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `mason -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry MASON_HOME/MASON_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("MASON_PROFILE", "zeus")
        monkeypatch.delenv("MASON_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "mason -p zeus gateway stop",
        "mason -p zeus gateway restart",
        "mason --profile zeus gateway restart",
        "mason --profile zeus gateway stop",
        "mason --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "mason -v -p zeus gateway restart",
        "mason -p zeus -v gateway restart",
        "mason --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "mason -p venus gateway stop",
        "mason -p venus gateway restart",
        "mason --profile venus gateway restart",
        "mason --profile=venus gateway stop",
        "mason -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "mason -p zeus gateway start",
        "mason -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("mason gateway restart")
        assert _contains_gateway_lifecycle_command("mason gateway stop")

    def test_mason_home_derived_profile(self, monkeypatch):
        # Without MASON_PROFILE the guard falls back to the MASON_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("MASON_PROFILE", raising=False)
        monkeypatch.delenv("MASON_PROFILE_NAME", raising=False)
        import mason_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("mason -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("mason -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("mason -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("mason gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_mason_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade mason then run mason gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_mason_gateway_once.sh). The script must live under
        # MASON_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("MASON_HOME", str(tmp_path / ".mason"))
        scripts_dir = tmp_path / ".mason" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_MASON_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _MASON_GATEWAY=1.

    Issue #37453: systemctl --user restart mason-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart mason-gateway",
        "systemctl --user restart mason-gateway",
        "systemctl stop mason-gateway.service",
        "mason gateway restart",
        "mason gateway uninstall",
        "launchctl kickstart gui/501/ai.mason.gateway",
        "launchctl bootout gui/501/ai.mason.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.mason.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.mason/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.mason.gateway.restart-once.plist",
        "pkill -f mason.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart mason-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.mason.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-mason label: label-independent detection is the point
        # (#62891 second reproduction used `ai.mason.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.mason.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _MASON_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _MASON_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_MASON_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="mason gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["mason gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.mason.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.mason.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-mason systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run mason gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("mason gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfemason gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under MASON_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("mason gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/mason\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 mason gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_mason_home()` falls back to Path.home(), which raises
        RuntimeError when neither MASON_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart mason-gateway' .",
        "grep -rn 'mason gateway restart' ./logs",
        "rg 'mason gateway restart' ../archive",
        "grep -c 'systemctl stop mason-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM mason_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell mason gateway restart"',
        'sqlite3 ./db ".system systemctl restart mason-gateway"',
        'psql ./x -c "\\! systemctl restart mason-gateway"',
        "grep -r 'mason gateway restart' . | sh",
        "grep -r 'mason gateway restart' ./logs | bash",
        "grep -r 'mason gateway restart' . | sudo sh",
        "grep -r 'x' . ; mason gateway restart",
        "grep -r 'x' . && systemctl restart mason-gateway",
        'grep -r "$(mason gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart mason-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell mason gateway restart"',
        'sqlite3 db ".system systemctl restart mason-gateway"',
        'psql -c "\\! systemctl restart mason-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    mason_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run mason gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(tmp_path / ".mason"))
        (tmp_path / ".mason").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run mason gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop mason-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart mason-gateway' /var/log/syslog",
        "rg 'mason gateway restart' /home/user/.mason/logs/",
        "journalctl -u mason-gateway --grep 'systemctl restart mason-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'mason_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell mason gateway restart"',
        'psql -c "\\! systemctl restart mason-gateway"',
        "grep 'systemctl restart mason-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart mason-gateway",
        'grep "$(systemctl restart mason-gateway)" f',
        "grep 'restart' log; systemctl restart mason-gateway",
        'sqlite3 db "SELECT 1"; mason gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "mason gateway restart",
        "sudo systemctl stop mason-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart mason-gateway%'\""
        )
