...
def test_presets_stale_when_a_staged_model_has_no_section(mason_home):
    from mason_cli.local_runtime.bootstrap import _presets_stale

    _stage(mason_home, "model-a")
    _stage(mason_home, "model-b")
    _write_presets(mason_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(mason_home):
    from mason_cli.local_runtime.bootstrap import _presets_stale
...
def test_no_models_is_never_stale(mason_home):
    from mason_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(mason_home, monkeypatch):
    ...
    import mason_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "mason_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "mason_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(mason_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(mason_home, monkeypatch):
    import mason_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "mason_cli.local_runtime.endpoint._state_endpoint", lambda: None)
