...
and the ``mason kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MASON_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `mason kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `mason kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated MASON_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".mason"
    home.mkdir()
