"""Tests for mason_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("mason_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from mason_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("mason_cli.config.load_config")
    ...
    @patch("mason_cli.config.load_config")
    @patch.dict("os.environ", {"MASON_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """MASON_SESSION_PLATFORM should be used when MASON_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_mason_platform_takes_precedence(self, tmp_path, monkeypatch):
        """MASON_PLATFORM should win over MASON_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from mason_cli.skills_config import _get_categories
        skills = [
