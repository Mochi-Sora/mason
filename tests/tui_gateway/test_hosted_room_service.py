...
def test_same_thread_followup_migrates_and_delivers_committed_peer_reply(
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@mason continue", "thread_id": "thread-1"},
    )
    ...
    assert "User (user): @mason continue" in prompt
...
def test_active_same_thread_followup_waits_for_current_task(tmp_path: Path):
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@mason follow up", "thread_id": "thread-1"},
    )
    ...
    assert service.stop(timeout=1.0)
    assert "User (user): @mason follow up" in service.rpc.prompts[1][1]
...
def test_service_derives_room_deadline_from_agent_timeout(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("MASON_AGENT_TIMEOUT", "90")
