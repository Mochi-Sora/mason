...
Behavior contracts, exercised against a real temp MASON_HOME (no mocks on the
...
def watcher_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_mason_home", str(tmp_path))
...
def test_served_profile_store_move_broadcasts_sessions_changed(watcher_home, monkeypatch):
    ...
    monkeypatch.setattr("mason_cli.profiles.get_profile_dir", lambda name: home / "profiles" / name)
