...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="mason/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is MASON_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.mason": ("/home/me/.mason", "/home/me/.mason"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.mason", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.mason"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.mason")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.mason"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/mason-agent": ("/www/mason-agent", "/www/mason-agent")})
    sessions = [
        _session("/www/mason-agent", branch="main"),
        _session("/www/mason-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/mason-agent"]
    assert _lane_ids(project) == ["/www/mason-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/mason-agent"
