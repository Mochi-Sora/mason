...
def mason_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MASON_HOME.
...
@pytest.fixture()
def server(mason_home, monkeypatch):
    ...
    # Pin config resolution to the isolated MASON_HOME. Sibling test
    ...
    # module-level ``_mason_home = get_mason_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_mason_home", mason_home)
...
def test_moa_bare_returns_usage(server, session, mason_home):
    _write_moa_config(mason_home, """
moa:
