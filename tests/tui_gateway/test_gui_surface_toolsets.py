...
locally, one reached over SSH, one behind a plain URL+token, or Mason Cloud —
and only the first two run with ``MASON_DESKTOP=1`` in their environment.
...
model "You are chatting inside the Mason desktop app".
...
class TestDesktopUiToolset:
    ...
    def test_stays_off_the_core_tool_list(self):
        ...
        assert GUI_TOOLS.isdisjoint(_MASON_CORE_TOOLS)
...
class TestSurfaceResolution:
    ...
    def test_desktop_env_alone_does_not_grant_them(self, no_desktop_env):
        ...
        The embedded terminal pane runs `mason --tui` against this same
        backend; env-keyed gating handed it GUI tools it cannot answer.
        """
        no_desktop_env.setenv("MASON_DESKTOP", "1")
        assert "desktop_ui" not in server._gui_surface_toolsets("tui")
...
class TestResolverPlumbing:
    ...
    def test_explicit_env_pin_still_wins(self, no_desktop_env):
        """MASON_TUI_TOOLSETS is an operator override; surface can't re-add."""
        no_desktop_env.setenv("MASON_TUI_TOOLSETS", "web,memory")
