    ...
    with patch.dict(
        "sys.modules",
        {
            "mason_constants": MagicMock(
                get_mason_home=MagicMock(return_value="/tmp/mason_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'mason-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'mason-release' patched"
    }
