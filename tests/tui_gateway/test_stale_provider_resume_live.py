...
def live_home(monkeypatch):
    """A REAL isolated MASON_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="mason-live-staleprov-"))
    home = tmp / ".mason"
    ...
    # mason_constants caches the resolved home at first read — the env var
    ...
    import mason_cli.config as hconfig
    ...
    # conftest also re-pins mason_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(mason_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_mason_home", str(home), raising=False)
    yield home, server
