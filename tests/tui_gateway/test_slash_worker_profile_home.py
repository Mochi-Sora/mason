...
def test_slash_worker_accepts_profile_home():
    ...
    # mason_state evaluates get_mason_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "mason_constants": MagicMock(
            get_mason_home=MagicMock(return_value=Path("/tmp/mason_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.mason/profiles/work"
            )

            # Verify Popen was called
            assert mock_popen.called

            # Check that MASON_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["MASON_HOME"] == "/home/luke/.mason/profiles/work"
