...
guard is covered in ``tests/mason_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``mason_cli.model_switch_providers.prewarm_picker_cache_async``
...
import mason_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from mason_cli.model_switch,
