"""Tests for state.db health/stats collection (mason doctor section).

Covers:
- ``mason_state_dbfile.collect_state_db_stats``: read-only, best-effort stats
  (page_count, freelist, WAL size, journal mode, row counts, FTS presence,
  pending v23 FTS-rebuild bookkeeping).
- ``mason_state_dbfile.count_db_holders``: /proc-based best-effort probe for how
  many processes hold the DB file open (Linux only; None elsewhere/on error).
- ``mason_cli.doctor_state._render_state_db_stats``: formatting/threshold helper
  the doctor state.db section prints from.
"""

import mason_state_dbfile
import json
import os
import sqlite3
import sys
...
def test_collect_and_render_stale_fts_holder_deferral(populated_db):
    ...
    from mason_cli.doctor_state import _render_state_db_stats
...
def test_render_warns_on_large_db():
    from mason_cli.doctor_state import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
