...
def test_env_var_enabled_uses_shared_truthy_rules(monkeypatch):
    monkeypatch.setenv("MASON_TEST_BOOL", "YeS")
    assert env_var_enabled("MASON_TEST_BOOL") is True

    monkeypatch.setenv("MASON_TEST_BOOL", "no")
    assert env_var_enabled("MASON_TEST_BOOL") is False
