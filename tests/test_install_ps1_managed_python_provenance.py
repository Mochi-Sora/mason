"""Behavioral regression for Mason-managed Python provenance on Windows."""
...
def test_venv_stage_rejects_third_party_python_and_uses_managed_path(
    ...
    run = _run_venv_stage(powershell, tmp_path, mason_home, install_dir, env)
...
def test_fallback_minor_is_reported_from_resolved_managed_interpreter(
    ...
    install_dir = tmp_path / "install"
    managed_python = (
        install_dir / ".mason-runtime" / "python" / "cpython-3.12" / "python.exe"
    )
    uv = mason_home / "bin" / "uv.exe"
    uv.parent.mkdir(parents=True)
    managed_python.parent.mkdir(parents=True)
...
def test_python_find_drains_large_stderr_without_deadlock(tmp_path: Path) -> None:
    ...
    install_dir = tmp_path / "install"
    managed_python = (
        install_dir / ".mason-runtime" / "python" / "cpython-3.11" / "python.exe"
    )
    uv = mason_home / "bin" / "uv.exe"
    uv.parent.mkdir(parents=True)
    managed_python.parent.mkdir(parents=True)
