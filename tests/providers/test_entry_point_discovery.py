"""Tests for pip entry-point provider discovery (mason_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``mason_agent.plugins`` entry point, supporting both a
...
class _FakeEP:
    def __init__(self, name, loader):
        self.name = name
        self.group = "mason_agent.plugins"
        self._loader = loader
