"""A provider installed by ``mason plugins install`` must actually be found.

The installer clones into ``$MASON_HOME/plugins/<name>/`` (flat), provider
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules):
        if mod.startswith(("plugins.model_providers", "_mason_user_provider")):
            del sys.modules[mod]
...
def test_flat_installed_model_provider_plugins_are_discovered_alongside_nested_ones(mason_home):
    _write_plugin(mason_home / "plugins" / "installed-acp", name="installed-acp",
                  manifest='name: installed-acp\nkind: "model-provider"\n')
    _write_plugin(mason_home / "plugins" / "model-providers" / "nested-acp", name="nested-acp",
...
def test_other_plugins_in_the_flat_directory_are_left_to_the_plugin_manager(mason_home):
    _write_plugin(mason_home / "plugins" / "other-standalone", name="other-standalone",
                  manifest="name: other-standalone\nkind: standalone\n")
    _write_plugin(mason_home / "plugins" / "manifestless", name="manifestless", manifest=None)
    _write_plugin(mason_home / "plugins" / "broken-manifest", name="broken-manifest",
