...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.mason/mason-agent/utils.py)
...
2. ``mason update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``mason_cli/main.py`` (the ``mason update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``mason_constants`` after the
