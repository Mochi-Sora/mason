...
#
# Installs a real, earlier Mason the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `mason`.
#
...
# Keep sandbox state out of the default .mason-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".mason-sandbox-e2e-$ROUTE"
export MASON_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/mason/.mason/mason-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/mason-agent.git"
...
LOG_DIR="${MASON_E2E_LOG_DIR:-$(mktemp -d -t mason-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED mason accept FLAG on `mason update`?
#
...
# that silently rots. `mason update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "mason update --help 2>&1" | grep -qF -- "$flag"
}
...
require_mason_works() {
  local when="$1" out
  out="$(in_sandbox "mason --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "mason --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "mason runs $when"
}

# ── install the earlier Mason ─────────────────────────────────────────────
...
require_mason_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: mason update'
    ...
    # mason which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="mason update --yes"
    else
      update_cmd="mason update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "mason update failed ($update_cmd)"
    fi
    require_landed_on_target 'mason update'
    require_mason_works 'after mason update'
    ;;
  installer)
    ...
    require_mason_works 'after installer re-run'
    ;;
esac
