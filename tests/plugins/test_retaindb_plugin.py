...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure MASON_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", mason_home=str(tmp_path / ".mason"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_HOME", str(mason_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", mason_home=str(mason_home))
        return p
