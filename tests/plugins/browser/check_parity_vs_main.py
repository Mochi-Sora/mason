...
Run from the PR worktree:
    ...
    cd ~/.mason/mason-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.mason/mason-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.mason/mason-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a mason-agent checkout"
)
...
# Isolated MASON_HOME for the config write.
