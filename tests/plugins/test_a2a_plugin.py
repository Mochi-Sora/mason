...
class TestAgentCardV1:
    def test_card_shape(self):
        card = protocol.build_agent_card(
            name="mason-test", url="http://localhost:9900/",
            description="test", skills=[], streaming=False, auth_required=False,
        )
        assert card["name"] == "mason-test"
...
class TestV1SpecRegressionFixes:
    ...
    def test_forward_to_profile_first_contact_creates_then_resumes_fake_mason(self, monkeypatch, tmp_path):
        from plugins.platforms.a2a.adapter import A2AAdapter
        from gateway.config import PlatformConfig
        ...
        mason = fakebin / "mason"
        mason.write_text("""#!/usr/bin/env python3
import json, os, sqlite3, sys, time
calls = os.environ['FAKE_MASON_CALLS']
with open(calls, 'a') as f:
    f.write(json.dumps(sys.argv[1:]) + '\\n')
home = os.environ['MASON_HOME']
...
""")
        ...
        monkeypatch.setenv("FAKE_MASON_CALLS", str(calls))
...
class TestMultiplexConstructionScope:
    ...
    def test_secondary_profile_never_borrows_default_profile_env(
        ...
        assert adapter._agents[""]["description"] == (
            "Mason Agent — a general-purpose agent reachable over A2A."
        )
