...
class TestFireworksHeaders:
    def test_attribution_matches_canonical_mason_values(self, fireworks_profile):
        """Fireworks requests carry the same attribution identity Mason sends
        everywhere else.
    ...
    def test_user_agent_identifies_mason(self, fireworks_profile):
        # Prefix, not the full string — the version moves every release.
        assert fireworks_profile.default_headers["User-Agent"].startswith("MasonAgent/")
