...
def _inject_fake_mason_cli(monkeypatch):
    """Inject fake mason_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("mason_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("mason_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_mason_cli = types.ModuleType("mason_cli")
    fake_mason_cli.config = fake_config_mod
    fake_mason_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "mason_cli", fake_mason_cli)
    monkeypatch.setitem(sys.modules, "mason_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "mason_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["mason", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_mason_home", lambda: tmp_path)
        _inject_fake_mason_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "mason-user"
