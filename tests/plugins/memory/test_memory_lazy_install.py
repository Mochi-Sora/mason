...
(``MASON_DISABLE_LAZY_INSTALLS=1``) and lazy installs are redirected to a
writable durable target (``MASON_LAZY_INSTALL_TARGET``). honcho/hindsight
...
class TestSupermemoryEnsureCalled:
    def test_client_construction_calls_ensure(self, monkeypatch):
        ...
        _SupermemoryClient(api_key="k", timeout=5.0, container_tag="mason")
...
#
# This is the exact hosted-Fly condition: MASON_DISABLE_LAZY_INSTALLS=1 seals
# the venv, but MASON_LAZY_INSTALL_TARGET redirects installs to a writable
...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        monkeypatch.setenv("MASON_LAZY_INSTALL_TARGET", str(tmp_path / "lazy"))
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("MASON_LAZY_INSTALL_TARGET")
    ...
    def test_sealed_venv_without_target_blocks(self, feature, monkeypatch):
        ...
        monkeypatch.setenv("MASON_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv("MASON_LAZY_INSTALL_TARGET", raising=False)
        monkeypatch.setattr(
            "mason_cli.config.load_config",
            lambda: {"security": {"allow_lazy_installs": True}},
        )
