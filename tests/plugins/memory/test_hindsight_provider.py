...
from mason_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_mason_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", mason_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", mason_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:mason-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:mason-agent",
    ]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Mason Agent and the User"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        mason_home = tmp_path / "mason-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_mason_home", lambda: mason_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("mason_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("mason_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(mason_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (mason_home / ".env").exists()
        assert not (mason_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "mason.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("mason_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(mason_home), {"memory": {}})
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "mason.env"
...
class TestToolHandlers:
    ...
    def test_retain_defaults_item_timestamp_when_no_occurred_at(self, provider, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight._mason_now", lambda: event_time)
        result = json.loads(provider.handle_tool_call(
            "hindsight_retain", {"content": "user likes dark mode"}
        ))
        assert result["result"] == "Memory stored successfully."
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config, monkeypatch):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Mason Agent and the User"
        ...
        assert item["metadata"]["source"] == "mason"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_mason_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", mason_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", mason_home=str(tmp_path), platform="cli")
...
class TestRetainIndicator:
    ...
    def test_status_callback_wired_from_initialize(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s", mason_home=str(tmp_path), status_callback=cb)
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("mason") == "mason"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "mason-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="mason",
        )
        assert p._bank_id == "mason-coder"
        assert p._bank_id_template == "mason-{profile}"
...
class TestAvailability:
    def test_available_with_api_key(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_mason_home",
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mason_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("mason_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", mason_home=str(tmp_path), platform="cli")
        return calls
...
class TestMultiplexBackgroundScope:
    ...
    def scoped_embedded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_mason_home", lambda: home)
        home_tok = set_mason_home_override(str(home))
        scope_tok = set_secret_scope(build_profile_secret_scope(home))
        yield created, home
        set_multiplex_active(False)
        reset_secret_scope(scope_tok)
        reset_mason_home_override(home_tok)

    def test_writer_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p._mode = "local_embedded"
        p._config = {"profile": "mason", "llm_provider": "openai", "llm_model": "m"}
        p._ensure_writer()
    ...
    def test_daemon_start_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p.initialize(session_id="s1", mason_home=str(home), platform="cli")
