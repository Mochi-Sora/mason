...
@pytest.mark.parametrize("peer", ["", "mason", "work-assistant"])
...
def test_new_setup_does_not_ask_for_or_save_peer(
    ...
    saved_config = ov._load_mason_openviking_config()
...
def test_mason_only_save_uses_the_same_clean_values_in_file_and_process(
    tmp_path, peer
):
    from dotenv import dotenv_values

    env_path = tmp_path / ".env"
    ov._setup._save_mason_only_config(
        config={"memory": {}},
        provider_config={},
        env_path=env_path,
        values={
...
def test_mason_only_save_failure_leaves_process_environment_unchanged(
...
def test_wire_requests_keep_writes_and_session_messages_in_the_selected_scope(
    ...
    provider = ov.OpenVikingMemoryProvider()
    try:
        provider.initialize("peer-test", mason_home=str(home))
    ...
    remember_messages = [
        ...
        if path.startswith("/api/v1/sessions/mason-remember-")
