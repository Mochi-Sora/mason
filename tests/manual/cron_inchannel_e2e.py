...
Uses a throwaway MASON_HOME so it never touches ~/.mason. No real names.
"""
...
def _fresh_home():
    """Point MASON_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_mason_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["MASON_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER MASON_HOME is set.
...
# a different get_mason_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"MASON_HOME (throwaway): {HOME}")
