...
``Invoke-MasonStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``mason update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``mason_cli/main.py``), so the
...
closes and ``Invoke-MasonStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.mason-update-result.json`` is never written, ``.mason-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Mason" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``mason update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``mason update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``mason_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``MASON_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-MasonStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``mason update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "MASON_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "MASON_UPDATE_STEP_IDLE_SECONDS": "3",
        "MASON_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Mason' forever) or metered a chatty step (backpressure on the running "
