"""mason_cli.plugin_compat: detect plugins on old import paths, tell the user, disable after the date.
...
from mason_cli import plugin_compat as pc
...
MANIFEST = {"tools.web_tools": {"prefers_gateway": "tools.tool_backend_helpers.prefers_gateway"},
            "mason_cli.kanban_db": {"connect": "mason_cli.kanban_db_connect.connect"}}
...
@pytest.mark.parametrize("src, expect", [
    ...
    ("from unittest.mock import patch\npatch('mason_cli.kanban_db.connect')\n", ["mason_cli.kanban_db.connect"]),
...
def test_scan_plugin_walks_dir_and_skips_tests(tmp_path):
    ...
    (tmp_path / "sub").mkdir(); (tmp_path / "sub" / "m.py").write_text("import mason_cli.kanban_db as k\nk.connect()\n")
...
def test_disable_only_after_the_date_and_not_when_allowed(tmp_path, monkeypatch):
    ...
    assert reason and pc.COMPAT_REMOVAL in reason and "mason plugins compat" in reason
...
def test_summary_lines_name_plugins_and_the_date():
    ...
    assert "mason plugins compat" in tail
...
def test_loader_skips_hitting_plugin_after_date(tmp_path, monkeypatch):
    ...
    from mason_cli.plugins_manifest import PluginManifest
