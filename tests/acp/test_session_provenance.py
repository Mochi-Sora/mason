...
def test_root_session_no_compression(db):
    ...
    assert prov["currentMasonSessionId"] == "root1"
    assert prov["rootMasonSessionId"] == "root1"
    assert prov["parentMasonSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_mason_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentMasonSessionId"] == "old"
    assert prov["rootMasonSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousMasonSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootMasonSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"mason"}
    assert "sessionProvenance" in meta["mason"]
    assert meta["mason"]["sessionProvenance"]["currentMasonSessionId"] == "root1"
