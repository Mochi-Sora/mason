...
package-lock.json. Without MASON_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV MASON_TUI_DIR=/opt/mason/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the mason user, return stdout."""
    inner = (
        "source /opt/mason/.venv/bin/activate && "
        "cd /opt/mason && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the mason user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "mason", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_mason_tui_dir_env_is_set(built_image: str) -> None:
    """MASON_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$MASON_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/mason/ui-tui", (
        f"MASON_TUI_DIR={r.stdout.strip()!r} (expected /opt/mason/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from mason_cli.main_tui_launch import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/mason/ui-tui')\n"
    ...
    # With MASON_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
