...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the mason user UID/GID at boot
  2. MASON_UID/MASON_GID take precedence over PUID/PGID aliases
...
def test_puid_pgid_remaps_mason_user(
    built_image: str, container_name: str,
) -> None:
    """PUID=1000 PGID=1000 must remap the mason user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected mason UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected mason GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    r = docker_exec_sh(container_name, "id -u mason", timeout=10)
    assert r.stdout.strip() == "99", (
        f"expected mason UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g mason", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected mason GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the mason user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
    ...
    assert "WRITE_OK" in r.stdout, (
        f"mason user cannot write to /opt/data after remap: {r.stdout}"
    )