...
Build the real image and verify at runtime:
  ...
  1. /opt/mason is not writable by the mason user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and MASON_DISABLE_LAZY_INSTALLS are set
  3. /opt/mason/.install_method contains "docker" (code-scoped stamp)
  4. $MASON_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $MASON_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_mason(
    built_image: str, container_name: str,
) -> None:
    """The mason user must not be able to modify /opt/mason.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/mason as the mason user
        "touch /opt/mason/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"mason user can write to /opt/mason (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/mason/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"mason user can write to /opt/mason/.venv: {r.stdout}"
    )


def test_mason_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    MASON_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$MASON_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"MASON_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $MASON_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $MASON_HOME was not healed on boot: "
        f"{r.stdout}"
    )
