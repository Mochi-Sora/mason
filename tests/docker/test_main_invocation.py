...
def test_no_args_starts_mason(built_image: str) -> None:
    """``docker run <image>`` should start mason cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``mason chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> mason batch ...`` returns a
