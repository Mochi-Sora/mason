...
def test_dashboard_service_resets_home(
    ...
    container with MASON_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    MASON_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "MASON_DASHBOARD=1", "MASON_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to mason:mason on every boot.
    ...
    # Verify files are now owned by mason.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "mason" in r.stdout, (
        f"expected mason-owned files after restart, got: {r.stdout!r} — "
