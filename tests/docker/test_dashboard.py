"""Harness: dashboard opt-in via MASON_DASHBOARD.

Today (tini): dashboard starts once when MASON_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``mason`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without MASON_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "mason dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without MASON_DASHBOARD"
    )
...
# whenever `MASON_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``MASON_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "MASON_DASHBOARD=1",
        "MASON_DASHBOARD_HOST=0.0.0.0",
        "MASON_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"MASON_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and MASON_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``MASON_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
