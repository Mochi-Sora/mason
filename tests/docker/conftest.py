...
Override the image with ``MASON_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("MASON_TEST_IMAGE", "mason-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``MASON_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("MASON_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged mason user
# ---------------------------------------------------------------------------
#
# Background: every Mason runtime path inside the container drops to UID
# 10000 (the ``mason`` user) via ``s6-setuidgid mason``. ``docker exec``
...
# fails (PermissionError swallowed) for mason, so a test that ran as root
...
# helpers below run every probe under ``-u mason`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: mason).
