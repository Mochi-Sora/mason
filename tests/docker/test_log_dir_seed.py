"""Runtime smoke test for Docker $MASON_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the mason user after container boot.
...
registered later runs its log service as the dropped mason user and
...
def test_logs_gateways_seeded_and_mason_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by mason after boot."""
    ...
    assert "logs=mason" in r.stdout, (
        f"logs/ not owned by mason: {r.stdout}"
    )
    assert "gateways=mason" in r.stdout, (
        f"logs/gateways/ not owned by mason: {r.stdout}"
    )
