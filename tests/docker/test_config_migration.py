...
Build the real image and verify: a config.yaml present in $MASON_HOME
is migrated by docker_config_migrate.py on boot, running as the mason
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $MASON_HOME must be migrated on boot by
    docker_config_migrate.py, running as the mason user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $MASON_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/mason/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by mason (migration ran as mason)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "mason", (
        f"config.yaml not owned by mason (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
