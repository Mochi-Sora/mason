"""Tests for the secret-source tracking in ``mason_cli.env_loader``.

These cover the small public surface that lets `mason model` / `mason setup`
...
from mason_cli import env_loader  # noqa: E402
...
def test_single_profile_scoped_load_keeps_override_behavior(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MASON_TEST_SHARED_ADAPTER_CONFIG", raising=False)
    other_home = tmp_path / "other"
    other_home.mkdir()
    (other_home / ".env").write_text("MASON_TEST_SHARED_ADAPTER_CONFIG=second\n")

    was_active = secret_scope.is_multiplex_active()
    secret_scope.set_multiplex_active(False)
    home_token = set_mason_home_override(other_home)
    try:
        loaded = env_loader.load_mason_dotenv(mason_home=other_home)
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_mason_home_override(home_token)

    try:
        assert os.environ.get("MASON_TEST_SHARED_ADAPTER_CONFIG") == "second"
        assert (other_home / ".env") in loaded
    finally:
        os.environ.pop("MASON_TEST_SHARED_ADAPTER_CONFIG", None)
...
def test_multiplex_dotenv_load_hydrates_sources_without_global_env(
    ...
    was_active = secret_scope.is_multiplex_active()
    home_token = set_mason_home_override(tmp_path)
    secret_scope.set_multiplex_active(True)
    try:
        assert env_loader.load_mason_dotenv(mason_home=tmp_path) == []
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_mason_home_override(home_token)
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_mason_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_mason_dotenv()`` is called at module-import time from several
    hot modules (cli.py, mason_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per MASON_HOME per process.
    """
    ...
    # cli.py, mason_cli/main.py, run_agent.py, trajectory_compressor.py,
