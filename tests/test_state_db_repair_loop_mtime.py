...
Incident (2026-08-17): a malformed-SCHEMA state.db sent Mason into an
...
from mason_state_repair import _MAX_MALFORMED_BACKUPS, _MAX_PERSISTENT_REPAIR_ATTEMPTS, _REPAIR_BACKUP_MIN_FREE_BYTES, _backup_content_identity, _backup_db_file, _db_fingerprint, _existing_malformed_backups, _persistent_repair_attempts_exhausted, _record_repair_outcome, _repair_backup_headroom_bytes
...
def test_repair_aborts_when_backup_refused_for_disk(tmp_path):
    ...
    with patch("shutil.disk_usage", return_value=tight):
        report = mason_state_repair.repair_state_db_schema(db)
...
def test_live_connection_keeps_its_write_lock_across_a_repair_pass(tmp_path):
    ...
    POSIX advisory locks, so it is immune. DELETE mode is what Mason falls
...
def test_forensic_backup_includes_the_rollback_journal(tmp_path):
    ...
    Rollback-journal mode is Mason's fallback on NFS/SMB/FUSE/ZFS and on
