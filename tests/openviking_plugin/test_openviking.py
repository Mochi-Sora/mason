...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/mason/.overview.md") == "viking://user/mason"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/mason/memories/profile.md") == "viking://user/mason/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_mason_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MASON_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp MASON_HOME
        mason_home = tmp_path / "mason_test"
        mason_home.mkdir(exist_ok=True)
        config_yaml = mason_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/mason"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/mason/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/mason/.overview.md"
        assert result["resolved_uri"] == "viking://user/mason"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/mason/memories/a.md",
            "viking://user/mason/memories/b.md",
            "viking://user/mason/memories/c.md",
            "viking://user/mason/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/mason/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/mason"}),
        ]


class TestOpenVikingAutoRecallPrefetch:
    @pytest.mark.parametrize("peer", ["", "mason"])
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch, peer):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/user/peers/mason/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/user/peers/mason/memories/e2e-full.md"]
        ...
        assert all(
            headers.get("user-agent") == f"openviking-memory-mason/{_MASON_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/mason/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/mason/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/mason"}))

        assert result["path"] == "viking://user/mason"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/mason/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/mason/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/mason"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="mason"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "mason")
        ...
        assert alice_uri.startswith("viking://user/alice/peers/mason/")
        assert bob_uri.startswith("viking://user/bob/peers/mason/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert out["session_id"].startswith("mason-remember-")
    ...
    def test_remember_uses_a_distinct_one_shot_session_for_each_call(self, monkeypatch):
        ...
        assert first["session_id"].startswith("mason-remember-")
        assert second["session_id"].startswith("mason-remember-")
    ...
    def test_remember_reports_commit_failure_with_recovery_command(self, monkeypatch):
        ...
        assert out["error"].startswith(
            "Memory message was accepted, but commit failed for session mason-remember-"
        )
        ...
        assert out["session_uri"].endswith(f"/sessions/{out['session_id']}")
        assert out["recovery_command"] == f"ov session commit {out['session_id']}"
        assert "same OpenViking profile and credentials as Mason" in out["recovery_note"]
        assert len(posts) == 2
        assert posts[0][0].endswith("/messages")
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting mason for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Mason run" not in message, message
