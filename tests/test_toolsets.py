...
class TestToolsetConsistency:
    ...
    def test_mason_platforms_share_core_tools(self):
        """All mason-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on mason-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["mason-cli", "mason-telegram", "mason-discord", "mason-whatsapp", "mason-slack", "mason-signal", "mason-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_mason_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("mason-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("mason-cli")
        second = resolve_toolset("mason-cli")
        ...
        assert (
            "mason-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("mason-cli", include_registry=False)
        second = resolve_toolset("mason-cli", include_registry=False)
        assert first == second
