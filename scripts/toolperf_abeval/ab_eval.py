...
measured in the production session DB) through `mason chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    MASON_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Mason home with credentials for the
...
(tracker: NousResearch/mason-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "MASON_HOME": str(HOME),
                "MASON_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "mason_cli.main", "chat", "--query", q,
