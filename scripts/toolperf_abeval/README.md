...
batch (tracker: [#77056](https://github.com/NousResearch/mason-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Mason home, same model, same tasks, same reps.
...
1. Create a dedicated Mason home with credentials for the models under test:
   ...
   integration at it; no Mason observability plugin needs to be enabled.
