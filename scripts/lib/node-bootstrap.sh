...
#   2. ~/.mason/node/ from a prior Mason-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.mason/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$MASON_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   MASON_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   MASON_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   MASON_HOME               (default: $HOME/.mason)
# ============================================================================

MASON_NODE_MIN_VERSION="${MASON_NODE_MIN_VERSION:-20}"
MASON_NODE_TARGET_MAJOR="${MASON_NODE_TARGET_MAJOR:-22}"
MASON_HOME="${MASON_HOME:-$HOME/.mason}"
MASON_NODE_AVAILABLE=false
...
# Redirect a Mason-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $MASON_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$MASON_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${MASON_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$MASON_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through mason_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$MASON_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$MASON_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$MASON_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $MASON_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$MASON_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$MASON_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $MASON_NODE_TARGET_MAJOR..."
    proto install node "$MASON_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $MASON_NODE_TARGET_MAJOR..."
    nvm install "$MASON_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$MASON_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${MASON_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${MASON_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${MASON_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${MASON_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${MASON_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $MASON_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $MASON_HOME/node/..."
    ...
    mv "$extracted" "$MASON_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # MASON_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${MASON_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$MASON_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$MASON_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$MASON_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $MASON_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Mason-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$MASON_HOME/node/bin/$tool" \
        "$MASON_HOME/node/${tool}.exe" \
        "$MASON_HOME/node/$tool"; do
...
# The managed node runs but is below MASON_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$MASON_HOME/node/bin/node" "$MASON_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$MASON_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by mason_constants.find_mason_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$MASON_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Mason-managed Node is broken — redownloading to $MASON_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        MASON_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$MASON_HOME/node/bin/node" ]; then
        export PATH="$MASON_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Mason-managed)"
            MASON_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { MASON_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { MASON_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { MASON_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { MASON_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { MASON_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { MASON_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $MASON_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
