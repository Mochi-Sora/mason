 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.mason/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.mason', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (MASON_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.MASON_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.mason', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.MASON_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.mason', 'document_cache');
const AUDIO_CACHE_DIR = process.env.MASON_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.mason', 'audio_cache');
...
// `mason update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Mason Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Mason itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Mason Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Mason and trigger
      ...
      // Ignore Mason' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `mason_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `mason_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
