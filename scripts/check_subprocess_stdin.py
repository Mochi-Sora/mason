...
When Mason runs in TUI mode, the gateway child process communicates with
...
# ``get_mason_home() / "plugins"`` (user) and ``./.mason/plugins/`` (project,
# gated behind ``MASON_ENABLE_PROJECT_PLUGINS``) — see
# ``mason_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``mason_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import mason_constants (this script
    ...
    # plugins in get_mason_home()/plugins/ and project plugins in
    # ./.mason/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_mason_home() / "plugins"]
    if os.environ.get("MASON_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".mason" / "plugins")
