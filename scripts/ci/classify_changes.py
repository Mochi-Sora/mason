...
# tree generates llms.txt — the index every LLM (Mason included, via the
# mason-agent skill) reads to learn what Mason can do — and
...
_MCP_CATALOG_FILES = {"mason_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Mason-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``mason serve`` backend, the Docker image)
