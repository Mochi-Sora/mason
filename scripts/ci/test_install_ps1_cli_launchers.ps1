# Behavioral test for install.ps1's mason launcher staging (PR #92092,
...
#
# The test lifts the real Install-MasonCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $MasonHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-MasonCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-MasonCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'mason-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'mason-agent'
    ...
    $missingThrew = $false
    try {
        Install-MasonCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing mason.exe fails the launcher stage'
    ...
    $masonV1 = [byte[]](77, 90, 1)
    $masonV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'mason.exe'), $masonV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-MasonCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $masonV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'mason.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'mason-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'mason.exe'), $masonV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'mason-acp.exe'), $acp)
    Install-MasonCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $masonV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'mason.exe'))) `
        'installer refreshes an existing Mason launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'mason-acp.exe'))) `
    ...
    Install-MasonCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'mason.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'mason.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'mason.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'mason.exe')) -and $cmdBody.Contains('%*')) `
