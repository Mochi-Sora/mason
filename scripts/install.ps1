...
$RepoUrlSsh = if ($env:MASON_REPO_URL) { $env:MASON_REPO_URL } else { "git@github.com:NousResearch/mason-agent.git" }
$RepoUrlHttps = if ($env:MASON_REPO_URL) { $env:MASON_REPO_URL } else { "https://github.com/NousResearch/mason-agent.git" }
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/mason-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/mason-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/mason-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://mason-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
