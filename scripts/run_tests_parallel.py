...
Environment:
    MASON_TEST_WORKERS  Override worker count (default: os.cpu_count())
    MASON_TEST_PATHS    Override discovery roots (colon-sep; on Windows
...
#                        nousresearch/mason-agent:test image, via
#                        ``MASON_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or MASON_TEST_FILE_TIMEOUT.
#
...
# Set to 0 to disable (env: MASON_TEST_FILE_RETRIES).
...
def _split_pathspec(value: str) -> List[str]:
    ...
    ``MASON_TEST_PATHS``) into individual paths.
...
def _run_one_file_once(
    ...
    env = os.environ.copy()
    temproot = tempfile.mkdtemp(prefix="mason-pytest-tmproot-")
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/mason-agent/mason-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("MASON_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $MASON_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("MASON_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("MASON_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: MASON_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(
            os.environ.get("MASON_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)
        ),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: MASON_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: MASON_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or MASON_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("MASON_TEST_SLICE")
