#!/usr/bin/env bash
# Canonical test runner for mason-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.mason/...)
#
# Usage:
...
# (MASON_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.mason/mason-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.mason/mason-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${MASON_PYTHON:-}" ] && [ -x "$MASON_PYTHON" ] \
    && "$MASON_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: MASON_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `mason` binary rather
  # than the devShell hook.
  PYTHON="$MASON_PYTHON"
  echo "▶ no local venv — using Nix dev venv via MASON_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and MASON_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.mason/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.mason"
...
#
#   * MASON_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * MASON_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     MASON_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# MASON_RUN_SLOW_PET_TESTS / MASON_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no MASON_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in MASON_TEST_IMAGE MASON_TEST_WORKERS MASON_TEST_PATHS \
  MASON_TEST_FILE_TIMEOUT MASON_TEST_FILE_RETRIES MASON_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${MASON_RUN_SLOW_PET_TESTS:+MASON_RUN_SLOW_PET_TESTS="$MASON_RUN_SLOW_PET_TESTS"} \
  ${MASON_E2E_BROWSER:+MASON_E2E_BROWSER="$MASON_E2E_BROWSER"} \
