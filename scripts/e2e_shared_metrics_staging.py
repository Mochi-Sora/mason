...
MASON_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="mason-telemetry-e2e-"))
    os.environ["MASON_HOME"] = str(scratch)
    ...
    from mason_cli.observability.shared_metrics_send_config import (
        resolve_send_config,
    )
    ...
    from mason_cli.observability.shared_metrics_sender import (
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "mason.shared_metrics.v2",
    ...
    print(f"scratch MASON_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://mason-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
