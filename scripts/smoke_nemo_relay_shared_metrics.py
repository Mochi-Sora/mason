"""Run a real Mason CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_mason_executable(mason_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "mason",
        Path(".venv") / "Scripts" / "mason.exe",
    ):
        candidate = mason_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("mason")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Mason executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Mason source checkout containing .venv/bin/mason",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated MASON_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["mason.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['mason.client.active']}"
        )
    [model] = by_name["mason.model_route.count"]
    expected_model = {
        "name": "mason.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['mason.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["mason.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['mason.task_run.started']}"
        )
    [terminal] = by_name["mason.task_run.finished"]
    ...
    [tool] = by_name["mason.tool_call.count"]
    ...
    lifecycle = by_name["mason.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["mason.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Mason development environment requires jsonschema"
        ) from exc
    ...
    if metrics["mason.client.active"] != [
        {
            "name": "mason.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['mason.client.active']}"
        )
    [model] = metrics["mason.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['mason.model_route.count']}"
        )
    [terminal] = metrics["mason.task_run.finished"]
    ...
    [tool] = metrics["mason.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["mason.skill.lifecycle.count"]
    ...
    loads = metrics["mason.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    mason_repo = args.mason_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    mason = _resolve_mason_executable(mason_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="mason-relay-shared-metrics-"))
    home = root / "mason-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["MASON_HOME"] = str(home)
        python_paths = [str(mason_repo)]
    ...
    (root / "mason.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "mason.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Mason exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Mason model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Mason did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Mason did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "mason.shared_metrics.v2.schema.json",
    )

    print("Mason -> NeMo Relay shared-metrics smoke test passed")
