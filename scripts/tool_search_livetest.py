#!/usr/bin/env python3
"""Live test harness for Mason Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any mason imports.
ORIGINAL_HOME = os.environ.get("MASON_HOME")
ORIGINAL_AUTH = Path.home() / ".mason" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.mason/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.mason/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="mason_ts_live_"))
    mason_home = home_dir / ".mason"
    mason_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, mason_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".mason" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, mason_home / ".env")
        ...
        load_mason_dotenv(mason_home=str(Path.home() / ".mason"))
    ...
    (mason_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return mason_home
...
def reset_module_state():
    """Drop cached modules so the new MASON_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "mason_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["MASON_HOME"] = str(home)
...
def main():
    ...
    # Restore original MASON_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MASON_HOME"] = ORIGINAL_HOME
    else:
