...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[mason] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[mason`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[mason`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        MasonHome = $(if ($paths) { $paths.mason_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\mason\mason-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-mason'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-mason') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
