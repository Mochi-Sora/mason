...
# changing PATH, installing software, or touching the user's Mason home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("mason-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$MasonHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -MasonHome $MasonHome -InstallDir $InstallDir
