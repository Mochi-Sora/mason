...
# (%MASON_HOME%\mason-setup.exe). That binary has no self-update path --
# copy_self_to_mason_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `mason update` refreshes
...
#     -InstallRoot <path>   repo checkout (MASON_HOME\mason-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Mason.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .mason-update-in-progress in place (tests)
#
...
# .mason-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim MASON_HOME\.mason-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining MASON_UPDATE_STARTED_AT from the Desktop hand-off.
# mason_cli/update_lock.py's ancestry rule lets our
# `mason update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Mason.exe pid.
try {
    Add-Type -Namespace MasonHandoff -Name Win32 -MemberDefinition @'
...
$MasonHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $MasonHome ".mason-update-in-progress"
$LogDir = Join-Path $MasonHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $MasonHome ".mason-update-result.json"
$script:Ui = $null
$script:UiStage = "Mason will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale mason-update-ui-* leftovers from interrupted
    ...
    # update marker (.mason-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "mason-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-MasonStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("mason-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [MasonHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `mason debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"mason debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [MasonHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Mason.exe before writing the replacement,
    ...
    # DO NOT spawn Mason.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [MasonHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [MasonHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [MasonHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Mason process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [MasonHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `mason update` deliberately runs its build steps with stdout
# inherited (mason_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:MASON_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:MASON_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:MASON_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:MASON_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `mason update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (mason_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:MASON_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:MASON_UPDATE_PROGRESS_LOG
}
...
public static class MasonUpdateJob {
...
function Invoke-MasonStep([string]$Exe, [string[]]$MasonArgs, [string]$Tag) {
    ...
    # afterwards, where `mason debug share` picks it up.
    #
    ...
    # .mason-update-result.json, clearing .mason-update-in-progress,
    ...
    # Mason" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($MasonArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [MasonUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `mason update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [MasonUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; MASON_SELFTEST_FAIL=1 exercises the
# error state, MASON_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:MASON_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MASON_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:MASON_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-MasonStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Mason install to
...
#            update log, the shape of every real `mason update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:MASON_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MASON_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:MASON_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:MASON_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "mason-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "mason-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "mason-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "mason-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "mason-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "mason-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "mason-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "mason-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `mason update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-MasonStep $powershell @(
    ...
    $flood = Invoke-MasonStep $powershell @(
    ...
    $stall = Invoke-MasonStep $powershell @(
    ...
    try {
        $logstall = Invoke-MasonStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-MasonStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-MasonStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:MASON_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Mason to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Mason window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Mason fully and try again."
    ...
    Publish-UiProgress "Preparing Mason files"
    $shim = Join-Path $InstallRoot "venv\Scripts\mason.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Mason install open (venv\Scripts\mason.exe locked after 20s). Nothing was changed. Close other Mason windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the mason.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\mason.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_mason_exe must first rename the running mason.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that mason.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Mason window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Mason.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m mason_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Mason installer or `mason doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "mason_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Mason windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m mason_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed mason predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-MasonStep $pythonExe $updateArgs "update"
    Write-HandoffLog "mason update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-MasonUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `mason update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "mason update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-MasonStep $pythonExe @("-m", "mason_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `mason desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `mason debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Mason is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Mason was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Mason."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Mason"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Mason to finish (it could not restart itself)."
