...
#
# The whole job: wait for the Desktop to exit, run `mason update`, tell the
...
# the MASON_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (MASON_HOME/mason-agent)
...
MASON_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
MASON_HOME="${MASON_HOME:-${TMPDIR:-/tmp}}"
MARKER="$MASON_HOME/.mason-update-in-progress"
LOG_DIR="$MASON_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$MASON_HOME/.mason-update-result.json"
STATUS="${TMPDIR:-/tmp}/mason-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Mason update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Mason update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Mason update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Mason update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "Mason" (#88410).
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `mason update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/mason-update-ui-$$" \
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${MASON_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  # _desktop_linux_userns_sandbox_available() probe in mason_cli/main.py).
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Mason to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Mason.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Mason.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Mason (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Mason also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Mason to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
# (`venv/bin/mason` has a `python3` shebang), so no Python-side heal —
...
# (mason_cli/macos_tcc_anchor.ensure_tcc_anchor, which re-anchors whenever
...
# touched; and when it does restore symlinks, the very `mason update` run it
...
tcc_pick_update_invoke() { # sets UPDATE_INVOKE; safety net past a failed heal
  ...
  # launchd gateway proves `venv/bin/python -m mason_cli.main` works when
  # every alias entrypoint is bricked — drive the update the same way.
  local bin="$1"
  UPDATE_INVOKE=("$bin/mason")
  if ! tcc_probe_python "$bin/python3" && tcc_probe_python "$bin/python"; then
    UPDATE_INVOKE=("$bin/python" -m mason_cli.main)
  fi
}
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${MASON_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${MASON_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `mason update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Mason while the
...
# so the shim server and the later `mason update` subprocess both inherit
...
# update_lock.py (the `mason update` child adopts it via process ancestry).
# The Desktop supplies one acquisition time for the whole ownership chain.
NOW="$(date +%s)"
STARTED_AT="${MASON_UPDATE_STARTED_AT:-$NOW}"
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Mason window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Mason fully and try again."
...
MASON_BIN="$INSTALL_ROOT/venv/bin/mason"
[ -x "$MASON_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $MASON_BIN is missing. The install needs repair (run the Mason installer or mason doctor)."; log "$FINAL_MSG"; exit 3; }

# Heal a venv the reverted TCC anchor left bricked BEFORE invoking the CLI:
# venv/bin/mason execs venv/bin/python3, so a dead alias kills every attempt
...
tcc_pick_update_invoke "$INSTALL_ROOT/venv/bin"
if [ "${UPDATE_INVOKE[0]}" != "$MASON_BIN" ]; then
  log "venv/bin/python3 still unbootable; invoking the update via ${UPDATE_INVOKE[*]}"
fi

# Run FROM the install root: `mason update` resolves the tree it mutates
...
# "close all Mason windows" sentinel.
KEEP_STASH=""
if "${UPDATE_INVOKE[@]}" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed mason predates --keep-stash; running without it"
fi
...
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "mason update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Mason windows") is not retryable.
  #
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "mason update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
# Truthful completion: `mason update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside mason update; retrying build"
  publish_stage "Rebuilding Desktop"
  "${UPDATE_INVOKE[@]}" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run mason desktop --force-build from a terminal to retry."
    exit 6
  }
fi
...
if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
  ...
  FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run mason debug share in a terminal to send a report."
  ...
  if ! tcc_probe_python "$INSTALL_ROOT/venv/bin/python3" \
    ...
    FINAL_MSG="Update failed: the Python interpreter inside $INSTALL_ROOT/venv cannot start (heal state: $TCC_HEAL_STATE). Reinstall the runtime with the Mason installer, or run mason doctor --fix from a terminal if any mason command still works."
  fi
fi
exit "$FINAL_CODE"
