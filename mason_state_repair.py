"""state.db repair, backup and writability preflight (split from mason_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``mason_state_repair.<name>``.
"""
...
from mason_startup_watchdog import report_startup_progress
...
# Log-record parity with the origin module (caplog tests pin "mason_state").
logger = logging.getLogger("mason_state")
...
_MANUAL_RECOVER_HINT = ("Free disk space, then retry (or recover manually with "
                        "`mason sessions recover --source {db_path} --inspect-only` first).")
...
def _read_offline(db_path: Path, what: str, reader) -> Optional[str]:
    """Run *reader()* under ``mason_cli.sqlite_safe_read.offline_file_access``.
    ...
    without mason_cli have no tracked connections."""
    try:
...
def _claim_repair_attempt(db_path: Path) -> bool:
    ...
    from mason_state import _repair_attempt_lock, _repair_attempted_paths
...
def _try_lock_nonblocking(handle) -> None:
    ...
    from mason_state import _IS_WINDOWS
    if _IS_WINDOWS:
...
def _cross_process_repair_lock(db_path: Path):
    ...
    #36644).
    """
    from mason_state import _IS_WINDOWS, _REPAIR_LOCK_TIMEOUT_SECONDS
...
def _repair_snapshot_timeout_seconds(source_path: Path) -> float:
    ...
    from mason_state import _REPAIR_LOCK_TIMEOUT_SECONDS
    source_bytes = 0
...
def _persistent_repair_exhausted_error(db_path: Path) -> str:
    ...
    return (f"automatic repair has already failed {_MAX_PERSISTENT_REPAIR_ATTEMPTS} times on this exact file — the "
            ...
            f"required: restore a backup, or salvage with `mason sessions recover --source {db_path} "
            f"--inspect-only`, then (if it reports recoverable) `mason sessions recover --source {db_path} "
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    ``close()`` a descriptor and cancel that connection's POSIX advisory locks (``mason_cli.sqlite_safe_read``) —
    ...
    `mason sessions recover --source <db> --inspect-only`, never at a raw sqlite3 shell on the live file.

    See #69603.
    """
    with contextlib.suppress(ImportError):  # scaffold/embed installs without mason_cli track no connections
...
def preflight_db_writability(db_path: Path, *, db_label: str = "state.db") -> None:
    ...
    "fix" (deleting the ``-wal``) loses committed transactions. ``chmod u+rw`` repair only inside the Mason home
    tree (Mason owns those files; ``chmod`` fails on files the user doesn't own, bounding the repair exactly);
    ...
    with contextlib.suppress(Exception):  # pragma: no cover - defensive
        home = Path(get_mason_home()).resolve()
    ...
    for p, is_dir in [(db_path.parent, True), *((p, False) for p in (db_path, *sidecars) if p.is_file())]:
        ...
        raise sqlite3.OperationalError(
            f"{db_label} is not writable: {'directory' if is_dir else 'file'} {p} is read-only for this user. Mason "
...
def _reapply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from mason_state_wal import _apply_macos_checkpoint_barrier, _enforce_macos_synchronous_full
    try:
...
def apply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from mason_state_wal import _apply_synchronous_pragma
    ok = _reapply_durability_barriers(conn)
    with contextlib.suppress(Exception):
        from mason_cli.config import cfg_get, load_config_readonly  # local: avoids an import cycle
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    See #50502.
    """
    from mason_state import SessionDB, load_fts5_cjk_extension
    ...
    try:
        with contextlib.closing(conn):
            ...
            # rolled back.
            probe_session_id = f"_mason_fts_health_probe_{time.time_ns()}"
            try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    (``mason_state_holders``) fails closed on deleted-WAL-generation, uninspectable, or unknown holders."""
    ...
    with _cross_process_repair_lock(db_path) as holding_lock:
        ...
        # exclusive guard in the locked routine excludes writers through promotion and sees DELETE-mode readers too.
            ...
            _repair_skip(report, "skipped", "a live writer still holds state.db; skipped schema surgery to avoid tearing "
                         "b-tree pages under a concurrent writer. Stop the gateway (mason gateway stop) and retry.")
        else:
...
def _probe_journal_mode_for_repair(db_path: Path) -> Optional[str]:
    ...
    from mason_state_wal import _on_disk_journal_mode
    try:
...
def _strategy_rebuild_fts(conn: sqlite3.Connection) -> None:
    ...
    from mason_state import load_fts5_cjk_extension
