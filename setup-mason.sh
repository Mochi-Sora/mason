...
# 5. Symlinks the 'mason' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Mason Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/mason-uv-install.$$.log")"
...
if is_termux; then
    # Mason currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Mason requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink mason into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up mason command..."

MASON_BIN="$SCRIPT_DIR/venv/bin/mason"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MASON_BIN" "$COMMAND_LINK_DIR/mason"
echo -e "${GREEN}✓${NC} Symlinked mason → $COMMAND_LINK_DISPLAY_DIR/mason"
...
# ============================================================================
# Seed bundled skills into ~/.mason/skills/
# ============================================================================

MASON_SKILLS_DIR="${MASON_HOME:-$HOME/.mason}/skills"
mkdir -p "$MASON_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.mason/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MASON_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  mason status        # Check configuration"
if is_termux; then
    echo "  mason gateway       # Run gateway in foreground"
else
    echo "  mason gateway install # Install gateway service (messaging + cron)"
fi
echo "  mason cron list     # View scheduled jobs"
echo "  mason doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m mason_cli.main setup
fi
