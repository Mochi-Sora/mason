...
Each probe runs in a FRESH subprocess with an isolated MASON_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "MASON_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "MASON_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("MASON_SESSION", "MASON_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "mason_cli.main", "gateway.run", "tools.registry", "mason_state", "tui_gateway.server", "mason_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "mason-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `mason --version`, `mason --help`, `mason doctor --help`, `mason config get model` (no network)
CLI = {"version": ["mason_cli/main.py", "--version"], "help": ["mason_cli/main.py", "--help"], "config_get": ["mason_cli/main.py", "config", "get", "model"], "tools_list": ["mason_cli/main.py", "tools", "--help"], "skills_help": ["mason_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_mason_default_ms", lambda: resolve_toolset("mason-default"), 200)
...
try:
    import mason_state, tempfile, uuid
    from pathlib import Path
    db=mason_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(mason_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
