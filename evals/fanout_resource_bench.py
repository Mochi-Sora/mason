#!/usr/bin/env python3
"""Fan-out resource benchmark for mason-agent.
...
#   turn 1: call write_file on <its worktree>/mason_cli/bench_<i>.py
...
def _snap(pid: int, db_path: str) -> dict:
    ...
    return {
        ...
        "tcp": int(tcp or 0), "pyright": kids.count("pyright"), "kernels": kids.count("mason_kernel_runner"),
...
def main() -> None:
    ...
    home = tempfile.mkdtemp(prefix="mason_bench_home_")
    os.environ["MASON_HOME"] = home
    os.environ["TERMINAL_ENV"] = "local"
    os.environ.pop("OPENROUTER_API_KEY", None)
    sys.path.insert(0, a.repo)
    os.chdir(a.repo)
    pyright = shutil.which("pyright-langserver", path=os.path.expanduser("~/.mason/lsp/bin") + os.pathsep + os.environ.get("PATH", ""))
    ...
    wts = []
    base = tempfile.mkdtemp(prefix="mason_bench_wt_")
    for w in range(a.worktrees):
        d = os.path.join(base, f"wt{w}")
        os.makedirs(os.path.join(d, "mason_cli"))
        subprocess.run(["git", "init", "-q", d], check=True)
        open(os.path.join(d, "pyproject.toml"), "w", encoding="utf-8").write("[project]\nname='b'\n")
        open(os.path.join(d, "mason_cli", "__init__.py"), "w", encoding="utf-8").write("")
        wts.append(d)
    ...
    tasks = [{"goal": json.dumps({"file": os.path.join(wts[i % len(wts)], "mason_cli", f"bench_{i}.py")}),
