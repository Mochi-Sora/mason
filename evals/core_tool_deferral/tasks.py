...
ctx keys: workspace, mason_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire MASON_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["mason_home"]):
        for fn in files:
            try:
