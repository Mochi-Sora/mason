...
# --- hermetic env BEFORE any mason import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("MASON_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
mason_home = os.path.join(tmp_root, ".mason")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(mason_home)
os.makedirs(workspace)
with open(os.path.join(mason_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Mason desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "mason_home": mason_home,
