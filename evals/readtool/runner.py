"""Run the read-tool eval through the REAL Mason AIAgent.

For each task: fresh temp MASON_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    mason_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".mason"
    ...
    try:
        ...
        shutil.rmtree(mason_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.mason/.env; set +a  — then relaunch."
        )
