...
Every tool-provider ABC shares the same identity + ``mason tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``mason tools``. Defaults to ``name``."""
        return self.name

    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``mason tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``mason tools``. Defaults to ``name.title()``."""
    ...
    def is_available(self) -> bool:
        ...
        ``mason setup`` call it on every paint.
        """
        return True
