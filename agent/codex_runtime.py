...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Mason accounting. Prompt bucket = uncached + cached
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Mason deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Mason UI bridge -------------------------------------
# The app-server bypasses the Mason tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Mason tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Mason' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.mason-tools.* prefix is stripped (users see Mason tools).
_INTERNAL_MCP_SERVER = "mason-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Mason tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Mason' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, MASON_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Mason' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. MASON_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("MASON_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
