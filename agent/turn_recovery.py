...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: mason auth add nous",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_mason_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `mason doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_mason_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Mason-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: mason config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: mason config set ANTHROPIC_API_KEY \"\"",
    )
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from mason_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `mason auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `mason model`.",
            )
        else:  # nous
            _vlines(
                agent,
                ...
                "      1. Re-authenticate: mason portal",
    ...
    _vlines(
        ...
        "      • Is the key valid? Run: mason setup",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        mason fallback add   (interactive picker — same as `mason model`)",
        )
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.mason/config.yaml` to extend the per-call timeout. (Mason's built-in floor is 600s for "
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from mason_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `mason model`.",
            )
