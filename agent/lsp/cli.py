"""``mason lsp`` CLI subcommand: status / list / install / install-all / restart / which.

Handlers live here (not in ``mason_cli/main.py``) so the LSP module ships self-contained.
"""
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``mason lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``mason lsp <subcommand>``."""
