...
Installs go to a Mason-owned staging dir, ``<MASON_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``mason lsp status`` reports it).
...
# ``bin`` in ``<MASON_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `mason lsp status` reports its presence.
...
def mason_lsp_bin_dir() -> Path:
    """Return the Mason-owned bin staging dir for LSP servers."""
    from mason_constants import get_mason_home

    p = get_mason_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(mason_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = mason_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $MASON_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Mason installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = mason_lsp_bin_dir().parent  # <MASON_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = mason_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``mason lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "mason_lsp_bin_dir"]
