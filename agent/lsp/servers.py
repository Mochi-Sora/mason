...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<MASON_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_mason_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <MASON_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = mason_lsp_session_dir()
    inner = (
        ...
        f"-HostName Mason -HostProfileId mason -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def mason_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from mason_constants import get_mason_home
    d = os.path.join(str(get_mason_home()), "lsp", "pses")
