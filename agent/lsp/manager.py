...
class _BackgroundLoop:
    ...
    def start(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(target=self._run_forever, name="mason-lsp-loop", daemon=True)
        self._thread.start()
...
class LSPService:
    ...
    def create_from_config(cls) -> Optional["LSPService"]:
        """Build a service from ``mason_cli.config``; ``None`` if config can't load."""
        try:
    ...
    def enabled_for(self, file_path: str) -> bool:
        ...
        and pair not broken (a failed server costs nothing until ``mason lsp restart`` / exit)."""
    ...
    def get_status(self) -> Dict[str, Any]:
        """Return a snapshot of the service for ``mason lsp status``."""
        with self._state_lock:
            clients = [
