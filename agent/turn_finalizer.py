...
def _resolve_budget_fallback(
    ...
    _kanban_task = os.environ.get("MASON_KANBAN_TASK") if budget_exhausted else None
...
def finalize_turn(
    ...
    result = {
        ...
        **{key: getattr(agent, f"session_{key}") for key in _SESSION_COST_KEYS},
        # Requested service tier, for billing audits (`mason -z --usage-file`).
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        result["error"] = final_response or (
            ...
            "health (`mason doctor`), then send your message again"
        )
