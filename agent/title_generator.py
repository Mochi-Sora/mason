...
# Mason' own machine-authored openers: a compaction handoff or resumed session must not be titled after them.
_MACHINE_PREFIXES = (
...
def _title_config() -> dict:
    """``auxiliary.title_generation`` (lazy read-only import: no mason_cli cycle, no migration writes)."""
...
def auto_title_session(
    ...
    trigger is the post-``mason update`` window where lazy imports read NEW source against OLD modules."""
    try:
        ...
        logger.warning("Auto-title failed (harmless; if this started after an update, restart the running Mason process): %s", e)
...
def _is_real_user_turn(message: Any) -> bool:
    """A question a person actually asked (Mason persists machinery under ``role="user"``)."""
