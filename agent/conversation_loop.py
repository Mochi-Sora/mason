...
try:
    ...
    def _evo_on_response(*a, **k):  # type: ignore
        pass
...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in mason_state.py; kept local so importing
# mason_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Mason tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Mason "
        ...
        "again. A known-good starting point is 65,536 tokens. In Mason config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `mason auth reset anthropic`. Until that "
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from mason_cli.moa_config import decode_moa_turn
...
def run_conversation(
    ...
    # Adopt ~/.mason/.env credential/base-url edits made since the last turn — a
    ...
    try:
        if _EVO_ENABLED and result.get("final_response"):
            _evo_on_response(None, s.user_message if hasattr(s, "user_message") else "", result["final_response"], "")
    except Exception:
        pass
    return result
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('mason_constants', 'PARTIAL_STREAM_STUB_ID'),
