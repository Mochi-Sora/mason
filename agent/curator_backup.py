"""Curator snapshot + rollback. Before any mutating curator pass, ``~/.mason/skills/`` is tar.gz'd under
``~/.mason/skills/.curator_backups/<utc-iso>/`` with a ``manifest.json``. Rollback first snapshots the CURRENT tree (so it is
...
doesn't re-fire), ``.bundled_manifest``, ``.curator_suppressed``. Each snapshot also copies ``~/.mason/cron/jobs.json`` as
...
def _backup_cron_jobs_into(dest: Path) -> Dict[str, Any]:
    """Copy the live ``~/.mason/cron/jobs.json`` into ``dest`` as ``cron-jobs.json``. Never raises: a missing/unreadable
    file yields ``backed_up=False`` plus a reason, and the snapshot proceeds."""
    src = get_mason_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.mason/skills/`` and prune old ones. Returns the snapshot dir, or None when
    ...
    if not skills.exists():
        logger.debug("No ~/.mason/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.mason/skills/`` from a snapshot (explicit id or newest): safety-snapshot the CURRENT tree; stage
    ...
    if target is None:
        return (False, "no matching backup found" + (f" for id '{backup_id}'" if backup_id else "")
                + " (use `mason curator rollback --list` to see available snapshots)", None)
