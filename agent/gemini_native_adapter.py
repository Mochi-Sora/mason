...
try:
    import mason_cli as _mason_cli

    _MASON_VERSION = str(_mason_cli.__version__)
except Exception:
    _MASON_VERSION = "0.0.0"
_API_CLIENT = f"mason-agent/{_MASON_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Mason typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.mason/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.mason/.env (get one at https://aistudio.google.com/app/apikey), or run `mason setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Mason retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No MASON_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
