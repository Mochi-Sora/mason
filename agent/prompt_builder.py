...
from mason_constants import (
    get_mason_home, get_skills_dir, is_wsl, reset_mason_home_override, set_mason_home_override,
)
...
def _find_mason_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.mason.md`` / ``MASON.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".mason.md", "MASON.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Mason, built by Mochi Sora. Be direct: match the length of your reply to the weight of the ask "
...
MASON_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the mason-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Mason (by Mochi Sora). When the user needs help with Mason itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the `mason-agent` skill and this repo (README.md, docs/) are your "
    "authoritative reference and always hold the latest, most up-to-date information. The `mason-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='mason-agent') "
    "before configuring, modifying, or troubleshooting Mason so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
MASON_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Mason (by Mochi Sora). When the user needs help with Mason itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the `mason-agent` skill and this repo (README.md, docs/) are the "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.mason/kanban.db`. Your task id is in "
    "`$MASON_KANBAN_TASK`; your workspace is `$MASON_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $MASON_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $MASON_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${MASON_KANBAN_BRANCH:-wt/$MASON_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$MASON_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`mason profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `mason kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The mason-agent runtime already executes a batch of tool
...
# to mason-agent's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Mason appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Mason "
         ...
         "rather than anything inside Mason. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Mason terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Mason desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-mason-send=\"prompt\" on any clickable element (or "
        "window.mason.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Mason itself is running. The host OS, "
            f"home, and cwd of the Mason process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Mason itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: MASON_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("MASON_ENVIRONMENT_HINT") or "").strip() or str(
...
def _skills_prompt_snapshot_path() -> Path:
    return get_mason_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("MASON_PLATFORM") or os.environ.get("MASON_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("MASON_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the MASON_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_mason_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_mason_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from MASON_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the MASON_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the MASON_HOME
    ...
    """
    try:
        from mason_cli.config import ensure_mason_home
        ensure_mason_home()
    except Exception as e:
        logger.debug("Could not ensure MASON_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_mason_home()) / "SOUL.md"
...
def _load_mason_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.mason.md / MASON.md — nearest match walking up to the git root."""
    mason_md_path = _find_mason_md(cwd_path)
    content = _read_context_file(mason_md_path) if mason_md_path else ""
    if not content:
        return ""
    label = str(mason_md_path.relative_to(cwd_path)) if mason_md_path.is_relative_to(cwd_path) else mason_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".mason.md", mason_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .mason.md/MASON.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from MASON_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # A FALLBACK-picked cwd inside the Mason install tree must not gain system-prompt authority (the desktop
    # default would load this repo's contributor AGENTS.md). An explicit cwd is honored verbatim.
    # An explicitly configured cwd is honored verbatim — the Mason tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Mason
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Mason "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_mason_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
