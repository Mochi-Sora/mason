...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``mason_home`` (profile-scoped storage; never hardcode
        ``~/.mason``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``mason memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE MASON_HOME for ``mason backup``/``import``
