"""Shared SKILL.md preprocessing helpers: ``${MASON_*}`` template tokens and
...
from mason_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${MASON_SKILL_DIR} / ${MASON_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(MASON_SKILL_DIR|MASON_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${MASON_SKILL_DIR} / ${MASON_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "MASON_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "MASON_SESSION_ID": str(session_id) if session_id else None,
    }
