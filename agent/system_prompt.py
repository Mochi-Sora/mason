...
from agent.prompt_builder import (
    ...
    MASON_AGENT_HELP_GUIDANCE, MASON_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE, MEMORY_GUIDANCE,
...
# Custom Agent — State History instruction (per-session tiny state + indexed backup)
try:
    from agent.state_history_prompt import STATE_HISTORY_INSTRUCTION
except Exception:
    STATE_HISTORY_INSTRUCTION = ""
# Mason cold-start tool protocol (bridge + manifest + working set)
try:
    from tools.working_set import PROTOCOL_BLOCK as WORKING_SET_PROTOCOL
except Exception:
    WORKING_SET_PROTOCOL = ""
from agent import prompt_builder as _pb
from agent.runtime_cwd import resolve_context_cwd
from mason_constants import get_default_mason_root, get_mason_home
from utils import is_truthy_value

logger = logging.getLogger(__name__)
_PLUGIN_SECTION_FRAME_RE = re.compile(
    r"^## Plugin Context: (?P<id>[a-z0-9][a-z0-9._-]{0,127})\n<!-- mason-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``MASON_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("MASON_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the MASON_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from mason_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from mason_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound MASON_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_mason_root()`` (NOT ``get_mason_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_mason_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.mason/skills``
    (default) with ``~/.mason/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_mason_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_mason_root() if _agent_home_path is not None else get_mason_home())
        return (
            "Active Mason profile: default. Other profiles (if any) live "
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_mason_home())
    ...
    # default profile's data sits at the ROOT (get_default_mason_root()), which in ambient profile mode is
    # NOT get_mason_home().
    default_root = get_default_mason_root()
    return (
        f"Active Mason profile: {active_profile}. This session reads "
...
def _timestamp_line(agent: Any) -> str:
    ...
    from mason_time import get_timezone as _mason_tz, now as _mason_now
    now = _mason_now()
    _bits = _zone_bits(now, _mason_tz())
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Mason's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # mason-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(MASON_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the mason-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- mason-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = MASON_AGENT_HELP_GUIDANCE
    ...
    # Custom Agent — State History (per-session tiny state + indexed backup, injected into stable for cache warmth)
    if STATE_HISTORY_INSTRUCTION.strip():
        stable_parts.append(STATE_HISTORY_INSTRUCTION.strip())
