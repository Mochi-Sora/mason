...
from mason_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        from mason_cli.nous_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Fail-open: any hiccup or timeout → []. MASON_DEV_CREDITS_FIXTURE renders from the fixture instead of the portal."""
    try:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        if getattr(state, "paid_access", True) is False:
            details.append(_DEPLETED_LINE)
        return _nous_snapshot(windows, details, ["(dev fixture — MASON_DEV_CREDITS_FIXTURE)"], source="dev-fixture")
    except (AttributeError, TypeError):
        return None
...
def _codex_reset_outcome(body: dict, available: int) -> CodexResetRedeemResult:
    ...
    if code == "reset":
        ...
        try:
            from mason_cli.auth import clear_codex_pool_quota_cooldowns
...
def redeem_codex_reset_credit(
    *, base_url: Optional[str] = None, api_key: Optional[str] = None, force: bool = False,
    ...
    try:
        ...
        return _unavailable("No Codex credentials available. Run `mason auth` to sign in with your ChatGPT account.")
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `mason auth` and sign in with your ChatGPT account.")
