...
``auth.json`` credential pool. ``~/.mason/.anthropic_oauth.json`` (Mason PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Mason so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".mason-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "mason_pkce": lambda: _get_mason_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from mason_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("MASON_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Mason historically persisted setup tokens
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``mason models``) never mutate
...
# ── Mason-native PKCE OAuth flow (~/.mason/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_mason_oauth_file() -> Path:
    return get_mason_home() / ".anthropic_oauth.json"


def _root_mason_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        root = get_default_mason_root()
        return None if root.resolve(strict=False) == get_mason_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_mason_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Mason-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Mason with your Claude Pro/Max subscription.", "",
    ...
    try:
        from mason_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_mason_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Mason-managed OAuth credentials from ~/.mason/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_mason_oauth_file(), "Mason OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_mason_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed mason_pkce tokens to ~/.mason/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own MASON_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_mason_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Mason OAuth credentials",
    )
