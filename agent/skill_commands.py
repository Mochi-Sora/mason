...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("MASON_PLATFORM") or get_session_env("MASON_SESSION_PLATFORM")
    except Exception:
        resolved_platform = os.getenv("MASON_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Effective Mason home the scan is scoped to (profiles carry their own
    ...
    ``set_mason_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    """Append a ``[Skill config: ...]`` block with resolved ``metadata.mason.config``
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_mason_home()}/config.yaml):")
...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]], resolve_command) -> None:
    ...
    if resolve_command(cmd_name) is not None:
        logger.warning("Skill %r generates slash command '/%s' which collides with a core Mason command; "
...
def build_preloaded_skills_prompt(skill_identifiers: list[str], task_id: str | None = None) -> tuple[str, list[str], list[str]]:
    ...
    this path bypasses the scan-time filter, and ``mason -s <skill>`` must not
