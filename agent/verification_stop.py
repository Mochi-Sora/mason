...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: ``MASON_VERIFY_ON_STOP`` env var, then ``agent.verify_on_stop``
    ...
    """
    env = os.environ.get("MASON_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether ``mason verify`` has a runtime recipe here: a saved
    ``.mason/environment.json`` or a statically detected recipe with a start
    ...
    if verify_commands:
        ...
        if has_recipe:
            command_instruction += (
                ...
                "start + readiness), prefer `mason verify --json` — a passing "
                "run records verification evidence for this workspace."
            )
    elif has_recipe:
        command_instruction = (
            "No canonical test/lint/build command was detected, but the "
            "project has a runnable verification recipe. Run `mason verify "
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `mason-verify-` filename prefix, run it "
