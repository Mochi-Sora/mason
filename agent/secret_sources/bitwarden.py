...
Pulls API keys from BSM at startup so they need not live in ``~/.mason/.env``.
``bws`` is auto-installed into ``<mason_home>/bin/bws`` (one pinned version,
...
# <mason_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"mason-bws-encrypted-cache-v1"
...
def _mason_bin_dir() -> Path:
    """Where Mason stores its managed binaries. Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<mason_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="mason-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `mason secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`mason secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `mason secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `mason secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `mason secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_mason_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `mason secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `mason secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `mason secrets bitwarden setup` to install."
        )
        return result
