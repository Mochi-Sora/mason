...
``MASON_SECRET_KEY`` (never interpolated, so a hostile key name is inert); hard
...
def _run_helper(command: str, secret_key: str, timeout_seconds: float, max_output_bytes: int) -> Optional[str]:
    ...
    The key travels as DATA in ``MASON_SECRET_KEY``. stdout/stderr are piped
    ...
    env["MASON_SECRET_KEY"] = secret_key
...
class CommandSource(SecretSource):
    ...
    KEY=VALUE blob in one run. Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/mason-secrets.env"
    """
    ...
    remediation_hints = {
        ...
        ErrorKind.INTERNAL: "Run the helper manually in a shell to see its real error — "
                            "Mason discards helper stderr so diagnostics can't leak "
                            "secret material.",
    }
...
def apply_command_secrets(
    *,
    ...
    # MASON_SECRET_KEY and parse its stdout as a dotenv blob.
...
def get_command_secret(
    *,
    ...
    ``MASON_SECRET_KEY``.  Returns None on any failure — never raises."""
    command = (command or "").strip()
    if not command:
        return None
