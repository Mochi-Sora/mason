...
headless, ``OP_SESSION_*`` interactive) — Mason never authenticates on the
user's behalf, and failures never block startup. Complete pulls are cached
in-process and under ``<mason_home>/cache/op_cache.json`` (values only; auth
...
# L1 key folds in str(home_path) so a MASON_HOME switch inside one long-lived
...
def apply_onepassword_secrets(
    *, enabled: bool, env: Optional[Dict[str, str]] = None, account: str = "",
    ...
    (``mason secrets onepassword sync --apply``). Never raises. Refs already
...
class OnePasswordSource(SecretSource):
    ...
    _AUTH_HINT = ("Run `mason secrets onepassword token` to paste a fresh service-account token "
