...
env-var-shaped values at process startup, AFTER ``~/.mason/.env`` has loaded
and BEFORE the rest of Mason reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
MASON_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("mason_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `mason secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `mason secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `mason secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `mason secrets {name} setup` to re-authenticate.",
