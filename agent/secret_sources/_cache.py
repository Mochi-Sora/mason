...
a cache problem must never block Mason startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_mason_dotenv()``, else ``$MASON_HOME``/``~/.mason``."""
    if home_path is None:
        from mason_constants import get_mason_home

        home_path = get_mason_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<mason_home>/cache/<basename>``::
