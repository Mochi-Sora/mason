...
:func:`apply_all` via ``mason_cli.env_loader``; plugins register through
...
def _validate_source(source: SecretSource) -> Optional[str]:
    ...
    if source.api_version != SECRET_SOURCE_API_VERSION:
        return (f"Ignoring secret source '{name}': built against secret-source API "
                f"v{source.api_version}, this Mason speaks v{SECRET_SOURCE_API_VERSION}")
...
def _merged(scope: Optional[str]) -> Dict[str, SecretSource]:
    ...
    merged.update(_SCOPED_SOURCES.get(scope or mason_home_key(), {}))
    return merged
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(mason_home_key(), {}))
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    """Active profile name (``~/.mason/profiles/<name>``); "" for the default profile."""
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(secrets_cfg, scope=mason_home_key(home_path))
    if not enabled:
        return report
