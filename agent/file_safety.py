...
def _constants_path(getter_name: str) -> Path:
    """Call ``mason_constants.<getter_name>()`` (local import avoids cycles); ``~/.mason`` on any failure."""
    try:
        import mason_constants

        return getattr(mason_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.mason"))


def _mason_home_path() -> Path:
    """Active MASON_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_mason_home")


def _mason_root_path() -> Path:
    """Mason root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_mason_root")


def _mason_dirs() -> list[Path]:
    """Resolved active MASON_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (MASON_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_mason_home_path(), _mason_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    mason_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in mason_files for base in (_mason_home_path(), _mason_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved MASON_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("MASON_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# MASON_HOME / root subpaths that the agent's generic file tools must not
...
_MASON_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _mason_dirs():
        for sub in _MASON_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside MASON_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under MASON_HOME / <root>. The agent never
...
# Directory-prefix read denies under MASON_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Mason MCP token directory and cannot be read directly.",
     "is a Mason MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Mason real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Mason real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Mason path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under MASON_HOME and the global root (exact files, plus anything
    ...
    mason_dirs = _mason_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in mason_dirs):
        reason = (
            "is an internal Mason cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in mason_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Mason credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in mason_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Mason read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from MASON_HOME: ``~/.mason`` -> ``"default"``,
    ``~/.mason/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _mason_home_path().resolve().relative_to(_mason_root_path().resolve() / "profiles").parts
...
#   <MASON_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.mason/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Mason state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.mason`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .mason / <thing>; inner_idx = the .mason part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".mason"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Mason process never reads — the "
        "authoritative file is likely {inner_path!r} under the real MASON_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.mason/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Mason "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Mason scope, or is inside
    ...
    try:
        ...
        root_real = _mason_root_path().resolve()
