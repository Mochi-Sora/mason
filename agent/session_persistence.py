...
def _safe_session_filename_component(session_id: str) -> str:
    """Path-safe component for a (possibly untrusted ``X-Mason-Session-Id``) ID: non ``[A-Za-z0-9_-]`` → ``_``,
...
def _db_flush_failed(agent, e: Exception, batch_rows: List[Dict[str, Any]], adoption_budget: int) -> bool:
    ...
    from mason_state import StateDbCorruptError, StateDbReplacedError, classify_persistence_error, divert_session_transcript_jsonl
...
def _session_log_entry(agent, msg: Dict[str, Any]) -> Dict[str, Any]:
    """Copy of ``msg`` with scratchpad tags normalised and credentials redacted (honours MASON_REDACT_SECRETS)."""
...
class SessionPersistenceMixin:
    ...
    def _redact_message_content(content):
        """Redact secrets in str or list-of-parts content (text fields only; honours MASON_REDACT_SECRETS)."""
