...
``~/.mason/plugins/video_gen/<name>/``; mirrors ``agent/image_gen_provider.py``.
...
def save_b64_video(b64_data: str,*, prefix: str="video", extension: str="mp4") -> Path:
    """Decode base64 video data into ``$MASON_HOME/cache/videos/``; return the path."""
...
def save_url_video(
    ...
    """Download an (often ephemeral) video URL into ``$MASON_HOME/cache/videos/``;
