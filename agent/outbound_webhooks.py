...
(``X-Mason-Signature-256: sha256=<hex>`` over the raw body) when a secret is configured;
``MASON_SAFE_MODE=1`` skips registration; registration is idempotent.
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("MASON_SAFE_MODE"):
        logger.info("MASON_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def iter_configured_targets(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    """Parse ``hooks.outbound`` without registering anything (``mason hooks list``)."""
    if not isinstance(cfg, dict):
        return []
...
def _serialize_payload(event: str, kwargs: Dict[str, Any], delivery_id: str) -> bytes:
    ...
    (also the ``X-Mason-Delivery`` header) and ``timestamp`` live inside the HMAC-signed
...
def _build_delivery(event: str, target: WebhookTarget, body: bytes, delivery_id: str) -> Dict[str, Any]:
    headers = {
        "Content-Type": "application/json", "User-Agent": "Mason-Agent-Outbound-Webhook",
        "X-Mason-Event": event, "X-Mason-Delivery": delivery_id,
    }
    if target.secret:
        digest = hmac.new(target.secret.encode("utf-8"), body, hashlib.sha256).hexdigest()
        headers["X-Mason-Signature-256"] = f"sha256={digest}"
