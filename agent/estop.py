...
``mason pause`` writes a sentinel at ``$MASON_HOME/ESTOP``; ``mason resume``
...
(fail safe, e.g. ``touch ~/.mason/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.mason).
from agent.file_safety import _mason_home_path as _mason_home, _mason_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `mason pause`."""
    return _mason_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (MASON_HOME=~/.mason/profiles/<n>) must still honor an operator's ~/.mason/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Mason is paused{tag}. New work is on hold; run `mason resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `mason resume` (%s)",
