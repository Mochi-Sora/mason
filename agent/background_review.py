...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    shim declaring ``SUPPORTS_MASON_TOOL_CALLS = False`` (instance or class) is skipped — the fork
    ...
    for candidate in (client, type(client) if client is not None else None):
        supported = getattr(candidate, "SUPPORTS_MASON_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Mason, e.g. 'mason-agent').\n"
    "  • Hub-installed skills (installed via 'mason skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'mason curator pin'). You are an autonomous no-user-present "
    ...
    "recommend 'mason curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'mason skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'mason curator pin'). Pin blocks autonomous writes entirely — "
    ...
    "recommend 'mason curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("MASON_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
...
def build_cache_parity_fork(
    ...
    # prompt from scratch (fresh _mason_now() timestamp, fresh session_id, narrower toolset → different
...
def _run_review_fork(
    ...
    from mason_cli.plugins import set_thread_tool_whitelist, clear_thread_tool_whitelist
...
def _run_review_in_thread(
    ...
    # A client that can't carry Mason tool calls back would spawn a fork that cannot write
    ...
    if not _parent_can_emit_tool_calls(agent) and not _resolve_review_runtime(agent, task_cfg).get("routed"):
        logger.warning(
            "Background review skipped: provider %r cannot emit Mason tool calls, "
