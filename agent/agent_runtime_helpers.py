...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-Mason-Session-Id header) so a
        ...
        if env_var_enabled("MASON_DUMP_REQUEST_STDOUT"):
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from mason_cli.config import load_config as _load_moa_cfg
...
def _route_may_be_custom(agent, eff_provider: str, provider_lower: str, eff_base_url: str) -> bool:
    ...
    if custom_providers:
        ...
        from mason_cli.providers import custom_provider_aliases
...
def anthropic_prompt_cache_policy(
    ...
    if supports_cache_markers and (
        ...
        try:
            from mason_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.mason/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to mason's outer conversation loop, which honors Retry-After and
    ...
    if agent.provider == "opencode-free":
        from mason_cli.models import opencode_zen_free_headers
        client_kwargs["default_headers"] = {**(client_kwargs.get("default_headers") or {}), **opencode_zen_free_headers()}
    # All primary construction and recovery paths must identify Mason to the official Codex
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from mason_cli.config import load_config, get_compatible_custom_providers
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from mason_cli.providers import determine_api_mode
...
def switch_model(
    ...
    try:
        ...
        from mason_cli.config import load_config as _sm_load_config
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from mason_cli.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from mason_cli.middleware import apply_tool_request_middleware
    ...
    from mason_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from mason_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `mason "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Mason under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import MASON_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(MASON_TRANSPORT_OWNER_EXT) != owner:
                    continue
