"""``$MASON_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$MASON_HOME/cache/<kind>/``, creating parents as needed."""
    from mason_constants import get_mason_home
    path = get_mason_home() / "cache" / kind
