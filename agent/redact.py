...
# `export MASON_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in mason_cli/main.py, gateway/run.py, and cli.py) or `MASON_REDACT_SECRETS=false` in
# ~/.mason/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("MASON_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``mason config`` / ``status`` / ``dump``):
