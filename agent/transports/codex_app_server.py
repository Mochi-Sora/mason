...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Mason secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Mason secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = mason_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("MASON_KANBAN_TASK"):
            kanban_db = spawn_env.get("MASON_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("MASON_HOME", os.path.expanduser("~/.mason")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("MASON_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "mason", client_title: str = "Mason Agent",
