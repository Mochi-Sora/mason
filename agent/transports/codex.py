...
_XAI_CLIENT_WEB_SEARCH_ALIAS = "mason_web_search"
...
# Grok's native Tool Search. Aliased as mason_<name>.
...
# (mason_<name>), map back in normalize_response so Mason dispatch is unaffected. See #85589.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_XAI_RESERVED_TOOL_NAMES = ("tool_search",)
_RESERVED_TOOL_ALIAS_PREFIX = "mason_"
...
def _alias_wire_tools(response_tools: Any, params: dict[str, Any], is_xai_responses: bool) -> tuple[Any, dict[str, str]]:
    ...
    swaps it 1:1 for the built-in, client mode keeps Mason dispatch under an alias.
    """
    ...
    # xAI server-side web search vs Mason web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Mason dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``mason_web_search`` so Grok cannot hijack the name. The alias is mapped back to
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Mason-side aliases; the backend knows only the base slug.
