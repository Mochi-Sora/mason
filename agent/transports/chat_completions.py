...
_XAI_TOOL_SEARCH_ALIAS = "mason_tool_search"
...
def _rename_tool_search_bridge_for_xai(tools: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    If a real tool already holds ``mason_tool_search``, the bridge takes a ``_2``/``_3`` suffix.
    """
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    """Clamp Mason' extended effort set (``ultra``) to the OpenAI-compat wire vocabulary.

    Mason' internal effort set extends the wire vocabulary with ``ultra`` (the /reasoning command documents
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Mason/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    if not isinstance(reasoning_config, dict):
        return None
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def _normalize_tool_call(self, tc: Any) -> ToolCall | None:
        ...
        if tc_function is None or name is None:
            return None
        # Reverse only aliases THIS request emitted; a real ``mason_tool_search`` tool stays itself.
