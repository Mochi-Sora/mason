"""Mason-tools-as-MCP server for the codex_app_server runtime.

Codex owns the loop and tool list there, so a curated subset of Mason tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.mason-tools]``. Run: ``python -m agent.transports.mason_tools_mcp_server``.
"""
...
# Each name MUST match a registered Mason tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read MASON_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on MASON_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Mason tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"mason-tools MCP server requires the 'mcp' package: {exc}") from exc
    ...
    mcp = MCPServer(
        "mason-tools",
        instructions=(
            "Mason Agent's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative Mason schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Mason process", name)
            continue
        description = spec.get("description") or f"Mason {name} tool"
    ...
    logger.info("mason-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp


def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.mason_tools_mcp_server`."""
    ...
    # Keep Mason' own banners off stdout (the MCP wire).
    os.environ.setdefault("MASON_QUIET", "1")
    os.environ.setdefault("MASON_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"mason-tools MCP server cannot start: {exc}\n")
        return 2
    try:
        ...
        logger.exception("mason-tools MCP server crashed")
        sys.stderr.write(f"mason-tools MCP server error: {exc}\n")
        return 1
    return 0
