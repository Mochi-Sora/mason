...
Owns one Codex thread per Mason session: drives ``turn/start``, consumes
...
# Mason' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_MASON_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Mason session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _MASON_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("MASON_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="mason", client_title="Mason Agent", client_version=_get_mason_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Mason' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own mason-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == "mason-tools" else "decline"
...
# Mason approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Mason approval choice onto codex's approval decision wire value."""
...
def _get_mason_version() -> str:
    """Best-effort Mason version string for codex's userAgent line."""
    try:
