"""Fold an agent-as-provider's own activity back into Mason' turn state.

Agent providers (ACP CLI shims, the codex app-server) run their own tools, so that
work must never come back as pending ``tool_calls`` (Mason would re-run it) — but
...
``reasoning``. The client hands back ``mason_projected_messages`` (completed
assistant/tool rows) and ``mason_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    """
    projected = getattr(response, "mason_projected_messages", None)
    ...
    try:
        iterations = int(getattr(response, "mason_provider_tool_iterations", 0) or 0)
