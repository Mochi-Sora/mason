...
Mason treats that as a clean exit → ``rc=0`` → dispatcher ``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    """On when ``MASON_KANBAN_TASK`` is set, unless ``MASON_KANBAN_STOP_NUDGE`` disables it."""
    if (os.environ.get("MASON_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
    return bool((os.environ.get("MASON_KANBAN_TASK") or "").strip())
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("MASON_KANBAN_TASK") or "").strip() or "this task"
    return (
        "[System: You are a Mason kanban worker. A plain-text reply is NOT a "
