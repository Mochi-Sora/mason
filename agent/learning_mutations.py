...
Shared by CLI ``mason journey``, the TUI ``/journey`` overlay and the desktop.
Deleting a skill *archives* it (``mason curator restore`` recovers it);
...
def _locate_memory(node_id: str) -> tuple[Path, list[str], int]:
    ...
    path = get_mason_home() / "memories" / _MEMORY_FILES[source]
    if not path.exists():
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (mason curator unpin {name})"}
    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()
    return {"ok": ok, "message": f"archived '{name}' — restore with: mason curator restore {name}" if ok else message}
