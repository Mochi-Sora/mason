...
class RateLimitCreditsMixin:
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        depletion-notice bug cannot vanish silently. MASON_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset MASON_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("MASON_DEV_CREDITS"))
        ...
        if dev:
            # MASON_DEV_CREDITS streams each capture to agent.log (`mason logs -f`, grep 'credits ▸').
