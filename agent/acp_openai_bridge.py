"""OpenAI-shape bridge shared by Mason' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Mason' tool schemas travel INTO the
...
read/edit/execute tools forwards only Mason' agent-level tools, since re-offering overlapping ones
makes Mason redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``mason_projected_messages``)
    Mason reads off the ``create`` result; a plain list would drop them on the stream path."""
