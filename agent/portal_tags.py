...
Every Mason request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=mason-agent", "client=mason-client-v<__version__>"]``.
The version is read live from ``mason_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("mason_affinity_scope", default=None)
...
def mason_client_tag() -> str:
    """``client=mason-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if mason_cli is unimportable)."""
    try:
        from mason_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=mason-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=mason-agent", mason_client_tag()]
