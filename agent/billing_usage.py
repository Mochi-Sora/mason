...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``MASON_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``MASON_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
    name = (os.getenv("MASON_DEV_CREDITS_FIXTURE") or "").strip().lower()
