...
``skill_manage`` per the Mason authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Mason skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Mason`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Mason.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.mason.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Mason tools.
...
Mason-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Mason tools by name in backticks: `terminal`, `read_file`,
