...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    ``mason_cli.models``' pricing cache (filled by the model picker; a miss never fetches). Fail-open to
    ...
    if not base_url:
        return False
    try:
        from mason_cli.models import _is_model_free
        from mason_cli.models_pricing import peek_cached_pricing
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning("credits header version %d unsupported, ignoring — update Mason", version_val)
            return None
        fields: dict[str, Any] = {
...
# ── Dev fixtures (MASON_DEV_CREDITS_FIXTURE): throwaway scaffolding to trigger any notice state
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Fixture CreditsState for MASON_DEV_CREDITS_FIXTURE, or None (unknown name / unset). Prod-leak guard:
    applies ONLY when MASON_DEV_CREDITS is also on, so a stray fixture env var never surfaces fabricated balances."""
    name = os.environ.get("MASON_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("MASON_DEV_CREDITS")):
        return None
