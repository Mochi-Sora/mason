...
class MonitoringEmitter:
    ...
    def _ensure_started(self) -> None:
        ...
        with self._lock:
            if self._started:
                return
            self._thread = threading.Thread(target=self._run, name="mason-monitoring-dispatch", daemon=True)
            self._thread.start()
            self._started = True
    ...
    def flush(self, timeout: float = 2.0) -> None:
        ...
        threading.Thread(target=_wait_for_completion, name="mason-monitoring-flush", daemon=True).start()
