...
_OBSERVABLE_METRIC_NAMES = (
    "mason.gateway.up", "mason.gateway.state", "mason.gateway.active_agents", "mason.gateway.busy",
    "mason.gateway.drainable", "mason.gateway.restart_requested", "mason.gateway.background_work",
    "mason.gateway.background_delegations", "mason.platform.up", "mason.platform.degraded",
    "mason.cron.scheduler.heartbeat_age_seconds", "mason.cron.scheduler.last_success_age_seconds",
    "mason.cron.scheduler.catch_up_occurrences", "mason.cron.jobs.enabled", "mason.cron.jobs.running",
    "mason.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="mason-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("mason.gateway.background_work", _read_background_work_count),
            ("mason.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("mason.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("mason.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'mason-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="mason-gateway-health-export", daemon=True)
            thread.start()
