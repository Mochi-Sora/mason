...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "mason.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("mason.gateway.up", int(bool(gateway_running))),
        metric("mason.gateway.active_agents", active_agents),
        metric("mason.gateway.busy", int(bool(busy))),
        metric("mason.gateway.drainable", int(bool(drainable))),
        metric("mason.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("mason.gateway.state", 1, **{"mason.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"mason.platform": str(platform), "mason.platform.state": state}
        metrics.append(metric("mason.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("mason.platform.degraded", int(is_degraded), **pattrs, **{"mason.error_code": error_code}))
        if is_degraded:
