...
from mason_time import now as _now
...
def _job_metrics(metrics: list[GatewayMetric]) -> None:
    ...
    metrics.append(GatewayMetric("mason.cron.jobs.enabled", len(enabled), {}))
    metrics.append(GatewayMetric("mason.cron.jobs.overdue", sum(1 for job in enabled if _is_overdue(job, _now())), {}))
...
_METRIC_GROUPS: tuple[tuple[Callable[[list[GatewayMetric]], None], str], ...] = (
    (_freshness_metric("mason.cron.scheduler.heartbeat_age_seconds", lambda: get_ticker_heartbeat_age()), "cron freshness metric unavailable"),
    (_freshness_metric("mason.cron.scheduler.last_success_age_seconds", lambda: get_ticker_success_age()), "cron freshness metric unavailable"),
    (_single_metric("mason.cron.scheduler.catch_up_occurrences", lambda: get_catch_up_occurrence_count()), "cron catch-up metric unavailable"),
    (_job_metrics, "cron job metrics unavailable"),
    (_single_metric("mason.cron.jobs.running", lambda: len(get_running_job_ids())), "cron running-job metric unavailable"),
)
