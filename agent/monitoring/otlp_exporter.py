...
``gateway_health_export``.  The OTel SDK is an optional extra (``mason-agent[otlp]``) imported
...
def _runtime_resource_attributes(config: Dict[str, Any], *, telemetry_scope: str) -> Dict[str, str]:
    ...
    attrs["service.name"] = "mason-gateway"
...
def _allowlisted_attrs(ev: Dict[str, Any], keys: Iterable[str]) -> Dict[str, Any]:
    """``mason.<key>`` attributes for present keys; string values are redacted and bounded."""
    attrs: Dict[str, Any] = {}
    for col in keys:
        v = ev.get(col)
        if v is not None:
            attrs[f"mason.{col}"] = redact_bounded(v) if isinstance(v, str) else v
    return attrs
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"mason.event": kind or "unknown"}
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("mason.monitoring")
    n = 0
    for ev in batch:
        try:
            tracer.start_span(f"mason.{ev.get('event', 'event')}", attributes=_span_attrs(ev)).end()
            n += 1
        except Exception:
...
def start_streaming(
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'mason-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
