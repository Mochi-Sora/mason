...
class EntraIdentityConfig:
    """Mason-managed Entra knobs; everything else (tenant, SP secret, federated token file, authority...) flows
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    block at a time. Only Mason knobs are passed as kwargs; the rest comes from ``AZURE_*`` env vars."""
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning("Bearer hook: Entra ID token provider returned empty (%s) "
                           ...
                           "Run `mason doctor` or `az login` to recover.", exc)
