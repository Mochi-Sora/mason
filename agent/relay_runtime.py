"""Profile-scoped NeMo Relay runtimes owned by the Mason agent core."""
...
from mason_cli.relay_plugin_cutover import (RELAY_PLUGINS_CONFIG_ENV, configured_legacy_relay_env_vars)

logger = logging.getLogger(__name__)

SESSION_SCOPE = "mason.session"
TURN_SCOPE = "mason.turn"
LOGICAL_LLM_SCOPE = "mason.logical_llm_call"
RUNTIME_SCHEMA_KEY = "mason.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "mason.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "mason.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "mason.nemo_relay.plugins"
...
def runtime_metadata(runtime_id: str, **extra: Any) -> dict[str, Any]:
    """Return the scope metadata that stamps every Mason-owned Relay scope."""
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Mason session."""
    ...
    def run_in_session(self, session: RelaySession, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._runtime is None:
                raise RuntimeError("Mason Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def __init__(self) -> None:
        ...
        self._relay: Any = None  # set while a Mason-owned configuration is active
        ...
        with self._lock:
            if not self._owners:
                ...
                if self._state is _RelayPluginConfigurationState.ACTIVE:
                    logger.info(
                        "Relay plugins are active process-wide and apply to all profiles hosted by this Mason process."
                    )
    ...
    def _activate(self, relay: Any) -> _RelayPluginConfigurationState:
        try:
            ...
            logger.warning("Mason Relay plugin initialization failed: %s", exc, exc_info=True)
    ...
    def _preflight(self, relay: Any) -> _RelayPluginConfigurationState | None:
        ...
        if self._relay is not None and not self._clear_active():
            logger.warning(
                "Mason Relay plugin cleanup is still pending; refusing to replace the process-global configuration"
            )
            return _RelayPluginConfigurationState.FAILED
        try:
            ...
            logger.warning(
                "Mason could not determine whether a process-global Relay plugin configuration is already "
                "active; refusing to replace it", exc_info=True,
            )
            return _RelayPluginConfigurationState.FAILED
        if existing_report is not None:
            logger.warning(
                "A process-global Relay plugin configuration is already active outside Mason native ownership; "
                "leaving it unchanged and disabling Mason-managed Relay middleware for this process"
            )
    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Mason Relay dynamic plugin activation failed") from exc
    ...
    def _clear_active(self) -> bool:
        ...
        for what, step in (
            ...
            try:
                ...
                logger.warning("Mason Relay plugin %s failed", what, exc_info=True)
                return False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Mason-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    session, self.relay.scope.pop, old_handle, output={"mason.session.segment_reason": reason},
                    metadata=runtime_metadata(self.runtime_id), timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning(
                    "Mason Relay segment close failed (session=%s segment=%d); abandoning the old segment span",
                    session.session_id, session.segment - 1, exc_info=True,
                )
            scope_metadata = runtime_metadata(
                self.runtime_id, **{"mason.session.segment": session.segment, "mason.session.segment_reason": reason},
            )
            try:
                self._open_session_scope(session, scope_metadata, resolve_parent=False)
            except Exception:
                logger.warning(
                    "Mason Relay segment open failed (session=%s segment=%d); keeping the prior scope handle",
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Mason Relay session without creating one."""
        with self._sessions_lock:
    ...
    def _session_context(self, session: RelaySession, *, allow_closing: bool) -> contextvars.Context:
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Mason Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Mason Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Mason Relay runtime is shutting down")
    ...
    def apply_tool_request_intercepts(self, *, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        """Apply Relay request rewriting before Mason authorizes a tool call."""
    ...
    def _pop_with_drain(
        ...
        for _ in range(drain_limit):
            ...
            if session_root is not None and _same_handle(top, session_root) and handle is not session_root:
                break
            try:
                orphan_output = {"outcome": "cancelled", "mason.orphan_drain": True}
                pop_relay_scope(self.relay, top, output=orphan_output, metadata=metadata)
                drained += 1
            except Exception:
                logger.warning("Mason Relay orphaned scope drain failed", exc_info=True)
                break
        if drained:
            logger.warning("Mason Relay drained %d orphaned scope(s) before closing %s", drained, handle)
        try:
            pop_relay_scope(self.relay, handle, output=output, metadata=metadata)
            return None
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        with self._sessions_lock:
            ...
            self._forget_subagent(session_id)
        if failure:
            logger.warning("Mason Relay session %s closed with errors: %s", session_id, failure)
    ...
    def shutdown(self) -> None:
        ...
        thread = threading.Thread(
            ...
            name=f"mason-nemo-relay-shutdown-{self.runtime_id[:8]}", daemon=True,
        )
        try:
            thread.start()
        except Exception:
            ...
            logger.warning("Mason Relay deferred shutdown could not start", exc_info=True)
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Mason Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Mason profile."""
    ...
    def for_profile(self, profile_key: str | None = None, *, create: bool = True) -> RelayHost | None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Mason Relay runtime initialization failed", exc_info=True)
...
class RelayTurnContext:
    """Runtime-only context for one Mason turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "mason_relay_turn", default=None
)

# >0 while the native pipeline is mid-dispatch of a Mason tool/LLM callback. Nested managed
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "mason_relay_managed_callback_depth", default=0
)
...
def _warn_on_error(what: str, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Run fail-open telemetry work: log ``Mason Relay <what> failed`` and return None on error."""
    try:
        return callback(*args, **kwargs)
    except Exception:
        logger.warning("Mason Relay %s failed", what, exc_info=True)
        return None
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Mason core."""
    ...
    def _prepare_session(self, host: RelayRuntime, context: dict[str, Any]) -> None:
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning("Mason Relay session initializer failed: %s", name, exc_info=True)
    ...
    def _open_conversation_session(self, host: RelayRuntime, context: dict[str, Any]) -> RelaySession | None:
        ...
        metadata = {"mason.execution_surface": context["platform"] or "unknown"}
    ...
    def begin_turn(self, lease: ConversationLease, *, turn_id: str, task_id: str) -> RelayTurnContext:
        if lease.released:
            raise RuntimeError("Mason Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            if self._active_turns.get(key):
                ...
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Mason turn %s in session %s",
        ...
        if host is not None:
            ...
            turn.handle = _warn_on_error(
                ...
                metadata=runtime_metadata(host.runtime_id, **{"mason.execution_surface": lease.platform or "unknown"}),
    ...
    def _close_turn_scope(self, host: RelayRuntime, turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        if failure:
            logger.warning("Mason Relay turn finalization failed: %s", failure)
    ...
    def _finish_logical_calls(turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        while logical_calls:
            ...
            logger.warning("Mason Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(session_id: str) -> tuple[RelayRuntime | None, RelaySession | None, Any]:
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0 or not relay_instrumentation_enabled():
        ...
        # ``tools.execute`` pipeline is mid-dispatch of a Mason tool). Nested managed execution here is
...
def apply_tool_request_intercepts(*, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    """Return Relay-rewritten arguments at Mason's authorization boundary."""
...
def get_runtime(*, create: bool = True, profile_key: str | None = None) -> RelayRuntime | None:
    """Return the Relay host for the active Mason profile."""
...
def current_profile_key() -> str:
    ...
    home = get_mason_home().expanduser()
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Mason [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Mason Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    return _run_on_daemon_thread(lambda: asyncio.run(value), name="mason-nemo-relay-plugin-lifecycle")
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Mason session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Mason Relay mark failed: %s", name, exc_info=True)
        return False

def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Mason core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Mason Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Mason session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Mason Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Mason Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Mason session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Mason Relay runtime is unavailable")
    session = runtime.get_session(session_id)
