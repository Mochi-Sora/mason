"""Public, plugin-safe lifecycle API for delegated Mason subagents: immutable contracts, not ``AIAgent``
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="mason-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar("mason_subagent_lifecycle_parent", default=None)
...
_REQUEST_REJECTIONS: tuple[tuple[Callable[[Any], bool], str], ...] = (
    ...
    (lambda r: r.working_directory is not None,
     "working_directory is not supported because Mason delegates use isolated task environments."),
    (lambda r: bool(r.blocked_tools),
     "Per-tool blocking is not supported; use allowed_toolsets. Mason always blocks unsafe child tools."),
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError("No active Mason parent session is available.")
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Mason failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
