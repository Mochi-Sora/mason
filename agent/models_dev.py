...
thread refreshes) → disk cache (~/.mason/models_dev_cache.json, any age) →
...
# Mason provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # opencode-free is Zen-hosted (mason_cli/models.py) and models.dev's "opencode" catalog lists
    ...
    # Mason provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Mason ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_mason_ids(mdev_id: str) -> List[str]:
    """Return the Mason provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for mason_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(mason_id)
...
def _mason_path(name: str) -> Path:
    from mason_constants import get_mason_home
    return get_mason_home() / name


def _get_cache_path() -> Path:
    return _mason_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _mason_path("models_dev_cache.etag")
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Mason provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Mason
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Mason OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Mason → models.dev id) and reverse (caller passed a models.dev id, config keyed by Mason id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_mason_ids(provider_key)]
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Mason or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Mason or models.dev provider ID (exact match, then case-insensitive), or
