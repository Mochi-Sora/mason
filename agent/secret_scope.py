...
_GLOBAL_ENV_EXACT = frozenset({
    # Mason runtime / deployment
    "MASON_HOME", "MASON_PROFILE", "MASON_GATEWAY_LOCK_DIR",
    "MASON_MAX_ITERATIONS", "MASON_MAX_TOKENS", "MASON_API_TIMEOUT",
    "MASON_REDACT_SECRETS", "MASON_NOUS_TIMEOUT_SECONDS",
    "_MASON_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "MASON_KANBAN_DB", "MASON_KANBAN_WORKSPACES_ROOT", "MASON_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MASON_KANBAN_",
    "MASON_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from mason_cli.config import _parse_env_value
...
def build_profile_secret_scope(mason_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(mason_home) / ".env")
    try:
        from mason_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(mason_home))
    except Exception:
        external_secrets = {}
