"""Lightweight i18n for Mason' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``MASON_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
def _locales_dir() -> Path:
    """Locale dir: ``MASON_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.

    The source path is returned even when missing so ``_load_catalog`` can log
    the path it looked at rather than raise.
    """
    override = os.getenv("MASON_BUNDLED_LOCALES", "").strip()
    if override and Path(override).is_dir():
        return Path(override)
    if override:
        logger.warning(
            "MASON_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("MASON_LANGUAGE")
