...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"MasonAgent/{_MASON_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("mason_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "mason_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from mason_cli import auth as _auth
            resolve = (
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            ...
            timeout = env_float("MASON_NOUS_TIMEOUT_SECONDS", 15)
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.mason/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from mason_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from mason_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
