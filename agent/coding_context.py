...
In a code workspace on an interactive surface Mason adopts a **coding posture**: a
...
from mason_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("mason.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "mason", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# Operating brief. Tool names referenced here are in the coding toolset and _MASON_CORE_TOOLS.
...
def _agent_config_value(config: Optional[dict[str, Any]], key: str, default: Any, *, readonly: bool) -> Any:
    ...
    if config is None:
        try:
            from mason_cli.config import load_config, load_config_readonly
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        apply it only when the user hasn't pinned a selection (``--toolsets``, ``MASON_TUI_TOOLSETS``)."""
...
def is_coding_context(*, platform: Optional[str] = None, cwd: Optional[str | Path] = None, config: Optional[dict[str, Any]] = None) -> bool:
    """Whether Mason should operate in its coding posture right now."""
