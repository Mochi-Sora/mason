...
def _fm_field(fm: dict[str, Any], key: str) -> Any:
    """Top-level ``key`` or ``metadata.mason.<key>``; tolerant of the string-valued
    ...
    return mason.get(key) if isinstance(mason, dict) else None
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return json.loads((get_mason_home() / "skills" / ".usage.json").read_text(encoding="utf-8"))
        except Exception:
            return {}
...
def build_learning_graph() -> dict[str, Any]:
    ...
    roots = [("base", Path(__file__).resolve().parent.parent / "skills"), ("profile", get_mason_home() / "skills")]
    learned_skills = {
