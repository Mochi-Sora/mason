...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Mason finds ``~/.openclaw/``: migrate first, cleanup (which breaks OpenClaw) after."""
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Mason, run `mason claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, run `mason claw cleanup` "
