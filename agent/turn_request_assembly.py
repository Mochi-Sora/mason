...
def assemble_api_request(
    ...
    # Mason working set: tools the session already uses ride as native schemas;
    # everything else stays deferred behind the bridge (cold start ≈ clarify + 3).
    # Growing the list mid-session re-fills the tool cache slice — amortized over
    # the session (a handful of growth turns), unlike history which never shrinks.
    try:
        from tools import working_set as _ws
        _sid = getattr(agent, "session_id", None) or "default"
        _vnames = getattr(agent, "valid_tool_names", None)
        _vset = _vnames if isinstance(_vnames, set) else None
        if _vset is not None:
            for _n in _ws.get(_sid):
                _vset.add(_n)
        _have = {t.get("function", {}).get("name")
                 for t in (tools_for_api or []) if isinstance(t, dict)}
        _ws_defs = _ws.resolve_for_api(_sid, exclude=frozenset(_have))
        # Heuristic pre-inject from the latest user text (deterministic, free,
        # request-local: joins the set only if the model actually calls it).
        _um = original_user_message
        if isinstance(_um, dict):
            _um = _um.get("content", "")
        _hnames = [n for n in _ws.heuristic_tools(_um if isinstance(_um, str) else "")
                   if n not in _have]
        if _hnames:
            try:
                from tools.registry import registry as _reg
                for _hd in _reg.get_definitions(set(_hnames), quiet=True) or []:
                    _hn = _hd.get("function", {}).get("name") if isinstance(_hd, dict) else None
                    if _hn and _hn not in _have:
                        _ws_defs.append(_hd)
                        _have.add(_hn)
                        if _vset is not None:
                            _vset.add(_hn)
                ...
                pass
        if _ws_defs:
            tools_for_api = list(tools_for_api or []) + _ws_defs
            for _t in _ws_defs:
                _nm = _t.get("function", {}).get("name") if isinstance(_t, dict) else None
                if _nm and _vset is not None:
                    _vset.add(_nm)
        # 1B predictor, fire-and-forget: schemas land before the model asks.
        try:
            _ut = _um if isinstance(_um, str) else ""
            if _ut.strip():
                _ws.predict_async(_sid, _ut, f"{_sid}:{current_turn_user_idx}")
        except Exception:
            pass
    except Exception:
        pass
