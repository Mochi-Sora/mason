...
import mason_cli.auth as auth_mod
...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``mason_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
        from mason_cli.auth import is_source_suppressed
...
# singleton to re-seed from and the user can re-add via ``mason auth add``.
...
# selection. On Windows several Mason processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Mason backend after 15000ms"). Logging the condition at
...
def custom_provider_pool_key_candidates(
    ...
    ``mason auth add <key>`` stores ``providers.<key>`` credentials under the
...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.mason/auth.json`` even when MASON_HOME
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".mason" / "auth.json"
            try:
...
def _write_through_provider_state_to_global_root(
    ...
    ``mason_cli.auth._write_through_xai_oauth_to_global_root``.
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    """Root ``.anthropic_oauth.json`` when *entry* is a borrowed mason_pkce row, else None."""
    if entry.source != "mason_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
        from agent.anthropic_credentials import _root_mason_oauth_file
        return _root_mason_oauth_file()
    except Exception:
        return None
...
# ``mason_cli.auth`` refresh function and terminal-error predicate (looked
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "MASON_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "MASON_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool:
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``mason model`` / ``mason auth`` login writes new tokens
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Mason processes, or two processes adopt the same on-disk
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        For ``claude_code`` / ``mason_pkce`` the singleton file — not
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "MASON_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). mason_pkce -> ~/.mason/.anthropic_oauth.json
        ...
        ``endswith``: manual:mason_pkce is pool-owned and a singleton for it
        would be a second authority for the same refresh-token family.
        """
        if entry.source == "claude_code":
            store = "~/.claude/.credentials.json"
        elif entry.source == "mason_pkce":
            store = "~/.mason/.anthropic_oauth.json"
        else:
            return
        try:
            ...
            if entry.source == "claude_code":
                ...
                ac._write_mason_oauth_credentials(*args, target=_singleton_target_for_entry(self, entry))
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("mason_pkce"))
        updated = replace(
            entry,
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            if entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("mason_pkce"),
                        )
                ...
                # Backstop for pool-owned sources (mason_pkce, manual:dashboard_pkce):
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        The user may have re-authed (``mason model`` / ``mason auth``, the
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `mason auth add %s`",
    ...
    def add_entry(self, entry: PooledCredential) -> PooledCredential:
        with self._lock:
            ...
            if borrowed_ids:
                # ``mason -p <profile> auth add <single-use provider>``: the
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``mason auth remove`` must stay stable across loads)."""
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    # Only auto-discover external credentials (Claude Code, Mason PKCE) when
    ...
    # API-key vs OAuth is a user-visible choice at `mason setup`. The API-key
    ...
    # ~/.mason/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"mason_pkce", "claude_code"})
        return

    from agent.anthropic_credentials import (
        read_claude_code_credentials,
        read_mason_oauth_credentials,
    )

    for source_name, creds in (
        ("mason_pkce", read_mason_oauth_credentials()),
...
def _seed_nous_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    if not (state and has_runtime_material):
        return
    # Prefer a user-supplied label embedded in the singleton state (``mason
...
def _seed_copilot_singleton(seed: _Seeder) -> None:
    ...
    try:
        from mason_cli.copilot_auth import (
            COPILOT_ENV_VARS,
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from mason_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Mason owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `mason auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        # `mason auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.mason/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "mason auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _env_payload(*, env_var: str, token: str, base_url: str) -> Dict[str, Any]:
    ...
    try:
        from mason_cli.env_loader import get_secret_source
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `mason auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons and Mason PKCE disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "mason_pkce"
