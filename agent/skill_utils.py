...
from mason_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("MASON_KANBAN_TASK") or os.getenv("MASON_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no mason_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at MASON_HOME; absolute paths pass through."""
    from mason_constants import get_mason_home
    return p if p.is_absolute() else get_mason_home() / p


# Never disableable: `mason-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"mason-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``MASON_PLATFORM`` / ``MASON_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("MASON_PLATFORM") or get_session_env("MASON_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``mason config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to MASON_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to MASON_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_mason_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.mason/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.mason/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".mason", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (MASON_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under MASON_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`mason skills trust`)
...
# high-confidence findings only. The scan cache lives under MASON_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from mason_constants import get_mason_home
        cache_dir = get_mason_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped MASON_HOME), so normalization
    ...
    # ~/.mason/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _mason_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.mason`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    mason = metadata.get("mason") if isinstance(metadata, dict) else None
    return mason if isinstance(mason, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    mason = _mason_metadata(frontmatter)
    return {key: mason.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.mason.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _mason_metadata(frontmatter).get("config")
