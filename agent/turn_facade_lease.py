...
def admit_durable_turn_lease(
    ...
    def _on_wait(elapsed: float) -> None:
        ...
        waited = True
        agent._emit_status(
            "⏳ Another Mason process is using this session; "
            "waiting for it to finish before starting your turn..."
            if elapsed < 1.0 else
            f"⏳ Still waiting for the other Mason process on this session ({int(elapsed)}s)..."
        )
...
def _lease_not_acquired_result(agent, session_id: str, conversation_history) -> Dict[str, Any]:
    ...
    if getattr(agent, "_interrupt_requested", False):
        ...
        result = {
            "final_response": (
                "Stopped waiting for another Mason process on this session. "
    ...
    timeout_msg = (
        "⏳ Another Mason process kept this session busy too long. Your message was not "
