...
from mason_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
_SENSITIVE_MASON_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, mason_home = Path(os.path.expanduser("~")).resolve(), get_mason_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {mason_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [mason_home / rel for rel in _SENSITIVE_MASON_DIRS]
    if path in blocked_exact:
        raise ValueError("path is a sensitive credential file and cannot be attached")
    if any(_is_under(path, blocked_dir) for blocked_dir in blocked_dirs):
        raise ValueError("path is a sensitive credential or internal Mason path and cannot be attached")
