...
# long"). Codex message ids can run 400+ chars; Mason ``msg_...`` ids stay under the cap.
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Mason threading encrypted reasoning back across turns for
    ...
    Mason' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
