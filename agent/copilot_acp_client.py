"""OpenAI-compatible shim that forwards Mason requests to `copilot --acp`.
...
_PROMPT_PREAMBLE = (
    "You are being used as the active ACP agent backend for Mason.",
...
_INITIALIZE_PARAMS = {
    ...
    "clientInfo": {"name": "mason-agent", "title": "Mason Agent", "version": "0.0.0"},
}
_DEPRECATED_CLI_ERROR = (
    "Mason ACP mode requires the NEW GitHub Copilot CLI (github.com/github/copilot-cli), but the binary it just "
    "spawned is the deprecated `gh copilot` extension.\n\n"
    "Install the new CLI:\n  npm install -g @github/copilot\n  # then verify with: copilot --help\n\n"
    "If `copilot` already resolves to the new CLI but you still see this,\npoint Mason at it explicitly:\n"
    "  export MASON_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
    "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\ndirectly with a Copilot subscription "
    "token) via `mason setup`.\n\nOriginal error:\n"
)
...
def _resolve_command() -> str:
    return os.getenv("MASON_COPILOT_ACP_COMMAND", "").strip() or os.getenv("COPILOT_CLI_PATH", "").strip() or "copilot"


def _resolve_args() -> list[str]:
    return shlex.split(os.getenv("MASON_COPILOT_ACP_ARGS", "").strip()) or ["--acp", "--stdio"]
...
def _build_subprocess_env() -> dict[str, str]:
    ...
    # See #29157.
    env = mason_subprocess_env(inherit_credentials=True)
...
def _format_messages_as_prompt(
    ...
    # one. Copilot has no tools of its own that collide with Mason', so forward the whole toolset.
...
class CopilotACPClient:
    ...
    MASON_SKIP_TRANSPORT_WRAP = True
    MASON_SKIP_ASYNC_WRAP = True
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                "MASON_COPILOT_ACP_COMMAND / MASON_COPILOT_ACP_ARGS to a working pair."
            )
        try:
            from mason_cli._subprocess_compat import windows_hide_flags  # hide the Windows console flash (#56747); pipes intact for the ACP wire
            ...
            raise RuntimeError(f"Could not start Copilot ACP command '{self._acp_command}'. Install GitHub Copilot CLI or set "
                               "MASON_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.") from exc
    ...
    def _handle_server_message(
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(message_id, -32601, f"ACP client method '{method}' is not supported by Mason yet.")
