...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Mason only
    initiates compaction in "mason" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Mason preflight compression for codex app-server "
            "(mode=%s); Mason will not start thread compaction here.",
