...
YAML files in ``<MASON_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``MASON_BUNDLES_DIR`` override (tests) or ``<MASON_HOME>/skill-bundles``."""
    override = os.environ.get("MASON_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_mason_home() / "skill-bundles"
...
# File-level CRUD — used by `mason bundles`.
