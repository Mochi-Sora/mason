...
``_mason_backend_name`` on the result so file-path resolution can identify plugin backends.
"""
...
class TerminalEnvironmentProvider(ProviderBase):
    ...
    * ``cache_path_base`` — where auto-synced ``~/.mason/cache`` files land inside the backend
      (``"~/.mason"``, ``"/root/.mason"``), or ``None`` when host paths remain correct.
    ...
    * ``session_isolated_when_nonpersistent`` — non-persistent mode gives each session its own
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``mason setup`` after selection (the wizard persists ``terminal.backend`` itself)."""
        return []

    def post_setup(self) -> None:
        """Optional interactive hook run by ``mason setup`` after selection (prompt for tokens, install SDKs)."""

    def doctor_checks(self) -> List[Tuple[bool, str, str]]:
        """``mason doctor`` rows ``(ok, label, detail)``; default reflects :meth:`is_available`."""
        try:
