...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("mason_summary_route_pin", default=None)
)
...
# (mason_state._rows_to_conversation), so any code that mutates a loaded or flushed dict's content in place
...
def _today_for_prompt() -> str:
    ...
    try:
        ...
        # via mason_time.now(). The compaction summary is a mid-conversation message that is NOT part of
        ...
        return _mason_now().strftime("%Y-%m-%d")
