"""Upload a Mason session transcript to Hugging Face as an agent trace, re-emitted in the **Claude Code
...
DEFAULT_DATASET_NAME = "mason-traces"
_MASON_VERSION = "mason-agent"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.mason/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `mason trace upload`)."
)
...
# --- Conversion: Mason OpenAI-format messages -> Claude Code JSONL ---
...
def build_trace_jsonl(messages: List[Dict[str, Any]], *, session_id: str, model: str = "", cwd: str = "", redact: bool = True) -> str:
    ...
    for msg in messages:
        ...
        entry = {  # key order is the wire order
            ...
            "sessionId": session_id, "version": _MASON_VERSION, "gitBranch": git_branch, "uuid": turn_uuid,
...
def upload_session_trace(
    ...
    """CLI/gateway entry point: load, convert, upload to ``{user}/mason-traces``. Status string, never raises."""
    if not session_id:
