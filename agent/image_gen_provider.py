...
``<repo>/plugins/image_gen/<name>/`` or ``~/.mason/plugins/image_gen/<name>/``);
...
def save_b64_image(b64_data: str, *, prefix: str = "image", extension: str = "png") -> Path:
    """Decode base64 image data into ``$MASON_HOME/cache/images/``; return the path."""
...
def save_url_image(
    ...
    """Download an (often ephemeral) image URL into ``$MASON_HOME/cache/images/``. Raises on
