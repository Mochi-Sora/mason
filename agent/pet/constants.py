...
inferred from the atlas shape so Mason renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Mason names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Mason/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Mason names -> accepted row-name aliases in descending preference.
