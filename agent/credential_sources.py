"""Unified removal contract for every credential source Mason reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``mason auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.mason/.env; hint when the shell exports it."""
    from mason_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like mason_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Mason.",
            f"  The pool entry is now suppressed — Mason will ignore "
            f"{env_var} until you run `mason auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_mason_pkce(provider: str, removed) -> RemovalResult:
    """~/.mason/.anthropic_oauth.json is ours — delete it outright."""
    from mason_constants import get_mason_home

    result = RemovalResult()
    oauth_file = get_mason_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Mason Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `mason model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `mason auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `mason auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `mason auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="mason_pkce",
        remove_fn=_remove_mason_pkce,
        description="~/.mason/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `mason auth add qwen-oauth` to re-enable if needed.",
        ),
