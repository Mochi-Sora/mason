...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    ``MASON_CONCURRENT_TOOL_TIMEOUT_S`` is the legacy bridge, ``0``/negative disables."""
    from agent.deadline import resolve_timeout

    return resolve_timeout(
        "tools.concurrent_batch",
        default=_DEFAULT_CONCURRENT_TOOL_TIMEOUT_S,
        env_var="MASON_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
class _ConcurrentToolAuthorizationGate:
    ...
    def run(self, callback):
        if not self._serialization_lock.acquire(timeout=self._lock_timeout):
            ...
            # ``mason mcp login``. Parking keeps the task alive so the 300s self-probe (and an explicit
...
def _dispatch_authorized_once(
    ...
    *,
    ...
    """Mason policy (scope → plugin pre-hooks → guardrails) then the one real dispatch.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Mason policy and dispatch exactly once."""
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state.dispatched:
                raise RuntimeError("Mason tool execution callback invoked more than once")
    ...
    def _mason_pipeline(relay_args: dict[str, Any]) -> Any:
