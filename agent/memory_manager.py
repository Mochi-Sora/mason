...
def _ctx_bound(fn: Callable[[], Any]) -> Callable[[], Any]:
    ...
    ContextVar-scoped MASON_HOME override, and an unbound worker would silently use the default profile."""
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in provider.get_tool_schemas():
            ...
            if tool_name in _MASON_CORE_TOOLS:
                logger.warning(
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _MASON_CORE_TOOLS and schema["name"] not in seen:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``mason_home`` so they resolve profile-scoped paths."""
        if "mason_home" not in kwargs:
            from mason_constants import get_mason_home
            kwargs["mason_home"] = str(get_mason_home())
