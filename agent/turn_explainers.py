...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Mason process "
    ...
    "locked": (
        ...
        "(another Mason process was writing to the state "
    ...
    "replaced": (
        ...
        "`mason doctor --fix` or in-place FTS repair — stop "
    ...
    "corrupt": (
        ...
        "   mason sessions recover --source {db_path} --inspect-only\n"
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    ...
    "Check the state database health (`mason doctor`), then "
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``mason_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # Mason-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``MASON_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="MASON_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``MASON_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="MASON_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
