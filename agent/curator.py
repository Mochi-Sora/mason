...
def _state_file() -> Path:
    return get_mason_home() / "skills" / ".curator_state"
...
def _read_config_section(*path: str, label: str, log: logging.Logger = logger) -> Dict[str, Any]:
    """Read a nested section of ~/.mason/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    """Bundled built-ins are curation candidates (ON by default); a suppression list keeps them archived across `mason update` re-seeds. Hub skills are never pruned."""
    return bool(_load_config().get("prune_builtins", True))


def get_consolidate() -> bool:
    """LLM consolidation pass — OFF by default (prune only, no aux-model fork); ``mason curator run --consolidate`` overrides per invocation."""
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``mason curator run`` bypasses this; the idle check is the caller's."""
    ...
    if last is None:
        try:
            ...
            state["last_run_summary"] = "deferred first run — curator seeded, will run after one interval; use `mason curator run --dry-run` to preview now"
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Mason' background skill CURATOR. This is an "
    ...
    "into ~/.mason/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: mason-config-*, mason-dashboard-*, gateway-*, codex-*, "
    ...
    "`mason curator rollback` restores a hollow skill (issue #96962).\n\n"
...
CURATOR_PRUNE_BUILTINS_NOTE = (
    ...
    "delete). It will be restored on `mason update` only if "
...
def _reports_root() -> Path:
    """``~/.mason/logs/curator/`` (telemetry next to agent.log, not under skills/). mkdir'd here too so gateway-only / bare-library entry paths work."""
    root = get_mason_home() / "logs" / "curator"
    try:
...
def _build_rename_summary(*, before_names: Set[str], after_report: List[Dict[str, Any]], tool_calls: List[Dict[str, Any]], model_final: str) -> str:
    ...
    lines.append("full report: mason curator status")
    umbrellas = sorted({e.get("into") for e in diff.consolidated if e.get("into")})
    if umbrellas:
        lines.append(f"keep an umbrella stable: mason curator pin {umbrellas[0]}")
    return "\n".join(lines)
...
_REPORT_SECTIONS = (
    ("consolidated", "Consolidated into umbrella skills",
     ...
     "The original directory was moved to `~/.mason/skills/.archive/` for safety and can be restored via "
     "`mason curator restore <name>` if the consolidation was wrong._\n", _consolidated_lines, 50, "see `run.json`"),
    ("pruned", "Pruned — archived for staleness",
     ...
     "Directories live under `~/.mason/skills/.archive/`. Restore any via `mason curator restore <name>`._\n",
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if final:
        ...
        lines += ["## LLM summary\n", p.get("llm_summary"), ""]
    lines += ["## Recovery\n", "- Restore an archived skill: `mason curator restore <name>`",
              "- All archives live under `~/.mason/skills/.archive/` and are recoverable by `mv`",
...
def run_curator_review(
    ...
    *dry_run* SKIPS the stale/archive transitions and instructs the fork to report only; REPORT.md is still written and
    ...
    # next real pass out) but still records a summary for `mason curator status`.
    ...
    def _llm_pass():
        ...
        # Per-run report, best-effort; path recorded for `mason curator status`.
        try:
