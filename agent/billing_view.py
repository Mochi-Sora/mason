...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    """Fetch + parse ``/api/billing/state``; fail-open. ``MASON_DEV_BILLING_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """``MASON_DEV_BILLING_FIXTURE`` -> :class:`BillingState` for offline UX; None when unset.
    ...
    """
    name = (os.getenv("MASON_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    if name not in overrides:
        return BillingState(logged_in=False, error=f"unknown MASON_DEV_BILLING_FIXTURE: {name}")
