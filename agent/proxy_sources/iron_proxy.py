...
into ``<mason_home>/bin``; CA, ``proxy.yaml``, ``mappings.json``, pidfile and logs live in
``<mason_home>/proxy``.  Failures warn and never block agent startup.
"""
...
_MGMT_API_KEY_ENV = "MASON_IRON_PROXY_MGMT_KEY"
...
# interchangeable env-var names for the SAME upstream credential (Mason' auth.py keys Google on both
...
_MASON_IRON_PROXY_NONCE_ENV = "MASON_IRON_PROXY_NONCE"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<mason_home>/bin`` copy first, then PATH; optionally auto-install."""
    managed = _mason_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    (bin_dir := _mason_bin_dir()).mkdir(parents=True, exist_ok=True)
    target = bin_dir / _platform_binary_name()
    if target.exists() and not force:
        return target
    asset_name = _platform_asset_name()
    with tempfile.TemporaryDirectory(prefix="mason-iron-proxy-") as tmpdir:
...
def _release_asset(name: str, dest: Path) -> None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "mason-agent"})
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="mason-proxy-ca-") as tmpdir:
        tmp_key, tmp_crt = Path(tmpdir) / "ca.key", Path(tmpdir) / "ca.crt"
        _run(["openssl", "genrsa", "-out", str(tmp_key), "4096"], timeout=60, check=True)
        _run(["openssl", "req", "-x509", "-new", "-nodes", "-key", str(tmp_key), "-sha256", "-days", "3650", "-subj", "/CN=mason iron-proxy CA",
...
def mint_proxy_token(prefix: str = "mason-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    if not force and (existing := _read_text_or_none(p)):
        return existing
    token = mint_proxy_token(prefix="mason-mgmt")
    _write_private_file(p, token.encode("utf-8"))
    return token


def _yaml():
    """PyYAML module or None (it is a Mason dep, but never a hard requirement here)."""
    try:
        import yaml
        return yaml
    except ImportError:
...
_RELOAD_HTTP_ERRORS = {
    ...
    401: "management API rejected our key (401).  The running daemon was started with a different management.token — run `mason egress restart`.",
}
...
def reload_proxy() -> bool:
    ...
    if not (pid := _read_pid()) or not _pid_alive(pid):
        raise RuntimeError("iron-proxy is not running — nothing to reload.  Run `mason egress start`.")
    if (mgmt := _read_management_listen_from_config()) is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written before reload support).  Re-run `mason egress setup` and use `mason egress restart` this one time."
        )
    if not (token := _read_text_or_none(_proxy_state_dir_ro() / "management.token")):
        raise RuntimeError("management.token is missing — re-run `mason egress setup`, then `mason egress restart`.")
    host, port = mgmt
    ...
    try:
        ...
        raise RuntimeError(
            f"could not reach the management API at {host}:{port} ({exc}).  If the daemon was started before reload support, run `mason egress restart` once."
        ) from exc
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<mason_home>/proxy/proxy.yaml`` (safe_dump, no Python tags)."""
...
def _pid_alive(pid: int) -> bool:
    ...
    if nonce_candidates:
        with suppress(OSError):
            ...
            if any(f"{_MASON_IRON_PROXY_NONCE_ENV}={n}".encode() in env_bytes for n in nonce_candidates):
                return True
    with suppress(OSError):
...
def start_proxy(
    *, binary: Optional[Path] = None, config_path: Optional[Path] = None, extra_env: Optional[Dict[str, str]] = None,
    ...
    if (bin_path := binary or find_iron_proxy(install_if_missing=install_if_missing)) is None:
        raise RuntimeError("iron-proxy binary not available — run `mason egress install`.")
    if not (cfg := config_path or (_proxy_state_dir() / "proxy.yaml")).exists():
        raise RuntimeError(f"iron-proxy config not found at {cfg}. Run `mason egress setup` first.")
    ...
    env[_MASON_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    log_path = _proxy_state_dir() / "iron-proxy.log"
    proc = _spawn_daemon(bin_path, cfg, env, log_path)
    # Pidfile BEFORE the listening poll so `mason egress stop` can clean an orphan if the parent dies mid-poll.
    pidfile = _pidfile()
    try:
...
def _write_pidfile_safely(pidfile: Path, pid: int) -> None:
    ...
    try:
        ...
        if (existing_pid := _read_pid()) and _pid_alive(existing_pid):
            raise RuntimeError(
                f"Another iron-proxy start appears to be in progress (pidfile {pidfile} -> pid {existing_pid}).  Run `mason egress stop` if that proxy is stuck."
            )
        pidfile.unlink(missing_ok=True)
        fd = os.open(str(pidfile), open_flags, 0o600)
    except OSError as exc:
...
def _refresh_secrets_from_bitwarden(env: Dict[str, str], needed: set, bitwarden_config: Dict, allow_env_fallback: bool) -> None:
    ...
    if missing:
        _bitwarden_shortfall(
            ...
            f"credential_source: env via `mason egress setup --no-bitwarden`, or set `proxy.allow_env_fallback: true` in "
    ...
    if warnings:  # log only the count: the taint analyzer can't tell bws status text is non-secret
        logger.warning("Bitwarden refresh produced %d warning(s); run `mason secrets bitwarden status` for detail.", len(warnings))
