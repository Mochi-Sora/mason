...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    ``MASON_DEV_SUBSCRIPTION_FIXTURE`` short-circuits to a fixture so every state is testable offline."""
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """``MASON_DEV_SUBSCRIPTION_FIXTURE`` (``free | mid | top | not-admin | downgrade | cancel | team |
    logged-out``) -> fixture state; None when unset; unknown name → logged-out with ``error`` set."""
    name = (os.getenv("MASON_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    if name not in states:
        return SubscriptionState(logged_in=False, error=f"unknown MASON_DEV_SUBSCRIPTION_FIXTURE: {name}")
