"""Core NeMo Relay adapter for Mason tool execution."""
...
def execute(
    ...
    try:
        ...
        if isinstance(exc, Exception) and callback_error is None and "value" in raw_result:
            logger.warning(
                "NeMo Relay tool post-processing failed after dispatch success; returning the Mason tool result",
                exc_info=True,
            )
...
def _run_awaitable(value: Any) -> Any:
    return relay_llm._run_awaitable(
        value, loop_error="Synchronous Mason Relay tool execution cannot run on an active event-loop thread",
    )
