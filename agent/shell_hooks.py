...
from mason_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, split_command_line, windows_hide_flags
...
def _home_key() -> str:
    return str(get_mason_home().expanduser().resolve())
...
def register_from_config(cfg: Optional[Dict[str, Any]], *, accept_hooks: bool = False) -> List[ShellHookSpec]:
    ...
    if env_var_enabled("MASON_SAFE_MODE"):  # hooks are user customizations too — fire zero user-configured code
        logger.info("MASON_SAFE_MODE=1 — shell-hook registration skipped")
        return []
    effective_accept = _resolve_effective_accept(cfg, accept_hooks)
    specs = _parse_hooks_block(cfg.get("hooks"))
    if not specs:
        return []
    from mason_cli.plugins import get_plugin_manager  # lazy: avoids import cycle
    ...
    for spec in specs:
        ...
        if not already_allowlisted and not _prompt_and_record(spec.event, spec.command, accept_hooks=effective_accept):
            logger.warning("shell hook for %s (%s) not allowlisted — skipped. Use --accept-hooks / "
                           "MASON_ACCEPT_HOOKS=1 / hooks_auto_accept: true, or approve at the TTY prompt next run.",
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    """Parse config hooks without registering (``mason hooks list`` / doctor)."""
...
def re_register_config_hooks() -> None:
    ...
    Only the idempotence keys for the *current* Mason home are cleared — ``discover_and_load(force=True)``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    from mason_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
# pre_tool_call dialects in check order — Mason ``action`` then Claude-Code ``decision`` — as (verb key,
# block-message primary, secondary, modify payload key); both translate to the canonical Mason shape.
...
def _parse_pre_verify(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    # "continue" (Mason) / "block" (Claude-Code Stop) both mean keep going; no message is a no-op.
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Mason wire-shape dict, or ``None``."""
...
def allowlist_path() -> Path:
    ...
    return get_mason_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning("Failed to persist shell hook allowlist to %s: %s. The approval is in-memory for this run, "
                       ...
                       "--accept-hooks / MASON_ACCEPT_HOOKS).", p, exc)
...
def _prompt_and_record(event: str, command: str, *, accept_hooks: bool) -> bool:
    ...
    if not sys.stdin.isatty():
        return False
    print(
        f"\n⚠ Mason is about to register a shell hook that will run a\n  command on your behalf.\n\n"
        f"    Event:   {event}\n    Command: {command}\n\n"
...
def _resolve_effective_accept(cfg: Dict[str, Any], accept_hooks_arg: bool) -> bool:
    """Any truthy opt-in channel wins: explicit arg, MASON_ACCEPT_HOOKS, hooks_auto_accept."""
    if accept_hooks_arg or os.environ.get("MASON_ACCEPT_HOOKS", "").strip().lower() in _TRUTHY:
        return True
    cfg_val = cfg.get("hooks_auto_accept", False)
    return cfg_val if isinstance(cfg_val, bool) else isinstance(cfg_val, str) and cfg_val.strip().lower() in _TRUTHY


# --- Introspection (used by `mason hooks` CLI) ---
...
def run_once(spec: ShellHookSpec, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    """Fire one hook with a synthetic payload (``mason hooks test`` / doctor) through the production path."""
