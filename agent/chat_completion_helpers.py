...
from mason_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from mason_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("MASON_STREAM_STALE_GIVEUP", 5)
...
def _configured_stale_base(agent) -> float:
    """Per-provider ``stale_timeout_seconds`` config, else MASON_STREAM_STALE_TIMEOUT (180s)."""
    cfg = get_provider_stale_timeout(agent.provider, agent.model)
    return cfg if cfg is not None else env_float("MASON_STREAM_STALE_TIMEOUT", 180.0)
...
def _bedrock_converse_call(api_kwargs: dict, *, stream: bool, on_stream_denied=None):
    """Pop the Mason routing keys and call ``converse`` / ``converse_stream`` (boto3
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from mason_cli.local_runtime.load_progress import get_loading_progress, get_prefill_progress
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    activity). Tunables: MASON_CODEX_TTFB_TIMEOUT_SECONDS,
    MASON_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each),
    MASON_CODEX_TTFB_DISABLE_ABOVE_TOKENS / MASON_CODEX_TTFB_STRICT,
    MASON_CODEX_TTFB_MAX_SECONDS, MASON_CODEX_HARD_TIMEOUT_SECONDS.
    """
    ...
    if codex and openai_codex_backend:
        ...
        hard_timeout = env_float("MASON_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
        if hard_timeout > 0:
    ...
    ttfb_enabled = codex
    ttfb_timeout = env_float("MASON_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if ttfb_timeout <= 0:
        ttfb_enabled = False
        ...
        disable_above = env_float("MASON_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        strict = os.environ.get("MASON_CODEX_TTFB_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
        if not strict and disable_above > 0 and est_tokens >= disable_above and ttfb_timeout < idle_default:
            logger.info("Scaling openai-codex no-byte TTFB watchdog from %.0fs to %.0fs "
                ...
                "Set MASON_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.", ttfb_timeout, idle_default,
                f"{est_tokens:,}", disable_above)
            ttfb_timeout = idle_default
        ttfb_cap = env_float("MASON_CODEX_TTFB_MAX_SECONDS", 120.0)
        if ttfb_cap > 0 and ttfb_timeout > ttfb_cap:
            logger.info("Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set MASON_CODEX_TTFB_MAX_SECONDS to tune.", ttfb_timeout, ttfb_cap,
                f"{est_tokens:,}")
            ttfb_timeout = ttfb_cap

    idle_timeout = env_float("MASON_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_default)
...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    _qwen_meta = {"sessionId": agent.session_id or "mason", "promptId": str(uuid.uuid4())} if _is_qwen else None
    _profile = None
    with contextlib.suppress(Exception):
        from providers import get_provider_profile
...
def _assistant_content_for_storage(agent, assistant_message):
    ...
    # history / state.db / gateway delivery (no-op with MASON_REDACT_SECRETS off).
...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    from mason_cli.providers import host_mandated_api_mode
...
def _iteration_summary_api_messages(agent, messages: list) -> list:
    ...
    for msg in messages:
        ...
        # every Mason-internal underscore-prefixed scaffolding key.
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        ...
        # Strict machine-readable mode (mason chat -Q, oneshot, background review): keep diagnostics out of
...
class _BedrockStream:
    ...
    def _on_stale(self, stale_elapsed: float) -> None:
        ...
        self.last_event = time.time()
        # Raises RuntimeError past MASON_STREAM_STALE_GIVEUP; otherwise end
...
class _StreamingCall:
    ...
    def _stream_timeouts(self) -> tuple[float, float, float]:
        ...
        ``request_timeout_seconds`` wins over MASON_API_TIMEOUT (1800s) and
        MASON_STREAM_READ_TIMEOUT (120s); connect/pool cover the handshake, not
        inference: 30s, or capped at 60s when configured."""
        cfg = get_provider_request_timeout(self.agent.provider, self.agent.model)
        base = cfg if cfg is not None else env_float("MASON_API_TIMEOUT", 1800.0)
        if cfg is not None:
            return base, cfg, min(base, 60.0)
        read = env_float("MASON_STREAM_READ_TIMEOUT", 120.0)
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        if self.agent.provider == "moa":
            # Mason interrupts the managed stream; Relay alone closes the provider stream.
    ...
    def _call(self):
        _max_stream_retries = env_int("MASON_STREAM_RETRIES", 2)
        try:
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        MASON_LOCAL_STREAM_STALE_TIMEOUT — an infinite one stalled sessions on a
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            ...
            self._stream_stale_timeout = env_float("MASON_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
