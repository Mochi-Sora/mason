...
from mason_cli.route_identity import normalize_route_base_url
from mason_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'mason memory status'. Note: "
        "systemd/gateway services do not inherit ~/.mason/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from mason_cli.providers import MASON_OVERLAYS, get_provider
        overlay = MASON_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from mason_cli.models import normalize_provider as normalize_model_provider
        from mason_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: mason config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$MASON_HOME`` is profile-scoped; not a config key)."""
    return get_mason_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if api_mode in _EXPLICIT_API_MODES:
        ...
        try:
            ...
            from mason_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from mason_cli.model_normalize import (
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, MASON_DEV_CREDITS) from x-nous-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from mason_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from mason_logging import setup_logging, setup_verbose_logging
    setup_logging(mason_home=_ra()._mason_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in mason_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from mason_cli.auth import build_minimax_oauth_token_provider
...
def _init_bedrock_client(agent, base_url):
    ...
    with suppress(Exception):
        from mason_cli.config import load_config_readonly as _load_br_cfg
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        raise RuntimeError(
            ...
            f"variable, or switch to a different provider with `mason model`."
        )
    raise RuntimeError(
        "No LLM provider configured. Run `mason model` to "
        "select a provider, or run `mason setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("mason_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "mason_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched MASON_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff MASON_KANBAN_TASK); resolve once.
...
def _ensure_custom_session_container(session_id: str):
    try:
        from sessions.container import init_session as _init_sess
        import pathlib as _pl
        if session_id and not (_pl.Path('sessions') / session_id).exists():
            _init_sess(session_id)
    except Exception:
        pass
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    _ensure_custom_session_container(agent.session_id)

    # ~/.mason/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_mason_home() / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot is opt-in (sessions.write_json_snapshots); state.db is canonical.
    agent._session_json_enabled = False
    with suppress(Exception):
        from mason_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # Process-scoped --yolo is persisted so `mason --resume` restores the bypass
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "mason", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, mason, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from mason_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from mason_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Mason Agent.  Choose a model with at least "
...
def _warn_nonagentic_mason_model(agent):
    # Nous Mason 3/4 are chat models, not tool-call-tuned. cli.py show_banner() already
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from mason_cli.model_switch import _check_mason_model_warning
        _mason_warn = _check_mason_model_warning(agent.model or "")
        if _mason_warn:
            _user_msg = (
                "⚠ Nous Research Mason 3 & 4 models are NOT agentic — they "
            ...
            _ra().logger.warning(_mason_warn)
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, mason_home=str(get_mason_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.mason.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.mason/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from mason_cli.config import load_config_readonly as _load_agent_config
    ...
    _warn_nonagentic_mason_model(agent)
