"""Anthropic Messages API adapter: client construction + the Messages call for Mason's
...
# Mason effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Mason tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://mason-agent.nousresearch.com", "X-Title": "Mason Agent",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    mason's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ``api_key`` unset, so the SDK fills it from ANTHROPIC_API_KEY (loaded from ~/.mason/.env) and
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to mason's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Mason tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Mason Agent", "Claude Code"), ("Mason agent", "Claude Code"),
    ("mason-agent", "claude-code"), ("Nous Research", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Mason shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that Mason surfaces in its CLI. We request "summarized" so the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'read_mason_oauth_credentials': ('agent.anthropic_credentials', 'read_mason_oauth_credentials'),
    ...
    'run_mason_oauth_login_pure': ('agent.anthropic_credentials', 'run_mason_oauth_login_pure'),
