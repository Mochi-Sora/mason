"""Core NeMo Relay adapters for physical Mason provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Mason knows the API mode."""
...
def _relay_metadata(provider_name: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    relay_metadata.setdefault("mason.provider", provider_name)
...
class _ManagedAttempt:
    ...
    def run_callback(self, callback: Callable[..., Any], *args: Any) -> Any:
        """Run a Mason callback in a fresh copy of the captured context.
        ...
        def guarded() -> Any:
            # See #77244.
            # See #77244.
            # Mason-side callbacks run while the native pipeline drives this stream; nested relay calls
...
def _current_session_id() -> str | None:
    """Return the inherited Mason turn's session id, or None outside a live turn."""
...
def execute(
    ...
    ``session_id`` defaults to the inherited Mason turn's session (unmanaged when there is none)."""
...
# Run under the inherited Mason turn when present (callers that do not know a session id).
...
def stream_current(
    ...
    """Run a provider stream under the inherited Mason turn when present.
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Mason."""
        ...
        with turn.logical_llm_lock:
            ...
            if handle is None:
                ...
                handle = turn.logical_llm_calls[request_id] = runtime.run_in_session(
                    ...
                    metadata=relay_runtime.runtime_metadata(runtime.runtime_id, **{"mason.call_role": call_role}),
                )
...
def _complete_logical(
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning("Mason Relay logical LLM finalization failed", exc_info=True)
            return
        with turn.logical_llm_lock:
