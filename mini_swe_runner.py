#!/usr/bin/env python3
"""SWE Runner with Mason Trajectory Format

Runs tool-calling agent tasks in Mason-Agent's execution environments (local,
docker, modal) and writes trajectories in Mason format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Mason execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Mason execution environment, emitting Mason trajectories."""
    ...
    def _convert_to_mason_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Mason trajectory format used by batch_runner.py."""
        system_msg = MASON_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + MASON_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_mason_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with Mason trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Mason Trajectory Format")
    print("=" * 60)
