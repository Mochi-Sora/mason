"""Shared constants for Mason Agent.
...
_UNSET = object()
_MASON_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_MASON_HOME_OVERRIDE", default=_UNSET)
...
def set_mason_home_override(path: str | Path | None) -> Token:
    """Set a context-local Mason home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _MASON_HOME_OVERRIDE.set(value)


def reset_mason_home_override(token: Token) -> None:
    """Restore the previous context-local Mason home override."""
    _MASON_HOME_OVERRIDE.reset(token)


def get_mason_home_override() -> str | None:
    """Return the active context-local Mason home override, if any."""
    override = _MASON_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_mason_home() -> Path:
    """Return the platform-native default Mason home path."""
...
def _warn_profile_fallback_once() -> None:
    """Warn once when MASON_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_mason_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[MASON_HOME fallback] MASON_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass MASON_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_mason_home() -> Path:
    """Mason home: context-local override → ``MASON_HOME`` env var → platform default."""
    override = get_mason_home_override()
    if override:
        return Path(override)
    if not os.environ.get("MASON_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_mason_home()
...
def mason_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Mason home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_mason_home()
    raw = str(candidate)
...
def reset_mason_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_mason_home() -> Path:
    """Mason home of the running process, ignoring task overrides.
    ...
    return Path(val) if val else _get_platform_default_mason_home()


# get_default_mason_root() memo keyed on (native home, MASON_HOME) so it stays
# fresh when a test or plugin mutates MASON_HOME; saves ~80us/call at 31+ sites.
_default_mason_root_memo: "tuple[str, str, Path] | None" = None


def get_default_mason_root() -> Path:
    """Root Mason dir for profile-level ops: ``<root>`` when ``MASON_HOME=<root>/profiles/<name>``."""
    global _default_mason_root_memo
    native_home = _get_platform_default_mason_home()
    env_home = os.environ.get("MASON_HOME", "")
    memo = _default_mason_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.mason (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else MASON_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_mason_root_memo = (str(native_home), env_home, result)
    return result
...
# Files marking a real Mason home; arbitrary dirs with a ``profiles`` segment lack them.
_MASON_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_mason_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<mason-home>/profiles``.

    Accepts the classic ``~/.mason`` layout, a root carrying Mason-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _MASON_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_mason_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Mason home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_mason_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".mason":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def mkdir_under_mason_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_mason_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("MASON_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``MASON_OPTIONAL_MCPS``)."""
    return _packaged_dir("MASON_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``MASON_BUNDLED_SKILLS``)."""
    return _packaged_dir("MASON_BUNDLED_SKILLS", default, "skills")


def get_mason_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Mason subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_mason_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_mason_node_dirs(home: Path | None = None) -> list[Path]:
    """Mason-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with masonManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_mason_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_mason_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Mason-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_mason_node_path())
    return result is not None and result.returncode == 0


_MASON_NODE_TARGET_MAJOR = int(os.environ.get("MASON_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/mason_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks mason-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def mason_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Mason-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Mason-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `mason update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_MASON_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_MASON_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%MASON_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the mason.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MASON_HOME": str(get_mason_home()), **extra_env},
...
def bootstrap_mason_managed_node() -> str | None:
    """Install a Mason-managed Node tree under ``$MASON_HOME/node`` and return its npm path.

    Mason never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_mason_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # MASON_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, MASON_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_mason_managed_node() -> bool:
    """Redownload Mason-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not mason_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _MASON_NODE_TARGET_MAJOR
    return False


def find_mason_node_executable(command: str) -> str | None:
    """Mason-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_mason_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_mason_node_executable(command)
    if managed:
        return managed
    if mason_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_mason_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Mason-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_mason_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``mason update``
...
def display_mason_home() -> str:
    """User-facing ``~/`` display string for MASON_HOME (``~/.mason/profiles/coder``)."""
    home = get_mason_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\mason/skills/
...
def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved MASON_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks mason-user traversal in Docker (UID 10000).
    # A credential file here means MASON_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "mason-agent install directory (%s). Credential files are "
            "normally stored under the mason home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{MASON_HOME}/home`` when the profile-home directory exists."""
    mason_home = get_mason_home_override() or (env or {}).get("MASON_HOME") or os.getenv("MASON_HOME")
    if not mason_home:
        return None
    profile_home = os.path.join(mason_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "MASON_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Mason profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={MASON_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{MASON_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Mason' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["MASON_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Mason runs in WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/mason-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under MASON_HOME."""
    return get_mason_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under MASON_HOME."""
    return get_mason_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under MASON_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_mason_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._mason_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``mason_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``mason_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Mason (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("mason_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    mason update",
        "If that also fails, reinstall from your Mason repo (MASON_REPO_URL).",
    ]
